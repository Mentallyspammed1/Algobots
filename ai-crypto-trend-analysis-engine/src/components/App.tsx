...
const ANALYSIS_COOLDOWN_MS = 10000; // 10-second cooldown to prevent API rate limiting
...
const App: React.FC = () => {
  ...
  const handleRunAnalysis = useCallback(async (symbol: string, interval: string, minConfidence: number) => {
    if (isLoading) return; // Prevent concurrent runs

    const now = Date.now();
    if (now - lastRunTimestamp < ANALYSIS_COOLDOWN_MS) {
        const timeLeft = Math.ceil((ANALYSIS_COOLDOWN_MS - (now - lastRunTimestamp)) / 1000);
        setError(`Rate limit: Please wait ${timeLeft} seconds before the next analysis.`);
        return;
    }
    ...
    try {
      ...
      if (!primaryKlines || primaryKlines.length < 52) { // Ichimoku requires at least 52 periods
  ...
  const handleReanalyze = useCallback((symbol: string, interval: string) => {
    // Uses the current confidence from the control panel for the re-analysis
    handleRunAnalysis(symbol, interval, minConfidence);
  }, [handleRunAnalysis, minConfidence]);
  ...
  }, [isAutoRefreshEnabled, lastAnalysisParams, isLoading, handleRunAnalysis, symbol, interval, minConfidence]);
