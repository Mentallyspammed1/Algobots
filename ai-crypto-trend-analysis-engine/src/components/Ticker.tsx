...
const Ticker: React.FC<TickerProps> = ({ symbols }) => {
  ...
  const receivedSymbolsCount = useRef(0);
  ...
  const handleMessage = useCallback((data: any) => {
    if (data.topic && data.data) {
      ...
      setTickerData(prevData => {
          ...
          // Check if this is the first data point for this symbol
          if (!prevData[symbol]) {
              receivedSymbolsCount.current += 1;
          }
  ...
  // Effect to check if animation should start
  useEffect(() => {
    if (!isAnimationReady) {
      // Start animation when we have data for a good portion of symbols.
      // This threshold ensures the initial width is stable.
      const threshold = Math.floor(symbols.length * 0.9);
      if (receivedSymbolsCount.current >= threshold) {
        // Use a short timeout to allow the DOM to update with the new prices 
        // before applying the animation class, preventing a race condition.
        setTimeout(() => setIsAnimationReady(true), 100);
      }
    }
  }, [tickerData, isAnimationReady, symbols.length]);
    ...
    <div key={symbol} className="flex items-center mx-4 flex-shrink-0" role="listitem">
      ...
      <div className={`ml-2 flex items-center ${getPriceColor(tickerData[symbol]?.direction ?? 'neutral')}`}>
        ...
        <span 
          ...
          aria-live="polite"
        >
          {formatPrice(tickerData[symbol]?.price)}
        </span>
      </div>
    </div>
  ));

  const animationClass = isAnimationReady ? 'animate-scroll' : '';
    ...
    <div className="fixed top-0 left-0 right-0 h-10 bg-gray-900/80 backdrop-blur-sm border-b border-gray-700 z-50 overflow-hidden" aria-label="Live price ticker">
      <div className="flex items-center h-full" role="list">
        <div className={`flex items-center whitespace-nowrap ${animationClass}`}>
          {tickerContent}
          {/* Render the second copy only when animation is ready to ensure correct width calculation */}
          {isAnimationReady && tickerContent}
        </div>
      </div>
    </div>
  );
