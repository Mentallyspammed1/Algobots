...
import { AnalysisResult, TrendDirection } from '../types';
...
const LiveOrderbookData: React.FC<{ symbol: string }> = ({ symbol }) => {
    ...
    const spread = orderbookData.bid && orderbookData.ask ? (parseFloat(orderbookData.ask) - parseFloat(orderbookData.bid)).toFixed(2) : null;
        ...
        <>
            <div className="flex justify-between pt-2 border-t border-gray-700/50">
                <div className="flex items-center">
                 ...
                 <span className={`font-mono transition-colors duration-300 ${getPriceColor(orderbookData.bidDirection)}`}>${orderbookData.bid ? parseFloat(orderbookData.bid).toLocaleString() : '...'}</span>
            </div>
            <div className="flex justify-between">
                <span>Best Ask:</span> 
                <span className={`font-mono transition-colors duration-300 ${getPriceColor(orderbookData.askDirection)}`}>${orderbookData.ask ? parseFloat(orderbookData.ask).toLocaleString() : '...'}</span>
            </div>
            {spread && <div className="flex justify-between"><span>Spread:</span> <span className="font-mono text-white">${spread}</span></div>}
        </>
    );
};

const TimeframeAlignment: React.FC<{ alignment?: { [key: string]: TrendDirection } }> = ({ alignment }) => {
    ...
    return (
        <div className="pt-2 border-t border-gray-700/50">
            ...
            <div className="flex gap-2">
                {Object.entries(alignment).map(([interval, trend]) => (
                    <div key={interval} className="flex-1 text-center bg-gray-900 rounded p-1">
                        <div className="text-xs text-gray-500">{interval === 'D' ? '1D' : (parseInt(interval) >= 60 ? `${parseInt(interval)/60}H` : `${interval}M`)}</div>
                        <div className={`text-xs font-bold ${getTrendColor(trend as TrendDirection)}`}>{trend}</div>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
          <div className="lg:col-span-1 md:col-span-2 bg-gray-900/50 p-4 rounded-lg">
            ...
            <div className="space-y-2">
                <div className="flex justify-between"><span>Last Price:</span> <span className="font-mono text-white">${current_price.toLocaleString()}</span></div>
                <div className="flex justify-between"><span>Entry Price:</span> <span className="font-mono text-white">${analysis.entryPrice.toLocaleString()}</span></div>
                <div className="flex justify-between"><span>Take Profit:</span> <span className="font-mono text-green-400">${analysis.takeProfitLevel.toLocaleString()}</span></div>
                <div className="flex justify-between"><span>Stop Loss:</span> <span className="font-mono text-red-400">${analysis.stopLossLevel.toLocaleString()}</span></div>
                
                <LiveOrderbookData symbol={symbol} />
                
                {vwap !== undefined && <div className="flex justify-between pt-2 border-t border-gray-700/50"><span>VWAP (Period):</span> <span className="font-mono text-white">${vwap.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 })}</span></div>}
                ...
                <div className="flex justify-between"><span>Support:</span> <span className="font-mono text-green-400">${analysis.supportLevel.toLocaleString()}</span></div>
                <div className="flex justify-between"><span>Resistance:</span> <span className="font-mono text-red-400">${analysis.resistanceLevel.toLocaleString()}</span></div>
            </div>
          </div>
