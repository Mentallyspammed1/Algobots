...
def supertrend(high, low, close, period=7, atr_multiplier=3):
    ...
    for i in range(len(high)):
        if i == 0:
            ...
            supertrend_series.iloc[i - 1] == final_upper_band.iloc[i - 1]
            ...
            supertrend_series.iloc[i - 1] == final_lower_band.iloc[i - 1]
...
def calculate_indicators(df):
    ...
    ROC_PERIOD = 12
    df["roc"] = (
        (df["close"] - df["close"].shift(ROC_PERIOD)) / df["close"].shift(ROC_PERIOD)
    ) * 100
    ...
    df["stochrsi_d"] = ta.momentum.stochrsi_d(
        ...
        smooth1=STOCHRSI_SMA_PERIOD,
        smooth2=STOCHRSI_SMA_PERIOD,
        fillna=False,
    )
...
def run_bot():
    ...
    while True:
        ...
        required_1m_data = (
            max(EMA_LONG_PERIOD, VOLUME_MA_PERIOD, RSI_PERIOD, ATR_PERIOD, 20) + 2
        )  # 20 for CMF, KC
