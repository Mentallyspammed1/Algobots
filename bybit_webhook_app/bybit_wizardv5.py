...
class BybitV5Wizard:
    ...
    def __init__(self, config_path: str = os.path.expanduser("~/.bybit_config")):
        ...
        self.ws_public_url = "wss://stream-testnet.bybit.com/v5/public/linear" if self.testnet else "wss://stream.bybit.com/v5/public/linear"
        ...
        print(Fore.CYAN + "# Bybit V5 Wizard awakened with enhanced ethereal power..." + Style.RESET_ALL)
    ...
    def _send_request(self, method: str, endpoint: str, params: dict = None, retries: int = None) -> dict:
        ...
        with self._timing_context(f"api_request_{endpoint}"):
            return self._execute_request_with_circuit_breaker(method, endpoint, params, retries)

    def _execute_request_with_circuit_breaker(self, method: str, endpoint: str, params: dict, retries: int) -> dict:
        """Execute request with circuit breaker protection."""
        try:
            return self.circuit_breaker.call(self._execute_request, method, endpoint, params, retries)
        except Exception as e:
            logger.error(f"Circuit breaker prevented request to {endpoint}: {e!s}")
            self.metrics.record_error("circuit_breaker_open")
            return {}

    def _execute_request(self, method: str, endpoint: str, params: dict, retries: int) -> dict:
        ...
        for attempt in range(retries):
            try:
                ...
                if result.get("retCode") == 0:
                    ...
                    print(Fore.GREEN + f"# Spell cast successfully: {endpoint}" + Style.RESET_ALL)
                ...
                last_exception = e
                logger.warning(f"Request timeout for {endpoint} (attempt {attempt + 1}/{retries})")
                self.metrics.record_error("request_timeout")

            except requests.exceptions.ConnectionError as e:
                last_exception = e
                logger.warning(f"Connection error for {endpoint} (attempt {attempt + 1}/{retries})")
                self.metrics.record_error("connection_error")
                self.connection_health["api"] = False

            except requests.exceptions.HTTPError as e:
                last_exception = e
                logger.warning(f"HTTP error {e.response.status_code} for {endpoint} (attempt {attempt + 1}/{retries})")
        ...
        logger.error(f"All retry attempts exhausted for {endpoint}. Last error: {last_exception!s}")
        self.metrics.record_error("all_retries_exhausted")
        return {}

    # --- Market Data Endpoints ---

    def get_kline(self, category: str, symbol: str, interval: str, start: int | None = None,
                  end: int | None = None, limit: int = 200) -> dict:
        ...
        print(Fore.CYAN + f"# Conjuring kline data for {symbol} ({interval})..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_mark_price_kline(self, category: str, symbol: str, interval: str, start: int | None = None,
                             end: int | None = None, limit: int = 200) -> dict:
        ...
        if end:
            params["end"] = end
        print(Fore.CYAN + f"# Fetching mark price kline for {symbol} ({interval})..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_index_price_kline(self, category: str, symbol: str, interval: str, start: int | None = None,
                              end: int | None = None, limit: int = 200) -> dict:
        ...
        if end:
            params["end"] = end
        print(Fore.CYAN + f"# Fetching index price kline for {symbol} ({interval})..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_premium_index_price_kline(self, category: str, symbol: str, interval: str, start: int | None = None,
                                      end: int | None = None, limit: int = 200) -> dict:
        ...
        if end:
            params["end"] = end
        print(Fore.CYAN + f"# Fetching premium index price kline for {symbol} ({interval})..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_instruments_info(self, category: str, symbol: str | None = None, status: str | None = None,
        ...
        if limit:
            params["limit"] = limit
        print(Fore.CYAN + f"# Fetching instruments info for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def get_tickers(self, category: str, symbol: str | None = None, base_coin: str | None = None,
                    exp_date: str | None = None) -> dict:
    ...
    def get_public_trade(self, category: str, symbol: str, base_coin: str | None = None,
                         option_type: str | None = None, limit: int = 500) -> dict:
    ...
    def get_open_interest(self, category: str, symbol: str, interval: str = "5min",
                          start: int | None = None, end: int | None = None, limit: int = 200) -> dict:
    ...
    def get_historical_volatility(self, category: str = "option", base_coin: str | None = None,
        ...
        print(Fore.CYAN + f"# Fetching historical volatility for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def get_delivery_price(self, category: str, symbol: str | None = None, base_coin: str | None = None,
                           limit: int = 200) -> dict:
        ...
        if base_coin:
            params["baseCoin"] = base_coin
        print(Fore.CYAN + f"# Fetching delivery price for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def get_long_short_ratio(self, category: str, symbol: str, period: str, limit: int = 50) -> dict:
        ...
        print(Fore.CYAN + f"# Fetching long-short ratio for {symbol} ({period})..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_top_long_short_account_ratio(self, category: str, symbol: str, period: str, limit: int = 50) -> dict:
        ...
        params = {"category": category, "symbol": symbol, "period": period, "limit": limit}
        print(Fore.CYAN + f"# Fetching top long-short account ratio for {symbol} ({period})..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_top_long_short_position_ratio(self, category: str, symbol: str, period: str, limit: int = 50) -> dict:
        ...
        params = {"category": category, "symbol": symbol, "period": period, "limit": limit}
        print(Fore.CYAN + f"# Fetching top long-short position ratio for {symbol} ({period})..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_taker_buy_sell_ratio(self, category: str, symbol: str, period: str, limit: int = 50) -> dict:
        ...
        params = {"category": category, "symbol": symbol, "period": period, "limit": limit}
        print(Fore.CYAN + f"# Fetching taker buy-sell ratio for {symbol} ({period})..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    # --- Trade Endpoints ---

    def place_market_order(self, symbol: str, side: str, qty: float, category: str = "linear",
                           break_even: bool = False, tsl: bool = False, tsl_distance: float | None = None,
        ...
        print(Fore.CYAN + f"# Casting market order for {symbol} ({side})..." + Style.RESET_ALL)
    ...
    def place_limit_order(self, symbol: str, side: str, qty: float, price: float, category: str = "linear",
                          break_even: bool = False, tsl: bool = False, tsl_distance: float | None = None,
                          profit_target: float | None = None) -> dict:
        ...
        print(Fore.CYAN + f"# Forging limit order for {symbol} at {price}..." + Style.RESET_ALL)
        result = self._send_request("POST", endpoint, params)
        if result and (break_even or tsl or profit_target):
    ...
    def place_conditional_order(self, symbol: str, side: str, qty: float, trigger_price: float,
                                order_type: str = "Limit", price: float | None = None, category: str = "linear") -> dict:
        ...
        print(Fore.CYAN + f"# Summoning conditional order for {symbol} at trigger {trigger_price}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def place_tpsl_order(self, symbol: str, side: str, qty: float, take_profit: float | None = None,
                         stop_loss: float | None = None, category: str = "linear") -> dict:
        ...
        print(Fore.CYAN + f"# Enchanting {symbol} order with TP/SL..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
            ...
            "request": [
                {
                    ...
                    **({"takeProfit": str(order["takeProfit"])} if order.get("takeProfit") else {}),
                    **({"stopLoss": str(order["stopLoss"])} if order.get("stopLoss") else {})
                } for order in orders
            ]
        }
        print(Fore.CYAN + f"# Casting {len(orders)} batch orders for {category}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def amend_order(self, symbol: str, category: str = "linear", order_id: str | None = None,
                    order_link_id: str | None = None, qty: float | None = None,
                    price: float | None = None, trigger_price: float | None = None) -> dict:
            ...
            "request": [
                {
                    ...
                    **({"orderLinkId": order["orderLinkId"]} if order.get("orderLinkId") else {}),
                    **({"qty": str(order["qty"])} if order.get("qty") else {}),
                    **({"price": str(order["price"])} if order.get("price") else {}),
                    **({"triggerPrice": str(order["triggerPrice"])} if order.get("triggerPrice") else {})
                } for order in orders
            ]
        }
        print(Fore.CYAN + f"# Amending {len(orders)} batch orders for {category}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def cancel_order(self, symbol: str, category: str = "linear", order_id: str | None = None,
                     order_link_id: str | None = None) -> dict:
    ...
    def close_position(self, symbol: str, category: str = "linear", order_type: str = "Market",
        ...
        if price and order_type == "Limit":
            params["price"] = str(price)
        print(Fore.CYAN + f"# Closing position for {symbol} ({side})..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def get_order_history(self, category: str = "linear", symbol: str | None = None, limit: int = 50) -> dict:
        ...
        if symbol:
            params["symbol"] = symbol
        print(Fore.CYAN + f"# Seeking order history for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_transaction_log(self, category: str = "linear", symbol: str | None = None) -> dict:
        ...
        if symbol:
            params["symbol"] = symbol
        print(Fore.CYAN + f"# Retrieving transaction logs for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def set_leverage(self, symbol: str, buy_leverage: float, sell_leverage: float, category: str = "linear") -> dict:
        ...
        print(Fore.CYAN + f"# Bending leverage for {symbol} to {buy_leverage}x..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def set_trading_stop(self, symbol: str, category: str = "linear", take_profit: float | None = None,
                         stop_loss: float | None = None, trailing_stop: float | None = None,
                         active_price: float | None = None, tpsl_mode: str = "Full") -> dict:
        ...
        print(Fore.CYAN + f"# Enchanting {symbol} with TP/SL/TSL wards..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def switch_position_mode(self, category: str, symbol: str, mode: int) -> dict:
        ...
        print(Fore.CYAN + f"# Switching position mode for {symbol} to {mode}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def set_tpsl_mode(self, category: str, symbol: str, tpsl_mode: str) -> dict:
        ...
        print(Fore.CYAN + f"# Setting TP/SL mode for {symbol} to {tpsl_mode}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def set_auto_add_margin(self, category: str, symbol: str, is_auto: bool) -> dict:
        ...
        params = {"category": category, "symbol": symbol, "autoAddMargin": 1 if is_auto else 0}
        print(Fore.CYAN + f"# {'Enabling' if is_auto else 'Disabling'} auto add margin for {symbol}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def get_closed_pnl(self, category: str, symbol: str | None = None, limit: int = 50) -> dict:
        ...
        if symbol:
            params["symbol"] = symbol
        print(Fore.CYAN + f"# Retrieving closed PNL for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def add_margin(self, category: str, symbol: str, margin: float, position_idx: int | None = None) -> dict:
    ...
    def get_wallet_balance(self, account_type: str = "UNIFIED", coin: str | None = None) -> dict:
        ...
        print(Fore.CYAN + f"# Unveiling wallet balance for {account_type} account..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def get_coin_greeks(self, category: str = "option", base_coin: str | None = None, symbol: str | None = None) -> dict:
    ...
    def get_account_transaction_log(self, account_type: str, category: str | None = None, coin: str | None = None, limit: int = 50) -> dict:
        ...
        if coin:
            params["coin"] = coin
        print(Fore.CYAN + f"# Retrieving account transaction logs for {account_type}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_borrow_history(self, category: str, coin: str | None = None, limit: int = 50) -> dict:
        ...
        if coin:
            params["coin"] = coin
        print(Fore.CYAN + f"# Retrieving borrow history for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def set_margin_mode(self, category: str, symbol: str, margin_mode: str) -> dict:
        ...
        print(Fore.CYAN + f"# Setting margin mode for {symbol} to {margin_mode}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def set_mmp(self, category: str, symbol: str, window_ms: int, frozen_period_ms: int, qty_limit: float, delta_limit: float) -> dict:
        ...
        params = {
            "category": category, "symbol": symbol, "windowMs": window_ms,
            "frozenPeriodMs": frozen_period_ms, "qtyLimit": str(qty_limit), "deltaLimit": str(delta_limit)
        }
    ...
    def get_margin_mode_info(self, category: str, symbol: str | None = None) -> dict:
        ...
        if symbol:
            params["symbol"] = symbol
        print(Fore.CYAN + f"# Retrieving margin mode info for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def get_asset_transfer_records(self, transfer_id: str | None = None, limit: int = 50) -> dict:
    ...
    def transfer_asset(self, transfer_id: str, coin: str, amount: float, from_account_type: str, to_account_type: str) -> dict:
        ...
        print(Fore.CYAN + f"# Transferring {amount} {coin} from {from_account_type} to {to_account_type}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def withdrawal(self, coin: str, chain: str, address: str, amount: float, tag: str | None = None) -> dict:
        """Withdraw an asset.
        ...
        if tag:
            params["tag"] = tag
        print(Fore.CYAN + f"# Initiating withdrawal of {amount} {coin} to {address}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def get_internal_deposit_records(self, coin: str | None = None, limit: int = 50) -> dict:
    ...
    def get_sub_member_deposit_records(self, coin: str | None = None, limit: int = 50) -> dict:
        ...
        if coin:
            params["coin"] = coin
        print(Fore.CYAN + "# Retrieving sub-member deposit records..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def get_deposit_address(self, coin: str, chain: str | None = None) -> dict:
        ...
        if chain:
            params["chain"] = chain
        print(Fore.CYAN + f"# Retrieving deposit address for {coin}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_universal_transfer_list(self, transfer_id: str | None = None, limit: int = 50) -> dict:
        ...
        if transfer_id:
            params["transferId"] = transfer_id
        print(Fore.CYAN + "# Retrieving universal transfer records..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_account_coins_balance(self, account_type: str, coin: str | None = None) -> dict:
        ...
        if coin:
            params["coin"] = coin
        print(Fore.CYAN + f"# Retrieving account coins balance for {account_type}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def universal_transfer(self, transfer_id: str, coin: str, amount: float, from_member_id: str, to_member_id: str, from_account_type: str, to_account_type: str) -> dict:
        ...
        params = {
            "transferId": transfer_id, "coin": coin, "amount": str(amount),
            "fromMemberId": from_member_id, "toMemberId": to_member_id,
            "fromAccountType": from_account_type, "toAccountType": to_account_type
        }
        print(Fore.CYAN + f"# Performing universal transfer of {amount} {coin}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def get_asset_info(self, account_type: str, coin: str | None = None) -> dict:
        ...
        if coin:
            params["coin"] = coin
        print(Fore.CYAN + f"# Retrieving asset info for {account_type}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def get_withdrawable_amount(self, coin: str) -> dict:
        ...
        params = {"coin": coin}
        print(Fore.CYAN + f"# Retrieving withdrawable amount for {coin}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def toggle_loan_union_interest_repay(self, status: int) -> dict:
        ...
        params = {"status": status}
        print(Fore.CYAN + f"# Toggling loan union interest repay to {status}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def get_loan_union_interest_repay_status(self) -> dict:
        ...
        print(Fore.CYAN + "# Retrieving loan union interest repay status..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, {})
    ...
    def create_api_key(self, api_key_name: str, read_only: int, ips: str | None = None, permissions: list[str] | None = None) -> dict:
    ...
    def frozen_sub_member(self, member_id: str, frozen: int) -> dict:
        ...
        print(Fore.CYAN + f"# {'Freezing' if frozen else 'Unfreezing'} sub-member {member_id}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def update_api_key(self, api_key: str, api_key_name: str | None = None, read_only: int | None = None, ips: str | None = None, permissions: list[str] | None = None) -> dict:
    ...
    def set_trading_permission(self, member_id: str, unified_margin_trade: int | None = None, inverse_futures_trade: int | None = None) -> dict:
        ...
        print(Fore.CYAN + f"# Setting trading permission for sub-member {member_id}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def get_spot_leveraged_token_order_record(self, lt_coin: str | None = None, order_id: str | None = None, limit: int = 50) -> dict:
        ...
        if order_id:
            params["orderId"] = order_id
        print(Fore.CYAN + "# Retrieving Spot Leveraged Token order records..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def purchase_spot_leveraged_token(self, lt_coin: str, amount: float, serial_no: str | None = None) -> dict:
    ...
    def redeem_spot_leveraged_token(self, lt_coin: str, amount: float, serial_no: str | None = None) -> dict:
    ...
    def set_spot_margin_leverage(self, symbol: str, leverage: float) -> dict:
        ...
        print(Fore.CYAN + f"# Setting Spot Margin leverage for {symbol} to {leverage}x..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def toggle_spot_margin_trade_mode(self, spot_margin_mode: str) -> dict:
        ...
        params = {"spotMarginMode": spot_margin_mode}
        print(Fore.CYAN + f"# Toggling Spot Margin Trade mode to {spot_margin_mode}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def borrow_spot_margin(self, coin: str, amount: float) -> dict:
        ...
        print(Fore.CYAN + f"# Borrowing {amount} {coin} for Spot Margin Trade..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def repay_spot_margin(self, coin: str, amount: float) -> dict:
        ...
        params = {"coin": coin, "amount": str(amount)}
        print(Fore.CYAN + f"# Repaying {amount} {coin} for Spot Margin Trade..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def get_spot_margin_repay_history(self, coin: str | None = None, limit: int = 50) -> dict:
    ...
    def get_spot_margin_borrow_history(self, coin: str | None = None, limit: int = 50) -> dict:
        ...
        if coin:
            params["coin"] = coin
        print(Fore.CYAN + "# Retrieving Spot Margin Borrow history..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_spot_margin_interest_history(self, coin: str | None = None, limit: int = 50) -> dict:
        ...
        if coin:
            params["coin"] = coin
        print(Fore.CYAN + "# Retrieving Spot Margin Interest history..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    # --- Copy Trading Endpoints ---

    def get_copy_trading_order_list(self, category: str, symbol: str | None = None, limit: int = 50) -> dict:
        ...
        if symbol:
            params["symbol"] = symbol
        print(Fore.CYAN + f"# Retrieving copy trading order list for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_copy_trading_order_history(self, category: str, symbol: str | None = None, limit: int = 50) -> dict:
        ...
        if symbol:
            params["symbol"] = symbol
        print(Fore.CYAN + f"# Retrieving copy trading order history for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_copy_trading_position_list(self, category: str, symbol: str | None = None) -> dict:
        ...
        if symbol:
            params["symbol"] = symbol
        print(Fore.CYAN + f"# Retrieving copy trading position list for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_copy_trading_closed_pnl(self, category: str, symbol: str | None = None, limit: int = 50) -> dict:
        ...
        if symbol:
            params["symbol"] = symbol
        print(Fore.CYAN + f"# Retrieving copy trading closed PNL for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def close_copy_trading_position(self, category: str, symbol: str, order_type: str = "Market", price: float | None = None) -> dict:
        ...
        if price:
            params["price"] = str(price)
        print(Fore.CYAN + f"# Closing copy trading position for {symbol}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def cancel_copy_trading_order(self, category: str, symbol: str, order_id: str | None = None, order_link_id: str | None = None) -> dict:
        ...
        if order_link_id:
            params["orderLinkId"] = order_link_id
        print(Fore.CYAN + f"# Cancelling copy trading order for {symbol}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    # --- Risk Management and Profit Checks ---

    def calculate_position_size(self, symbol: str, risk_percentage: float, stop_loss_distance: float,
                                account_type: str = "UNIFIED", leverage: float = 1.0) -> float:
        ...
        if risk_percentage <= 0 or risk_percentage > 100:
            ...
            print(Fore.RED + "# Risk percentage must be between 0 and 100!" + Style.RESET_ALL)
            return 0.0

        if stop_loss_distance <= 0:
            logger.error("Stop-loss distance must be positive")
            print(Fore.RED + "# Stop-loss distance cannot be zero or negative for position sizing!" + Style.RESET_ALL)
            return 0.0
        ...
        try:
            ...
            logger.info(f"Position size calculated for {symbol}: {position_size} units (Risk: {risk_percentage}%, SL Distance: {stop_loss_distance})")
            print(Fore.CYAN + f"# Calculated position size for {symbol}: {position_size} units" + Style.RESET_ALL)

            return position_size

        except (KeyError, ValueError, TypeError) as e:
            logger.error(f"Error calculating position size: {e!s}")
            print(Fore.RED + f"# Error calculating position size: {e!s}" + Style.RESET_ALL)
            return 0.0

    def monitor_profit(self, symbol: str, profit_target: float, action: str = "partial_close", close_percentage: float = 50):
        ...
        if symbol in self.positions:
            ...
            print(Fore.CYAN + f"# Monitoring {symbol} for {profit_target}% profit with action {action}..." + Style.RESET_ALL)
        else:
            print(Fore.RED + f"# No position found for {symbol} to monitor profit!" + Style.RESET_ALL)
    ...
    def enable_break_even(self, symbol: str, enable: bool = True):
        ...
        if symbol in self.positions:
            ...
            print(Fore.CYAN + f"# Break-even {'enabled' if enable else 'disabled'} for {symbol}..." + Style.RESET_ALL)
        else:
            print(Fore.RED + f"# No position found for {symbol}!" + Style.RESET_ALL)

    def enable_trailing_stop(self, symbol: str, enable: bool = True, distance: float | None = None):
        ...
        if symbol in self.positions:
            ...
            print(Fore.CYAN + f"# Trailing stop {'enabled' if enable else 'disabled'} for {symbol}..." + Style.RESET_ALL)
        else:
            print(Fore.RED + f"# No position found for {symbol}!" + Style.RESET_ALL)
    ...
    def _handle_ws_reconnection(self, ws_type: str):
        ...
        logger.info(f"Reconnecting {ws_type} WebSocket in {delay} seconds (attempt {self.ws_reconnect_attempts})")
        time.sleep(delay)
    ...
    def _on_open_private(self, ws):
        ...
        print(Fore.GREEN + "# Private WebSocket portal opened and authenticated!" + Style.RESET_ALL)
        ...
        try:
            ...
            for pos_data in positions_data:
                ...
                if symbol and symbol in self.positions:
                    ...
                    logger.info(f"Position updated for {symbol}: PnL={position.unrealized_pnl}")
    ...
    def _check_position_conditions(self, symbol: str, position: Position):
        ...
        try:
            ...
            profit_pct = (position.unrealized_pnl / abs(position.entry_price * position.size)) * 100
    ...
    def check_health(self) -> bool:
        ...
        print(Fore.CYAN + "# Performing an enhanced ethereal health check..." + Style.RESET_ALL)
        ...
        all_healthy = (
            self.connection_health["api"] and
            self.connection_health["ws_private"] and
            self.connection_health["ws_public"]
        )

        print(Fore.GREEN + f"# Enhanced Health Report:\n{json.dumps(health_status, indent=2)}" + Style.RESET_ALL)
...
if __name__ == "__main__":
    print(Fore.MAGENTA + "### Enhanced mystical ritual to test the Bybit V5 Wizard ###" + Style.RESET_ALL)

    config_path = os.path.expanduser("~/.bybit_config")
    if not os.path.exists(config_path):
        print(Fore.RED + "Error: ~/.bybit_config not found. Please create it with your API credentials." + Style.RESET_ALL)
        print(Fore.YELLOW + "Example: echo '{\"api_key\": \"YOUR_API_KEY\", \"api_secret\": \"YOUR_API_SECRET\", \"testnet\": true}' > ~/.bybit_config" + Style.RESET_ALL)
        exit()
    ...
    try:
        ...
        print(Fore.CYAN + f"\n# System Status:\n{json.dumps(status, indent=2)}" + Style.RESET_ALL)
        ...
        kline_data = bybit_wizard.get_kline(category="linear", symbol="BTCUSDT", interval="1")
        print(f"BTCUSDT 1-min Kline (first entry): {kline_data.get('list', [])[0] if kline_data.get('list') else 'N/A'}")

        print(Fore.CYAN + "\n--- Enhanced Account Management Spells ---" + Style.RESET_ALL)
        account_balance = bybit_wizard.get_wallet_balance(account_type="UNIFIED", coin="USDT")
        ...
        # Monitor for updates
        print(Fore.CYAN + "\n# Enhanced wizard is now watching the market with mystical algorithms. Press Ctrl+C to stop." + Style.RESET_ALL)
        ...
        print(Fore.RED + "\n# The enhanced wizard's journey ends. Script terminated by user." + Style.RESET_ALL)
    except Exception as e:
        logger.error(f"Unexpected error: {e!s}")
        logger.error(traceback.format_exc())
        print(Fore.RED + f"\n# An unexpected error befell the enhanced wizard: {e!s}" + Style.RESET_ALL)
    finally:
        if 'bybit_wizard' in locals():
            bybit_wizard.close_ws()
            final_status = bybit_wizard.get_system_status()
            print(Fore.CYAN + f"\n# Final System Status:\n{json.dumps(final_status, indent=2)}" + Style.RESET_ALL)
