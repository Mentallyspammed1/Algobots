...
__all__ = ["WebSocket", "WebSocketWSGI"]
    ...
    def close(self):
        ...
        try:
            ...
            if e.errno != errno.ENOTCONN:
                self.log.write(f'{self.log_context} socket shutdown error: {e}')
        finally:
            self.socket.close()
...
class RFC6455WebSocket(WebSocket):
            ...
            if not self.opcode & 8 and self.decompressor:
                ...
                if self.decompressor.unconsumed_tail:
                    raise FailedConnectionError(
                        1009,
                        f"Incoming compressed frame exceeds length limit of {self.max_frame_length} bytes.")
    ...
    def _recv_frame(self, message=None):
        ...
        if length > self.max_frame_length:
            raise FailedConnectionError(1009, f"Incoming frame of {length} bytes is above length limit of {self.max_frame_length} bytes.")
        if masked:
