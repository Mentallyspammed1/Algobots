...
__all__ = ['format_date_time', 'server']
        ...
        try:
            ...
            if hasattr(conn, 'send') and hasattr(conn, 'recv'):
                ...
                raise NotImplementedError(
                    f'''eventlet.wsgi doesn't support sockets of type {type(conn)}''')
    ...
    def _read_request_line(self):
        ...
        try:
            ...
            if last_errno in BROKEN_SOCK:
                self.server.log.debug(f'({self.server.pid}) connection reset by peer {self.client_address!r}')
 ...
 Most likely, you need to fix HTTP parsing in your client software.""",
    ...
    def process_request(self, conn_state):
        try:
            ...
            self.log.debug(f'({self.pid}) timed out {conn_state[0]!r}')
...
If unsure, use eventlet.GreenPool.''')
    ...
    try:
        serv.log.info(f'({serv.pid}) wsgi starting up on {socket_repr(sock)}')
        while is_accepting:
            try:
                client_socket, client_addr = sock.accept()
                client_socket.settimeout(serv.socket_timeout)
                serv.log.debug(f'({serv.pid}) accepted {client_addr!r}')
        ...
        pool.waitall()
        serv.log.info(f'({serv.pid}) wsgi exited, is_accepting={is_accepting}')
        try:
