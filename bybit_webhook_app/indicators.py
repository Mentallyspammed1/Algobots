...
def calculate_supertrend(df, period=10, multiplier=3):
    ...
    for i in range(len(df)):
        if i == 0:
            ...
            if df['close'].iloc[i] > supertrend.iloc[i-1] and df['close'].iloc[i-1] <= supertrend.iloc[i-1]:
                trend.iloc[i] = True
            elif df['close'].iloc[i] < supertrend.iloc[i-1] and df['close'].iloc[i-1] >= supertrend.iloc[i-1]:
                trend.iloc[i] = False
            else:
            ...
            # Handle crossovers
            if trend.iloc[i] == True and supertrend.iloc[i-1] > final_lower_band.iloc[i]:
                supertrend.iloc[i] = final_lower_band.iloc[i]
            elif trend.iloc[i] == False and supertrend.iloc[i-1] < final_upper_band.iloc[i]:
                supertrend.iloc[i] = final_upper_band.iloc[i]
