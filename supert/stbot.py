...
class EnhancedConfig:
    ...
    TESTNET: bool = field(default_factory=lambda: os.getenv("BYBIT_TESTNET", "true").lower() in ['true', '1'])

    # Trading Configuration
    SYMBOL: str = field(default_factory=lambda: os.getenv("BYBIT_SYMBOL", "BTCUSDT"))
    CATEGORY: str = field(default_factory=lambda: os.getenv("BYBIT_CATEGORY", "linear")) # "linear", "spot", "inverse"
    LEVERAGE: Decimal = field(default_factory=lambda: Decimal(os.getenv("BYBIT_LEVERAGE", "10")))

    # Multi-timeframe Configuration
    PRIMARY_TIMEFRAME: str = field(default_factory=lambda: os.getenv("PRIMARY_TIMEFRAME", "15")) # e.g., "1", "5", "15", "60", "240", "D"
    SECONDARY_TIMEFRAMES: list[str] = field(default_factory=lambda: os.getenv("SECONDARY_TIMEFRAMES", "5,60").split(',')) # Additional TFs for confirmation
    LOOKBACK_PERIODS: int = field(default_factory=lambda: int(os.getenv("LOOKBACK_PERIODS", "200"))) # Klines to fetch initially

    # Adaptive SuperTrend Parameters
    ST_PERIOD_BASE: int = field(default_factory=lambda: int(os.getenv("ST_PERIOD", "10")))
    ST_MULTIPLIER_BASE: Decimal = field(default_factory=lambda: Decimal(os.getenv("ST_MULTIPLIER", "3.0")))
    ADAPTIVE_PARAMS: bool = field(default_factory=lambda: os.getenv("ADAPTIVE_PARAMS", "true").lower() in ['true', '1'])

    # Risk Management
    RISK_PER_TRADE_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("RISK_PER_TRADE_PCT", "1.0"))) # % of equity risked per trade
    MAX_POSITION_SIZE_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("MAX_POSITION_SIZE_PCT", "30.0"))) # Max % of equity for position
    STOP_LOSS_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("STOP_LOSS_PCT", "1.5"))) # Default SL % if ATR_STOPS is false
    TAKE_PROFIT_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("TAKE_PROFIT_PCT", "3.0"))) # Default TP % if ATR_STOPS is false
    USE_ATR_STOPS: bool = field(default_factory=lambda: os.getenv("USE_ATR_STOPS", "true").lower() in ['true', '1'])
    ATR_STOP_MULTIPLIER: Decimal = field(default_factory=lambda: Decimal(os.getenv("ATR_STOP_MULTIPLIER", "1.5"))) # ATR Multiplier for SL
    ATR_TP_MULTIPLIER: Decimal = field(default_factory=lambda: Decimal(os.getenv("ATR_TP_MULTIPLIER", "3.0"))) # ATR Multiplier for TP

    # Daily Limits
    MAX_DAILY_LOSS_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("MAX_DAILY_LOSS_PCT", "5.0")))
    ...
    MAX_DRAWDOWN_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("MAX_DRAWDOWN_PCT", "10.0"))) # Max % drop from peak equity

    # Order Type & Execution
    ORDER_TYPE: str = field(default_factory=lambda: os.getenv("ORDER_TYPE", "Market")) # "Market" or "Limit"
    LIMIT_ORDER_OFFSET_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("LIMIT_ORDER_OFFSET_PCT", "0.01")))  # Place limit orders 0.01% better than market
    POST_ONLY_LIMIT_ORDERS: bool = field(default_factory=lambda: os.getenv("POST_ONLY_LIMIT_ORDERS", "true").lower() in ['true', '1'])
    # HEDGE_MODE and POSITION_IDX are critical for Bybit V5 for derivatives
    HEDGE_MODE: bool = field(default_factory=lambda: os.getenv("BYBIT_HEDGE_MODE", "false").lower() in ['true', '1'])
    POSITION_IDX: int = field(default_factory=lambda: int(os.getenv("BYBIT_POSITION_IDX", "0")))  # 0=One-way mode, 1=Long, 2=Short in hedge mode

    # Signal Filters
    ADX_TREND_FILTER: bool = field(default_factory=lambda: os.getenv("ADX_TREND_FILTER", "true").lower() in ['true', '1'])
    ADX_MIN_THRESHOLD: int = field(default_factory=lambda: int(os.getenv("ADX_MIN_THRESHOLD", "25")))
    ADX_WINDOW: int = field(default_factory=lambda: int(os.getenv("ADX_WINDOW", "14"))) # ADX period
    VOLUME_FILTER: bool = field(default_factory=lambda: os.getenv("VOLUME_FILTER", "true").lower() in ['true', '1'])
    VOLUME_MULTIPLIER: Decimal = field(default_factory=lambda: Decimal(os.getenv("VOLUME_MULTIPLIER", "1.5"))) # Volume must be 1.5x avg
    RSI_FILTER: bool = field(default_factory=lambda: os.getenv("RSI_FILTER", "true").lower() in ['true', '1'])
    RSI_WINDOW: int = field(default_factory=lambda: int(os.getenv("RSI_WINDOW", "14"))) # RSI period
    RSI_OVERSOLD: int = field(default_factory=lambda: int(os.getenv("RSI_OVERSOLD", "30")))
    RSI_OVERBOUGHT: int = field(default_factory=lambda: int(os.getenv("RSI_OVERBOUGHT", "70")))
    MACD_FILTER: bool = field(default_factory=lambda: os.getenv("MACD_FILTER", "true").lower() in ['true', '1'])
    ...
    # Market Structure
    USE_ORDER_BOOK: bool = field(default_factory=lambda: os.getenv("USE_ORDER_BOOK", "true").lower() in ['true', '1'])
    ORDER_BOOK_IMBALANCE_THRESHOLD: Decimal = field(default_factory=lambda: Decimal(os.getenv("ORDER_BOOK_IMBALANCE_THRESHOLD", "0.6"))) # 60% imbalance
    ORDER_BOOK_DEPTH_LEVELS: int = field(default_factory=lambda: int(os.getenv("ORDER_BOOK_DEPTH_LEVELS", "10"))) # Levels to fetch
    USE_VOLUME_PROFILE: bool = field(default_factory=lambda: os.getenv("USE_VOLUME_PROFILE", "true").lower() in ['true', '1'])

    # Partial Take Profit
    PARTIAL_TP_ENABLED: bool = field(default_factory=lambda: os.getenv("PARTIAL_TP_ENABLED", "true").lower() in ['true', '1'])
    ...
    BREAKEVEN_ENABLED: bool = field(default_factory=lambda: os.getenv("BREAKEVEN_ENABLED", "true").lower() in ['true', '1'])
    BREAKEVEN_PROFIT_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("BREAKEVEN_PROFIT_PCT", "0.5"))) # Move SL to entry at 0.5% profit
    BREAKEVEN_OFFSET_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("BREAKEVEN_OFFSET_PCT", "0.01"))) # Offset above entry for breakeven SL
    TRAILING_SL_ENABLED: bool = field(default_factory=lambda: os.getenv("TRAILING_SL_ENABLED", "true").lower() in ['true', '1'])
    TRAILING_SL_ACTIVATION_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("TRAILING_SL_ACTIVATION_PCT", "1.0"))) # Activate trailing SL at 1% profit
    TRAILING_SL_DISTANCE_PCT: Decimal = field(default_factory=lambda: Decimal(os.getenv("TRAILING_SL_DISTANCE_PCT", "0.5"))) # Trail by 0.5%

    # Performance & Logging
    LOG_LEVEL: str = field(default_factory=lambda: os.getenv("LOG_LEVEL", "INFO"))
    SAVE_TRADES_CSV: bool = field(default_factory=lambda: os.getenv("SAVE_TRADES_CSV", "true").lower() in ['true', '1'])
    TRADES_FILE: str = "trades_history.csv"
    STATE_FILE: str = "bot_state.pkl"
    SAVE_STATE_INTERVAL: int = field(default_factory=lambda: int(os.getenv("SAVE_STATE_INTERVAL", "300")))  # seconds

    # API Retry Settings
    MAX_API_RETRIES: int = field(default_factory=lambda: int(os.getenv("MAX_API_RETRIES", "5")))
    API_RETRY_DELAY_SEC: int = field(default_factory=lambda: int(os.getenv("API_RETRY_DELAY_SEC", "5")))

    # Kelly Criterion
    USE_KELLY_SIZING: bool = field(default_factory=lambda: os.getenv("USE_KELLY_SIZING", "false").lower() in ['true', '1'])
    KELLY_FRACTION_CAP: Decimal = field(default_factory=lambda: Decimal(os.getenv("KELLY_FRACTION_CAP", "0.25")))  # Max 25% of Kelly fraction

    # Auto-close on shutdown
    AUTO_CLOSE_ON_SHUTDOWN: bool = field(default_factory=lambda: os.getenv("AUTO_CLOSE_ON_SHUTDOWN", "false").lower() in ['true', '1'])
    ...
    def __post_init__(self):
        ...
        try:
            ...
            raise ValueError(f"Invalid ORDER_TYPE: '{self.ORDER_TYPE}'. Choose from {[ot.name for ot in OrderType]}")

        # Validate API Keys
        if not self.API_KEY or not self.API_SECRET or self.API_KEY == "YOUR_BYBIT_API_KEY":
            raise ValueError("Bybit API Key or Secret not configured. Please set BYBIT_API_KEY and BYBIT_API_SECRET environment variables.")
        ...
        for attr in ['RISK_PER_TRADE_PCT', 'STOP_LOSS_PCT', 'TAKE_PROFIT_PCT',
            ...
            if not isinstance(val, Decimal) or val < 0:
                raise ValueError(f"Configuration error: {attr} must be a non-negative Decimal. Current: {val}")

        # Validate integer fields
        for attr in ['LOOKBACK_PERIODS', 'ST_PERIOD_BASE', 'ADX_MIN_THRESHOLD', 'ADX_WINDOW',
                     'RSI_WINDOW', 'RSI_OVERSOLD', 'RSI_OVERBOUGHT', 'MACD_FAST', 'MACD_SLOW',
                     'MACD_SIGNAL', 'ORDER_BOOK_DEPTH_LEVELS', 'MAX_DAILY_TRADES',
                     'MAX_CONSECUTIVE_LOSSES', 'SAVE_STATE_INTERVAL', 'MAX_API_RETRIES',
                     'API_RETRY_DELAY_SEC', 'POSITION_IDX']:
            val = getattr(self, attr)
            if not isinstance(val, int) or val < 0:
                raise ValueError(f"Configuration error: {attr} must be a non-negative integer. Current: {val}")
        ...
        if not self.PARTIAL_TP_TARGETS:
            ...
            self.PARTIAL_TP_TARGETS = [{"profit_pct": Decimal("1.0"), "close_qty_pct": Decimal("0.5")}]


        # Ensure leverage is 1 for SPOT, and warn if it was set otherwise
        if self.CATEGORY_ENUM == Category.SPOT and self.LEVERAGE != Decimal('1'):
            print(f"WARNING: Leverage is not applicable for SPOT category. Setting LEVERAGE to 1 for {self.SYMBOL}.")
            self.LEVERAGE = Decimal('1')

        # Validate positionIdx based on mode
        if self.HEDGE_MODE:
            if self.POSITION_IDX not in [1, 2]:
                raise ValueError(f"Invalid BYBIT_POSITION_IDX '{self.POSITION_IDX}' for Hedge Mode. Must be 1 (Long) or 2 (Short).")
        elif self.POSITION_IDX != 0:
            raise ValueError(f"Invalid BYBIT_POSITION_IDX '{self.POSITION_IDX}' for One-Way Mode. Must be 0.")
...
class PerformanceMetrics:
    ...
    historical_returns_pct: list[Decimal] = field(default_factory=list) # Percentage returns per trade
    ...
    def update(self, trade: Trade, current_balance: Decimal):
        ...
        if self.avg_loss_pct != Decimal('0'):
            ...
            gross_profit = sum(pnl for pnl in self.historical_returns_pct if pnl > Decimal('0'))
            gross_loss = sum(pnl for pnl in self.historical_returns_pct if pnl < Decimal('0'))
            if gross_loss != Decimal('0'):
                self.profit_factor = abs(gross_profit / gross_loss)
            else:
                self.profit_factor = Decimal('inf') if gross_profit > Decimal('0') else Decimal('0') # Infinite if no losses
        else:
            self.profit_factor = Decimal('inf') if self.winning_trades > 0 else Decimal('0') # Infinite if no losses
    ...
    def calculate_sharpe_ratio(self, risk_free_rate: Decimal = Decimal('0')) -> Decimal:
        ...
        for date_str in sorted_dates:
            ...
            if self.peak_balance > 0: # Use peak_balance as a base for daily returns if not tracked specifically
                daily_pct_returns.append((self.daily_pnl[date_str] / self.peak_balance) * Decimal('100'))
            else:
    ...
    def calculate_kelly_fraction(self, kelly_fraction_cap: Decimal = Decimal('1.0')) -> Decimal:
        ...
        """
        if self.winning_trades == 0 or self.losing_trades == 0 or self.total_trades < 10:
            return Decimal('0')
        ...
        if W == Decimal('0') or L == Decimal('0'): # Avoid division by zero
            return Decimal('0')
...
class BotState:
    ...
    # Thread safety
    lock: threading.RLock = field(default_factory=threading.RLock, compare=False) # Exclude lock from comparison and hashing
                ...
                with open(filepath, 'rb') as f:
                    ...
                    with self.lock:
                        ...
                        if today_str not in self.metrics.daily_pnl:
                            self.daily_start_balance = self.account_balance # This will be updated by get_account_balance
...
class EnhancedUI(threading.Thread):
    ...
    def _display_header(self):
        ...
        status_color = Fore.GREEN if "Live" in self.state.bot_status else Fore.YELLOW if "Analyzing" in self.state.bot_status else Fore.RED
    ...
    def _display_market_overview(self):
        ...
        print(f"Current Price: {price_color}{self.state.current_price:.4f} {arrow}{Style.RESET_ALL}")
        ...
        if primary_tf_data:
            if primary_tf_data.bid > 0 and primary_tf_data.ask > 0:
                print(f"Bid/Ask: {primary_tf_data.bid:.4f} / {primary_tf_data.ask:.4f} (Spread: {primary_tf_data.spread_pct:.3f}%)")
            if primary_tf_data.volume > 0:
                print(f"Volume ({self.config.PRIMARY_TIMEFRAME}): {primary_tf_data.volume:,.0f}")
            if primary_tf_data.order_book_imbalance != Decimal('0'):
                imb_color = (Fore.GREEN if primary_tf_data.order_book_imbalance > self.config.ORDER_BOOK_IMBALANCE_THRESHOLD else
                            Fore.RED if primary_tf_data.order_book_imbalance < -self.config.ORDER_BOOK_IMBALANCE_THRESHOLD else Fore.YELLOW)
                print(f"Order Book Imbalance: {imb_color}{primary_tf_data.order_book_imbalance:.2%}{Style.RESET_ALL}")
    ...
    def _display_indicators(self):
        ...
        if indicators:
            ...
            if rsi is not None and not pd.isna(rsi):
                ...
                print(f"RSI({self.config.RSI_WINDOW}): {rsi_color}{rsi:.1f}{Style.RESET_ALL}", end=" | ")

            # ADX
            adx = indicators.get('adx')
            if adx is not None and not pd.isna(adx):
                adx_color = Fore.GREEN if adx > self.config.ADX_MIN_THRESHOLD else Fore.YELLOW
                print(f"ADX({self.config.ADX_WINDOW}): {adx_color}{adx:.1f}{Style.RESET_ALL}", end=" | ")
            ...
            if vol_ratio is not None and not pd.isna(vol_ratio):
                ...
                print(f"Volume Ratio: {vol_color}{vol_ratio:.2f}x{Style.RESET_ALL}", end=" | ")
    ...
    def _display_position(self):
        ...
        if self.state.position:
            ...
            if pos.partial_closes:
                ...
                closed_pct = (closed_qty_sum / (closed_qty_sum + pos.size)) * Decimal('100') if (closed_qty_sum + pos.size) > 0 else Decimal('0')
            ...
            if self.state.account_balance > Decimal('0'):
                ...
                balance_color = Fore.GREEN if balance_change >= Decimal('0') else Fore.RED
    ...
    def _display_performance(self):
        ...
        # Overall PnL
        total_color = Fore.GREEN if metrics.total_pnl > Decimal('0') else Fore.RED if metrics.total_pnl < Decimal('0') else Fore.WHITE
        ...
        if metrics.best_trade_pct != Decimal('0') or metrics.worst_trade_pct != Decimal('0'):
            print(f"Best/Worst: {Fore.LIGHTGREEN_EX}+{metrics.best_trade_pct:.1f}%{Style.RESET_ALL} / "
        ...
        if metrics.avg_win_pct != Decimal('0') or metrics.avg_loss_pct != Decimal('0'):
            print(f"Avg Win/Loss: {Fore.LIGHTGREEN_EX}+{metrics.avg_win_pct:.1f}%{Style.RESET_ALL} / "
    ...
    def _display_logs(self):
        ...
        for msg in list(self.state.log_messages):
            ...
            elif "[SUCCESS]" in msg or "profit" in msg.lower() or "filled" in msg.lower() or "opened" in msg.lower():
    ...
    def _display_footer(self):
        ...
        if self.state.errors_count > 0:
            print(f"{Fore.RED}Errors: {self.state.errors_count}{Style.RESET_ALL} | ", end="")
        else:
            print("Errors: 0 | ", end="")
...
class MarketAnalyzer:
    ...
    def analyze_market_regime(self, df: pd.DataFrame) -> MarketRegime:
        ...
        if close_prices_series.empty or not pd.api.types.is_numeric_dtype(close_prices_series):
            return MarketRegime.UNKNOWN
        ...
        try:
            ...
            self.logger.warning("LinAlgError during polyfit for market regime. Returning UNKNOWN.")
            return MarketRegime.UNKNOWN
        ...
        trend_strength_threshold_abs = (Decimal('0.0005') * Decimal(str(avg_price))) # e.g., 0.05% change over window
        ...
        atr_pct = (atr / close_price_dec) * Decimal('100') if close_price_dec > Decimal('0') else Decimal('0')
        ...
        elif abs(Decimal(str(slope))) > trend_strength_threshold_abs: # Strong trend based on slope
    ...
    def analyze_volume_profile(self, df: pd.DataFrame, bins: int = 20) -> dict[str, Any]:
        ...
        for col in ['close', 'volume']:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors='coerce')
        df.dropna(inplace=True)
        if len(df) < bins: return {'poc': Decimal('0'), 'vwap': Decimal('0'), 'volume_profile': {}}

        # Calculate VWAP
        total_volume = Decimal(str(df['volume'].sum()))
        vwap = (Decimal(str((df['close'] * df['volume']).sum())) / total_volume) if total_volume > Decimal('0') else Decimal(str(df['close'].iloc[-1]))
        ...
        for _, row in df.iterrows():
            ...
            for i in range(bins):
                if bin_edges[i] <= price_val < bin_edges[i+1]:
                    volume_bins_dict[f"{bin_edges[i]:.4f}-{bin_edges[i+1]:.4f}"] += volume_val
                    break
            # Edge case for max price
            if price_val == float(max_price) and bins > 0: # Ensure bins exist for max_price case
                volume_bins_dict[f"{bin_edges[bins-1]:.4f}-{bin_edges[bins]:.4f}"] += volume_val
        ...
        if volume_bins_dict:
            ...
            if poc_range_str:
                try:
                    ...
                    self.logger.warning(f"Failed to parse POC range string: {poc_range_str}")
        ...
        return {
            'volume_profile': volume_bins_dict,
            'poc': poc_price,
            'vwap': vwap
        }

    def analyze_order_book_imbalance(self, bids: list[list[str]], asks: list[list[str]], depth: int) -> Decimal:
...
class IndicatorCalculator:
    ...
    def calculate_ehlers_filter(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if df.empty or len(df) < 2:
            df['ehlers_filter'] = np.nan # Or df['close'] if you prefer to fill for small data
            return df
    ...
    def calculate_adaptive_supertrend(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        if df.empty or len(df) < max(self.config.ST_PERIOD_BASE, 14): # Need enough data for ATR
        ...
        ta.supertrend(high=df['high'], low=df['low'], close=df['close'],
                      length=period, multiplier=multiplier, append=True, df=df)

        # Rename columns for consistency
        st_col_base = f'SUPERT_{period}_{multiplier:.1f}' # .1f for multiplier as it's a float
        ...
        if st_col in df.columns and st_dir_col in df.columns:
            ...
            df.drop([c for c in df.columns if c.startswith(st_col_base)], axis=1, inplace=True, errors='ignore')
        else:
            self.logger.warning(f"SuperTrend columns {st_col}, {st_dir_col} not found in DataFrame. Check pandas_ta version or parameters.")
            df['supertrend_line'] = np.nan
            df['supertrend_direction'] = np.nan
    ...
    def calculate_all_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.config.MACD_FILTER:
            macd_results = ta.macd(df['close'], fast=self.config.MACD_FAST, slow=self.config.MACD_SLOW, signal=self.config.MACD_SIGNAL, append=True)
            if macd_results is not None and not macd_results.empty:
                ...
                if macd_cols:
                    ...
                    df['macd_signal'] = df[macd_cols[1]] if len(macd_cols) > 1 else np.nan
        ...
        if bb_results is not None and not bb_results.empty:
            bb_cols = [c for c in df.columns if c.startswith('BBU_') or c.startswith('BBM_') or c.startswith('BBL_')]
            if bb_cols:
        ...
        if self.config.ADX_TREND_FILTER:
            adx_results = ta.adx(df['high'], df['low'], df['close'], length=self.config.ADX_WINDOW, append=True)
...
class PrecisionManager:
    ...
    def get_specs(self, symbol: str, category: Category | str) -> InstrumentSpecs | None:
        ...
        try:
            ...
            if hasattr(self.session, '_api_call'):
                response = self.session._api_call(self.session.get_instruments_info, category=category_str, symbol=symbol)
            else:
                response = self.session.get_instruments_info(category=category_str, symbol=symbol)

            if response and response['retCode'] == 0:
                info_list = response['result'].get('list', [])
                if not info_list:
                    self.logger.warning(f"No instrument info found for {symbol} in category {category_str}.")
                    return None
                ...
                if category_str == "spot":
                    ...
                    qty_step = Decimal(lot_size_filter.get('baseTickSize', '0.00000001'))
                    ...
                    max_order_qty = Decimal(lot_size_filter.get('baseMaxTradeQty', '1e9'))
                    ...
                    min_order_value = Decimal(lot_size_filter.get('minOrderAmt', '0')) # min notional for derivatives
                ...
                self.logger.info(f"Fetched instrument specs for {symbol} ({category_str}): {specs}")
                return specs
            else:
                error_msg = response.get('retMsg', 'Unknown error') if response else 'No response'
                self.logger.error(f"Failed to get instrument info for {symbol} ({category_str}): {error_msg}")
        except Exception as e:
            self.logger.error(f"Error fetching instrument specs for {symbol} ({category_str}): {e}", exc_info=True)
        return None
    ...
    def round_price(self, specs: InstrumentSpecs, price: Decimal) -> Decimal:
        ...
        if specs.tick_size == Decimal('0'):
            self.logger.warning(f"Tick size is zero for price {price}. Cannot round accurately.")
            return price
    ...
    def round_quantity(self, specs: InstrumentSpecs, quantity: Decimal) -> Decimal:
        ...
        if specs.qty_step == Decimal('0'):
            self.logger.warning(f"Quantity step is zero for quantity {quantity}. Cannot round accurately.")
            return quantity
...
class OrderSizingCalculator:
    ...
    def calculate_position_size(self, specs: InstrumentSpecs, balance: Decimal, entry: Decimal, sl: Decimal, leverage: Decimal, metrics: PerformanceMetrics) -> Decimal | None:
        ...
        if self.config.USE_KELLY_SIZING and metrics.total_trades >= 10: # Need enough data for Kelly
            ...
            if kelly_fraction > Decimal('0'):
                ...
                self.logger.debug(f"Using Kelly Criterion: Kelly Fraction={kelly_fraction:.2%}, Base Position Value=${position_value_usd:,.2f}")
            else:
        ...
        max_position_value_by_config = balance * (self.config.MAX_POSITION_SIZE_PCT / Decimal('100'))
        ...
        if qty < specs.min_order_qty:
            self.logger.warning(f"Calculated quantity {qty:.4f} is less than min order qty {specs.min_order_qty}. Adjusting to min order qty.")
            qty = specs.min_order_qty

        if qty > specs.max_order_qty:
            self.logger.warning(f"Calculated quantity {qty:.4f} exceeds max order qty {specs.max_order_qty}. Adjusting to max order qty.")
            qty = specs.max_order_qty

        # Check minimum order value for spot, or notional for derivatives
        if (self.config.CATEGORY_ENUM == Category.SPOT and (qty * entry) < specs.min_order_value) or \
           (self.config.CATEGORY_ENUM != Category.SPOT and (qty * entry) < specs.min_order_value): # for derivatives, min_order_value is min_order_amt
            self.logger.warning(f"Calculated order value {qty * entry:.2f} is less than min allowed value {specs.min_order_value:.2f}. No trade.")
            return None
    ...
    def _calculate_fixed_risk_size(self, specs: InstrumentSpecs, balance: Decimal, entry: Decimal, sl: Decimal, leverage: Decimal) -> Decimal | None:
        ...
        if final_qty < specs.min_order_qty:
            self.logger.warning(f"Calculated quantity {final_qty:.4f} is less than min order qty {specs.min_order_qty}. Adjusting to min order qty.")
            final_qty = specs.min_order_qty

        if final_qty > specs.max_order_qty:
            self.logger.warning(f"Calculated quantity {final_qty:.4f} exceeds max order qty {specs.max_order_qty}. Adjusting to max order qty.")
            final_qty = specs.max_order_qty

        # Check minimum order value for spot, or notional for derivatives
        if (self.config.CATEGORY_ENUM == Category.SPOT and (final_qty * entry) < specs.min_order_value) or \
           (self.config.CATEGORY_ENUM != Category.SPOT and (final_qty * entry) < specs.min_order_value):
            self.logger.warning(f"Calculated order value {final_qty * entry:.2f} is less than min allowed value {specs.min_order_value:.2f}. No trade.")
            return None
...
class AdvancedSupertrendBot:
    ...
    def __init__(self, config: EnhancedConfig):
        ...
        self.state_saver_thread = threading.Thread(target=self._state_saver_loop, daemon=True, name="StateSaver")
        ...
        for attempt in range(1, self.config.MAX_API_RETRIES + 1):
            try:
                ...
                if response and response['retCode'] == 0:
                    ...
                    error_msg = response.get('retMsg', 'Unknown error') if response else 'No response'
                    self.logger.error(f"API Error ({response.get('retCode', 'N/A')}): {error_msg} (Attempt {attempt}/{self.config.MAX_API_RETRIES}) for {api_method.__name__}")
                    with self.bot_state.lock:
                        self.bot_state.errors_count += 1
                        self.bot_state.last_error = error_msg
            except InvalidRequestError as e:
                self.logger.error(f"Invalid API Request: {e}. Not retrying this type of error. (Attempt {attempt}/{self.config.MAX_API_RETRIES}) for {api_method.__name__}")
                with self.bot_state.lock:
                    self.bot_state.errors_count += 1
                    self.bot_state.last_error = str(e)
                return None # Don't retry on bad requests
            except FailedRequestError as e:
                self.logger.error(f"Bybit Request Error: {e} (Attempt {attempt}/{self.config.MAX_API_RETRIES}) for {api_method.__name__}")
                with self.bot_state.lock:
                    self.bot_state.errors_count += 1
                    self.bot_state.last_error = str(e)
            except Exception as e:
                self.logger.error(f"General API Exception: {e} (Attempt {attempt}/{self.config.MAX_API_RETRIES}) for {api_method.__name__}", exc_info=True)
                with self.bot_state.lock:
                    self.bot_state.errors_count += 1
                    self.bot_state.last_error = str(e)

            if attempt < self.config.MAX_API_RETRIES:
                time.sleep(min(self.config.API_RETRY_DELAY_SEC * (2 ** (attempt - 1)), 60)) # Exponential backoff, max 60 seconds
        self.logger.critical(f"API call failed after {self.config.MAX_API_RETRIES} retries: {api_method.__name__} with kwargs {kwargs}")
        return None
    ...
    def run(self):
        ...
        with self.bot_state.lock:
            ...
            if self.bot_state.initial_balance == Decimal('0'): # Set initial balance only once
                ...
                self.bot_state.metrics.peak_balance = current_balance # Set peak balance initially
            ...
            if today_str not in self.bot_state.metrics.daily_pnl:
                 ...
                 self.bot_state.add_log("New trading day detected. Initializing daily balance and metrics.", "INFO")
            else:
                 self.bot_state.daily_start_balance = current_balance # Always update to current balance when bot starts for the day
                 self.bot_state.add_log(f"Resumed on existing trading day {today_str}. Daily start balance set to current.", "INFO")
        ...
        if not self._set_leverage_and_margin():
            self.bot_state.add_log("Failed to set leverage or margin mode. Shutting down.", "ERROR")
            self.logger.critical("Failed to set leverage or margin mode. Shutting down.")
            return
    ...
    def _websocket_manager(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                self.bot_state.add_log(f"WebSocket connection error: {e}. Reconnecting in {self.config.API_RETRY_DELAY_SEC}s...", "ERROR")
                self.logger.error(f"WebSocket connection error: {e}. Reconnecting in {self.config.API_RETRY_DELAY_SEC}s...", exc_info=True)
                self.ws_reconnect_event.set() # Set event to signal reconnection needed
                time.sleep(self.config.API_RETRY_DELAY_SEC) # Wait before attempting to reconnect
            finally:
    ...
    def _connect_websockets(self):
        ...
        try:
            ...
            self.ws_public = WebSocket(
                ...
                channel_type='spot' if self.config.CATEGORY_ENUM == Category.SPOT else 'unified', # unified for linear/inverse/option
            )
            # Private WebSocket for Positions, Wallet
            self.ws_private = WebSocket(
                testnet=self.config.TESTNET,
                channel_type='spot' if self.config.CATEGORY_ENUM == Category.SPOT else 'unified',
    ...
    def _handle_kline_message(self, message: dict[str, Any], timeframe: str):
        ...
        if kline['confirm']: # Only process confirmed candles
            ...
            self.logger.debug(f"New confirmed candle received for {timeframe} at {kline_timestamp}")
    ...
    def _handle_ticker_message(self, message: dict[str, Any]):
        ...
        with self.bot_state.lock:
            ...
            if self.config.PRIMARY_TIMEFRAME in self.bot_state.market_data:
                self.bot_state.market_data[self.config.PRIMARY_TIMEFRAME].close = new_price # Use 'close' for current price
                ...
                if 'bidPrice' in message['data'] and 'askPrice' in message['data']:
                    ...
                    if ask > Decimal('0'):
                        self.bot_state.market_data[self.config.PRIMARY_TIMEFRAME].spread_pct = ((ask - bid) / ask) * Decimal('100')
    ...
    def _handle_position_message(self, message: dict[str, Any]):
        ...
        """
        self.logger.info("Position update received via WebSocket. Triggering state update.")
    ...
    def _handle_wallet_message(self, message: dict[str, Any]):
        ...
        """
        self.logger.info("Wallet update received via WebSocket. Triggering balance update.")
        with self.bot_state.lock:
            ...
            self.logger.info(f"Account balance updated: {old_balance:,.2f} -> {new_balance:,.2f} USDT")
    ...
    def _handle_orderbook_message(self, message: dict[str, Any]):
        ...
        with self.bot_state.lock:
            ...
            if self.config.PRIMARY_TIMEFRAME in self.bot_state.market_data:
                ...
                if self.bot_state.market_data[self.config.PRIMARY_TIMEFRAME].bid > Decimal('0') and self.bot_state.market_data[self.config.PRIMARY_TIMEFRAME].ask > Decimal('0'):
    ...
    def _check_daily_limits(self) -> bool:
        ...
        if daily_pnl < (-self.config.MAX_DAILY_LOSS_PCT / Decimal('100')) * self.bot_state.daily_start_balance:
            self.bot_state.add_log(f"Daily loss limit ({self.config.MAX_DAILY_LOSS_PCT:.1f}%) exceeded ({daily_pnl:,.2f} USDT). Stopping new trades.", "WARNING")
    ...
    def _generate_signal(self) -> tuple[Signal, str, Decimal]:
        ...
        if pd.isna(ehlers_filter_latest) or pd.isna(ehlers_filter_prev) or \
           pd.isna(supertrend_line_latest) or pd.isna(supertrend_line_prev) or \
           pd.isna(st_dir_primary):
            return Signal.NEUTRAL, "Missing Ehlers Filter or Supertrend data in primary TF.", Decimal('0')

        ehlers_cross_up = ehlers_filter_prev < supertrend_line_prev and ehlers_filter_latest > supertrend_line_latest
        ehlers_cross_down = ehlers_filter_prev > supertrend_line_prev and ehlers_filter_latest < supertrend_line_latest
        ...
        if self.config.SECONDARY_TIMEFRAMES:
            for tf in self.config.SECONDARY_TIMEFRAMES:
                ...
                if df_tf is None or df_tf.empty or 'supertrend_direction' not in df_tf.columns:
                    self.logger.debug(f"Secondary TF {tf} data missing for ST confirmation.")
        ...
        # 2. ADX Trend Filter
        adx_ok = False
        if self.config.ADX_TREND_FILTER and 'adx' in latest_primary and not pd.isna(latest_primary['adx']):
            if latest_primary['adx'] > self.config.ADX_MIN_THRESHOLD:
                if (base_buy_signal and latest_primary.get('dmp', 0) > latest_primary.get('dmn', 0)) or \
                   (base_sell_signal and latest_primary.get('dmn', 0) > latest_primary.get('dmp', 0)):
                    adx_ok = True
        ...
        if adx_ok:
            ...
            self.logger.debug(f"ADX ({latest_primary.get('adx', 'N/A'):.1f}) below threshold {self.config.ADX_MIN_THRESHOLD} or direction not confirmed.")

        # 3. RSI Filter
        rsi_ok = False
        if self.config.RSI_FILTER and 'rsi' in latest_primary and not pd.isna(latest_primary['rsi']):
            if (base_buy_signal and latest_primary['rsi'] < self.config.RSI_OVERBOUGHT) or \
        ...
        # 4. MACD Confirmation
        macd_ok = False
        if self.config.MACD_FILTER and 'macd_hist' in latest_primary and not pd.isna(latest_primary['macd_hist']):
        ...
        # 5. Volume Filter
        volume_ok = False
        if self.config.VOLUME_FILTER and 'volume_ratio' in latest_primary and not pd.isna(latest_primary['volume_ratio']):
        ...
        if self.config.USE_ORDER_BOOK:
            ...
            if (base_buy_signal and imbalance > self.config.ORDER_BOOK_IMBALANCE_THRESHOLD) or \
               (base_sell_signal and imbalance < -self.config.ORDER_BOOK_IMBALANCE_THRESHOLD):
                order_book_ok = True
    ...
    def _execute_trade(self, signal: Signal):
        ...
        """
        specs = self.precision_manager.get_specs(self.config.SYMBOL, self.config.CATEGORY_ENUM)
        if not specs:
        ...
        # --- Calculate SL and TP ---
        primary_df = self.klines_dfs.get(self.config.PRIMARY_TIMEFRAME)
        if primary_df.empty or 'atr' not in primary_df.columns or primary_df['atr'].iloc[-1] == 0 or pd.isna(primary_df['atr'].iloc[-1]):
        ...
        if (side == "Buy" and sl_price >= entry_price) or (side == "Sell" and sl_price <= entry_price):
            self.bot_state.add_log(f"Invalid SL price calculated ({sl_price:.4f}) for entry {entry_price:.4f}. Using fallback.", "WARNING")
        ...
        if (side == "Buy" and tp_price <= entry_price) or (side == "Sell" and tp_price >= entry_price):
            self.bot_state.add_log(f"Invalid TP price calculated ({tp_price:.4f}) for entry {entry_price:.4f}. Using fallback.", "WARNING")
    ...
    def _manage_active_position(self):
        ...
        specs = self.precision_manager.get_specs(self.config.SYMBOL, self.config.CATEGORY_ENUM)
        if not specs:
            self.bot_state.add_log("Could not get instrument specs for position management. Skipping advanced management.", "ERROR")
            return
            ...
            if (pos.side == "Buy" and pnl_pct >= self.config.BREAKEVEN_PROFIT_PCT) or \
               (pos.side == "Sell" and pnl_pct >= self.config.BREAKEVEN_PROFIT_PCT):
                ...
                if (pos.side == "Buy" and new_sl > pos.stop_loss) or \
                   (pos.side == "Sell" and (new_sl < pos.stop_loss or pos.stop_loss == Decimal('0'))): # If SL was 0 or current SL is less favorable
        ...
        if self.config.TRAILING_SL_ENABLED:
            ...
            if pos.trailing_sl_activated:
                ...
                if (pos.side == "Buy" and new_trailing_sl > pos.stop_loss) or \
                   (pos.side == "Sell" and (new_trailing_sl < pos.stop_loss or pos.stop_loss == Decimal('0'))):
        ...
        if self.config.PARTIAL_TP_ENABLED and pos.size > specs.min_order_qty:
            for target in self.config.PARTIAL_TP_TARGETS:
                ...
                current_closed_pct_sum = sum(pc['closed_qty_pct'] for pc in pos.partial_closes) * Decimal('100')

                # Check if this specific target profit_pct has already been actioned
                if any(p['profit_pct'] == target['profit_pct'] for p in pos.partial_closes):
                    continue
                ...
                if (pos.side == "Buy" and pnl_pct >= target['profit_pct']) or \
                   (pos.side == "Sell" and pnl_pct >= target['profit_pct']):
                    ...
                    qty_to_close = self.precision_manager.round_quantity(specs, qty_to_close_from_remaining)
                    ...
                    if qty_to_close < specs.min_order_qty:
                        self.bot_state.add_log(f"Calculated partial TP qty {qty_to_close:.4f} is too small (Min: {specs.min_order_qty}). Skipping.", "WARNING")
                        continue
    ...
    def _fetch_initial_klines(self, timeframe: str) -> bool:
        ...
        if result and result.get('list'):
            ...
            if df.empty:
                self.bot_state.add_log(f"Fetched kline data for {timeframe} is empty after cleanup.", "WARNING")
                return False
            ...
            self.bot_state.add_log(f"Initial kline data fetched successfully for {timeframe} ({len(df)} candles).", "SUCCESS")
            return True
        self.bot_state.add_log(f"Failed to fetch initial kline data for {timeframe}. Result: {result}", "ERROR")
        return False
    ...
    def _update_kline_data(self, kline: dict[str, Any], timeframe: str):
        ...
        with self.bot_state.lock:
            ...
            new_df_row = pd.DataFrame([new_row], index=[new_row['timestamp']]) # Use timestamp as index
            ...
            df.reset_index(drop=True, inplace=True) # Reset index after dropping first row
            self.klines_dfs[timeframe] = df
            self.logger.debug(f"Kline data updated for {timeframe}. New length: {len(df)}")
            ...
            for tf, df in self.klines_dfs.items():
                ...
                if tf == self.config.PRIMARY_TIMEFRAME and not processed_df.empty:
                    ...
                    self.bot_state.indicator_values = {
                        k: v for k, v in latest_indicators_series.items()
                        if k not in ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'turnover']
                    }
                    self.bot_state.supertrend_line = Decimal(str(latest_indicators_series.get('supertrend_line', '0')))
                    self.bot_state.supertrend_direction = "Uptrend" if latest_indicators_series.get('supertrend_direction') == 1 else \
                                                          "Downtrend" if latest_indicators_series.get('supertrend_direction') == -1 else "Neutral"
                    ...
                    self.bot_state.market_data[tf] = MarketData(
                        ...
                        order_book_imbalance=self.bot_state.market_data[tf].order_book_imbalance,
    ...
    def _analyze_market_conditions(self):
        ...
        if primary_df is None or primary_df.empty:
            self.logger.warning("No primary timeframe data available for market condition analysis.")
            return
        ...
        if self.config.USE_VOLUME_PROFILE:
            vol_profile_data = self.market_analyzer.analyze_volume_profile(primary_df.copy())
            if vol_profile_data:
                with self.bot_state.lock:
                    self.bot_state.market_data[self.config.PRIMARY_TIMEFRAME].poc = vol_profile_data.get('poc', Decimal('0'))
                    self.bot_state.market_data[self.config.PRIMARY_TIMEFRAME].vwap = vol_profile_data.get('vwap', Decimal('0'))
    ...
    def _update_position_state(self):
        ...
        try:
            ...
            response = self._api_call(self.session.get_positions,
                                      category=self.config.CATEGORY,
                                      symbol=self.config.SYMBOL)
            ...
            if response and 'list' in response:
                ...
                for pos_data in response.get('list', []):
                    ...
                    if pos_data['symbol'] == self.config.SYMBOL and Decimal(pos_data.get('size', '0')) > Decimal('0'):
                        # If in hedge mode, ensure we pick the correct position (Long/Short)
                        if self.config.HEDGE_MODE:
                            if (self.config.POSITION_IDX == 1 and pos_data['side'] == 'Buy') or \
                               (self.config.POSITION_IDX == 2 and pos_data['side'] == 'Sell'):
                ...
                with self.bot_state.lock:
                    if current_pos_data:
                        ...
                        if self.bot_state.position and self.bot_state.position.symbol == self.config.SYMBOL:
                            ...
                            self.logger.debug(f"Existing position state updated from API: {side} {size:.4f} @ {entry_price:.4f}")
                        else:
                            ...
                            self.bot_state.position = Position(
                                ...
                                entry_time=datetime.now(timezone.utc) # Approximation, actual entry time might be harder to get
                            )
                            self.bot_state.add_log(f"New position opened via API: {side} {size:.4f} @ {entry_price:.4f}", "INFO")
                    else:
                        # No active position found for the symbol
                        if self.bot_state.position and self.bot_state.position.symbol == self.config.SYMBOL:
                            self.bot_state.add_log("Position closed (no active position found in API).", "INFO")
                            # Log the closed trade if it was a valid position
                            if self.bot_state.position.size > Decimal('0'):
                                self._log_closed_trade(self.bot_state.position, "Position Closed (API)")
    ...
    def _get_account_balance(self) -> Decimal:
        ...
        try:
            ...
            account_type = "UNIFIED" if self.config.CATEGORY_ENUM != Category.SPOT else "SPOT"
            ...
            quote_currency = "USDT" # Default to USDT, adjust if other quote currencies are used

            response = self._api_call(self.session.get_wallet_balance,
                                      accountType=account_type)

            if response and 'list' in response:
                for account_info in response['list']:
                    for coin_data in account_info.get('coin', []): # Bybit response structure
                        if coin_data.get('coin') == quote_currency:
                            # Use availableBalance for trading decisions
                            balance = Decimal(coin_data.get('availableToWithdraw', '0')) # availableToWithdraw or walletBalance
                            return balance
                self.logger.warning(f"Balance for coin {quote_currency} not found in wallet response for {account_type} account.")
            else:
                self.logger.warning(f"Wallet balance response was empty or invalid: {response}")
        except Exception as e:
    ...
    def _set_leverage_and_margin(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Setting leverage to {leverage_str}x for {self.config.SYMBOL}...")

            # Set leverage
            leverage_response = self._api_call(self.session.set_leverage,
                                               category=self.config.CATEGORY,
                                               symbol=self.config.SYMBOL,
            ...
            elif leverage_response and "leverage is already set" in leverage_response.get('retMsg', '').lower():
                self.logger.info(f"Leverage for {self.config.SYMBOL} is already {leverage_str}x. No change needed.")
            else:
                self.logger.warning(f"Failed to set leverage for {self.config.SYMBOL}: {leverage_response.get('retMsg', 'Unknown error')}")
                return False

            # Set margin mode to 'Both' (Cross Margin) if not already set.
            # 'is_isolated=False' for cross margin.
            self.logger.info(f"Setting margin mode to Cross Margin for {self.config.SYMBOL}...")
            margin_mode_response = self._api_call(self.session.switch_isolated_margin,
                                                  category=self.config.CATEGORY,
                                                  symbol=self.config.SYMBOL,
            ...
            elif margin_mode_response and "no actual change" in margin_mode_response.get('retMsg', '').lower():
                self.logger.info(f"Margin mode for {self.config.SYMBOL} is already Cross Margin. No change needed.")
            else:
                self.logger.warning(f"Failed to set margin mode to Cross Margin for {self.config.SYMBOL}: {margin_mode_response.get('retMsg', 'Unknown error')}")
            ...
            self.bot_state.add_log(f"Error setting leverage or margin mode: {e}", "ERROR")
            self.logger.error(f"Error setting leverage or margin mode: {e}", exc_info=True)
            return False

    def _place_order(self, side: str, qty: Decimal, order_type: OrderType, entry_price: Decimal, sl: Decimal, tp: Decimal, is_partial_close: bool = False, order_link_id: str | None = None) -> str | None:
        ...
        specs = self.precision_manager.get_specs(self.config.SYMBOL, self.config.CATEGORY_ENUM)
        if not specs:
            self.bot_state.add_log("Cannot place order: Instrument specs not available.", "ERROR")
            return None
        ...
        if qty_rounded < specs.min_order_qty:
            self.bot_state.add_log(f"Order quantity {qty_rounded:.4f} is below minimum {specs.min_order_qty}. Cannot place order.", "ERROR")
            return None
        ...
        if order_type == OrderType.LIMIT:
            ...
            if side == "Buy":
                limit_price = entry_price * (Decimal('1') - (self.config.LIMIT_ORDER_OFFSET_PCT / Decimal('100')))
            else: # Sell
                limit_price = entry_price * (Decimal('1') + (self.config.LIMIT_ORDER_OFFSET_PCT / Decimal('100')))
            order_params['price'] = str(self.precision_manager.round_price(specs, limit_price))
            if self.config.POST_ONLY_LIMIT_ORDERS:
                order_params['orderFilter'] = 'PostOnly' # Use 'PostOnly' for maker fees
        ...
        if self.config.CATEGORY_ENUM != Category.SPOT:
            if sl > Decimal('0'):
                order_params['stopLoss'] = str(self.precision_manager.round_price(specs, sl))
            if tp > Decimal('0'):
                order_params['takeProfit'] = str(self.precision_manager.round_price(specs, tp))
        ...
        if order_link_id:
            ...
            order_params['orderLinkId'] = f"bot_{side}_{int(time.time())}" # Generate a unique ID
        ...
        try:
            ...
            if response and 'orderId' in response.get('result', {}):
                ...
                self.bot_state.add_log(f"Order placed successfully: ID {order_id}", "SUCCESS")
                return order_id
            else:
                error_msg = response.get('retMsg', 'Unknown error during order placement') if response else 'No response received'
    ...
    def _update_trading_stop(self, stop_loss: Decimal | None = None, take_profit: Decimal | None = None) -> bool:
        ...
        pos = self.bot_state.position
        if not pos:
            self.bot_state.add_log("Cannot update stop loss/take profit: No active position.", "WARNING")
            return False

        if self.config.CATEGORY_ENUM == Category.SPOT:
            self.logger.warning("Stop loss/take profit updates are not directly supported for SPOT category via set_trading_stop.")
            return False

        specs = self.precision_manager.get_specs(self.config.SYMBOL, self.config.CATEGORY_ENUM)
        if not specs:
            self.bot_state.add_log("Cannot update stop loss/take profit: Instrument specs not available.", "ERROR")
            return False
        ...
        if stop_loss is not None:
            ...
            if (pos.side == "Buy" and stop_loss >= pos.current_price) or \
               (pos.side == "Sell" and stop_loss <= pos.current_price):
                self.bot_state.add_log(f"Calculated SL {stop_loss:.4f} invalid for {pos.side} position at {pos.current_price:.4f}. Skipping SL update.", "WARNING")
        ...
        if take_profit is not None:
            ...
            if (pos.side == "Buy" and take_profit <= pos.current_price) or \
               (pos.side == "Sell" and take_profit >= pos.current_price):
                self.bot_state.add_log(f"Calculated TP {take_profit:.4f} invalid for {pos.side} position at {pos.current_price:.4f}. Skipping TP update.", "WARNING")
        ...
        try:
            ...
            if response and response.get('retCode') == 0:
                self.bot_state.add_log("Trading stop/take profit updated successfully.", "SUCCESS")
                ...
                error_msg = response.get('retMsg', 'Unknown error') if response else 'No response'
                self.bot_state.add_log(f"Failed to update trading stop/take profit: {error_msg}", "ERROR")
                return False
        except Exception as e:
            self.bot_state.add_log(f"Exception updating trading stop/take profit: {e}", "ERROR")
            self.logger.error(f"Exception updating trading stop/take profit: {e}", exc_info=True)
            return False
    ...
    def _close_position(self, partial_qty: Decimal | None = None) -> bool:
        ...
        pos = self.bot_state.position
        if not pos:
            self.bot_state.add_log("Cannot close position: No active position.", "WARNING")
            return False

        specs = self.precision_manager.get_specs(self.config.SYMBOL, self.config.CATEGORY_ENUM)
        if not specs:
            self.bot_state.add_log("Cannot close position: Instrument specs not available.", "ERROR")
            return False
        ...
        if qty_to_close < specs.min_order_qty:
            self.bot_state.add_log(f"Quantity to close ({qty_to_close:.4f}) is below minimum {specs.min_order_qty}. Cannot close.", "WARNING")
            return False
        ...
        self.bot_state.add_log(f"Attempting to close {qty_to_close:.4f} of {pos.symbol} {pos.side} position.", "INFO")
        ...
        order_id = self._place_order(
            ...
            order_link_id=f"close_{pos.order_id}_{int(time.time())}" if pos.order_id else f"close_auto_{int(time.time())}"
        )

        if order_id:
            self.bot_state.add_log(f"Close order {order_id} placed for {qty_to_close:.4f} of {pos.symbol}.", "SUCCESS")
            ...
            with self.bot_state.lock:
                ...
                if pos.size <= specs.min_order_qty: # If position is effectively closed
                    self.bot_state.add_log(f"Position for {pos.symbol} fully closed locally.", "INFO")
    ...
    def _log_closed_trade(self, position: Position, exit_reason: str):
        ...
        specs = self.precision_manager.get_specs(position.symbol, self.config.CATEGORY_ENUM)
        if not specs:
        ...
        self.bot_state.add_log(f"Trade closed: {trade.side} {trade.size:.4f} {trade.symbol} | PnL: ${trade.pnl:,.2f} ({trade.pnl_pct:+.2f}%) | Reason: {exit_reason}", "SUCCESS")
    ...
    def _save_trade_to_csv(self, trade: Trade):
        ...
        with open(self.config.TRADES_FILE, 'a', newline='') as f:
            headers = ['symbol', 'side', 'entry_price', 'exit_price', 'size',
                       'pnl', 'pnl_pct', 'entry_time', 'exit_time', 'duration_seconds', 'exit_reason', 'fees']
    ...
    def _signal_handler(self, sig, frame):
        ...
        self.bot_state.add_log(f"Received signal {sig}. Shutting down gracefully...", "INFO")
    ...
    def _state_saver_loop(self):
        ...
        while not self.bot_state.stop_requested and not self.stop_event.is_set():
            ...
            if not self.bot_state.stop_requested and not self.stop_event.is_set(): # Check again after sleep
    ...
    def cleanup(self):
        ...
        if self.config.AUTO_CLOSE_ON_SHUTDOWN and self.bot_state.position:
            self.bot_state.add_log("Auto-close on shutdown enabled. Attempting to close open position.", "WARNING")
            self._close_position() # This will log the trade
        elif self.bot_state.position:
            self.bot_state.add_log("Bot shutting down with open position. Manual intervention may be required.", "WARNING")
            self.logger.warning("Bot shutting down with open position. Manual intervention may be required.")
            ...
            self._log_closed_trade(self.bot_state.position, "Shutdown (Unclosed Position)")
        ...
        if final_balance > Decimal('0'):
            with self.bot_state.lock:
                ...
                total_pnl_pct = (total_pnl / self.bot_state.initial_balance * 100) if self.bot_state.initial_balance > Decimal('0') else Decimal('0')
            ...
            self.bot_state.add_log(f"Total PnL: {total_pnl:,.2f} USDT ({total_pnl_pct:+.2f}%)", "INFO")
            self.bot_state.add_log("=" * 60, "INFO")
        else:
            self.bot_state.add_log("Final balance retrieval failed or zero. Cannot calculate total PnL.", "WARNING")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if bot_instance:
            ...
            logging.basicConfig(level=logging.CRITICAL, format='%(asctime)s | FATAL - %(message)s', datefmt='%H:%M:%S')
