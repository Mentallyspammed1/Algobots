...
class Config:
    ...
    CATEGORY: str = field(
        default="linear"
    )  # Store as string initially, convert to Enum later
    ...
    # Supertrend Indicator Parameters (Dual Supertrend)
    ST_FAST_PERIOD: int = field(default=10)
    ST_FAST_MULTIPLIER: float = field(default=3.0)
    ST_SLOW_PERIOD: int = field(default=20)
    ST_SLOW_MULTIPLIER: float = field(default=4.0)
    ...
    MAX_OPEN_POSITIONS: int = field(
        default=1
    )  # Not actively used in current logic, but good for future
    ...
    LOG_FILE: str = field(default="supertrend_bot_v13.log")
    ...
    def __post_init__(self):
        ...
        self.RISK_PER_TRADE_PCT = float(
            os.getenv("BYBIT_RISK_PER_TRADE_PCT", self.RISK_PER_TRADE_PCT)
        )
        ...
        # Load Dual Supertrend Parameters
        self.ST_FAST_PERIOD = int(os.getenv("BYBIT_ST_FAST_PERIOD", self.ST_FAST_PERIOD))
        self.ST_FAST_MULTIPLIER = float(os.getenv("BYBIT_ST_FAST_MULTIPLIER", self.ST_FAST_MULTIPLIER))
        self.ST_SLOW_PERIOD = int(os.getenv("BYBIT_ST_SLOW_PERIOD", self.ST_SLOW_PERIOD))
        self.ST_SLOW_MULTIPLIER = float(os.getenv("BYBIT_ST_SLOW_MULTIPLIER", self.ST_SLOW_MULTIPLIER))

        self.STOP_LOSS_PCT = float(os.getenv("BYBIT_STOP_LOSS_PCT", self.STOP_LOSS_PCT))
        self.TAKE_PROFIT_PCT = float(
            os.getenv("BYBIT_TAKE_PROFIT_PCT", self.TAKE_PROFIT_PCT)
        )
        self.TRAILING_STOP_PCT = float(
            os.getenv("BYBIT_TRAILING_STOP_PCT", self.TRAILING_STOP_PCT)
        )
        self.MAX_DAILY_LOSS_PCT = float(
            os.getenv("BYBIT_MAX_DAILY_LOSS_PCT", self.MAX_DAILY_LOSS_PCT)
        )
        ...
        self.LOG_TO_STDOUT_ONLY = os.getenv(
            "BYBIT_LOG_TO_STDOUT_ONLY", str(self.LOG_TO_STDOUT_ONLY)
        ).lower() in ["true", "1", "t"]
        ...
        if self.HEDGE_MODE:
            if self.POSITION_IDX not in [1, 2]:
                print(
                    f"Configuration Error: Invalid POSITION_IDX '{self.POSITION_IDX}' for Hedge Mode. Must be 1 (Long) or 2 (Short)."
                )
                sys.exit(1)
        elif self.POSITION_IDX != 0:
            print(
                f"Configuration Error: Invalid POSITION_IDX '{self.POSITION_IDX}' for One-Way Mode. Must be 0."
            )
            sys.exit(1)
...
class PrecisionManager:
    ...
    def _parse_instrument_specs(self, inst: dict, category: str) -> InstrumentSpecs:
        ...
        qty_step = safe_decimal(
            lot_size_filter.get("qtyStep", lot_size_alt.get("qtyStep", "0.00000001"))
        )  # Default to high precision
        min_order_qty = safe_decimal(
            lot_size_filter.get("minOrderQty", lot_size_alt.get("minOrderQty", "0"))
        )
        max_order_qty = safe_decimal(
            lot_size_filter.get("maxOrderQty", lot_size_alt.get("maxOrderQty", "1e9"))
        )
        ...
        max_position_value = safe_decimal(
            lot_size_filter.get(
                "maxMktOrderQty", lot_size_filter.get("maxOrderAmt", "1e9")
            )
        )
        min_position_value = safe_decimal(
            lot_size_filter.get("minOrderAmt", lot_size_filter.get("minOrderQty", "1"))
        ...
        try:
            ...
            if quantize_exp.as_tuple().exponent < 0:
                rounded_value = rounded_value.quantize(
                    quantize_exp, rounding=ROUND_DOWN
                )
    ...
    def round_price(self, symbol: str, price: float | Decimal) -> Decimal:
        ...
        if not specs:
            self.logger.warning(
                f"Symbol {symbol} specs not found. Using default high precision rounding for price."
            )
            try:
                ...
                return Decimal(
                    str(price)
                )  # Return original if even high precision fails
    ...
    def round_quantity(self, symbol: str, quantity: float | Decimal) -> Decimal:
        ...
        if not specs:
            self.logger.warning(
                f"Symbol {symbol} specs not found. Using default high precision rounding for quantity."
            )
            try:
                # Default to a high precision if specs are missing
...
class OrderSizingCalculator:
    ...
    def calculate_position_size_usd(
        ...
        if position_value_usd < specs.min_position_value:
            self.logger.warning(
                f"Calculated position value ({position_value_usd:.4f} USD) is below minimum ({specs.min_position_value:.4f} USD). Using minimum."
            )
        ...
        actual_risk_percent = (
            (actual_risk_amount_usdt / account_balance_usdt) * 100
            if account_balance_usdt > 0
            else Decimal("0")
        )
...
class TrailingStopManager:
    ...
    def __init__(
        self, session: HTTP, precision_manager: PrecisionManager, logger: logging.Logger
    ):
        self.session = session
        self.precision = precision_manager
        self.logger = logger
    ...
    def initialize_trailing_stop(
        ...
        if specs.category == "spot":
            self.logger.debug(
                f"Trailing stops are not applicable for spot category {symbol}."
            )
            return None
        ...
        if position_side == "Buy":
            ...
            if is_activated and highest_price > 0:
                current_stop = self.precision.round_price(
                    symbol, highest_price * (Decimal("1") - trail_pct)
                )
        else:  # Sell/Short
            activation_price = entry_price * (Decimal("1") - activation_pct)
            is_activated = current_price <= activation_price
            ...
            if is_activated and lowest_price > 0:
                current_stop = self.precision.round_price(
                    symbol, lowest_price * (Decimal("1") + trail_pct)
                )
        ...
        if is_activated and current_stop > 0:
            self.logger.info(
                f"Initialized and activated trailing stop for {symbol} ({position_side}): Initial Stop={current_stop}, Activation={activation_price}"
            )
        else:
            self.logger.info(
                f"Initialized trailing stop for {symbol} ({position_side}): Activation={activation_price}"
            )
        return trailing_stop_info

    def update_trailing_stop(
        self, symbol: str, current_price: Decimal, update_exchange: bool = True
    ) -> bool:
        """
        ...
        if not ts_info.get("is_activated", False):
            if (
                ts_info["side"] == "Buy"
                and current_price >= ts_info["activation_price"]
            ) or (
                ts_info["side"] == "Sell"
                and current_price <= ts_info["activation_price"]
            ):
                ts_info["is_activated"] = True
                ts_info["highest_price"] = (
                    current_price
                    if ts_info["side"] == "Buy"
                    else ts_info.get("highest_price")
                )
                ts_info["lowest_price"] = (
                    current_price
                    if ts_info["side"] == "Sell"
                    else ts_info.get("lowest_price")
                )
                self.logger.info(
                    f"Trailing stop for {symbol} ({ts_info['side']}) activated at price {current_price}."
                )
                if ts_info["side"] == "Buy":
                    new_stop_price = ts_info["highest_price"] * (
                        Decimal("1") - ts_info["trail_percent"]
                    )
                else:
                    new_stop_price = ts_info["lowest_price"] * (
                        Decimal("1") + ts_info["trail_percent"]
                    )

        # --- Update Stop Price if Activated ---
        if ts_info.get("is_activated", False):
            if ts_info["side"] == "Buy" and current_price > ts_info.get(
            ...
            elif ts_info["side"] == "Sell" and current_price < ts_info.get(
        ...
        if new_stop_price is not None:
            ...
            if (ts_info["side"] == "Buy" and rounded_new_stop > current_stop) or (
                ts_info["side"] == "Sell"
                and (rounded_new_stop < current_stop or current_stop == 0)
            ):
            ...
            if is_more_favorable:
                ...
                self.logger.debug(
                    f"Trailing stop for {symbol} locally updated to: {ts_info['current_stop']}"
                )

                if update_exchange:
                    self.logger.info(
                        f"Attempting to update exchange stop loss for {symbol} to {ts_info['current_stop']}"
                    )
                    try:
                        specs = self.precision.get_specs(symbol)
                        if not specs or specs.category == "spot":
                            self.logger.warning(
                                f"Cannot set trading stop for {symbol} in category {specs.category if specs else 'N/A'}"
                            )
                        ...
                        if response and response["retCode"] == 0:
                            self.logger.info(
                                f"Successfully updated exchange stop loss for {symbol} to {ts_info['current_stop']}"
                            )
                        else:
                            error_msg = (
                                response.get("retMsg", "Unknown error")
                            ...
                            self.logger.error(
                                f"Failed to update exchange stop loss for {symbol}: {error_msg}"
                            )
                    except Exception as e:
                        self.logger.error(
                            f"Exception updating exchange stop loss for {symbol}: {e}",
                            exc_info=True,
                        )
...
class SupertrendBot:
    def __init__(self, config: Config):
        ...
        self.logger.info("Initializing Supertrend Trading Bot (v13.0 - Dual Cross)...")
        ...
        self.trailing_stop_manager = TrailingStopManager(
            self.session, self.precision_manager, self.logger
        )
        ...
        self.logger.info(
            f"  Dual Supertrend Params: Fast(P={config.ST_FAST_PERIOD}, M={config.ST_FAST_MULTIPLIER}), Slow(P={config.ST_SLOW_PERIOD}, M={config.ST_SLOW_MULTIPLIER})"
        )
        self.logger.info(
    ...
    def _init_api_session(self):
        ...
        for attempt in range(1, self.config.MAX_API_RETRIES + 1):
            try:
                ...
                # Ensure to use a category that is relevant to the user's intended trading.
                category_to_test = self.config.CATEGORY_ENUM.value
                # If category is SPOT, might not have standard 'leverageFilter' etc.
                # Fetching instruments for the primary category should suffice.
                self.session.get_instruments_info(
                    category=category_to_test
                )
                self.logger.info(
                    f"Successfully connected to Bybit API ({'Testnet' if self.config.TESTNET else 'Mainnet'})."
                )
                return  # Exit loop on success
            except FailedRequestError as e:
                error_code = e.status_code if hasattr(e, 'status_code') else 'N/A'
                self.logger.critical(
                    f"Failed to initialize Bybit API session or validate keys (Attempt {attempt}/{self.config.MAX_API_RETRIES}), Status Code: {error_code}: {e}",
                    exc_info=True,
                )
                if attempt < self.config.MAX_API_RETRIES:
                    self.logger.info(
                        f"Retrying in {self.config.API_RETRY_DELAY_SEC} seconds..."
                    )
                ...
                self.logger.critical(
                    f"Unexpected error during API session initialization (Attempt {attempt}/{self.config.MAX_API_RETRIES}): {e}",
                    exc_info=True,
                )
                if attempt < self.config.MAX_API_RETRIES:
                    self.logger.info(
    ...
    def fetch_klines(self, limit: int | None = None) -> pd.DataFrame:
        ...
        try:
            # Ensure we fetch enough data for the slower indicator's calculation
            # ST_SLOW_PERIOD + a few extra candles for lookback on signal generation
            required_lookback = max(self.config.ST_SLOW_PERIOD, self.config.ST_FAST_PERIOD) + 10
            fetch_limit = limit if limit else max(required_lookback, self.config.LOOKBACK_PERIODS)
            ...
            if response and response["retCode"] == 0:
                ...
                self.logger.error(
                    f"Failed to fetch klines for {self.config.SYMBOL}: {error_msg}"
                )
                return pd.DataFrame()
    ...
    def get_ticker(self) -> dict | None:
        ...
        try:
            response = self.session.get_tickers(
                category=self.config.CATEGORY_ENUM.value, symbol=self.config.SYMBOL
            )
            ...
            if response and response["retCode"] == 0:
                ...
                if tickers:
                    ...
                    self.logger.warning(
                        f"Ticker data list is empty for {self.config.SYMBOL}."
                    )
                    return None
            else:
                error_msg = (
                ...
                self.logger.error(
                    f"Failed to fetch ticker for {self.config.SYMBOL}: {error_msg}"
                )
                return None

        except Exception as e:
            self.logger.error(
                f"Exception fetching ticker for {self.config.SYMBOL}: {e}",
                exc_info=True,
            )
            return None
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        """Calculate Dual Supertrend and ATR using pandas_ta. Enhanced with robust column handling and NaN removal."""
        if df.empty:
        ...
        try:
            ...
            for col in required_cols:
                if col not in df.columns or not pd.api.types.is_numeric_dtype(df[col]):
                    self.logger.error(
                        f"Required column '{col}' is missing or not numeric in DataFrame. Cannot calculate indicators."
                    )
                    return df  # Return original df if essential columns are missing

            # Calculate ATR (used for Supertrend)
            df["atr_fast"] = ta.atr(
                df["high"], df["low"], df["close"], length=self.config.ST_FAST_PERIOD
            )
            df["atr_slow"] = ta.atr(
                df["high"], df["low"], df["close"], length=self.config.ST_SLOW_PERIOD
            )

            # Calculate Fast Supertrend
            st_fast_prefix = f"SUPERT_{self.config.ST_FAST_PERIOD}_{self.config.ST_FAST_MULTIPLIER}"
            st_fast_results = ta.supertrend(
                high=df["high"],
                low=df["low"],
                close=df["close"],
                length=self.config.ST_FAST_PERIOD,
                multiplier=self.config.ST_FAST_MULTIPLIER,
                fillna=0,
            )

            if st_fast_results is not None and not st_fast_results.empty:
                st_fast_col = f"{st_fast_prefix}"
                st_fast_dir_col = f"{st_fast_prefix}d"
                if st_fast_col in st_fast_results.columns and st_fast_dir_col in st_fast_results.columns:
                    df["st_fast"] = st_fast_results[st_fast_col]
                    df["dir_fast"] = st_fast_results[st_fast_dir_col]
                else:
                    self.logger.warning("Could not find expected fast Supertrend columns.")
                    df["st_fast"] = df["close"]
                    df["dir_fast"] = 0
            else:
                self.logger.warning("Fast Supertrend calculation returned empty DataFrame.")
                df["st_fast"] = df["close"]
                df["dir_fast"] = 0

            # Calculate Slow Supertrend
            st_slow_prefix = f"SUPERT_{self.config.ST_SLOW_PERIOD}_{self.config.ST_SLOW_MULTIPLIER}"
            st_slow_results = ta.supertrend(
                high=df["high"],
                low=df["low"],
                close=df["close"],
                length=self.config.ST_SLOW_PERIOD,
                multiplier=self.config.ST_SLOW_MULTIPLIER,
                fillna=0,
            )

            if st_slow_results is not None and not st_slow_results.empty:
                st_slow_col = f"{st_slow_prefix}"
                st_slow_dir_col = f"{st_slow_prefix}d"
                if st_slow_col in st_slow_results.columns and st_slow_dir_col in st_slow_results.columns:
                    df["st_slow"] = st_slow_results[st_slow_col]
                    df["dir_slow"] = st_slow_results[st_slow_dir_col]
                else:
                    self.logger.warning("Could not find expected slow Supertrend columns.")
                    df["st_slow"] = df["close"]
                    df["dir_slow"] = 0
            else:
                self.logger.warning("Slow Supertrend calculation returned empty DataFrame.")
                df["st_slow"] = df["close"]
                df["dir_slow"] = 0

            # Ensure all required columns exist before dropna, adding them with default if missing
            required_indicator_cols = ["st_fast", "dir_fast", "st_slow", "dir_slow", "atr_fast", "atr_slow"]
            ...
            # Ensure directions are integers
            df["dir_fast"] = df["dir_fast"].astype(int)
            df["dir_slow"] = df["dir_slow"].astype(int)

            self.logger.debug(
                f"Dual Supertrend and ATR indicators calculated. DataFrame shape: {df.shape}"
            )
            return df
    ...
    # =====================================================================
    # STRATEGY LOGIC (DUAL SUPERTRAND CROSS)
    # =====================================================================

    def dual_supertrend_cross_signal(self, df: pd.DataFrame) -> Signal:
        """
        Generate trading signal based on the cross of Fast and Slow Supertrend lines.
        Uses the last two data points to detect trend flips.
        """
        if df.empty or len(df) < 2:
            self.logger.warning(
                "Not enough data points to generate Dual ST Cross signal (need at least 2)."
            )
            return Signal.NEUTRAL

        try:
            # Get the last two rows for cross-comparison
            latest = df.iloc[-1]
            previous = df.iloc[-2]

            # Extract Supertrend values and directions
            fast_st_val = latest.get("st_fast", Decimal("0.0"))
            slow_st_val = latest.get("st_slow", Decimal("0.0"))
            prev_fast_st_val = previous.get("st_fast", Decimal("0.0"))
            prev_slow_st_val = previous.get("st_slow", Decimal("0.0"))

            fast_st_dir = latest.get("dir_fast", 0)
            slow_st_dir = latest.get("dir_slow", 0) # Slow ST direction can act as a filter


            # --- Signal Logic ---

            # LONG CROSS: Fast ST crosses ABOVE Slow ST
            # Condition: Fast ST was below or equal to Slow ST previously, and now is above.
            # We can also add a filter: The current fast ST direction should be bullish (1).
            if (fast_st_val > slow_st_val) and (prev_fast_st_val <= prev_slow_st_val):
                if fast_st_dir == 1: # Ensure direction is also bullish for a strong long signal
                    self.logger.debug(f"Detected LONG CROSS: Fast ST ({fast_st_val:.2f}) crossed above Slow ST ({slow_st_val:.2f}). Dir_Fast={fast_st_dir}.")
                    return Signal.STRONG_BUY # Use STRONG_BUY for the initial cross entry
                else:
                    self.logger.debug(f"LONG CROSS detected but Fast ST direction is not bullish ({fast_st_dir}). Neutralizing.")
                    return Signal.NEUTRAL


            # SHORT CROSS: Fast ST crosses BELOW Slow ST
            # Condition: Fast ST was above or equal to Slow ST previously, and now is below.
            # We can also add a filter: The current fast ST direction should be bearish (-1).
            elif (fast_st_val < slow_st_val) and (prev_fast_st_val >= prev_slow_st_val):
                if fast_st_dir == -1: # Ensure direction is also bearish for a strong short signal
                    self.logger.info(f"Detected SHORT CROSS: Fast ST ({fast_st_val:.2f}) crossed below Slow ST ({slow_st_val:.2f}). Dir_Fast={fast_st_dir}.")
                    return Signal.STRONG_SELL # Use STRONG_SELL for the initial cross entry
                else:
                    self.logger.debug(f"SHORT CROSS detected but Fast ST direction is not bearish ({fast_st_dir}). Neutralizing.")
                    return Signal.NEUTRAL

            # If no cross, check for continuation signals (optional, but can be useful)
            # For this specific Dual Cross strategy, we focus primarily on the cross event.
            # However, we can still provide a 'BUY' or 'SELL' signal if the fast ST is trending
            # in the desired direction and stays on the same side of the slow ST.
            # This can help with trailing or holding positions.

            # Continuation BUY: Fast ST is above Slow ST and Fast ST direction is bullish
            elif fast_st_val > slow_st_val and fast_st_dir == 1:
                 self.logger.debug(f"Continuation BUY signal: Fast ST ({fast_st_val:.2f}) > Slow ST ({slow_st_val:.2f}), Dir_Fast={fast_st_dir}.")
                 return Signal.BUY

            # Continuation SELL: Fast ST is below Slow ST and Fast ST direction is bearish
            elif fast_st_val < slow_st_val and fast_st_dir == -1:
                 self.logger.info(f"Continuation SELL signal: Fast ST ({fast_st_val:.2f}) < Slow ST ({slow_st_val:.2f}), Dir_Fast={fast_st_dir}.")
                 return Signal.SELL

            # Otherwise, neutral
            else:
                self.logger.debug(
                    f"Dual ST Cross: No clear signal. Fast ST={fast_st_val:.2f}, Slow ST={slow_st_val:.2f}, Dir Fast={fast_st_dir}"
                    )
                return Signal.NEUTRAL

        except Exception as e:
            self.logger.error(f"Error generating Dual ST Cross signal: {e}", exc_info=True)
            return Signal.NEUTRAL
    ...
    def calculate_position_size_usd(
        ...
        try:
            ...
            if quantity is not None and quantity <= 0:
                self.logger.warning(
                    f"Calculated position size for {self.config.SYMBOL} is zero or negative ({quantity})."
                )
                return None
            ...
            self.logger.error(
                f"Error calculating position size for {self.config.SYMBOL}: {e}",
                exc_info=True,
            )
            return None
    ...
    def check_daily_loss_limit(self) -> bool:
        ...
        if (
            self.start_balance_usdt <= 0
        ):  # If start balance is not set or invalid, cannot check limit
            self.logger.warning(
                "Start balance not initialized or zero. Daily loss limit check bypassed."
            )
            return True
        ...
        try:
            ...
            if (
                current_balance <= 0
            ):  # If current balance is zero, we've lost everything
                self.logger.warning(
                    "Current account balance is zero or negative. Assuming loss limit hit."
                )
                return False
            ...
            if loss_amount >= max_allowed_loss:
                self.logger.warning(
                    f"Daily loss limit reached. Current loss: {loss_amount:.2f} USDT (Limit: {max_allowed_loss:.2f} USDT). Halting trading operations."
                )
                return False
            else:
                self.logger.debug(
                    f"Daily loss check: Current Loss={loss_amount:.2f} USDT, Limit={max_allowed_loss:.2f} USDT. Within limits."
                )
                return True
    ...
    def place_order(
        ...
        try:
            ...
            rounded_qty = self.precision_manager.round_quantity(
                self.config.SYMBOL, qty
            )  # Use precision_manager
            ...
            params: dict[str, Any] = {
                ...
                else 0,  # Use configured positionIdx if hedge mode is enabled
            }
            ...
            if order_type == OrderType.LIMIT:
                if entry_price is None:
                    self.logger.error(
                        f"Limit order requires an entry_price. Aborting order for {self.config.SYMBOL}."
                    )
                    return None
                rounded_price = self.precision_manager.round_price(
                    self.config.SYMBOL, entry_price
                )  # Use precision_manager
                if rounded_price <= 0:
                    self.logger.warning(
                        f"Invalid entry price ({entry_price} rounded to {rounded_price}) for limit order in {self.config.SYMBOL}. Aborting order."
                    )
                    return None
            ...
            if stop_loss_price is not None:
                rounded_sl = self.precision_manager.round_price(
                    self.config.SYMBOL, stop_loss_price
                )  # Use precision_manager
                if rounded_sl > 0:  # Ensure SL is valid
                    params["stopLoss"] = str(rounded_sl)
                    params["slOrderType"] = (
                        "Market"  # Common practice: SL triggers as Market order
                    )

            # Add take profit if provided
            if take_profit_price is not None:
                rounded_tp = self.precision_manager.round_price(
                    self.config.SYMBOL, take_profit_price
                )  # Use precision_manager
                if rounded_tp > 0:  # Ensure TP is valid
                    params["takeProfit"] = str(rounded_tp)
                    params["tpOrderType"] = (
                        "Limit"  # Common practice: TP triggers as Limit order
                    )
            ...
            if response and response["retCode"] == 0:
                ...
                order_link_id = response["result"].get(
                    "orderLinkId", "N/A"
                )  # orderLinkId is optional but good to track
                self.logger.info(
                    f"Order placed successfully: {side} {rounded_qty} {self.config.SYMBOL} ({order_type.value}), OrderID: {order_id}, OrderLinkId: {order_link_id}"
                )
                ...
                self.logger.error(
                    f"Failed to place order for {self.config.SYMBOL}: {error_msg}"
                )
                return None

        except Exception as e:
            self.logger.error(
                f"Exception placing order for {self.config.SYMBOL}: {e}",
                exc_info=True,
            )
            return None
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            ...
            response = self.session.cancel_order(
                category=specs.category, symbol=self.config.SYMBOL, orderId=order_id
            )
            ...
            if response and response["retCode"] == 0:
                ...
                self.logger.error(
                    f"Failed to cancel order {order_id} for {self.config.SYMBOL}: {error_msg}"
                )
                return False
    ...
    def cancel_all_orders(self) -> bool:
        ...
        try:
            ...
            response = self.session.cancel_all_orders(
                category=specs.category, symbol=self.config.SYMBOL
            )
            ...
            if response and response["retCode"] == 0:
                ...
                self.logger.error(
                    f"Failed to cancel all orders for {self.config.SYMBOL}: {error_msg}"
                )
                return False
    ...
    def fetch_open_orders(self):
        ...
        try:
            ...
            if response and response["retCode"] == 0:
                ...
                self.logger.error(
                    f"Failed to fetch open orders for {self.config.SYMBOL}: {error_msg}"
                )
        except Exception as e:
            self.logger.error(
                    ...
                    if Decimal(pos.get("size", "0")) > 0:
                        ...
                        if pos["symbol"] == self.config.SYMBOL:
                            ...
                            if (
                                ...
                                and self.config.ORDER_TYPE_ENUM == OrderType.MARKET
                            ):
                                ...
                                if mark_price_str:
                                    ...
                                    if (
                                        self.config.SYMBOL
                                        not in self.trailing_stop_manager.active_trailing_stops
                                        or self.trailing_stop_manager.active_trailing_stops[
                                            self.config.SYMBOL
                                        ]["current_stop"]
                                        == 0
                                    ):
                                        self.logger.info(
                                            f"Initializing trailing stop for {self.config.SYMBOL} at entry {self.current_position_entry_price:.4f}"
                                        )
                                        self.trailing_stop_manager.initialize_trailing_stop(
                                            ...
                                            * 100,  # Pass as percentage
                                            activation_percent=self.config.TRAILING_STOP_PCT
                                            * 100,  # Simple activation for now (can be made configurable)
                                        )
                                else:
                                    self.logger.warning(
                                        f"Mark price not available for {self.config.SYMBOL} position, cannot initialize trailing stop."
                                    )
    ...
    def close_position(self) -> bool:
        ...
        if (
            ...
            or not self.current_position_side
            or self.current_position_size <= 0
        ):
            self.logger.warning(
                f"No active position or position size is zero to close for {self.config.SYMBOL}."
            )
            return False
        ...
        if not specs:
            self.logger.error(
                f"Cannot close position for {self.config.SYMBOL}: Specs not found."
            )
            return False
        ...
        if specs.category == "spot":
            self.logger.warning(
                f"Attempting to close position for spot symbol {self.config.SYMBOL}. Spot position management is not explicitly handled here. Consider manual closing."
            )
            return False
        ...
        try:
            ...
            self.logger.info(
                f"Attempting to close {self.config.SYMBOL} position ({self.current_position_side} {self.current_position_size} {specs.base_currency})."
            )
            ...
            if result:
                self.logger.info(
                    f"Market order to close {self.config.SYMBOL} position placed successfully."
                )
            ...
            self.logger.error(
                f"Exception closing position for {self.config.SYMBOL}: {e}",
                exc_info=True,
            )
            return False
    ...
    def update_stop_loss(self, stop_loss_price: Decimal) -> bool:
        ...
        if not self.position_active:
            self.logger.warning(
                f"No active position to update stop loss for {self.config.SYMBOL}."
            )
            return False
        ...
        if not specs:
            self.logger.error(
                f"Cannot update stop loss for {self.config.SYMBOL}: Specs not found."
            )
            return False
        ...
        if specs.category == "spot":
            self.logger.warning(
                "Stop loss updates via set_trading_stop not supported for spot category."
            )
            return False
        ...
        try:
            ...
            if rounded_sl <= 0:
                self.logger.warning(
                    f"Invalid stop loss price provided ({stop_loss_price} rounded to {rounded_sl}). Cannot update."
                )
                return False

            self.logger.debug(
                f"Attempting to update stop loss for {self.config.SYMBOL} to {rounded_sl}..."
            )
            response = self.session.set_trading_stop(
                category=specs.category,
                symbol=self.config.SYMBOL,
            ...
            if response and response["retCode"] == 0:
                self.logger.info(
                    f"Stop loss updated successfully on exchange for {self.config.SYMBOL} to {rounded_sl}"
                )
                ...
                self.logger.error(
                    f"Failed to update stop loss on exchange for {self.config.SYMBOL}: {error_msg}"
                )
                return False

        except Exception as e:
            self.logger.error(
                f"Exception updating stop loss on exchange for {self.config.SYMBOL}: {e}",
                exc_info=True,
            )
            return False
    ...
    def get_account_balance_usdt(self) -> Decimal:
        ...
        try:
            self.logger.debug(
                f"Fetching wallet balance for account type: {account_type}..."
            )
            ...
            if response and response["retCode"] == 0:
                ...
                self.logger.error(
                    f"Failed to get account balance ({account_type}): {error_msg}"
                )
                self.account_balance_usdt = Decimal(
                    "0.0"
                )  # Assume zero balance on error
                return Decimal("0.0")
    ...
    def set_leverage(self) -> bool:
        ...
        if self.config.CATEGORY_ENUM == Category.SPOT:
            self.logger.info(
                "Spot trading does not use leverage. Skipping leverage setting."
            )
            return True
        ...
        try:
            ...
            if response and response["retCode"] == 0:
                ...
                if (
                    "leverage is already set" in error_msg.lower()
                    or "same as the current" in error_msg.lower()
                ):
                    self.logger.info(
                        f"Leverage for {self.config.SYMBOL} is already set to {leverage_str}x. No change needed."
                    )
                    return True
                ...
                # Check for common error codes or messages associated with unsupported leverage.
                    ...
                    "leverage not modified" in error_msg.lower()
                    or "illegal category" in error_msg.lower()
                    or "110043" in str(response.get("retCode"))
                ):
                    self.logger.warning(
                        f"Could not set leverage for {self.config.SYMBOL} ({self.config.CATEGORY_ENUM.value}): Leverage may not be supported, applicable for this symbol/category, or already set correctly. Error: {error_msg}"
                    )
                    # Return True as the bot should not fail if leverage is not applicable, as long as it doesn't actively prevent trading.
                    return True
                else:
                    self.logger.error(
                        f"Failed to set leverage for {self.config.SYMBOL}: {error_msg}"
                    )
                    return False
            ...
            if (
                ...
                or "illegal category" in error_msg_lower
                or "110043" in error_msg_lower
            ):
                self.logger.warning(
                    f"Could not set leverage for {self.config.SYMBOL} ({self.config.CATEGORY_ENUM.value}): Leverage may not be supported, applicable for this symbol/category, or already set correctly. Error: {e}"
                )
                return True  # Return True to allow bot to continue
            else:
                self.logger.error(
                    f"Invalid request error setting leverage for {self.config.SYMBOL}: {e}. This might indicate the symbol or category does not support leverage adjustments.",
                    exc_info=True,
                )
    ...
    def execute_trade_based_on_signal(self, signal: Signal):
        ...
        if not self.check_daily_loss_limit():
            self.logger.warning(
                "Daily loss limit reached. Skipping trade execution for this cycle."
            )
            return
        ...
        if not ticker:
            self.logger.warning(
                "Could not retrieve ticker data. Cannot execute trade based on signal."
            )
            return
        ...
        try:
            ...
            if not current_price_str:
                self.logger.warning(
                    f"Ticker data for {self.config.SYMBOL} is missing 'lastPrice'. Cannot proceed."
                )
                return
            ...
            # If we are NOT in a position and receive a STRONG BUY signal (initial cross)
            if not self.position_active and signal == Signal.STRONG_BUY:
                self.logger.info(
                    f"Received STRONG BUY signal ({signal.name}). Attempting to open long position."
                )
                ...
                position_qty = self.calculate_position_size_usd(
                    entry_price=float(
                        current_price
                    ),  # Pass as float for calculator's internal usage, will be converted to Decimal
                ...
                if position_qty is not None and position_qty > 0:
                    ...
                    if order_result:
                        ...
                        self.position_active = (
                            True  # Mark as active, will be confirmed next loop
                        )
                        self.current_position_side = "Buy"
                        self.current_position_entry_price = current_price
                        self.current_position_size = position_qty
                        self.logger.info(
                            "BUY order placed successfully. Waiting for position confirmation."
                        )
                    else:
                        self.logger.error("Failed to place BUY order.")
                else:
                    self.logger.warning(
                        "Could not calculate a valid position size for the STRONG BUY signal. Skipping order placement."
                    )

            # If we are NOT in a position and receive a STRONG SELL signal (initial cross)
            elif not self.position_active and signal == Signal.STRONG_SELL:
                self.logger.info(
                    f"Received STRONG SELL signal ({signal.name}). Attempting to open short position."
                )
                ...
                if position_qty is not None and position_qty > 0:
                    ...
                    if order_result:
                        ...
                        self.current_position_size = position_qty
                        self.logger.info(
                            "SELL order placed successfully. Waiting for position confirmation."
                        )
                    else:
            ...
            # 2. Handle Position Continuation or Closing based on signals
                ...
                if self.current_position_side == "Buy" and signal in [
                    Signal.SELL,
                    Signal.STRONG_SELL,
                ]:
                    self.logger.info(
                        f"Signal reversal to {signal.name} detected while in BUY position. Closing position."
                    )
                    self.close_position()  # Attempt to close the current position

                elif self.current_position_side == "Sell" and signal in [
                    Signal.BUY,
                    Signal.STRONG_BUY,
                ]:
                    self.logger.info(
                ...
                if self.config.TRAILING_STOP_PCT > 0 and specs.category != "spot":
                    ...
                    pos_data = self.current_positions.get(
                        self.config.SYMBOL
            ...
            self.logger.error(
                f"An error occurred during trade execution logic for {self.config.SYMBOL}: {e}",
                exc_info=True,
            )
    ...
    def run_strategy(self):
        ...
        try:
            ...
            if not self.set_leverage():
                ...
                self.logger.critical(
                    "Failed to set initial leverage. Bot cannot proceed."
                )
                return
            ...
            if initial_balance <= 0:
                self.logger.critical(
                    "Failed to get initial account balance or balance is zero. Bot cannot proceed."
                )
                return
            self.start_balance_usdt = initial_balance
            # Calculate daily loss amount based on start balance and percentage
            self.daily_loss_amount = self.start_balance_usdt * (
                Decimal(str(self.config.MAX_DAILY_LOSS_PCT)) / Decimal("100")
            )
            self.logger.info(
                f"Starting Balance: {self.start_balance_usdt:.4f} USDT. Daily Loss Limit: {self.daily_loss_amount:.4f} USDT."
            )
            ...
            while not self._stop_requested:  # Loop until shutdown signal is received
                try:
                    ...
                    pnl_pct = (
                        (pnl / self.start_balance_usdt * 100)
                    ...
                    if not self.market_data.empty and len(self.market_data) >= 1:
                        ...
                        current_price_log = latest.get(
                            "close", Decimal("0.0")
                        )  # Use close price as current price for logging
                        st_fast_log = latest.get("st_fast", Decimal("0.0"))
                        dir_fast_log = latest.get("dir_fast", 0)
                        st_slow_log = latest.get("st_slow", Decimal("0.0"))
                        dir_slow_log = latest.get("dir_slow", 0)
                    ...
                    self.logger.info(
                        ...
                        f"ST_Fast={st_fast_log:<10.4f} (Dir:{dir_fast_log}) | "
                        f"ST_Slow={st_slow_log:<10.4f} (Dir:{dir_slow_log}) | "
                    ...
                    self.logger.info(
                        "Trading loop interrupted by user (KeyboardInterrupt). Shutting down."
                    )
                    ...
                    self.logger.info(
                        f"Waiting {self.config.LOOP_INTERVAL_SEC} seconds before retrying..."
                    )
                    time.sleep(self.config.LOOP_INTERVAL_SEC)

        except Exception as e:
            self.logger.critical(
                f"A critical error occurred during strategy execution setup or loop: {e}",
                exc_info=True,
            )
        finally:
    ...
    def cleanup(self):
        ...
        try:
            ...
            if (
                ...
                and self.current_position_size > 0
            ):
                self.logger.warning(
            ...
            total_pnl_pct = (
                (total_pnl / self.start_balance_usdt * 100)
                if self.start_balance_usdt > 0
                else Decimal("0")
            )
