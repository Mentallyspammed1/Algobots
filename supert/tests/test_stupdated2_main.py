...
class TestEhlersSuperTrendBotMain(unittest.TestCase):
    ...
    def setUp(
        ...
        MockBotUI,
        MockSMSNotifier,
        MockCandlestickDetector,
        MockNewsManager,
        MockTSManager,
        MockOrderSizer,
        MockPrecisionManager,
        MockBybitClient,
        MockSetupLogger,
    ):
        ...
        self.mock_precision_manager.get_decimal_places.return_value = (
            2,
            3,
        )  # (price, qty)

        # Mock methods called during bot initialization
        self.mock_bybit_client.get_server_time.return_value = {
            "retCode": 0,
            "result": {"timeNano": "1672531200000000000"},
        }
        self.mock_bybit_client.get_instruments_info.return_value = {
            "retCode": 0,
            "result": {"list": [{"symbol": "TRUMPUSDT"}]},
        }
        self.mock_bybit_client.get_wallet_balance.return_value = {
            "retCode": 0,
        ...
        with (
            patch.object(
                EhlersSuperTrendBot, "_validate_api_credentials", return_value=None
            ),
            patch.object(
                EhlersSuperTrendBot, "_validate_symbol_timeframe", return_value=None
            ),
            patch.object(
                EhlersSuperTrendBot, "_capture_initial_equity", return_value=None
            ),
            patch.object(
                EhlersSuperTrendBot, "_configure_trading_parameters", return_value=None
            ),
        ):
    ...
    def test_bot_initialization(self):
        ...
        self.mock_logger.info.assert_any_call(
            "Initializing Ehlers SuperTrend Trading Bot..."
        )
    ...
    def test_run_method_flow(self, mock_cleanup, mock_main_loop, mock_start_websockets):
        ...
        self.mock_logger.info.assert_any_call(
            "Starting Ehlers SuperTrend Bot main loop..."
        )
