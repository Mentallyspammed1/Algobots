...
def test_precision_manager_parse_instrument_specs_missing_filters(
    mock_bybit_session, mock_logger
):
    pm = PrecisionManager(mock_bybit_session, mock_logger)
...
def test_precision_manager_round_price(precision_manager_instance):
    ...
    assert pm.round_price("BTCUSDT", 45000.129) == Decimal(
        "45000.12"
    )  # Should round down
...
def test_precision_manager_round_quantity(precision_manager_instance):
    ...
    assert pm.round_quantity("BTCUSDT", 0.00199) == Decimal(
        "0.001"
    )  # Should round down
...
def test_calculate_position_size_fixed_risk_buy(
    order_sizer_instance, precision_manager_instance
):
...
def test_calculate_position_size_fixed_risk_sell(
    order_sizer_instance, precision_manager_instance
):
    osc = order_sizer_instance
...
def test_calculate_position_size_fixed_risk_zero_stop_distance(
    order_sizer_instance, mock_logger
):
    osc = order_sizer_instance
    ...
    mock_logger.warning.assert_called_with(
        f"Stop loss distance is zero for {symbol}. Cannot calculate size."
    )


def test_calculate_position_size_fixed_risk_below_min_qty(
    order_sizer_instance, precision_manager_instance, mock_logger
):
    osc = order_sizer_instance
    pm = precision_manager_instance
    ...
    symbol = "BTCUSDT"
    account_balance = (
        0.1  # Very small balance to ensure quantity is below min_order_qty
    )
    ...
    mock_logger.warning.assert_called_with(
        f"Calculated quantity {calculated_qty} for {symbol} is below minimum {pm.instruments[symbol].min_order_qty}."
    )
...
def supertrend_bot_instance(default_config, mock_bybit_session, mock_logger):
    ...
    with (
        patch.object(PrecisionManager, "load_all_instruments", return_value=None),
        patch("st.PrecisionManager", return_value=MagicMock(spec=PrecisionManager)),
        patch(
            "st.OrderSizingCalculator",
            return_value=MagicMock(spec=OrderSizingCalculator),
        ),
        patch(
            "st.TrailingStopManager", return_value=MagicMock(spec=TrailingStopManager)
        ),
    ):
