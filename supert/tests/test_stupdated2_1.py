...
class TestTemporalUtils(unittest.TestCase):
    def test_parse_to_utc(self):
        ...
        self.assertEqual(
            parse_to_utc("2025-08-25 10:00:00 EDT"), datetime(2025, 8, 25, 14, 0, 0)
        )
        self.assertEqual(
            parse_to_utc("2025-08-25 10:00:00 PST"), datetime(2025, 8, 25, 17, 0, 0)
        )  # PDT is active in August
        self.assertEqual(
            parse_to_utc("2025-01-01 10:00:00 PST"), datetime(2025, 1, 1, 18, 0, 0)
        )
        self.assertEqual(
            parse_to_utc("2025-08-25 10:00:00 GMT"), datetime(2025, 8, 25, 10, 0, 0)
        )
...
class TestPrecisionManager(unittest.TestCase):
    ...
    def test_round_price(self):
        ...
        self.assertEqual(
            self.precision_manager.round_price("BTCUSDT", 12345.678), Decimal("12345.6")
        )
        self.assertEqual(
            self.precision_manager.round_price("ETHUSDT", 3456.789), Decimal("3456.78")
        )
        # Test clamping
        self.assertEqual(
            self.precision_manager.round_price("BTCUSDT", 0.05), Decimal("0.1")
        )  # Below min
        self.assertEqual(
            self.precision_manager.round_price("BTCUSDT", 2000000), Decimal("1000000")
        )  # Above max

    def test_round_quantity(self):
        """Test quantity rounding based on step size."""
        self.assertEqual(
            self.precision_manager.round_quantity("BTCUSDT", 1.23456), Decimal("1.234")
        )
        self.assertEqual(
            self.precision_manager.round_quantity("ETHUSDT", 2.3456), Decimal("2.34")
        )
        # Test clamping
        self.assertEqual(
            self.precision_manager.round_quantity("BTCUSDT", 0.0005), Decimal("0.001")
        )  # Below min
        self.assertEqual(
            self.precision_manager.round_quantity("BTCUSDT", 101), Decimal("100")
        )  # Above max
...
class TestOrderSizingCalculator(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_precision_manager.round_quantity.side_effect = (
            lambda symbol, qty: Decimal(str(qty)).quantize(Decimal("0.001"))
        )
        self.config = Config()  # Use default config
        self.order_sizer = OrderSizingCalculator(
            self.mock_precision_manager, test_logger, self.config
        )
...
class TestEhlersSuperTrendBotCleanup(unittest.TestCase):
    ...
    def test_cleanup_with_autoclose_enabled_and_position_active(self):
        ...
        self.bot.close_position.assert_called_once_with(
            side="Buy", reason="Graceful shutdown with auto-close enabled."
        )

        # Verify final log messages and notifications
        self.mock_logger.info.assert_any_call(
            "Closing active position due to auto-close on shutdown setting..."
        )
        self.mock_subprocess_run.assert_called_with(
            [
                "termux-toast",
                f"Ehlers SuperTrend Bot for {self.bot.config.SYMBOL} has ceased operations.",
            ]
        )
        self.bot.sms_notifier.send_sms.assert_called_with(
            f"Ehlers SuperTrend Bot for {self.bot.config.SYMBOL} has ceased operations."
        )
