...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.CATEGORY_ENUM == Category.SPOT:
            if self.LEVERAGE != 1:
                print(
                    f"WARNING: Leverage set to 1 for SPOT category '{self.SYMBOL}' as it is not applicable. Original value was {self.LEVERAGE}."
                )
            self.LEVERAGE = 1
...
class PrecisionManager:
    ...
    def _parse_instrument_specs(self, inst: dict, category: str) -> InstrumentSpecs:
        ...
        max_price = safe_decimal(
            price_filter.get("maxPrice"), "1000000000"
        ...
        qty_step = safe_decimal(
            unified_lot_size_filter.get("qtyStep", lot_size_filter.get("qtyStep")),
            "0.00000001",
        )
        ...
        max_position_value_usd = safe_decimal(
            unified_lot_size_filter.get("maxOrderAmt", "1000000000")
        )
    ...
    def round_price(self, symbol: str, price: float | Decimal) -> Decimal:
        ...
        if not specs:
            self.logger.error(
                f"Cannot round price for {symbol}: Specs not found. Returning original price as Decimal."
            )
    ...
    def round_quantity(self, symbol: str, quantity: float | Decimal) -> Decimal:
        ...
        if not specs:
            self.logger.error(
                f"Cannot round quantity for {symbol}: Specs not found. Returning original quantity as Decimal."
            )
...
class OrderSizingCalculator:
    ...
    def calculate_position_size_usd(
        ...
        if risk_percent <= Decimal("0"):
            self.logger.warning(
                f"Risk percentage is zero or negative ({risk_percent}). Cannot calculate position size."
            )
            return None
        ...
        if entry_price > Decimal("0"):
            ...
            self.logger.warning(
                "Entry price is zero, cannot convert position value to quantity. Returning None."
            )
            return None
        ...
        self.logger.debug(
            f"Order Sizing for {symbol}: Entry={entry_price}, SL={stop_loss_price}, Risk%={risk_percent * 100:.4f}, Balance={account_balance_usdt:.4f} USDT"
        )
        self.logger.debug(
        ...
        if actual_risk_percent > risk_percent * Decimal("100") * Decimal(
            "1.01"
        ):  # Allow for slight rounding discrepancies
            self.logger.warning(
                f"Calculated actual risk ({actual_risk_percent:.4f}%) slightly exceeds allowed risk ({risk_percent * 100:.4f}%). Review parameters."
            )
        ...
        if symbol in self.active_trailing_stops:
            try:
                ...
                if specs and specs.category != Category.SPOT.value:
                    ...
                    self.logger.info(
                        f"Attempted to clear trailing stop on Bybit for {symbol}."
                    )
            except Exception as e:
                self.logger.warning(
                    f"Failed to clear trailing stop on Bybit for {symbol}: {e}"
                )
...
class TermuxSMSNotifier:
    ...
    def send_sms(self, message: str):
        ...
        try:
            ...
            self.logger.info(
                Fore.GREEN
                + f"SMS sent to {self.recipient_number}: {message[:70]}..."
                + Style.RESET_ALL
            )
            ...
            self.logger.error(
                Fore.RED
                + f"Termux SMS command failed with error: {e.stderr.decode().strip()}"
                + Style.RESET_ALL
            )
        except Exception as e:
            self.logger.error(
    ...
    def send_pnl_update(self, pnl: float, balance: float):
        emoji = (
            "✅" if pnl >= 0 else "❌"
        )  # Use >= 0 for green for break-even or profit
...
def setup_logger(config: Config) -> logging.Logger:
    ...
    if logger.handlers:
        for handler in logger.handlers[
            :
        ]:  # Iterate over a slice to allow modification during iteration
...
class BotUI(threading.Thread):
    ...
    def _render_ui(self):
        ...
        with self.bot_state.lock:
            ...
            print(
                f"{Fore.BLUE}║ Current Price:          {Fore.YELLOW}{current_price_str}{Fore.BLUE:<{73 - len('Current Price:          ') - len(current_price_str)}}║{Style.RESET_ALL}"
            )

            # Bid Price string, with dynamic precision
            bid_price_str = f"${state.bid_price:.{state.price_precision}f}"
            print(
                f"{Fore.BLUE}║ Bid:                    {Fore.YELLOW}{bid_price_str}{Fore.BLUE:<{73 - len('Bid:                    ') - len(bid_price_str)}}║{Style.RESET_ALL}"
            )

            # Ask Price string, with dynamic precision
            ask_price_str = f"${state.ask_price:.{state.price_precision}f}"
            print(
                f"{Fore.BLUE}║ Ask:                    {Fore.YELLOW}{ask_price_str}{Fore.BLUE:<{73 - len('Ask:                    ') - len(ask_price_str)}}║{Style.RESET_ALL}"
            )
            print(
                f"{Fore.BLUE}╚═══════════════════════════════════════════════════════════════════════════╝{Style.RESET_ALL}\n"
            )
            ...
            print(
                f"{Fore.MAGENTA}║ Ehlers SuperTrend:      {ehlers_color}{ehlers_st_display_str}{Fore.MAGENTA:<{73 - len('Ehlers SuperTrend:      ') - ehlers_st_display_len_no_color}}{Fore.CYAN}║{Style.RESET_ALL}"
            )
            ...
            print(
                f"{Fore.MAGENTA}║ ADX:                    {adx_color}{adx_str}{Fore.MAGENTA:<{73 - len('ADX:                    ') - adx_str_len_no_color}}{Fore.CYAN}║{Style.RESET_ALL}"
            )
            print(
                f"{Fore.MAGENTA}║ ADX +DI:                {Fore.WHITE}{state.adx_plus_di:.1f}{Fore.MAGENTA:<{73 - len('ADX +DI:                ') - len(f'{state.adx_plus_di:.1f}')}}║{Style.RESET_ALL}"
            )
            print(
                f"{Fore.MAGENTA}║ ADX -DI:                {Fore.WHITE}{state.adx_minus_di:.1f}{Fore.MAGENTA:<{73 - len('ADX -DI:                ') - len(f'{state.adx_minus_di:.1f}')}}║{Style.RESET_ALL}"
            )

            rsi_str = f"{state.rsi_value:.1f} (State: {state.rsi_state})"
            rsi_str_len_no_color = len(rsi_str)
            print(
                f"{Fore.MAGENTA}║ RSI:                    {rsi_color}{rsi_str}{Fore.MAGENTA:<{73 - len('RSI:                    ') - rsi_str_len_no_color}}{Fore.CYAN}║{Style.RESET_ALL}"
            )
            ...
            print(
                f"{Fore.GREEN}║ Current Equity:         {equity_color}{current_equity_display_str}{Fore.GREEN:<{73 - len('Current Equity:         ') - current_equity_display_len_no_color}}{Fore.CYAN}║{Style.RESET_ALL}"
            )
            print(
            ...
            if state.open_position_qty > Decimal("0"):
                ...
                print(
                    f"{Fore.GREEN}║ Unrealized PNL:         {pnl_color_unrealized}{unrealized_pnl_str}{Fore.GREEN:<{73 - len('Unrealized PNL:         ') - unrealized_pnl_len_no_color}}{Fore.CYAN}║{Style.RESET_ALL}"
                )
                ...
                print(
                    f"{Fore.GREEN}║ Stop Loss:              {Fore.WHITE}${'0.0':.{state.price_precision}f} (N/A){Fore.GREEN:<{73 - len('Stop Loss:              ') - len(f'${"0.0":.{state.price_precision}f} (N/A)')}}║{Style.RESET_ALL}"
                )
                print(
                    f"{Fore.GREEN}║ Take Profit:            {Fore.WHITE}${'0.0':.{state.price_precision}f} (N/A){Fore.GREEN:<{73 - len('Take Profit:            ') - len(f'${"0.0":.{state.price_precision}f} (N/A)')}}║{Style.RESET_ALL}"
                )
                ...
                print(
                    f"{Fore.GREEN}║ Open Position:          {Fore.WHITE}{Decimal('0.0'):.{state.qty_precision}f} {state.symbol}{Fore.GREEN:<{73 - len('Open Position:          ') - len(f'{Decimal("0.0"):.{state.qty_precision}f} {state.symbol}')}}║{Style.RESET_ALL}"
                )
                print(
                    f"{Fore.GREEN}║ Avg Entry Price:        {Fore.WHITE}${Decimal('0.0'):.{state.price_precision}f}{Fore.GREEN:<{73 - len('Avg Entry Price:        ') - len(f'${Decimal("0.0"):.{state.price_precision}f}')}}║{Style.RESET_ALL}"
                )
                print(
            ...
            print(
                f"{Fore.GREEN}║ Realized PNL (Total):   {pnl_color_realized}{realized_pnl_str}{Fore.GREEN:<{73 - len('Realized PNL (Total):   ') - realized_pnl_len_no_color}}{Fore.CYAN}║{Style.RESET_ALL}"
            )
            print(
                f"{Fore.GREEN}╚═══════════════════════════════════════════════════════════════════════════╝{Style.RESET_ALL}"
            )
...
class EhlersSuperTrendBot:
    def __init__(self, config: Config):
        ...
        self.cumulative_pnl: Decimal = Decimal(
            "0.0"
        )  # Total realized PnL for cumulative loss guard
        ...
        for attempt in range(1, self.config.MAX_API_RETRIES + 1):
            try:
                ...
                if isinstance(e, (FailedRequestError, InvalidRequestError)):
                    ...
                    error_code_match = next(
                        (code for code in no_retry_codes if str(code) in str(e)), None
                    )
                    if error_code_match:
                        self.logger.debug(
                            f"Received non-retriable error code {error_code_match} from pybit exception. Re-raising exception."
                        )
    ...
    def _cumulative_loss_guard(self) -> bool:
        ...
        if self.initial_equity <= Decimal("0") or current_equity <= Decimal(
            ...
            if (
                self.cumulative_pnl
                < -Decimal(str(self.config.MAX_DAILY_LOSS_PCT)) * self.initial_equity
            ):  # Assuming initial_equity can be 1 for a 'percentage' based PnL in case of 0 initial equity
                self.logger.critical(
                    Fore.RED
    ...
    def fetch_klines(self, limit: int | None = None) -> pd.DataFrame:
        ...
        try:
            ...
            if fetch_limit < min_required_candles:
                self.logger.warning(
                    f"Requested lookback {fetch_limit} is less than required for indicators ({min_required_candles}). Adjusting to {min_required_candles}."
                )
                fetch_limit = min_required_candles

            self.logger.debug(
                f"Fetching {fetch_limit} klines for {self.config.SYMBOL} ({self.config.TIMEFRAME})..."
            )
            ...
            if response_data is not None and response_data.get("list"):
                ...
                if not klines:
                    self.logger.warning(
                        f"No kline data returned for {self.config.SYMBOL}."
                    )
                    return pd.DataFrame()
                ...
                self.logger.debug(
                    f"Successfully fetched and processed {len(df)} klines for {self.config.SYMBOL}."
                )
                return df
            else:
                self.logger.error(
                    f"Failed to fetch klines for {self.config.SYMBOL}: API call wrapper returned None or empty list."
                )
                return pd.DataFrame()

        except Exception as e:
            self.logger.error(
                f"Exception fetching klines for {self.config.SYMBOL}: {e}",
                exc_info=True,
            )
            return pd.DataFrame()
    ...
    def _configure_trading_parameters(self) -> bool:
        ...
        try:
            ...
            self.api_call(
                ...
                no_retry_codes=[
                    100028
                ],  # Error code for "Unified account is forbidden to set margin mode"
            )
            self.logger.info(
            ...
            if "100028" in str(e) or "unified account is forbidden" in str(e):
                self.logger.warning(
                    Fore.YELLOW
                    + "Could not switch margin mode via API (Error 100028). This is common for UTA accounts."
                )
                self.logger.warning(
                    Fore.YELLOW
                    + f"Please ensure your account is set to your desired margin mode ({margin_mode_str}) for the {self.config.CATEGORY} category in your Bybit settings."
                    + Style.RESET_ALL
                )
                self.logger.info(
                    "The bot will continue assuming the margin mode is already correctly configured."
                )
            else:
                self.logger.warning(
        ...
        try:
            ...
            if leverage_to_set_decimal < min_lev:
                self.logger.warning(
                    f"Requested leverage {leverage_to_set_decimal} for {self.config.SYMBOL} is below minimum {min_lev}. Setting to minimum."
                )
                leverage_to_set_decimal = min_lev
            elif leverage_to_set_decimal > max_lev:
                self.logger.warning(
                    f"Requested leverage {leverage_to_set_decimal} for {self.config.SYMBOL} exceeds maximum {max_lev}. Setting to maximum."
                )
            ...
            if response_data is not None:
                ...
                self.logger.error(
                    f"Failed to set leverage for {self.config.SYMBOL}: API call wrapper returned None."
                )
                return False

        except Exception as e:
            self.logger.error(
                f"Exception setting leverage for {self.config.SYMBOL}: {e}",
                exc_info=True,
            )
            return False
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        min_len = max(
            self.config.EHLERS_LENGTH
            + self.config.SMOOTHING_LENGTH
            + 5,  # Ehlers adaptive trend
            ...
            60,
        )  # A reasonable minimum for general TA
        ...
        volatility = (
            vol_series.rolling(
                self.config.EHLERS_LENGTH,
                min_periods=max(1, self.config.EHLERS_LENGTH // 2),
            )
            .std()
        ...
        if not df.empty:
            ...
            if len(df) >= 1:
                ...
                supertrend_line.iloc[0] = final_lower.iloc[
                    0
                ]  # Default to lower for first bar
            ...
            for i in range(1, len(df)):
                ...
                if prev_supertrend_direction == 1:  # Previous was bullish
                    if (
                        df["close"].iloc[i] <= final_upper.iloc[i]
                    ):  # Price crosses or stays below upper band
                    ...
                    if (
                        df["close"].iloc[i] >= final_lower.iloc[i]
                    ):  # Price crosses or stays above lower band
                        supertrend_line.iloc[i] = final_lower.iloc[i]
        ...
        rsi = ta.momentum.RSIIndicator(
            df["close"], window=self.config.RSI_WINDOW, fillna=True
        )
        df["rsi"] = rsi.rsi()
        ...
        if df.empty:
            self.logger.warning(
                "All rows dropped due to NaN indicators after calculation. Cannot proceed."
            )
            return pd.DataFrame()
    ...
    def generate_signal(self, df: pd.DataFrame) -> tuple[str | None, str]:
        ...
        if any(pd.isna(latest[col]) for col in check_cols) or any(
            pd.isna(second_latest[col]) for col in check_cols
        ):
            self.logger.warning(
                f"NaN values found in latest indicator data. Cannot generate signal. Latest: {latest[check_cols].to_dict()}"
            )
            return None, "Latest indicator values are NaN, cannot generate signal."

        # Individual conditions
        ehlers_flip_up = (
            latest["ehlers_trend"] == 1 and second_latest["ehlers_trend"] == -1
        )
        ehlers_flip_down = (
            latest["ehlers_trend"] == -1 and second_latest["ehlers_trend"] == 1
        )
        ...
        if self.config.SIGNAL_CONFIRM_BARS > 1:
            recent_supertrend_directions = df["supertrend_direction"].iloc[
                -self.config.SIGNAL_CONFIRM_BARS :
            ]
            if ehlers_flip_up:
                confirmed_supertrend_bullish = (
                    supertrend_is_bullish and (recent_supertrend_directions == 1).all()
                )
            if ehlers_flip_down:
                confirmed_supertrend_bearish = (
                    supertrend_is_bearish and (recent_supertrend_directions == -1).all()
                )

        # --- Build reason string based on conditions ---
        buy_conditions_met = []
        if ehlers_flip_up:
            buy_conditions_met.append(
                f"Ehlers Trend flipped UP (Current: {int(latest['ehlers_trend'])})"
            )
        if confirmed_supertrend_bullish:
            buy_conditions_met.append(
                f"Supertrend is BULLISH (Direction: {int(latest['supertrend_direction'])})"
            )
        if rsi_over_50:
            buy_conditions_met.append(f"RSI ({latest['rsi']:.1f}) > 50")
        if macd_cross_up:
            buy_conditions_met.append(
                f"MACD Diff ({latest['macd_diff']:.4f}) crossed UP"
            )
        if adx_strong_trend and adx_bullish:
            buy_conditions_met.append(
                f"ADX ({latest['adx']:.1f}) shows STRONG BULLISH trend"
            )
        elif adx_strong_trend:
            buy_conditions_met.append(f"ADX ({latest['adx']:.1f}) shows STRONG trend")
        elif adx_bullish:
            buy_conditions_met.append("ADX +DI > -DI (Bullish bias)")

        sell_conditions_met = []
        if ehlers_flip_down:
            sell_conditions_met.append(
                f"Ehlers Trend flipped DOWN (Current: {int(latest['ehlers_trend'])})"
            )
        if confirmed_supertrend_bearish:
            sell_conditions_met.append(
                f"Supertrend is BEARISH (Direction: {int(latest['supertrend_direction'])})"
            )
        if rsi_under_50:
            sell_conditions_met.append(f"RSI ({latest['rsi']:.1f}) < 50")
        if macd_cross_down:
            sell_conditions_met.append(
                f"MACD Diff ({latest['macd_diff']:.4f}) crossed DOWN"
            )
        if adx_strong_trend and adx_bearish:
            sell_conditions_met.append(
                f"ADX ({latest['adx']:.1f}) shows STRONG BEARISH trend"
            )
        elif adx_strong_trend:
            sell_conditions_met.append(f"ADX ({latest['adx']:.1f}) shows STRONG trend")
        elif adx_bearish:
            sell_conditions_met.append("ADX -DI > +DI (Bearish bias)")

        # Combined signal with all filters: The true incantation
        if (
            ehlers_flip_up
            and confirmed_supertrend_bullish
            and rsi_over_50
            and macd_cross_up
            and adx_strong_trend
            and adx_bullish
        ):
            ...
            ehlers_flip_down
            and confirmed_supertrend_bearish
            and rsi_under_50
            and macd_cross_down
            and adx_strong_trend
            and adx_bearish
        ...
        return (
            None,
            "No clear signal, market is calm.",
        )  # No clear signal, the ether is calm
    ...
    def calculate_trade_sl_tp(
        self, side: str, entry_price: Decimal
    ...
    def place_order(
        ...
        if self.config.DRY_RUN:
            ...
            simulated_entry_price = entry_price or Decimal(
                str(self.market_data["close"].iloc[-1])
            )
            return {
                "orderId": "DRY_RUN_ORDER_ID_" + str(int(time.time())),
                "orderStatus": "Filled",
                "avgPrice": str(simulated_entry_price),
                "cumExecQty": str(rounded_qty),
            }

        try:
            # Set tpslMode to 'Full' if either SL or TP is provided
            tpsl_mode = (
                "Full" if (stopLoss is not None or takeProfit is not None) else None
            )
            ...
            if order_response and order_response.get("orderId"):
                ...
                self.logger.info(
                    Fore.GREEN
                    + f"Order submitted with ID: {order_id}. Verifying execution..."
                    + Style.RESET_ALL
                )
                ...
                for i in range(max_retries):
                    if self.stop_event.is_set():
                        self.logger.warning(
                            f"Shutdown requested during order fill verification for {order_id}. Aborting."
                        )
                        return None
                        ...
                        if order_status == "Filled":
                            ...
                            filled_price = (
                                ...
                                and Decimal(avg_price_str) > Decimal("0")
                                    ...
                                    entry_price
                                    or Decimal(str(self.market_data["close"].iloc[-1]))
                                )
                            )  # Fallback
                            ...
                            self.logger.debug(
                                f"Order {order_id} partially filled, status: {order_status}. Waiting for full fill..."
                            )
    ...
    def get_positions(self) -> dict[str, Any] | None:
        ...
        if specs.category == Category.SPOT.value:
            self.logger.debug(
                "Position status reset for spot category (no open derivatives positions)."
            )
        ...
        try:
            ...
            if response_data is not None:
                    ...
                    if Decimal(pos.get("size", "0")) > Decimal("0"):
                        ...
                        if pos["symbol"] == self.config.SYMBOL:
                            ...
                            with self.bot_state.lock:
                                ...
                                pos_value = Decimal(pos["size"]) * Decimal(
                                    pos.get("markPrice", "0")
                                )
                ...
                self.logger.debug(
                    f"Position status update: Active={self.position_active}, Side={self.current_position_side}, Size={self.current_position_size}, Entry={self.current_position_entry_price}"
                )
                return self.current_positions.get(
                    self.config.SYMBOL
                )  # Return the specific position data
    ...
    def close_position(self) -> bool:
        ...
        if self.config.DRY_RUN:
            ...
            self.logger.info(
                Fore.YELLOW
                + f"[DRY RUN] Would close {current_pos['side']} position of {qty_to_close:.{self.bot_state.qty_precision}f} {self.config.SYMBOL}. Simulated PnL: ${pnl_realized:.2f}"
                + Style.RESET_ALL
            )
            self.sms_notifier.send_sms(
                f"DRY RUN: Close {current_pos['side']} {self.config.SYMBOL}. Simulated PnL: ${pnl_realized:.2f}"
            )
        ...
        try:
            self.logger.info(
                f"Attempting to close {current_pos['side']} position of {qty_to_close} {self.config.SYMBOL}..."
            )
            ...
            if close_order_data:  # place_order returns filled order data on success
                ...
                self.logger.info(
                    Fore.MAGENTA
                    + f"✅ Position Closed: {side_to_close} {qty_to_close:.{self.bot_state.qty_precision}f} {self.config.SYMBOL} | PnL: ${pnl_realized:.{self.bot_state.price_precision}f}"
                    + Style.RESET_ALL
                )
                subprocess.run(
                    [
    ...
    def execute_trade_based_on_signal(self, signal_type: str | None, reason: str):
        ...
        if not self.position_active and signal_type in ["BUY", "SELL"]:
            ...
            stop_loss_price, take_profit_price = self.calculate_trade_sl_tp(
                trade_side, current_market_price
            )
            ...
            position_qty = self.order_sizer.calculate_position_size_usd(
                ...
                risk_percent=Decimal(
                    str(self.config.RISK_PER_TRADE_PCT / 100)
        ...
        # 2. Handle Managing Existing Positions (Signal Reversal)
            ...
            if (
                not perform_close
                and self.config.TRAILING_STOP_PCT > 0
                and specs.category != Category.SPOT.value
            ):
    ...
    def _update_market_data_and_state(self):
        ...
        if self.market_data.empty:
            self.logger.warning(
                "Indicators could not be calculated. State not updated."
            )
            return
        ...
        with self.bot_state.lock:
            ...
            if self.bot_state.open_position_qty > Decimal(
                ...
                if pos_size > Decimal("0") and entry_price > Decimal("0"):
                    ...
                    )  # Use entry price for position value calculation for PnL %
                    if pos_value > Decimal("0"):
                        self.bot_state.unrealized_pnl_pct = (
                            self.bot_state.unrealized_pnl / pos_value
                        ) * Decimal("100")
    ...
    def cleanup(self):
        ...
        try:
            ...
            if (
                ...
                if self.position_active:
                    self.logger.warning(
                        f"Bot shutting down with open position ({self.current_position_side} {self.current_position_size} {self.config.SYMBOL}). Manual intervention may be required."
                    )
            ...
            total_pnl_pct = (
                (total_pnl / self.initial_equity * Decimal("100"))
            ...
            self.logger.info("=" * 60)
            subprocess.run(
                ["termux-toast", "Ehlers Bot: Shut down. Check logs for summary."]
            )

        except Exception as e:
            self.logger.error(
                f"An error occurred during the cleanup process: {e}", exc_info=True
            )
...
if __name__ == "__main__":
    print(
        Fore.LIGHTYELLOW_EX
        + Style.BRIGHT
        + "\nPyrmethus, the Termux Coding Wizard, at your service! Let the arcane trading begin!"
        + Style.RESET_ALL
    )
    ...
    print(
        Fore.CYAN
        + "\n# Demonstrating the temporal conversion incantation:"
        + Style.RESET_ALL
    )
    example_local_times = [
    ...
    print(
        Fore.MAGENTA
        + "\n# May your digital journey be ever enlightened."
        + Style.RESET_ALL
    )
