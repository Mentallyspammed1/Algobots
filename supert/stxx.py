...
RSI_CONFIRMATION_LEVEL = 50  # Suggestion 2: RSI > 50 for long, < 50 for short
...
TRAILING_STOP_ACTIVATION_BARS = 10  # Suggestion 13: Activate trailing stop
TRAILING_STOP_ATR_MULTIPLIER = 0.5  # For trailing stop
...
# WARNING: If the bot restarts, it will lose track of the order_id for open
# positions and thus will not be able to dynamically amend SL/TP for those
# positions. For a production bot, this state should be persisted.
current_trade_state = {
    'position': None,  # Stores details of the open position
    ...
    'partial_profit_taken': False,  # Flag for partial profit
    'order_id': None,  # The orderId of the initial market order
}
...
def log_message(message, level="INFO"):
    ...
    print(log_entry)
    with Path("supertrendbot.log").open("a") as f:
        f.write(log_entry + "\n")
...
def _calculate_supertrend_manual(df, multiplier):
    """
    Calculates Supertrend indicator manually.

    Returns two series: supertrend_line and supertrend_direction (1 for uptrend, 0 for downtrend).
    """
    high = df['high']
    low = df['low']
    close = df['close']
    atr = df['atr']  # Assumes ATR is pre-calculated
    ...
    for i in range(1, len(df.index)):
        ...
        if close.iloc[current] > upper_band.iloc[previous]:
            in_uptrend.iloc[current] = True
        elif close.iloc[current] < lower_band.iloc[previous]:
    ...
    direction = in_uptrend.astype(int)
...
def calculate_indicators(df):
    ...
    # ATR must be calculated first for Supertrend
    ...
    df['supertrend_line'], df['supertrend_direction'] = _calculate_supertrend_manual(
        df, multiplier=SUPER_TREND_MULTIPLIER
    )
...
def run_bot():
    ...
    while True:
        ...
        required_1m_data = max(EMA_LONG_PERIOD, VOLUME_MA_PERIOD, RSI_PERIOD, ATR_PERIOD) + 2
        ...
        if exchange_position and not current_trade_state['position']:
            ...
            current_trade_state['partial_profit_taken'] = False  # Assume not taken
            current_trade_state['order_id'] = None  # Cannot get order_id from get_positions
            log_message("WARNING: For untracked positions, dynamic SL/TP adjustments will NOT work.", "WARNING")
            # We cannot set entry_bar_time accurately here.
            ...
            # Update internal position with latest from exchange
            current_trade_state['position'] = exchange_position
        ...
        if current_trade_state['position']:
            ...
            if pd.isna(current_atr) or current_atr == 0:
                log_message("ATR is not available or zero. Skipping ATR-based dynamic SL/TP.", "WARNING")
                current_atr = 0.00000001  # Prevent division by zero
            ...
            if current_trade_state['order_id']:
                ...
                if not current_trade_state['partial_profit_taken']:
                    profit_target_price = (
                        ...
                        if pos['side'] == "Buy"
                        else current_trade_state['entry_price'] - (current_atr * PARTIAL_PROFIT_ATR_MULTIPLIER)
                    )
                    ...
                    if (pos['side'] == "Buy" and current_price >= profit_target_price) or (
                        ...
                        # Ensure qty_to_close is not zero or too small
                        if qty_to_close >= 0.001:
                            log_message(
                                f"Taking partial profit ({PARTIAL_PROFIT_PERCENT * 100}%) on {pos['side']} position.",
                                "INFO",
                            )
                            close_position(SYMBOL, pos['side'], qty_to_close)
                            current_trade_state['partial_profit_taken'] = True

                            # After partial profit, move SL to break-even
                            new_sl_after_partial = current_trade_state['entry_price']

                            # Only amend if the new SL is better (more protective)
                            if (pos['side'] == "Buy" and (
                                pos['stop_loss'] is None or new_sl_after_partial > pos['stop_loss']
                            )) or (
                                pos['side'] == "Sell"
                                and (pos['stop_loss'] is None or new_sl_after_partial < pos['stop_loss'])
                            ):
                                log_message(
                                    f"Amending SL to {new_sl_after_partial:.2f} after partial profit.", "INFO"
                                )
                                amend_stop_loss_take_profit(
                                    SYMBOL, current_trade_state['order_id'], stop_loss=new_sl_after_partial
                                )
                                time.sleep(5)
                        else:
                            log_message(
                                f"Calculated partial profit quantity ({qty_to_close:.3f}) is too small.",
                                "WARNING",
                            )

                # 3. Adaptive Stop Loss (Break-Even) (Suggestion 11)
                # Only move SL to BE if it hasn't been moved yet and partial profit not taken
                if (
                    current_trade_state['entry_price']
                    and current_trade_state['initial_sl']
                    and not current_trade_state['partial_profit_taken']
                    and (pos['stop_loss'] == current_trade_state['initial_sl'] or pos['stop_loss'] is None)
                ):
                    profit_in_atr = (
                        (current_price - current_trade_state['entry_price']) / current_atr
                        if pos['side'] == "Buy"
                        else (current_trade_state['entry_price'] - current_price) / current_atr
                    )

                    if profit_in_atr >= BREAK_EVEN_PROFIT_ATR:
                        log_message(f"Profit of {profit_in_atr:.2f} ATR reached. Moving SL to break-even.", "INFO")
                        new_sl_breakeven = current_trade_state['entry_price']

                        # Only amend if the new SL is better
                        if (pos['side'] == "Buy" and (
                            pos['stop_loss'] is None or new_sl_breakeven > pos['stop_loss']
                        )) or (
                            pos['side'] == "Sell"
                            and (pos['stop_loss'] is None or new_sl_breakeven < pos['stop_loss'])
                        ):
                            log_message(f"Amending SL to {new_sl_breakeven:.2f} for break-even.", "INFO")
                            amend_stop_loss_take_profit(
                                SYMBOL, current_trade_state['order_id'], stop_loss=new_sl_breakeven
                            )
                            time.sleep(5)
                ...
                if current_trade_state['partial_profit_taken'] or (
                    ...
                    and (last_1m_candle.name - current_trade_state['entry_bar_time']) / (60 * 1000)
                    ...
                    # Only amend if the new trailing SL is better
                    if (pos['side'] == "Buy" and (
                        pos['stop_loss'] is None or new_trailing_sl > pos['stop_loss']
                    )) or (
                        pos['side'] == "Sell"
                        and (pos['stop_loss'] is None or new_trailing_sl < pos['stop_loss'])
                    ):
                        log_message(f"Amending SL to {new_trailing_sl:.2f} due to trailing stop.", "INFO")
                        amend_stop_loss_take_profit(
                            SYMBOL, current_trade_state['order_id'], stop_loss=new_trailing_sl
                        )
                        time.sleep(5)
            else:
                log_message("WARNING: order_id is missing. Dynamic SL/TP adjustments are disabled.", "WARNING")
            ...
            is_rsi_bullish = (
                last_1m_candle['rsi'] > RSI_CONFIRMATION_LEVEL and last_1m_candle['rsi'] > prev_1m_candle['rsi']
            )
            is_rsi_bearish = (
                last_1m_candle['rsi'] < RSI_CONFIRMATION_LEVEL and last_1m_candle['rsi'] < prev_1m_candle['rsi']
            )
            ...
            if side:
                ...
                if price_diff == 0:
                    log_message("Stop loss price is too close to entry price. Cannot place trade.", "ERROR")
                    time.sleep(60)
                    continue
                ...
                order_id = place_order(
                    SYMBOL, side, qty, stop_loss=stop_loss_price, take_profit=take_profit_price
                )
