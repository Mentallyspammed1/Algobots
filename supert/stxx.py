...
def _calculate_supertrend_manual(df, multiplier):
    ...
    for i in range(1, len(df.index)):
        ...
        if close.iloc[current] > upper_band.iloc[previous]:
            ...
            if (
                in_uptrend.iloc[current]
                and lower_band.iloc[current] < lower_band.iloc[previous]
            ):
                lower_band.iloc[current] = lower_band.iloc[previous]

            if (
                not in_uptrend.iloc[current]
                and upper_band.iloc[current] > upper_band.iloc[previous]
            ):
...
def get_current_price(symbol):
    ...
    try:
        ...
        if response and response["retCode"] == 0:
            ...
            log_message(
                f"Error getting current price for {symbol}: {response}", "ERROR"
            )
            return None
    except Exception as e:
...
def amend_stop_loss_take_profit(symbol, order_id, stop_loss=None, take_profit=None):
    ...
    try:
        ...
        if response and response["retCode"] == 0:
            ...
            log_message(
                f"Error amending SL/TP for order {order_id}: {response}", "ERROR"
            )
            return False
    except Exception as e:
    ...
    while True:
        ...
        if exchange_position and not current_trade_state["position"]:
            ...
            current_trade_state["order_id"] = (
                None  # Cannot get order_id from get_positions
            )
            log_message(
                "WARNING: For untracked positions, dynamic SL/TP adjustments will NOT work.",
                "WARNING",
            )
        ...
        if current_trade_state["position"]:
            ...
            if pd.isna(current_atr) or current_atr == 0:
                log_message(
                    "ATR is not available or zero. Skipping ATR-based dynamic SL/TP.",
                    "WARNING",
                )
            ...
            if current_trade_state["order_id"]:
                    ...
                    if (
                        ...
                        if qty_to_close >= 0.001:
                            ...
                            if (
                                ...
                                log_message(
                                    f"Amending SL to {new_sl_after_partial:.2f} after partial profit.",
                                    "INFO",
                                )
                ...
                if (
                    ...
                    profit_in_atr = (
                        ...
                        / current_atr
                        if pos["side"] == "Buy"
                        else (current_trade_state["entry_price"] - current_price)
                        / current_atr
                    )

                    if profit_in_atr >= BREAK_EVEN_PROFIT_ATR:
                        log_message(
                            f"Profit of {profit_in_atr:.2f} ATR reached. Moving SL to break-even.",
                            "INFO",
                        )
                        ...
                        if (
                            ...
                            log_message(
                                f"Amending SL to {new_sl_breakeven:.2f} for break-even.",
                                "INFO",
                            )
                            amend_stop_loss_take_profit(
                                SYMBOL,
                ...
                if current_trade_state["partial_profit_taken"] or (
                    ...
                    if (
                        ...
                        log_message(
                            f"Amending SL to {new_trailing_sl:.2f} due to trailing stop.",
                            "INFO",
                        )
                        amend_stop_loss_take_profit(
                            SYMBOL,
                ...
                log_message(
                    "WARNING: order_id is missing. Dynamic SL/TP adjustments are disabled.",
                    "WARNING",
                )
            ...
            if side:
                ...
                if price_diff == 0:
                    log_message(
                        "Stop loss price is too close to entry price. Cannot place trade.",
                        "ERROR",
                    )
                    time.sleep(60)
                    continue
