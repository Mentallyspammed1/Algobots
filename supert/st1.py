...
class TrailingStopManager:
    ...
    def initialize_trailing_stop(
        ...
        if specs.category == "spot":
            self.logger.debug(
                f"Trailing stops may not be fully supported or applicable for spot category {symbol}. Skipping initialization logic."
            )
        ...
        if position_side == "Buy":
            ...
            highest_price = (
                current_price if is_activated else entry_price
            )  # Start tracking from current price if activated
    ...
    def update_trailing_stop(
            ...
            if ts_info["is_activated"]:
                if current_price > ts_info["highest_price"]:
                    ...
                    if new_stop > ts_info["current_stop"]:
                        ts_info["current_stop"] = self.precision.round_price(
                            symbol, new_stop
                        )
                        updated_locally = True
                        self.logger.debug(
                            f"Trailing stop updated for {symbol}: New Stop={ts_info['current_stop']}"
                        )
        ...
        if updated_locally and update_exchange and ts_info["current_stop"] > 0:
            self.logger.info(
                f"Attempting to update stop loss on exchange for {symbol} to {ts_info['current_stop']}"
            )
            success = self._update_stop_loss_on_exchange(
                symbol, ts_info["current_stop"]
            )
            if success:
                return (
                    True  # Indicate that an exchange update was attempted and succeeded
                )
            else:
                # Log failure but indicate local update might have occurred
                self.logger.error(
                    f"Failed to update trailing stop on exchange for {symbol}. Local state updated, but exchange state may be stale."
                )
    ...
    def _update_stop_loss_on_exchange(self, symbol: str, stop_price: Decimal) -> bool:
        ...
        if not specs:
            self.logger.error(
                f"Failed to update stop loss for {symbol}: Specs not found."
            )
            return False
        ...
        try:
            ...
            if response and response["retCode"] == 0:
                self.logger.info(
                    f"Successfully updated stop loss on exchange for {symbol} to {stop_price}"
                )
                return True
            else:
                error_msg = (
                ...
                self.logger.error(
                    f"Failed to update stop loss on exchange for {symbol}: {error_msg}"
                )
                return False

        except Exception as e:
            self.logger.error(
                f"Exception updating stop loss on exchange for {symbol}: {e}",
                exc_info=True,
            )
            return False
        ...
        try:
            ...
            if response and response["retCode"] == 0:
                ...
                if tickers:
                    return tickers[
                        0
                    ]  # Expecting a single ticker for the specified symbol
                else:
                    self.logger.warning(
