...
class SupertrendBot:
    def __init__(self, config: Config):
        ...
        self.logger.info(
            f"  Supertrend Params: Period={config.ST_PERIOD}, Multiplier={config.ST_MULTIPLIER}"
        )
        self.logger.info(
        ...
        for attempt in range(1, self.config.MAX_API_RETRIES + 1):
            try:
                ...
                self.logger.critical(
                    f"Failed to initialize Bybit API session or validate keys (Attempt {attempt}/{self.config.MAX_API_RETRIES}): {e}",
                    exc_info=True,
                )
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            st_col_prefix = (
                f"SUPERT_{self.config.ST_PERIOD}_{self.config.ST_MULTIPLIER}"
            )
            ...
            if st_results is not None and not st_results.empty:
                ...
                if expected_st_col and expected_dir_col:
                    ...
                    self.logger.debug(
                        f"Supertrend columns joined: '{expected_st_col}' -> 'supertrend', '{expected_dir_col}' -> 'supertrend_direction'"
                    )
                else:
                    self.logger.warning(
                        "Could not find expected Supertrend columns. Setting neutral indicators."
                    )
                    df["supertrend"] = df["close"]  # Fallback to close price
                    df["supertrend_direction"] = 0
            else:
                self.logger.warning(
                    "Supertrend calculation returned empty DataFrame. Setting neutral indicators."
                )
                df["supertrend"] = df["close"]  # Fallback to close price
                df["supertrend_direction"] = 0
            ...
            for col in required_indicator_cols:
                if col not in df.columns:
                    df[col] = (
                        0  # Add missing columns with default if they didn't get created
                    )
            ...
            self.logger.debug(
                f"Supertrend and ATR indicators calculated. DataFrame shape: {df.shape}"
            )
            return df
    ...
    def generate_signal(self, df: pd.DataFrame) -> Signal:
        ...
        if df.empty or len(df) < 2:
            self.logger.warning(
                "Not enough data points to generate signal (need at least 2)."
            )
        ...
        try:
            ...
            if current_st_dir == 1:
                if prev_st_dir == -1:  # Trend flip from down to up
                    ...
                    current_close > current_st_price
                ):  # Trend is up and price is above ST line
            ...
            # 2. Sell: Trend is DOWN (-1) and price is below Supertrend line
                ...
                if prev_st_dir == 1:  # Trend flip from up to down
                    ...
                    current_close < current_st_price
                ):  # Trend is down and price is below ST line
            ...
            self.logger.debug(
                f"Signal generated: {self.last_signal.name} (Close={current_close:.2f}, ST={current_st_price:.2f}, ST_Dir={current_st_dir})"
            )
    ...
    def place_order(
        ...
        try:
            ...
            self.logger.error(
                f"Exception placing order for {self.config.SYMBOL}: {e}", exc_info=True
            )
            return None
    ...
    def execute_trade_based_on_signal(self, signal: Signal):
        ...
        try:
            ...
            if not self.position_active and signal in [Signal.BUY, Signal.STRONG_BUY]:
                self.logger.info(
                    f"Received BUY signal ({signal.name}). Attempting to open long position."
                )
                ...
                if position_qty is not None and position_qty > 0:
                    ...
                    self.logger.warning(
                        "Could not calculate a valid position size for the BUY signal. Skipping order placement."
                    )

            # If we are NOT in a position and receive a SELL signal (SELL or STRONG_SELL)
            elif not self.position_active and signal in [
                Signal.SELL,
                Signal.STRONG_SELL,
            ]:
                self.logger.info(
                    f"Received SELL signal ({signal.name}). Attempting to open short position."
                )
                ...
                if position_qty is not None and position_qty > 0:
                    ...
                    self.logger.warning(
                        "Could not calculate a valid position size for the SELL signal. Skipping order placement."
                    )
