...
class PrecisionManager:
    ...
    def _parse_instrument_specs(self, inst: dict, category: str) -> InstrumentSpecs:
        ...
        )  # Check for unified specific keys
    ...
    def get_specs(self, symbol: str) -> InstrumentSpecs | None:
        ...
        return self.instruments.get(
            symbol.upper()
        )  # Ensure symbol lookup is case-insensitive
...
class OrderSizingCalculator:
    ...
    def calculate_position_size_usd(
        ...
        if actual_risk_percent > risk_percent * Decimal(
            ...
            self.logger.warning(
                f"Calculated risk ({actual_risk_percent:.4f}%) slightly exceeds allowed risk ({risk_percent:.4f}%). Clamping might be needed or review parameters."
            )
...
class SupertrendBot:
    def __init__(self, config: Config):
        ...
        try:
            ...
            self.logger.info(
                f"Successfully connected to Bybit API ({'Testnet' if config.TESTNET else 'Mainnet'})."
            )
        except FailedRequestError as e:
            # Handle specific Bybit API errors like rate limiting or IP blocks
            self.logger.critical(
                f"Failed to initialize Bybit API session or validate keys: {e}",
                exc_info=True,
            )
            self.logger.critical(
                "This error often indicates an issue with your API keys, network connectivity,"
            )
            self.logger.critical(
                "or you might be experiencing Bybit's IP rate limits or geo-restrictions (e.g., from the USA)."
            )
            self.logger.critical(
                "Please check your API key permissions, network, and Bybit's API documentation regarding IP access."
            )
            sys.exit(1)  # Exit the program as essential initialization failed
        except Exception as e:
            # Catch any other unexpected errors during initialization
            self.logger.critical(
                f"An unexpected error occurred during Bybit API session initialization: {e}",
                exc_info=True,
            )
            sys.exit(1)
        ...
        self.logger.info(
            f"  Execution: OrderType={config.ORDER_TYPE_ENUM.value}, TimeInForce={config.TIME_IN_FORCE}"
        )
    ...
    def generate_signal(self, df: pd.DataFrame) -> Signal:
        ...
        try:
            ...
            current_st_dir = latest.get(
                "supertrend_direction", 0
            )  # Default to 0 if missing
            prev_st_dir = previous.get(
                "supertrend_direction", 0
            )  # Default to 0 if missing
    ...
    def get_account_balance_usdt(self) -> Decimal:
        ...
        account_type = (
            "UNIFIED"
            if self.config.CATEGORY_ENUM.value in ["linear", "option"]
            else "CONTRACT"
        )  # Generally CONTRACT for derivatives, UNIFIED can also work for linear. SPOT is separate.
        ...
        try:
            ...
            if response and response["retCode"] == 0:
                ...
                self.logger.warning(
                    f"USDT balance not found in response for account type {account_type}. Checking other quote currencies if applicable or returning 0."
                )
        ...
        try:
            ...
            # 2. Handle Managing Existing Positions
                ...
                if self.config.TRAILING_STOP_PCT > 0:
                    ...
                    ):  # If we have position info, check for markPrice
