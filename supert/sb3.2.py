...
class Position:
    ...
    order_id: Optional[str] = None # Bybit order ID for the entry order
...
class BotState:
    ...
    supertrend_line: Optional[Decimal] = None # From primary TF
...
class OrderSizingCalculator:
    ...
    def calculate_position_size(self, specs: InstrumentSpecs, balance: Decimal, entry: Decimal, sl: Decimal, leverage: Decimal, metrics: PerformanceMetrics) -> Optional[Decimal]:
    ...
    def _calculate_fixed_risk_size(self, specs: InstrumentSpecs, balance: Decimal, entry: Decimal, sl: Decimal, leverage: Decimal) -> Optional[Decimal]:
        if abs(entry - sl) == 0:
...
class AdvancedSupertrendBot:
    ...
    def _api_call(self, api_method: Callable, **kwargs) -> Optional[Dict[str, Any]]:
    ...
    def _generate_signal(self) -> Tuple[Signal, str, Decimal]:
        ...
        if self.config.RSI_FILTER and 'rsi' in latest_primary:
            if base_buy_signal and latest_primary['rsi'] < self.config.RSI_OVERBOUGHT: # Not overbought
                rsi_ok = True
            elif base_sell_signal and latest_primary['rsi'] > self.config.RSI_OVERSOLD: # Not oversold
                rsi_ok = True
        ...
        if 'macd_hist' in latest_primary:
            if base_buy_signal and latest_primary['macd_hist'] > 0: # Bullish momentum
                macd_ok = True
            elif base_sell_signal and latest_primary['macd_hist'] < 0: # Bearish momentum
                macd_ok = True
        ...
        if self.config.USE_ORDER_BOOK:
            ...
            if base_buy_signal and imbalance > self.config.ORDER_BOOK_IMBALANCE_THRESHOLD:
                order_book_ok = True
            elif base_sell_signal and imbalance < -self.config.ORDER_BOOK_IMBALANCE_THRESHOLD:
                order_book_ok = True
    ...
    # --- Order Execution ---
    def _place_order(self, side: str, qty: Decimal, order_type: OrderType, price: Optional[Decimal], sl: Decimal, tp: Decimal) -> Optional[str]:
    ...
    def _update_trading_stop(self, stop_loss: Optional[Decimal] = None, take_profit: Optional[Decimal] = None) -> bool:
    ...
    def _close_position(self, partial_qty: Optional[Decimal] = None) -> bool:
