...
class EhlersSuperTrendBot:
    def __init__(self, config: Config):
        ...
        if self.config.CATEGORY_ENUM != Category.SPOT:
            self.set_leverage() # Set leverage for derivatives (not applicable for spot)
        else:
    ...
    def _process_websocket_message(self, msg: Dict[str, Any]):
        ...
        if "topic" in msg and str(msg["topic"]).startswith(f"kline.{self.config.TIMEFRAME}.{self.config.SYMBOL}"):
            ...
            with self.bot_state.lock:
                ...
                if 'adx_plus_di' in latest_indicator_row and not np.isnan(latest_indicator_row['adx_plus_di']):
                    self.bot_state.adx_plus_di = Decimal(str(latest_indicator_row['adx_plus_di']))
                else:
                    self.bot_state.adx_plus_di = Decimal('0.0')
                if 'adx_minus_di' in latest_indicator_row and not np.isnan(latest_indicator_row['adx_minus_di']):
                    self.bot_state.adx_minus_di = Decimal(str(latest_indicator_row['adx_minus_di']))
                else:
                    self.bot_state.adx_minus_di = Decimal('0.0')
            ...
            self.get_positions() # This will update self.position_active, current_position_side etc. and bot_state
