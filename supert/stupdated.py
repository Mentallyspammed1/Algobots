...
class OrderSizingCalculator:
    ...
    def calculate_position_size_usd(
        ...
        if self.config.VOLATILITY_ADJUSTED_SIZING_ENABLED and current_atr is not None and current_atr > Decimal('0'):
            ...
            # Let's re-think: the goal is to risk 'X' dollars per trade.
            ...
            # The existing logic already implicitly does this: smaller stop_distance_price -> larger position_value_needed_usd.
...
class EhlersSuperTrendBot:
    ...
    def _cumulative_loss_guard(self) -> bool:
        ...
        if self.initial_equity <= Decimal('0') or current_equity <= Decimal('0'): # Ensure valid initial and current equity
            ...
            if self.cumulative_pnl < -abs(Decimal(str(self.config.MAX_DAILY_LOSS_PCT))) * self.initial_equity: # Assuming initial_equity is available for this. If not, this logic needs adjustment.
    ...
    def calculate_indicators(self, df: pd.DataFrame, is_higher_timeframe: bool = False) -> pd.DataFrame:
        ...
        if self.config.ADAPTIVE_INDICATORS_ENABLED and not is_higher_timeframe: # Only apply to primary timeframe
            if len(df) >= self.config.VOLATILITY_MEASURE_WINDOW:
                # Calculate volatility using ATR or standard deviation of returns
                # Using ATR as a volatility measure
                ...
                current_volatility = atr_series.iloc[-1] / df['close'].iloc[-1] # ATR as percentage of price
        ...
        if self.config.PRICE_ACTION_CONFIRMATION_ENABLED:
            ...
            if st_flipped_up: # Look for bullish patterns
                ...
                if not pattern_detected:
                    return None, f"Bullish signal detected, but no required bullish candlestick pattern found."
                ...
                if not pattern_detected:
                    return None, f"Bearish signal detected, but no required bearish candlestick pattern found."
    ...
    def _get_timeframe_in_ms(self) -> int:
        ...
        if tf.endswith('D'): return 24 * 60 * 60 * 1000
        if tf.endswith('W'): return 7 * 24 * 60 * 60 * 1000
        if tf.endswith('M'): return 30 * 24 * 60 * 60 * 1000 # Approx month
    ...
    def run(self):
        ...
        try:
            ...
            if self.config.AUTO_CLOSE_ON_SHUTDOWN and self.position_active and self.current_position_size > Decimal('0'):
              ...
              self.logger.info("Auto-close on shutdown is disabled or no active position. Not closing positions.")                                  except Exception as e:                                                     self.logger.error(f"Error during cleanup: {e}", exc_info=True)                                                                            finally:                                                                   self.logger.info(Fore.LIGHTBLUE_EX + Style.BRIGHT + "Ehlers SuperTrend Bot has ceased its operations. Until next time!" + Style.RESET_ALL)                                                                           subprocess.run(["termux-toast", f"Ehlers SuperTrend Bot for {self.config.SYMBOL} has ceased operations."])                                    if self.sms_notifier.is_enabled:                                           self.sms_notifier.send_sms(f"Ehlers SuperTrend Bot for {self.config.SYMBOL} has ceased operations.")
