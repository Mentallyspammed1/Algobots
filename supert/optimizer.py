...
class BotBacktester:
    ...
    def mock_api_call(self, api_method, **kwargs):
        logger.debug(
            f"Mock API call intercepted for {api_method.__name__} with args {kwargs}"
        )
    ...
    def run(self):
        ...
        if self.bot.market_data.empty:
            logger.error(
                "Indicator calculation resulted in empty dataframe. Aborting backtest."
            )
            return 0.0
    ...
    def _check_exit_conditions(self, candle):
        ...
        if not exit_price and (
            (self.position["side"] == "Buy" and candle["supertrend_direction"] == -1)
            or (self.position["side"] == "Sell" and candle["supertrend_direction"] == 1)
        ):
    ...
    def _check_entry_conditions(self, index):
        ...
        if signal in ["BUY", "SELL"]:
            ...
            sl_price, tp_price = self.bot.calculate_trade_sl_tp(
                trade_side, Decimal(str(entry_price)), df_slice
            )
            risk_per_trade = self.capital * (
                Decimal(self.config.RISK_PER_TRADE_PCT) / Decimal(100)
            )
    ...
    def _close_position(self, time, exit_price, reason):
        ...
        pnl = (
            (Decimal(str(exit_price)) - entry_price) * quantity
            if self.position["side"] == "Buy"
            else (entry_price - Decimal(str(exit_price))) * quantity
        )
        pnl -= (entry_price * quantity + Decimal(str(exit_price)) * quantity) * Decimal(
            "0.0006"
        )
        self.capital += pnl
        self.trades.append(
        ...
        logger.debug(
            f"Closed {self.position['side']} position at {exit_price} on {time}. PnL: {pnl:.2f}"
        )
        self.position = None
...
def objective(trial: optuna.trial.Trial, historical_data: pd.DataFrame, symbol: str):
    ...
    config.ADX_TREND_FILTER_ENABLED = trial.suggest_categorical(
        "ADX_TREND_FILTER_ENABLED", [True, False]
    )
