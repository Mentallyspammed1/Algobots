...
class Config:
    ...
    ST_MULTIPLIER: float = float(
        os.getenv("ST_MULTIPLIER", 1.5)
    )  # Lower for sensitivity
    RISK_PER_TRADE_PCT: float = float(
        os.getenv("RISK_PER_TRADE_PCT", 0.5)
    )  # Smaller risk for high-frequency
    STOP_LOSS_PCT: float = float(
        os.getenv("STOP_LOSS_PCT", 0.5)
    )  # Tight SL for scalping
    TAKE_PROFIT_PCT: float = float(os.getenv("TAKE_PROFIT_PCT", 1.0))  # Tight TP
    BREAKEVEN_PCT: float = float(
        os.getenv("BREAKEVEN_PCT", 0.5)
    )  # Move SL to breakeven after this profit %
    MAX_DAILY_LOSS_PCT: float = float(
        os.getenv("MAX_DAILY_LOSS_PCT", 3.0)
    )  # Tighter for scalping
    ORDER_TYPE: str = os.getenv("ORDER_TYPE", "Market")
    ADX_TREND_FILTER_ENABLED: bool = os.getenv(
        "ADX_TREND_FILTER_ENABLED", "true"
    ).lower() in ["true", "1", "t"]
    ADX_MIN_THRESHOLD: int = int(os.getenv("ADX_MIN_THRESHOLD", 25))
    RSI_FILTER_ENABLED: bool = os.getenv("RSI_FILTER_ENABLED", "true").lower() in [
        "true",
        "1",
        "t",
    ]
    RSI_PERIOD: int = int(os.getenv("RSI_PERIOD", 14))
    VOLUME_FILTER_ENABLED: bool = os.getenv(
        ...
        default_factory=lambda: [
            {
                "profit_pct": 0.5,
                "close_qty_pct": 0.5,
            },  # First partial at 0.5% profit, close 50%
            {
                "profit_pct": 1.0,
                "close_qty_pct": 0.5,
            },  # Second at 1.0%, close remaining 50%
        ]
    )
    BACKTEST_MODE: bool = os.getenv("BACKTEST_MODE", "false").lower() in [
        "true",
        "1",
        "t",
    ]
...
def setup_logger(config: Config) -> logging.Logger:
    ...
    handler_file = logging.handlers.RotatingFileHandler(
        "supertrend_bot.log", maxBytes=5 * 1024 * 1024, backupCount=3
    )
    ...
    # JSON File Handler
    handler_json = logging.handlers.RotatingFileHandler(
        "supertrend_bot.json.log", maxBytes=5 * 1024 * 1024, backupCount=3
    )
    ...
    def display(self):
        with self.state.lock:
            ...
            print(
                Style.BRIGHT
                + Fore.CYAN
                + f"=== Supertrend Scalping Bot | {self.state.symbol} | Status: {self.state.bot_status} ==="
                + Style.RESET_ALL
            )
            ...
            print(
                f"Current Price: {price_color}{self.state.current_price:.2f}{Style.RESET_ALL}"
            )
            print(
                f"Supertrend: {st_color}{self.state.supertrend_line:.2f} ({self.state.supertrend_direction}){Style.RESET_ALL}"
            )
            print(
                f"Volume: {Fore.YELLOW}{self.state.volume:.2f} (SMA: {self.state.volume_sma:.2f}){Style.RESET_ALL}"
            )

            # --- Indicator Values ---
            rsi_color = (
                Fore.RED
                if self.state.rsi > 70
                else Fore.GREEN
            ...
            print(
                f"RSI: {rsi_color}{self.state.rsi:.2f}{Style.RESET_ALL} | MACD Hist: {macd_color}{self.state.macd_hist:.4f}{Style.RESET_ALL} | ADX: {adx_color}{self.state.adx:.2f}{Style.RESET_ALL}"
            )
            ...
            if self.state.position_size > 0:
                pos_color = (
                    Fore.GREEN if self.state.position_side == "Buy" else Fore.RED
                )
                pnl_color = Fore.GREEN if self.state.unrealized_pnl >= 0 else Fore.RED
                print(
                    f"Side: {pos_color}{self.state.position_side}{Style.RESET_ALL} | Size: {self.state.position_size} | Entry: {self.state.entry_price:.2f}"
                )
                print(
                    f"Unrealized PnL: {pnl_color}${self.state.unrealized_pnl:.2f}{Style.RESET_ALL}"
                )
                print(
                    f"SL: {self.state.stop_loss:.2f} | TP: {self.state.take_profit:.2f}"
                )
            else:
            ...
            print(
                f"Daily Loss: {Fore.RED if self.state.daily_loss > 0 else Fore.GREEN}{self.state.daily_loss:.2f}%{Style.RESET_ALL}"
            )
...
class SupertrendBot:
    ...
    def __init__(self, config: Config):
        ...
        self.ws.kline_stream(
            callback=self.handle_kline, symbol=config.SYMBOL, interval=config.TIMEFRAME
        )
    ...
    def check_daily_loss(self) -> bool:
        ...
        self.daily_loss = (
            (self.start_balance - current_balance) / self.start_balance * 100
        )
        ...
        if self.daily_loss > max_loss:
            self.bot_state.add_log(
                f"Daily loss limit exceeded: {self.daily_loss:.2f}% > {max_loss}%"
            )
            return True
        return False
    ...
    def fetch_historical_klines(self, limit=1000) -> pd.DataFrame | None:
        try:
            ...
            if res["retCode"] == 0:
                df = pd.DataFrame(
                    res["result"]["list"], columns=["ts", "o", "h", "l", "c", "v", "t"]
                )
                df.rename(
                    columns={
                        "ts": "timestamp",
    ...
    def generate_signal(self, df: pd.DataFrame) -> Signal:
        ...
        # ADX Filter
        if (
            self.config.ADX_TREND_FILTER_ENABLED
            and latest["adx"] < self.config.ADX_MIN_THRESHOLD
        ):
    ...
    def handle_kline(self, message):
        ...
        try:
            ...
            self.klines_df = (
                pd.concat([self.klines_df, new_row]).tail(200).reset_index(drop=True)
            )
            ...
            self.bot_state.update(
                ...
                price_direction=1
                if latest["close"] > self.klines_df.iloc[-2]["close"]
                else -1
                if len(self.klines_df) > 1
                else 0,
    ...
    def process_signal(self):
        ...
        tp_pct = Decimal(
            str(self.config.TAKE_PROFIT_PCT / 100)
        )  # Base TP, but partials override
        sl = (
        ...
        qty = self.order_sizer.calculate(
            specs,
    ...
    def place_entry_order(
        ...
        params = {
            ...
            "takeProfit": str(self.precision_manager.round_price(specs, tp))
            if not self.config.PARTIAL_TP_ENABLED
            else "0",  # No full TP if partial
            "tpslMode": "Full",
        }
        ...
        try:
            ...
            if res["retCode"] == 0:
                self.logger.info(
                    f"Entry order placed: {side} {qty} {self.config.SYMBOL}"
                )
    ...
    def place_partial_tp_orders(
        ...
        for target in self.config.PARTIAL_TP_TARGETS:
            ...
            tp_price = (
                entry * (Decimal(1) + profit_pct)
                if entry_side == "Buy"
                else entry * (Decimal(1) - profit_pct)
            )
            close_qty = self.precision_manager.round_quantity(
                specs, total_qty * close_qty_pct
            )
            ...
            try:
                ...
                if res["retCode"] == 0:
                    self.logger.info(
                        f"Partial TP order placed: {close_side} {close_qty} at {tp_price}"
                    )
                else:
    ...
    def run_backtest(self):
        ...
        for atr, mult in itertools.product(atr_lengths, multipliers):
            ...
            df_test["strategy_returns"] = df_test["returns"] * df_test["signal"].shift(
                1
            )
        ...
        self.logger.info(
            f"Backtest Complete. Best Params: ATR={best_params}, Mult={best_params}, Return={best_return:.2%}"
        )
        print(
            f"Best Params: ATR={best_params}, Mult={best_params}, Return={best_return:.2%}"
        )
...
if __name__ == "__main__":
    try:
        ...
        logging.getLogger("SupertrendBot").critical(
            f"Fatal error on startup: {e}", exc_info=True
        )
        sys.exit(1)
