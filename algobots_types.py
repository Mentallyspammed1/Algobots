...
class OrderBlock(TypedDict):
    ...
    timestamp: (
        datetime.datetime
    )  # Timestamp of the candle that formed the OB (its close time)
    ...
    violation_ts: datetime.datetime | None  # Timestamp when the OB was violated
    extended_to_ts: (
        datetime.datetime
    )  # The latest timestamp where the OB was still considered relevant/active
