#!/usr/bin/env python3
"""Bybit Supertrend Trading Bot Framework v2.5 (Enhanced Robustness, Scalping, & Neon Console)
...
while maintaining scalping-optimized parameters and an UPDATED "neon" color-coded
console output for even more vivid visual feedback.
...
9.  **Enhanced Entry/Exit Logic:** Incorporates signal confirmation (tunable for speed),
    ...
    Ensures `initial_stop_loss` is correctly managed.
10. **Comprehensive Logging with NEW Neon Console Scheme:** Detailed logging for all critical
    operations, facilitating debugging and performance monitoring. Console output is
    color-coded with a fresh, vibrant neon palette for immediate visual comprehension
    of log levels.
...
class BybitTradingBot:
    ...
    def _calculate_position_size(self, signal: StrategySignal) -> float:
        ...
        if (
            stop_loss_distance_raw <= Decimal("0", DECIMAL_CONTEXT)
            or stop_loss_distance_raw < self.market_info.tick_size
        ):
            logger.warning(
                f"Stop loss distance is non-positive or too small ({stop_loss_distance_raw:.5f} < {self.market_info.tick_size}), cannot calculate position size safely. Returning 0.",
            )
            return 0.0
