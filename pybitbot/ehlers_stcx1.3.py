    ...
    async def calculate_indicators(self, data: Dict[str, pd.DataFrame]):
        ...
        for current in range(1, len(df_copy.index)):
            ...
            if df_copy.loc[df_copy.index[current], 'in_uptrend']:
                ...
                prev_supertrend = df_copy.loc[df_copy.index[previous], 'supertrend_line'] if not np.isnan(df_copy.loc[df_copy.index[previous], 'supertrend_line']) else df_copy.loc[df_copy.index[current], 'lowerband']
                df_copy.loc[df_copy.index[current], 'supertrend_line'] = max(df_copy.loc[df_copy.index[current], 'lowerband'], prev_supertrend)
            else:
                ...
                prev_supertrend = df_copy.loc[df_copy.index[previous], 'supertrend_line'] if not np.isnan(df_copy.loc[df_copy.index[previous], 'supertrend_line']) else df_copy.loc[df_copy.index[current], 'upperband']
                df_copy.loc[df_copy.index[current], 'supertrend_line'] = min(df_copy.loc[df_copy.index[current], 'upperband'], prev_supertrend)
