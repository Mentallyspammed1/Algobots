...
from colorama import (
    Fore,  # Import Fore and Style for convenience
    Style,  # Import Fore and Style for convenience
)
...
class NeonFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        record.levelname = record.levelname[
            0:4
        ]  # Truncate levelname for concise output
    ...
    async def calculate_indicators(self, data: dict[str, pd.DataFrame]):
        ...
        for current in range(1, len(df_copy.index)):
            ...
            if df_copy.loc[df_copy.index[current], "in_uptrend"]:
                ...
                prev_supertrend_val = df_copy.loc[
                    df_copy.index[previous], "supertrend_line",
                ]
                if (
                    np.isnan(prev_supertrend_val)
                    or not df_copy.loc[df_copy.index[previous], "in_uptrend"]
                ):  # If it just flipped up
                    df_copy.loc[df_copy.index[current], "supertrend_line"] = (
                        df_copy.loc[df_copy.index[current], "lowerband"]
                    )
                ...
                if (
                    np.isnan(prev_supertrend_val)
                    or df_copy.loc[df_copy.index[previous], "in_uptrend"]
                ):  # If it just flipped down
                    df_copy.loc[df_copy.index[current], "supertrend_line"] = (
                        df_copy.loc[df_copy.index[current], "upperband"]
                    )
...
class EhlersSupertrendCrossStrategy(BaseStrategy):
    ...
    async def calculate_indicators(self, data: dict[str, pd.DataFrame]):
        ...
        low_close_filtered = np.abs(
            df_filtered["low"] - df_filtered["filtered_close"].shift(),
        )  # Corrected here: should be filtered_close
...
class BybitTradingBot:
    ...
    def __init__(
        ...
        self.balance: Decimal = to_decimal(
            "0",
