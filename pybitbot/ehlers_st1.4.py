...
colorama.init(autoreset=True) # autoreset ensures colors reset after each print
...
# Colorama-based "neon" style logging colors
class NeonColors:
    RESET = Style.RESET_ALL
    BOLD = Style.BRIGHT
    
    # Text colors using Colorama's Fore and Style.BRIGHT for "neon" effect
    BRIGHT_GREEN = Fore.GREEN + Style.BRIGHT    # INFO
    BRIGHT_YELLOW = Fore.YELLOW + Style.BRIGHT   # WARNING
    BRIGHT_RED = Fore.RED + Style.BRIGHT      # ERROR
    BRIGHT_MAGENTA = Fore.MAGENTA + Style.BRIGHT # CRITICAL (vibrant pink/purple)
    BRIGHT_CYAN = Fore.CYAN + Style.BRIGHT     # DEBUG
    BRIGHT_BLUE = Fore.BLUE + Style.BRIGHT     # General/Other
    BRIGHT_WHITE = Fore.WHITE + Style.BRIGHT    # Default/Subtle

class NeonFormatter(logging.Formatter):
    """A custom formatter for neon-style colored console output using Colorama."""

    FORMATS = {
        logging.DEBUG: NeonColors.BRIGHT_CYAN + '%(asctime)s - ' + NeonColors.BOLD + 'DBG' + NeonColors.RESET + NeonColors.BRIGHT_CYAN + ' - %(name)s - %(funcName)s:%(lineno)d - %(message)s' + NeonColors.RESET,
        logging.INFO: NeonColors.BRIGHT_GREEN + '%(asctime)s - ' + NeonColors.BOLD + 'INFO' + NeonColors.RESET + NeonColors.BRIGHT_GREEN + ' - %(message)s' + NeonColors.RESET,
        logging.WARNING: NeonColors.BRIGHT_YELLOW + '%(asctime)s - ' + NeonColors.BOLD + 'WARN' + NeonColors.RESET + NeonColors.BRIGHT_YELLOW + ' - %(message)s' + NeonColors.RESET,
        logging.ERROR: NeonColors.BRIGHT_RED + '%(asctime)s - ' + NeonColors.BOLD + 'ERROR' + NeonColors.RESET + NeonColors.BRIGHT_RED + ' - %(funcName)s:%(lineno)d - %(message)s' + NeonColors.RESET,
        logging.CRITICAL: NeonColors.BOLD + NeonColors.BRIGHT_MAGENTA + '%(asctime)s - ' + NeonColors.BOLD + 'CRIT' + NeonColors.RESET + NeonColors.BRIGHT_MAGENTA + ' - %(funcName)s:%(lineno)d - %(message)s' + NeonColors.RESET
    }

    def format(self, record):
        log_fmt = self.FORMATS.get(record.levelno)
        # Use short level names to fit better with colorama output
        record.levelname = record.levelname[0:4] # Truncate levelname for concise output
...
class Config:
    ...
    symbols: List[str] = field(default_factory=lambda: ["XLMUSDT", "LINKUSDT"]) # List of symbols to trade
    ...
    async def _confirm_signal(self, current_candle_time: datetime) -> bool:
        ...
        if self.last_signal and not self.signal_confirmed and self.signal_candle_time:
            ...
            if df is None or df.empty or self.signal_candle_time not in df.index:
                logger.debug(f"[{self.symbol}] DataFrame for {self.primary_timeframe} is empty or signal_candle_time {self.signal_candle_time} not in index for confirmation.")
                return False
            ...
            try:
                ...
                logger.warning(f"[{self.symbol}] Could not find signal_candle_time {self.signal_candle_time} or current_candle_time {current_candle_time} in DataFrame index for signal confirmation.")
                return False

            if current_idx - signal_idx >= confirmation_candles_needed:
                self.signal_confirmed = True
                logger.info(f"[{self.symbol}] Signal for {self.last_signal.action} confirmed after {confirmation_candles_needed} candles.")
                return True
...
class SupertrendStrategy(BaseStrategy):
    ...
    async def calculate_indicators(self, data: Dict[str, pd.DataFrame]):
        ...
        df = data.get(self.primary_timeframe) # Get raw market data for this strategy's symbol and timeframe
        min_data_needed = max(self.supertrend_period, self.atr_period)
        if df is None or df.empty or len(df) < min_data_needed:
            logger.debug(f"[{self.symbol}] Insufficient data for Supertrend calculation ({len(df) if df is not None else 0} < {min_data_needed} candles).")
            return
        ...
        for current in range(1, len(df_copy.index)):
            ...
            # Trend determination based on previous bands
            if df_copy.loc[df_copy.index[current], 'close'] > df_copy.loc[df_copy.index[previous], 'upperband']:
                df_copy.loc[df_copy.index[current], 'in_uptrend'] = True
            elif df_copy.loc[df_copy.index[current], 'close'] < df_copy.loc[df_copy.index[previous], 'lowerband']:
                df_copy.loc[df_copy.index[current], 'in_uptrend'] = False
            else:
                # Maintain previous trend if no crossover
                df_copy.loc[df_copy.index[current], 'in_uptrend'] = df_copy.loc[df_copy.index[previous], 'in_uptrend']

            # Set the Supertrend line, ensuring it trails and respects trend direction
            # It should stay at the current band if price is 'inside' but the trend is maintained,
            # or trail the previous Supertrend line if it's tighter.
            if df_copy.loc[df_copy.index[current], 'in_uptrend']:
                # If in uptrend, Supertrend is the greater of the current lowerband or the previous Supertrend line
                # (but only if previous was also uptrend, otherwise it sets to current lowerband)
                prev_supertrend_val = df_copy.loc[df_copy.index[previous], 'supertrend_line']
                if np.isnan(prev_supertrend_val) or not df_copy.loc[df_copy.index[previous], 'in_uptrend']: # If it just flipped up
                    df_copy.loc[df_copy.index[current], 'supertrend_line'] = df_copy.loc[df_copy.index[current], 'lowerband']
                else: # Continue uptrend
                    df_copy.loc[df_copy.index[current], 'supertrend_line'] = max(df_copy.loc[df_copy.index[current], 'lowerband'], prev_supertrend_val)
            else: # In downtrend
                # If in downtrend, Supertrend is the lesser of the current upperband or the previous Supertrend line
                # (but only if previous was also downtrend, otherwise it sets to current upperband)
                prev_supertrend_val = df_copy.loc[df_copy.index[previous], 'supertrend_line']
                if np.isnan(prev_supertrend_val) or df_copy.loc[df_copy.index[previous], 'in_uptrend']: # If it just flipped down
                    df_copy.loc[df_copy.index[current], 'supertrend_line'] = df_copy.loc[df_copy.index[current], 'upperband']
                else: # Continue downtrend
                    df_copy.loc[df_copy.index[current], 'supertrend_line'] = min(df_copy.loc[df_copy.index[current], 'upperband'], prev_supertrend_val)
        
        df_copy['supertrend'] = df_copy['supertrend_line'] # The final Supertrend value
        self.indicators[self.primary_timeframe] = df_copy  # Store the processed DataFrame
    ...
    async def generate_signal(self) -> Optional[StrategySignal]:
        ...
        if df_indicators is None or df_indicators.empty or len(df_indicators) < 2:
            logger.debug(f"[{self.symbol}] Insufficient indicator data for signal generation.")
            return None

        df_cleaned = df_indicators.dropna(subset=['supertrend', 'atr', 'in_uptrend']).copy() # Include 'in_uptrend'
        if df_cleaned.empty or len(df_cleaned) < 2:
            logger.debug(f"[{self.symbol}] DataFrame too small after dropping NaNs for signal generation from indicators.")
            return None
        ...
        if self.last_signal:
            if (self.last_signal.action == 'BUY' and not current['in_uptrend']) or \
               (self.last_signal.action == 'SELL' and current['in_uptrend']):
                logger.info(f"[{self.symbol}] Trend changed opposite to pending signal ({self.last_signal.action}), resetting pending signal.")
            ...
            if not self.signal_confirmed:
                if await self._confirm_signal(current.name):
                    logger.info(f"[{self.symbol}] Confirmed PENDING TRADE SIGNAL: {self.last_signal.action} for {self.symbol}.")
        ...
        if not previous['in_uptrend'] and current['in_uptrend']:
            signal_to_generate = StrategySignal(
                ...
                symbol=self.symbol,
                strength=1.0,
                stop_loss=float(current['supertrend']), # Current supertrend is the new SL
        ...
        if signal_to_generate:
            ...
            logger.info(f"[{self.symbol}] PENDING TRADE SIGNAL: {signal_to_generate.action} for {self.symbol}. Reason: {signal_to_generate.metadata['reason']}. SL: {signal_to_generate.stop_loss:.5f}. Waiting for confirmation (0 for immediate).")
            return None
...
class EhlersSupertrendCrossStrategy(BaseStrategy):
    ...
    async def calculate_indicators(self, data: Dict[str, pd.DataFrame]):
        ...
        if df is None or df.empty or len(df) < min_data_needed:
            logger.debug(f"[{self.symbol}] Insufficient data for Ehlers Supertrend calculation ({len(df) if df is not None else 0} < {min_data_needed} candles).")
            return
        ...
        if df_filtered.empty or len(df_filtered) < self.atr_period:  # Ensure enough data for ATR after filtering
            logger.debug(f"[{self.symbol}] DataFrame too small after filtering for Ehlers Supertrend calculation ({len(df_filtered)} < {self.atr_period} candles).")
            return
        ...
        low_close_filtered = np.abs(df_filtered['low'] - df_filtered['filtered_close'].shift()) # Corrected here: should be filtered_close
        ...
        if df_filtered['atr_filtered'].isnull().all() or (not df_filtered['atr_filtered'].empty and df_filtered['atr_filtered'].iloc[-1] == 0):
            logger.debug(f"[{self.symbol}] ATR filtered values are all NaN or last ATR is zero, cannot calculate Supertrend.")
            return
        ...
        df_filtered['supertrend_fast_line'] = np.nan # Initialize Supertrend line
        for current in range(1, len(df_filtered.index)):
            ...
            if df_filtered.loc[df_filtered.index[current], 'in_uptrend_fast']:
                prev_supertrend = df_filtered.loc[df_filtered.index[previous], 'supertrend_fast_line'] if not np.isnan(df_filtered.loc[df_filtered.index[previous], 'supertrend_fast_line']) else df_filtered.loc[df_filtered.index[current], 'lowerband_fast']
                df_filtered.loc[df_filtered.index[current], 'supertrend_fast_line'] = max(df_filtered.loc[df_filtered.index[current], 'lowerband_fast'], prev_supertrend)
            else:
                prev_supertrend = df_filtered.loc[df_filtered.index[previous], 'supertrend_fast_line'] if not np.isnan(df_filtered.loc[df_filtered.index[previous], 'supertrend_fast_line']) else df_filtered.loc[df_filtered.index[current], 'upperband_fast']
                df_filtered.loc[df_filtered.index[current], 'supertrend_fast_line'] = min(df_filtered.loc[df_filtered.index[current], 'upperband_fast'], prev_supertrend)
        ...
        for current in range(1, len(df_filtered.index)):
            ...
            if df_filtered.loc[df_filtered.index[current], 'in_uptrend_slow']:
                prev_supertrend = df_filtered.loc[df_filtered.index[previous], 'supertrend_slow_line'] if not np.isnan(df_filtered.loc[df_filtered.index[previous], 'supertrend_slow_line']) else df_filtered.loc[df_filtered.index[current], 'lowerband_slow']
                df_filtered.loc[df_filtered.index[current], 'supertrend_slow_line'] = max(df_filtered.loc[df_filtered.index[current], 'lowerband_slow'], prev_supertrend)
            else:
                prev_supertrend = df_filtered.loc[df_filtered.index[previous], 'supertrend_slow_line'] if not np.isnan(df_filtered.loc[df_filtered.index[previous], 'supertrend_slow_line']) else df_filtered.loc[df_filtered.index[current], 'upperband_slow']
                df_filtered.loc[df_filtered.index[current], 'supertrend_slow_line'] = min(df_filtered.loc[df_filtered.index[current], 'upperband_slow'], prev_supertrend)
    ...
    async def generate_signal(self) -> Optional[StrategySignal]:
        ...
        df_indicators = self.indicators.get(self.primary_timeframe)
        if df_indicators is None or df_indicators.empty or len(df_indicators) < 2:
            logger.debug(f"[{self.symbol}] Insufficient indicator data for Ehlers Supertrend Cross signal generation.")
            return None

        # Drop any NaN rows that might result from indicator calculation to get clean data for signal
        df_cleaned = df_indicators.dropna(subset=['supertrend_fast', 'supertrend_slow', 'atr_filtered', 'filtered_close', 'in_uptrend_slow']).copy()
        if df_cleaned.empty or len(df_cleaned) < 2:
            logger.debug(f"[{self.symbol}] DataFrame too small after dropping NaNs for Ehlers Supertrend Cross signal generation from indicators.")
            return None
        ...
        if self.last_signal:
            if (self.last_signal.action == 'BUY' and not in_uptrend_overall) or \
               (self.last_signal.action == 'SELL' and in_uptrend_overall):
                logger.info(f"[{self.symbol}] Overall trend based on slow ST changed opposite to pending signal ({self.last_signal.action}), resetting pending signal.")
                self.last_signal = None
                self.signal_confirmed = False
                self.signal_candle_time = None
                return None
        ...
        if signal_to_generate:
            self.last_signal = signal_to_generate
            self.signal_candle_time = current.name
            logger.info(f"[{self.symbol}] PENDING TRADE SIGNAL: {signal_to_generate.action} for {self.symbol}. Reason: {signal_to_generate.metadata['reason']}. SL: {signal_to_generate.stop_loss:.5f}, TP: {signal_to_generate.take_profit if signal_to_generate.take_profit else 'N/A':.5f}. Waiting for confirmation (0 for immediate).")
            return None
...
class BybitTradingBot:
    ...
    def __init__(self, config: Config, strategies: Dict[str, BaseStrategy], session: Optional[HTTP] = None):
        self.config = config
        ...
        self.market_data: Dict[str, Dict[str, pd.DataFrame]] = {symbol: {} for symbol in config.symbols} # Raw kline data per symbol and timeframe
        self.positions: Dict[str, Optional[Position]] = {symbol: None for symbol in config.symbols} # Position per symbol
        self.balance: Decimal = to_decimal('0')  # Initialize balance with Decimal context
        self.is_running = False
        self.loop: Optional[asyncio.AbstractEventLoop] = None
        self.last_processed_candle_time: Dict[str, Optional[datetime]] = {symbol: None for symbol in config.symbols} # Last candle time per symbol
    ...
    async def initialize(self):
        ...
        for symbol in self.config.symbols:
            ...
            await self._load_historical_data(symbol) # Load historical for primary timeframe for each symbol
    ...
    async def _load_market_info(self, symbol: str):
        ...
        try:
            ...
            if response and response['retCode'] == 0:
                ...
                logger.info(f"[{symbol}] Market info loaded: Tick Size {self.market_info[symbol].tick_size}, Lot Size {self.market_info[symbol].lot_size}")
            else:
                raise Exception(f"Failed to get instrument info for {symbol}: {response.get('retMsg', 'Unknown error')}")
        except Exception as e:
            logger.critical(f"[{symbol}] Critical Error loading market info: {e}", exc_info=True)
    ...
    async def _load_historical_data(self, symbol: str):
        ...
        logger.info(f"[{symbol}] Loading historical data for {self.config.timeframe} timeframe...")
        try:
            ...
            if response and response['retCode'] == 0:
                ...
                df['time'] = pd.to_datetime(df['time'].astype(int), unit='ms', utc=True) # Ensure 'time' is int before unit='ms'
                df.set_index('time', inplace=True)
                df = df.astype(float)
                df.sort_index(inplace=True)
                
                # Store data under symbol -> timeframe
                if symbol not in self.market_data: # Defensive check, should be initialized in __init__
                    self.market_data[symbol] = {}
                self.market_data[symbol][self.config.timeframe] = df
                
                if not df.empty:
                    self.last_processed_candle_time[symbol] = df.index[-1]
                logger.info(f"[{symbol}] Loaded {len(df)} historical candles. Last candle: {self.last_processed_candle_time.get(symbol)}")
            else:
                raise Exception(f"Failed to get kline data for {symbol}: {response.get('retMsg', 'Unknown error')}")
        except Exception as e:
            logger.critical(f"[{symbol}] Critical Error loading historical data: {e}", exc_info=True)
    ...
    async def place_market_order(self, symbol: str, side: OrderSide, quantity: float, stop_loss: Optional[float] = None, take_profit: Optional[float] = None) -> Optional[str]:
        ...
        return await self._place_single_order(symbol, side, quantity, OrderType.MARKET, stop_loss=stop_loss, take_profit=take_profit)

    async def place_limit_order(self, symbol: str, side: OrderSide, quantity: float, price: float, stop_loss: Optional[float] = None, take_profit: Optional[float] = None) -> Optional[str]:
        """Place a limit order for a specific symbol."""
        return await self._place_single_order(symbol, side, quantity, OrderType.LIMIT, price=price, stop_loss=stop_loss, take_profit=take_profit)

    async def update_stop_loss_and_take_profit(self, symbol: str, position_side: str, new_stop_loss: Optional[Decimal] = None, new_take_profit: Optional[Decimal] = None):
        ...
        if current_position is None or current_position.side != position_side:
            logger.warning(f"[{symbol}] No active {position_side} position found for SL/TP update. Current position: {current_position}")
            return
        ...
        if new_stop_loss is not None:
            ...
            if current_position.trailing_stop_loss is None or new_stop_loss != current_position.trailing_stop_loss:
                params["stopLoss"] = market_info.format_price(float(new_stop_loss))
                params["slTriggerBy"] = "MarkPrice"
                logger.info(f"[{symbol}] Updating stop loss for {current_position.side} position from {current_position.trailing_stop_loss:.5f if current_position.trailing_stop_loss else 'N/A'} to {new_stop_loss:.5f}")
                current_position.trailing_stop_loss = new_stop_loss # Update internal state immediately
                updated_any = True
            else:
                logger.debug(f"[{symbol}] New stop loss {new_stop_loss:.5f} is same as current {current_position.trailing_stop_loss:.5f}, skipping SL update.")

        # Check and update Take Profit
        if new_take_profit is not None:
            # Only update if the new TP is different from the current effective TP on record
            if current_position.take_profit is None or new_take_profit != current_position.take_profit:
                params["takeProfit"] = market_info.format_price(float(new_take_profit))
                params["tpTriggerBy"] = "MarkPrice"
                logger.info(f"[{symbol}] Updating take profit for {current_position.side} position from {current_position.take_profit:.5f if current_position.take_profit else 'N/A'} to {new_take_profit:.5f}")
                current_position.take_profit = new_take_profit # Update internal state immediately
                updated_any = True
            else:
                logger.debug(f"[{symbol}] New take profit {new_take_profit:.5f} is same as current {current_position.take_profit:.5f}, skipping TP update.")

        if not updated_any:
            logger.debug(f"[{symbol}] No changes in stop loss or take profit requested, skipping API call.")
            return

        try:
            response = self.session.set_trading_stop(**params)
            if response and response['retCode'] == 0:
                logger.info(f"[{symbol}] Successfully sent SL/TP update request for {current_position.symbol} {current_position.side} position.")
            else:
                error_msg = response.get('retMsg', 'Unknown error') if response else 'No response from API'
                logger.error(f"[{symbol}] Failed to update SL/TP: {error_msg} (Code: {response.get('retCode', 'N/A')})")
        except Exception as e:
            ...
            if response and response['retCode'] == 0 and response['result']['list']:
                ...
                for pos_data in response['result']['list']:
                    ...
                    if symbol not in self.config.symbols:
                        logger.warning(f"[{symbol}] Found open position for {symbol} which is not in configured symbols. Ignoring.")
                    ...
                    if size > 0:
                        ...
                        existing_initial_sl = existing_pos_state.initial_stop_loss if existing_pos_state and existing_pos_state.side == current_position_side else None
                        existing_entry_signal_price = existing_pos_state.entry_signal_price if existing_pos_state and existing_pos_state.side == current_position_side else None

                        # Use Bybit API's reported SL/TP if available, otherwise fall back to saved internal state.
                        bybit_sl = to_decimal(pos_data['stopLoss']) if 'stopLoss' in pos_data and pos_data['stopLoss'] else None
                        bybit_tp = to_decimal(pos_data['takeProfit']) if 'takeProfit' in pos_data and pos_data['takeProfit'] else None

                        final_trailing_sl = bybit_sl if bybit_sl is not None else (existing_pos_state.trailing_stop_loss if existing_pos_state and existing_pos_state.side == current_position_side else None)
                        final_take_profit = bybit_tp if bybit_tp is not None else (existing_pos_state.take_profit if existing_pos_state and existing_pos_state.side == current_position_side else None)
                        ...
                        logger.debug(f"[{symbol}] Position updated from API: {self.positions[symbol]}")
                    else:
                        self.positions[symbol] = None # No open position for this symbol from API
                        logger.debug(f"[{symbol}] No active position found for {symbol} via API response (size zero).")
                ...
                for symbol_in_config in self.config.symbols:
                    if symbol_in_config not in active_symbols_from_api and self.positions.get(symbol_in_config) is not None:
                        logger.info(f"[{symbol_in_config}] Position closed on exchange, updating internal state to None.")
                        self.positions[symbol_in_config] = None


            else:
                # If API returns no list or an error, assume no positions for any configured symbols
                logger.info("No active positions reported by API for any symbols, resetting all internal states to None.")
                self.positions = {s: None for s in self.config.symbols} # Reset all positions
    ...
    def _calculate_position_size(self, signal: StrategySignal) -> float:
        ...
        if signal.stop_loss is None or signal.signal_price is None:
            logger.error(f"[{symbol}] Cannot calculate position size without a valid stop-loss and signal price from the strategy.")
            return 0.0

        if symbol not in self.market_info:
            logger.error(f"[{symbol}] Market info not loaded for {symbol}, cannot calculate position size.")
            return 0.0
        ...
        if stop_loss_distance_raw <= to_decimal(0) or stop_loss_distance_raw < market_info.tick_size:
            logger.warning(f"[{symbol}] Stop loss distance is non-positive or too small ({stop_loss_distance_raw:.5f} < {market_info.tick_size}), cannot calculate position size safely. Returning 0.0.")
            return 0.0
        ...
        leveraged_position_size_asset = position_size_asset_unleveraged * to_decimal(self.config.leverage)
        
        # Format to market's lot size precision
        formatted_position_size = to_decimal(market_info.format_quantity(float(leveraged_position_size_asset)))
        ...
        if order_value_usdt < to_decimal(self.config.min_order_value_usdt):
            logger.warning(f"[{symbol}] Calculated order value ({order_value_usdt:.2f} USDT) is below minimum required ({self.config.min_order_value_usdt} USDT). Skipping trade.")
            return 0.0

        logger.info(f"[{symbol}] Risk Amount: {risk_amount_usdt:.2f} USDT, SL Distance: {stop_loss_distance_raw:.5f} USDT, "
    ...
    async def process_signal(self, signal: StrategySignal):
        ...
        symbol = signal.symbol
        logger.info(f"[{symbol}] Processing signal: {signal.action} {symbol} (Reason: {signal.metadata.get('reason', 'N/A')})")
        
        # Access market data for the specific symbol and timeframe
        symbol_market_data_for_timeframe = self.market_data.get(symbol, {}).get(self.config.timeframe)
        if symbol_market_data_for_timeframe is None or symbol_market_data_for_timeframe.empty:
            logger.warning(f"[{symbol}] No market data available for signal processing, skipping.")
            return

        current_close_price = symbol_market_data_for_timeframe.iloc[-1]['close']
        current_position = self.positions.get(symbol)
        current_strategy = self.strategies.get(symbol) # Ensure current_strategy is fetched for this symbol

        if current_strategy is None:
            logger.error(f"[{symbol}] No strategy instance found for this symbol, cannot process signal.")
            return

        # Dynamically set/refine Take Profit if not provided by strategy or if ATR changes
        df_indicators = current_strategy.indicators.get(self.config.timeframe) # Get indicators for THIS symbol's strategy
        ...
        if signal.take_profit is None and signal.signal_price is not None and current_atr > 0:
            tp_multiplier = to_decimal(self.config.strategy_params.get("take_profit_atr_multiplier", 1.0)) # Scalping tuned default
            signal_price_dec = to_decimal(signal.signal_price)
            ...
            if signal.action == 'BUY':
                ...
                signal.take_profit = float(signal_price_dec - current_atr_dec * tp_multiplier)
            logger.info(f"[{symbol}] Dynamically calculated Take Profit: {signal.take_profit:.5f}")


        position_size = self._calculate_position_size(signal)
        if position_size <= 0:
            logger.warning(f"[{symbol}] Calculated position size is zero or too small, aborting trade.")
            return
        ...
        if current_position:
            ...
            if (signal.action == 'BUY' and current_position.side == 'Sell') or \
               (signal.action == 'SELL' and current_position.side == 'Buy'):
                logger.info(f"[{symbol}] Reversing position: Closing existing {current_position.side} ({current_position.size:.5f}) to open {signal.action} ({position_size:.5f}).")
                
                # Close existing position first
                close_side = OrderSide.BUY if current_position.side == 'Sell' else OrderSide.SELL
                close_order_id = await self.place_market_order(symbol=symbol, side=close_side, quantity=float(current_position.size))
                
                if close_order_id:
                    logger.info(f"[{symbol}] Close order {close_order_id} placed. Waiting for position to settle before opening new one...")
                    ...
                    if self.positions.get(symbol) is None: # Successfully closed, now open new one
                        logger.info(f"[{symbol}] Existing position successfully closed. Proceeding to open new one.")
                        ...
                        if new_order_id:
                            ...
                            self.positions[symbol] = Position(
                                symbol=symbol, side=signal.action, size=to_decimal(position_size),
                                avg_price=to_decimal(current_close_price), unrealized_pnl=to_decimal('0'),
                                mark_price=to_decimal(current_close_price), leverage=self.config.leverage,
                                entry_signal_price=to_decimal(signal.signal_price) if signal.signal_price else None,
                                initial_stop_loss=to_decimal(signal.stop_loss) if signal.stop_loss else None,
                                trailing_stop_loss=to_decimal(signal.stop_loss) if signal.stop_loss else None, # Initial trailing SL is the provided SL
                                take_profit=to_decimal(signal.take_profit) if signal.take_profit else None
                            )
                            self._save_state()
                        else:
                            logger.error(f"[{symbol}] Failed to open new position after closing existing one. Check logs for details.")
                    else:
                        logger.error(f"[{symbol}] Failed to confirm closure of existing position. Aborting new trade to prevent conflicting positions.")
                else:
                    logger.error(f"[{symbol}] Failed to place order to close existing position. Aborting new trade.")
                return # Handled reversing position

            # Scenario 2: Signal is in the same direction as the current position (ADJUST SL/TP)
            elif (signal.action == 'BUY' and current_position.side == 'Buy') or \
                 (signal.action == 'SELL' and current_position.side == 'Sell'):
                logger.info(f"[{symbol}] Signal to {signal.action} received, but already in a {current_position.side} position. Considering updating SL/TP.")
                ...
                if signal.stop_loss is not None:
                    ...
                    if current_position.initial_stop_loss is None: 
                        should_update_initial_sl = True
                    elif (current_position.side == 'Buy' and new_signal_sl > current_position.initial_stop_loss):
                        should_update_initial_sl = True
                    elif (current_position.side == 'Sell' and new_signal_sl < current_position.initial_stop_loss):
                        should_update_initial_sl = True

                    if should_update_initial_sl:
                        current_position.initial_stop_loss = new_signal_sl
                        logger.info(f"[{symbol}] Internal initial stop loss updated to new strategy SL: {new_signal_sl:.5f}")
                        ...
                        if current_position.trailing_stop_loss is None or \
                           (current_position.side == 'Buy' and new_signal_sl > current_position.trailing_stop_loss) or \
                           (current_position.side == 'Sell' and new_signal_sl < current_position.trailing_stop_loss):
                           new_sl_to_set = new_signal_sl
                           logger.info(f"[{symbol}] Trailing stop loss on exchange will be set to new initial SL: {new_sl_to_set:.5f}")
                ...
                if signal.take_profit is not None:
                    ...
                    if current_position.take_profit is None:
                        should_update_tp = True
                    elif (current_position.side == 'Buy' and new_signal_tp > current_position.take_profit):
                        should_update_tp = True
                    elif (current_position.side == 'Sell' and new_signal_tp < current_position.take_profit):
                        should_update_tp = True
                    
                    if should_update_tp:
                        new_tp_to_set = new_signal_tp
                        logger.info(f"[{symbol}] Take profit updated to a more favorable level: {new_tp_to_set:.5f}")
                
                if new_sl_to_set or new_tp_to_set:
                    await self.update_stop_loss_and_take_profit(symbol, current_position.side, new_sl_to_set, new_tp_to_set)
                else:
                    logger.debug(f"[{symbol}] No beneficial SL/TP updates from current signal, skipping API call.")
                return # Handled adjusting position

        # Scenario 3: No current position, open a new one
        if not current_position:
            logger.info(f"[{symbol}] Opening new {signal.action} position with size {position_size:.5f}.")
            ...
            if order_id:
                ...
                logger.error(f"[{symbol}] Failed to open new position. Check logs for details.")
    ...
    async def _update_trailing_stop_loss(self, symbol: str):
        ...
        if current_position is None or current_position.entry_signal_price is None or current_position.initial_stop_loss is None or \
           current_strategy is None or current_strategy.indicators.get(self.config.timeframe) is None or current_strategy.indicators[self.config.timeframe].empty:
            logger.debug(f"[{symbol}] Cannot update trailing stop: no position, no entry price/initial SL, or no indicators.")
            return
        ...
        current_atr = to_decimal(current_df_indicators.iloc[-1].get('atr_filtered', current_df_indicators.iloc[-1].get('atr', 0.0)))
        if current_atr <= to_decimal(0):
            logger.warning(f"[{symbol}] ATR not available or non-positive for trailing stop calculation ({current_atr}).")
            return

        activation_multiplier = to_decimal(self.config.strategy_params.get("trailing_stop_loss_activation_atr_multiplier", 0.5)) # Scalping tuned
        trailing_multiplier = to_decimal(self.config.strategy_params.get("trailing_stop_loss_atr_multiplier", 0.75)) # Scalping tuned
        break_even_profit_multiplier = to_decimal(self.config.strategy_params.get("break_even_profit_atr_multiplier", 0.2)) # Scalping tuned
        ...
        if current_position.side == 'Buy':
            ...
            profit_in_usd = current_position.entry_signal_price - current_price # Corrected: profit for sell is entry - current
        
        profit_in_atr = profit_in_usd / current_atr if current_atr > to_decimal(0) else to_decimal(0)

        # Determine potential new trailing stop price
        potential_new_stop_price: Optional[Decimal] = None
        current_trailing_sl = current_position.trailing_stop_loss # The last SL value we set on the exchange
        initial_sl = current_position.initial_stop_loss # The initial SL from the strategy signal
        ...
        if current_position.side == 'Buy':
            ...
            if profit_in_atr >= break_even_profit_multiplier:
                ...
                calculated_be_sl = current_position.entry_signal_price + (current_atr * to_decimal('0.05'))
        ...
        if potential_new_stop_price is not None:
            ...
            elif current_trailing_sl == potential_new_stop_price: # No actual change, no need to update
                should_update_exchange = False
            elif current_position.side == 'Buy' and potential_new_stop_price > current_trailing_sl: # Move SL up for a Buy
                should_update_exchange = True
            elif current_position.side == 'Sell' and potential_new_stop_price < current_trailing_sl: # Move SL down for a Sell
                should_update_exchange = True
            ...
            if should_update_exchange:
                ...
                logger.info(f"[{symbol}] Trailing SL update triggered for {current_position.side} position. Old: {current_trailing_sl:.5f if current_trailing_sl else 'N/A'}, New: {potential_new_stop_price:.5f}")
                await self.update_stop_loss_and_take_profit(symbol, current_position.side, new_stop_loss=potential_new_stop_price)
            else:
                logger.debug(f"[{symbol}] Calculated trailing stop {potential_new_stop_price:.5f} is not better than current {current_trailing_sl:.5f if current_trailing_sl else 'N/A'}, skipping update.")
        else:
            logger.debug(f"[{symbol}] Profit {profit_in_atr:.2f} ATR. Trailing stop conditions not met yet or no beneficial move calculated.")
    ...
    def _handle_kline_message(self, message):
        ...
        try:
            ...
            symbol = parts[2]
            if symbol not in self.config.symbols:
                logger.debug(f"Received kline for unconfigured symbol {symbol}. Ignoring.")
                return
            ...
            if symbol not in self.market_data: # This should be initialized in __init__, but defensive check
                self.market_data[symbol] = {}
            if self.config.timeframe not in self.market_data[symbol]:
                ...
                logger.warning(f"[{symbol}] Market data DataFrame for timeframe {self.config.timeframe} not found. Initializing empty DataFrame for WS. Indicators might produce NaNs initially.")
                self.market_data[symbol][self.config.timeframe] = pd.DataFrame(columns=["open", "high", "low", "close", "volume", "turnover"], index=pd.to_datetime([], utc=True)) # Empty DF with correct columns and index type
            ...
            if self.last_processed_candle_time.get(symbol) and current_candle_time < self.last_processed_candle_time[symbol]:
                logger.debug(f"[{symbol}] Skipping older/already processed kline message for {current_candle_time} (last processed: {self.last_processed_candle_time[symbol]})")
                return
            ...
            if current_candle_time in df.index:
                ...
                logger.debug(f"[{symbol}] Updated existing candle data for {current_candle_time}.")
            else:
                ...
                if current_strategy:
                    required_lookback = max(
                        ...
                        getattr(current_strategy, 'fast_st_period', 0), # Safe access for Ehlers
                        getattr(current_strategy, 'slow_st_period', 0), # Safe access for Ehlers
                        (getattr(current_strategy, 'filter_poles', 0) * 2) # For recursive filter warm-up (0 if not Ehlers)
                    )
                else:
                    logger.warning(f"[{symbol}] Strategy not found for dynamic lookback calculation, using default config lookback.")

                df = df.iloc[-required_lookback:].copy() 
                self.last_processed_candle_time[symbol] = current_candle_time # Update last processed time for new candles
                logger.debug(f"[{symbol}] Appended new candle: {current_candle_time}. DataFrame size: {len(df)}")

            self.market_data[symbol][self.config.timeframe] = df
            
            # Schedule strategy cycle and trailing stop update for THIS symbol as a task thread-safely
            self.loop.call_soon_threadsafe(asyncio.create_task, self._async_kline_processing(symbol))
    ...
    async def _async_kline_processing(self, symbol: str):
        ...
        if current_strategy and symbol_market_data_for_timeframe is not None and not symbol_market_data_for_timeframe.empty:
            ...
            if signal and signal.action != 'HOLD':
                ...
                logger.debug(f"[{symbol}] No actionable signal generated or signal to HOLD.")
            ...
            if df_indicators is not None and not df_indicators.empty:
                ...
                if current_strategy.__class__.__name__ == "SupertrendStrategy":
                    ...
                    logger.info(f"[{symbol}] Current Price: {current_close:.5f}, Supertrend: {supertrend_value:.5f}")
                elif current_strategy.__class__.__name__ == "EhlersSupertrendCrossStrategy":
                    fast_st = df_indicators.iloc[-1].get('supertrend_fast', np.nan)
                    slow_st = df_indicators.iloc[-1].get('supertrend_slow', np.nan)
                    logger.info(f"[{symbol}] Current Price: {current_close:.5f}, Fast ST: {fast_st:.5f}, Slow ST: {slow_st:.5f}")
            else:
                logger.debug(f"[{symbol}] No indicators available yet for {self.config.timeframe} after calculation. Raw data size: {len(self.market_data[symbol].get(self.config.timeframe, {}).get(self.config.timeframe, []))}")
        else:
            logger.debug(f"[{symbol}] Skipping _async_kline_processing: strategy or sufficient market data not available.")
    ...
    def _handle_private_message(self, message):
        ...
        try:
            ...
            if topic == 'position':
                ...
                self.loop.call_soon_threadsafe(asyncio.create_task, self.get_positions())
            elif topic == 'order':
    ...
    async def run_strategies_cycle(self):
        ...
        """
        logger.debug("`run_strategies_cycle` called. This method is deprecated; kline messages directly trigger strategy processing.")
    ...
    def _save_state(self):
        ...
        state = {
            'positions': {s: asdict(p) if p else None for s, p in self.positions.items()},
            ...
            'strategies_state': {
                symbol: {
                    ...
                    'signal_candle_time': s.signal_candle_time.isoformat() if s.signal_candle_time else None,
                } for symbol, s in self.strategies.items()
            },
            'last_processed_candle_time': {s: t.isoformat() if t else None for s, t in self.last_processed_candle_time.items()},
        }
        try:
        ...
        try:
            if os.path.exists('bot_state.pkl'):
                with open('bot_state.pkl', 'rb') as f:
                    ...
                    if not isinstance(loaded_positions, dict): # Defensive check for malformed state
                        logger.warning(f"Unexpected type for 'positions' in state file: {type(loaded_positions)}. Resetting.")
                        loaded_positions = {}

                    for symbol in self.config.symbols: # Iterate over configured symbols
                        ...
                        if pos_data:
                            try:
                                ...
                                self.positions[symbol] = Position(
                                    ...
                                    unrealized_pnl=to_decimal(pos_data['unrealized_pnl']),
                                    mark_price=to_decimal(pos_data['mark_price']),
                                    leverage=pos_data['leverage'],
                                    entry_signal_price=to_decimal(pos_data['entry_signal_price']) if pos_data.get('entry_signal_price') else None,
                                    initial_stop_loss=to_decimal(pos_data['initial_stop_loss']) if pos_data.get('initial_stop_loss') else None,
                                    trailing_stop_loss=to_decimal(pos_data['trailing_stop_loss']) if pos_data.get('trailing_stop_loss') else None,
                                    take_profit=to_decimal(pos_data['take_profit']) if pos_data.get('take_profit') else None
                                )
                            except Exception as e:
                                logger.error(f"[{symbol}] Error loading position data from state: {e}. Resetting position for this symbol.", exc_info=True)
                    ...
                    if not isinstance(loaded_strategies_state, dict): # Defensive check
                        logger.warning(f"Unexpected type for 'strategies_state' in state file: {type(loaded_strategies_state)}. Resetting.")
                        loaded_strategies_state = {}

                    for symbol, strat_state in loaded_strategies_state.items():
                        if symbol in self.strategies: # Only load for actively configured strategies
                            try:
                                # Access strategy instance and load its state
                                strategy_instance = self.strategies[symbol]
                                if strat_state.get('last_signal'):
                                    strategy_instance.last_signal = StrategySignal(**strat_state['last_signal'])
                                strategy_instance.signal_confirmed = strat_state.get('signal_confirmed', False)
                                if strat_state.get('signal_candle_time'):
                                    strategy_instance.signal_candle_time = datetime.fromisoformat(strat_state['signal_candle_time'])
                            except Exception as e:
                                logger.error(f"[{symbol}] Error loading strategy state: {e}. Resetting state for this strategy.", exc_info=True)
                                strategy_instance.last_signal = None
                                strategy_instance.signal_confirmed = False
                                strategy_instance.signal_candle_time = None
                        else:
                            logger.warning(f"Strategy state for unconfigured symbol {symbol} found in state file. Skipping.")


                    # Load last processed candle times
                    loaded_last_processed_times = state.get('last_processed_candle_time', {})
                    ...
                    if isinstance(loaded_last_processed_times, dict):
                        for symbol in self.config.symbols: # Iterate over configured symbols
                            ...
                            if time_str:
                                try:
                                    self.last_processed_candle_time[symbol] = datetime.fromisoformat(time_str)
                                except ValueError:
                                    logger.error(f"[{symbol}] Invalid datetime format for last_processed_candle_time: '{time_str}'. Resetting to None.", exc_info=True)
                                    self.last_processed_candle_time[symbol] = None
                            else:
                                 self.last_processed_candle_time[symbol] = None # Explicitly set None if string was None
                    else:
                        logger.warning(f"Unexpected type for last_processed_candle_time in state file: {type(loaded_last_processed_times)}. Resetting to fresh.")
                        self.last_processed_candle_time = {symbol: None for symbol in self.config.symbols}
            ...
            self.positions = {symbol: None for symbol in self.config.symbols}
    ...
    async def start(self):
        ...
        for symbol in self.config.symbols:
            ...
            logger.info(f"Subscribed to kline stream for {symbol} on {self.config.timeframe}.")
        ...
        logger.info("Trading bot started successfully. Waiting for market data and signals (driven by kline updates)...")

        # Keep the main task alive. Core logic is now event-driven by WebSocket kline messages.
        while self.is_running:
            await asyncio.sleep(1)  # Sleep to prevent busy-waiting, can add other high-level checks here
...
if __name__ == "__main__":
    ...
    for symbol in bot_config.symbols:
        if bot_config.strategy_name == "Supertrend":
            all_strategies[symbol] = SupertrendStrategy(symbol=symbol, config=bot_config)
        elif bot_config.strategy_name == "EhlersSupertrendCross":
            all_strategies[symbol] = EhlersSupertrendCrossStrategy(symbol=symbol, config=bot_config)
        else:
            logger.critical(f"Unknown strategy specified in config: {bot_config.strategy_name}. Exiting.")
            sys.exit(1)
        
    bot = BybitTradingBot(config=bot_config, strategies=all_strategies) # Pass the dictionary of strategies
