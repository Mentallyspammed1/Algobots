...
class Config:
    ...
    risk_per_trade_pct: float = (
        0.01  # 1% of equity risked per trade (float for easy calculation)
    )
    leverage: int = 10
    ...
    # Strategy Parameters
    strategy_name: str = (
        "EhlersSupertrendCross"  # Can be "Supertrend" or "EhlersSupertrendCross"
    )
    timeframe: str = (
        "15"  # Primary timeframe for the strategy (e.g., "1", "5", "15", "60", "D")
    )
    lookback_periods: int = (
        400  # Number of historical candles to load (increased for Ehlers filter)
    )
        ...
        if self.last_signal and not self.signal_confirmed and self.signal_candle_time:
            ...
            if df is not None and not df.empty:
                try:
                    ...
                    signal_idx = df.index.get_loc(
                        self.signal_candle_time, method="bfill"
                    )
...
class SupertrendStrategy(BaseStrategy):
    ...
    async def calculate_indicators(self, data: dict[str, pd.DataFrame]):
        ...
        if (
            df is None
            or df.empty
            or len(df) < max(self.supertrend_period, self.atr_period)
        ):
            logger.debug(
                f"Insufficient data for Supertrend calculation: {len(df) if df is not None else 0} candles."
            )
            return
        ...
        self.indicators[self.primary_timeframe] = (
            df.copy()
        )  # Store a copy to avoid SettingWithCopyWarning
    ...
    async def generate_signal(
        ...
        if self.last_signal and (
            (self.last_signal.action == "BUY" and not current["in_uptrend"])
            or (self.last_signal.action == "SELL" and current["in_uptrend"])
        ):
            logger.info(
                f"Trend changed opposite to last signal ({self.last_signal.action}), resetting pending signal."
            )
        ...
        ):  # Only generate a new signal if no active pending signal
        ...
        if signal_to_return:
            ...
            logger.info(
                f"PENDING TRADE SIGNAL: {signal_to_return.action} for {self.symbol}. Reason: {signal_to_return.metadata['reason']}. SL: {signal_to_return.stop_loss:.5f}. Waiting for confirmation."
            )
...
class EhlersSupertrendCrossStrategy(BaseStrategy):
    ...
    async def calculate_indicators(self, data: dict[str, pd.DataFrame]):
        ...
        if (
            df is None
            or df.empty
            or len(df) < max(self.fast_st_period, self.slow_st_period, self.atr_period)
        ):
            logger.debug(
                f"Insufficient data for Ehlers Supertrend calculation: {len(df) if df is not None else 0} candles."
            )
            return
        ...
        df_filtered = df.dropna(
            subset=["filtered_close", "filtered_high", "filtered_low"]
        ).copy()
        ...
        if (
            ...
            logger.debug(
                f"DataFrame too small after filtering for Ehlers Supertrend calculation: {len(df_filtered)} candles."
            )
            return
        ...
        if df_filtered["atr_filtered"].isnull().all():
            logger.debug(
                "ATR filtered values are all NaN, cannot calculate Supertrend."
            )
            return
    ...
    async def generate_signal(
        ...
        df_cleaned = df.dropna(
            subset=["supertrend_fast", "supertrend_slow", "atr_filtered"]
        ).copy()
        if df_cleaned.empty or len(df_cleaned) < 2:
            logger.debug(
                "DataFrame too small after dropping NaNs for signal generation."
        ...
        if self.last_signal:
            ...
            if (self.last_signal.action == "BUY" and not in_uptrend_overall) or (
                ...
                logger.info(
                    "Overall trend based on slow ST changed, resetting pending signal."
                )
                self.last_signal = None
                self.signal_confirmed = False
                self.signal_candle_time = None
        ...
        if (
            ...
            if (
                ...
                signal_to_return = StrategySignal(
                    ...
                    signal_price=float(current["close"]),
                    metadata={
                        "reason": "Ehlers Fast ST Crosses Above Slow ST (Uptrend)"
                    },
                )
            ...
            # Sell signal: Fast ST crosses below Slow ST and overall trend is down
                ...
                signal_to_return = StrategySignal(
                    ...
                    signal_price=float(current["close"]),
                    metadata={
                        "reason": "Ehlers Fast ST Crosses Below Slow ST (Downtrend)"
                    },
                )
        ...
        if signal_to_return:
            ...
            self.signal_candle_time = current.name
            logger.info(
                f"PENDING TRADE SIGNAL: {signal_to_return.action} for {self.symbol}. Reason: {signal_to_return.metadata['reason']}. SL: {signal_to_return.stop_loss:.5f}, TP: {signal_to_return.take_profit if signal_to_return.take_profit else 'N/A':.5f}. Waiting for confirmation."
            )
            return None  # Always wait for confirmation
...
class BybitTradingBot:
    ...
    def __init__(
        self, config: Config, strategy: BaseStrategy, session: HTTP | None = None
    ):
        self.config = config
        ...
        self.balance: Decimal = Decimal(
            "0", DECIMAL_CONTEXT
        )  # Initialize balance with Decimal context
    ...
    async def _load_market_info(self):
        ...
        try:
            ...
            if response and response["retCode"] == 0:
                ...
                logger.info(
                    f"Market info loaded for {self.config.symbol}: Tick Size {self.market_info.tick_size}, Lot Size {self.market_info.lot_size}"
                )
            else:
                raise Exception(
                    f"Failed to get instrument info: {response.get('retMsg', 'Unknown error')}"
                )
    ...
    async def _load_historical_data(self):
        ...
        logger.info(
            f"Loading historical data for {self.config.symbol} on {self.config.timeframe} timeframe..."
        )
        try:
            ...
            if response and response["retCode"] == 0:
                ...
                logger.info(
                    f"Loaded {len(df)} historical candles. Last candle: {self.last_processed_candle_time}"
                )
            else:
                raise Exception(
                    f"Failed to get kline data: {response.get('retMsg', 'Unknown error')}"
                )
        except Exception as e:
    ...
    async def _place_single_order(
        ...
        try:
            ...
            if response and response["retCode"] == 0:
                ...
                logger.info(
                    f"TRADE: Order placed successfully: ID {order_id}, Side {side.value}, Qty {formatted_qty}, SL: {stop_loss}, TP: {take_profit}"
                )
                return order_id
            error_msg = (
            ...
            logger.error(
                f"Failed to place order: {error_msg} (Code: {response.get('retCode', 'N/A')})"
            )
            return None
        except Exception as e:
    ...
    async def update_stop_loss_and_take_profit(
        ...
        if not self.position or self.position.side != position_side:
            logger.warning(
                f"No active {position_side} position found for SL/TP update. Current position: {self.position}"
            )
            return
        ...
        if new_stop_loss is not None:
            ...
            if (
                ...
                or new_stop_loss != self.position.trailing_stop_loss
            ):
                params["stopLoss"] = self.market_info.format_price(float(new_stop_loss))
                params["slTriggerBy"] = "MarkPrice"
                logger.info(
                    f"Updating stop loss for {self.position.side} position to {new_stop_loss:.5f}"
                )
                self.position.trailing_stop_loss = (
        ...
        if new_take_profit is not None:
            ...
            if (
                ...
                or new_take_profit != self.position.take_profit
            ):
                params["takeProfit"] = self.market_info.format_price(
                    float(new_take_profit)
                )
                params["tpTriggerBy"] = "MarkPrice"
                logger.info(
                    f"Updating take profit for {self.position.side} position to {new_take_profit:.5f}"
                )
        ...
        try:
            ...
            if response and response["retCode"] == 0:
                logger.info(
                    f"Successfully sent SL/TP update request for {self.position.symbol} {self.position.side} position."
                )
            else:
                error_msg = (
                    response.get("retMsg", "Unknown error")
                ...
                logger.error(
                    f"Failed to update SL/TP: {error_msg} (Code: {response.get('retCode', 'N/A')})"
                )
        except Exception as e:
        ...
        try:
            ...
            if response and response["retCode"] == 0 and response["result"]["list"]:
                ...
                if size > 0:
                    ...
                    existing_initial_sl = (
                        self.position.initial_stop_loss
                        if self.position and self.position.side == current_position_side
                        else None
                    )
                    existing_trailing_sl = (
                        self.position.trailing_stop_loss
                        if self.position and self.position.side == current_position_side
                        else None
                    )
                    ...
                    existing_entry_signal_price = (
                        self.position.entry_signal_price
                        if self.position and self.position.side == current_position_side
                        else None
                    )
    ...
    def _calculate_position_size(self, signal: StrategySignal) -> float:
        ...
        if signal.stop_loss is None or signal.signal_price is None:
            logger.error(
                "Cannot calculate position size without a valid stop-loss and signal price from the strategy."
            )
            return 0.0
        ...
        if signal.action == "BUY":
            stop_loss_distance_raw = Decimal(
                str(signal.signal_price), DECIMAL_CONTEXT
            ) - Decimal(str(signal.stop_loss), DECIMAL_CONTEXT)
        elif signal.action == "SELL":
            stop_loss_distance_raw = Decimal(
                str(signal.stop_loss), DECIMAL_CONTEXT
            ) - Decimal(str(signal.signal_price), DECIMAL_CONTEXT)

        if stop_loss_distance_raw <= 0:
            logger.warning(
                f"Stop loss distance is non-positive ({stop_loss_distance_raw}), cannot calculate position size safely. Returning 0."
            )
            return 0.0
        ...
        formatted_position_size = Decimal(
            self.market_info.format_quantity(float(leveraged_position_size_asset)),
            DECIMAL_CONTEXT,
        )
    ...
    async def process_signal(self, signal: StrategySignal):
        ...
        if (
            self.market_data.get(self.config.timeframe) is None
            or self.market_data[self.config.timeframe].empty
        ):
        ...
        if (
            ...
            tp_multiplier = self.config.strategy_params.get(
                "take_profit_atr_multiplier", 2.0
            )
            if signal.action == "BUY":
                signal.take_profit = float(
                    Decimal(str(signal.signal_price), DECIMAL_CONTEXT)
                    + Decimal(str(current_atr), DECIMAL_CONTEXT)
                    * Decimal(str(tp_multiplier), DECIMAL_CONTEXT)
                )
            elif signal.action == "SELL":
                signal.take_profit = float(
                    Decimal(str(signal.signal_price), DECIMAL_CONTEXT)
        ...
        if position_size <= 0:
            logger.warning(
                "Calculated position size is zero or negative, aborting trade."
            )
            return
        ...
        if self.position:
            ...
            if (signal.action == "BUY" and self.position.side == "Sell") or (
                ...
                logger.info(
                    f"Reversing position: Closing existing {self.position.side} ({self.position.size:.5f}) to open {signal.action} ({position_size:.5f})."
                )

                # Close existing position first
                close_side = (
                    OrderSide.BUY if self.position.side == "Sell" else OrderSide.SELL
                )
                close_order_id = await self.place_market_order(
                    side=close_side, quantity=float(self.position.size)
                )

                if close_order_id:
                    logger.info(
                        f"Close order {close_order_id} placed. Waiting for position to settle before opening new one..."
                    )
                    ...
                    if not self.position:  # Successfully closed
                        ...
                        if new_order_id:
                            self.position = (
                                Position(  # Temporarily update internal position state
                                    symbol=self.config.symbol,
                                    side=signal.action,
                            ...
                            logger.error(
                                "Failed to open new position after closing existing one."
                            )
                    else:
                        logger.error(
                            "Failed to close existing position. Aborting new trade."
                        )
                else:
                    logger.error(
                        "Failed to place order to close existing position. Aborting new trade."
                    )
            ...
            if (signal.action == "BUY" and self.position.side == "Buy") or (
                ...
                logger.info(
                    f"Signal to {signal.action} received, but already in a {self.position.side} position. Adjusting SL/TP if beneficial."
                )
                ...
                if signal.stop_loss:
                    ...
                    if (
                        ...
                        logger.info(
                            f"Initial stop loss updated to a more favorable level: {new_signal_sl:.5f}"
                        )
                    ...
                    if (
                        ...
                        new_sl_to_set = new_signal_sl
                        self.position.trailing_stop_loss = (
                            new_signal_sl  # Reset trailing SL
                        )
                        logger.info(
                            f"Trailing stop loss also reset to new initial SL: {new_signal_sl:.5f}"
                        )
                ...
                if signal.take_profit:
                    ...
                    if (
                        ...
                        logger.info(
                            f"Take profit updated to a more favorable level: {new_signal_tp:.5f}"
                        )
    ...
    async def _update_trailing_stop_loss(self):
        ...
        if (
            ...
            logger.debug(
                "Cannot update trailing stop: no position, no entry price, or no indicators."
            )
            return
        ...
        activation_multiplier = Decimal(
            str(
                self.config.strategy_params.get(
                    "trailing_stop_loss_activation_atr_multiplier", 1.0
                )
            ),
            DECIMAL_CONTEXT,
        )
        trailing_multiplier = Decimal(
            str(
                self.config.strategy_params.get(
                    "trailing_stop_loss_atr_multiplier", 1.0
                )
            ),
            DECIMAL_CONTEXT,
        )
        break_even_profit_multiplier = Decimal(
            str(
                self.config.strategy_params.get("break_even_profit_atr_multiplier", 0.5)
            ),
            DECIMAL_CONTEXT,
        )
        ...
        if self.position.side == "Buy":
            ...
            break_even_stop = self.position.entry_signal_price + (
                current_atr * break_even_profit_multiplier
            )
            if profit_in_atr >= break_even_profit_multiplier and (
                self.position.trailing_stop_loss is None
                or break_even_stop > self.position.trailing_stop_loss
            ):
            ...
            if profit_in_atr >= activation_multiplier:
                trailing_point_stop = current_price - (
                    current_atr * trailing_multiplier
                )
                if (
                    potential_new_stop_price is None
                    or trailing_point_stop > potential_new_stop_price
                ):
                    potential_new_stop_price = trailing_point_stop

            # Ensure SL never goes below initial SL if it was set
            if (
                potential_new_stop_price
                and self.position.initial_stop_loss
                and potential_new_stop_price < self.position.initial_stop_loss
            ):
        ...
        if potential_new_stop_price is not None:
            ...
            ):  # No current trailing SL set
                should_update_exchange = True

            if should_update_exchange:
                logger.info(
                    f"Updating trailing stop-loss for {self.position.side} position. Old: {current_trailing_sl:.5f if current_trailing_sl else 'N/A'}, New: {potential_new_stop_price:.5f}"
                )
                await self.update_stop_loss_and_take_profit(
                ...
                logger.debug(
                    f"Calculated trailing stop {potential_new_stop_price:.5f} is not better than current {current_trailing_sl:.5f if current_trailing_sl else 'N/A'}."
                )
        else:
            logger.debug(
                f"Profit {profit_in_atr:.2f} ATR. Trailing stop conditions not met yet or no beneficial move."
            )
    ...
    def _handle_kline_message(self, message):
        ...
        try:
            ...
            if (
                ...
                logger.debug(
                    f"Skipping older/already processed candle: {current_candle_time} (last processed: {self.last_processed_candle_time})"
                )
                return
            ...
            if df is None or df.empty:
                logger.warning(
                    f"Market data for {self.config.timeframe} is not initialized or empty. Waiting for more data."
                )
            ...
            if current_candle_time in df.index:
                ...
                df = df.iloc[
                    -(
                        ...
                        + max(
                            self.strategy.fast_st_period,
                            self.strategy.slow_st_period,
                            self.strategy.atr_period,
                            self.strategy.filter_poles * 2,
                        )
                    ) :
                ].copy()  # Keep enough for filtering and lookback
                self.last_processed_candle_time = (
    ...
    async def run_strategy_cycle(self):
        ...
        if (
            signal and signal.action != "HOLD"
        ):  # 'HOLD' signals are for internal strategy logic, not external actions
    ...
    def _load_state(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to load bot state: {e}. Starting fresh.", exc_info=True
            )
