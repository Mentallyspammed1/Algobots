...
class Config:
    ...
    symbols: list[str] = field(default_factory=lambda: ["XLMUSDT", "LINKUSDT", "DOTUSDT", "TRUMPUSDT"])
    category: str = "linear"
...
class EhlersSupertrendCrossStrategy(BaseStrategy):
    ...
    async def generate_signal(self, data: dict[str, pd.DataFrame]) -> StrategySignal | None:
        ...
        take_profit_multiplier = self.config.strategy_params.get("take_profit_atr_multiplier", 1.0)
...
class BybitTradingBot:
    ...
    async def _load_market_info(self, symbol: str):
        try:
            ...
            if response and response['retCode'] == 0:
                ...
                logger.info(f"Market info loaded for {symbol}: Tick Size {self.market_info[symbol].tick_size}, Lot Size {self.market_info[symbol].lot_size}")
            else:
                raise Exception(f"Failed to get instrument info for {symbol}: {response.get('retMsg', 'Unknown error')}")
        except Exception as e:
            logger.critical(f"Critical Error loading market info for {symbol}: {e}", exc_info=True)
            sys.exit(1)

    async def _load_historical_data(self, symbol: str):
        logger.info(f"Loading historical data for {symbol} on {self.config.timeframe} timeframe...")
        try:
            ...
            if response and response['retCode'] == 0:
                ...
                logger.info(f"Loaded {len(df)} historical candles for {symbol}. Last candle: {self.last_processed_candle_time.get(symbol)}")
            else:
                raise Exception(f"Failed to get kline data for {symbol}: {response.get('retMsg', 'Unknown error')}")
        except Exception as e:
            logger.critical(f"Critical Error loading historical data for {symbol}: {e}", exc_info=True)
            sys.exit(1)
    ...
    async def _place_single_order(self, symbol: str, side: OrderSide, quantity: float, order_type: OrderType,
        ...
        try:
            ...
            if float(formatted_qty) <= float(market_info.lot_size):
                logger.warning(f"Formatted quantity for order is too small ({formatted_qty}), skipping. Original: {quantity}. Minimum lot size: {market_info.lot_size}")
                return None
            ...
            if response and response['retCode'] == 0:
                ...
                logger.info(f"TRADE: Order placed successfully: ID {order_id}, Side {side.value}, Qty {formatted_qty}, SL: {stop_loss}, TP: {take_profit} for {symbol}")
                return order_id
            error_msg = response.get('retMsg', 'Unknown error') if response else 'No response from API'
            logger.error(f"Failed to place order for {symbol}: {error_msg} (Code: {response.get('retCode', 'N/A')})")
            return None
        except Exception as e:
    ...
    async def update_stop_loss_and_take_profit(self, symbol: str, position_side: str, new_stop_loss: Decimal | None = None, new_take_profit: Decimal | None = None):
        if symbol not in self.positions or self.positions[symbol] is None or self.positions[symbol].side != position_side:
            logger.warning(f"No active {position_side} position found for {symbol} for SL/TP update. Current position: {self.positions.get(symbol)}")
            return
        ...
        if new_stop_loss is not None:
            if current_position.trailing_stop_loss is None or new_stop_loss != current_position.trailing_stop_loss:
                ...
                params["slTriggerBy"] = "MarkPrice"
                logger.info(f"Updating stop loss for {symbol} {current_position.side} position from {current_position.trailing_stop_loss:.5f if current_position.trailing_stop_loss else 'N/A'} to {new_stop_loss:.5f}")
                current_position.trailing_stop_loss = new_stop_loss
                updated_any = True
            else:
                logger.debug(f"New stop loss {new_stop_loss:.5f} is same as current {current_position.trailing_stop_loss:.5f}, skipping SL update for {symbol}.")
        ...
        if new_take_profit is not None:
            if current_position.take_profit is None or new_take_profit != current_position.take_profit:
                ...
                params["tpTriggerBy"] = "MarkPrice"
                logger.info(f"Updating take profit for {symbol} {current_position.side} position from {current_position.take_profit:.5f if current_position.take_profit else 'N/A'} to {new_take_profit:.5f}")
                current_position.take_profit = new_take_profit
                updated_any = True
            else:
                logger.debug(f"New take profit {new_take_profit:.5f} is same as current {current_position.take_profit:.5f}, skipping TP update for {symbol}.")

        if not updated_any:
            logger.debug(f"No changes in stop loss or take profit requested for {symbol}, skipping API call.")
            return

        try:
            response = self.session.set_trading_stop(**params)
            if response and response['retCode'] == 0:
                logger.info(f"Successfully sent SL/TP update request for {symbol} {current_position.side} position.")
            else:
                error_msg = response.get('retMsg', 'Unknown error') if response else 'No response from API'
                logger.error(f"Failed to update SL/TP for {symbol}: {error_msg} (Code: {response.get('retCode', 'N/A')})")
        except Exception as e:
            ...
            if response and response['retCode'] == 0 and response['result']['list']:
                for pos_data in response['result']['list']:
                    ...
                    if size > 0:
                        ...
                        existing_initial_sl = self.positions.get(symbol) and self.positions[symbol].initial_stop_loss if self.positions.get(symbol) and self.positions[symbol].side == current_position_side else None
                        existing_entry_signal_price = self.positions.get(symbol) and self.positions[symbol].entry_signal_price if self.positions.get(symbol) and self.positions[symbol].side == current_position_side else None
                        ...
                        final_trailing_sl = bybit_sl if bybit_sl is not None else (self.positions.get(symbol) and self.positions[symbol].trailing_stop_loss if self.positions.get(symbol) and self.positions[symbol].side == current_position_side else None)
                        final_take_profit = bybit_tp if bybit_tp is not None else (self.positions.get(symbol) and self.positions[symbol].take_profit if self.positions.get(symbol) and self.positions[symbol].side == current_position_side else None)
                        ...
                        logger.debug(f"Position updated from API for {symbol}: {self.positions[symbol]}")
                    else:
                        self.positions[symbol] = None
                        logger.debug(f"No active position found for {symbol} via API response.")
            else:
                self.positions = dict.fromkeys(self.config.symbols)
                logger.debug("No active positions found for any symbol (API response list empty or retCode issue).")
            self._save_state()
        except Exception as e:
    ...
    def _calculate_position_size(self, signal: StrategySignal) -> float:
        if signal.stop_loss is None or signal.signal_price is None:
            logger.error(f"Cannot calculate position size for {signal.symbol} without a valid stop-loss and signal price from the strategy.")
            return 0.0

        if signal.symbol not in self.market_info:
            logger.error(f"Market info not loaded for {signal.symbol}, cannot calculate position size.")
            return 0.0
        ...
        if stop_loss_distance_raw <= Decimal('0', DECIMAL_CONTEXT) or stop_loss_distance_raw < market_info.tick_size:
            logger.warning(f"Stop loss distance for {signal.symbol} is non-positive or too small ({stop_loss_distance_raw:.5f} < {market_info.tick_size}), cannot calculate position size safely. Returning 0.")
            return 0.0
        ...
        formatted_position_size = Decimal(market_info.format_quantity(float(leveraged_position_size_asset)), DECIMAL_CONTEXT)
    ...
    async def process_signal(self, signal: StrategySignal):
        ...
        if current_strategy is None:
            logger.error(f"No strategy found for symbol {symbol}, cannot process signal.")
            return

        if symbol not in self.market_data or self.market_data[symbol].get(self.config.timeframe) is None or self.market_data[symbol][self.config.timeframe].empty:
            logger.warning(f"No market data available for {symbol} for signal processing, skipping.")
            return

        current_close_price = self.market_data[symbol][self.config.timeframe].iloc[-1]['close']
        ...
        if signal.take_profit is None and signal.signal_price is not None and current_atr > 0:
            tp_multiplier = Decimal(str(self.config.strategy_params.get("take_profit_atr_multiplier", 1.0)), DECIMAL_CONTEXT)
            signal_price_dec = Decimal(str(signal.signal_price), DECIMAL_CONTEXT)
            ...
            if signal.action == 'BUY':
                ...
                signal.take_profit = float(signal_price_dec - current_atr_dec * tp_multiplier)
            logger.info(f"Dynamically calculated Take Profit for {symbol}: {signal.take_profit:.5f}")

        position_size = self._calculate_position_size(signal)
        if position_size <= 0:
            logger.warning(f"Calculated position size for {symbol} is zero or too small, aborting trade.")
            return

        if current_position:
            if (signal.action == 'BUY' and current_position.side == 'Sell') or \
               (signal.action == 'SELL' and current_position.side == 'Buy'):
                logger.info(f"Reversing position for {symbol}: Closing existing {current_position.side} ({current_position.size:.5f}) to open {signal.action} ({position_size:.5f}).")
                ...
                if close_order_id:
                    logger.info(f"Close order {close_order_id} placed for {symbol}. Waiting for position to settle before opening new one...")
                    await asyncio.sleep(5)
                    await self.get_positions()
                    await self.update_account_balance()

                    if not self.positions.get(symbol):
                        logger.info(f"Existing position for {symbol} successfully closed. Proceeding to open new one.")
                        ...
                        if new_order_id:
                            self.positions[symbol] = Position(
                                symbol=symbol, side=signal.action, size=Decimal(str(position_size), DECIMAL_CONTEXT),
                            ...
                            logger.error(f"Failed to open new position for {symbol} after closing existing one. Check logs for details.")
                    else:
                        logger.error(f"Failed to confirm closure of existing position for {symbol}. Aborting new trade to prevent conflicting positions.")
                else:
                    logger.error(f"Failed to place order to close existing position for {symbol}. Aborting new trade.")
                return

            if (signal.action == 'BUY' and current_position.side == 'Buy') or \
                 (signal.action == 'SELL' and current_position.side == 'Sell'):
                logger.info(f"Signal to {signal.action} received for {symbol}, but already in a {current_position.side} position. Considering updating SL/TP.")
                ...
                if signal.stop_loss is not None:
                    ...
                    if should_update_initial_sl:
                        ...
                        if self.position.trailing_stop_loss is None or \
                           (self.position.side == 'Buy' and new_signal_sl > self.position.trailing_stop_loss) or (self.position.side == 'Sell' and new_signal_sl < self.position.trailing_stop_loss):
    ...
    async def _update_trailing_stop_loss(self):
        ...
        activation_multiplier = Decimal(str(self.config.strategy_params.get("trailing_stop_loss_activation_atr_multiplier", 0.5)), DECIMAL_CONTEXT)
        trailing_multiplier = Decimal(str(self.config.strategy_params.get("trailing_stop_loss_atr_multiplier", 0.75)), DECIMAL_CONTEXT)
        break_even_profit_multiplier = Decimal(str(self.config.strategy_params.get("break_even_profit_atr_multiplier", 0.2)), DECIMAL_CONTEXT)
        ...
        if potential_new_stop_price is not None:
            ...
            if current_trailing_sl is None or (self.position.side == 'Buy' and potential_new_stop_price > current_trailing_sl) or (self.position.side == 'Sell' and potential_new_stop_price < current_trailing_sl):
