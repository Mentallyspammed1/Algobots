...
while maintaining scalping-optimized parameters and the "neon" color-coded console
output.
...
3.  **State Persistence & Recovery:** Saves critical state, allowing the bot
    ...
    Improved error handling during state load/save.
4.  **Ehlers Supertrend Cross Strategy (Scalping Tuned):** Parameters are
    adjusted for rapid trend identification on low timeframes, using a custom
    recursive low-pass filter for smoothing without external scientific libraries.
    Enhanced data validation within indicator calculations.
5.  **Advanced Risk Management:** Features fixed-risk position sizing, dynamic
    ...
    of drawdown and daily loss limits (framework ready).
    Tuned for tighter stops and faster profit-taking, with robust Decimal handling.
6.  **Advanced Order Management:** Supports market/limit orders with strategy-defined
    ...
    robust position adjustment on new signals. Includes pre-order quantity validation.
7.  **Robust WebSocket Handling:** Dedicated manager for WebSocket connections with
    automatic reconnection and exponential backoff, ensuring high uptime.
    Improved logging for data stream events.
...
9.  **Enhanced Entry/Exit Logic:** Incorporates signal confirmation (tunable for speed),
    dynamic take-profit calculation, and intelligent position sizing based on calculated
    risk and stop-loss distance. Refined logic for reversing and adjusting positions.
10. **Comprehensive Logging with Neon Console:** Detailed logging for all critical operations,
    facilitating debugging and performance monitoring. Console output is color-coded
    for immediate visual comprehension of log levels.
...
3.  Configure the `Config` class below with your desired settings.
    Pay special attention to `timeframe` and `strategy_params` for scalping.
4.  Run the bot:
...
# ANSI color codes for "neon" style logging
class NeonColors:
    RESET = "\033[0m"
    BOLD = "\033[1m"
    ...
    RED = "\033[91m"  # ERROR
    PINK = "\033[95m"  # CRITICAL
    CYAN = "\033[96m"  # DEBUG
    BLUE = "\033[94m"  # Other (e.g., custom levels or just general info)
    WHITE = "\033[97m"  # Default


class NeonFormatter(logging.Formatter):
    """A custom formatter for neon-style colored console output."""

    FORMATS = {
        logging.DEBUG: NeonColors.CYAN
        + "%(asctime)s - %(levelname)s - %(name)s - %(funcName)s:%(lineno)d - %(message)s"
        + NeonColors.RESET,
        logging.INFO: NeonColors.GREEN
        + "%(asctime)s - %(levelname)s - %(message)s"
        + NeonColors.RESET,
        logging.WARNING: NeonColors.YELLOW
        + "%(asctime)s - %(levelname)s - %(message)s"
        + NeonColors.RESET,
        logging.ERROR: NeonColors.RED
...
class EhlersSupertrendCrossStrategy(BaseStrategy):
    ...
    async def calculate_indicators(self, data: dict[str, pd.DataFrame]):
        ...
        if (
            df_filtered["atr_filtered"].isnull().all()
            or df_filtered["atr_filtered"].iloc[-1] == 0
        ):
            logger.debug(
...
class BybitTradingBot:
    ...
    async def _load_historical_data(self):
        ...
        try:
            ...
            logger.critical(
                f"Critical Error loading historical data: {e}", exc_info=True
            )
    ...
    async def _place_single_order(
        ...
        try:
            ...
            if (
                float(formatted_qty) <= self.market_info.lot_size
            ):  # Ensure quantity is not zero or too small after formatting
                logger.warning(
                    f"Formatted quantity for order is too small ({formatted_qty}), skipping: original {quantity}. Minimum lot size: {self.market_info.lot_size}"
                )
                return None
    ...
    async def update_stop_loss_and_take_profit(
        ...
        if new_stop_loss is not None:
            # Only update if the new SL is different from the current effective (trailing) SL
            if (
                ...
                logger.info(
                    f"Updating stop loss for {self.position.side} position from {self.position.trailing_stop_loss:.5f if self.position.trailing_stop_loss else 'N/A'} to {new_stop_loss:.5f}"
                )
                ...
                logger.debug(
                    f"New stop loss {new_stop_loss:.5f} is same as current {self.position.trailing_stop_loss:.5f}, skipping SL update."
                )

        # Check and update Take Profit
        if new_take_profit is not None:
            # Only update if the new TP is different from the current effective TP
            if (
                ...
                params["tpTriggerBy"] = "MarkPrice"
                logger.info(
                    f"Updating take profit for {self.position.side} position from {self.position.take_profit:.5f if self.position.take_profit else 'N/A'} to {new_take_profit:.5f}"
                )
                ...
                logger.debug(
                    f"New take profit {new_take_profit:.5f} is same as current {self.position.take_profit:.5f}, skipping TP update."
                )

        if not updated_any:
            logger.debug(
                "No changes in stop loss or take profit requested, skipping API call."
            )
            return
                ...
                if size > 0:
                    ...
                    final_trailing_sl = (
                        ...
                        if bybit_sl is not None
                            ...
                            self.position.trailing_stop_loss
                            if self.position
                            and self.position.side == current_position_side
                            else None
                        )
                    )
                    final_take_profit = (
                    ...
                    self.position = None
                    logger.debug(
                        "No active position found for symbol via API response."
                    )
            else:
                self.position = None
                logger.debug(
                    "No active position found for symbol (API response list empty or retCode issue)."
                )
    ...
    def _calculate_position_size(self, signal: StrategySignal) -> float:
        ...
        if stop_loss_distance_raw <= 0:
            logger.warning(
                f"Stop loss distance is non-positive or zero ({stop_loss_distance_raw:.5f}), cannot calculate position size safely. Returning 0."
            )
            return 0.0
    ...
    async def process_signal(self, signal: StrategySignal):
        ...
        if position_size <= 0:
            logger.warning(
                "Calculated position size is zero or too small, aborting trade."
            )
            return
        ...
        if self.position:
            ...
            if (signal.action == "BUY" and self.position.side == "Sell") or (
                ...
                if close_order_id:
                    ...
                    await (
                        self.get_position()
                    )  # Update position state (should become None if closed)
                    await self.update_account_balance()  # Update balance

                    if not self.position:  # Successfully closed, now open new one
                        logger.info(
                            "Existing position successfully closed. Proceeding to open new one."
                        )
                        ...
                        if new_order_id:
                            ...
                            logger.error(
                                "Failed to open new position after closing existing one. Check logs for details."
                            )
                    else:
                        logger.error(
                            "Failed to confirm closure of existing position. Aborting new trade to prevent conflicting positions."
                        )
                else:
                    logger.error(
            ...
            if (signal.action == "BUY" and self.position.side == "Buy") or (
                ...
                logger.info(
                    f"Signal to {signal.action} received, but already in a {self.position.side} position. Considering updating SL/TP."
                )
                ...
                if signal.stop_loss is not None:
                    ...
                    if should_update_initial_sl:
                        ...
                        if (
                            ...
                            logger.info(
                                f"Trailing stop loss on exchange will be set to new initial SL: {new_signal_sl:.5f}"
                            )
    ...
    async def _update_trailing_stop_loss(self):
        ...
        # For SELL position
            ...
            if profit_in_atr >= break_even_profit_multiplier:
                ...
                if (
                    ...
                    potential_new_stop_price = min(
                        calculated_be_sl,
                        initial_sl if initial_sl else Decimal("inf", DECIMAL_CONTEXT),
                    )
        ...
        if potential_new_stop_price is not None:
            ...
            ):  # No current trailing SL set on exchange
        ...
        try:
            ...
            if current_candle_time in df.index:
                ...
                required_lookback = max(
                    ...
                    getattr(
                        self.strategy, "fast_st_period", 0
                    ),  # Safe access for Ehlers
                    getattr(
                        self.strategy, "slow_st_period", 0
                    ),  # Safe access for Ehlers
                    (getattr(self.strategy, "filter_poles", 0) * 2)
                    if hasattr(self.strategy, "filter_poles")
                    else 0,  # For recursive filter warm-up
                )
