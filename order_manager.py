...
class OrderManager:
    ...
    async def _get_instrument_info(self, symbol: str) -> Optional[Dict[str, Decimal]]:
        ...
        try:
            ...
            if not instrument_info_resp or instrument_info_resp.get('retCode') != 0 or not instrument_info_resp.get('result', {}).get('list'):
                raise ValueError(f"Failed to fetch instrument info: {instrument_info_resp.get('retMsg', 'N/A')}")

            instrument = instrument_info_resp['result']['list'][0]
            return {
                'min_qty': Decimal(instrument.get('lotSizeFilter', {}).get('minOrderQty', '0.001')),
                'qty_step': Decimal(instrument.get('lotSizeFilter', {}).get('qtyStep', '0.001')),
                'min_order_value': Decimal(instrument.get('lotSizeFilter', {}).get('minOrderIv', '10')),
            }
        except BybitAPIError as e:
            await self.bot._handle_api_error(e, f"fetching instrument info for {symbol}")
            return None
        except Exception as e:
            log_exception(self.logger, f"Error fetching or parsing instrument info for {symbol}: {e}", e)
            return None
    ...
    async def _get_usdt_balance(self) -> Decimal:
        ...
        try:
            ...
            self.logger.warning(f"{PYRMETHUS_YELLOW}Could not retrieve USDT balance. Response: {balance_response.get('retMsg', 'N/A')}{COLOR_RESET}")
            return Decimal('0')
        except BybitAPIError as e:
    ...
    async def _get_current_execution_price(self, symbol: str) -> Decimal:
        ...
        try:
            ticker_info = await self.bybit_client.get_symbol_ticker(category=config.BYBIT_CATEGORY, symbol=symbol)
            if not ticker_info or ticker_info.get('retCode') != 0 or not ticker_info.get('result', {}).get('list'):
                raise ValueError(f"Failed to fetch ticker info: {ticker_info.get('retMsg', 'N/A')}")
    ...
    async def execute_entry(self, signal_type: str, signal_price: Decimal, signal_timestamp: Any, signal_info: Dict[str, Any]) -> bool:
        ...
        if usdt_balance < config.USDT_AMOUNT_PER_TRADE and not config.USE_PERCENTAGE_ORDER_SIZING:
            self.logger.warning(f"{PYRMETHUS_YELLOW}Insufficient balance ({usdt_balance:.2f}) for fixed trade amount. Skipping.{COLOR_RESET}")
            return False
        ...
        if current_price <= 0:
            self.logger.error(f"{COLOR_RED}Invalid execution price ({current_price}). Aborting order.{COLOR_RESET}")
            return False
        ...
        if config.USE_PERCENTAGE_ORDER_SIZING:
            ...
            if self.bot.cached_atr and current_price > 0:
                volatility_factor = min(Decimal('1'), self.bot.cached_atr / current_price)
            target_usdt_value = usdt_balance * (config.ORDER_SIZE_PERCENT_OF_BALANCE / Decimal('100')) * volatility_factor
            self.logger.info(f"{PYRMETHUS_BLUE}Dynamic sizing: Balance={usdt_balance:.2f}, Volatility Factor={volatility_factor:.3f}, Target USDT={target_usdt_value:.2f}{COLOR_RESET}")
        ...
        if quantity <= 0:
            self.logger.error(f"{COLOR_RED}Final calculated quantity is zero or negative. Aborting order.{COLOR_RESET}")
            return False
        ...
        order_kwargs = {
            ...
            "order_type": "Limit", "qty": f"{quantity:.8f}", "price": f"{signal_price:.4f}"
        }
        ...
        self.logger.info(f"{PYRMETHUS_ORANGE}Placing {signal_type.upper()} Limit order: Qty={quantity:.4f} @ {signal_price:.4f}{COLOR_RESET}")

        try:
            response = await self.bybit_client.create_order(**order_kwargs)
            if response and response.get('retCode') == 0:
                order_id = response.get('result', {}).get('orderId')
                self.logger.info(f"{PYRMETHUS_GREEN}Successfully placed order {order_id}.{COLOR_RESET}")
                if order_id:
    ...
    async def execute_exit(self, inventory: Decimal, exit_type: str, exit_price: Decimal, exit_timestamp: Any, exit_info: Dict[str, Any]) -> bool:
        ...
        if inventory == 0:
            self.logger.warning(f"{COLOR_YELLOW}No open position to exit. Signal ignored.{COLOR_RESET}")
            return False
        ...
        exit_order_kwargs = {
            ...
            "order_type": "Market", "qty": f"{exit_quantity:.8f}"
        }
        if config.HEDGE_MODE:
        ...
        self.logger.info(f"{PYRMETHUS_ORANGE}Placing {side_for_exit} Market exit for {exit_quantity:.4f} {config.SYMBOL}{COLOR_RESET}")
    ...
    async def chase_limit_order(self, order_id: str, symbol: str, side: str, chase_aggressiveness: float = 0.0005):
        ...
        self.logger.info(f"{PYRMETHUS_ORANGE}Chasing limit order {order_id} for {symbol}...{COLOR_RESET}")
        max_amendments = 10
        amendment_count = 0
        ...
        while amendment_count < max_amendments:
            ...
            try:
                ...
                if order_status.get('orderStatus') not in ['New', 'PartiallyFilled']:
                    self.logger.info(f"{PYRMETHUS_GREEN}Order {order_id} is no longer active ({order_status.get('orderStatus')}). Stopping chase.{COLOR_RESET}")
                    break
                ...
                if not order_book_resp or order_book_resp.get('retCode') != 0:
                    self.logger.warning(f"{COLOR_YELLOW}Could not fetch order book for chasing.{COLOR_RESET}")
                    continue
                ...
                if new_price:
                    ...
                    if amendment_result and amendment_result.get('retCode') == 0:
                        self.logger.info(f"{PYRMETHUS_BLUE}Amended order {order_id} to new price {new_price:.4f}{COLOR_RESET}")
                        amendment_count += 1
                    else:
                        self.logger.error(f"{COLOR_RED}Failed to amend order {order_id}: {amendment_result.get('retMsg', 'N/A')}{COLOR_RESET}")

            except BybitAPIError as e:
                if e.ret_code in [10009, 110001]: # Order not found
                    self.logger.info(f"Order {order_id} not found, assuming it was filled or cancelled.")
                    break
        ...
        if amendment_count >= max_amendments:
            self.logger.warning(f"{COLOR_YELLOW}Reached max amendments for order {order_id}. Stopping chase.{COLOR_RESET}")
