...
class BybitClient:
    ...
    def __init__(
        ...
        ws_callbacks: dict[str, Callable] | None = None,
        ...
        if self.use_websocket:
            self.ws_session = WebSocket(
                testnet="testnet"
                in self.api_endpoint,  # Determine testnet from endpoint
                api_key=self.api_key,
                api_secret=self.api_secret,
    ...
    def place_order(
        ...
        try:
            ...
            if stop_loss_pct is not None or take_profit_pct is not None:
                ...
                current_price_for_sl_tp = (
                    price if order_type == "Limit" and price is not None else None
                )
                ...
                if stop_loss_pct is not None:
                    if side.upper() == "BUY":
                        calculated_stop_loss_price = current_price_for_sl_tp * (
                            1 - stop_loss_pct
                        )
                ...
                if take_profit_pct is not None:
                    if side.upper() == "BUY":
                        calculated_take_profit_price = current_price_for_sl_tp * (
                            1 + take_profit_pct
                        )
                    elif side.upper() == "SELL":
                        calculated_take_profit_price = current_price_for_sl_tp * (
