...
def _load_api_creds():
    ...
    if not api_key or not api_secret:
        ...
        if api_key and api_secret:
            ...
            print(NEON_ERROR + "CRITICAL: Keys lost in the void. Forge 'authcreds.json' or set env vars." + NEON_RESET)
            exit(1)
...
def _initialize_session(config):
    ...
    """
    if config['backtest']:
        print(NEON_INFO + "Backtest mode active—forging a session for historical data." + NEON_RESET)
    ...
    try:
        ...
        if position_info and position_info.get("result", {}).get("list"):
            for pos in position_info["result"]["list"]:
                if pos["symbol"] == symbol:
                    ...
                    current_leverage = pos.get("lever") or pos.get("buyLeverage") or pos.get("sellLeverage")
                    if current_leverage:
        ...
        if current_leverage is None:
            ...
            print(NEON_INFO + f"Current leverage for {symbol} is {current_leverage}x, desired is {desired_leverage}x. Adjusting..." + NEON_RESET)
            session.set_leverage(
                category=category,
                symbol=symbol,
                buyLeverage=desired_leverage,
        ...
        if "110043" in str(e):
            print(NEON_WARNING + f"Leverage is already set correctly (ErrCode: 110043). Continuing." + NEON_RESET)
        else:
...
def _fetch_kline_data(session, symbol, category, interval, limit=200, start_time=None, end_time=None):
    ...
    while True:
        for attempt in range(5):
            try:
                ...
                if end_time:
                    params["end"] = end_time
                print(NEON_INFO + f"Channeling klines from {current_start}... " + NEON_RESET)
...
def _calculate_indicators(df, config):
    ...
    """
    df.ta.supertrend(length=config['super_trend_length'], multiplier=config['super_trend_multiplier'], append=True)
...
def _plot_supertrend(df, config):
    ...
    try:
        ...
        ax1.fill_between(df['startTime'], df['close'], df[st_col], where=(df['close'] > df[st_col]), color='lime', alpha=0.3)
        ax1.fill_between(df['startTime'], df['close'], df[st_col], where=(df['close'] < df[st_col]), color='red', alpha=0.3)

        # Signal markers
        buy_signals = df[(df[st_dir_col].shift(1) == -1) & (df[st_dir_col] == 1) & (df['rsi'] < config['rsi_overbought'])]
        sell_signals = df[(df[st_dir_col].shift(1) == 1) & (df[st_dir_col] == -1) & (df['rsi'] > config['rsi_oversold'])]
        ax1.scatter(buy_signals['startTime'], buy_signals['close'], color='lime', label='Buy Signal', marker='^', s=100)
        ax1.scatter(sell_signals['startTime'], sell_signals['close'], color='red', label='Sell Signal', marker='v', s=100)
...
def _place_order_with_tpsl(session, side, symbol, category, quantity, entry_price, config):
...
def _run_backtest(df_ind, config):
    ...
    for i in range(1, len(df_ind)):
        ...
        st_dir = current[f"SUPERTd_{config['super_trend_length']}_{config['super_trend_multiplier']}"]
        close = Decimal(str(current['close']))
        rsi = current['rsi']

        is_buy = (st_dir == 1 and last_dir == -1) and _check_rsi_filter(rsi, config, "Buy")
        is_sell = (st_dir == -1 and last_dir == 1) and _check_rsi_filter(rsi, config, "Sell")
        last_dir = st_dir

        if positions and ((is_buy and positions[-1]['side'] == 'Sell') or (is_sell and positions[-1]['side'] == 'Buy')):
            pos = positions.pop()
            exit_price = close
            pnl = (exit_price - pos['entry']) * pos['qty'] if pos['side'] == 'Buy' else (pos['entry'] - exit_price) * pos['qty']
            equity += pnl
            trades.append({'exit_time': current['startTime'], 'side': pos['side'], 'pnl': pnl})
        ...
        if (is_buy or is_sell) and len(positions) < config['max_open_trades']:
            ...
            if qty > Decimal('0'):
                positions.append({'side': 'Buy' if is_buy else 'Sell', 'entry': close, 'qty': qty})
                logging.info(f"Backtest opened {positions[-1]['side']}: {qty} at {close}")
    ...
    if positions:
        ...
        pnl = (exit_price - pos['entry']) * pos['qty'] if pos['side'] == 'Buy' else (pos['entry'] - exit_price) * pos['qty']
        equity += pnl
        trades.append({'exit_time': df_ind['startTime'].iloc[-1], 'side': pos['side'], 'pnl': pnl})

    total_pnl = sum(t['pnl'] for t in trades)
    print(NEON_SUCCESS + f"Backtest PNL: {total_pnl:.4f} | Final Equity: {equity:.4f}" + NEON_RESET)
    ...
    if config['plot_enabled'] and trades:
        ...
        plt.plot([t['exit_time'] for t in trades], [float(v) for v in equity_curve[1:]], color='cyan')
...
def main():
    ...
    print(NEON_SUCCESS + f"Summoning ascended bot for {config['symbol']}... " + NEON_RESET)
    logging.info("Bot awakened.")

    if config['backtest']:
        start_ts = int(datetime.fromisoformat(config['backtest_start']).timestamp() * 1000)
        end_ts = int(datetime.fromisoformat(config['backtest_end']).timestamp() * 1000)
        df = _fetch_kline_data(session, config['symbol'], config['category'], config['interval'], limit=1000, start_time=start_ts, end_time=end_ts)
        if df.empty:
    ...
    try:
        while True:
            df = _fetch_kline_data(session, config['symbol'], config['category'], config['interval'])
            if df.empty:
            ...
            st_dir_current = current_data[f"SUPERTd_{config['super_trend_length']}_{config['super_trend_multiplier']}"]
            ...
            current_position = positions[0] if positions and Decimal(positions[0]['size']) > 0 else None
            ...
            print(NEON_INFO + f"Epoch: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}" + NEON_RESET)
            print(NEON_INFO + f"Close: {close_current} | Dir: {'Up' if st_dir_current == 1 else 'Down'} | RSI: {rsi_current:.2f}" + NEON_RESET)
            print(NEON_POSITION + f"Balance: {balance:.2f} USDT | Position: {'Long' if has_long else 'Short' if has_short else 'None'}" + NEON_RESET)
            ...
            if balance < required_balance:
                ...
                if qty <= Decimal(config['max_position_size']):
                    _place_order_with_tpsl(session, "Sell", config['symbol'], config['category'], qty, close_current, config)
                else:
                    print(NEON_WARNING + "Position size exceeds max." + NEON_RESET)
            else:
            ...
            sleep_dur = (next_time - datetime.now(timezone.utc)).total_seconds() + 10
