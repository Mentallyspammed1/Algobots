...
def get_perpetual_price(symbol: str = 'ETHUSDT'):
    ...
    try:
        ...
        print(Fore.CYAN + f"  # Summoning the latest ticker information for {symbol}..." + Style.RESET_ALL)

        # Fetch ticker information for linear perpetual futures
        # The category for linear perpetual futures is 'linear'
        response = session.get_tickers(category="linear", symbol=symbol)

        if response and response['retCode'] == 0:
            if response['result'] and response['result']['list']:
                last_price = response['result']['list'][0]['lastPrice']
                print(Fore.GREEN + f"  # The current last traded price of {symbol} (Linear Perpetual) is: {last_price}" + Style.RESET_ALL)
            else:
                print(Fore.YELLOW + f"  # No ticker data found for {symbol}." + Style.RESET_ALL)
        else:
