...
class BybitV5Wizard:
    ...
    def __init__(self, config_path: str = "/data/data/com.termux/files/home/.bybit_config"):
        ...
        try:
            ...
            raise ValueError(Fore.RED + f"# Failed to load config: {e!s}" + Style.RESET_ALL)
        ...
        self.ws_trade_url = "wss://stream-testnet.bybit.com/v5/trade" if self.testnet else "wss://stream.bybit.com/v5/trade"
        ...
        print(Fore.CYAN + "# Bybit V5 Wizard awakened with ethereal power..." + Style.RESET_ALL)

    def _generate_signature(self, params: dict, timestamp: str) -> str:
        """Forge an HMAC SHA256 signature for the API's sacred gate."""
        param_str = timestamp + self.api_key + "5000" + json.dumps(params, separators=(",", ":"))
        return hmac.new(
    ...
    def _send_request(self, method: str, endpoint: str, params: dict = None, retries: int = 3) -> dict:
        ...
        for attempt in range(retries):
            ...
            try:
                if method == "GET":
                    ...
                    raise ValueError(Fore.RED + "Invalid method invoked!" + Style.RESET_ALL)
                response.raise_for_status()
                result = response.json()
                logging.info(f"Request: {method} {endpoint} - Params: {params} - Response: {result}")
                if result.get("retCode") == 0:
                    print(Fore.GREEN + f"# Spell cast successfully: {endpoint}" + Style.RESET_ALL)
                    return result.get("result", {})
                print(Fore.RED + f"# Spell failed: {result.get('retMsg')}" + Style.RESET_ALL)
                logging.error(f"API error: {result.get('retMsg')} for endpoint {endpoint}")
                return {}
            except Exception as e:
                print(Fore.RED + f"# Attempt {attempt + 1}/{retries}: The ether rejects the call: {e!s}" + Style.RESET_ALL)
                logging.error(f"Request failed: {e!s} for endpoint {endpoint} (attempt {attempt + 1})")
    ...
    def get_kline(self, category: str, symbol: str, interval: str, start: int | None = None,
        ...
        if end:
            params["end"] = end
        print(Fore.CYAN + f"# Fetching kline for {symbol} ({interval})..." + Style.RESET_ALL)
    ...
    def place_batch_orders(self, symbol: str, orders: list[dict], category: str = "linear") -> dict:
        ...
        print(Fore.CYAN + f"# Casting {len(orders)} batch orders for {symbol}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def amend_order(self, symbol: str, order_id: str, category: str = "linear", qty: float | None = None,
        ...
        print(Fore.CYAN + f"# Amending order {order_id} for {symbol}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def amend_batch_orders(self, symbol: str, orders: list[dict], category: str = "linear") -> dict:
        ...
        print(Fore.CYAN + f"# Amending {len(orders)} batch orders for {symbol}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def cancel_order(self, symbol: str, order_id: str, category: str = "linear") -> dict:
        ...
        params = {"category": category, "symbol": symbol, "orderId": order_id}
        print(Fore.CYAN + f"# Banishing order {order_id} for {symbol}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def cancel_all_orders(self, category: str = "linear", symbol: str | None = None) -> dict:
        ...
        if symbol:
            params["symbol"] = symbol
        print(Fore.CYAN + f"#发展和# Obliterating all {category} orders..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def set_auto_add_margin(self, symbol: str, auto_add_margin: bool, category: str = "linear") -> dict:
        ...
        print(Fore.CYAN + f"# Setting auto-add margin for {symbol} to {auto_add_margin}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def set_margin_mode(self, symbol: str, mode: str = "ISOLATED", category: str = "linear") -> dict:
        ...
        print(Fore.CYAN + f"# Setting {symbol} to {mode} margin mode..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def get_pre_upgrade_order_history(self, category: str, base_coin: str | None = None,
                                    limit: int = 50) -> dict:
        ...
        if base_coin:
            params["baseCoin"] = base_coin
        print(Fore.CYAN + f"# Fetching pre-upgrade order history for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_pre_upgrade_trade_history(self, category: str, base_coin: str | None = None,
                                    limit: int = 50) -> dict:
        ...
        if base_coin:
            params["baseCoin"] = base_coin
        print(Fore.CYAN + f"# Fetching pre-upgrade trade history for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_pre_upgrade_closed_pnl(self, category: str, symbol: str | None = None,
        ...
        print(Fore.CYAN + f"# Fetching pre-upgrade closed PNL for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_pre_upgrade_transaction_log(self, category: str | None = None, base_coin: str | None = None,
                                      start_time: int | None = None, end_time: int | None = None,
                                      limit: int = 50) -> dict:
    ...
    def get_pre_upgrade_option_delivery_record(self, category: str = "option", symbol: str | None = None,
        ...
        print(Fore.CYAN + "# Fetching pre-upgrade option delivery record..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_pre_upgrade_settlement_record(self, category: str, symbol: str | None = None,
                                        limit: int = 50, cursor: str | None = None) -> dict:
        ...
        if cursor:
            params["cursor"] = cursor
        print(Fore.CYAN + "# Fetching pre-upgrade settlement record..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def get_coin_exchange_records(self, from_coin: str | None = None, to_coin: str | None = None,
                                limit: int = 50, cursor: str | None = None) -> dict:
    ...
    def get_delivery_record(self, category: str, symbol: str | None = None, limit: int = 50,
        ...
        if cursor:
            params["cursor"] = cursor
        print(Fore.CYAN + f"# Fetching delivery record for {category}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_usdc_settlement(self, category: str = "option", cursor: str | None = None) -> dict:
    ...
    def get_asset_info(self, account_type: str, coin: str | None = None) -> dict:
        ...
        if coin:
            params["coin"] = coin
        print(Fore.CYAN + f"# Fetching asset info for {account_type}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_all_coins_balance(self, account_type: str, member_id: str | None = None,
                            coin: str | None = None, with_bonus: int | None = None) -> dict:
        ...
        print(Fore.CYAN + f"# Fetching all coins balance for {account_type}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_single_coin_balance(self, account_type: str, member_id: str | None = None,
                              coin: str | None = None, with_bonus: int | None = None) -> dict:
        ...
        if with_bonus is not None:
            params["withBonus"] = with_bonus
        print(Fore.CYAN + f"# Fetching single coin balance for {account_type}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_transferable_coin(self, from_account_type: str, to_account_type: str) -> dict:
        ...
        params = {"fromAccountType": from_account_type, "toAccountType": to_account_type}
        print(Fore.CYAN + f"# FetchSix transferable coins from {from_account_type} to {to_account_type}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def create_internal_transfer(self, transfer_id: str, coin: str, amount: str,
                               from_account_type: str, to_account_type: str) -> dict:
        ...
        print(Fore.CYAN + f"# Creating internal transfer of {amount} {coin}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def get_internal_transfer_records(self, transfer_id: str | None = None, coin: str | None = None,
                                    status: str | None = None, start_time: int | None = None,
                                    end_time: int | None = None, direction: str | None = None,
                                    limit: int = 50, cursor: str | None = None) -> dict:
    ...
    def get_sub_account_transfer_records(self, transfer_id: str | None = None, coin: str | None = None,
                                       status: str | None = None, start_time: int | None = None,
                                       end_time: int | None = None, direction: str | None = None,
                                       limit: int = 50, cursor: str | None = None) -> dict:
        ...
        if cursor:
            params["cursor"] = cursor
        print(Fore.CYAN + "# Fetching sub-account transfer records..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_universal_transfer_records(self, transfer_id: str | None = None, coin: str | None = None,
                                     status: str | None = None, start_time: int | None = None,
                                     end_time: int | None = None, limit: int = 50,
                                     cursor: str | None = None) -> dict:
    ...
    def create_universal_transfer(self, transfer_id: str, coin: str, amount: str, from_member_id: str,
                                to_member_id: str, from_account_type: str, to_account_type: str) -> dict:
        ...
        print(Fore.CYAN + f"# Creating universal transfer of {amount} {coin}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)

    def get_allowed_deposit_coin_info(self, coin: str | None = None, chain: str | None = None,
                                    limit: int = 50, cursor: str | None = None) -> dict:
    ...
    def get_deposit_records(self, coin: str | None = None, start_time: int | None = None,
                           end_time: int | None = None, limit: int = 50, cursor: str | None = None) -> dict:
    ...
    def get_sub_deposit_records(self, sub_member_id: str, coin: str | None = None,
                               start_time: int | None = None, end_time: int | None = None,
                               limit: int = 50, cursor: str | None = None) -> dict:
    ...
    def get_internal_deposit_records(self, coin: str | None = None, start_time: int | None = None,
                                   end_time: int | None = None, limit: int = 50,
                                   cursor: str | None = None) -> dict:
    ...
    def get_master_deposit_address(self, coin: str, chain_type: str) -> dict:
        ...
        print(Fore.CYAN + f"# Fetching master deposit address for {coin}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)

    def get_sub_deposit_address(self, sub_member_id: str, coin: str, chain_type: str) -> dict:
        ...
        print(Fore.CYAN + f"# Fetching sub-account deposit address for {coin}..." + Style.RESET_ALL)
        return self._send_request("GET", endpoint, params)
    ...
    def get_withdrawal_records(self, coin: str | None = None, withdraw_id: str | None = None,
                             start_time: int | None = None, end_time: int | None = None,
                             withdraw_type: int | None = None, limit: int = 50,
                             cursor: str | None = None) -> dict:
    ...
    def withdraw(self, coin: str, chain: str, address: str, amount: str, tag: str | None = None,
                 force_chain: int | None = None, account_type: str = "SPOT",
                 fee_type: int | None = None) -> dict:
        ...
        print(Fore.CYAN + f"# Creating withdrawal of {amount} {coin}..." + Style.RESET_ALL)
        return self._send_request("POST", endpoint, params)
    ...
    def monitor_profit(self, symbol: str, profit_target: float, action: str = "partial_close",
    ...
    def start_websocket(self, symbols: list[str] = None, public_streams: list[str] = None):
        """Summon WebSocket spirits for private and public streams."""
        private_topics = [f"position.{symbol}" for symbol in symbols] + [f"order.{symbol}" for symbol in symbols] if symbols else ["position", "order"]
        ...
        self.ws_thread = threading.Thread(target=self._ws_run_with_reconnect, args=(self.ws, self.ws_url))
        ...
        if public_streams:
            ...
            self.ws_public_thread = threading.Thread(target=self._ws_run_with_reconnect, args=(self.ws_public, self.ws_public_url))
        ...
        self.ws_trade_thread = threading.Thread(target=self._ws_run_with_reconnect, args=(self.ws_trade, self.ws_trade_url))
    ...
    def _ws_run_with_reconnect(self, ws: websocket.WebSocketApp, url: str):
        ...
        while True:
            ...
            print(Fore.YELLOW + f"# Reconnecting WebSocket in {delay}s (attempt {attempt})..." + Style.RESET_ALL)
            time.sleep(delay)
            ws.url = url
            if attempt >= 5:
                print(Fore.RED + "# Max reconnection attempts reached!" + Style.RESET_ALL)
                break

    def _on_open(self, ws, topics: list[str]):
        """Authenticate and subscribe to private streams."""
        print(Fore.CYAN + "# Private WebSocket connected, channeling the ether..." + Style.RESET_ALL)
    ...
    def _on_open_public(self, ws, topics: list[str]):
        ...
        print(Fore.CYAN + "# Public WebSocket connected, channeling market whispers..." + Style.RESET_ALL)
        subscribe_message = {
            "op": "subscribe",
            "args": topics
        }
    ...
    def _on_open_trade(self, ws):
        ...
        print(Fore.CYAN + "# Trade WebSocket connected, ready to cast orders..." + Style.RESET_ALL)
        expires = int((time.time() + 1) * 1000)
        signature = hmac.new(
            bytes(self.api_secret, "utf-8"),
            bytes(f"GET/realtime{expires}", "utf-8"),
    ...
    def _on_message(self, ws, message):
        ...
        if data.get("op") == "pong":
            return
        print(Fore.YELLOW + f"# Received private message: {json.dumps(data, indent=2)}" + Style.RESET_ALL)
    ...
    def _on_message_public(self, ws, message):
        ...
        if data.get("op") == "pong":
            return
        print(Fore.YELLOW + f"# Received market whisper: {json.dumps(data, indent=2)}" + Style.RESET_ALL)
    ...
    def _on_message_trade(self, ws, message):
        ...
        if data.get("op") == "pong":
            return
        print(Fore.YELLOW + f"# Received trade message: {json.dumps(data, indent=2)}" + Style.RESET_ALL)
    ...
    def _handle_position_update(self, data: dict):
        ...
        for position in data.get("data", []):
            ...
            # Break-even logic
            if self.positions[symbol]["break_even_enabled"] and unrealised_pnl > 0 and stop_loss != entry_price:
                self.set_trading_stop(symbol=symbol, category="linear", stop_loss=entry_price)
                print(Fore.GREEN + f"# Break-even stop set for {symbol} at {entry_price}" + Style.RESET_ALL)

            # Trailing stop-loss logic
            if self.positions[symbol]["tsl_enabled"] and self.positions[symbol]["tsl_distance"]:
                if side == "Buy" and mark_price > entry_price:
                    new_stop = mark_price - self.positions[symbol]["tsl_distance"]
                    if new_stop > stop_loss:
                        self.set_trading_stop(symbol=symbol, category="linear", stop_loss=new_stop)
                        print(Fore.GREEN + f"# Trailing stop updated for {symbol} to {new_stop}" + Style.RESET_ALL)
            ...
            if self.positions[symbol]["profit_target"]:
                ...
                if profit_pct >= self.positions[symbol]["profit_target"]:
                    ...
                    if action == "partial_close":
                        close_qty = size * (self.positions[symbol]["close_percentage"] / 100)
                        self.place_market_order(symbol, "Sell" if side == "Buy" else "Buy", close_qty)
                        print(Fore.GREEN + f"# Partial close triggered for {symbol}: {close_qty} units" + Style.RESET_ALL)
                    elif action == "set_take_profit":
                        take_profit = mark_price * (1 + 0.01 if side == "Buy" else 1 - 0.01)
                        self.set_trading_stop(symbol=symbol, category="linear", take_profit=take_profit)
                        print(Fore.GREEN + f"# Take-profit set for {symbol} at {take_profit}" + Style.RESET_ALL)
                    elif action == "close":
                        self.close_position(symbol)
                        print(Fore.GREEN + f"# Full position closed for {symbol}" + Style.RESET_ALL)
    ...
    def _handle_order_update(self, data: dict):
        ...
        for order in data.get("data", []):
            ...
            if status == "Filled" and symbol in self.positions:
                ...
                print(Fore.GREEN + f"# Position entry price updated for {symbol}: {self.positions[symbol]['entry_price']}" + Style.RESET_ALL)
    ...
    def _handle_ticker_update(self, data: dict):
        ...
        print(Fore.YELLOW + f"# Market whisper for {symbol}: Last price {last_price}" + Style.RESET_ALL)

    def send_ws_order(self, symbol: str, side: str, qty: float, order_type: str = "Market",
                     price: float | None = None, category: str = "linear") -> None:
        ...
        with self.lock:
            if self.ws_trade and self.ws_trade.sock and self.ws_trade.sock.connected:
                ...
                print(Fore.CYAN + f"# Casting WebSocket order for {symbol}..." + Style.RESET_ALL)
            else:
    ...
    def check_health(self) -> bool:
        ...
        try:
            ...
            if server_time_response and server_time_response.get("timeNano"):
                print(Fore.GREEN + "# API connection healthy. Server time retrieved." + Style.RESET_ALL)
                return True
            print(Fore.RED + "# API connection unhealthy. Could not retrieve server time." + Style.RESET_ALL)
            return False
        except Exception as e:
        ...
        if self.ws_thread and self.ws_thread.is_alive():
            ...
            print(Fore.RED + "# Private WebSocket thread is not alive." + Style.RESET_ALL)
            ws_healthy = False
        ...
        if self.ws_public_thread and self.ws_public_thread.is_alive():
            ...
            print(Fore.YELLOW + "# Public WebSocket thread not started or not alive." + Style.RESET_ALL)
