...
class StrategyBase(ABC):
    ...
    def __init__(self, client, state: Dict[str, Any], params: Dict[str, Any]):
        ...
        self.logger.info(f"StrategyBase initialized for {self.symbol} ({self.interval})")
    ...
    async def on_order_update(self, order_data: Dict[str, Any]):
        ...
        """
        self.logger.debug(f"Order update received (default handler): {order_data.get('orderId', 'N/A')}")
    ...
    async def enter_position(self, side: str, price: float, qty: str):
        ...
        """
        self.logger.info(f"Attempting to enter {side} position for {self.symbol} at {price} with qty {qty}")
        ...
        if current_pos and current_pos.get("symbol") == self.symbol:
            self.logger.warning(f"Already in a position for {self.symbol}. Skipping entry.")
            return
        ...
        try:
            ...
            stop_loss_price = str(price * (1 - stop_loss_pct) if side == "Buy" else price * (1 + stop_loss_pct))
            take_profit_price = str(price * (1 + take_profit_pct) if side == "Buy" else price * (1 - take_profit_pct))
            ...
            if order_result and order_result.get('retCode') == 0:
                self.logger.success(f"Successfully placed {side} Market order for {self.symbol}. Order ID: {order_result.get('result', {}).get('orderId')}")
                # Note: Actual state update should ideally happen upon receiving position update
            else:
                self.logger.error(f"Failed to place {side} Market order: {order_result}")
    ...
    async def exit_position(self, side: str, price: float):
        ...
        """
        self.logger.info(f"Attempting to exit {side} position for {self.symbol} at {price}")
        ...
        try:
            ...
            order_result = await self.client.create_order(
                ...
                side=exit_side,
                order_type="Market",
                qty=str(abs(float(current_pos.get("size")))), # Use the size of the current position
                reduce_only=True # Ensure this order only closes the position
            )
            
            if order_result and order_result.get('retCode') == 0:
                self.logger.success(f"Successfully placed exit order for {self.symbol}. Order ID: {order_result.get('result', {}).get('orderId')}")
            else:
                self.logger.error(f"Failed to place exit order for {self.symbol}: {order_result}")
