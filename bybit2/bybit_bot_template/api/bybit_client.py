...
class BybitClient:
    ...
    def __init__(self):
        ...
        self.logger.info(Fore.MAGENTA + "# Pyrmethus: Forging the Bybit API conduit... #" + Style.RESET_ALL)
        ...
        if not self.api_key or not self.api_secret:
            self.logger.error(Fore.RED + "  # ERROR: BYBIT_API_KEY or BYBIT_API_SECRET not found. Please set them in your .env file." + Style.RESET_ALL)
        ...
        self.websocket_reconnect_delay = int(os.getenv('WEBSOCKET_RECONNECT_DELAY', 5)) # Default 5 seconds

    async def initialize(self):
        """Initializes HTTP and WebSocket clients."""
        self.logger.info(Fore.CYAN + "  # Initializing Bybit HTTP client..." + Style.RESET_ALL)
        try:
            ...
            self.logger.success(Fore.GREEN + "  # Bybit HTTP client initialized successfully." + Style.RESET_ALL)
            
            # Verify connection by fetching server time
            await self.verify_connection()

        except Exception as e:
            self.logger.error(Fore.RED + f"  # Failed to initialize HTTP client: {e}" + Style.RESET_ALL)
            raise

        self.logger.info(Fore.CYAN + "  # Initializing Bybit WebSocket client..." + Style.RESET_ALL)
        try:
            ...
            self.logger.success(Fore.GREEN + "  # Bybit WebSocket client initialized." + Style.RESET_ALL)
        except Exception as e:
            self.logger.error(Fore.RED + f"  # Failed to initialize WebSocket client: {e}" + Style.RESET_ALL)
            raise

    async def verify_connection(self):
        """Verifies the API connection by fetching server time."""
        self.logger.info(Fore.CYAN + "  # Verifying connection by fetching server time..." + Style.RESET_ALL)
        try:
            ...
            if response and response.get('retCode') == 0:
                ...
                self.logger.success(Fore.GREEN + f"  # Connection verified. Server Time (ms): {time_ms}" + Style.RESET_ALL)
            else:
                self.logger.error(Fore.RED + f"  # Connection verification failed. Response: {response}" + Style.RESET_ALL)
                raise ConnectionError(f"Failed to verify connection: {response}")
        except Exception as e:
            self.logger.error(Fore.RED + f"  # Error during connection verification: {e}" + Style.RESET_ALL)
            raise

    async def start_websocket(self, topics: List[str], market_type: str = 'linear'):
        """Subscribes to specified WebSocket topics."""
        if not self.ws_client:
            self.logger.error(Fore.RED + "  # WebSocket client not initialized. Cannot subscribe." + Style.RESET_ALL)
            return

        if not self.websocket_connected:
            self.logger.info(Fore.CYAN + "  # Starting WebSocket connection..." + Style.RESET_ALL)
            try:
                # pybit's connect() is synchronous, so we run it in an executor
                await asyncio.get_event_loop().run_in_executor(None, self.ws_client.connect)
                # Give it a moment to establish connection
                await asyncio.sleep(1) 
            except Exception as e:
                self.logger.error(Fore.RED + f"  # Failed to connect WebSocket: {e}" + Style.RESET_ALL)
                # Implement reconnection logic here if needed
                return

        self.logger.info(Fore.CYAN + f"  # Subscribing to topics: {topics} (Market Type: {market_type})..." + Style.RESET_ALL)
        try:
            # pybit's subscribe() is synchronous
            await asyncio.get_event_loop().run_in_executor(None, self.ws_client.subscribe, topics, market_type)
            for topic in topics:
                self.subscribed_topics.add(f"{topic}:{market_type}")
            self.logger.success(Fore.GREEN + "  # Subscription request sent." + Style.RESET_ALL)
        except Exception as e:
            self.logger.error(Fore.RED + f"  # Failed to subscribe to topics: {e}" + Style.RESET_ALL)
    ...
    def handle_websocket_message(self, message: Dict[str, Any]):
        ...
        if not topic or not data:
            self.logger.warning(Fore.YELLOW + f"  # Received message with missing topic or data: {message}" + Style.RESET_ALL)
            return
        ...
        try:
            ...
            # elif topic.startswith("orderbook."): ...
                ...
                self.logger.warning(Fore.YELLOW + f"  # Unhandled WebSocket topic: {topic}" + Style.RESET_ALL)

        except Exception as e:
            self.logger.error(Fore.RED + f"  # Error processing WebSocket message for topic {topic}: {e}" + Style.RESET_ALL)
    ...
    def get_latest_order_update(self, order_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _make_http_request(self, method_name: str, *args, **kwargs) -> Optional[Dict[str, Any]]:
        """Internal helper to make HTTP requests with error handling."""
        if not self.http_client:
            self.logger.error(Fore.RED + "  # HTTP client not initialized." + Style.RESET_ALL)
            return None
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(method):
                ...
                response = await asyncio.get_event_loop().run_in_executor(None, method, *args, **kwargs)
            
            # Basic check for successful response structure from pybit
            if response and response.get('retCode') == 0:
                return response
            else:
                self.logger.error(Fore.RED + f"  # HTTP Request '{method_name}' failed. Response: {response}" + Style.RESET_ALL)
                return response # Return response even on failure for inspection
        except Exception as e:
            self.logger.error(Fore.RED + f"  # Exception during HTTP request '{method_name}': {e}" + Style.RESET_ALL)
            return None
    ...
    async def get_wallet_balance(self, account_type: str = "UNIFIED", coin: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Fetches wallet balance."""
        return await self._make_http_request("get_wallet_balance", accountType=account_type, coin=coin)

    async def create_order(self, **kwargs) -> Optional[Dict[str, Any]]:
        """Creates an order."""
        # pybit's create_order is sync, run in executor
        return await self._make_http_request("create_order", **kwargs)

    async def amend_order(self, **kwargs) -> Optional[Dict[str, Any]]:
    ...
    def on_websocket_open(self):
        ...
        self.logger.success(Fore.GREEN + "  # WebSocket connection established." + Style.RESET_ALL)
        # Re-subscribe to topics if connection was lost and re-established
        if self.subscribed_topics:
            self.logger.info(Fore.CYAN + "  # Re-subscribing to topics after connection..." + Style.RESET_ALL)
            # Note: pybit's subscribe is sync, needs executor
            asyncio.get_event_loop().run_in_executor(None, self.ws_client.subscribe, list(self.subscribed_topics), 'linear') # Assuming linear for now

    def on_websocket_close(self):
        """Callback when WebSocket connection is closed."""
        self.websocket_connected = False
        self.logger.warning(Fore.YELLOW + "  # WebSocket connection closed." + Style.RESET_ALL)
    ...
    async def reconnect_websocket(self):
        ...
        if not self.websocket_connected:
            self.logger.info(Fore.CYAN + f"  # Attempting to reconnect WebSocket in {self.websocket_reconnect_delay} seconds..." + Style.RESET_ALL)
            ...
            try:
                ...
                if self.ws_client:
                    ...
                    await asyncio.sleep(1) 
                    if self.subscribed_topics:
                        self.logger.info(Fore.CYAN + "  # Re-subscribing to topics after reconnection..." + Style.RESET_ALL)
                        await asyncio.get_event_loop().run_in_executor(None, self.ws_client.subscribe, list(self.subscribed_topics), 'linear')
                else:
                    self.logger.error(Fore.RED + "  # WebSocket client is no longer available. Cannot reconnect." + Style.RESET_ALL)
            except Exception as e:
                self.logger.error(Fore.RED + f"  # Failed to reconnect WebSocket: {e}" + Style.RESET_ALL)
    ...
    def close(self):
        ...
        if self.ws_client and self.websocket_connected:
            self.logger.info(Fore.CYAN + "  # Closing WebSocket connection..." + Style.RESET_ALL)
            try:
                self.ws_client.close()
                self.websocket_connected = False
                self.logger.success(Fore.GREEN + "  # WebSocket connection closed." + Style.RESET_ALL)
            except Exception as e:
                self.logger.error(Fore.RED + f"  # Error closing WebSocket: {e}" + Style.RESET_ALL)
...
if __name__ == '__main__':
    ...
    print(Fore.MAGENTA + "# Pyrmethus: Conjuring the Bybit API Client for testing... #" + Style.RESET_ALL)
    ...
    async def test_client():
        try:
            ...
            if wallet_balance:
                logger.info(f"BTC Wallet Balance: {json.dumps(wallet_balance, indent=2)}")

            # Example: Subscribe to kline data
            await client.start_websocket(topics=["kline.5.BTCUSDT", "position.BTCUSDT"], market_type="linear")
            
            # Keep the script running for a bit to receive WebSocket messages
            logger.info("Keeping client running for 10 seconds to receive WS messages...")
