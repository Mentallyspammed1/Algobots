...
def main():
    ...
    if not os.path.exists(config_file_path):
        ...
        print(
            Fore.YELLOW
            + f"  # A dummy config file has been created at {config_file_path}."
            + Style.RESET_ALL
        )
        print(
            Fore.YELLOW
    ...
    try:
        ...
        if tickers and tickers.get("list"):
            ...
            print(
                Fore.GREEN
                + f"    Current BTCUSDT Spot Price: {btc_price}"
                + Style.RESET_ALL
            )
        else:
            print(
                Fore.RED + "    Failed to fetch BTCUSDT Spot price." + Style.RESET_ALL
            )

        print(
            Fore.CYAN
            + "\n  ## Fetching ETHUSDT Linear Perpetual Klines (last 5 1-hour candles)..."
            + Style.RESET_ALL
        )
        klines = wizard.get_kline(
            category="linear", symbol="ETHUSDT", interval="60", limit=5
        )
        if klines and klines.get("list"):
            ...
            for kline in reversed(klines["list"]):
                print(
                    Fore.WHITE
                    + f"      Time: {kline[0]}, Open: {kline[1]}, Close: {kline[4]}"
                    + Style.RESET_ALL
                )
        else:
            ...
            for acc in balance["list"]:
                for coin_info in acc["coins"]:
                    if coin_info["coin"] == "USDT":
                        print(
                            Fore.GREEN
                            + f"    USDT Available Balance: {coin_info['availableToWithdraw']}"
                            + Style.RESET_ALL
                        )
                        print(
                            Fore.GREEN
                            + f"    USDT Total Balance: {coin_info['walletBalance']}"
                            + Style.RESET_ALL
                        )
                        break
        else:
        ...
        print(
            Fore.CYAN
            + "\n  ## Attempting to place a small market buy order for BTCUSDT (Testnet)..."
            + Style.RESET_ALL
        )
        print(
            Fore.YELLOW
            + "    (This will only work if your API keys are configured for testnet and have trading permissions)"
            + Style.RESET_ALL
        )
        ...
        print(
            Fore.RED
            + f"# A critical error occurred during the integration ritual: {e}"
            + Style.RESET_ALL
        )
        print(
            Fore.YELLOW
            + "# Please ensure your .bybit_config file is correctly set up and dependencies are installed."
            + Style.RESET_ALL
        )
