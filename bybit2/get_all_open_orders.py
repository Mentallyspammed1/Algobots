...
def get_all_open_orders():
    ...
    try:
        ...
        for category in categories:
            print(Fore.CYAN + f"  # Fetching open orders for category: {category}..." + Style.RESET_ALL)
            ...
            if response and response['retCode'] == 0:
                if response['result'] and response['result']['list']:
                    ...
                    print(Fore.YELLOW + f"  # No open orders found for category: {category}." + Style.RESET_ALL)
            else:
                print(Fore.RED + f"  # Failed to retrieve open orders for category {category}." + Style.RESET_ALL)
                print(Fore.RED + f"  # Response: {response}" + Style.RESET_ALL)

        if all_open_orders:
            print(Fore.GREEN + "  # All Active Open Orders:" + Style.RESET_ALL)
            for order in all_open_orders:
                print(Fore.WHITE + f"    Symbol: {order.get('symbol')}, Order ID: {order.get('orderId')}, Side: {order.get('side')}, "
                      f"Order Type: {order.get('orderType')}, Price: {order.get('price')}, Qty: {order.get('qty')}, "
                      f"Status: {order.get('orderStatus')}" + Style.RESET_ALL)
        else:
            print(Fore.YELLOW + "  # No active open orders found across all categories." + Style.RESET_ALL)
