...
from decimal import Decimal, ROUND_DOWN, ROUND_UP, ROUND_HALF_EVEN, getcontext, InvalidOperation
from logging.handlers import RotatingFileHandler
from pathlib import Path
from typing import Any, Dict, Optional, Tuple, List, Union, Final, Callable
...
try:
    ...
    def load_dotenv(): pass  # Dummy function if not installed
...
class _FallbackZoneInfo:
    ...
    def __init__(self, key: str):
        if not _FallbackZoneInfo._warning_printed:
            print(f"{Fore.YELLOW}Warning: 'zoneinfo' or 'tzdata' module (for full timezone support) could not be imported. For full timezone support, please install 'tzdata': {Style.BRIGHT}pip install tzdata{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}Falling back to a basic UTC timezone handler. This may affect timestamp accuracy for non-UTC timezones.{Style.RESET_ALL}")
        ...
        if key.lower() != 'utc':
            print(f"{Fore.YELLOW}Warning: Timezone '{key}' not fully supported by fallback ZoneInfo. Using UTC.{Style.RESET_ALL}")
...
try:
    ...
    print(f"{Fore.YELLOW}Warning: 'zoneinfo' module (for full timezone support) could not be imported: {e}{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}If you are on Python 3.9+ and encounter issues, please install 'tzdata': {Style.BRIGHT}pip install tzdata{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}Falling back to a basic UTC timezone handler. For full timezone support, consider installing 'tzdata' or using Python 3.9+.{Style.RESET_ALL}")
    ZoneInfo = _FallbackZoneInfo
...
NG: Final[str] = Fore.LIGHTGREEN_EX + \
    Style.BRIGHT  # Success, positive outcomes
...
AK: Final[Optional[str]] = os.getenv("BYBIT_API_KEY")
AS: Final[Optional[str]] = os.getenv("BYBIT_API_SECRET")
if not AK or not AS:
    raise ValueError(
...
try:
    ...
    print(f"{NY}Warning: Could not load timezone '{os.getenv('TIMEZONE', DDTZ)}'. Using UTC. Error: {tz_err}{RST}")
    TZ = ZoneInfo("UTC")

MAR: Final[int] = 3                                # Max API retry attempts
RDS: Final[int] = 5                                # Retry delay in seconds
VI: Final[List[str]] = ["1", "3", "5", "15", "30", "60",
                        "120", "240", "D", "W", "M"]  # Valid intervals
CIM: Final[Dict[str, str]] = {"1": "1m", "3": "3m", "5": "5m", "15": "15m", "30": "30m",
...
DIP: Final[Dict[str, Union[int, Decimal]]] = {
    "atr_period": 14, "cci_window": 20, "williams_r_window": 14, "mfi_window": 14, "rsi_window": 14,
    "bollinger_bands_period": 20, "bollinger_bands_std_dev": Decimal("2.0"),
    "sma_10_window": 10, "ema_short_period": 9, "ema_long_period": 21,
    "momentum_period": 7, "volume_ma_period": 15, "fib_window": 50,
    "psar_af": Decimal("0.02"), "psar_max_af": Decimal("0.2")
    # Removed: "stoch_rsi_window": 14, "stoch_window": 12, "k_window": 3, "d_window": 3
}
FL: Final[List[Decimal]] = [Decimal("0.0"), Decimal("0.236"), Decimal("0.382"), Decimal(
    # Fibonacci Levels
    "0.5"), Decimal("0.618"), Decimal("0.786"), Decimal("1.0")]
...
def format_metric(
    ...
    if isinstance(value, (Decimal, float)):
        if is_pnl:
            ...
            formatted_value = f"{actual_value_color}{sign}{value:,.{value_precision}f}{unit}{RST}"
        else:
            formatted_value = f"{actual_value_color}{value:,.{value_precision}f}{unit}{RST}"
...
def print_table_row(
    ...
    for i, cell_value in enumerate(row_data):
        width = column_widths[i] if i < len(
            column_widths) else 10  # Default width
        color = default_color
...
def display_open_positions(
    ...
    columns = [
        ("Symbol", 18), ("Side", 6), ("Size", 12), ("Entry", 14),
        ("Current", 14), (f"uPnL ({quote_currency})", 18), ("uPnL (%)", 10),
    ...
    for trade_record in open_trades.values():
        ...
        if not market_info:
            ...
            row_data = [
                symbol, trade_record.side.upper(), str(trade_record.size),
                str(trade_record.entry_price), "N/A", "N/A", "N/A",
                "N/A", "N/A", "N/A", "N/A"  # For new columns
            ]
            cell_colors = [NC, NG if trade_record.side ==
                           "long" else NR] + [NC] * 9
        ...
        if trade_record.trailing_stop_active:
            act_price_str = f"{trade_record.tsl_activation_price:.{price_prec}f}" if trade_record.tsl_activation_price else "N/A"
            # Assuming distance is price units
            dist_str = f"{trade_record.trailing_stop_distance:.{price_prec}f}" if trade_record.trailing_stop_distance else "N/A"
        ...
        # TP Info column
        tp_info_str = f"{trade_record.take_profit_price:.{price_prec}f}" if trade_record.take_profit_price else "N/A"
        ...
        if current_price and trade_record.entry_price and trade_record.entry_price != 0:
            ...
            if sl_target_for_dist_calc:
                if trade_record.side == "long":
                    dist_sl_pct = (
                        (current_price - sl_target_for_dist_calc) / trade_record.entry_price) * 100
                else:  # Short
                    dist_sl_pct = (
                        (sl_target_for_dist_calc - current_price) / trade_record.entry_price) * 100
            ...
            if trade_record.take_profit_price:
                if trade_record.side == "long":
                    dist_tp_pct = ((trade_record.take_profit_price -
                                   current_price) / trade_record.entry_price) * 100
                else:  # Short
                    dist_tp_pct = (
                        (current_price - trade_record.take_profit_price) / trade_record.entry_price) * 100
        ...
        row_data_list = [
            ...
            trade_record.pnl_percentage if trade_record.pnl_percentage is not None else "N/A",
            sl_info_str,
            tp_info_str,
            dist_sl_pct_str,
            dist_tp_pct_str
        ...
        print_table_row(row_data_list, column_widths, cell_colors=cell_custom_colors, default_color=NC, pnl_columns=[
                        5, 6], decimal_precision=2)  # Default precision for percentages
...
def display_recent_closed_trades(
    ...
    columns = [
        ("Symbol", 18), ("Side", 6), ("Size", 12), ("Entry", 14),
        ("Exit", 14), (f"rPnL ({quote_currency})", 18), ("rPnL (%)", 10),
        ("Status", 12),
        ("E:RSI", 8),    # Entry RSI
    ...
    for trade_record in recent_trades_to_show:
        ...
        side_color = NG if trade_record.side == "long" else NR
        status_text = trade_record.status.replace(
            "CLOSED_", "")  # Shorten status
        status_color = NC
        ...
        row_data_list = [
            ...
            trade_record.side.upper(),
            f"{trade_record.size:.{size_prec}f}",
            f"{trade_record.entry_price:.{price_prec}f}" if trade_record.entry_price else "N/A",
            f"{trade_record.exit_price:.{price_prec}f}" if trade_record.exit_price else "N/A",
            trade_record.realized_pnl_quote if trade_record.realized_pnl_quote is not None else "N/A",
            trade_record.pnl_percentage if trade_record.pnl_percentage is not None else "N/A",
            status_text,
            f"{trade_record.entry_rsi:.2f}" if trade_record.entry_rsi is not None else "N/A",
            f"{trade_record.entry_ema_short:.{price_prec}f}" if trade_record.entry_ema_short is not None else "N/A",
            f"{trade_record.entry_ema_long:.{price_prec}f}" if trade_record.entry_ema_long is not None else "N/A",
            f"{trade_record.entry_bbl:.{price_prec}f}" if trade_record.entry_bbl is not None else "N/A",
            f"{trade_record.entry_bbm:.{price_prec}f}" if trade_record.entry_bbm is not None else "N/A",
            f"{trade_record.entry_bbu:.{price_prec}f}" if trade_record.entry_bbu is not None else "N/A",
            f"{trade_record.entry_momentum:.4f}" if trade_record.entry_momentum is not None else "N/A",
            f"{trade_record.entry_psar:.{price_prec}f}" if trade_record.entry_psar is not None else "N/A",
        ]
        ...
        cell_custom_colors = [
            ...
            status_color,  # Status
            NC, NC, NC, NC, NC, NC, NC, NC # Colors for new indicator columns
        ]
        ...
        print_table_row(
            ...
            decimal_precision=4 if quote_currency != "USDT" else 2  # PnL quote precision
        )
...
def _eck(current_config: Dict[str, Any], default_config: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def _vncv(
    config_data: Dict[str, Any], key: str, default_value: Union[int, float, Decimal, None],
    min_value: Optional[Union[int, float, Decimal]] = None, max_value: Optional[Union[int, float, Decimal]] = None,
    is_integer: bool = False, allow_none: bool = False, logger: logging.Logger = logging.getLogger(__name__)
) -> bool:
    """
...
def lc(file_path: Path) -> Dict[str, Any]:
    ...
    default_config: Final[Dict[str, Any]] = {
        ...
        "atr_period": DIP["atr_period"], "ema_short_period": DIP["ema_short_period"],
        "ema_long_period": DIP["ema_long_period"], "rsi_period": DIP["rsi_window"],
        "bollinger_bands_period": DIP["bollinger_bands_period"], "bollinger_bands_std_dev": DIP["bollinger_bands_std_dev"],
        "cci_window": DIP["cci_window"], "williams_r_window": DIP["williams_r_window"],
        "mfi_window": DIP["mfi_window"],
        "fisher_period": 12,
        "psar_af": DIP["psar_af"], "psar_max_af": DIP["psar_max_af"],
        "sma_10_window": DIP["sma_10_window"], "momentum_period": DIP["momentum_period"],
        ...
        "orderbook_limit": 25, "signal_score_threshold": Decimal("1.5"),
        "stop_loss_multiple": Decimal("1.8"), "take_profit_multiple": Decimal("0.7"),
        ...
        "volume_confirmation_multiplier": Decimal("1.5"), "scalping_signal_threshold": Decimal("2.5"),
        "fibonacci_window": DIP["fib_window"], "enable_trading": False, "use_sandbox": True,
        "risk_per_trade": Decimal("0.01"), "leverage": 20, "max_concurrent_positions": 1,
        "quote_currency": "USDT", "entry_order_type": "market", "limit_order_offset_buy": Decimal("0.0005"),
        ...
        "position_confirm_delay_seconds": PCDS, "time_based_exit_minutes": None,
        ...
        "weight_sets": {"scalping": {"ema_alignment": Decimal("0.2"), "momentum": Decimal("0.3"), "volume_confirmation": Decimal("0.2"),
                                     "rsi": Decimal("0.2"), "bollinger_bands": Decimal("0.3"),
                                     "vwap": Decimal("0.4"), "cci": Decimal("0.3"), "wr": Decimal("0.3"),
                                     "psar": Decimal("0.2"), "sma_10": Decimal("0.1"), "mfi": Decimal("0.2"),
                                     "orderbook": Decimal("0.15"), "fisher": Decimal("0.15"), "adaptive_stoch": Decimal("0.4")},
                        "default": {"ema_alignment": Decimal("0.3"), "momentum": Decimal("0.2"), "volume_confirmation": Decimal("0.1"),
                                    "rsi": Decimal("0.3"), "bollinger_bands": Decimal("0.2"),
                                    "vwap": Decimal("0.3"), "cci": Decimal("0.2"), "wr": Decimal("0.2"),
                                    "psar": Decimal("0.3"), "sma_10": Decimal("0.1"), "mfi": Decimal("0.2"),
                                    "orderbook": Decimal("0.1"), "fisher": Decimal("0.15"), "adaptive_stoch": Decimal("0.4")}},
        "indicators": {  # Enable/disable individual indicators
            "atr": True, "ema_alignment": True, "momentum": True, "volume_confirmation": True,
            "rsi": True, "bollinger_bands": True, "vwap": True,
            "cci": True, "wr": True, "psar": True, "sma_10": True, "mfi": True,
            "orderbook": True, "fisher": {"enabled": True, "period": 12}, "adaptive_stoch": True,
        },
        "exit_strategies": {
    ...
    if not file_path.exists():
        try:
            ...
            file_path.write_text(json.dumps(
                serialized_config, indent=4), encoding="utf-8")
            logger.info(
    ...
    try:
        ...
        numeric_params_to_validate = {
            ...
            "trailing_stop_profit_threshold_percentage": {"min": Decimal('0'), "max": Decimal('1')},
        ...
        for param_key, default_val in DIP.items():
            ...
            if param_key in ["psar_af", "psar_max_af"]:
                ...
                if param_key == "psar_af":
                    if _vncv(user_config, param_key, default_val, min_value=min_val_for_psar, max_value=max_val_for_psar_af, is_integer=False, logger=logger):
                        save_needed = True
                elif param_key == "psar_max_af":
                    if _vncv(user_config, param_key, default_val, min_value=min_val_for_psar, max_value=max_val_for_psar_max_af, is_integer=False, logger=logger):
                        save_needed = True
            else:
                if _vncv(user_config, param_key, default_val, min_value=1, is_integer=isinstance(default_val, int), logger=logger):
                    save_needed = True
        ...
        for indicator_key, weight_value in current_active_weights.items():
            if _vncv(current_active_weights, indicator_key, Decimal("0.0"), min_value=Decimal("0.0"), logger=logger):
                save_needed = True
                user_config["weight_sets"][user_config["active_weight_set"]
                                           ][indicator_key] = current_active_weights[indicator_key]
        ...
        for threshold_key, default_val in default_thresholds.items():
            ...
            if _vncv(indicator_thresholds, threshold_key, default_val, min_value=None, max_value=None, is_integer=is_int, logger=logger):
                save_needed = True
                user_config["indicator_thresholds"][threshold_key] = indicator_thresholds[threshold_key]

        # Validate Fisher specific period
        fisher_indicator_config = user_config.get(
            "indicators", {}).get("fisher", {})
        if isinstance(fisher_indicator_config, dict):  # Ensure fisher config is a dict
            if _vncv(
                fisher_indicator_config, "period",
                default_config.get("indicators", {}).get(
                    "fisher", {}).get("period", 12),
                min_value=2, is_integer=True, logger=logger
            ):
                save_needed = True
                ...
                user_config["indicators"]["fisher"]["period"] = fisher_indicator_config["period"]
        # If fisher exists but is not a dict (e.g. old format True/False)
            ...
            user_config["indicators"]["fisher"] = default_config.get(
                "indicators", {}).get("fisher", {"enabled": True, "period": 12})
            save_needed = True
        ...
        if not isinstance(exit_strategies_config.get("opposing_signal_enabled"), bool):
            ...
            exit_strategies_config["opposing_signal_enabled"] = default_exit_strategies.get(
                "opposing_signal_enabled", True)
            save_needed = True

        if not isinstance(exit_strategies_config.get("extreme_oscillator_enabled"), bool):
            logger.warning(
                f"{NY}Invalid type for 'extreme_oscillator_enabled' in 'exit_strategies'. Setting to default.{RST}")
            exit_strategies_config["extreme_oscillator_enabled"] = default_exit_strategies.get(
                "extreme_oscillator_enabled", True)
            save_needed = True

        if _vncv(exit_strategies_config, "opposing_signal_threshold",
                 default_exit_strategies.get(
                     "opposing_signal_threshold", Decimal("0.5")),
                 min_value=Decimal("0.1"), max_value=Decimal("1.0"), logger=logger):
            save_needed = True

        if _vncv(exit_strategies_config, "min_profit_percentage_for_advanced_exits",
                 default_exit_strategies.get(
                     "min_profit_percentage_for_advanced_exits", Decimal("0.25")),
                 min_value=Decimal("0.0"), logger=logger):
            save_needed = True
...
def retry_api_call(max_retries: int = MAR, retry_delay: int = RDS, catch_exceptions: Tuple = (
        ccxt.NetworkError, ccxt.RequestTimeout, requests.exceptions.ConnectionError, requests.exceptions.Timeout, ccxt.RateLimitExceeded)):  # Removed ccxt.ExchangeError from default
    """
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    logger.error(
                        f"{NR}Unexpected error in {func.__name__}: {e}. Aborting. A cosmic interference!{RST}", exc_info=True)
                    raise
...
@retry_api_call()
def ie(config: Dict[str, Any], logger: logging.Logger) -> Optional[ccxt.Exchange]:
    """
...
def fcp(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Decimal]:
    ...
    last_price = Decimal(str(ticker.get('last'))) if ticker.get(
        'last') is not None else None
    bid_price = Decimal(str(ticker.get('bid'))) if ticker.get(
        'bid') is not None else None
    ask_price = Decimal(str(ticker.get('ask'))) if ticker.get(
        'ask') is not None else None

    if last_price is not None and last_price > 0:
        price = last_price
        logger.debug(
            f"Using 'last' price: {price}. A clear echo from the market.")
    elif bid_price is not None and ask_price is not None and bid_price > 0 and ask_price > 0 and ask_price >= bid_price:
        price = (bid_price + ask_price) / 2
        logger.debug(
            f"Using bid/ask midpoint: {price}. The equilibrium revealed.")
...
@retry_api_call()
def fkc(exchange: ccxt.Exchange, symbol: str, timeframe: str, limit: int = 250, logger: logging.Logger = None) -> pd.DataFrame:
    """
...
@retry_api_call()
def fobc(exchange: ccxt.Exchange, symbol: str, limit: int, logger: logging.Logger) -> Optional[Dict]:
    """
...
class TA:
    ...
    def __init__(self, data_frame: pd.DataFrame, logger: logging.Logger, config: Dict[str, Any], market_info: Dict[str, Any]):
        self.d = data_frame
        self.lg = logger
        self.cfg = config
        self.mi = market_info
        ...
        self.tcn: Dict[str, Optional[str]] = {}
        self.indicator_thresholds = self.cfg.get("indicator_thresholds", {})
        self.dynamic_stoch_k: Optional[int] = None
    ...
    def _gtcn(self, base_name: str, processed_df: pd.DataFrame) -> Optional[str]:
        """
        ...
        params_config = {
            ...
            "BB_Period": self.cfg.get("bollinger_bands_period", DIP["bollinger_bands_period"]),
            "BB_StdDev": float(self.cfg.get("bollinger_bands_std_dev", DIP["bollinger_bands_std_dev"])),
            ...
            "Volume_MA_Period": self.cfg.get("volume_ma_period", DIP["volume_ma_period"]),
            "Fisher_Period": self.cfg.get("indicators", {}).get("fisher", {}).get("period", 12),
            # Adaptive_Stoch_K will use self.dynamic_stoch_k directly in the pattern generation
            "Adaptive_Stoch_Smooth_K": self.cfg.get("adaptive_stoch_smooth_k_period", 3),
            "Adaptive_Stoch_D": self.cfg.get("adaptive_stoch_d_period", 3),
        }

        psar_af_str = f"{Decimal(str(params_config['PSAR_AF'])).normalize():.2f}".rstrip(
            '0').rstrip('.')
        psar_max_af_str = f"{Decimal(str(params_config['PSAR_MaxAF'])).normalize():.1f}".rstrip(
            '0').rstrip('.')
        ...
        column_name_patterns = {
            ...
            "BB_Lower": [f"BBL_{params_config['BB_Period']}_{params_config['BB_StdDev']:.1f}".replace('.0', ''), f"BBL_{params_config['BB_Period']}"],
            "BB_Middle": [f"BBM_{params_config['BB_Period']}_{params_config['BB_StdDev']:.1f}".replace('.0', ''), f"BBM_{params_config['BB_Period']}"],
            "BB_Upper": [f"BBU_{params_config['BB_Period']}_{params_config['BB_StdDev']:.1f}".replace('.0', ''), f"BBU_{params_config['BB_Period']}"],
        ...
        if base_name == "Adaptive_STOCHk":
            k_val = self.dynamic_stoch_k if self.dynamic_stoch_k is not None else self.cfg.get(
                "adaptive_stoch_min_lookback", 5)
    ...
    def _cai(self):
        ...
        params_config = {
            ...
            "Adaptive_Stoch_ATR_Smoothing_Period": self.cfg.get("adaptive_stoch_atr_smoothing_period", 50),
            "Adaptive_Stoch_Min_Lookback": self.cfg.get("adaptive_stoch_min_lookback", 5),
            "Adaptive_Stoch_Max_Lookback": self.cfg.get("adaptive_stoch_max_lookback", 20),
            "Adaptive_Stoch_Smooth_K_Period": self.cfg.get("adaptive_stoch_smooth_k_period", 3),
        ...
        required_periods.append(self.cfg.get(
            "fibonacci_window", DIP["fib_window"]))

        min_recommended_data = max(required_periods) + \
            20 if required_periods else 50
        min_recommended_data = max(min_recommended_data, 2)

        if len(self.d) < min_recommended_data:
            self.lg.warning(f"{NY}Insufficient data ({len(self.d)} points) for {self.s} to calculate all indicators reliably (min recommended: {min_recommended_data}). Results may contain NaNs. The data tapestry is too short!{RST}")
        ...
        try:
            ...
            if indicators_config.get("ema_alignment", False):
                ...
                if ema_short_results is not None and not ema_short_results.empty:
                    data_copy = pd.concat(
                        [data_copy, ema_short_results], axis=1)
                ...
                if ema_long_results is not None and not ema_long_results.empty:
                    data_copy = pd.concat(
                        [data_copy, ema_long_results], axis=1)
                self.tcn["EMA_Long"] = self._gtcn("EMA_Long", data_copy)

            if indicators_config.get("momentum", False):
                momentum_period = params_config["Momentum"]
                mom_results = data_copy.ta.mom(
                    length=momentum_period, append=False)
            ...
            if indicators_config.get("wr", False):
                ...
                willr_results = data_copy.ta.willr(
                    length=willr_period, append=False)
            ...
            if indicators_config.get("mfi", False):
                ...
                try:
                    ...
                    self.lg.error(
                        f"{NR}MFI calculation failed for {self.s}: {e}. Skipping MFI. The money flow is obscured!{RST}", exc_info=True)
            ...
            if indicators_config.get("sma_10", False):
                ...
                sma_results = data_copy.ta.sma(
                    length=sma10_period, append=False)
            ...
            if indicators_config.get("adaptive_stoch", False):
                ...
                if not atr_col_name_for_stoch or atr_col_name_for_stoch not in data_copy.columns:
                    self.lg.warning(
                    ...
                    if atr_results_temp is not None and not atr_results_temp.empty:
                        data_copy = pd.concat(
                            [data_copy, atr_results_temp], axis=1)
                ...
                if atr_col_name_for_stoch and atr_col_name_for_stoch in data_copy.columns:
                    atr_series = data_copy[atr_col_name_for_stoch].dropna()
                    if not atr_series.empty:
                        smoothing_window = params_config["Adaptive_Stoch_ATR_Smoothing_Period"]
                        ...
                        if (max_atr - min_atr) > Decimal("1e-9"):
                            normalized_atr = (Decimal(str(latest_atr)) - Decimal(str(min_atr))) / (
                                Decimal(str(max_atr)) - Decimal(str(min_atr)) + Decimal("1e-10"))

                        normalized_atr = max(Decimal("0.0"), min(
                            Decimal("1.0"), normalized_atr))
                        ...
                        self.dynamic_stoch_k = max(int(min_lb), min(
                            int(max_lb), self.dynamic_stoch_k))
                        self.lg.debug(
                ...
                stoch_results = data_copy.ta.stoch(
                    k=stoch_k_val, d=stoch_d_val, smooth_k=stoch_smooth_k_val, append=False)
                if stoch_results is not None and not stoch_results.empty:
                    data_copy = pd.concat([data_copy, stoch_results], axis=1)

                expected_stochk_col = f"STOCHk_{stoch_k_val}_{stoch_d_val}_{stoch_smooth_k_val}"
                expected_stochd_col = f"STOCHd_{stoch_k_val}_{stoch_d_val}_{stoch_smooth_k_val}"

                self.tcn["Adaptive_STOCHk"] = expected_stochk_col if expected_stochk_col in data_copy.columns else self._gtcn(
                    "Adaptive_STOCHk", data_copy)
                self.tcn["Adaptive_STOCHd"] = expected_stochd_col if expected_stochd_col in data_copy.columns else self._gtcn(
                    "Adaptive_STOCHd", data_copy)
            ...
            if indicators_config.get("volume_confirmation", False):
                ...
                data_copy['volume'] = data_copy['volume'].astype(
                    float)
                data_copy[volume_ma_col_name] = ta.sma(data_copy['volume'].fillna(
                    0), length=volume_ma_period)
            ...
            if isinstance(fisher_config, dict) and fisher_config.get("enabled", False):
                ...
                if fisher_results_df is not None and not fisher_results_df.empty:
                    data_copy = pd.concat(
                        [data_copy, fisher_results_df], axis=1)
            ...
            self.lg.error(
                f"{NR}AttributeError calculating indicators for {self.s}: {e}. Check pandas_ta usage and data. A missing thread in the weave!{RST}", exc_info=True)
        except Exception as e:
            self.lg.error(
                f"{NR}Error calculating indicators with pandas_ta for {self.s}: {e}. The indicator spells faltered!{RST}", exc_info=True)
    ...
    def _uliv(self):
        ...
        if self.d.empty:
            ...
            self.iv = {k: Decimal(np.nan) for k in list(
                self.tcn.keys()) + ["Close", "Volume", "High", "Low", "Open"]}
            return
        ...
        try:
            ...
            for key, col_name in self.tcn.items():
                if col_name and col_name in latest_row.index:
                    ...
                    if pd.notna(value):
                        ...
                        except (ValueError, TypeError, InvalidOperation) as conversion_error:
                            self.lg.warning(
            ...
            for k, v in self.iv.items():
                if pd.notna(v):
                    if isinstance(v, Decimal):
                        precision_for_log = price_precision if k in [
                            'Open', 'High', 'Low', 'Close', 'ATR'] else 6
            ...
            self.lg.error(
                f"{NR}Unexpected error updating latest indicator values for {self.s}: {e}. A glitch in the scrying mirror!{RST}", exc_info=True)
            self.iv = {k: Decimal(np.nan) for k in list(
                self.tcn.keys()) + ["Close", "Volume", "High", "Low", "Open"]}

    def cfl(self, window: Optional[int] = None) -> Dict[str, Decimal]:
        """
        ...
        try:
            ...
            if price_difference > 0:
                for level_percent in FL:
                    ...
                    if min_tick_size > 0:
                        quantized_level = (calculated_level / min_tick_size).quantize(
                            Decimal('1'), rounding=ROUND_HALF_EVEN) * min_tick_size
                    else:
                ...
                quantized_level_zero_range = highest_price.quantize(min_tick_size if min_tick_size > 0 else Decimal(
                    '1e-' + str(price_precision)), rounding=ROUND_HALF_EVEN)
                for level_percent in FL:
                    levels[f"Fib_{level_percent * 100:.1f}%"] = quantized_level_zero_range
            ...
            self.lg.error(
                f"{NR}Unexpected Fibonacci calculation error for {self.s}: {e}. A cosmic disturbance in the patterns!{RST}", exc_info=True)
            self.fld = {}
            return {}
    ...
    def gpp(market_info: Dict[str, Any], logger: logging.Logger) -> int:
        ...
        try:
            ...
            if price_precision_value is not None:
                if isinstance(price_precision_value, int) and price_precision_value >= 0:
                    logger.debug(
        ...
        try:
            ...
            if min_price_value is not None:
                try:
                    ...
                    if min_tick_from_limits > 0 and min_tick_from_limits < Decimal('0.1'):
                        logger.debug(
    ...
    def gnfl(self, current_price: Decimal, num_levels: int = 5) -> List[Tuple[str, Decimal]]:
        """
        ...
        try:
            ...
            for name, level_price in self.fld.items():
                if isinstance(level_price, Decimal) and level_price > 0:
                    levels_with_distance.append(
                        {'name': name, 'level': level_price, 'distance': abs(current_price - level_price)})
                else:
                    self.lg.warning(
            ...
            self.lg.error(
                f"{NR}Error finding nearest Fibonacci levels for {self.s}: {e}. The pattern's proximity is obscured!{RST}", exc_info=True)
            return []
    ...
    def _cm(self) -> Decimal:
        ...
        try:
            ...
            if momentum_percentage > threshold_positive:
                score = min(Decimal("1.0"), (momentum_percentage - threshold_positive) / (
                ...
                score = max(Decimal("-1.0"), (momentum_percentage + threshold_positive) / (
                    threshold_strong_positive - threshold_positive))
                return score if score < 0 else Decimal("0.0")
            else:
                return Decimal("0.0")
    ...
    def _cvc(self) -> Decimal:
        ...
        multiplier = self.cfg.get(
            "volume_confirmation_multiplier", Decimal("1.5"))

        if pd.isna(current_volume) or current_volume < 0 or pd.isna(volume_ma) or volume_ma <= 0:
            self.lg.debug(
        ...
        try:
            ...
            if volume_ratio > multiplier:
                score = min(Decimal("1.0"), Decimal(
                    "0.5") + ((volume_ratio - multiplier) / (multiplier * Decimal("2"))))
                return score
    ...
    def _cadstoch(self) -> Decimal:
        ...
        overbought_threshold = Decimal(
            str(self.indicator_thresholds.get("adaptive_stoch_overbought_threshold", 80)))
    ...
    def _cr(self) -> Decimal:
        ...
        rsi_approaching_overbought = Decimal(
            str(self.indicator_thresholds.get("rsi_approaching_overbought_threshold", 60)))
    ...
    def _csma(self) -> Decimal:
        ...
        sma10_score = self.indicator_thresholds.get(
            "sma10_score", Decimal("0.6"))
    ...
    def _cv(self) -> Decimal:
        ...
        vwap_score = self.indicator_thresholds.get(
            "vwap_score", Decimal("0.7"))
    ...
    def _cbb(self) -> Decimal:
        ...
        if pd.isna(bb_lower) or pd.isna(bb_middle) or pd.isna(bb_upper) or pd.isna(last_close_decimal):
            self.lg.debug(
        ...
        if band_width > 0:
            ...
            score = max(Decimal("-1.0"),
                        min(Decimal("1.0"), relative_position))
    ...
    def _cfisher(self) -> Decimal:
        ...
        """
        fisher_value = self.iv.get(
            "FISHERT")  # Using "FISHERT" as the key from self.iv
    ...
    def _cob(self, orderbook_data: Optional[Dict], current_price: Decimal) -> Decimal:
        """
        ...
        try:
            ...
            bid_volumes_sum = sum(Decimal(str(bid[1]))
                                  for bid in top_bids if len(bid) == 2)
            ask_volumes_sum = sum(Decimal(str(ask[1]))
                                  for ask in top_asks if len(ask) == 2)
            ...
            order_book_imbalance_decimal = (
                bid_volumes_sum - ask_volumes_sum) / total_volume
            ...
            self.lg.warning(
                f"{NY}Orderbook analysis failed for {self.s} (data conversion error): {e}. A numerical distortion in the market's desires!{RST}", exc_info=False)
            return Decimal(np.nan)
        except Exception as e:
            self.lg.warning(
                f"{NY}Orderbook analysis failed for {self.s} (unexpected error): {e}. A cosmic interference in the market's intentions!{RST}", exc_info=True)
            return Decimal(np.nan)
    ...
    def gts(self, current_price: Decimal, orderbook_data: Optional[Dict]) -> str:
        ...
        core_indicators_present = any(pd.notna(v) for k, v in self.iv.items() if k not in [
        ...
        method_map = {
            "ema_alignment": "_cea", "momentum": "_cm", "volume_confirmation": "_cvc",
            "adaptive_stoch": "_cadstoch", "rsi": "_cr", "bollinger_bands": "_cbb",
            "vwap": "_cv", "cci": "_cc", "wr": "_cwr", "psar": "_cpsar",
            "sma_10": "_csma", "mfi": "_cmfi", "orderbook": "_cob", "fisher": "_cfisher",
        }
        ...
        for indicator_key, ind_config in self.cfg.get("indicators", {}).items():
            ...
            if method_name and hasattr(self, method_name) and callable(getattr(self, method_name)):
                ...
                try:
                    ...
                    self.lg.error(
                        f"{NR}Error executing indicator check method {method_name} for {self.s}: {e}. The indicator's spell fizzled!{RST}", exc_info=True)

                debug_scores[indicator_key] = f"{indicator_score_decimal:.3f}" if pd.notna(
                    indicator_score_decimal) else "NaN"

                if pd.notna(indicator_score_decimal):
                    try:
                        clamped_score = max(
                            Decimal("-1.0"), min(Decimal("1.0"), indicator_score_decimal))
                        final_signal_score += clamped_score * weight
                        total_weighted_active += weight
                        active_indicator_count += 1
                    except (InvalidOperation, ValueError, TypeError) as conversion_error:
                        self.lg.error(
        ...
        log_message = (f"Signal Summary ({self.s} @ {current_price:.{price_precision_log}f}): "
    ...
    def cets(self, entry_price_estimate: Decimal, signal: str) -> Tuple[Optional[Decimal], Optional[Decimal], Optional[Decimal]]:
        """
        ...
        if not isinstance(entry_price_estimate, Decimal) or entry_price_estimate <= Decimal('0'):
            self.lg.warning(
        ...
        take_profit_offset: Optional[Decimal] = None
        stop_loss_offset: Optional[Decimal] = None
        ...
        try:
            if use_atr_volume_tp_sl:
                ...
                if isinstance(current_volume, Decimal) and isinstance(volume_ma, Decimal) and volume_ma > Decimal('0'):
                    if current_volume > (volume_ma * volume_tp_threshold_multiplier) and tp_atr_boost_on_high_volume > Decimal('0'):
                    ...
                    if current_volume > (volume_ma * volume_sl_threshold_multiplier) and sl_atr_widening_on_high_volume > Decimal('0'):
            ...
            raw_take_profit_price: Optional[Decimal] = None
            raw_stop_loss_price: Optional[Decimal] = None
            ...
            quantized_take_profit_price: Optional[Decimal] = None
            quantized_stop_loss_price: Optional[Decimal] = None

            if raw_take_profit_price is not None:
                rounding_mode_tp = ROUND_UP if signal == "BUY" else ROUND_DOWN
                if min_tick_size > Decimal('0'):
                    quantized_take_profit_price = (raw_take_profit_price / min_tick_size).quantize(
                        Decimal('1'), rounding=rounding_mode_tp) * min_tick_size
                else:
            ...
            if raw_stop_loss_price is not None:
                ...
                if min_tick_size > Decimal('0'):
                    quantized_stop_loss_price = (raw_stop_loss_price / min_tick_size).quantize(
                        Decimal('1'), rounding=rounding_mode_sl) * min_tick_size
                else:
            ...
            if final_sl is not None and min_tick_size > Decimal('0'):
                if signal == "BUY" and final_sl >= entry_price_estimate:
                    ...
                    final_sl = ((entry_price_estimate - min_tick_size) / min_tick_size).quantize(
                        Decimal('1'), rounding=ROUND_DOWN) * min_tick_size
                    ...
                    final_sl = ((entry_price_estimate + min_tick_size) / min_tick_size).quantize(
                        Decimal('1'), rounding=ROUND_UP) * min_tick_size
            ...
            self.lg.error(
                f"{NR}Error calculating TP/SL for {self.s} {signal} (Decimal/Type Error): {e}. A numerical distortion in the boundaries!{RST}", exc_info=False)
            return entry_price_estimate, None, None
        except Exception as e:
            self.lg.error(
                f"{NR}Unexpected error calculating TP/SL for {self.s} {signal}: {e}. A cosmic interference in setting the path!{RST}", exc_info=True)
            return entry_price_estimate, None, None


@retry_api_call()
def fb(exchange: ccxt.Exchange, currency: str, logger: logging.Logger) -> Optional[Decimal]:
    """
    ...
    for acct_type in account_types_to_try:
        try:
            ...
            if 'info' in balance_info and isinstance(balance_info['info'], dict) and isinstance(balance_info['info'].get('list'), list):
                    ...
                    if isinstance(account.get('coin'), list):  # Unified account format
                        for coin_data in account['coin']:
                            if coin_data.get('coin') == currency:
                                found_balance = coin_data.get('availableToWithdraw') or coin_data.get(
                                    'availableBalance') or coin_data.get('walletBalance')
                        ...
                        if available_balance_str is not None:
                            break
                    # Single coin in list
                    elif isinstance(account.get('coin'), dict) and account['coin'].get('coin') == currency:
                        found_balance = account['coin'].get('availableToWithdraw') or account['coin'].get(
                            'availableBalance') or account['coin'].get('walletBalance')
                        if found_balance is not None:
                            available_balance_str = str(found_balance)
                            logger.debug(
            ...
            if available_balance_str is not None:  # If found via Bybit V5 parsing, break from account type loop
                break
    ...
    if available_balance_str is None:  # If not found via specific types, try default fetch
        logger.debug(
        ...
        if currency in balance_info and balance_info[currency].get('free') is not None:
            ...
            total_balance_fallback = balance_info.get(
                currency, {}).get('total')
...
def send_termux_sms(recipient: str, message: str, logger: logging.Logger) -> bool:
    ...
    if not message or not isinstance(message, str):
        logger.error(
            f"{NR}SMS sending failed: Message is empty or not a string.{RST}")
        return False
    ...
    try:
        ...
        logger.error(
            f"{NR}An unexpected error occurred while sending SMS to {recipient}: {e}{RST}", exc_info=True)
        return False


@retry_api_call()
def gmi(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Dict]:
    """
    ...
    if symbol not in exchange.markets:
        ...
        if '/' in symbol:
            ...
            possible_matches = [sym for sym in exchange.markets if sym.startswith(
                base) and ('PERP' in sym or ':USDT' in sym or 'USD' in sym)]
...
@retry_api_call()
def cps(
    balance: Decimal, risk_per_trade: Decimal, initial_stop_loss_price: Decimal, entry_price: Decimal,
    market_info: Dict, exchange: ccxt.Exchange, logger: Optional[logging.Logger] = None
) -> Optional[Decimal]:
    """
    ...
    if not isinstance(risk_per_trade, Decimal) or not (Decimal('0') < risk_per_trade <= Decimal('1')):
        raise ValueError(
    ...
    logger.info(f"{NC}Position Sizing ({symbol}): Balance={balance:.2f} {quote_currency}, Risk={risk_per_trade:.2%}, RiskAmt={risk_amount_quote:.4f} {quote_currency}. Balancing the scales of fortune.{RST}")
    logger.info(
    ...
    min_amount = Decimal(str(amount_limits.get('min'))) if amount_limits.get(
        'min') is not None else Decimal('0')
    max_amount = Decimal(str(amount_limits.get('max'))) if amount_limits.get(
        'max') is not None else Decimal('inf')
    min_cost = Decimal(str(cost_limits.get('min'))) if cost_limits.get(
        'min') is not None else Decimal('0')
    max_cost = Decimal(str(cost_limits.get('max'))) if cost_limits.get(
        'max') is not None else Decimal('inf')
    ...
    if min_cost > 0 and estimated_cost < min_cost:
        ...
        allowed_size_for_max_cost: Optional[Decimal] = None
        if market_info.get('linear', True) or not is_contract:
            denominator = entry_price * contract_size
            if denominator > 0:
...
def _pmap(amount_decimal: Decimal, market_info: Dict, logger: logging.Logger) -> Optional[Decimal]:
    """
    ...
    try:
        if isinstance(amount_precision_value, int):
            ...
            final_amount = amount_decimal.quantize(
            ...
            if amount_step_size > 0:
                ...
                step_decimal_places = abs(amount_step_size.normalize(
                ).as_tuple().exponent) if amount_step_size.is_normal() else 0
        ...
        logger.error(
            f"{NR}Error during manual amount precision for {symbol}: {e}. Returning original amount. A numerical distortion in the shaping!{RST}", exc_info=True)
        return amount_decimal
    except Exception as e:
        logger.error(
            f"{NR}Unexpected error during manual amount precision for {symbol}: {e}. Returning original amount. A cosmic interference!{RST}", exc_info=True)
        return amount_decimal


@retry_api_call()
def gop(exchange: ccxt.Exchange, symbol: str, market_info: Dict, logger: logging.Logger) -> Optional[Dict]:
    """
    ...
    try:
        if isinstance(amount_precision_value, (float, str)):
            amount_step_size = Decimal(str(amount_precision_value))
            size_threshold = amount_step_size / \
                Decimal('2') if amount_step_size > 0 else Decimal('1e-9')
        elif isinstance(amount_precision_value, int):
            size_threshold = Decimal(
                '1e-' + str(amount_precision_value + 1))
        else:
    ...
    for position in positions_list:
        ...
        elif isinstance(position.get('info'), dict) and position['info'].get('size') is not None:
    ...
    if active_position_data:
        try:
            size_decimal = Decimal(str(active_position_data.get(
                'contracts', active_position_data.get('info', {}).get('size', '0'))))
            ...
            entry_price_raw = active_position_data.get(
                'entryPrice') or active_position_data.get('info', {}).get('avgPrice')
            active_position_data['entryPriceDecimal'] = Decimal(
                str(entry_price_raw)) if entry_price_raw else None

            info_dict = active_position_data.get('info', {})

            # Retrieve raw values
            raw_sl = active_position_data.get(
                'stopLossPrice') or info_dict.get('stopLoss')
            raw_tp = active_position_data.get(
                'takeProfitPrice') or info_dict.get('takeProfit')
            ...
            active_position_data['trailingStopActivationPrice'] = _gop_decimal_converter(
                raw_tsl_activation, logger, symbol, 'trailingStopActivationPrice')

            timestamp_ms = active_position_data.get(
                'timestamp') or info_dict.get('updatedTime')
            active_position_data['timestamp_ms'] = timestamp_ms

            def format_log_value(value_raw, is_price=True, is_size=False):
                if value_raw is None or str(value_raw).strip() == '' or str(value_raw) == '0':
                    return 'N/A'
                try:
            ...
            contract_size_formatted = format_log_value(
                size_decimal, is_size=True)
            ...
            leverage_formatted = f"{Decimal(str(leverage_raw)):.1f}x" if leverage_raw is not None else 'N/A'
...
@retry_api_call()
def slc(exchange: ccxt.Exchange, symbol: str, leverage: int, market_info: Dict, logger: logging.Logger) -> bool:
    """
    ...
    try:
        ...
        return True
    except ccxt.BadRequest as e:  # More specific catch for errors that might contain retCode
        ...
        is_leverage_not_modified_error = (
            "bybit" in exchange.id.lower() and
            ('"retcode":110043' in error_message or
             '"retmsg":"leveragenotmodified"' in error_message or
        ...
        if is_leverage_not_modified_error:
            ...
            try:
                ...
                if '"retmsg":"' in str(e).lower():
                    original_ret_msg = str(e).lower().split(
                        '"retmsg":"')[1].split('"')[0]
            except Exception:
            ...
            raise
    except ccxt.ExchangeError as e:  # Catch other exchange errors that are not BadRequest
        logger.warning(
...
@retry_api_call()
def pt(
    exchange: ccxt.Exchange, symbol: str, trade_side: str, position_size: Decimal, market_info: Dict, logger: Optional[logging.Logger] = None,
    order_type: str = 'market', limit_price: Optional[Decimal] = None, reduce_only: bool = False, extra_params: Optional[Dict] = None
) -> Optional[Dict]:
    """
    ...
    if 'bybit' in exchange.id.lower():
        order_params['category'] = 'linear' if market_info.get(
            'linear') else 'inverse'

    logger.info(f"{NB}Attempting to place {action_description} {side_lower.upper()} {order_type.upper()} order for {symbol}: Casting the trade spell!{RST}")
    ...
    logger.info(
        f"{NB}  Params: {order_params}. The specific incantations.{RST}")

    order_response: Optional[Dict] = None
    if order_type == 'market':
        order_response = exchange.create_order(
            symbol=symbol, type='market', side=side_lower, amount=formatted_amount_float, params=order_params)
    elif order_type == 'limit':
        order_response = exchange.create_order(
            symbol=symbol, type='limit', side=side_lower, amount=formatted_amount_float, price=formatted_price_float, params=order_params)
    else:
        raise ValueError(
    ...
    if order_response:
        ...
        logger.info(
            f"{NG}  Order ID: {order_id}, Initial Status: {order_status}{RST}")
...
def spp(
    ...
    protection_result: Dict[str, Any] = {
        ...
        'success': False,
        'stopLoss': stop_loss_price if isinstance(stop_loss_price, Decimal) and stop_loss_price > 0 else None,
        'takeProfit': take_profit_price if isinstance(take_profit_price, Decimal) and take_profit_price > 0 else None,
        'trailingStopDistance': trailing_stop_distance if isinstance(trailing_stop_distance, Decimal) and trailing_stop_distance > 0 else None,
        'trailingStopActivationPrice': trailing_stop_activation_price if isinstance(trailing_stop_activation_price, Decimal) and trailing_stop_activation_price > 0 else None,
        'stopLossOrder': None,  # Explicitly None as this function won't place it
        'takeProfitOrder': None  # Explicitly None
    }

    logger.info(f"{NB}spp: Received request for {symbol}. SL: {stop_loss_price}, TP: {take_profit_price}, TSL Dist: {trailing_stop_distance}, TSL Act: {trailing_stop_activation_price}.{RST}")
    logger.warning(f"{NY}spp: This function is now a parameter validator/structurer due to 'set_trading_stop' being bypassed. It will NOT place SL/TP orders itself for Bybit. The caller is responsible for using appropriate Bybit V5 API methods (e.g., private_post_position_set_trading_stop).{RST}")
    ...
    has_any_protection = any([
        ...
        protection_result['trailingStopDistance'] and protection_result['trailingStopActivationPrice']
    ])
...
@retry_api_call()
def stsl(
    exchange: ccxt.Exchange, symbol: str, market_info: Dict, position_info: Dict, config: Dict[str, Any], logger: logging.Logger,
    ...
    calculated_tsl_distance: Optional[Decimal] = None
    calculated_tsl_activation_price: Optional[Decimal] = None

    if attempt_tsl and config.get("enable_trailing_stop", False):
        try:
            callback_rate = config.get(
                "trailing_stop_callback_rate", DIP["psar_af"])
            activation_percentage = config.get(
                "trailing_stop_activation_percentage", DIP["psar_af"])

            if not (isinstance(callback_rate, Decimal) and isinstance(activation_percentage, Decimal) and callback_rate > 0 and activation_percentage >= 0):
                logger.error(
                ...
                entry_price = position_info.get(
                    'entryPriceDecimal', trade_record.entry_price)  # Prefer live position info
                ...
                if not entry_price or entry_price <= 0 or not pos_side:
                    ...
                    if calculated_tsl_activation_price <= 0:
                        ...
                        calculated_tsl_activation_price = calculated_tsl_activation_price.quantize(
                            min_tick_size if min_tick_size > 0 else Decimal(f'1e-{price_precision}'), rounding=ROUND_HALF_EVEN)

                        calculated_tsl_distance = callback_rate
                        if not (isinstance(calculated_tsl_distance, Decimal) and calculated_tsl_distance > 0):
                            logger.error(
            ...
            logger.error(
                f"{NR}stsl: Error during TSL param calculation for {symbol}: {e}. Skipping TSL.{RST}", exc_info=True)
            should_set_tsl = False
    ...
    current_tsl_act_on_exchange = position_info.get(
    ...
    if should_set_tsl and calculated_tsl_distance and calculated_tsl_activation_price:
        ...
        logger.info(f"{NB}stsl: Preparing to set TSL for {symbol}. Activation: {calculated_tsl_activation_price}, Distance: {calculated_tsl_distance}. Fixed SL will be cleared.{RST}")
    ...
    if (params_to_set.get('stopLoss', "0") == "0" and
        params_to_set.get('takeProfit', "0") == "0" and
        params_to_set.get('trailingStop', "0") == "0" and
            not current_fixed_sl_on_exchange and not current_tp_on_exchange and not current_tsl_dist_on_exchange):
        logger.info(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"{NR}[xrscalper_bot_{symbol}] - stsl: JSONDecodeError setting protections for {symbol}. "
        ...
        logger.error(
            f"{NR}stsl: HTTP RequestException setting protections for {symbol}: {e}{RST}", exc_info=True)
    ...
    if response_data and response_data.get('retCode') == 0:
        ...
        if should_set_tsl and calculated_tsl_distance and calculated_tsl_activation_price:
            ...
            if params_to_set.get('stopLoss') and params_to_set['stopLoss'] != "0":
                trade_record.stop_loss_price = Decimal(
                    params_to_set['stopLoss'])
            else:
                trade_record.stop_loss_price = None

        if params_to_set.get('takeProfit') and params_to_set['takeProfit'] != "0":
            trade_record.take_profit_price = Decimal(
                params_to_set['takeProfit'])
        else:
            trade_record.take_profit_price = None
        ...
        logger.error(f"{NR}stsl: Failed to set protections for {symbol} via HTTP POST. retCode: {ret_code}, retMsg: {ret_msg}. Full Response: {json.dumps(response_data, default=str) if response_data else 'No response data'}{RST}")
        return False
...
class Tr:
    ...
    pnl_quote: Optional[Decimal] = None
    ...
    status: str = "OPEN"
    entry_rsi: Optional[Decimal] = None
    exit_rsi: Optional[Decimal] = None
    entry_ema_short: Optional[Decimal] = None
    exit_ema_short: Optional[Decimal] = None
    entry_ema_long: Optional[Decimal] = None
    exit_ema_long: Optional[Decimal] = None
    entry_bbl: Optional[Decimal] = None
    exit_bbl: Optional[Decimal] = None
    entry_bbm: Optional[Decimal] = None
    exit_bbm: Optional[Decimal] = None
    entry_bbu: Optional[Decimal] = None
    exit_bbu: Optional[Decimal] = None
    entry_momentum: Optional[Decimal] = None
    exit_momentum: Optional[Decimal] = None
    entry_cci: Optional[Decimal] = None
    exit_cci: Optional[Decimal] = None
    entry_willr: Optional[Decimal] = None
    exit_willr: Optional[Decimal] = None
    entry_mfi: Optional[Decimal] = None
    exit_mfi: Optional[Decimal] = None
    entry_vwap: Optional[Decimal] = None
    exit_vwap: Optional[Decimal] = None
    entry_psar: Optional[Decimal] = None
    exit_psar: Optional[Decimal] = None
    entry_fisher: Optional[Decimal] = None
    exit_fisher: Optional[Decimal] = None
    entry_adx: Optional[Decimal] = None
    exit_adx: Optional[Decimal] = None
    highest_price_reached: Optional[Decimal] = None
    lowest_price_reached: Optional[Decimal] = None
    ...
    def from_dict(cls, data: Dict[str, Any]):
        ...
        for key, value in data.items():
            if key in ['entry_price', 'size', 'leverage', 'initial_capital',
                       'stop_loss_price', 'take_profit_price',
                       'trailing_stop_distance', 'tsl_activation_price',
                       'exit_price', 'pnl_quote', 'pnl_percentage',
                       'fees_in_quote', 'realized_pnl_quote',
                       'entry_rsi', 'exit_rsi', 'entry_ema_short', 'exit_ema_short',
                       'entry_ema_long', 'exit_ema_long', 'entry_bbl', 'exit_bbl',
                       'entry_bbm', 'exit_bbm', 'entry_bbu', 'exit_bbu',
                       'entry_momentum', 'exit_momentum', 'entry_cci', 'exit_cci',
                       'entry_willr', 'exit_willr', 'entry_mfi', 'exit_mfi',
                       'entry_vwap', 'exit_vwap', 'entry_psar', 'exit_psar',
                       'entry_fisher', 'exit_fisher', 'entry_adx', 'exit_adx',
                ...
                if value is not None:
                    try:
                        data[key] = datetime.fromisoformat(value).replace(
                            tzinfo=TZ)  # Ensure timezone-aware
                    except (TypeError, ValueError):  # Handles if value is not a valid ISO string
                        data[key] = None
        ...
        new_decimal_fields = [
            ...
            'entry_vwap', 'exit_vwap', 'entry_psar', 'exit_psar',
            'entry_fisher', 'exit_fisher', 'entry_adx', 'exit_adx',
            'highest_price_reached', 'lowest_price_reached' # Added new decimal fields
        ]
        for decimal_field in new_decimal_fields:
            if decimal_field in data and not isinstance(data[decimal_field], Decimal) and data[decimal_field] is not None:
    ...
    def upnl(self, current_price: Decimal, market_info: Dict, logger: logging.Logger):
        ...
        try:
            ...
            if self.side == "long":
                ...
                unrealized_pnl = (self.entry_price -
                                  current_price) * self.size * contract_size

            pnl_percentage = (unrealized_pnl / self.initial_capital) * \
                Decimal('100') if self.initial_capital > 0 else Decimal('0')
            ...
            logger.error(
                f"{NR}Error calculating unrealized PnL for {self.symbol}: {e}. A numerical distortion in PnL calculation!{RST}", exc_info=False)
        except Exception as e:
            logger.error(
                f"{NR}Unexpected error updating PnL for {self.symbol}: {e}. A cosmic interference in PnL tracking!{RST}", exc_info=True)
...
class TMT:
    ...
    def __init__(self, quote_currency_symbol: str, logger: logging.Logger, exchange_ref: Optional[ccxt.Exchange] = None):
        self.open_trades: Dict[str, Tr] = {}
        self.closed_trades: List[Tr] = []
        self.qc = quote_currency_symbol
        self.ex_ref: Optional[ccxt.Exchange] = exchange_ref
        self.lg = logger
    ...
    def _load_trades(self):
        ...
        if TRP.exists():
            try:
                with open(TRP, 'r', encoding='utf-8') as f:
                    ...
                    for trade_dict in data.get('open_trades', []):
                        ...
                        if trade_record.symbol not in self.mi:
                            ...
                            if self.ex_ref:  # Check if the exchange reference was provided
                                ...
                                if market_info_on_the_fly:
                                    self.mi[trade_record.symbol] = market_info_on_the_fly
                                    self.lg.info(
                ...
                self.lg.error(
                    f"{NR}Unexpected error during trade record loading: {e}. A cosmic interference!{RST}", exc_info=True)
                self._reset_trades()
        else:
            self.lg.info(
    ...
    def _save_trades(self):
        ...
        try:
            ...
            self.lg.error(
                f"{NR}Unexpected error during trade record saving: {e}. A cosmic interference!{RST}", exc_info=True)
    ...
    def aot(self, trade_record: Tr):
        ...
        """
        self.open_trades[trade_record.symbol] = trade_record
        self.lg.info(f"{NG}TRADE TRACKER: Added new OPEN trade for {trade_record.symbol} ({trade_record.side}) at {trade_record.entry_price} with size {trade_record.size}. A new quest begins!{RST}")
        self._save_trades()

    def ct(self, symbol: str, exit_price: Decimal, exit_time: datetime, current_balance: Decimal, fees_in_quote: Decimal = Decimal('0')):
        """
        ...
        if trade_record:
            ...
            contract_size = Decimal(
                str(symbol_market_info.get('contractSize', '1')))

            if trade_record.side == "long":
                trade_record.pnl_quote = (
                    trade_record.exit_price - trade_record.entry_price) * trade_record.size * contract_size
            else:
                trade_record.pnl_quote = (
                    trade_record.entry_price - trade_record.exit_price) * trade_record.size * contract_size

            trade_record.pnl_percentage = (trade_record.pnl_quote / trade_record.initial_capital) * Decimal(
                '100') if trade_record.initial_capital > 0 else Decimal('0')
            ...
            self.lg.info(f"{color_pnl}TRADE TRACKER: Closed {trade_record.symbol} trade. Realized PnL: {trade_record.realized_pnl_quote:.4f} {self.qc} ({trade_record.pnl_percentage:.2f}%). Fees: {trade_record.fees_in_quote:.4f}. The trade's fate is sealed!{RST}")
            self._save_trades()
        else:
            self.lg.warning(
    ...
    def dm(self):
        ...
        winning_trades = sum(
            1 for trade in self.closed_trades if trade.realized_pnl_quote is not None and trade.realized_pnl_quote >= 0)
        losing_trades = total_closed_trades - winning_trades
        win_rate = (winning_trades / total_closed_trades *
                    100) if total_closed_trades > 0 else 0

        total_pnl_percentage = Decimal('0')
        if self.initial_balance > 0:
            total_pnl_percentage = (
                self.total_pnl / self.initial_balance) * Decimal('100')

        avg_pnl_per_trade = (
            self.total_pnl / total_closed_trades) if total_closed_trades > 0 else Decimal('0')

        winning_scores = [
            trade.realized_pnl_quote for trade in self.closed_trades if trade.realized_pnl_quote is not None and trade.realized_pnl_quote >= 0]
        losing_scores = [
            trade.realized_pnl_quote for trade in self.closed_trades if trade.realized_pnl_quote is not None and trade.realized_pnl_quote < 0]

        average_win = sum(winning_scores) / \
            len(winning_scores) if winning_scores else Decimal('0')
        average_loss = sum(losing_scores) / \
            len(losing_scores) if losing_scores else Decimal('0')

        max_win_amount = max(
            winning_scores) if winning_scores else Decimal('0')
        max_loss_amount = min(losing_scores) if losing_scores else Decimal('0')

        max_win_percentage = max(
            trade.pnl_percentage for trade in self.closed_trades if trade.pnl_percentage is not None and trade.pnl_percentage >= 0) if winning_scores else Decimal('0')
        max_loss_percentage = min(
            trade.pnl_percentage for trade in self.closed_trades if trade.pnl_percentage is not None and trade.pnl_percentage < 0) if losing_scores else Decimal('0')

        metrics_output = (f"Total PnL={self.total_pnl:,.2f} {self.qc} | "
        ...
        # Overall PnL
        print(format_metric("Total Realized PnL", self.total_pnl, NG if self.total_pnl >= 0 else NR, # Changed NP to NG/NR
              label_width=label_width, unit=f" {self.qc}", is_pnl=True, value_precision=4))
        if self.initial_balance > 0:
            print(format_metric("Total PnL % (on Initial)", total_pnl_percentage,
                  NG if total_pnl_percentage >= 0 else NR, label_width=label_width, unit="%", is_pnl=True)) # Changed NP to NG/NR

        print_neon_separator(length=70, char="·", color=NP) # Changed NC to NP for section separator

        # Trade Counts & Win Rate
        print(format_metric("Total Closed Trades",
              total_closed_trades, NB, label_width=label_width))
        print(format_metric("Winning Trades",
              winning_trades, NG, label_width=label_width))
        print(format_metric("Losing Trades",
              losing_trades, NR, label_width=label_width))
        print(format_metric("Win Rate", win_rate, NB,
              label_width=label_width, unit="%", value_precision=2))

        print_neon_separator(length=70, char="·", color=NC)

        # Averages
        print(format_metric("Avg PnL per Trade", avg_pnl_per_trade, NG if avg_pnl_per_trade >=0 else NR, # Changed NB to NG/NR
              label_width=label_width, unit=f" {self.qc}", is_pnl=True, value_precision=4))
        print(format_metric("Avg Winning Trade", average_win, NG,
              label_width=label_width, unit=f" {self.qc}", is_pnl=True, value_precision=4))
        print(format_metric("Avg Losing Trade", average_loss, NR,
              label_width=label_width, unit=f" {self.qc}", is_pnl=True, value_precision=4))

        print_neon_separator(length=70, char="·", color=NP) # Changed NC to NP for section separator

        # Max Win/Loss
        print(format_metric(f"Max Win ({self.qc})", max_win_amount, NG,
              label_width=label_width, unit=f" {self.qc}", is_pnl=True, value_precision=4))
        print(format_metric("Max Win %", max_win_percentage, NG,
              label_width=label_width, unit="%", is_pnl=True))

        print(format_metric(f"Max Loss ({self.qc})", max_loss_amount, NR,
              label_width=label_width, unit=f" {self.qc}", is_pnl=True, value_precision=4))
        print(format_metric("Max Loss %", max_loss_percentage,
              NR, label_width=label_width, unit="%", is_pnl=True))
        ...
        print(format_metric("Gross Profit", gross_profit, NG,
              label_width=label_width, unit=f" {self.qc}", value_precision=4))
        print(format_metric("Gross Loss", -gross_loss_abs, NR, label_width=label_width,
              # Display gross loss as negative
                            unit=f" {self.qc}", value_precision=4))
        print(format_metric("Profit Factor",
              profit_factor_str, profit_factor_color, label_width=label_width))
        ...
        for trade in self.closed_trades:
            if trade.entry_time and trade.exit_time and isinstance(trade.entry_time, datetime) and isinstance(trade.exit_time, datetime):
                duration_seconds = (
        ...
        avg_duration_str = format_duration(
            total_duration_seconds, trades_with_duration)
        ...
        print(format_metric("Avg Trade Duration",
              avg_duration_str, NB, label_width=label_width))
        print(format_metric("Avg Win Duration",
              avg_win_duration_str, NG, label_width=label_width))
        print(format_metric("Avg Loss Duration",
              avg_loss_duration_str, NR, label_width=label_width))
...
class BybitWebSocketClient:
    ...
    def __init__(self, api_key: str, api_secret: str, use_testnet: bool, logger: logging.Logger, public_base_url: str):
        ...
        if use_testnet:
            ...
            self.logger.warning(
                f"{NY}WebSocket client is configured for TESTNET.{RST}")
    ...
    def _on_message_public(self, ws, message):
        try:
            ...
            if topic and topic.startswith('tickers.'):
                ...
                if ticker_data:
                    ...
                    if last_price is not None:
                        try:
                            self.last_prices[symbol_id] = Decimal(
                                str(last_price))
                            for callback in self.callbacks["ticker"]:
                                callback(
                                    {"symbol_id": symbol_id, "lastPrice": self.last_prices[symbol_id]})
                        except InvalidOperation:
                            self.logger.warning(
            ...
            self.logger.error(
                f"{NR}WS Public: Error processing message: {e}. Message: {message}{RST}", exc_info=True)
    ...
    def _on_message_private(self, ws, message):
        try:
            ...
            if data.get('op') == 'auth':
                if data.get('success'):
                    self.logger.info(
                        f"{NG}WS Private: Authentication successful.{RST}")
            ...
            self.logger.error(
                f"{NR}WS Private: Error processing message: {e}. Message: {message}{RST}", exc_info=True)
    ...
    def _on_open_private(self, ws):
        ...
        while not self.auth_success and time.time() - start_time < 10:  # 10 second timeout for auth
            time.sleep(0.1)
        ...
        if self.auth_success:
            for topic in self.private_subscriptions:
                ws.send(json.dumps({"op": "subscribe", "args": [topic]}))
                self.logger.debug(
                    f"{NB}WS Private: Subscribed to {topic}.{RST}")
        else:
            self.logger.error(
...
class PB:
    ...
    def __init__(self, symbol: str, exchange: ccxt.Exchange, config: Dict[str, Any], logger: logging.Logger, ws_client: BybitWebSocketClient):
        self.s = symbol
        self.ex = exchange
        self.cfg = config
        self.lg = logger
        self.ws_client = ws_client  # WebSocket client instance
        self.mi: Optional[Dict] = None
        self.lct: Optional[pd.Timestamp] = None
        self.ta_analyzer: Optional[TA] = None
        ...
        if not self.bybit_symbol_id:
            ...
            self.bybit_symbol_id = self.s.replace(
                '/', '').replace(':', '')  # Best effort conversion
        ...
        self.ws_client.register_callback(
            "position", self._handle_position_update)

    def _get_indicator_value(self, primary_key: str, *alternative_keys: str) -> Optional[Decimal]:
        """
        ...
        if not self.ta_analyzer or not hasattr(self.ta_analyzer, 'iv'):
            self.lg.warning(f"{NY}_get_indicator_value: ta_analyzer or its 'iv' attribute is not available for {self.s}.{RST}")
            return None
        ...
        if not isinstance(val, Decimal):
            try:
                ...
                self.lg.warning(f"{NY}_get_indicator_value: Could not convert value '{val}' for key '{primary_key}' to Decimal for {self.s}. Returning None.{RST}")
                return None

        # self.lg.debug(f"_get_indicator_value: Successfully retrieved value for key '{primary_key}' (or alternatives): {val} for {self.s}.")
        return val

    def _get_psar_value(self) -> Optional[Decimal]:
        """Helper to retrieve PSAR value, checking PSAR_long then PSAR_short."""
        if not self.ta_analyzer or not hasattr(self.ta_analyzer, 'iv'):
            self.lg.warning(f"{NY}_get_psar_value: ta_analyzer or 'iv' not available for {self.s}.{RST}")
            return None
    ...
    def _cet(self, exit_reason: str, current_price: Decimal) -> bool:
        ...
        if self.ta_analyzer and hasattr(self.ta_analyzer, 'iv'):
            ...
            current_open_trade.exit_willr = self._get_indicator_value("Williams_R", "WILLR")
            ...
            current_open_trade.exit_adx = self._get_indicator_value("ATR") # Using ATR as per instruction
            self.lg.debug(f"Populated exit indicators for {self.s} in Tr object.")
        else:
            self.lg.warning(f"{NY}Could not populate exit indicators for {self.s}: ta_analyzer or iv not available.{RST}")
        ...
        try:
            ...
            if not refreshed_position or abs(Decimal(str(refreshed_position.get('size', '0')))) <= Decimal('1e-9'):
                self.lg.info(
                ...
                tt.ct(self.s, exit_price=current_price, exit_time=datetime.now(
                    TZ), current_balance=latest_balance)
                ...
                if enable_sms_alerts and sms_recipient_number and sms_alert_on_trade_close:
            ...
            if order_response:
                ...
                start_time = time.time()
                while time.time() - start_time < PCDS:  # PCDS seconds timeout for WS confirmation
                    latest_ws_position = self.ws_client.get_latest_position_update(
                        self.bybit_symbol_id)
                    if latest_ws_position and abs(Decimal(str(latest_ws_position.get('size', '0')))) <= Decimal('1e-9'):
                        self.lg.info(
                        ...
                        tt.ct(self.s, exit_price=current_price, exit_time=datetime.now(
                            TZ), current_balance=latest_balance, fees_in_quote=fees)

                        # SMS Alert for trade closure
                        enable_sms_alerts = self.cfg.get(
                            "enable_sms_alerts", False)
                        sms_recipient_number = self.cfg.get(
                            "sms_recipient_number", "")
                        sms_alert_on_trade_close = self.cfg.get(
                            "sms_alert_on_trade_close", True)
                ...
                final_position_check_rest = gop(
                    self.ex, self.s, self.mi, self.lg)
                if not final_position_check_rest or abs(final_position_check_rest.get('contractsDecimal', Decimal('0'))) <= Decimal('1e-9'):
                    self.lg.info(
            ...
            self.lg.error(
                f"{NR}An unexpected error occurred while trying to close trade for {self.s}: {e}. A cosmic interference in sealing the trade!{RST}", exc_info=True)
            return False
        ...
        try:
            ...
            for order in open_orders:
                try:
                    ...
                    self.lg.error(
                        f"{NR}Error cancelling order {order['id']} for {self.s}: {e}. The revocation spell faltered!{RST}", exc_info=True)
            self.lg.info(
            ...
            self.lg.error(
                f"{NR}Unexpected error fetching/cancelling open orders for {self.s}: {e}. A cosmic interference in order management!{RST}", exc_info=True)
        return False
    ...
    def _mp(self, current_price: Decimal) -> bool:
        ...
        if not current_open_trade:
            pos_info_early_check = self.ws_client.get_latest_position_update(self.bybit_symbol_id)
            ...
            if not pos_info_early_check or abs(Decimal(str(pos_info_early_check.get('size', pos_info_early_check.get('contracts', '0'))))) < TA.gmts(self.mi, self.lg):
                self.lg.debug(f"_mp: No open trade in tracker and no live position for {self.s}.")
                return False

        # Fetch TSL configuration, ensuring Decimals
        try:
            tsl_profit_thresh_raw = self.cfg.get("trailing_stop_profit_threshold_percentage", "0.001")
            ...
            if tsl_percentage <= Decimal("0"): # TSL percentage must be positive
                self.lg.warning(f"{NY}TSL percentage is zero or negative ({tsl_percentage}). Using default 0.005.{RST}")
                tsl_percentage = Decimal("0.005")
        except InvalidOperation:
            self.lg.error(f"{NR}_mp: Invalid TSL configuration strings. Using default TSL values.{RST}")
        ...
        if not pos_info:
            self.lg.debug(f"_mp: No WebSocket position data for {self.s}, fetching via REST.")
        ...
        live_pos_size_raw = pos_info.get('size') if pos_info and 'size' in pos_info else (pos_info.get('contractsDecimal') if pos_info else None)
        live_pos_size = Decimal(str(live_pos_size_raw)) if live_pos_size_raw is not None else Decimal('0')

        if abs(live_pos_size) < TA.gmts(self.mi, self.lg): # Position effectively closed
            if current_open_trade: # But tracker still has it open
                ...
                exit_price_for_calc = current_price if current_price and current_price > 0 else current_open_trade.entry_price
        ...
        if not current_open_trade:
            ...
            entry_price_raw = pos_info.get('entryPriceDecimal', pos_info.get('avgPrice', pos_info.get('entryPrice')))
            leverage_raw = pos_info.get('leverage', self.cfg.get('leverage', 1))
            
            if entry_price_raw is None or live_pos_size == Decimal('0') or leverage_raw is None:
                 self.lg.error(f"{NR}_mp: Cannot reconstruct trade for {self.s} due to missing critical info: entry={entry_price_raw}, size={live_pos_size}, lev={leverage_raw}.{RST}")
            ...
            if leverage_dec == Decimal('0'): leverage_dec = Decimal('1') # Avoid division by zero

            initial_capital_est = (entry_price_dec * abs(live_pos_size)) / leverage_dec
            if initial_capital_est <= Decimal('0'): initial_capital_est = Decimal('1')


            current_open_trade = Tr(
                symbol=self.s, side=pos_info.get('side', 'long' if live_pos_size > 0 else 'short'), # Infer side if missing
                ...
                highest_price_reached=entry_price_dec if (pos_info.get('side') == 'long' or live_pos_size > 0) else None,
                lowest_price_reached=entry_price_dec if (pos_info.get('side') == 'short' or live_pos_size < 0) else None,
            ...
            current_open_trade.stop_loss_price = Decimal(str(sl_from_pos)) if sl_from_pos and str(sl_from_pos) != "0" else None
            current_open_trade.take_profit_price = Decimal(str(tp_from_pos)) if tp_from_pos and str(tp_from_pos) != "0" else None
            tt.aot(current_open_trade)
            trade_modified_in_tracker = True
            self.lg.info(f"{NB}_mp: Reconstructed trade record for {self.s} added to tracker.{RST}")
        
        # --- 2. Update PnL and Peak Prices ---
        if not isinstance(current_price, Decimal) or current_price <= 0:
            self.lg.error(f"{NR}_mp: Invalid current_price ({current_price}) for {self.s}. Cannot manage position.{RST}")
            if trade_modified_in_tracker: tt._save_trades() # Save if reconstruction happened
        ...
        if not isinstance(entry_price, Decimal): # Should not happen if Tr is well-managed
            entry_price = Decimal(str(entry_price))
            self.lg.warning(f"{NY}_mp: current_open_trade.entry_price was not Decimal. Converted: {entry_price}")
        ...
        if not current_open_trade.tsl_activated and \
           current_open_trade.pnl_percentage is not None and \
           current_open_trade.pnl_percentage >= tsl_profit_thresh:
            
            current_open_trade.tsl_activated = True
            trade_modified_in_tracker = True
            self.lg.info(f"{NG}TSL activated for {self.s} at profit {current_open_trade.pnl_percentage:.2f}%.{RST}")

            if current_open_trade.side == "long":
                current_open_trade.highest_price_reached = current_price # Reset peak price at TSL activation
            else:
                current_open_trade.lowest_price_reached = current_price # Reset peak price at TSL activation
            
            # Cancel fixed Take Profit if it exists on exchange
            live_tp_raw = pos_info.get('takeProfitPrice') or pos_info.get('takeProfit')
            live_tp_on_exchange = Decimal(str(live_tp_raw)) if live_tp_raw and str(live_tp_raw) != "0" else None

            if live_tp_on_exchange is not None:
                self.lg.info(f"{NB}Cancelling existing Take Profit ({live_tp_on_exchange}) for {self.s} as TSL is now active.{RST}")
                sl_to_preserve_raw = pos_info.get('stopLossPrice') or pos_info.get('stopLoss')
                sl_to_preserve = Decimal(str(sl_to_preserve_raw)) if sl_to_preserve_raw and str(sl_to_preserve_raw) != "0" else None
                
                stsl_tp_cancel_success = stsl(
                    exchange=self.ex, symbol=self.s, market_info=self.mi,
                    position_info=pos_info, config=self.cfg, logger=self.lg,
                ...
                if stsl_tp_cancel_success:
                    ...
                    self.lg.warning(f"{NY}Failed to cancel Take Profit for {self.s} during TSL activation.{RST}")
            else:
                 self.lg.info(f"{NB}No Take Profit found on exchange for {self.s} during TSL activation. No TP cancellation needed.{RST}")


        # --- 4. TSL Price Calculation & Order Placement (if tsl_activated) ---
        if current_open_trade.tsl_activated:
            new_tsl_price: Optional[Decimal] = None
            min_tick_size = TA.gmts(self.mi, self.lg)
            
            if current_open_trade.side == "long" and current_open_trade.highest_price_reached is not None:
                calc_tsl = current_open_trade.highest_price_reached * (Decimal("1") - tsl_percentage)
                if min_tick_size > Decimal("0"):
                    new_tsl_price = (calc_tsl / min_tick_size).quantize(Decimal("1"), rounding=ROUND_UP) * min_tick_size
                else:
                    price_prec = TA.gpp(self.mi, self.lg)
                    new_tsl_price = calc_tsl.quantize(Decimal('1e-' + str(price_prec)), rounding=ROUND_UP)

            elif current_open_trade.side == "short" and current_open_trade.lowest_price_reached is not None:
                calc_tsl = current_open_trade.lowest_price_reached * (Decimal("1") + tsl_percentage)
                if min_tick_size > Decimal("0"):
                    new_tsl_price = (calc_tsl / min_tick_size).quantize(Decimal("1"), rounding=ROUND_DOWN) * min_tick_size
                else:
                    price_prec = TA.gpp(self.mi, self.lg)
                    new_tsl_price = calc_tsl.quantize(Decimal('1e-' + str(price_prec)), rounding=ROUND_DOWN)

            if new_tsl_price is not None and new_tsl_price > Decimal("0"):
                current_sl_on_record = current_open_trade.stop_loss_price # From our tracker
                should_update_sl = False

                if current_open_trade.side == "long":
                    if new_tsl_price > entry_price: # Ensure TSL is profitable or at entry
                        if current_sl_on_record is None or new_tsl_price > current_sl_on_record:
                ...
                if current_sl_on_record is not None and min_tick_size > Decimal("0"):
                    significant_change = abs(new_tsl_price - current_sl_on_record) >= (min_tick_size / Decimal("2"))

                if should_update_sl and significant_change:
                    self.lg.info(f"{NG}Updating TSL for {self.s} from {current_sl_on_record} to {new_tsl_price}. Peak: {current_open_trade.highest_price_reached if current_open_trade.side == 'long' else current_open_trade.lowest_price_reached}{RST}")
                    update_success = stsl(
                        exchange=self.ex, symbol=self.s, market_info=self.mi,
                        position_info=pos_info, config=self.cfg, logger=self.lg,
                        fixed_stop_loss_price=new_tsl_price,
                        take_profit_price_target=Decimal("0"), # TSL implies TP is managed by TSL exit or manual close
                        attempt_tsl=False 
                    )
                    if update_success:
                        current_open_trade.stop_loss_price = new_tsl_price
                        current_open_trade.take_profit_price = None # TSL overrides fixed TP
                        trade_modified_in_tracker = True
                        self.lg.info(f"{NG}TSL for {self.s} successfully updated to {new_tsl_price} on exchange.{RST}")
                    else:
                        self.lg.warning(f"{NY}Failed to update TSL for {self.s} to {new_tsl_price} on exchange.{RST}")
                elif new_tsl_price is not None:
                    self.lg.debug(f"TSL for {self.s} ({current_open_trade.side}) calculated to {new_tsl_price}. Current SL: {current_sl_on_record}. No update needed based on conditions (profitable: {new_tsl_price > entry_price if current_open_trade.side == 'long' else new_tsl_price < entry_price}, ratchet: {current_sl_on_record is None or (new_tsl_price > current_sl_on_record if current_open_trade.side == 'long' else new_tsl_price < current_sl_on_record)}, significant: {significant_change}).")
            elif new_tsl_price is not None and new_tsl_price <= Decimal("0"):
                 self.lg.warning(f"{NY}_mp: Calculated TSL price for {self.s} is zero or negative ({new_tsl_price}). Skipping TSL update.{RST}")


        # --- 5. Advanced Exit Strategies (Profit-Taking Biased) ---
        exit_strategies_cfg = self.cfg.get("exit_strategies", {})
        min_profit_req_pct = exit_strategies_cfg.get("min_profit_percentage_for_advanced_exits", Decimal("0.25"))
        pnl_percentage = current_open_trade.pnl_percentage if current_open_trade.pnl_percentage is not None else Decimal("-100") # Treat None PnL as not profitable

        is_profitable_enough_for_adv_exit = pnl_percentage >= min_profit_req_pct

        if is_profitable_enough_for_adv_exit:
            self.lg.debug(f"_mp: Trade {self.s} is profitable enough ({pnl_percentage:.2f}% >= {min_profit_req_pct:.2f}%) for advanced exits.")
            if exit_strategies_cfg.get("opposing_signal_enabled", False):
                self.lg.debug(f"_mp: Checking opposing signal exit for {self.s}.")
                orderbook_data = None
                if self.cfg.get("indicators", {}).get("orderbook", False) and \
                   self.cfg.get("weight_sets", {}).get(self.ta_analyzer.aw_name, {}).get("orderbook", Decimal(0)) > 0:
                    try:
                        orderbook_data = fobc(self.ex, self.s, self.cfg.get("orderbook_limit", 25), self.lg)
                    except Exception as e_ob:
                        self.lg.warning(f"{NY}_mp: Could not fetch orderbook for {self.s} for opposing signal check: {e_ob}{RST}")
                
                _ = self.ta_analyzer.gts(current_price, orderbook_data) 
                opposing_signal_score = self.ta_analyzer.final_signal_score 
                opp_threshold = exit_strategies_cfg.get("opposing_signal_threshold", Decimal("0.5"))

                if current_open_trade.side == "long" and opposing_signal_score <= -opp_threshold:
                    self.lg.info(f"{NY}_mp: Opposing signal (SELL score {opposing_signal_score:.2f} <= -{opp_threshold}) detected for LONG {self.s}. Triggering exit.{RST}")
                    if self._cet(f"Opposing Signal (Score: {opposing_signal_score:.2f})", current_price):
                        if trade_modified_in_tracker: tt._save_trades()
                        return True 
                elif current_open_trade.side == "short" and opposing_signal_score >= opp_threshold:
                    self.lg.info(f"{NY}_mp: Opposing signal (BUY score {opposing_signal_score:.2f} >= {opp_threshold}) detected for SHORT {self.s}. Triggering exit.{RST}")
                    if self._cet(f"Opposing Signal (Score: {opposing_signal_score:.2f})", current_price):
                        if trade_modified_in_tracker: tt._save_trades()
                        return True
            ...
            if exit_strategies_cfg.get("extreme_oscillator_enabled", False):
                ...
                rsi_val, mfi_val, stoch_k_val = iv.get("RSI"), iv.get("MFI"), iv.get("Adaptive_STOCHk")

                if current_open_trade.side == "long":
                    if rsi_val is not None and not rsi_val.is_nan() and rsi_val >= Decimal(str(thresholds.get("rsi_extreme_sell_threshold", "80.0"))): extreme_count += 1
                    if mfi_val is not None and not mfi_val.is_nan() and mfi_val >= Decimal(str(thresholds.get("mfi_extreme_sell_threshold", "90.0"))): extreme_count += 1
                    if stoch_k_val is not None and not stoch_k_val.is_nan() and stoch_k_val >= Decimal(str(thresholds.get("adaptive_stoch_extreme_sell_threshold", "90.0"))): extreme_count += 1
                elif current_open_trade.side == "short":
                    if rsi_val is not None and not rsi_val.is_nan() and rsi_val <= Decimal(str(thresholds.get("rsi_extreme_buy_threshold", "20.0"))): extreme_count += 1
                    if mfi_val is not None and not mfi_val.is_nan() and mfi_val <= Decimal(str(thresholds.get("mfi_extreme_buy_threshold", "10.0"))): extreme_count += 1
                    if stoch_k_val is not None and not stoch_k_val.is_nan() and stoch_k_val <= Decimal(str(thresholds.get("adaptive_stoch_extreme_buy_threshold", "10.0"))): extreme_count += 1
                
                if extreme_count >= 2:
                    self.lg.info(f"{NY}_mp: Extreme oscillator condition (count: {extreme_count}) met for {self.s}. Triggering exit.{RST}")
                    if self._cet(f"Extreme Oscillator (Count: {extreme_count})", current_price):
                        if trade_modified_in_tracker: tt._save_trades()
                        return True
        else:
            self.lg.debug(f"_mp: Trade {self.s} not profitable enough ({pnl_percentage:.2f}% < {min_profit_req_pct:.2f}%) for advanced exits.")

        # --- Standard Fixed Protections (if TSL not active) & Break-Even ---
        if not current_open_trade.tsl_activated:
            self.lg.debug(f"_mp: TSL not active for {self.s}. Managing fixed SL/TP and Break-Even.")
            _, tp_target_from_cets, sl_target_from_cets = self.ta_analyzer.cets(entry_price, current_open_trade.side.upper())
            
            live_tp_raw = pos_info.get('takeProfitPrice') or pos_info.get('takeProfit')
            live_tp_on_exchange = Decimal(str(live_tp_raw)) if live_tp_raw and str(live_tp_raw) != "0" else None

            # Manage Fixed Take Profit (only if TSL is not active)
            if tp_target_from_cets and (live_tp_on_exchange is None or abs(live_tp_on_exchange - tp_target_from_cets) > TA.gmts(self.mi, self.lg)):
                self.lg.info(f"{NB}_mp: Updating fixed Take Profit for {self.s} to {tp_target_from_cets}. Current on exch: {live_tp_on_exchange}{RST}")
                sl_to_use_with_tp = current_open_trade.stop_loss_price if current_open_trade.stop_loss_price else sl_target_from_cets
                stsl_tp_success = stsl(
                    exchange=self.ex, symbol=self.s, market_info=self.mi, position_info=pos_info,
                    config=self.cfg, logger=self.lg,
        ...
        if self._tbe(pos_info): # _tbe calls _cet if triggered
            if trade_modified_in_tracker: tt._save_trades() # Save if any prior modifications
            return True 

        # --- 7. Save Trade Record if modified ---
        if trade_modified_in_tracker:
            tt._save_trades()
            self.lg.debug(f"_mp: Trade record for {self.s} saved due to modifications in TSL, peak prices or protections.{RST}")

        return True # Position exists and was managed

    def _check_and_set_fixed_protections(self, position_info: Dict, target_sl_price: Optional[Decimal], target_tp_price: Optional[Decimal], current_trade_record: Tr, min_tick_size: Decimal):
        """Helper function to check and set fixed SL and TP.
        DEPRECATED: Logic moved to stsl and _mp main flow."""
        pass  # This function is no longer needed as its logic is integrated into _mp calling stsl.

    def _ot(self, signal: str, current_price: Decimal, orderbook_data: Optional[Dict]) -> bool:
        """
        ...
        entry_price_estimate, take_profit_price, stop_loss_price = self.ta_analyzer.cets(
        ...
        if order_type == "limit":
            offset_multiplier = self.cfg.get(
                "limit_order_offset_buy" if signal == "BUY" else "limit_order_offset_sell", Decimal("0.0005"))
            if signal == "BUY":
                limit_price = current_price * \
                    (Decimal("1.0") - offset_multiplier)
            else:
                limit_price = current_price * \
                    (Decimal("1.0") + offset_multiplier)
            ...
            min_tick_size = TA.gmts(self.mi, self.lg)
            if min_tick_size > 0:
                limit_price = (limit_price / min_tick_size).quantize(Decimal('1'),
                                                                     rounding=ROUND_HALF_EVEN) * min_tick_size
            else:
                limit_price = limit_price.quantize(
                    Decimal('1e-' + str(price_precision)), rounding=ROUND_HALF_EVEN)

            self.lg.info(f"{NB}Calculated limit price for {self.s} {signal}: {limit_price:.{price_precision}f} (Current: {current_price:.{price_precision}f}, Offset: {offset_multiplier:.4f}). Setting a precise entry point.{RST}")
        ...
        if order_response:
            ...
            tt.aot(Tr(
                ...
                entry_adx = self._get_indicator_value("ATR") # Using ATR as per instruction
            ...
            if self.mi.get('is_contract', False):
                ...
                if trade_to_protect:  # Ensure trade record exists
                    ...
                    protection_info_dict['entryPriceDecimal'] = trade_to_protect.entry_price
                    ...
                    time.sleep(self.cfg.get(
                        "position_confirm_delay_seconds", PCDS))
                    confirmed_position_info = gop(
                        self.ex, self.s, self.mi, self.lg)

                    if confirmed_position_info and confirmed_position_info.get('contractsDecimal', Decimal(0)) > 0:
                        # Determine if TSL should be attempted immediately
                        attempt_immediate_tsl = False
    ...
    def run_symbol_logic(self):
        ...
        try:
            kline_data = fkc(self.ex, self.s, CIM.get(self.cfg["interval"]), limit=self.cfg.get(
                "fibonacci_window", DIP["fib_window"]) + 50, logger=self.lg)
        except Exception as e:
            self.lg.error(
        ...
        if self.cfg.get("indicators", {}).get("orderbook", False):
            try:
                orderbook_data = fobc(self.ex, self.s, self.cfg.get(
                    "orderbook_limit", 25), self.lg)
            except Exception as e:
                self.lg.warning(
...
class MainBot:
    ...
    def initialize(self):
        ...
        public_ws_base_url = "wss://stream.bybit.com/v5/public/linear"  # Default to linear
        ...
        for symbol_str in self.cfg.get("symbols_to_trade", []):
            try:
                per_symbol_bot = PB(symbol_str, self.ex, self.cfg, slg(
                    symbol_str.replace('/', '_').replace(':', '_')), self.ws_client)
                ...
                self.lg.error(
                    f"{NR}Unexpected error initializing bot for symbol {symbol_str}: {e}. Skipping this symbol. A cosmic interference!{RST}", exc_info=True)
        ...
        if self.cfg.get("enable_sms_alerts", False):
            ...
            if recipient_number:
                ...
                if send_termux_sms(recipient_number, startup_message, self.lg):
                    self.lg.info(f"{NB}Bot startup SMS alert sent to {recipient_number}.{RST}")
                else:
                    self.lg.warning(f"{NY}Failed to send bot startup SMS alert to {recipient_number}.{RST}")
            else:
                self.lg.info(f"{NB}SMS alerts enabled, but no recipient number configured. Skipping startup SMS.{RST}")
        else:
        ...
        while self.is_running:
            try:
                ...
                for symbol, bot in self.per_symbol_bots.items():
                    ...
                    if symbol not in last_run_time or (current_time - last_run_time[symbol]) >= interval_seconds:
                ...
                enable_sms_alerts = self.cfg.get("enable_sms_alerts", False)
                sms_recipient_number = self.cfg.get("sms_recipient_number", "")
                enable_periodic_pnl_sms = self.cfg.get(
                    "enable_periodic_pnl_sms", False)
                periodic_pnl_sms_interval_minutes = self.cfg.get(
                    "periodic_pnl_sms_interval_minutes", 60)

                if enable_sms_alerts and sms_recipient_number and enable_periodic_pnl_sms:
                    ...
                    if elapsed_time_seconds >= (periodic_pnl_sms_interval_minutes * 60):
                        ...
                        if send_termux_sms(sms_recipient_number, pnl_summary_message, self.lg):
                            self.lg.info(
                ...
                self.lg.error(
                    f"{NR}An unexpected error occurred in the main loop: {e}. A cosmic interference in the bot's operation!{RST}", exc_info=True)
                time.sleep(self.cfg.get("retry_delay", RDS)
    ...
    def stop(self):
        ...
        self.lg.info(
            f"{NG}Bot stopped. The trading session has concluded!{RST}")
        ...
        if self.cfg.get("enable_sms_alerts", False):
            ...
            if recipient_number:
                ...
                if send_termux_sms(recipient_number, shutdown_message, self.lg):
                    self.lg.info(f"{NB}Bot shutdown SMS alert sent to {recipient_number}.{RST}")
                else:
                    self.lg.warning(f"{NY}Failed to send bot shutdown SMS alert to {recipient_number}.{RST}")
            else:
                self.lg.info(f"{NB}SMS alerts enabled, but no recipient number configured. Skipping shutdown SMS.{RST}")
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        bot.lg.critical(
            f"{NR}Bot encountered a critical error during initialization or main loop execution: {final_e}. Aborting!{RST}", exc_info=True)
    finally:
