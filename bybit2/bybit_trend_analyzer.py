...
class BybitTrendAnalyzer:
    ...
    def calculate_indicators(
        ...
        ob_percentage_threshold: float = 0.01,
    ) -> pd.DataFrame:
    ...
    def analyze_trend(
        ...
        max_lookback = max(
            ...
            ichimoku_senkou,
            ehlers_fisher_period,
            ehlers_ssf_period,
        )  # ichimoku_senkou is 52 by default
        ...
        if pd.notna(adx_val) and pd.notna(adx_pos) and pd.notna(adx_neg):
            ...
            if adx_val > 25:
                ...
                analysis["details"]["adx_strength"] = (
                    f"Developing Trend ({adx_val:.2f})"
                )
            else:
            ...
            # Trend Direction
            if adx_pos > adx_neg:
                analysis["details"]["adx_direction"] = (
                    f"Positive (DI+ {adx_pos:.2f} > DI- {adx_neg:.2f})"
                )
                ...
                analysis["details"]["adx_direction"] = (
                    f"Negative (DI- {adx_neg:.2f} > DI+ {adx_pos:.2f})"
                )
                # Refine overall trend based on ADX
        ...
        if pd.notna(bb_upper) and pd.notna(bb_lower) and pd.notna(bb_middle):
            if close_price > bb_upper:
                analysis["details"]["bollinger_bands_status"] = (
                    "Price above Upper Band (Overbought/Strong Uptrend)"
                )
            elif close_price < bb_lower:
                analysis["details"]["bollinger_bands_status"] = (
                    "Price below Lower Band (Oversold/Strong Downtrend)"
                )
            elif close_price > bb_middle:
                analysis["details"]["bollinger_bands_status"] = (
                    "Price above Middle Band (Bullish Bias)"
                )
            elif close_price < bb_middle:
                analysis["details"]["bollinger_bands_status"] = (
        ...
        if pd.notna(macd_val) and pd.notna(macd_signal) and pd.notna(macd_diff):
            if macd_val > macd_signal and macd_diff > 0:
                analysis["details"]["macd_crossover"] = (
                    "Bullish Crossover (MACD above Signal, Histogram positive)"
                )
            elif macd_val < macd_signal and macd_diff < 0:
                analysis["details"]["macd_crossover"] = (
                    "Bearish Crossover (MACD below Signal, Histogram negative)"
                )
            else:
        ...
        if all(
            ...
            if conversion_line > base_line:
                ...
                tenkan_kijun_crossover = (
                    "Conversion Line and Base Line are flat (Sideways)"
                )
        ...
        if pd.notna(ehlers_fisher_val):
            if ehlers_fisher_val > 0.5:  # Thresholds can be adjusted
                analysis["details"]["ehlers_fisher_status"] = (
                    f"Bullish ({ehlers_fisher_val:.2f})"
                )
            elif ehlers_fisher_val < -0.5:
                analysis["details"]["ehlers_fisher_status"] = (
                    f"Bearish ({ehlers_fisher_val:.2f})"
                )
            else:
                analysis["details"]["ehlers_fisher_status"] = (
                    f"Neutral ({ehlers_fisher_val:.2f})"
                )
        else:
        ...
        if pd.notna(ehlers_ssf_val):
            if close_price > ehlers_ssf_val:
                analysis["details"]["ehlers_ssf_status"] = (
                    f"Price above SSF (Bullish) ({ehlers_ssf_val:.2f})"
                )
            elif close_price < ehlers_ssf_val:
                analysis["details"]["ehlers_ssf_status"] = (
                    f"Price below SSF (Bearish) ({ehlers_ssf_val:.2f})"
                )
            else:
                analysis["details"]["ehlers_ssf_status"] = (
                    f"Price at SSF (Neutral) ({ehlers_ssf_val:.2f})"
                )
        else:
            logging.warning(
        ...
        if not bullish_obs.empty:
            ...
            analysis["details"]["order_block_status"] = (
                f"Recent Bullish OB detected (High: {ob_high:.2f}, Low: {ob_low:.2f}). "
            )
            ...
            analysis["overall_trend"] = (
                "Potential Reversal Up (Bullish OB)"  # Tentative
            )
        ...
        if not bearish_obs.empty:
            ...
            analysis["details"]["order_block_status"] = (
                f"Recent Bearish OB detected (High: {ob_high:.2f}, Low: {ob_low:.2f}). "
            )
            if latest_data["close"] < ob_low:
                ...
                analysis["details"]["order_block_status"] += "Price is within OB."
            analysis["overall_trend"] = (
                "Potential Reversal Down (Bearish OB)"  # Tentative
            )
        ...
        if (
            "Strong Uptrend" in analysis["overall_trend"]
            or "Emerging Uptrend" in analysis["overall_trend"]
        ):
            bullish_signals += 1
        elif (
            "Strong Downtrend" in analysis["overall_trend"]
            or "Emerging Downtrend" in analysis["overall_trend"]
        ):
            bearish_signals += 1

        if (
            "Price above Upper Band" in analysis["details"]["bollinger_bands_status"]
            or "Price above Middle Band"
            in analysis["details"]["bollinger_bands_status"]
        ):
            bullish_signals += 1
        elif (
            "Price below Lower Band" in analysis["details"]["bollinger_bands_status"]
            or "Price below Middle Band"
            in analysis["details"]["bollinger_bands_status"]
        ):
            bearish_signals += 1
        ...
        if isinstance(analysis["details"]["ichimoku_status"], dict):
            if (
                "Price above Cloud"
                in analysis["details"]["ichimoku_status"]["cloud_status"]
                and "Conversion Line above Base Line"
                in analysis["details"]["ichimoku_status"]["tenkan_kijun_crossover"]
                and "Lagging Span above Price"
                in analysis["details"]["ichimoku_status"]["chikou_span_status"]
            ):
                bullish_signals += 1.5  # Strong Ichimoku bullish signal
            elif (
                "Price below Cloud"
                in analysis["details"]["ichimoku_status"]["cloud_status"]
                and "Conversion Line below Base Line"
                in analysis["details"]["ichimoku_status"]["tenkan_kijun_crossover"]
                and "Lagging Span below Price"
                in analysis["details"]["ichimoku_status"]["chikou_span_status"]
            ):
    ...
    def get_trend_summary(
        ...
        if df_cleaned.empty or len(df_cleaned) < max_lookback:
            ...
            return {
                "status": "error",
                "message": "Not enough valid data after indicator calculation.",
            }
