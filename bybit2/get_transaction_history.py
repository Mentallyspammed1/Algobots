...
def get_transaction_history(limit: int = 10):
    ...
    try:
        ...
        print(Fore.CYAN + f"  # Requesting last {limit} transaction logs for Unified Account..." + Style.RESET_ALL)
        ...
        if response and response['retCode'] == 0:
            ...
            if transactions:
                print(Fore.GREEN + f"  # Last {len(transactions)} Transaction History Records:" + Style.RESET_ALL)
                for tx in transactions:
                    print(Fore.WHITE + f"    Time: {tx.get('createdTime')}, Type: {tx.get('type')}, "
                                  f"Coin: {tx.get('coin')}, Amount: {tx.get('amount')}, "
                                  f"Tx ID: {tx.get('txID')}" + Style.RESET_ALL)
            else:
                print(Fore.YELLOW + "  # No transaction history records found for your Unified Account." + Style.RESET_ALL)
        else:
            print(Fore.RED + "  # Failed to retrieve transaction history." + Style.RESET_ALL)
