...
def get_specific_symbol_open_orders(symbol: str = 'BTCUSDT', category: str = 'linear'):
    ...
    try:
        ...
        print(Fore.CYAN + f"  # Requesting open orders for {symbol} in category {category}..." + Style.RESET_ALL)
        ...
        if response and response['retCode'] == 0:
            ...
            if open_orders:
                print(Fore.GREEN + f"  # Active Open Orders for {symbol}:" + Style.RESET_ALL)
                for order in open_orders:
                    print(Fore.WHITE + f"    Order ID: {order.get('orderId')}, Side: {order.get('side')}, "
                                  f"Order Type: {order.get('orderType')}, Price: {order.get('price')}, "
                                  f"Qty: {order.get('qty')}, Status: {order.get('orderStatus')}" + Style.RESET_ALL)
            else:
                print(Fore.YELLOW + f"  # No active open orders found for {symbol} in category {category}." + Style.RESET_ALL)
        else:
