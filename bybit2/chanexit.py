...
DEFAULT_SLEEP_DURATION = (
    60  # seconds Default wait time between cycles if calculation fails
)
...
def _load_config():
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            config["max_position_size"] = to_decimal(
                config.get("max_position_size", defaults["max_position_size"]),
                rounding=ROUND_HALF_UP,
            )
...
def _fetch_kline_data(
    ...
    fetch_end_time = (
        end_time if end_time else int(time.time() * 1000)
    )  # End time for backtesting or approximate current time
    ...
    while True:
        ...
        for attempt in range(MAX_API_RETRIES):
            try:
                ...
                if (
                    not data
                ):  # Handle empty list explicitly (received valid response but no rows)
                    print(
                        NEON_INFO
                ...
                current_fetch_start = int(last_timestamp_ms) + (
        ...
        if (
            not data
            or len(data) < limit
            or (start_time and end_time and current_fetch_start > end_time)
        ):
            break
...
def _calculate_indicators(df, config):
    ...
    st_multiplier = config[
        "super_trend_multiplier"
    ]  # This is a Decimal value from config.py
    ...
    if use_ce:
        ...
        try:
            ...
            if ce_long_col not in df.columns:
                df[ce_long_col] = pd.NA
            if ce_short_col not in df.columns:
                df[ce_short_col] = pd.NA
            ...
            config["use_chandelier_exit"] = (
                False  # Disable the feature if calculation fails
            )
    ...
    if config.get("use_stochrsi_crossover", False):
        ...
        try:
            ...
            config["use_stochrsi_crossover"] = (
                False  # Disable the filter if calculation fails
            )
    ...
    if rsi_col in df.columns:
        return_cols.append(rsi_col)
    if ce_long_col and ce_short_col in df.columns:
        return_cols.extend([ce_long_col, ce_short_col])
    if stoch_rsi_k_name and stoch_rsi_d_name in df.columns:
        return_cols.extend([stoch_rsi_k_name, stoch_rsi_d_name])
    # Include ATR column if calculated, useful for debugging/plotting
    if atr_col in df.columns:
        return_cols.append(atr_col)
...
def _plot_supertrend(df, config):
    ...
    try:
        ...
        ax1.set_title(
            f"Ascended Neon Bot - {config['symbol']} ({config['interval']}m Interval)",
            color="cyan",
        )
...
def _check_rsi_filter(rsi_val, config, side):
    ...
    if pd.isna(rsi_val):
        return False  # Cannot check if RSI is NaN
...
def _execute_trade(
    ...
    try:
        ...
        if "Insufficient margin" in error_msg:
            ...
            or "step incorrect" in error_msg
            or "min_qty" in error_msg
        ):
            print(
                NEON_ERROR
...
def _send_email(config, subject, body):
    ...
    try:
        ...
        smtp_server = (
            "smtp.gmail.com"  # Assuming Gmail; adjust if using another provider
        )
        smtp_port = 465
...
def _run_backtest(df_ind, config):
    ...
    for i in range(len(df_ind)):
        ...
        ce_long = (
            to_decimal(current.get(ce_long_col))
            if use_ce and ce_long_col in current
            else None
        )
        ce_short = (
            to_decimal(current.get(ce_short_col))
            if use_ce and ce_short_col in current
            else None
        )
        stoch_k_curr = (
            to_decimal(current.get(stoch_rsi_k_name))
            if stoch_rsi_k_name and stoch_rsi_k_name in current
            else None
        )
        stoch_d_curr = (
            to_decimal(current.get(stoch_rsi_d_name))
            if stoch_rsi_d_name and stoch_rsi_d_name in current
            else None
        )
        ...
        stoch_rsi_cross_up = (
            ...
            and stoch_k_curr is not None
            and stoch_d_curr is not None
            and stoch_k_prev < stoch_d_prev
            and stoch_k_curr > stoch_d_curr
        )
        ...
        if positions:
            ...
            if (
                pos["side"] == "Buy"
                and use_ce
                and ce_long is not None
                and close < ce_long
            ) or (
                pos["side"] == "Sell"
                and use_ce
                and ce_short is not None
                and close > ce_short
            ):
            ...
            if (
                pos["side"] == "Buy" and st_flip_down
            ):  # Potential exit for Long if ST flips down
                signal_ok = False
                if config.get("use_stochrsi_crossover", False):
                    signal_ok = stoch_rsi_cross_down  # Use Stoch RSI if enabled
                elif rsi is not None:
                    signal_ok = rsi_extreme_down  # Fallback to RSI extremes
                if signal_ok:
                    close_reversal = True
                    exit_reason = "Reversal Signal"

            elif (
                pos["side"] == "Sell" and st_flip_up
            ):  # Potential exit for Short if ST flips up
                signal_ok = False
                if config.get("use_stochrsi_crossover", False):
                    signal_ok = stoch_rsi_cross_up
                elif rsi is not None:
                    signal_ok = rsi_extreme_up
                if signal_ok:
                    close_reversal = True
                    exit_reason = "Reversal Signal"
            ...
            if current_stop_level is not None:
                if (pos["side"] == "Buy" and close < current_stop_level) or (
                    pos["side"] == "Sell" and close > current_stop_level
                ):
                    violate_stop = True
                if violate_stop:
                    exit_reason = (
                        "Stop Loss Hit"  # Override reason if stop level is breached
                    )
            ...
            if should_close_position:
                ...
                if pos["side"] == "Buy":
                    ...
                    pnl = (entry_price - close) * qty  # Sell side
            ...
            if stop_distance_usdt <= Decimal("0"):
                ...
                if stop_dist_pct > Decimal("0"):
                    if st_flip_up:  # Potential Long Entry
                        stop_price = close * (
                            Decimal("1") - stop_dist_pct
                        )  # Calculate approximate stop price
                ...
                if qty > Decimal("0"):  # Proceed only if calculated quantity is valid
                    ...
                    if config.get("use_stochrsi_crossover", False):
                        ...
                        sell_entry_condition = (
                            sell_entry_condition and stoch_rsi_cross_down
                        )

                    # Add Chandelier confirmation for entries if enabled (price must be 'safe' relative to CE)
                    if use_ce:
                        buy_entry_condition = buy_entry_condition and (
                            ce_short is not None and close > ce_short
                        )
                        sell_entry_condition = sell_entry_condition and (
                            ce_long is not None and close < ce_long
                        )
                    ...
                    if buy_entry_condition:
                        ...
                        stop_level = max(
                            to_decimal(initial_stop_price, rounding=ROUND_DOWN),
                            to_decimal("0.00000001"),
                        )  # Prevent zero or negative stop
                        ...
                        initial_stop_price = close + stop_distance_usdt
                        stop_level = max(
                            to_decimal(initial_stop_price, rounding=ROUND_UP),
                            to_decimal("0.00000001"),
                        )
    ...
    win_rate = (
        (sum(1 for t in trades if t["pnl"] > 0) / len(trades) * 100) if trades else 0
    )
    ...
    if config["plot_enabled"] and trades:
        try:
            ...
            plot_equities = [
                float(equity_val)
                for tpl in equity_curve_points
                for equity_val in [tpl[1]]
            ]  # Correct extraction
...
def main():
    ...
    last_loop_check_time = (
        time.time()
    )  # Track time for potential cycle duration monitoring
    ...
    try:
        while True:  # Infinite loop for continuous operation
            ...
            indicator_lookback = max(
                ...
                config["rsi_length"],
                config["chandelier_atr_length"]
                if config.get("use_chandelier_exit", False)
                else 0,
                config.get("stochrsi_k_period", 14)
                if config.get("use_stochrsi_crossover", False)
                else 0,
            )
            ...
            ce_long_current = (
                to_decimal(current_data.get("chandelier_exit_long"))
                if config.get("use_chandelier_exit", False)
                else None
            )
            ce_short_current = (
                to_decimal(current_data.get("chandelier_exit_short"))
                if config.get("use_chandelier_exit", False)
                else None
            )

            # Safely extract Stoch RSI values (current and previous candle) if filter is enabled
            stoch_k_curr, stoch_d_curr, stoch_k_prev, stoch_d_prev = (
                None,
                None,
                None,
                None,
            ...
            st_flip_down = (
                state.get("last_supertrend_dir") == 1 and st_dir_current == -1
            )
            ...
            if balance > Decimal("0"):  # Only check if balance is positive
                ...
                required_margin_estimate = (
                    close_current / to_decimal(config["leverage"])
            ...
            executed_exit = (
                False  # Flag to track if an exit trade was executed in this cycle
            )
            ...
            if long_pos_active:
                ...
                close_long_chandelier = (
                    config.get("use_chandelier_exit", False)
                    and ce_long_current is not None
                    and close_current < ce_long_current
                )
            ...
            # Logic for closing SHORT position
                ...
                close_short_chandelier = (
                    config.get("use_chandelier_exit", False)
                    and ce_short_current is not None
                    and close_current > ce_short_current
                )
            ...
            if executed_exit:
                ...
                if (
                    not long_pos_active
                    and not short_pos_active
                    and config["max_open_trades"] > 0
                ):
                    ...
                    if stop_distance_usdt <= Decimal("0"):
                        ...
                        if stop_dist_pct > Decimal("0"):
                            if st_flip_up:  # Potential Long Entry
                                stop_price = close_current * (
                                    Decimal("1") - stop_dist_pct
                                )  # Calculate approximate stop price
                                stop_distance_usdt = (
                                    close_current - stop_price
                                )  # Distance in USDT
                                entry_signal_type = "Entry_Long_Pct"
                            elif st_flip_down:  # Potential Short Entry
                                stop_price = close_current * (
                    ...
                    if stop_distance_usdt > Decimal("0"):
                        ...
                        if qty > Decimal(
                            ...
                            if config.get("use_stochrsi_crossover", False):
                                ...
                                buy_entry_condition = (
                                    buy_entry_condition and stoch_rsi_cross_up
                                )
                                sell_entry_condition = (
                                    sell_entry_condition and stoch_rsi_cross_down
                                )
                            ...
                            if config.get("use_chandelier_exit", False):
                                buy_entry_condition = buy_entry_condition and (
                                    ce_short_current is not None
                                    and close_current > ce_short_current
                                )
                                sell_entry_condition = sell_entry_condition and (
                                    ce_long_current is not None
                                    and close_current < ce_long_current
                                )
            ...
            try:
                ...
                if next_candle_expected_start > now_utc + timedelta(
                    ...
                    ).total_seconds() + 15  # Add buffer (e.g., 15 seconds)
