...
def get_order_book_depth(symbol: str = 'XRPUSDT', limit: int = 5):
    ...
    try:
        ...
        print(Fore.CYAN + f"  # Fetching order book for {symbol} with limit {limit}..." + Style.RESET_ALL)
        ...
        if response and response['retCode'] == 0:
            ...
            if orderbook:
                ...
                for bid in orderbook['bids']:
                    print(Fore.GREEN + f"    Price: {bid[0]}, Quantity: {bid[1]}" + Style.RESET_ALL)

                print(Fore.WHITE + "  Asks (Sell Orders):" + Style.RESET_ALL)
                for ask in orderbook['asks']:
                    print(Fore.RED + f"    Price: {ask[0]}, Quantity: {ask[1]}" + Style.RESET_ALL)
            else:
                print(Fore.YELLOW + f"  # No order book data found for {symbol}." + Style.RESET_ALL)
        else:
            print(Fore.RED + "  # Failed to retrieve order book data." + Style.RESET_ALL)
