...
def get_klines_data(symbol: str = 'SOLUSDT', interval: str = '60', limit: int = 100):
    ...
    try:
        ...
        print(Fore.CYAN + f"  # Fetching Klines for {symbol} with interval {interval} and limit {limit}..." + Style.RESET_ALL)
        ...
        if response and response['retCode'] == 0:
            ...
            if klines:
                print(Fore.GREEN + f"  # Successfully retrieved {len(klines)} Klines for {symbol}:" + Style.RESET_ALL)
                ...
                for kline in reversed(klines): # Print in chronological order
                    print(Fore.WHITE + f"    Timestamp: {kline[0]}, Open: {kline[1]}, High: {kline[2]}, Low: {kline[3]}, Close: {kline[4]}, Volume: {kline[5]}" + Style.RESET_ALL)
            else:
                print(Fore.YELLOW + f"  # No Klines data found for {symbol} with interval {interval}." + Style.RESET_ALL)
        else:
