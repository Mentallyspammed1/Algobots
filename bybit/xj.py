...
class RateLimiter:
    ...
    def can_call(self, endpoint: str) -> bool:
        ...
        with self._lock:
            ...
            while (
                self.calls[endpoint] and self.calls[endpoint][0] < now - limit["period"]
            ):
...
class MarketMicrostructureAnalyzer:
    ...
    def get_order_flow_signal(self, symbol: str) -> float:
        ...
        with self._lock:
            if (
                symbol not in self.order_flow_imbalance
                or len(self.order_flow_imbalance[symbol]) < 10
            ):
                return 0.0

            recent_imbalances = [
                imb for _, imb in list(self.order_flow_imbalance[symbol])[-20:]
            ]
    ...
    def detect_market_regime(
        ...
        if abs(trend) > volatility * 2:
            regime = (
                MarketRegime.TRENDING_UP if trend > 0 else MarketRegime.TRENDING_DOWN
            )
...
class EnhancedBybitWebSocketManager:
    ...
    def _init_ws(self, private: bool):
        ...
        try:
            ...
            logger.critical(
                Fore.RED
                + f"  # Catastrophic failure initializing {ws_type} WebSocket: {e}"
                + Style.RESET_ALL,
                exc_info=True,
            )
            self._handle_reconnect(private)
    ...
    def handle_orderbook(self, msg: dict):
        ...
        try:
            ...
            logger.error(
                Fore.RED + f"  # Error in orderbook handling: {e}" + Style.RESET_ALL,
                exc_info=True,
            )
    ...
    def handle_execution(self, msg: dict):
        ...
        try:
            ...
            logger.error(
                Fore.RED + f"  # Error handling execution data: {e}" + Style.RESET_ALL,
                exc_info=True,
            )
    ...
    def handle_position(self, msg: dict):
        ...
        try:
            ...
            logger.error(
                Fore.RED + f"  # Error handling position data: {e}" + Style.RESET_ALL,
                exc_info=True,
            )
    ...
    def is_public_connected(self) -> bool:
        ...
        return (
            self.ws_public is not None
            and self.ws_public.is_connected()
            and (time.time() - self._last_ws_heartbeat.get("linear", 0)) < 30
        )

    def is_private_connected(self) -> bool:
        """Check private WebSocket health."""
        return (
            self.ws_private is not None
            and self.ws_private.is_connected()
            and (time.time() - self._last_ws_heartbeat.get("private", 0)) < 30
        )
...
class UltimateBybitTradingBot:
    ...
    async def _http_call(self, method: Callable, **kwargs) -> dict | None:
        ...
        for attempt in range(3):
            try:
                ...
                logger.error(
                    Fore.RED + f"  # {endpoint} failed: {e}" + Style.RESET_ALL,
                    exc_info=True,
                )
    ...
    async def place_order_with_layers(
        self,
        symbol: str,
        side: str,
        base_price: Decimal,
        base_qty: Decimal,
        layers: int = 1,
        layer_spread: Decimal = Decimal("0.0005"),
    ) -> list[dict]:
    ...
    async def update_performance_snapshot(self, account_info: dict):
        ...
        if (
            time.time() - self._last_performance_snapshot
            < self._performance_snapshot_interval
        ):
            return
                ...
                if self.strategies and all_market_data:
                    for strategy in self.strategies:
                        try:
                            ...
                            logger.error(
                                Fore.RED + f"  # Strategy error: {e}" + Style.RESET_ALL,
                                exc_info=True,
                            )
