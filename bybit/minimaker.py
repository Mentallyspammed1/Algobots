...
# Load from environment variables using the Config class pattern
...
class BybitWebSocketManager:
    ...
    def is_public_connected(self) -> bool:
        return self.ws_public and self.ws_public.is_connected()

    def is_private_connected(self) -> bool:
        return self.ws_private and self.ws_private.is_connected()
...
class BybitTradingBot:
    ...
    async def _get_market_data(self, symbol: str) -> Optional[Dict]:
        ...
        if ws_data and (now_ms - ws_data.get("timestamp", 0)) < int(os.getenv("DATA_FRESH_MS", "5000")):
            return ws_data
        ...
        if response and response.get('retCode') == 0:
            ...
            return {"orderbook": {"b": ob['b'], "a": ob['a'], "ts": ob['ts']}}
        return None
...
async def market_making_strategy(
    ...
    for symbol in symbols:
        ...
        if not md.get("orderbook", {}).get("b"): continue
...
async def main():
    ...
    bot = BybitTradingBot(api_key=API_KEY, api_secret=API_SECRET, testnet=USE_TESTNET, dry_run=DRY_RUN)
