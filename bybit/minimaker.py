...
class BybitTradingBot:
    ...
    async def _get_market_data(self, symbol: str) -> dict | None:
        ...
        if ws_data and (now_ms - ws_data.get("timestamp", 0)) < int(
            os.getenv("DATA_FRESH_MS", "5000")
        ):
            return ws_data
...
async def main():
    ...
    bot = BybitTradingBot(
        api_key=API_KEY, api_secret=API_SECRET, testnet=USE_TESTNET, dry_run=DRY_RUN
    )
