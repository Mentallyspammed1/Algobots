...
CONFIG = {
    ...
    "ORDER_SIZE_USDT": Decimal("10.0"), # Order size in USDT (as Decimal) - this will be PER LEVEL
    ...
    "LEVEL_PRICE_OFFSET_FACTOR": Decimal("0.00005"), # Additional spread per level (e.g., 0.005% per level)
    ...
    "ORDER_AMEND_TOLERANCE_PERCENT": Decimal("0.00005"), # Price/Qty diff % to trigger amend instead of cancel/place
}
...
class MarketMaker:
    ...
    def __init__(self, config: dict):
        ...
        # Concurrency control
        self.api_lock = Lock()
        self.data_lock = Lock() # For protecting mid_price, current_inventory, active_orders
    ...
    def _connect_ws(self):
        ...
        while not self.ws_connected and self.ws_reconnect_attempts < self.config["WS_MAX_RECONNECT_ATTEMPTS"]:
            try:
                logging.info(f"Attempting WebSocket connection (Attempt {self.ws_reconnect_attempts + 1}/{self.config['WS_MAX_RECONNECT_ATTEMPTS']})...")
                self.ws = WebSocket(
                    testnet=True,
                ...
                if self.ws_reconnect_attempts < self.config["WS_MAX_RECONNECT_ATTEMPTS"]:
                    sleep_time = self.config["WS_RECONNECT_INTERVAL_SEC"] * (2 ** (self.ws_reconnect_attempts - 1)) # Exponential backoff
                    logging.warning(f"Retrying WebSocket connection in {min(sleep_time, 60)} seconds...")
                    time.sleep(min(sleep_time, 60)) # Cap sleep time to avoid excessive delays
                else:
                    logging.error("Max WebSocket reconnect attempts reached.")
                    self.activate_circuit_breaker(f"Max WebSocket reconnect attempts reached: {e}")
    ...
    def _on_order_update(self, message: dict):
        ...
        try:
            ...
            for order in data:
                ...
                order_status = order.get('orderStatus') # New, PartiallyFilled, Filled, Cancelled, Rejected, Expired
                side = order.get('side')
                ...
                with self.data_lock:
                    if order_status in ["New", "PartiallyFilled"]:
                        ...
                        level = int(order_link_id.split('_')[-1]) if order_link_id.split('_')[-1].isdigit() else -1
                        ...
                        logging.debug(f"Order updated: {order_link_id} Status: {order_status} Price: {price} Qty: {qty}")
                        ...
                        if order_link_id in self.active_orders:
                            ...
                            logging.info(f"Order removed: {order_link_id} Status: {order_status}")
                    else:
                        logging.warning(f"Unhandled order status for {order_link_id}: {order_status}")
            ...
            try:
                ...
                if open_orders and open_orders.get('list'):
                    orders_to_cancel = [
                        ...
                        if order.get('orderLinkId', '').startswith(self.config["ORDER_ID_PREFIX"])
                    ]
                    if orders_to_cancel:
                        logging.info(f"Found {len(orders_to_cancel)} bot orders to cancel.")
                        for order in orders_to_cancel:
                            ...
                            self._handle_api_response(cancel_resp, f"Cancel Order {order.get('orderId')}")
    ...
    def _calculate_desired_orders(self, mid_price: Decimal, inventory: Decimal) -> list[dict]:
        ...
        for i in range(self.order_levels):
            ...
            # Ask side
            ask_price = mid_price * (Decimal("1") + self.spread_percent / Decimal("2") + level_offset) - skew_amount
            ...
            ask_price = ask_price.quantize(self.price_tick_size, rounding=ROUND_UP) # ROUND_UP for asks
            ...
            if bid_qty < self.min_order_qty:
                logging.warning(f"Calculated bid qty {bid_qty} is below min {self.min_order_qty}. Skipping bid level {i}.")
            else:
    ...
    def manage_orders(self):
        ...
        with self.data_lock:
            ...
            current_active_orders = self.active_orders.copy() # Work on a copy to avoid lock contention during API calls

        if current_mid_price == Decimal("0.0") or self.price_tick_size == Decimal("0.0"):
        ...
        try:
            ...
            existing_bids = {order_info['level']: order_info for order_info in current_active_orders.values() if order_info['side'] == "Buy"}
            existing_asks = {order_info['level']: order_info for order_info in current_active_orders.values() if order_info['side'] == "Sell"}
            ...
            for desired_order in desired_orders:
                ...
                if existing_order:
                    ...
                    price_diff_percent = abs(existing_order['price'] - desired_price) / desired_price if desired_price else Decimal('inf')
                    qty_diff_percent = abs(existing_order['qty'] - desired_qty) / desired_qty if desired_qty else Decimal('inf')

                    if price_diff_percent > self.order_amend_tolerance or \
                       qty_diff_percent > self.order_amend_tolerance:
                        ...
                        logging.info(f"Marking {side} order {existing_order['orderLinkId']} for amendment (Level {level}). Old P/Q: {existing_order['price']}/{existing_order['qty']} New P/Q: {desired_price}/{desired_qty}")
                    else:
                        logging.debug(f"Existing {side} order {existing_order['orderLinkId']} (Level {level}) is still optimal.")
                else:
                    # No existing order for this side/level, place a new one
                    orders_to_place.append(desired_order)
                    logging.info(f"Marking new {side} order for placement (Level {level}). Price: {desired_price} Qty: {desired_qty}")
            ...
            for olink, order_info in current_active_orders.items():
                if (order_info['side'], order_info['level']) not in desired_order_keys:
                    ...
                    logging.info(f"Marking stale order {olink} for cancellation (not in desired set).")
            ...
            for amend_data in orders_to_amend:
                with self.api_lock:
                    response = self.session.amend_order(
                        ...
                        orderLinkId=amend_data['orderLinkId'] # Pass orderLinkId for reference
                    )
                    result = self._handle_api_response(response, f"Amend {amend_data['side']} Order {amend_data['orderId']}")
                    if result:
                        logging.info(f"Amended {amend_data['side']} order {amend_data['orderId']}: Qty={amend_data['qty']}, Price={amend_data['price']}")
                time.sleep(0.05) # Small delay between API calls
            ...
            for order_data in orders_to_place:
                ...
                with self.api_lock:
                    ...
                    result = self._handle_api_response(response, f"Place {order_data['side']} Order (Level {order_data['level']})")
                    if result:
                        logging.info(f"Placed {order_data['side']} order (Level {order_data['level']}): Qty={order_data['qty']}, Price={order_data['price']}")
    ...
    def run(self):
        ...
        while True:
            ...
            if not self.ws_connected:
                ...
                self._connect_ws()
                if not self.ws_connected:
                    logging.error("Failed to reconnect WebSocket. Circuit breaker might be activated.")
                    time.sleep(self.config["HEARTBEAT_INTERVAL_SEC"]) # Wait before next check
                    continue
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.critical(f"An unhandled exception occurred: {e}", exc_info=True) # exc_info to log traceback
        mm_bot.cancel_all_bot_orders()
        if mm_bot.ws:
            mm_bot.ws.exit()
