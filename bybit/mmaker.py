...
try:
    ...
    print(
        f"{RED}config.json not found in the current realm. Please forge it with your trading parameters.{NC}",
    )
    ...
    print(
        f"{RED}A distortion detected in config.json. Please verify its crystalline structure (JSON format).{NC}",
    )
...
PRICE_THRESHOLD = Decimal(
    str(config.get("PRICE_THRESHOLD", "0.0002")),
)  # 0.02% price movement to trigger re-placement
USE_TESTNET = config.get("USE_TESTNET", True)
POSITION_MODE = config.get("POSITION_MODE", "HedgeMode")  # "HedgeMode" or "OneWayMode"
ORDER_REFRESH_INTERVAL = config.get(
    "ORDER_REFRESH_INTERVAL", 5,
)  # How often to check for order refreshes in seconds
MIN_ORDER_REFRESH_INTERVAL = config.get(
    "MIN_ORDER_REFRESH_INTERVAL", 1,
)  # Minimum time between order refreshes to prevent API spam
MAX_ORDER_REFRESH_INTERVAL = config.get(
    "MAX_ORDER_REFRESH_INTERVAL", 60,
)  # Maximum time between order refreshes to prevent stale orders
...
def _check_termux_toast_availability():
    ...
    if os.system("command -v termux-toast > /dev/null 2>&1") == 0:
        ...
        logger.warning(
            f"{YELLOW}Warning: 'termux-toast' command not found. Toasts will be disabled. "
...
def send_toast(
    ...
    if _TERMUX_TOAST_ENABLED:
        try:
            os.system(
                f"termux-toast -b '{background_color}' -c '{text_color}' -t {duration} '{message}'",
            )
...
def format_metric(
    ...
    label_width: int = 25,
    value_precision: int
    | None = None,  # Make optional, use _calculate_decimal_precision if None
    unit: str = "",
    is_pnl: bool = False,
) -> str:
    ...
    if isinstance(value, (Decimal, float)):
        ...
        current_precision = (
            ...
            else _calculate_decimal_precision(Decimal(str(value)))
        )
        ...
        if is_pnl:
            ...
            formatted_value = (
                f"{actual_value_color}{sign}{value:,.{current_precision}f}{unit}{NC}"
            )
        else:
            formatted_value = (
...
def format_order(
    order: dict[str, Any], price_precision: int, qty_precision: int,
) -> str:
    ...
    formatted_price = (
        f"{order['price']:.{price_precision}f}"
        if price_precision is not None
        else str(order["price"])
    )
    formatted_qty = (
        f"{order['qty']:.{qty_precision}f}"
...
def format_position(
    position: dict[str, Any], side: str, price_precision: int, qty_precision: int,
) -> str:
    """Formats a position for display in the UI."""
    side_color = GREEN if side == "Long" else RED
    # Use f-string formatting with dynamic precision
    formatted_size = (
        f"{position['size']:.{qty_precision}f}"
        if qty_precision is not None
        else str(position["size"])
    )
    formatted_avg_price = (
        f"{position['avg_price']:.{price_precision}f}"
        if price_precision is not None
        else str(position["avg_price"])
    )
    ...
    try:
        ...
        if message.get("topic") == f"orderbook.1.{SYMBOL}":
            if data.get("b") and data.get("a"):
                ...
                logger.debug(
                    f"WS Orderbook: Mid Price: {ws_state['mid_price']:.{_calculate_decimal_precision(symbol_info['price_precision'])}f}",
                )
    except (KeyError, IndexError, ValueError, TypeError, json.JSONDecodeError) as e:
        logger.error(
            f"Error processing public WS message: {type(e).__name__} - {e} | Message: {message}",
            exc_info=True,
        )
            ...
            for order_update in message.get("data", []):
                ...
                if order_update["orderStatus"] in ["Filled", "Canceled", "Deactivated"]:
                    if order_id in ws_state["open_orders"]:
                        logger.info(
                            f"WS Order Update: Order {order_id} (Client ID: {order_update.get('orderLinkId')}) for {order_update.get('symbol')} is {order_update['orderStatus']}.",
                        )
                ...
                if position_update.get("symbol") == SYMBOL:
                    ...
                    ws_state["positions"][side] = {
                        ...
                        "unrealisedPnl": Decimal(
                            position_update.get("unrealisedPnl", "0"),
                        ),
                    }
    except (KeyError, ValueError, TypeError, json.JSONDecodeError) as e:
        logger.error(
            f"Error processing private WS message: {type(e).__name__} - {e} | Message: {message}",
            exc_info=True,
        )
...
class BybitClient:
    ...
    def __init__(self, api_key: str, api_secret: str, testnet: bool):
        ...
        self.ws_public = WebSocket(
            testnet=testnet, channel_type="linear",
        )  # For linear/inverse
        ...
        self.api_call_cooldown = (
            0.1  # Minimum time between API calls to prevent rate limiting
        )
    ...
    async def _make_api_call(self, api_method, *args, **kwargs):
        ...
        if current_time - self.last_api_call_time < self.api_call_cooldown:
            await asyncio.sleep(
                self.api_call_cooldown - (current_time - self.last_api_call_time),
            )
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                error_msg = response.get(
                    "retMsg", f"Unknown error (retCode: {ret_code})",
                )
                logger.warning(
                    f"API call to {api_method.__name__} failed (Attempt {attempt + 1}/{self.MAX_RETRIES}) with retCode {ret_code}: {error_msg} | Args: {args}, Kwargs: {kwargs}",
                )
                ...
                if ret_code in [
                    ...
                    30034,
                    30035,
                ]:  # Add 10002 if parameter errors are sometimes transient
                    if attempt < self.MAX_RETRIES - 1:
                        delay = self.RETRY_DELAY_SECONDS * (2**attempt)
                        logger.warning(
                            f"Retrying {api_method.__name__} in {delay} seconds...",
                        )
                        await asyncio.sleep(delay)
                    else:
                        logger.error(
                            f"API call {api_method.__name__} exhausted retries: {error_msg}",
                        )
                        return None
                else:
                    # For non-retryable errors (e.g., invalid parameters), don't retry
                    logger.error(
                        f"Non-retryable API error for {api_method.__name__}: {error_msg}",
                    )
                    return None
            except Exception as e:
                logger.error(
                    f"Exception during API call to {api_method.__name__} (Attempt {attempt + 1}/{self.MAX_RETRIES}): {type(e).__name__} - {e} | Args: {args}, Kwargs: {kwargs}",
                    exc_info=True,
                )
                if attempt < self.MAX_RETRIES - 1:
                    delay = self.RETRY_DELAY_SECONDS * (2**attempt)
                    ...
                    logger.error(
                        f"API call exception for {api_method.__name__} exhausted retries: {type(e).__name__} - {e}",
                    )
                    return None
        ...
        if response and response.get("retCode") == 0:
            ...
            if instruments:
                ...
                if tick_size_str:
                    ...
                    logger.warning(
                        f"Could not find tickSize for {SYMBOL}. Using default: {symbol_info['price_precision']}",
                    )
                ...
                if qty_step_str:
                    ...
                    logger.warning(
                        f"Could not find qtyStep for {SYMBOL}. Using default: {symbol_info['qty_precision']}",
                    )

                logger.info(
                    f"Fetched symbol info for {SYMBOL}: Price Precision = {symbol_info['price_precision']}, Quantity Precision = {symbol_info['qty_precision']}",
                )
                return True
    ...
    async def set_position_mode(self, mode: str) -> bool:
        ...
        if CATEGORY not in ["linear", "inverse"]:
            logger.warning(
                f"Position mode setting is typically for linear/inverse. Current category: {CATEGORY}. Skipping.",
            )
        ...
        if mode not in ["HedgeMode", "OneWayMode"]:
            logger.error(
                f"Invalid position mode '{mode}'. Must be 'HedgeMode' or 'OneWayMode'.",
            )
            return False
        ...
        if response and response.get("retCode") == 0:
            logger.info(
                f"Successfully set position mode to {mode} for {SYMBOL} in {CATEGORY} category.",
            )
            return True
        logger.warning(
            f"Failed to set position mode to {mode} for {SYMBOL}: {response.get('retMsg', 'No error message')}. "
    ...
    def start_websocket_streams(self):
        ...
        try:
            ...
            logger.info(
                "WebSocket streams started and listening for real-time updates.",
            )
        except Exception as e:
            logger.error(
                f"Error starting WebSocket streams: {type(e).__name__} - {e}",
                exc_info=True,
            )
            send_toast(
                f"MMXCEL Critical: WS streams failed to start for {SYMBOL}!",
                "red",
                "white",
            )
    ...
    async def get_balance(
        self, account_type: str = "UNIFIED", coin: str = COIN_FOR_BALANCE,
    ) -> Decimal:
        """Fetches available balance via REST API, discerning the available arcane energies."""
        response = await self._make_api_call(
            self.http_session.get_wallet_balance, accountType=account_type, coin=coin,
        )
        if response and response.get("retCode") == 0:
            if (
                "result" in response
                ...
                and response["result"]["list"][0]
                and "coin" in response["result"]["list"][0]
                and response["result"]["list"][0]["coin"]
            ):
                ...
                if available_balance_str is None or available_balance_str == "":
                    self.current_balance = Decimal("0")
                    logger.warning(
                        f"Available balance for {coin} was None or empty string from API, defaulting to 0. (Raw: '{available_balance_str}')",
                    )
                else:
                    try:
                        ...
                        logger.error(
                            f"Failed to transmute '{available_balance_str}' to Decimal: {type(e).__name__} - {e}. Defaulting to 0.",
                            exc_info=True,
                        )
                        self.current_balance = Decimal("0")
                return self.current_balance
            logger.error(
                f"Unexpected balance response structure: {response.get('retMsg', 'No error message')} | Raw: {response}",
            )
            return Decimal("0")
        logger.error(
            f"Failed to get balance for {coin}: {response.get('retMsg', 'No error message') if response else 'No response'}",
        )
        return Decimal("0")
    ...
    async def get_open_orders_rest(self) -> dict[str, Any]:
        ...
        logger.error(
            f"Failed to get open orders via REST for {SYMBOL}: {response.get('retMsg', 'No error message') if response else 'No response'}",
        )
        return {}
    ...
    async def get_positions_rest(self) -> dict[str, Any]:
        ...
        if response and response.get("retCode") == 0:
            ...
            for position in response["result"]["list"]:
                ...
                if (
                    ...
                    and Decimal(position.get("size", "0")) > 0
                ):
        ...
        logger.error(
            f"Failed to get positions via REST for {SYMBOL}: {response.get('retMsg', 'No error message') if response else 'No response'}",
        )
        return {}
    ...
    async def place_order(
        ...
        if price is not None:
            quantized_price = price.quantize(
                symbol_info["price_precision"], rounding=ROUND_DOWN,
            )

        if quantized_qty <= 0:
            logger.error(
                f"Attempted to place order with non-positive quantized quantity: {quantized_qty} for {SYMBOL} {side}.",
            )
            return None
        ...
        try:
            order_payload = {
                ...
                "positionIdx": 1
                if side == "Buy"
                else 2,  # 1 for long, 2 for short in HedgeMode
            }
            ...
            response = await self._make_api_call(
                self.http_session.place_order, **order_payload,
            )
            ...
            if response and response.get("retCode") == 0:
                ...
                return order_info
            logger.error(
                f"Failed to place {order_type} {side} order for {SYMBOL} {quantized_qty}@{quantized_price if quantized_price else 'N/A'}: {response.get('retMsg', 'No error message') if response else 'No response'}",
            )
            return None
        except Exception as e:
            logger.error(
                f"Error placing order for {SYMBOL} {side} {quantized_qty}@{quantized_price if quantized_price else 'N/A'} (Type: {order_type}): {type(e).__name__} - {e}",
                exc_info=True,
            )
            return None
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        if response and response.get("retCode") == 0:
            ...
            return True
        logger.error(
            f"Failed to cancel order {order_id} for {SYMBOL}: {response.get('retMsg', 'No error message') if response else 'No response'}",
        )
        return False
    ...
    async def cancel_all_orders(self) -> None:
        ...
        if response and response.get("retCode") == 0:
            ...
            logger.error(
                f"Failed to cancel all orders for {SYMBOL}: {response.get('retMsg', 'No error message') if response else 'No response'}",
            )
    ...
    async def place_batch_orders(
        ...
        batch_request_payload = {"category": CATEGORY, "request": []}
        for order in orders:
            # Quantize price and quantity for each order in the batch
            quantized_qty = order["qty"].quantize(
                symbol_info["qty_precision"], rounding=ROUND_DOWN,
            )
            if quantized_qty <= 0:
                logger.warning(
                    f"Skipping batch order with non-positive quantized quantity: {quantized_qty} for {order.get('side')} {SYMBOL}",
                )
                continue

            quantized_price = None
            if "price" in order and order["price"] is not None:
                quantized_price = order["price"].quantize(
                    symbol_info["price_precision"], rounding=ROUND_DOWN,
                )
        ...
        if not batch_request_payload["request"]:
            logger.warning(
                "All orders in batch filtered out due to invalid quantities after quantization.",
            )
            return None

        response = await self._make_api_call(
            self.http_session.place_batch_order, batch_request_payload,
        )

        if response and response.get("retCode") == 0:
            logger.info(
                f"Successfully placed {len(batch_request_payload['request'])} batch orders for {SYMBOL}.",
            )
            return response["result"]
        logger.error(
            f"Failed to place batch orders for {SYMBOL}: {response.get('retMsg', 'No error message') if response else 'No response'}",
        )
        return None
    ...
    async def get_orderbook_snapshot(self) -> dict[str, Any] | None:
        ...
        response = await self._make_api_call(
            self.http_session.get_orderbook, category=CATEGORY, symbol=SYMBOL,
        )
        if response and response.get("retCode") == 0:
            orderbook = response["result"]
            if orderbook and "bids" in orderbook and "asks" in orderbook:
                best_bid = (
                    Decimal(orderbook["bids"][0][0])
                    if orderbook["bids"]
                    else Decimal("0")
                )
                best_ask = (
                    Decimal(orderbook["asks"][0][0])
                    if orderbook["asks"]
                    else Decimal("0")
                )
                ...
                ws_state["last_update_time"] = time.time()
                logger.debug(
                    f"Orderbook snapshot: Mid Price: {mid_price:.{_calculate_decimal_precision(symbol_info['price_precision'])}f}",
                )
                return orderbook
            logger.error(
                f"Orderbook snapshot for {SYMBOL} returned but missing bids/asks data.",
            )
            return None
        logger.error(
            f"Failed to get orderbook snapshot for {SYMBOL}: {response.get('retMsg', 'No error message') if response else 'No response'}",
        )
        return None
...
class MarketMakingStrategy:
    ...
    def __init__(self, client: BybitClient):
        ...
        self.last_mid_price = Decimal(
            "0",
        )  # Track last mid-price for dynamic order replacement
        ...
        self.max_order_refresh_attempts = (
            3  # Maximum attempts to refresh orders before giving up
        )
    ...
    def _validate_config(self) -> bool:
        ...
        if not isinstance(QUANTITY, Decimal) or QUANTITY <= 0:
            logger.critical(
                f"{RED}Config Error: QUANTITY must be a positive Decimal. Current: {QUANTITY}{NC}",
            )
            is_valid = False
        if not isinstance(SPREAD_PERCENTAGE, Decimal) or SPREAD_PERCENTAGE <= 0:
            logger.critical(
                f"{RED}Config Error: SPREAD_PERCENTAGE must be a positive Decimal. Current: {SPREAD_PERCENTAGE}{NC}",
            )
            is_valid = False
        if not isinstance(PROFIT_PERCENTAGE, Decimal) or PROFIT_PERCENTAGE <= 0:
            logger.critical(
                f"{RED}Config Error: PROFIT_PERCENTAGE must be a positive Decimal. Current: {PROFIT_PERCENTAGE}{NC}",
            )
            is_valid = False
        if not isinstance(STOP_LOSS_PERCENTAGE, Decimal) or STOP_LOSS_PERCENTAGE <= 0:
            logger.critical(
                f"{RED}Config Error: STOP_LOSS_PERCENTAGE must be a positive Decimal. Current: {STOP_LOSS_PERCENTAGE}{NC}",
            )
            is_valid = False
        if (
            not isinstance(PRICE_THRESHOLD, Decimal) or PRICE_THRESHOLD < 0
        ):  # Can be zero for continuous re-placement
            logger.critical(
                f"{RED}Config Error: PRICE_THRESHOLD must be a non-negative Decimal. Current: {PRICE_THRESHOLD}{NC}",
            )
            is_valid = False
        if not isinstance(MAX_OPEN_ORDERS, int) or MAX_OPEN_ORDERS <= 0:
            logger.critical(
                f"{RED}Config Error: MAX_OPEN_ORDERS must be a positive integer. Current: {MAX_OPEN_ORDERS}{NC}",
            )
            is_valid = False
        if not isinstance(ORDER_LIFESPAN_SECONDS, int) or ORDER_LIFESPAN_SECONDS <= 0:
            logger.critical(
                f"{RED}Config Error: ORDER_LIFESPAN_SECONDS must be a positive integer. Current: {ORDER_LIFESPAN_SECONDS}{NC}",
            )
            is_valid = False
        if not isinstance(ORDER_REFRESH_INTERVAL, int) or ORDER_REFRESH_INTERVAL <= 0:
            logger.critical(
                f"{RED}Config Error: ORDER_REFRESH_INTERVAL must be a positive integer. Current: {ORDER_REFRESH_INTERVAL}{NC}",
            )
            is_valid = False
        if (
            not isinstance(MIN_ORDER_REFRESH_INTERVAL, int)
            or MIN_ORDER_REFRESH_INTERVAL <= 0
            or MIN_ORDER_REFRESH_INTERVAL > ORDER_REFRESH_INTERVAL
        ):
            logger.critical(
                f"{RED}Config Error: MIN_ORDER_REFRESH_INTERVAL must be a positive integer <= ORDER_REFRESH_INTERVAL. Current: {MIN_ORDER_REFRESH_INTERVAL}{NC}",
            )
            is_valid = False
        if (
            not isinstance(MAX_ORDER_REFRESH_INTERVAL, int)
            or MAX_ORDER_REFRESH_INTERVAL <= 0
            or MAX_ORDER_REFRESH_INTERVAL < ORDER_REFRESH_INTERVAL
        ):
            logger.critical(
                f"{RED}Config Error: MAX_ORDER_REFRESH_INTERVAL must be a positive integer >= ORDER_REFRESH_INTERVAL. Current: {MAX_ORDER_REFRESH_INTERVAL}{NC}",
            )
            is_valid = False

        if SPREAD_PERCENTAGE >= PRICE_THRESHOLD > 0:
            logger.warning(
                f"{YELLOW}Warning: SPREAD_PERCENTAGE ({SPREAD_PERCENTAGE:.4%}) is greater than or equal to PRICE_THRESHOLD ({PRICE_THRESHOLD:.4%}). "
        ...
        if PROFIT_PERCENTAGE <= SPREAD_PERCENTAGE:
            logger.warning(
                f"{YELLOW}Warning: PROFIT_PERCENTAGE ({PROFIT_PERCENTAGE:.4%}) is less than or equal to SPREAD_PERCENTAGE ({SPREAD_PERCENTAGE:.4%}). "
        ...
        if (
            symbol_info["qty_precision"] * 2 > QUANTITY
        ):  # Check if QUANTITY is very close to minimum step
            logger.warning(
                f"{YELLOW}Warning: Configured QUANTITY ({QUANTITY}) is very small compared to the exchange's minimum quantity step ({symbol_info['qty_precision']}). "
    ...
    async def place_market_making_orders(self):
        ...
        if current_time - self.last_order_refresh_time >= self.order_refresh_interval:
            ...
            if self.order_refresh_counter >= self.max_order_refresh_attempts:
                self.order_refresh_interval = min(
                    self.order_refresh_interval * 2, self.max_order_refresh_interval,
                )
                logger.info(
                    f"Increasing order refresh interval to {self.order_refresh_interval} seconds due to multiple attempts without successful order placement/management.",
                )
                ...
                if self.order_refresh_interval > self.min_order_refresh_interval:
                    self.order_refresh_interval = max(
                        self.order_refresh_interval * 0.9,
                        self.min_order_refresh_interval,
                    )
                    logger.debug(
                        f"Decreasing order refresh interval to {self.order_refresh_interval:.2f} seconds.",
                    )
            ...
            if self.last_mid_price != Decimal("0") and mid_price != Decimal("0"):
                if self.last_mid_price == mid_price:
                    ...
                    price_change_percentage = (
                        abs(mid_price - self.last_mid_price) / self.last_mid_price
                    )

                if price_change_percentage >= PRICE_THRESHOLD:
                    logger.info(
                        f"Price moved {price_change_percentage:.4%} (>= {PRICE_THRESHOLD:.4%}). Cancelling all orders to re-place for {SYMBOL}.",
                    )
                ...
                elif ws_state["open_orders"]:
                    logger.debug(
                        f"No significant price change for {SYMBOL} and orders exist. Skipping new order placement.",
                    )
            ...
            orders_to_cancel_ids = [
                order_id
                for order_id, details in list(ws_state["open_orders"].items())
                if time.time() - details["timestamp"] > ORDER_LIFESPAN_SECONDS
            ]
            if orders_to_cancel_ids:
                logger.info(
                    f"Cancelling {len(orders_to_cancel_ids)} stale orders for {SYMBOL} to keep the ledger clean.",
                )
                # Use asyncio.gather for concurrent cancellation if multiple stale orders
                await asyncio.gather(
                    *[self.client.cancel_order(oid) for oid in orders_to_cancel_ids],
                )
            ...
            if len(ws_state["open_orders"]) >= MAX_OPEN_ORDERS:
                logger.info(
                    f"Max open orders ({MAX_OPEN_ORDERS}) reached for {SYMBOL}. Skipping new order placement.",
                )
                return
            ...
            bid_price = bid_price.quantize(
                symbol_info["price_precision"], rounding=ROUND_DOWN,
            )
            ask_price = ask_price.quantize(
                symbol_info["price_precision"], rounding=ROUND_UP,
            )
            ...
            has_buy_order = any(
                order["side"] == "Buy" for order in ws_state["open_orders"].values()
            )
            ...
            has_sell_order = any(
                order["side"] == "Sell" for order in ws_state["open_orders"].values()
            )
            if (
                not has_sell_order
                and (len(ws_state["open_orders"]) + len(orders_to_place))
                < MAX_OPEN_ORDERS
            ):
            ...
            if orders_to_place:
                ...
                self.last_mid_price = (
                    mid_price  # Update last mid price after placing orders
                )
                ...
                logger.debug(
                    f"No new orders to place for {SYMBOL} at this time (e.g., max orders reached or existing orders are fine).",
                )
    ...
    def _is_market_data_available(self) -> bool:
        ...
        if (
            ...
            or ws_state["best_bid"] == Decimal("0")
            or ws_state["best_ask"] == Decimal("0")
        ):
            logger.warning(
                f"Market data (mid, bid, ask) for {SYMBOL} not available yet. Skipping operation.",
            )
            return False
        return True
    ...
    async def rebalance_inventory(self):
        ...
        if abs(net_position) > REBALANCE_THRESHOLD_QTY:
            ...
            quantized_qty_to_rebalance = qty_to_rebalance.quantize(
                symbol_info["qty_precision"], rounding=ROUND_DOWN,
            )
            if quantized_qty_to_rebalance <= 0:
                logger.warning(
                    f"Quantized rebalance quantity for {SYMBOL} is zero ({quantized_qty_to_rebalance}), skipping rebalance.",
                )
                return

            logger.warning(
                f"{YELLOW}Inventory imbalance detected for {SYMBOL}. Net position: {net_position:.{_calculate_decimal_precision(symbol_info['qty_precision'])}f}. "
                f"Attempting to rebalance by placing a {side_to_rebalance} market order for {quantized_qty_to_rebalance}.{NC}",
            )
            send_toast(
                f"MMXCEL: Rebalancing {quantized_qty_to_rebalance:.{_calculate_decimal_precision(symbol_info['qty_precision'])}f} {SYMBOL}!",
                "yellow",
                "black",
            )
            ...
            asyncio.create_task(
                self.client.place_order(
                    side_to_rebalance,
                    quantized_qty_to_rebalance,
                    order_type="Market",
                    client_order_id=client_rebalance_id,
                ),
            )
        else:
            logger.debug(
                f"No significant inventory imbalance for {SYMBOL}. Net position: {net_position:.{_calculate_decimal_precision(symbol_info['qty_precision'])}f}.",
            )
    ...
    async def manage_stop_loss_and_profit_take(self):
        ...
        if long_size > 0 and long_avg_price > 0:  # Ensure valid position
            ...
            if (long_avg_price * long_size) > 0:
                pnl_percentage = pnl_usdt / (
                    long_avg_price * long_size
                )  # Corrected: PnL as percentage of entry value

            # Quantize the size before placing a closing order
            quantized_long_size = long_size.quantize(
                symbol_info["qty_precision"], rounding=ROUND_DOWN,
            )
            if quantized_long_size <= 0:
                logger.warning(
                    f"Quantized long closing quantity for {SYMBOL} is zero ({quantized_long_size}), skipping close.",
                )
            elif pnl_percentage < -STOP_LOSS_PERCENTAGE:
                logger.critical(
                    f"{RED}Stop-loss triggered for LONG position on {SYMBOL}! Unrealised PnL: {pnl_usdt:.2f} USDT ({pnl_percentage:.2%}). "
                    f"Placing market sell order to close {quantized_long_size}.{NC}",
                )
                send_toast(
                    f"MMXCEL: LONG SL triggered for {SYMBOL}! PnL: {pnl_usdt:.2f} USDT",
                    "red",
                    "white",
                )
                asyncio.create_task(
                    self.client.place_order(
                        ...
                        quantized_long_size,
                        order_type="Market",
                        client_order_id=f"mmxcel-sl-long-{int(time.time() * 1000)}",
                    ),
                )
            elif pnl_percentage >= PROFIT_PERCENTAGE:
                logger.info(
                    f"{GREEN}Profit-take triggered for LONG position on {SYMBOL}. Unrealised PnL: {pnl_usdt:.2f} USDT ({pnl_percentage:.2%}). "
                    f"Placing market sell order to close {quantized_long_size}.{NC}",
                )
                send_toast(
                    f"MMXCEL: LONG TP triggered for {SYMBOL}! PnL: {pnl_usdt:.2f} USDT",
                    "green",
                    "black",
                )
                asyncio.create_task(
                    self.client.place_order(
                        ...
                        quantized_long_size,
                        order_type="Market",
                        client_order_id=f"mmxcel-tp-long-{int(time.time() * 1000)}",
                    ),
                )
            else:
                logger.debug(
                    f"Long position for {SYMBOL} within acceptable PnL range. Current PnL: {pnl_usdt:.2f} USDT ({pnl_percentage:.2%}).",
                )
        ...
        if short_size > 0 and short_avg_price > 0:  # Ensure valid position
            ...
            if (short_avg_price * short_size) > 0:
                pnl_percentage = pnl_usdt / (
                    short_avg_price * short_size
                )  # Corrected: PnL as percentage of entry value

            # Quantize the size before placing a closing order
            quantized_short_size = short_size.quantize(
                symbol_info["qty_precision"], rounding=ROUND_DOWN,
            )
            if quantized_short_size <= 0:
                logger.warning(
                    f"Quantized short closing quantity for {SYMBOL} is zero ({quantized_short_size}), skipping close.",
                )
            elif pnl_percentage < -STOP_LOSS_PERCENTAGE:
                logger.critical(
                    f"{RED}Stop-loss triggered for SHORT position on {SYMBOL}! Unrealised PnL: {pnl_usdt:.2f} USDT ({pnl_percentage:.2%}). "
                    f"Placing market buy order to close {quantized_short_size}.{NC}",
                )
                send_toast(
                    f"MMXCEL: SHORT SL triggered for {SYMBOL}! PnL: {pnl_usdt:.2f} USDT",
                    "red",
                    "white",
                )
                asyncio.create_task(
                    self.client.place_order(
                        "Buy",
                        quantized_short_size,
                        order_type="Market",
                        client_order_id=f"mmxcel-sl-short-{int(time.time() * 1000)}",
                    ),
                )
            elif pnl_percentage >= PROFIT_PERCENTAGE:
                logger.info(
                    f"{GREEN}Profit-take triggered for SHORT position on {SYMBOL}. Unrealised PnL: {pnl_usdt:.2f} USDT ({pnl_percentage:.2%}). "
                    f"Placing market buy order to close {quantized_short_size}.{NC}",
                )
                send_toast(
                    f"MMXCEL: SHORT TP triggered for {SYMBOL}! PnL: {pnl_usdt:.2f} USDT",
                    "green",
                    "black",
                )
                asyncio.create_task(
                    self.client.place_order(
                        ...
                        quantized_short_size,
                        order_type="Market",
                        client_order_id=f"mmxcel-tp-short-{int(time.time() * 1000)}",
                    ),
                )
            else:
                logger.debug(
                    f"Short position for {SYMBOL} within acceptable PnL range. Current PnL: {pnl_usdt:.2f} USDT ({pnl_percentage:.2%}).",
                )
    ...
    async def shutdown(self):
        ...
        if self.running:
            ...
            send_toast(
                f"MMXCEL: Shutting down {SYMBOL}, cancelling orders...",
                "#FFA500",
                "white",
            )
            await self.client.cancel_all_orders()
    ...
    async def run(self):
        ...
        if not await self.client.get_symbol_info():
            logger.critical(
                "Failed to fetch symbol information. Cannot proceed without it.",
            )
            send_toast(
                f"MMXCEL Error: Symbol info fetch failed for {SYMBOL}!", "red", "white",
            )
            return
        ...
        if not await self.client.set_position_mode(POSITION_MODE):
            logger.critical(
                f"Failed to set position mode to {POSITION_MODE}. This is critical for the bot's strategy. Aborting.",
            )
            send_toast(
                f"MMXCEL Error: Failed to set {POSITION_MODE} for {SYMBOL}!",
                "red",
                "white",
            )
            return
        ...
        self.rebalance_task = asyncio.create_task(
            self._periodic_task(self.rebalance_inventory, interval=10, name="Rebalance"),
        )
        self.stop_loss_task = asyncio.create_task(
            self._periodic_task(
                self.manage_stop_loss_and_profit_take, interval=2, name="PnL Management",
            ),
        )
        ...
        try:
            while self.running:
                clear_screen()
                print_neon_header(
                    f"MMXCEL Bybit Market Making Bot - {SYMBOL}", color=MAGENTA,
                )
                print_neon_separator()

                # Display initial configuration for clarity
                print(f"{BOLD}{CYAN}--- Active Configuration ---{NC}")
                price_disp_prec = _calculate_decimal_precision(
                    symbol_info["price_precision"],
                )
                qty_disp_prec = _calculate_decimal_precision(
                    symbol_info["qty_precision"],
                )

                print(
                    format_metric(
                        "Symbol", SYMBOL, YELLOW, value_color=WHITE, label_width=20,
                    ),
                )
                print(
                    format_metric(
                        ...
                        QUANTITY,
                        YELLOW,
                        value_precision=_calculate_decimal_precision(QUANTITY),
                        label_width=20,
                    ),
                )
                print(
                ...
                print(
                    format_metric(
                        ...
                        symbol_info["price_precision"],
                        YELLOW,
                        value_precision=price_disp_prec,
                        label_width=20,
                    ),
                )
                print(
                    format_metric(
                        ...
                        symbol_info["qty_precision"],
                        YELLOW,
                        value_precision=qty_disp_prec,
                        label_width=20,
                    ),
                )
                print(
                    format_metric(
                        "Order Refresh Interval (s)",
                        self.order_refresh_interval,
                        YELLOW,
                        value_color=WHITE,
                        label_width=20,
                    ),
                ...
                await self.client.get_balance(
                    coin=COIN_FOR_BALANCE,
                )  # Refresh balance every loop for display
                print(
                    format_metric(
                        f"Available Balance ({COIN_FOR_BALANCE})",
                        self.client.current_balance,
                        YELLOW,
                        value_precision=2,
                    ),
                )
                ...
                last_update_str = (
                    datetime.fromtimestamp(ws_state["last_update_time"]).strftime(
                        "%Y-%m-%d %H:%M:%S",
                    )
                    if ws_state["last_update_time"]
                    else "N/A"
                )
                print(
                    format_metric(
                ...
                print(
                    format_position(short_pos, "Short", price_disp_prec, qty_disp_prec),
                )
                print_neon_separator()

                # Open Orders Details
                print(
                    f"{BOLD}{CYAN}--- Open Orders ({len(ws_state['open_orders'])}) ---{NC}",
                )
                if ws_state["open_orders"]:
                    # Sort orders by timestamp for consistent display
                    sorted_orders = sorted(
                        list(ws_state["open_orders"].values()),
                ...
                await asyncio.sleep(
                    1,
                )  # Main loop delay, balances responsiveness with API usage
        except asyncio.CancelledError:
            logger.info("Main loop task cancelled.")
        except Exception as e:
            logger.critical(
                f"An unhandled critical error occurred in the main loop: {type(e).__name__} - {e}",
                exc_info=True,
            )
            send_toast(
                f"MMXCEL Critical Error: Bot crashed! Check logs for {SYMBOL}!",
                "red",
                "white",
            )
        finally:
    ...
    async def _periodic_task(self, func, interval, name):
        ...
        try:
            ...
            logger.error(
                f"Error in periodic task '{name}': {type(e).__name__} - {e}",
                exc_info=True,
            )
            send_toast(
                f"MMXCEL Critical: {name} task failed for {SYMBOL}!", "red", "white",
            )
        finally:
...
def signal_handler(sig, frame):
    ...
    send_toast(
        f"MMXCEL: Termination signal received for {SYMBOL}. Shutting down...",
        "#FFA500",
        "white",
    )
    ...
    if loop.is_running():
        ...
        for task in asyncio.all_tasks(loop):
            if (
                task.get_name() == "MarketMakingStrategy_run_task"
            ):  # Identify by its assigned name
                task.cancel()
                break
    else:
...
async def main():
    ...
    if not API_KEY or not API_SECRET:
        print(
            f"{RED}BYBIT_API_KEY or BYBIT_API_SECRET not found in .env file. "
    ...
    )  # Termination signal (e.g., system shutdown)

    # Create the main strategy task and assign it a name for easier identification in signal handler
    main_strategy_task = asyncio.create_task(
        strategy.run(), name="MarketMakingStrategy_run_task",
    )
    ...
    try:
        ...
        logger.critical(
            f"{RED}A critical error occurred during the main invocation: {type(e).__name__} - {e}",
            exc_info=True,
        )
        send_toast(
            f"MMXCEL Critical Error: Bot crashed! Check logs for {SYMBOL}!",
        ...
        logger.info(
            "Bot execution finished. May your digital journey be ever enlightened.",
        )
