...
async def market_making_strategy(market_data: dict, account_info: dict, http_client: HTTP, bot_instance: Any, symbols_to_trade: list[str]):
    ...
    for symbol in symbols_to_trade:
        ...
        if order_quantity < min_order_qty:
            ...
            logger.warning(f"[{symbol}] Calculated quantity too small. Using minimum: {order_quantity}")
        ...
        if not bot_instance.ws_manager.positions.get(symbol) or decimal.Decimal(bot_instance.ws_manager.positions[symbol].get('size', '0')) == 0:
            try:
                ...
                logger.info(f"[{symbol}] No position found. Placing new buy and sell limit orders.")
            ...
            logger.info(f"[{symbol}] Position already exists. Skipping new order placement.")
...
class BybitTradingBot:
    ...
    async def fetch_symbol_info(self, symbols: list[str]):
        ...
        try:
            ...
            if response and response['retCode'] == 0:
                ...
                logger.error(f"Failed to fetch instrument info: {response.get('retMsg')}")
        except Exception as e:
    ...
    async def run(self, symbols: list[str], interval: int = 5):
        ...
        while True:
            try:
                account_info = await self.session.get_wallet_balance(accountType="UNIFIED")
