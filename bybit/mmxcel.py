...
def send_termux_toast(
    message: str, background_color: str = "black", text_color: str = "white"
):
...
class BybitClient:
    ...
    def start_websocket_streams(self):
        ...
        try:
            ...
            send_termux_toast(
                "MMXCEL Critical: WS streams failed to start!", "red", "white"
            )
    ...
    async def place_order(
        ...
        try:
            ...
            if order_type == "Limit":
                ...
                order_payload["postOnly"] = (
                    True  # Ensure order is placed as a maker order
                )
...
class MarketMakingStrategy:
    ...
    async def rebalance_inventory(self):
        ...
        if abs(net_position) > REBALANCE_THRESHOLD_QTY:
            ...
            send_termux_toast(
                f"MMXCEL: Rebalancing {quantized_qty_to_rebalance:.2f} {SYMBOL}!",
                "yellow",
                "black",
            )
    ...
    async def manage_stop_loss_and_profit_take(self):
        ...
        if long_size > 0 and long_avg_price > 0:  # Ensure valid position
            ...
            if quantized_long_size <= 0:
                ...
                send_termux_toast(
                    f"MMXCEL: LONG SL triggered! PnL: {pnl_usdt:.2f}", "red", "white"
                )
                asyncio.create_task(
                    self.client.place_order(
                        "Sell",
                ...
                send_termux_toast(
                    f"MMXCEL: LONG TP triggered! PnL: {pnl_usdt:.2f}", "green", "black"
                )
                asyncio.create_task(
                    self.client.place_order(
                        "Sell",
        ...
        if short_size > 0 and short_avg_price > 0:  # Ensure valid position
            ...
            if quantized_short_size <= 0:
                ...
                send_termux_toast(
                    f"MMXCEL: SHORT SL triggered! PnL: {pnl_usdt:.2f}", "red", "white"
                )
                asyncio.create_task(
                    self.client.place_order(
                        "Buy",
                ...
                send_termux_toast(
                    f"MMXCEL: SHORT TP triggered! PnL: {pnl_usdt:.2f}", "green", "black"
                )
                asyncio.create_task(
                    self.client.place_order(
                        "Buy",
    ...
    async def shutdown(self):
        ...
        if self.running:
            ...
            send_termux_toast(
                "MMXCEL: Shutting down, cancelling orders...", "#FFA500", "white"
            )
            await self.client.cancel_all_orders()
    ...
    async def run(self):
        ...
        try:
            ...
            send_termux_toast(
                "MMXCEL Critical Error: Bot crashed! Check logs!", "red", "white"
            )
        finally:
            await self.shutdown()
