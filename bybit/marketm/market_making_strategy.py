...
async def market_making_strategy(
    ...
    for symbol in symbols:
        ...
        if klines_data and klines_data.get("result", {}).get("list"):
            closes = [
                Decimal(k[4]) for k in klines_data.get("result", {}).get("list", [])
            ]
            if len(closes) > 1:
                returns = np.diff([float(c) for c in closes]) / [
                    float(c) for c in closes[:-1]
                ]
        ...
        if current_position_size != 0:
            inventory_skew = (
                current_position_size / max_inventory_units
            ) * adjusted_spread

        if (
            abs(current_position_size) < max_inventory_units
            and best_bid_price > 0
            and best_ask_price > 0
        ):
            if (
                bot_instance.get_open_positions_count()
                ...
                buy_qty = await bot_instance.calculate_position_size(
                    symbol,
                    float(capital_percentage_per_order),
                    best_bid_price,
                    account_info,
                )
                if buy_qty > 0:
                    limit_buy_price = bot_instance._round_to_tick_size(
                        symbol,
                        best_bid_price
                        * (Decimal("1") - adjusted_spread - inventory_skew),
                    )
                ...
                if sell_qty > 0:
                    limit_sell_price = bot_instance._round_to_tick_size(
                        symbol,
                        best_ask_price
                        * (Decimal("1") + adjusted_spread + inventory_skew),
                    )
