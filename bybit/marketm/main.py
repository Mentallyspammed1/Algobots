...
# --- Logging Setup ---
log_level = (
    logging.DEBUG if os.getenv("DEBUG_LOG", "false").lower() == "true" else logging.INFO
)
logging.basicConfig(
    level=log_level,
    format="%(asctime)s - %(levelname)s - %(name)s - %(message)s",
    handlers=[logging.StreamHandler(), logging.FileHandler("bybit_bot.log")],
)
...
USE_TESTNET = (
    os.getenv("BYBIT_USE_TESTNET", "false") or config.get("BYBIT_USE_TESTNET", "false")
).lower() == "true"
...
class BybitWebSocketManager:
    ...
    async def subscribe(self, symbols: list[str]):
        ws = self._get_ws()
        public_channels = (
            [f"orderbook.1.{symbol}" for symbol in symbols]
        ...
        try:
            ...
            send_email_alert(
                ...
                f"Failed to subscribe to WebSocket channels: {e}",
            )
    ...
    async def reconnect(self, symbols: list[str]):
        ...
        if self.reconnect_attempts > MAX_RETRIES:
            ...
            send_email_alert(
                "Critical: Max Reconnects",
                "Bot shutting down due to persistent connection issues.",
            )
            sys.exit(1)
...
class BybitTradingBot:
        ...
        try:
            for symbol in symbols:
                for attempt in range(MAX_RETRIES):
                    ...
                    send_email_alert(
                        "Symbol Info Failure",
                        f"Failed to fetch instrument info for {symbol} after retries.",
                    )
        except Exception as e:
            logger.error(
                f"An unexpected error occurred during fetch_symbol_info: {e}",
                exc_info=True,
            )
    ...
    async def get_market_data(self, symbol: str) -> dict | None:
        ...
        if (
            ws_data
            and ws_data.get("orderbook")
            and ws_data.get("ticker")
            and (time.time() * 1000 - ws_data.get("timestamp", 0)) < 2000
        ):
            return ws_data
        logger.debug(
    ...
    def get_open_positions_count(self) -> int:
        return sum(
            1
            for position_data in self.ws_manager.positions.values()
        ...
        **kwargs,
        ...
        try:
            ...
            send_email_alert(
                ...
                f"For {symbol} ({order_type} {side} {qty}): {error_msg}",
            )
            return None
        except Exception as e:
            logger.error(
                f"Exception occurred while placing order for {symbol}: {e}",
                exc_info=True,
            )
            return None
    ...
    async def cancel_all_orders(self, symbol: str) -> bool:
        try:
            ...
            logger.error(
                f"Exception occurred while cancelling all orders for {symbol}: {e}",
                exc_info=True,
            )
            return False
    ...
    async def run(self, symbols: list[str], interval: int):
        ...
        try:
            ...
            send_email_alert(
                "Critical Bot Error",
                f"An unhandled critical error occurred:\n{traceback.format_exc()}",
            )
        finally:
