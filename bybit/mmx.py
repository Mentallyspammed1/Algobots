...
class MarketState:
    ...
    open_orders: dict[str, dict[str, Any]] = field(default_factory=dict) # order_id -> {details}
    positions: dict[str, dict[str, Any]] = field(default_factory=dict) # "Long" or "Short" -> {details}
    ...
    price_history: deque[dict[str, Decimal]] = field(default_factory=lambda: deque(maxlen=100)) # Stores {'price': Decimal, 'timestamp': float}
    trade_history: deque[dict[str, Any]] = field(default_factory=lambda: deque(maxlen=500)) # Stores filled trade details
    ...
    def update_price_history(self):
        ...
        if self.mid_price > Decimal("0"):
            self.price_history.append({'price': self.mid_price, 'timestamp': time.time()})
    ...
    def is_data_fresh(self, timeout_seconds: int) -> bool:
        ...
        if self.mid_price <= Decimal("0") or self.best_bid <= Decimal("0") or self.best_ask <= Decimal("0"):
            bot_health.update_component('market_data_freshness', 0.0, "Market data invalid (zero prices)")
            self.data_quality_score = 0.0
            return False

        if age > timeout_seconds:
            bot_health.update_component('market_data_freshness', 0.0, f"Market data stale: {age:.1f}s")
            self.data_quality_score = 0.0
            return False
        ...
        bot_health.update_component('market_data_freshness', float(freshness_score), f"Market data age: {age:.1f}s")
...
class SymbolInfo:
    ...
    bid_levels: list[tuple[Decimal, Decimal]] = field(default_factory=list) # (price, quantity)
    ask_levels: list[tuple[Decimal, Decimal]] = field(default_factory=list) # (price, quantity)
...
class SessionStats:
    ...
    # PnL tracking
    profit_history: deque[tuple[float, Decimal]] = field(default_factory=lambda: deque(maxlen=100)) # (timestamp, total_pnl)
    max_drawdown: Decimal = Decimal("0") # As a percentage of initial capital or peak equity
    peak_pnl: Decimal = Decimal("0")

    api_error_counts: defaultdict[str, int] = field(default_factory=lambda: defaultdict(int))
    ...
    def update_pnl(self, current_pnl: Decimal):
        ...
        elif current_pnl < Decimal("0"): # If peak is 0 or negative, and current is negative
            self.max_drawdown = max(self.max_drawdown, abs(current_pnl)) # Simple absolute drawdown if no profit yet
...
class SystemMonitor:
    ...
    _cpu_usage_history: deque[float] = field(default_factory=lambda: deque(maxlen=60)) # Last 60 seconds of CPU usage
    ...
    def update_stats(self):
        ...
        mem_score = max(0.0, 1.0 - (current_mem / config.CB_HIGH_MEMORY_MB)) if config.CB_HIGH_MEMORY_MB > 0 else 1.0
        cpu_score = max(0.0, 1.0 - (current_cpu / 100.0)) # 100% CPU is 0 score

        bot_health.update_component('system_memory', float(mem_score), f"Mem: {current_mem:.1f}MB")
        bot_health.update_component('system_cpu', float(cpu_score), f"CPU: {current_cpu:.1f}%")
    ...
    def cleanup_memory(self) -> int:
        ...
        current_mem = self.get_memory_usage()
        mem_score = max(0.0, 1.0 - (current_mem / config.CB_HIGH_MEMORY_MB)) if config.CB_HIGH_MEMORY_MB > 0 else 1.0
        bot_health.update_component('system_memory_after_cleanup', float(mem_score), f"Mem after cleanup: {current_mem:.1f}MB")
        return collected
# --- END NEW DATACLASSES ---
...
class BotHealth:
    ...
    def update_component(self, name: str, score: float, message: str = 'OK', weight: float | None = None):
...
class BotConfig:
    ...
    STRATEGY_PLUGIN_NAME: str | None = None # Name of the specific strategy plugin to use, e.g., "my_custom_strategy"
    ...
    def _load_config_safe(self) -> BotConfig:
        ...
        try:
            ...
            if self.load_errors >= self.max_load_errors:
                print("CRITICAL: Max configuration load errors reached. Using default config. Please fix config.json immediately.")
                return BotConfig()
...
class EnhancedBybitClient:
    ...
    def __init__(self, key: str, secret: str, testnet: bool,
                 ...
                 send_toast_func: Callable[[str, str, str], None], loop: asyncio.AbstractEventLoop):

        self.key = key
        self.secret = secret
        self.testnet = testnet
        self.loop = loop
        self.http = HTTP(testnet=testnet, api_key=key, api_secret=secret, recv_window=20000)
        ...
        def _on_ws_close(ws_type: str):
            ...
            if not _SHUTDOWN_REQUESTED and self.loop.is_running():
                ...
                if ws_type not in self._reconnect_tasks:
                    future = asyncio.run_coroutine_threadsafe(self.reconnect_ws(ws_type), self.loop)
    ...
    async def _subscribe_private_topics(self):
        ...
        if self.private_ws:
            try:
                ...
                self.log.info("Subscribed to private streams (order, position, wallet, execution)")
            except Exception as e:
    ...
    def _process_execution(self, exec_data: dict[str, Any]):
        ...
        if not order_details:
            self.log.warning("Execution received for an untracked or already closed order.", order_id=order_id)
        ...
        if expected_price > 0 and actual_price > 0:
            ...
            if abs(slippage) > self.config.MAX_SLIPPAGE_PERCENTAGE:
                ...
                self.log.warning(f"High slippage detected on fill: {side} order",
                                 order_id=order_id,
            ...
            if topic == "execution":
                ...
                for order_data in msg["data"]:
                    ...
                    if order_status == "Filled":
                        if order_id in self.market_state.open_orders:
                            ...
                            self.log.info("Order fully filled and closed.", order_id=order_id)
                        ...
                        if order_id in self.market_state.open_orders:
                            ...
                            self.log.info("Order cancelled by exchange or deactivated", order_id=order_id, status=order_status)

                    elif order_status == "Rejected":
                        if order_id in self.market_state.open_orders:
                            self.market_state.open_orders.pop(order_id, None)
                        self.session_stats.orders_rejected += 1
                        self.log.warning("Order rejected", order_id=order_id, reject_reason=order_data.get('rejectReason'))
                        ...
                        if order_status in ["New", "PartiallyFilled", "PendingNew"]:
                            ...
                            self.log.debug("Order status update", order_id=order_id, status=order_status)

            elif topic == "position":
                for pos_data in msg["data"]:
                    if pos_data.get("symbol") == self.config.SYMBOL: # Fixed typo: pos_data.get("symbol")
                ...
                for wallet_data in msg["data"]:
                    if wallet_data.get("coin") == 'USDT':
                        ...
                        self.log.debug("Wallet balance updated via WS", balance=float(self.market_state.available_balance))
    ...
    async def _send_ws_command(self, op: str, args: list[Any], timeout: int = 10) -> dict | None:
    ...
    async def api_call_with_retry(self, api_method: Callable, *args, **kwargs) -> dict | None:
    ...
    async def get_symbol_info(self) -> bool:
        ...
        if response and response.get('retCode') == 0:
            ...
            if instruments:
                ...
                self.log.info("📊 Symbol info loaded successfully", symbol=self.config.SYMBOL)
    ...
    async def test_credentials(self) -> bool:
        ...
        self.log.critical("❌ API credentials validation failed. Check API key/secret and permissions.")
                ...
                for coin in balance.get('coin', []):
                    if coin.get('coin') == 'USDT':
                        ...
                        if available_to_withdraw:
                            self.market_state.available_balance = Decimal(str(available_to_withdraw))
                        else:
                            self.market_state.available_balance = Decimal('0')
                        self.market_state.last_balance_update = time.time()
                        self.log.debug("Wallet balance updated via HTTP", balance=float(self.market_state.available_balance))
                        return True
    ...
    async def place_order(self, side: str, order_type: str, qty: Decimal, price: Decimal | None = None, post_only: bool = False) -> dict | None:
        ...
        try:
            ...
            if quantized_qty <= 0 or quantized_qty < self.symbol_info.min_qty:
                self.log.warning("Invalid quantity rejected",
            ...
            if quantized_qty > self.symbol_info.max_qty:
                self.log.warning("Quantity exceeds maximum allowed by exchange",
            ...
            if order_type == "Limit":
                ...
                if quantized_price <= 0 or quantized_price < self.symbol_info.min_price:
                    self.log.warning("Price below minimum allowed by exchange rejected",
                ...
                if order_value < self.symbol_info.min_order_value:
                    self.log.warning("Order value below minimum required by exchange",
            ...
            if response and response.get('retCode') == 0:
                ...
                self.log.info("🎯 Order placed successfully via WS",
                          order_id=order_id,
                          side=side,
                          quantity=float(quantized_qty),
            ...
            if http_response and http_response.get('retCode') == 0:
                ...
                self.log.info("🎯 Order placed successfully via HTTP (WS fallback)",
                          order_id=order_id,
                          side=side,
                          quantity=float(quantized_qty),
                          price=float(price) if price else "Market",
            ...
            self.log.error("Order placement failed via both WS and HTTP.", response=response, http_response=http_response)
            return None
    ...
    async def amend_order(self, order_id: str | None = None, client_order_id: str | None = None, new_price: Decimal | None = None, new_qty: Decimal | None = None) -> bool:
        """Amends an existing order, prioritizing WS then HTTP."""
        if not order_id and not client_order_id:
            self.log.error("Either order_id or client_order_id must be provided for amendment.")
            return False
        ...
        if response and response.get('retCode') == 0:
            self.log.info("✏️ Order amended successfully via WS", order_id=order_id, new_price=new_price, new_qty=new_qty)
        ...
        if http_response and http_response.get('retCode') == 0:
            self.log.info("✏️ Order amended successfully via HTTP (WS fallback)", order_id=order_id, new_price=new_price, new_qty=new_qty)
            if order_id in self.market_state.open_orders:
                if new_price is not None: self.market_state.open_orders[order_id]['price'] = new_price
                if new_qty is not None: self.market_state.open_orders[order_id]['qty'] = new_qty
        ...
        self.log.error("Order amendment failed via both WS and HTTP.", order_id=order_id, response=response, http_response=http_response)
        return False
    ...
    async def cancel_order(self, order_id: str | None = None, client_order_id: str | None = None) -> bool:
        ...
        if not order_id and not client_order_id:
            self.log.error("Either order_id or client_order_id must be provided for cancellation.")
            return False
        ...
        http_response = await self.api_call_with_retry(self.http.cancel_order, **cancel_params)

        if http_response and http_response.get('retCode') == 0:
            self.log.info("🗑️ Order cancelled successfully via HTTP (WS fallback)", order_id=order_id)
            self.market_state.open_orders.pop(order_id, None)
            self.session_stats.orders_cancelled += 1
            return True
        ...
        self.log.error("Order cancellation failed via both WS and HTTP.", order_id=order_id, response=response, http_response=http_response)
        return False
    ...
    async def cancel_all_orders(self) -> bool:
        ...
        if http_response and http_response.get('retCode') == 0:
            order_count = len(self.market_state.open_orders)
            self.log.info("🧹 All orders cancelled via HTTP (WS fallback)", count=order_count)
            self.market_state.open_orders.clear()
            self.session_stats.orders_cancelled += order_count
            self.send_toast(f"🧹 {order_count} orders cancelled (HTTP)", "orange", "white")
            return True

        self.log.error("Cancellation of all orders failed via both WS and HTTP.", response=response, http_response=http_response)
        return False
