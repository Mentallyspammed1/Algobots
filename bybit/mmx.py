...
class EnhancedBybitClient:
    ...
    async def _subscribe_private_topics(self):
        ...
        if self.private_ws:
            try:
                ...
                self.log.info(f"Subscribed to private streams (order, position, wallet, execution)")
            except Exception as e:
    ...
    def _process_execution(self, exec_data: Dict[str, Any]):
        ...
        if not order_details:
            self.log.warning(f"Execution received for an untracked or already closed order.", order_id=order_id)
        ...
        self.log.info(f"Trade executed",
                      order_id=order_id,
                      side=side,
                ...
                for order_data in msg["data"]:
                    ...
                    if order_status == "Filled":
                        if order_id in self.market_state.open_orders:
                            ...
                            self.log.info(f"Order fully filled and closed.", order_id=order_id)
    ...
    async def amend_order(self, order_id: Optional[str] = None, client_order_id: Optional[str] = None, new_price: Optional[Decimal] = None, new_qty: Optional[Decimal] = None) -> bool:
    ...
    async def cancel_all_orders(self) -> bool:
        ...
        self.log.error(f"Cancellation of all orders failed via both WS and HTTP.", response=response, http_response=http_response)
        return False