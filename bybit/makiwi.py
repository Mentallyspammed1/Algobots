...
REBALANCE_PRICE_OFFSET_PERCENTAGE = Decimal(str(config.get("REBALANCE_PRICE_OFFSET_PERCENTAGE", "0")))
    ...
    try:
        ...
        if topic == "order":
                ...
                if o.get("orderStatus") in ("Filled", "Canceled", "Deactivated"):
                    if oid in ws_state["open_orders"]:
                        ...
                        if order_details and o["orderStatus"] == "Filled":
                            ...
                            filled_price = Decimal(o.get('avgPrice', o.get('price', '0')))
                            filled_qty = Decimal(o.get('qty', '0'))
                            side = o.get('side', 'N/A')
                            filled_value = filled_price * filled_qty
                            logger.info(f"Order filled: {side} {filled_qty} @ {filled_price:.4f} (Value: {filled_value:.2f})")
                            ...
                            if side == 'Buy' and 'Long' in positions:
                                ...
                                if position_info['size'] > 0:
                                    ...
                                    pnl = (position_info['avg_price'] - filled_price) * filled_qty
                                ...
                                if position_info['size'] > 0:
                                    ...
                                    pnl = (filled_price - position_info['avg_price']) * filled_qty
                                    session_stats['total_pnl'] += pnl
            ...
            for w in msg["data"]:
                ...
                if w.get("coin") == 'USDT':
                    ws_state['available_balance'] = Decimal(w.get('availableBalance', '0'))
                    ws_state['last_balance_update'] = time.time()
                    logger.debug(f"WS Wallet Update: Available Balance={ws_state['available_balance']:.2f}")
...
class BybitClient:
    ...
    async def _api(self, api_method, *args, **kwargs):
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                ...
                if ret_code in [10001, 10006, 30034, 30035, 10018]: # 10018: order not found, retry if needed.
    ...
    async def get_symbol_info(self) -> bool:
        ...
        if response and response.get('retCode') == 0:
            ...
            if instruments:
                ...
                logger.info(f"{Fore.CYAN}Symbol Info: {SYMBOL} | Price Precision: {symbol_info['price_precision']} | Qty Precision: {symbol_info['qty_precision']}{NC}")
                return True
        ...
        if response and response.get('retCode') == 0:
            for balance in response.get('result', {}).get('list', []):
                if balance.get('coin') == 'USDT':
                    ws_state['available_balance'] = Decimal(balance.get('availableToWithdraw', '0'))
                    ws_state['last_balance_update'] = time.time()
                    logger.info(f"{Fore.CYAN}Updated wallet balance: {ws_state['available_balance']:.2f} USDT{NC}")
                    return True
    ...
    async def get_open_orders(self):
        ...
        if response and response.get('retCode') == 0:
            ...
            ws_state['open_orders'] = {o.get('orderId'): {
                "client_order_id": o.get("orderLinkId", "N/A"),
                "symbol": o.get("symbol"),
                "side": o.get("side"),
                "price": Decimal(o.get("price", "0")),
    ...
    async def place_order(self, side: str, order_type: str, qty: Decimal, price: Optional[Decimal] = None):
        ...
        if order_type == "Limit":
            ...
            if order_value < symbol_info["min_order_value"]:
                logger.warning(f"{Fore.YELLOW}Skipping order due to minimum value constraint. Value: {order_value:.2f}, Min: {symbol_info['min_order_value']}{NC}")
                return None

        response = await self._api(self.http.place_order, **order_params)
        if response and response.get('retCode') == 0:
            session_stats["orders_placed"] += 1
            logger.info(f"{Fore.GREEN}Placed {order_type} {side} order: {qty} @ {price}{NC}")
    ...
    async def cancel_all_orders(self):
        ...
        response = await self._api(self.http.cancel_all_orders, category=CATEGORY, symbol=SYMBOL)
        if response and response.get('retCode') == 0:
    ...
    async def set_leverage(self, leverage: int = 1):
        ...
        if response and response.get('retCode') == 0:
            ...
            return True
        logger.error(f"{Fore.RED}Failed to set leverage. Error: {response.get('retMsg') if response else 'Unknown'}{NC}")
        return False
...
class MakiwiStrategy:
    ...
    async def startup_sequence(self):
        ...
        if not ws_state['available_balance']:
            logger.error(f"{RED}Available balance is zero. Cannot start trading. Exiting.{NC}")
            sys.exit(1)
    ...
    async def dashboard_updater(self):
        ...
        while self.running and not _SHUTDOWN_REQUESTED:
            if time.time() - self.last_dashboard_update > DASHBOARD_REFRESH_INTERVAL:
                clear_screen()
                print_neon_header("MMXCEL Bybit Market-Maker Dashboard", color=UNDERLINE)
                print_neon_separator()
                print(format_metric("Status", BOT_STATE, label_color=WHITE, value_color=GREEN if BOT_STATE == "RUNNING" else YELLOW))
                print(format_metric("Symbol", SYMBOL, label_color=WHITE, value_color=CYAN))
                print(format_metric("Mid Price", ws_state['mid_price'], label_color=WHITE, value_precision=_calculate_decimal_precision(symbol_info['price_precision'])))
                print(format_metric("Best Bid", ws_state['best_bid'], label_color=WHITE, value_precision=_calculate_decimal_precision(symbol_info['price_precision'])))
                ...
                print(format_metric("Total Orders Placed", session_stats['orders_placed'], label_color=WHITE, value_color=CYAN))
        ...
        while self.running and not _SHUTDOWN_REQUESTED:
            try:
                ...
                if time.time() - ws_state['last_update_time'] > MAX_DATA_AGE_SECONDS:
                    ...
                    logger.warning(f"{Fore.YELLOW}Market data is stale! Age: {time.time() - ws_state['last_update_time']:.1f}s. Cancelling all orders.{NC}")
                    await self.client.cancel_all_orders()
                ...
                if mid_price == Decimal("0"):
                    logger.warning(f"{Fore.YELLOW}Mid price is zero, waiting for WebSocket data.{NC}")
                ...
                if position_imbalance >= REBALANCE_THRESHOLD_QTY and not self.is_rebalancing:
                    ...
                    logger.warning(f"{Fore.MAGENTA}Position imbalance detected ({position_imbalance}). Initiating rebalance.{NC}")
...
async def shutdown(loop, signame):
    ...
    print_neon_header(f"Shutdown signal received ({signame}). Starting graceful shutdown...", color=RED)
