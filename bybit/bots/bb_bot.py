...
class BybitTradingBot:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt >= max_retries:
                    logger.error(f"HTTP call {getattr(method, '__name__', str(method))} failed after {attempt} attempts: {e}", exc_info=True)
                    return None
        ...
        try:
            for symbol in symbols:
                ...
                if response and response.get('retCode') == 0:
                    ...
                    logger.error(f"Failed to fetch instrument info for {symbol}: {response.get('retMsg') if response else 'No response'}")
        except Exception as e:
    ...
    async def get_market_data(self, symbol: str) -> dict | None:
        ...
        try:
            orderbook_resp, ticker_resp = await asyncio.gather(
                self._http_call(self.session.get_orderbook, category=self.category, symbol=symbol),
                self._http_call(self.session.get_tickers, category=self.category, symbol=symbol)
            )
    ...
    async def calculate_position_size(self, symbol: str, capital_percentage: float, price: Decimal, account_info: dict) -> Decimal:
        ...
        try:
                ...
                if isinstance(coins, list):
                    for c in coins:
                        if c.get('coin') in ('USDT', 'USD'):
                            available_balance_usd = to_decimal(c.get('availableToWithdraw', '0'))
                            break
                elif isinstance(coins, dict) and coins.get('coin') in ('USDT', 'USD'):
                    available_balance_usd = to_decimal(coins.get('availableToWithdraw', '0'))
            ...
            logger.info(f"Calculated position size for {symbol}: {rounded_qty} (Target Capital: {target_capital.quantize(Decimal('0.01'))} USDT, Price: {price})")
            return rounded_qty
    ...
    async def place_order(self, symbol: str, side: str, order_type: str, qty: Decimal, price: Decimal | None = None, stop_loss_price: Decimal | None = None, take_profit_price: Decimal | None = None, trigger_by: str = "LastPrice", time_in_force: str = "GTC", **kwargs) -> dict | None:
        ...
        current_position = to_decimal(self.ws_manager.positions.get(symbol, {}).get('size', '0'))
        ...
        if stop_loss_price:
            stop_loss_price = self._round_to_tick_size(symbol, to_decimal(stop_loss_price))
        if take_profit_price:
            take_profit_price = self._round_to_tick_size(symbol, to_decimal(take_profit_price))

        # Enforce min/max price if available
        si = self.symbol_info.get(symbol, {})
        min_p, max_p = si.get("minPrice", Decimal("0")), si.get("maxPrice", Decimal("0"))
    ...
    async def log_current_pnl(self):
        ...
        for symbol, position_data in list(self.ws_manager.positions.items()):
            ...
            if unrealized_pnl != Decimal('0'):
                ...
                logger.info(f"Unrealized PnL for {symbol} ({position_data.get('side')} {position_data.get('size')}): {unrealized_pnl}")
                has_pnl = True
        if has_pnl:
            ...
            while True:
                ...
                for symbol in symbols:
                    ...
                    if ws_md and ws_md.get("orderbook") and ws_md.get("ticker"):
                        ts = ws_md.get("timestamp") or ws_md.get("orderbook", {}).get("ts") or 0
                        try:
                            now_ms = int(time.time() * 1000)
                            if ts and (now_ms - int(ts)) < self.data_freshness_ms:
