            ...
            for attempt, delay in enumerate(delays):
                try:
                    ...
                    if attempt == len(delays) - 1:
                        logger.error(f"Function {func.__name__} failed after {attempt + 1} attempts: {e}", exc_info=True)
                        raise
                    logger.warning(f"Attempt {attempt + 1}/{len(delays)} failed for {func.__name__}: {e}. Retrying in {delay:.2f}s...")
...
def _dec(value: Any, default: Decimal = Decimal(0)) -> Decimal:
    ...
    try:
        ...
        logger.debug(f"Decimal conversion failed for value: '{value}' (Type: {type(value)}): {e}", exc_info=True)
        return default
...
class BotConfiguration:
    ...
    def __post_init__(self):
        ...
        if self.ws_liveness_timeout < self.ws_ping_interval * 2:
            logger.warning("ws_liveness_timeout should ideally be at least twice ws_ping_interval.")

        if self.paper_trading:
            logger.warning("PAPER TRADING MODE IS ENABLED. NO REAL ORDERS WILL BE PLACED.")
...
def setup_logging(log_level: str, log_queue: queue.Queue) -> QueueListener:
    ...
    class CustomJsonFormatter(jsonlogger.JsonFormatter):
        def add_fields(self, log_record, record, message_dict):
            ...
            log_record['timestamp'] = datetime.utcnow().isoformat() + "Z" # Consistent UTC
...
config = BotConfiguration(
    ...
    max_reconnect_attempts=_parse_env_int("MAX_RECONNECT_ATTEMPTS", 5), # IMPROVEMENT #1
...
if config.enable_metrics:
    metrics = {
        'total_trades': prometheus_client.Counter('bot_total_trades', 'Total number of trades initiated', ['mode']), # IMPROVEMENT #13
        'successful_trades': prometheus_client.Counter('bot_successful_trades', 'Number of successful trades', ['mode']), # IMPROVEMENT #13
        'failed_trades': prometheus_client.Counter('bot_failed_trades', 'Number of failed trades', ['mode']), # IMPROVEMENT #13
        'api_calls': prometheus_client.Counter('bot_api_calls', 'Total API calls made', ['endpoint']),
        'ws_messages': prometheus_client.Counter('bot_ws_messages', 'WebSocket messages received', ['channel']),
        'order_latency': prometheus_client.Histogram('bot_order_latency_seconds', 'Order execution latency in seconds'),
...
class EnhancedPerformanceMetrics:
    ...
    start_time: datetime = field(default_factory=datetime.utcnow) # IMPROVEMENT #11: UTC
    last_update: datetime = field(default_factory=datetime.utcnow) # IMPROVEMENT #11: UTC
    ...
    def add_trade(self, trade: Dict):
        ...
        if pnl > 0:
            ...
            self.avg_win = (self.avg_win * (self.successful_trades - 1) + pnl) / self.successful_trades if self.successful_trades > 0 else pnl
        else:
            self.failed_trades += 1
            self.avg_loss = (self.avg_loss * (self.failed_trades - 1) + abs(pnl)) / self.failed_trades if self.failed_trades > 0 else abs(pnl)
...
class Order:
    ...
    client_order_id: Optional[str] = field(default_factory=lambda: f"bot-{int(time.time() * 1000)}_{os.getpid()}") # Add PID for uniqueness
    created_at: datetime = field(default_factory=datetime.utcnow) # IMPROVEMENT #11: UTC
...
class RiskManager:
    ...
    def check_order_risk(self, order: Order, account_info: Dict, symbol_info: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if not info:
            return False, f"Symbol information not available for {order.symbol}. Cannot validate order."
        ...
        if order.qty % qty_step != 0:
            return False, f"Order quantity {order.qty} for {order.symbol} is not a multiple of qtyStep {qty_step}."
        if order.qty < min_qty:
            return False, f"Order quantity {order.qty} for {order.symbol} is below minimum {min_qty}."
        ...
        if not order.reduce_only and position_value > 0:
            ...
            if required_margin > available_balance * Decimal("0.8"): # Max 80% of available balance
    ...
    def get_position_size(self, account_balance: Decimal, risk_per_trade: Decimal = Decimal("0.02")) -> Decimal:
        ...
        calculated_size = account_balance * risk_per_trade * self.max_leverage # Max leverage can be used for max position size calc
...
class CacheManager(metaclass=SingletonMeta):
    ...
    def __init__(self):
        ...
        if config.redis_host and config.redis_port:
            try:
                ...
                logger.warning(f"Redis not available at {config.redis_host}:{config.redis_port}: {e}. Falling back to in-memory cache.")
            except Exception as e:
                logger.error(f"Failed to initialize Redis client: {e}. Falling back to in-memory cache.")
    ...
    async def get(self, key: str, default: Any = None) -> Any:
        ...
        if self.enabled and self.redis_client:
            ...
            if key in self.cache_timestamps:
                ...
                if (datetime.utcnow() - self.cache_timestamps[key]).total_seconds() > 300:
    ...
    async def set(self, key: str, value: Any, expire: int = 300):
        ...
        if self.enabled and self.redis_client:
            try:
                await asyncio.to_thread(self.redis_client.setex, key, expire, json.dumps(value))
            except Exception as e:
    ...
    async def _cache_maintenance(self, stop_event: threading.Event):
        ...
        while not stop_event.is_set():
            ...
            keys_to_delete = [
                ...
                if (now - timestamp).total_seconds() > 3600 # 1 hour expiry for background cleanup
            ]
    ...
    def __init__(self):
        ...
        if config.enable_encryption:
            ...
            if not key_str:
                ...
                logger.warning(f"ENCRYPTION_KEY environment variable not set. Generated a new temporary key: {key.decode()}. For production, set this environment variable for persistent encryption.")
            else:
                try:
                    key = key_str.encode()
                    self.fernet = Fernet(key)
                except Exception as e:
                    logger.error(f"Failed to initialize Fernet with provided ENCRYPTION_KEY: {e}. Encryption disabled.", exc_info=True)
                    self.fernet = None
            if self.fernet:
...
class EnhancedWebSocketManager:
    ...
    def __init__(self, api_key: str, api_secret: str, testnet: bool, paper_trading: bool):
    ...
    async def initialize(self):
        ...
        if not self._paper_trading:
            self.ws_private = WebSocket(
                testnet=self._testnet, channel_type="private",
    ...
    async def _liveness_check_loop(self):
        ...
        while self._is_running.is_set():
            await self._is_running.wait_for(config.ws_liveness_timeout) # Wait responsive to shutdown
            if not self._is_running.is_set(): # Check again after wait_for for immediate shutdown
                break

            if time.monotonic() - self._last_message_time > config.ws_liveness_timeout:
                logger.warning("No WebSocket message received recently. Triggering reconnect.")
    ...
    async def _heartbeat_loop(self):
        ...
        while self._is_running.is_set():
            try:
                ...
                await self._is_running.wait_for(config.ws_ping_interval) # Wait responsive to shutdown
    ...
    @advanced_retry(max_retries=config.max_reconnect_attempts, strategy=RetryStrategy.EXPONENTIAL)
    async def reconnect(self):
        ...
        async with self._reconnect_lock:
            ...
            current_subscriptions = list(self._subscriptions) # Copy to avoid modification during iteration
            ...
            for sub_key in current_subscriptions:
                ...
                await self.subscribe(channel, symbol) # This will add them back to _subscriptions
    ...
    async def subscribe(self, channel: str, symbol: Optional[str] = None):
        ...
        try:
            if channel == "orderbook":
                ws_instance.orderbook_stream(
                    depth=50, symbol=symbol, callback=callback
                )
            elif channel == "trade":
                ws_instance.trade_stream(
                    symbol=symbol, callback=callback
                )
            elif channel == "ticker":
                ws_instance.ticker_stream(
                    symbol=symbol, callback=callback
                )
        ...
        def callback(message: Dict):
            ...
            try:
                ...
                if "ts" in message:
                    ...
                    send_time = datetime.fromtimestamp(message["ts"] / 1000, tz=None) # Assume UTC from exchange
    ...
    def _handle_orderbook(self, message: Dict, symbol: str):
        ...
        if "U" in data and _dec(data["U"]) > last_update_id:
            logger.warning(f"Orderbook update for {symbol} out of sequence. Last: {last_update_id}, Prev: {data['U']}, Current: {current_update_id}. Requesting new snapshot.")
        ...
        self.market_data[symbol]["last_update"] = datetime.utcnow() # IMPROVEMENT #11: UTC
    ...
    def _handle_ticker(self, message: Dict, symbol: str):
        ...
        data = message.get("data", {})
        if not data or not symbol: return
        ...
        for exec_data in message.get("data", []):
            ...
            if config.enable_metrics:
                if "T" in exec_data: # Timestamp of execution in milliseconds
                    exec_time = datetime.fromtimestamp(exec_data["T"] / 1000, tz=None) # Assume UTC
                    latency = (datetime.utcnow() - exec_time).total_seconds() # IMPROVEMENT #11: UTC
...
class AdvancedBybitTradingBot:
    ...
    def __init__(self, config: BotConfiguration):
        ...
        self._api_executor = ThreadPoolExecutor(max_workers=config.rate_limit_per_second * 2) 
        ...
        self._emergency_stop = threading.Event() # IMPROVEMENT #3: Use threading.Event for cross-thread shutdown
            ...
            if strategy_params_path.exists():
                with open(strategy_params_path, 'r') as f:
                    ...
                    if strategy.name in all_params:
                        ...
                        logger.info(f"Strategy '{strategy.name}' loaded parameters from {self.config.strategy_params_file}: {strategy.parameters}")
            else:
                logger.warning(f"Strategy params file not found at '{self.config.strategy_params_file}'. Using strategy defaults.")
        except (json.JSONDecodeError, KeyError, Exception) as e:
            logger.error(f"Error loading parameters for strategy '{strategy.name}' from {self.config.strategy_params_file}: {e}", exc_info=True)
    ...
    async def _api_call(self, func: Callable, *args, **kwargs):
        ...
        await asyncio.sleep(1 / self.config.rate_limit_per_second) # Simple rate limiting for demo
        ...
        try:
            ...
            result = await asyncio.wait_for(
                asyncio.get_event_loop().run_in_executor(self._api_executor, func, *args, **kwargs),
            ...
            logger.error(f"API call to {func.__name__} timed out after {self.config.api_call_timeout}s.")
    ...
    async def fetch_symbol_info(self, symbols: List[str], category: str = "linear"):
        ...
        try:
            ...
            if updated_info: # Only update if new info was actually fetched
                ...
                await self.cache.set(cache_key, self.symbol_info, expire=3600 * 24) # Cache for 24 hours
                logger.info(f"Refreshed symbol info for {len(self.symbol_info)} symbols.")
            else:
    ...
    async def place_order(self, order: Order) -> Optional[Order]:
        ...
        try:
            ...
            if order.qty <= 0: raise ValueError("Order quantity must be positive.")
            if order.order_type == OrderType.LIMIT and order.price is None: raise ValueError("Limit order requires a price.")
            
        except ValueError as e:
            logger.error(f"Pre-validation failed for order {order.symbol} {order.side.value} {order.qty}: {e}")
            self.performance.failed_trades += 1
            return None

        # Risk check
        account_info = await self.get_account_info()
        approved, reason = self.risk_manager.check_order_risk(order, account_info, self.symbol_info) # IMPROVEMENT #7
        if not approved:
        ...
        if self.config.paper_trading:
            ...
            if order.order_type == OrderType.MARKET:
                try:
                    current_price = _dec(self.ws_manager.market_data.get(order.symbol, {}).get('ticker', {}).get('last_price'))
                    if current_price == 0:
                        logger.error(f"PAPER TRADING: Cannot simulate market order fill for {order.symbol}, no current price.")
                        self.performance.failed_trades += 1
                        return None
                    ...
                    if order.side == OrderSide.BUY:
                        fill_price = fill_price * Decimal("1.0001") # Buy slightly higher
                    else:
                        fill_price = fill_price * Decimal("0.9999") # Sell slightly lower

                    cost = order.qty * fill_price
                    if order.side == OrderSide.BUY:
                        if self._paper_balance["USDT"]["availableToWithdraw"] < cost:
                            logger.warning(f"PAPER TRADING: Insufficient balance to simulate buy order for {order.symbol}.")
                            self.performance.failed_trades += 1
                            return None
                    ...
                    if pos.size == 0: # New position
                        ...
                        pos.size = order.qty
                    else: # Averaging or closing
                        if pos.side == order.side.value: # Same direction, average
                        ...
                        else: # Opposite direction, reduce or flip
                            if order.qty >= pos.size:
                                # Closing or flipping
                                pnl = (pos.mark_price - pos.entry_price) * pos.size * (1 if pos.side == "Buy" else -1)
                                pos.realized_pnl += pnl
                                self._paper_balance["USDT"]["walletBalance"] += pnl # Add realized PnL to balance
                                ...
                                # Proportional PnL
                                pnl = (pos.mark_price - pos.entry_price) * order.qty * (1 if pos.side == "Buy" else -1)
                                pos.realized_pnl += pnl
                    ...
                    self.ws_manager.positions[order.symbol] = pos # Also update WS manager's view
                    ...
                    self.performance.add_trade({
                        ...
                        'qty': order.qty,
                        'price': fill_price,
                        'pnl': pnl if 'pnl' in locals() else Decimal(0), # Only for closing trades
                        'volume': order.qty * fill_price
                    })
                    logger.info(f"PAPER TRADING: Market order {order.symbol} filled at {fill_price:.8f}. Current balance: {self._paper_balance['USDT']['availableToWithdraw']:.2f}")
                    return order
                except Exception as e:
                    logger.error(f"PAPER TRADING: Error simulating market order fill: {e}", exc_info=True)
                    self.performance.failed_trades += 1
                    return None
            else: # Limit orders in paper trading are currently just stored, not actively matched
                self._paper_orders[order.order_id] = order
                logger.info(f"PAPER TRADING: Limit order {order.symbol} placed (not yet filled).")
                return order
    ...
    async def cancel_order(self, order_id: str, symbol: str) -> bool:
        ...
        if self.config.paper_trading:
            if order_id in self._paper_orders:
                ...
                return True
            logger.warning(f"PAPER TRADING: Order {order_id} not found for cancellation.")
            return False
    ...
    async def execute_strategies(self, symbols: List[str]):
        ...
        filtered_market_data = {
            sym: data for sym, data in self.ws_manager.market_data.items()
        ...
        for name, strategy in self.strategies.items():
            ...
            try:
                ...
                orders_to_place = await strategy.analyze(filtered_market_data, account_info)
                
                # Place generated orders
                for order in orders_to_place:
                    if self._emergency_stop.is_set():
                        logger.info("Emergency stop received, abandoning further order placement.")
                        break
                    
                    # Check max open positions (a general bot-wide limit)
                    open_positions_count = len([p for p in self.ws_manager.positions.values() if p.size != Decimal(0)])
                    if open_positions_count >= self.config.max_open_positions:
                        logger.warning(f"Max positions ({self.config.max_open_positions}) reached. Skipping new order for {order.symbol}.")
    ...
    async def monitor_positions(self):
        ...
        positions_to_close = []
        for symbol, position in list(self.ws_manager.positions.items()): # Iterate over copy
    ...
    async def run(self, symbols: List[str], interval: int = 5):
        ...
        for symbol in symbols:
            ...
            await self.ws_manager.subscribe("trade", symbol) # Optional: if strategy needs trade stream
        ...
        if config.enable_metrics:
            try:
                ...
                logger.error(f"Failed to start Prometheus server (port 8000 likely in use): {e}")

        # Start periodic background tasks (IMPROVEMENT #5, #6)
        asyncio.create_task(self._periodic_task(3600, self.fetch_symbol_info, symbols)) # Refresh symbol info hourly
        asyncio.create_task(self._periodic_task(3600, self.save_performance_data)) # Save performance data hourly
        asyncio.create_task(self._periodic_task(300, self.cache._cache_maintenance, self._emergency_stop)) # Cache cleanup
        ...
        try:
            while not self._emergency_stop.is_set():
                ...
                if self.performance.total_trades % 5 == 0 or self.performance.total_trades == 0:
    ...
    async def cleanup(self):
        ...
        if self._emergency_stop.is_set(): # Only proceed if shutdown signal was received
        ...
        with suppress(asyncio.CancelledError): # Suppress CancelledError here as we expect it
    ...
    async def save_performance_data(self):
        ...
        try:
            data = {
                ...
                "timestamp": datetime.utcnow().isoformat() + "Z" # IMPROVEMENT #11: UTC
            }
            ...
            filename = output_dir / f"performance_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
            async with aiofiles.open(filename, 'w') as f:
                await f.write(json.dumps(data, indent=2, default=str)) # Use default=str for Decimal/datetime
...
class MarketMakerStrategy(StrategyInterface):
    ...
    async def analyze(self, market_data: Dict[str, Any], account_info: Dict[str, Any]) -> List[Order]:
        ...
        for order_id, order in list(self.pending_orders.items()): # Iterate over copy
            ...
            if (current_time - order.created_at).total_seconds() > self.parameters['order_timeout_seconds']:
                logger.info(f"MarketMaker: Order {order.order_id} for {order.symbol} timed out. Cancelling.")
        ...
        for symbol, data in market_data.items():
            if symbol not in self.bot.symbol_info:
                logger.debug(f"MarketMaker: Skipping {symbol}, no symbol info available.")
                continue
            ...
            if not ob or not ob["bids"] or not ob["asks"] or not ticker:
                logger.debug(f"MarketMaker: Skipping {symbol}, insufficient market data (orderbook/ticker missing).")
                continue
            ...
            if best_bid == 0 or best_ask == 0 or best_ask <= best_bid:
                logger.warning(f"MarketMaker: Invalid bid/ask prices for {symbol}. Bid: {best_bid}, Ask: {best_ask}. Skipping.")
                continue
            ...
            if order_qty == Decimal(0):
                logger.warning(f"MarketMaker: Calculated order quantity for {symbol} is zero after adjusting to qtyStep. Skipping.")
                continue
            ...
            has_buy_order = any(o.side == OrderSide.BUY for o in self.pending_orders.values() if o.symbol == symbol)
            has_sell_order = any(o.side == OrderSide.SELL for o in self.pending_orders.values() if o.symbol == symbol)
...
async def main():
    ...
    try:
        ...
        symbols = [s.strip() for s in symbols_str.split(',') if s.strip()] # Clean and filter empty
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.getLogger(__name__).critical(f"Unhandled exception outside asyncio.run: {e}", exc_info=True)
        sys.exit(1)
