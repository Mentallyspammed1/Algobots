...
def round_to_step(value: Decimal, step: Decimal, rounding=ROUND_DOWN) -> Decimal:
    ...
    try:
        ...
        return (value // step) * step if exp > 0 else value.quantize(step, rounding=rounding)
    except Exception:
...
def _parse_env_bool(var_name: str, default: bool = False) -> bool:
    ...
    return os.getenv(var_name, str(default)).strip().lower() in {"true", "1", "yes", "y", "on"}
...
def _parse_env_int(var_name: str, default: int = 0) -> int:
    ...
    try:
        ...
        logging.getLogger(__name__).warning(f"Invalid integer for {var_name}, using default: {default}")
        return default
...
def _dec(value: Any, default: Decimal = Decimal(0)) -> Decimal:
    ...
    try:
        ...
        logging.getLogger(__name__).debug(
            f"Decimal conversion failed for value: '{value}' (Type: {type(value)}): {e}", exc_info=True
        )
        return default
...
if config.enable_metrics:
    metrics = {
        ...
        'successful_trades': prometheus_client.Counter('bot_successful_trades', 'Number of profitable fills', ['mode']),
        'failed_trades': prometheus_client.Counter('bot_failed_trades', 'Number of losing fills', ['mode']),
...
class RiskManager:
    ...
    def check_order_risk(self, order: Order, account_info: dict, symbol_info: dict[str, Any], market_data: dict[str, Any]) -> tuple[bool, str]:
        ...
        if order.order_type == OrderType.MARKET and estimated_price is None:
            ...
            tick = md.get('ticker', {})
            estimated_price = _dec(tick.get('last_price')) or _dec(tick.get('bid')) or _dec(tick.get('ask'))
...
class CacheManager(metaclass=SingletonMeta):
    ...
    async def _cache_maintenance(self, stop_event: threading.Event):
        ...
        while not stop_event.is_set():
            ...
            now = datetime.utcnow()
            keys_to_delete = [
                k for k, timestamp in list(self.cache_timestamps.items())
        ...
        if config.enable_encryption:
            ...
            try:
                if not key_str:
                    ...
                    logger.warning(f"ENCRYPTION_KEY not set. Generated a temporary key: {key.decode()}. For production, set ENCRYPTION_KEY for persistence.")
                    self.fernet = Fernet(key)
                else:
                    self.fernet = Fernet(key_str.encode())
                logger.info("Encryption enabled.")
            except Exception as e:
                logger.error(f"Failed to initialize Fernet: {e}. Encryption disabled.", exc_info=True)
                self.fernet = None
        else:
...
# --- Async utilities ---
async def sleep_until_stop(stop_event: threading.Event, timeout: float, check_interval: float = 0.2):
    ...
    def _handle_execution(self, message: dict):
        for exec_data in message.get("data", []):
            try:
                ...
                pnl = _dec(exec_data.get('execValue', 0)) - fee  # rough placeholder if needed
                logger.info(f"Execution: {symbol} - Price: {exec_price}, Qty: {exec_qty}, Side: {side}, Fee: {fee}")
...
class AdvancedBybitTradingBot:
            ...
            if strategy_params_path.exists():
                with open(strategy_params_path) as f:
                    ...
                    if strategy.name in all_params:
                        ...
                        logger.info(f"Strategy '{strategy.name}' loaded parameters: {strategy.parameters}")
            else:
    ...
    async def place_order(self, order: Order) -> Order | None:
        ...
        approved, reason = self.risk_manager.check_order_risk(order, account_info, self.symbol_info, self.ws_manager.market_data)
        if not approved:
        ...
        if self.config.paper_trading:
            ...
            if order.order_type == OrderType.MARKET:
                try:
                    ...
                    current_price = _dec(ticker.get('last_price')) or _dec(ticker.get('ask')) or _dec(ticker.get('bid'))
                    if current_price == 0:
                    ...
                    logger.info(f"PAPER TRADING: Market order {order.symbol} filled at {fill_price:.8f}. Balance: {self._paper_balance['USDT']['availableToWithdraw']:.2f}")
                    return order
                except Exception as e:
    ...
    async def run(self, symbols: list[str], interval: int = 5):
        ...
        asyncio.create_task(self._periodic_task(300, self.cache._cache_maintenance, self._emergency_stop))
    ...
    async def analyze(self, market_data: dict[str, Any], account_info: dict[str, Any]) -> list[Order]:
        ...
        for symbol, data in market_data.items():
            ...
            if order_qty == Decimal(0):
                logger.warning(f"MarketMaker: Calculated order quantity for {symbol} is zero after step adjust. Skipping.")
                continue

            # Check if existing quotes need re-quote (drift > threshold)
            def drift_bps(old: Decimal, new: Decimal) -> Decimal:
                if new == 0: return Decimal(0)
                return abs((old - new) / new) * Decimal(10000)
            requote_bps = Decimal(str(self.parameters.get("requote_bps", 5)))

            # Identify existing buy/sell
            existing_buys = [o for o in self.pending_orders.values() if o.symbol == symbol and o.side == OrderSide.BUY]
            existing_sells = [o for o in self.pending_orders.values() if o.symbol == symbol and o.side == OrderSide.SELL]
