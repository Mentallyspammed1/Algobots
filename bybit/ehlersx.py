...
def calculate_ema(prices: list[float], period: int) -> list[float]:
    ...
    for i in range(period, len(prices)):
        ema_values[i] = (prices[i] * smoothing_factor) + (
            ema_values[i - 1] * (1 - smoothing_factor)
        )
...
async def ehlers_ma_cross_strategy(
    market_data: dict,
    account_info: dict,
    http_client: HTTP,
    bot_instance: Any,
    ...
    for symbol in symbols_to_trade:  # Iterate over each symbol
        ...
        closing_prices = [
            float(kline[4]) for kline in klines_data_response["result"]["list"]
        ]
