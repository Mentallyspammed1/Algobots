...
class TestEhlersSupertrendStrategy(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.min_required_bars = max(self.strategy.ehlers_period, self.strategy.supertrend_period, self.strategy.sma_period) + 2
        if len(self.df) < self.min_required_bars:
            self.df = create_sample_kline_df(rows=self.min_required_bars + 10) # Ensure enough rows

    @patch('indicators.calculate_ehlers_fisher_strategy')
    @patch('indicators.calculate_supertrend')
    @patch('indicators.calculate_sma')
    def test_calculate_indicators(self, mock_calculate_sma, mock_calculate_supertrend, mock_calculate_ehlers_fisher_strategy):
        ...
        mock_calculate_sma.return_value = pd.Series([Decimal('100')] * len(self.df), index=self.df.index)

        result_df = self.strategy._calculate_indicators(self.df.copy())

        mock_calculate_ehlers_fisher_strategy.assert_called_once_with(self.df.copy(), length=self.strategy.ehlers_period)
        mock_calculate_supertrend.assert_called_once_with(self.df.copy(), period=self.strategy.supertrend_period, multiplier=self.strategy.supertrend_multiplier)
        mock_calculate_sma.assert_called_once_with(self.df.copy(), length=self.strategy.sma_period)
        self.assertIn('sma', result_df.columns)

    @patch('strategies.ehlerssupertrendstrategy.EhlersSupertrendStrategy._calculate_indicators')
    def test_generate_signals_buy(self, mock_calculate_indicators):
        ...
        mock_df['supertrend_direction'] = [Decimal('-1')] * (len(mock_df) - 2) + [Decimal('1'), Decimal('1')]
        mock_df['ehlers_fisher'] = [Decimal('0')] * (len(mock_df) - 2) + [Decimal('0.5'), Decimal('1.0')]
        mock_df['ehlers_signal'] = [Decimal('0')] * (len(mock_df) - 2) + [Decimal('0.6'), Decimal('0.5')]
        mock_df['sma'] = [Decimal('90')] * len(mock_df)
        mock_df['close'] = [Decimal('100')] * (len(mock_df) - 2) + [Decimal('100'), Decimal('100')]
    ...
    def test_generate_exit_signals_long_position(self, mock_calculate_indicators):
        ...
        mock_df = self.df.copy()
        mock_df['supertrend_direction'] = [Decimal('1')] * (len(mock_df) - 2) + [Decimal('-1'), Decimal('-1')]
        mock_df['ehlers_fisher'] = [Decimal('0')] * (len(mock_df) - 2) + [Decimal('0.5'), Decimal('0.0')]
        mock_df['ehlers_signal'] = [Decimal('0')] * (len(mock_df) - 2) + [Decimal('0.0'), Decimal('0.5')]
