...
class TestOrderBook(unittest.TestCase):
    ...
    def test_handle_snapshot(self):
        ...
        self.assertEqual(self.order_book.bids, {Decimal('100.0'): Decimal('10'), Decimal('99.9'): Decimal('5')})
        self.assertEqual(self.order_book.asks, {Decimal('100.1'): Decimal('8'), Decimal('100.2'): Decimal('7')})
    ...
    def test_apply_delta_add_new(self):
        ...
        self.order_book.apply_delta(delta_data)
        self.assertEqual(self.order_book.bids, {Decimal('100.0'): Decimal('10'), Decimal('99.8'): Decimal('7')})
        self.assertEqual(self.order_book.asks, {Decimal('100.1'): Decimal('8')})
        self.assertEqual(self.order_book.last_seq, 101)
    ...
    def test_get_imbalance(self):
        ...
        self.order_book.asks = {Decimal('101'): Decimal('8'), Decimal('102'): Decimal('7')}
...
class TestCalculateOrderQuantity(unittest.TestCase):
    ...
    def test_basic_calculation(self):
        qty = calculate_order_quantity(self.usdt_amount, self.current_price, self.min_qty, self.qty_step, self.min_order_value, self.mock_logger)
    ...
    def test_below_min_qty_adjust_up(self):
        ...
        qty = calculate_order_quantity(usdt_amount, self.current_price, self.min_qty, self.qty_step, self.min_order_value, self.mock_logger)
        self.assertEqual(qty, self.min_qty) # Should adjust to min_qty
        self.mock_logger.warning.assert_called_with(f"{utils_logger.NEON_YELLOW}Initial quantity {Decimal('0.0000002'):.8f} was below min_qty {self.min_qty}. Adjusting to min_qty.{utils_logger.RESET_ALL_STYLE}")

    def test_below_min_order_value_adjust_up(self):
        usdt_amount = Decimal('5') # Results in 0.0001 BTC, value 5, below min_order_value 10
        qty = calculate_order_quantity(usdt_amount, self.current_price, self.min_qty, self.qty_step, self.min_order_value, self.mock_logger)
        ...
        self.mock_logger.warning.assert_called_with(f"{utils_logger.NEON_YELLOW}Order value {Decimal('5.0000'):.4f} is below min_order_value {self.min_order_value}. Recalculating quantity.{utils_logger.RESET_ALL_STYLE}")
        self.mock_logger.info.assert_called_with(f"{utils_logger.NEON_BLUE}Quantity adjusted to {Decimal('0.0002'):.8f} to meet minimum order value.{utils_logger.RESET_ALL_STYLE}")

    def test_zero_usdt_amount(self):
        qty = calculate_order_quantity(Decimal('0'), self.current_price, self.min_qty, self.qty_step, self.min_order_value, self.mock_logger)
        self.assertEqual(qty, Decimal('0'))
        self.mock_logger.error.assert_called_with(f"{utils_logger.NEON_RED}USDT amount and current price must be positive for order calculation.{utils_logger.RESET_ALL_STYLE}")

    def test_zero_current_price(self):
        qty = calculate_order_quantity(self.usdt_amount, Decimal('0'), self.min_qty, self.qty_step, self.min_order_value, self.mock_logger)
        self.assertEqual(qty, Decimal('0'))
        self.mock_logger.error.assert_called_with(f"{utils_logger.NEON_RED}USDT amount and current price must be positive for order calculation.{utils_logger.RESET_ALL_STYLE}")
    ...
    def test_final_qty_zero_after_adjustments(self):
        ...
        qty = calculate_order_quantity(Decimal('1'), Decimal('1'), Decimal('1'), Decimal('1'), Decimal('100000'), self.mock_logger)
        self.assertEqual(qty, Decimal('0'))
        self.mock_logger.error.assert_called_with(f"{utils_logger.NEON_RED}Final calculated quantity is zero or negative. Aborting order calculation.{utils_logger.RESET_ALL_STYLE}")
...
class TestPrecisionFunctions(unittest.TestCase):
    ...
    def test_get_price_precision_default_fallback(self):
        ...
        self.assertEqual(get_price_precision(market_info, self.mock_logger), 4)
        self.mock_logger.warning.assert_called_with("Could not determine price precision for UNKNOWN from market_info. Using default: 4.")

    def test_get_min_tick_size_from_precision_float(self):
        market_info = {'precision': {'price': 0.0001}, 'symbol': 'BTCUSDT'}
        self.assertEqual(get_min_tick_size(market_info, self.mock_logger), Decimal('0.0001'))

    def test_get_min_tick_size_from_precision_int(self):
        market_info = {'precision': {'price': 3}, 'symbol': 'ETHUSDT'}
        self.assertEqual(get_min_tick_size(market_info, self.mock_logger), Decimal('0.001'))

    def test_get_min_tick_size_from_limits_min(self):
        market_info = {'limits': {'price': {'min': "0.005"}}, 'symbol': 'XRPUSDT'}
        self.assertEqual(get_min_tick_size(market_info, self.mock_logger), Decimal('0.005'))

    def test_get_min_tick_size_default_fallback(self):
        market_info = {'symbol': 'UNKNOWN'}
        self.assertEqual(get_min_tick_size(market_info, self.mock_logger), Decimal('0.0001'))
        self.mock_logger.warning.assert_called_with("Could not determine specific min_tick_size for UNKNOWN from market_info. Using fallback based on derived price precision (4 places): 0.0001")
    ...
    def test_get_timezone_from_env(self):
        with patch.dict(os.environ, {'TIMEZONE': 'Europe/London'}):
            ...
            with patch('utils._ActualZoneInfo') as MockZoneInfo:
                ...
                self.mock_logger.info.assert_any_call("TIMEZONE environment variable found: 'Europe/London'. Attempting to use it.")
    ...
    def test_get_timezone_default(self):
        with patch.dict(os.environ, {}, clear=True): # Ensure no TIMEZONE env var
            ...
            with patch('utils._ActualZoneInfo') as MockZoneInfo:
                ...
                self.mock_logger.info.assert_any_call("TIMEZONE environment variable not set. Using default timezone: 'America/Chicago'.")
