...
class ChandelierEhlersSuperTrendStrategy:
    ...
    1. Chandelier Exit - A volatility-based indicator used for setting trailing
       stop-loss levels
    ...
    - Buy signal: When price crosses above the SuperTrend and Chandelier Exit
      confirms the trend
    - Sell signal: When price crosses below the SuperTrend and Chandelier Exit
      confirms the trend
    """
    ...
    def get_account_balance(self) -> float:
        ...
        try:
            ...
            if account_info and account_info['retCode'] == 0:
                ...
                elif 'list' in account_info['result'] and \
                        isinstance(account_info['result']['list'], list):
                    for balance_entry in account_info['result']['list']:
                        if balance_entry.get('coin') == 'USDT':
                            return float(balance_entry.get('equity', 0.0))

                print("Warning: Could not find USDT balance in account info. "
                      "Returning 0.0.")
                return 0.0

            else:
                print(f"Error fetching account balance: "
                      f"{account_info.get('retMsg', 'Unknown error')}")
                return 0.0

        except AttributeError:
            print("Error: 'session' object does not have the expected method "
    ...
    def calculate_position_size(self, symbol: str, signal_type: str) -> float:
        ...
        if not hasattr(self, 'db_manager') or not hasattr(self.db_manager, 'get_trades'):
            print("Error: db_manager or its get_trades method not available. "
        ...
        capital_to_risk = balance * kelly_fraction * \
            self.config.get('KELLY_RISK_PER_TRADE_MULTIPLIER', 0.02)  # Use config value

        # Get current price for the symbol
        if not hasattr(self, 'current_prices') or symbol not in self.current_prices:
            print(f"Warning: Could not get valid current price for {symbol}. "
        ...
        if current_price <= 0:
            print(f"Warning: Current price for {symbol} is zero or negative ({current_price}). "
                  "Using fallback price from config.")
            current_price = self.config.get('fallback_price', 50000)  # Use config value for fallback price
