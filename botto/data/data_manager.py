    ...
    def get_historical_data(self, symbol: str, timeframe: str, limit: int) -> pd.DataFrame:
        ...
        try:
            ...
            if response['retCode'] != 0:
                logger.error(f"Error fetching kline data for {symbol}: {response.get('retMsg', 'Unknown error')}")
                return pd.DataFrame()
            ...
            try:
                ...
                logger.error(f"Error converting data types for {symbol}: {str(e)}")
                return pd.DataFrame()
            ...
            logger.error("API session object does not have 'get_kline' method or is not properly initialized.")
            return pd.DataFrame()
        except Exception as e:
            logger.error(f"An unexpected error occurred fetching historical data for {symbol}: {str(e)}")
            return pd.DataFrame()
...
class Trade:
    ...
    def __repr__(self):
        return f"Trade(symbol='{self.symbol}', entry={self.entry_price}, exit={self.exit_price}, pnl={self.pnl:.4f}, ts={self.timestamp.isoformat()})"

class DataManager:
    """Manages historical trade data in memory."""
    def __init__(self, max_trades_per_symbol=1000):
        self.trades: List[Trade] = []
        self.max_trades_per_symbol = max_trades_per_symbol # Limit the number of stored trades
    ...
    def add_trade(self, trade: Trade):
        ...
        # Consider alternative data structures or sorting only when needed if performance is an issue.
    ...
    def get_trades(self, symbol: str, limit: int) -> List[Trade]:
        ...
        Returns:
            A list of Trade objects. Returns empty list if no trades found or symbol is invalid.
        """
        if not symbol:
