...
class ChandelierExit:
    ...
    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['chandelier_long'] = df['high'].rolling(window=self.period).max() - self.multiplier * df['atr']
        df['chandelier_short'] = df['low'].rolling(window=self.period).min() + self.multiplier * df['atr']
