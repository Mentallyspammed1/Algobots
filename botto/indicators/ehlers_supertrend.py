...
class EhlersSuperTrend:
    ...
    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        alpha1 = (np.cos(2 * np.pi / self.period) + np.sin(2 * np.pi / self.period) - 1) / \
            np.cos(2 * np.pi / self.period)
        df['hp'] = 0.0

        for i in range(1, len(df)):
            if i == 1:
                df.at[df.index[i], 'hp'] = (0.5 * (1 + alpha1) *
                                            (df.at[df.index[i], 'typical_price'] -
                                             df.at[df.index[i-1], 'typical_price']))
            else:
                df.at[df.index[i], 'hp'] = (0.5 * (1 + alpha1) *
                                            (df.at[df.index[i], 'typical_price'] -
                                             df.at[df.index[i-1], 'typical_price']) +
                                            alpha1 * df.at[df.index[i-1], 'hp'])
        ...
        for i in range(2, len(df)):
            if i == 2:
                ...
                if pd.isna(hp_prev_sum):
                    hp_prev_sum = 0.0

                df.at[df.index[i], 'ss'] = (c1 * hp_prev_sum +
                                            c2 * df.at[df.index[i-1], 'ss'] +
                                            c3 * df.at[df.index[i-2], 'ss'])
            else:
                hp_prev_sum = (df.at[df.index[i], 'hp'] + df.at[df.index[i-1], 'hp']) / 2
                if pd.isna(hp_prev_sum):
                    hp_prev_sum = 0.0
        ...
        for i in range(self.period, len(df)):
            if df.at[df.index[i], 'typical_price'] > df.at[df.index[i-self.period], 'typical_price']:
                # Uptrend
                df.at[df.index[i], 'supertrend'] = (df.at[df.index[i], 'typical_price'] -
                                                    self.multiplier * df.at[df.index[i], 'atr'])
            else:
                # Downtrend
                df.at[df.index[i], 'supertrend'] = (df.at[df.index[i], 'typical_price'] +
                                                    self.multiplier * df.at[df.index[i], 'atr'])
