...
KELLY_RISK_PER_TRADE_MULTIPLIER = safe_get_float("KELLY_RISK_PER_TRADE_MULTIPLIER", 0.02) # Multiplier for Kelly fraction in risk calculation
...
def validate_config():
    ...
    if not API_KEY or not API_SECRET:
        logger.error("API Key and Secret are missing. Please set BYBIT_API_KEY and BYBIT_API_SECRET in your .env file.")
        raise ValueError("API credentials not set.")
    
    if not (0 < STOP_LOSS_PERCENT <= 1):
        logger.warning(f"STOP_LOSS_PERCENT ({STOP_LOSS_PERCENT}) is outside the typical range (0, 1]. Adjusting to nearest valid value.")
        # Optionally clamp or raise error
        
    if not (0 < TAKE_PROFIT_PERCENT <= 1):
        logger.warning(f"TAKE_PROFIT_PERCENT ({TAKE_PROFIT_PERCENT}) is outside the typical range (0, 1]. Adjusting to nearest valid value.")
        # Optionally clamp or raise error
