...
class Indicators:
    ...
    def cci(high: pd.Series, low: pd.Series, close: pd.Series, window: int = 20, constant: Decimal = Decimal('0.015')) -> pd.Series:
        ...
        mean_dev = typical_price.rolling(window=window).apply(lambda x: (x - x.mean()).abs().mean(), raw=True).apply(
...
class TradingAnalyzer:
    ...
    def _select_weight_set(self) -> Dict[str, float]:
        ...
        if self.atr_value > Decimal(str(self.cfg["atr_change_threshold"])):
            vol_mode = "high_volatility"
        else:
            vol_mode = "low_volatility"
    ...
    def _analyze_order_book_walls(self, order_book: Dict[str, Any]) -> Tuple[bool, bool, Dict[str, Decimal], Dict[str, Decimal]]:
        ...
        wall_threshold = avg_qty * Decimal(str(self.cfg.get("order_book_analysis", {}).get("wall_threshold_multiplier", 2.0)))
    ...
    def _interpret_and_color_indicator(self, name: str, value: Any) -> str:
        ...
        if name == "ema_alignment":
            ...
            line = f"  RSI               : {color}{value:.2f}{RESET}"
        elif name == "macd":
            ...
            current_price = self.df.close.iloc[-1]
            if current_price > value: color = NEON_GREEN
            elif current_price < value: color = NEON_RED
            line = f"  PSAR              : {color}{value:.5f}{RESET}"
        elif name == "cmf":
            if value > Decimal('0'): color = NEON_GREEN
            elif value < Decimal('0'): color = NEON_RED
            line = f"  CMF               : {color}{value:.4f}{RESET}"
        elif name == "ao":
            if value > Decimal('0'): color = NEON_GREEN
            elif value < Decimal('0'): color = NEON_RED
            line = f"  AO                : {color}{value:.5f}{RESET}"
        elif name == "vi":
            plus_vi, minus_vi = value.get('+VI'), value.get('-VI')
            if plus_vi > minus_vi: color = NEON_GREEN
            elif plus_vi < minus_vi: color = NEON_RED
    ...
    def analyze(self, price: Decimal, ts: str, order_book: Optional[Dict[str, Any]]):
        ...
        for key, val in cfg_prd.items():
            if key in indicator_data: indicator_data[key] = val
        indicator_data.update({
            'window': cfg_prd.get('atr', 14), 'span': cfg_prd.get('atr', 14), 'period': cfg_prd.get('momentum', 10),
            'fast': 12, 'slow': 26, 'signal': 9, 'k_period': cfg_prd.get('stoch_osc_k', 14),
            'd_period': cfg_prd.get('stoch_osc_d', 3), 'rsi_period': cfg_prd.get('stoch_rsi_period', 14),
            'short_period': cfg_prd.get('ao_short', 5), 'long_period': cfg_prd.get('ao_long', 34),
        })
        ...
        log_lines = [f"\n{'─'*20} ANALYSIS FOR {self.symbol} ({self.interval}) {'─'*20}",
                     ...
                     f" Price History: {self.df['close'].iloc[-3]:.2f} | {self.df['close'].iloc[-2]:.2f} | {self.df['close'].iloc[-1]:.2f}",
    ...
    def generate_trading_signal(self, current_price: Decimal) -> Tuple[Union[str, None], float, List[str], Dict[str, Decimal]]:
        ...
        for name, weight in self.weights.items():
            ...
            if name == "ema_alignment":
                if value > Decimal('0'): raw_score += weight_dec; conditions_met.append("EMA Alignment (Bullish)")
                elif value < Decimal('0'): raw_score -= weight_dec; conditions_met.append("EMA Alignment (Bearish)")
            elif name == "volume_confirmation":
                ...
                pass # These indicators are primarily for analysis logging or market condition detection

        # Order book walls add a confidence boost
        ob_walls = self.indicator_values.get("order_book_walls", {})
        if ob_walls.get("bullish"):
            raw_score += Decimal(str(self.config.get("order_book_support_confidence_boost", 0))); conditions_met.append("Order Book: Bullish Wall")
        ...
        signal = None
        min_threshold = Decimal(str(self.config["signal_score_threshold"]))
        ...
        if signal and self.atr_value > Decimal('0'):
            atr_multiple_sl = Decimal(str(self.config['stop_loss_multiple']))
            atr_multiple_tp = Decimal(str(self.config['take_profit_multiple']))
            if signal == "buy":
        ...
        max_possible_score = sum(abs(w) for w in self.weights.values()) + \
                             self.config.get("order_book_support_confidence_boost", 0) + \
                             self.config.get("order_book_resistance_confidence_boost", 0)
        confidence = abs(float(raw_score)) / max_possible_score if max_possible_score > 0 else 0.0
        confidence = round(min(1.0, confidence), 2)

        return signal, confidence, conditions_met, {"stop_loss": sl, "take_profit": tp}

def interpret_indicator(logger: logging.Logger, indicator_name: str, values: Any) -> Union[str, None]:
    """
    Provides a human-readable interpretation of indicator values.
    """
    if values is None or (isinstance(values, list) and not values) or (isinstance(values, dict) and not values):
    ...
    try:
        ...
        last_value = values[-1] if isinstance(values, list) and values else values[0] if isinstance(values, list) else values

        if indicator_name == "rsi":
            if last_value > 70: return f"{NEON_RED}RSI:{RESET} Overbought ({last_value:.2f})"
            elif last_value < 30: return f"{NEON_GREEN}RSI:{RESET} Oversold ({last_value:.2f})"
            else: return f"{NEON_YELLOW}RSI:{RESET} Neutral ({last_value:.2f})"
        elif indicator_name == "mfi":
            if last_value > 80: return f"{NEON_RED}MFI:{RESET} Overbought ({last_value:.2f})"
            elif last_value < 20: return f"{NEON_GREEN}MFI:{RESET} Oversold ({last_value:.2f})"
            else: return f"{NEON_YELLOW}MFI:{RESET} Neutral ({last_value:.2f})"
        elif indicator_name == "cci":
            if last_value > 100: return f"{NEON_RED}CCI:{RESET} Overbought ({last_value:.2f})"
            elif last_value < -100: return f"{NEON_GREEN}CCI:{RESET} Oversold ({last_value:.2f})"
            else: return f"{NEON_YELLOW}CCI:{RESET} Neutral ({last_value:.2f})"
        elif indicator_name == "wr":
            if last_value < -80: return f"{NEON_GREEN}Williams %R:{RESET} Oversold ({last_value:.2f})"
            elif last_value > -20: return f"{NEON_RED}Williams %R:{RESET} Overbought ({last_value:.2f})"
            else: return f"{NEON_YELLOW}Williams %R:{RESET} Neutral ({last_value:.2f})"
        elif indicator_name == "adx":
            if last_value > 25: return f"{NEON_GREEN}ADX:{RESET} Trending ({last_value:.2f})"
            else: return f"{NEON_YELLOW}ADX:{RESET} Ranging ({last_value:.2f})"
...
def main():
    ...
    interval_input = input(f"{NEON_BLUE}Enter timeframe ({', '.join(VALID_INTERVALS)} or press Enter for default {CFG['interval']}): {RESET}").strip()
    interval = interval_input if interval_input and interval_input in VALID_INTERVALS else CFG["interval"]
    ...
    while True:
        try:
            ...
            min_data_needed = 30 # Base minimum for general analysis
            for name, enabled in CFG["indicators"].items():
                ...
                if enabled and name == 'stoch_rsi': # Ensure we have enough data for RSI within Stoch RSI
            ...
            if current_time - last_ob_fetch_time >= CFG["order_book_debounce_s"]:
                ...
                symbol_logger.debug(f"{NEON_YELLOW}Order book fetch debounced. Next fetch in ~{CFG['order_book_debounce_s'] - (current_time - last_ob_fetch_time):.1f}s{RESET}")

            analyzer = TradingAnalyzer(df, CFG, symbol_logger, symbol, interval)
            timestamp_str = datetime.now(TIMEZONE).strftime("%Y-%m-%d %H:%M:%S %Z")
            analyzer.analyze(current_price, timestamp_str, order_book)

            trade_signal = analyzer.generate_trading_signal(current_price)
            ...
            symbol_logger.error(f"{NEON_RED}Network or API communication error: {e}. Retrying in {CFG['retry_delay']} seconds...{RESET}")
            time.sleep(CFG["retry_delay"])
        except KeyboardInterrupt:
            symbol_logger.info(f"\n{NEON_YELLOW}User stopped analysis. Shutting down...{RESET}")
            break
        except Exception as e:
            symbol_logger.exception(f"{NEON_RED}An unexpected error occurred: {e}. Retrying in {CFG['retry_delay']} seconds...{RESET}")
            time.sleep(CFG["retry_delay"])
