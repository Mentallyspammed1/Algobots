...
patch('whalebot.logger', new=MockLogger()).start()
...
class TestWhalebot(unittest.TestCase):
    ...
    def test_interpret_indicator(self):
        ...
        # Test RSI
        self.assertEqual(self.analyzer._interpret_and_color_indicator("rsi", 75.0), f"{NEON_RED}RSI:{RESET} Overbought (75.00)")
        self.assertEqual(self.analyzer._interpret_and_color_indicator("rsi", 25.0), f"{NEON_GREEN}RSI:{RESET} Oversold (25.00)")
        self.assertEqual(self.analyzer._interpret_and_color_indicator("rsi", 50.0), f"{NEON_YELLOW}RSI:{RESET} Neutral (50.00)")

        # Test OBV
        self.assertEqual(self.analyzer._interpret_and_color_indicator("obv", [100.0, 110.0]), f"{NEON_BLUE}OBV:{RESET} Bullish")
        self.assertEqual(self.analyzer._interpret_and_color_indicator("obv", [110.0, 100.0]), f"{NEON_BLUE}OBV:{RESET} Bearish")
        self.assertEqual(self.analyzer._interpret_and_color_indicator("obv", [100.0, 100.0]), f"{NEON_BLUE}OBV:{RESET} Neutral")

        # Test mom (momentum)
        mom_data = {"trend": "Uptrend", "strength": 0.75}
        self.assertEqual(self.analyzer._interpret_and_color_indicator("mom", mom_data), f"{NEON_PURPLE}Momentum Trend:{RESET} Uptrend (Strength: 0.75)")

        # Test unknown indicator
        self.assertEqual(self.analyzer._interpret_and_color_indicator("unknown_indicator", 123.0), f"{NEON_YELLOW}UNKNOWN_INDICATOR:{RESET} No specific interpretation available.")

        # Test no data
        self.assertIn("No data available", self.analyzer._interpret_and_color_indicator("rsi", None))
        self.assertIn("No data available", self.analyzer._interpret_and_color_indicator("rsi", []))
        self.assertIn("No data available", self.analyzer._interpret_and_color_indicator("rsi", pd.Series(dtype=float)))
...
=======
import whalebot
print(f"Imported whalebot from: {whalebot.__file__}")
from whalebot import load_config, TradingAnalyzer, interpret_indicator, CONFIG_FILE, LOG_DIRECTORY, setup_custom_logger
>>>>>>> 1917b20 (WIP)
