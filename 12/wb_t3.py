    ...
    for retry in range(3):
        try:
            ...
            if e.response.status_code in RETRY_ERROR_CODES:
                if logger: logger.warning(f"{NEON_YELLOW}API Error {e.response.status_code}, retrying {retry+1}/3...{RESET}")
                time.sleep(5 * (2**retry))
            else:
                if logger: logger.error(f"{NEON_RED}Fatal API Error: {e.response.text}{RESET}")
                return None
        except requests.exceptions.RequestException as e:
            if logger: logger.error(f"{NEON_RED}Request exception: {e}, retrying {retry+1}/3...{RESET}")
            time.sleep(5 * (2**retry))

    if logger: logger.error(f"{NEON_RED}Max retries reached for {method} {endpoint}{RESET}")
    return None
...
def fetch_order_book(symbol: str, api_key: str, api_secret: str, logger: logging.Logger, limit: int = 50) -> Optional[dict]:
...
class TradingAnalyzer:
    ...
    def analyze(self, price: Decimal, ts: str, order_book: Optional[Dict[str, Any]]):
        ...
        for name in sorted(cfg_ind.keys()):
            ...
            try:
                if name == "atr": self.indicator_values[name] = self.atr_value
                elif name == "ema_alignment": self.indicator_values[name] = self._calculate_ema_alignment()
                elif name == "volume_confirmation": self.indicator_values[name] = self._calculate_volume_confirmation()
                elif name == "divergence": self.indicator_values[name] = self._detect_macd_divergence()
                elif name == "sma_10": self.indicator_values[name] = Indicators.sma(self.df.close, 10).iloc[-1]
                else:
                    indicator_method = getattr(Indicators, name, None)
                    if indicator_method:
                        params = {"window": cfg_prd.get(name, 14)}
                        if name == "macd": params = {"fast": 12, "slow": 26, "signal": 9}
                        if name == "stochastic_oscillator": params = {"k_period": cfg_prd["stoch_osc_k"], "d_period": cfg_prd["stoch_osc_d"]}
                        if name == "stoch_rsi": params = {"rsi_period": cfg_prd["stoch_rsi_period"], "k_period": cfg_prd["stoch_rsi_k_period"], "d_period": cfg_prd["stoch_rsi_d_period"]}
                        if name == "vi": params = {"window": cfg_prd["vi"]}
                        if name == "ao": params = {"short_period": cfg_prd["ao_short"], "long_period": cfg_prd["ao_long"]}
                        if name == "bb": params = {"window": cfg_prd["bb"]}
                        
                        series = indicator_method(self.df.high, self.df.low, self.df.close, self.df.volume, **params) if name in ["adi", "mfi", "cci", "wr", "adx", "vi", "cmf", "fve"] else \
                                 (indicator_method(self.df.close, **params) if name in ["rsi", "macd", "momentum", "stoch_rsi", "stochastic_oscillator", "ao", "bb"] else \
                                 (indicator_method(self.df.high, self.df.low, self.df.close, **params) if name == "psar" else \
                                 (indicator_method(self.df.close, self.df.volume, **params) if name in ["obv", "vwap"] else None)))
...
def main():
    ...
    try:
        while True:
            ...
            df = fetch_klines(symbol, interval, API_KEY, API_SECRET, symbol_logger, min_data_needed)
            ...
            if current_time - last_ob_fetch_time >= CFG["order_book_debounce_s"]:
                order_book = fetch_order_book(symbol, API_KEY, API_SECRET, symbol_logger, CFG["order_book_depth_to_check"])
