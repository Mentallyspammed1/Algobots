            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if status_code in RETRY_ERROR_CODES:
                        LOGGER.warning(
                            f"{NEON_YELLOW}API request failed (Attempt {attempt}/{max_attempts}, Status: {status_code}). Retrying in {delay}s...{RESET}",
                        )
...
def load_config(fp: Path) -> BotConfig:
    ...
    try:
        ...
        LOGGER.error(
            f"{NEON_RED}Error with config file: {e}. Rebuilding default.{RESET}",
        )
    ...
    if merged["interval"] not in VALID_INTERVALS:
        LOGGER.warning(
            f"{NEON_YELLOW}Invalid interval '{merged['interval']}'. Falling back to default.{RESET}",
        )
        merged["interval"] = "15"
    ...
    for retry in range(3):
        try:
            ...
            if e.response.status_code in RETRY_ERROR_CODES:
                if logger:
                    logger.warning(
                        f"{NEON_YELLOW}API Error {e.response.status_code}, retrying {retry + 1}/3...{RESET}",
                    )
                time.sleep(5 * (2**retry))
            else:
                if logger:
                    logger.error(f"{NEON_RED}Fatal API Error: {e.response.text}{RESET}")
                return None
        except requests.exceptions.RequestException as e:
            if logger:
                logger.error(
                    f"{NEON_RED}Request exception: {e}, retrying {retry + 1}/3...{RESET}",
                )
            time.sleep(5 * (2**retry))
...
def fetch_klines(
    ...
    if (
        ...
        # Explicitly convert 'start_time' to datetime and others to Decimal
...
class Indicators:
    ...
    def ema(series: pd.Series, span: int) -> pd.Series:
        ...
        return (
            series.ewm(span=span, adjust=False)
            .mean()
            .apply(lambda x: Decimal(str(x)) if pd.notna(x) else Decimal("NaN"))
        )
        ...
        for i in range(1, len(close)):
            ...
            if trend == 1:
                ...
                if curr_l < psar.iloc[i]:
                    trend, psar.iloc[i], ep, af = -1, ep, curr_l, acceleration
            else:
                psar.iloc[i] = prev_psar + af * (ep - prev_psar)
                psar.iloc[i] = max(
                ...
                if curr_h > psar.iloc[i]:
                    trend, psar.iloc[i], ep, af = 1, ep, curr_h, acceleration
        return psar.ffill()
    ...
    def cci(
        ...
        mean_dev = (
            typical_price.rolling(window=window)
            .apply(lambda x: (x - x.mean()).abs().mean(), raw=True)
            .apply(lambda x: Decimal(str(x)) if pd.notna(x) else Decimal("NaN"))
        )
        return (
            (typical_price - sma_tp)
            / (constant * mean_dev.replace(Decimal("0"), Decimal("NaN")))
        ).fillna(Decimal("0"))
    ...
    def cmf(
        ...
        mfm = (((close - low) - (high - close)) / mfm_denom).fillna(Decimal("0"))
        mfv = mfm * volume
        return mfv.rolling(window=window).sum() / volume.rolling(
            window=window,
        ).sum().replace(Decimal("0"), Decimal("NaN"))

    @staticmethod
    def ao(close: pd.Series, short_period: int = 5, long_period: int = 34) -> pd.Series:
        """Decimal-safe Awesome Oscillator (AO)."""
        median_price = (close.shift(1) + close) / Decimal("2")
    ...
    def vi(
        ...
        plus_vi = (high.diff().abs() + low.diff().abs()).rolling(
            window=window,
        ).sum() / tr
        minus_vi = (high.diff(-1).abs() + low.diff(-1).abs()).shift(-1).rolling(
            window=window,
        ).sum() / tr
    ...
    def bb(
        ...
        rolling_std = (
            close.rolling(window=window).std().apply(lambda x: Decimal(str(x)))
        )
...
class TradingAnalyzer:
    ...
    def _calculate_ema_alignment(self) -> Decimal:
        ...
        if len(ema_short) < 2 or len(ema_long) < 2 or len(price) < 2:
            return Decimal("0")
    ...
    def calculate_pivot_points(self, high: Decimal, low: Decimal, close: Decimal):
        ...
        self.levels.update(
            {label: val.quantize(precision) for label, val in pivots.items()},
        )
            ...
            if isinstance(value, dict):
                for sub_label, sub_value in value.items():
                    if sub_value < current_price:
                        supports.append((f"{label} ({sub_label})", sub_value))
                    else:
                        resistances.append((f"{label} ({sub_label})", sub_value))
            elif isinstance(value, Decimal):
                if value < current_price:
                    supports.append((label, value))
                else:
                    resistances.append((label, value))
        return sorted(supports, key=lambda x: current_price - x[1])[
            :num_levels
        ], sorted(resistances, key=lambda x: x[1] - current_price)[:num_levels]
    ...
    def _interpret_and_color_indicator(self, name: str, value: Any) -> str:
        ...
        if name == "ema_alignment":
            ...
            if self.df.close.iloc[-1] > value:
                color = NEON_GREEN
            elif self.df.close.iloc[-1] < value:
                color = NEON_RED
    ...
    def analyze(self, price: Decimal, ts: str, order_book: dict[str, Any] | None):
        ...
        for name in sorted(cfg_ind.keys()):
            ...
            try:
                if name == "atr":
                    self.indicator_values[name] = self.atr_value
                elif name == "ema_alignment":
                    self.indicator_values[name] = self._calculate_ema_alignment()
                elif name == "volume_confirmation":
                    self.indicator_values[name] = self._calculate_volume_confirmation()
                elif name == "divergence":
                    self.indicator_values[name] = self._detect_macd_divergence()
                elif name == "sma_10":
                    self.indicator_values[name] = Indicators.sma(
                        self.df.close, 10,
                    ).iloc[-1]
                else:
                    indicator_method = getattr(Indicators, name, None)
                    if indicator_method:
                        params = {"window": cfg_prd.get(name, 14)}
                        if name == "macd":
                            params = {"fast": 12, "slow": 26, "signal": 9}
                        if name == "stochastic_oscillator":
                            params = {
                                "k_period": cfg_prd["stoch_osc_k"],
                                "d_period": cfg_prd["stoch_osc_d"],
                            }
                        if name == "stoch_rsi":
                            params = {
                                "rsi_period": cfg_prd["stoch_rsi_period"],
                                "k_period": cfg_prd["stoch_rsi_k_period"],
                                "d_period": cfg_prd["stoch_rsi_d_period"],
                            }
                        if name == "vi":
                            params = {"window": cfg_prd["vi"]}
                        if name == "ao":
                            params = {
                                "short_period": cfg_prd["ao_short"],
                                "long_period": cfg_prd["ao_long"],
                            }
                        if name == "bb":
                            params = {"window": cfg_prd["bb"]}
                        ...
                        series = (
                            ...
                            if name
                            in ["adi", "mfi", "cci", "wr", "adx", "vi", "cmf", "fve"]
                            else (
                                indicator_method(self.df.close, **params)
                                if name
                                in [
                                    "rsi",
                                    "macd",
                                    ...
                                    if name == "psar"
                                        ...
                                        indicator_method(
                                            self.df.close, self.df.volume, **params,
                                        )
                                        if name in ["obv", "vwap"]
                                        else None
                                    )
    ...
    def generate_trading_signal(self, current_price: Decimal) -> TradeSignal:
        ...
        for name, weight in self.weights.items():
            ...
            if name == "ema_alignment":
                ...
                k, d = value.get("stoch_rsi_k"), value.get("stoch_rsi_d")
                if k < Decimal(str(self.cfg["stoch_rsi_oversold_threshold"])) and k > d:
                    raw_score += weight_dec
                    conditions_met.append("StochRSI Oversold")
                elif (
                    k > Decimal(str(self.cfg["stoch_rsi_overbought_threshold"]))
                    and k < d
                ):
                    raw_score -= weight_dec
                    conditions_met.append("StochRSI Overbought")
            elif name == "rsi":
                if value < Decimal("30"):
                    raw_score += weight_dec
                ...
                if k < Decimal("20") and k > d:
                    raw_score += weight_dec
                    conditions_met.append("Stoch Osc Oversold")
                elif k > Decimal("80") and k < d:
                    raw_score -= weight_dec
                    conditions_met.append("Stoch Osc Overbought")
            elif name == "cmf":
                if value > Decimal("0"):
                    raw_score += weight_dec
        ...
        max_score = sum(self.weights.values()) + self.cfg.get(
            "order_book_support_confidence_boost", 0,
        )
        ...
        while True:
            ...
            for name, enabled in CFG["indicators"].items():
                if enabled and name in CFG["indicator_periods"]:
                    min_data_needed = max(
                        min_data_needed, CFG["indicator_periods"][name],
                    )
                if enabled and name == "stoch_rsi":
                    min_data_needed = max(
                        min_data_needed, CFG["indicator_periods"]["stoch_rsi_period"],
                    )
            min_data_needed += 10

            df = fetch_klines(
                symbol, interval, API_KEY, API_SECRET, symbol_logger, min_data_needed,
            )
            ...
            if trade_signal.signal and (
                ...
                if trade_signal.levels:
                    sl, tp = (
                        trade_signal.levels["stop_loss"],
                        trade_signal.levels["take_profit"],
                    )
                    symbol_logger.info(
