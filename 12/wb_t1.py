...
class TradingAnalyzer:
    ...
    def _select_weight_set(self) -> Dict[str, float]:
        ...
        self.log.info(f"Market Volatility: {NEON_YELLOW}{vol_mode.replace('_', ' ').upper()}{RESET} (ATR: {self.atr_value:.5f})")
    ...
    def analyze(self, price: Decimal, ts: str, order_book: Optional[Dict[str, Any]]):
        ...
        rsi_val = Indicators.rsi(self.df.close, self.cfg["indicator_periods"]["rsi"]).iloc[-1]
        ema_score = self._calculate_ema_alignment()
        vol_spike = self._calculate_volume_confirmation()
        adx_df = Indicators.adx(self.df.high, self.df.low, self.df.close, self.cfg["indicator_periods"]["adx"])
        ...
        # --- Logging ---
        trend_status = f"{NEON_GREEN}Trending{RESET}" if adx_val > 25 else f"{NEON_YELLOW}Ranging{RESET}"
        rsi_status = f"{NEON_RED}Overbought{RESET}" if rsi_val > 70 else f"{NEON_GREEN}Oversold{RESET}" if rsi_val < 30 else f"{NEON_YELLOW}Neutral{RESET}"
        ...
        if order_book and "bids" in order_book and "asks" in order_book:
            ...
            self.log.info(f" Order Book: Bid {bid0_qty} @ {bid0_price} | Ask {ask0_qty} @ {ask0_price}")
    ...
    def generate_trading_signal(self, price: Decimal) -> TradeSignal:
        ...
        if self.indicator_values.get("ema_alignment", 0) > 0:
            score += Decimal(str(self.weights.get("ema_alignment", 0))) * Decimal(str(abs(self.indicator_values["ema_alignment"])))
        ...
        if self.indicator_values.get("ema_alignment", 0) < 0:
            bearish_score += Decimal(str(self.weights.get("ema_alignment", 0))) * Decimal(str(abs(self.indicator_values["ema_alignment"])))
...
def main() -> None:
    ...
    if not API_KEY or not API_SECRET:
        LOGGER.critical(f"{NEON_RED}BYBIT_API_KEY and BYBIT_API_SECRET must be set in .env file.{RESET}")
        return

    symbol = (input(f"{NEON_BLUE}Enter symbol (default BTCUSDT): {RESET}") or "BTCUSDT").upper()
    interval = (input(f"{NEON_BLUE}Enter interval {VALID_INTERVALS} (default {CFG['interval']}): {RESET}") or CFG["interval"])
    if interval not in VALID_INTERVALS:
        LOGGER.warning(f"{NEON_YELLOW}Invalid interval. Using default: {CFG['interval']}{RESET}")
