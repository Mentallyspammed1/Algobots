...
class TestConfigLoading(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        if os.path.exists(self.test_config_file):
            os.remove(self.test_config_file)
        backup_files = [f for f in os.listdir('.') if f.startswith(f"{self.test_config_file}.bak")]
    ...
    def test_merge_with_default_config(self):
        ...
        self.assertTrue(config['indicators']['macd']) # A default that wasn't in user config
...
class TestTradingAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data = {
            ...
            'high': np.linspace(102, 155, 50) + np.sin(np.linspace(0, 3 * np.pi, 50)) * 2,
        ...
        self.analyzer = tt.TradingAnalyzer(self.df, self.config, self.mock_logger, "BTCUSDT", "15")
    ...
    def test_calculate_laguerre_rsi(self):
        ...
        self.assertTrue(laguerre_series.iloc[-1] > self.config['laguerre_rsi_overbought'])
    ...
    def test_generate_buy_signal(self):
        ...
        close_prices = [150, 140, 130, 120, 110, 100, 90, 85, 80, 75, 70, 72, 75, 78, 82]
