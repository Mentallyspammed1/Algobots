...
def _get_default_config() -> dict[str, Any]:
    ...
    return {
        ...
        "momentum_period": 10, "momentum_ma_short": 12, "momentum_ma_long": 26,
        "volume_ma_period": 20, "atr_period": 14, "signal_score_threshold": 1.0,
        "stop_loss_multiple": 1.5, "take_profit_multiple": 1.0,
        ...
        "indicators": {
            ...
            "obv": True, "adi": True, "cci": True, "wr": True, # Enabled by default
        ...
        "weight_sets": {
            "low_volatility": {
                ...
                "vwap": 0.2, # New weight
                "obv": 0.1, "adi": 0.1, "cci": 0.1, "wr": 0.1, # New weights
        ...
        "stoch_rsi_confidence_boost": 5, "stoch_rsi_mandatory": False,
        ...
        "indicator_periods": {
            ...
            "volume_ma": 20, "atr": 14, "sma_10": 10, "fve_price_ema": 10,
            "fve_obv_sma": 20, "fve_atr_sma": 20, "stoch_osc_k": 14, "stoch_osc_d": 3,
            "vwap": 14, # New period
        },
        "order_book_analysis": {"enabled": True, "wall_threshold_multiplier": 2.0, "depth_to_check": 10},
        "trailing_stop_loss": {"enabled": False, "initial_activation_percent": 0.5, "trailing_stop_multiple_atr": 1.5},
        "take_profit_scaling": {"enabled": False, "targets": [{"level": 1.5, "percentage": 0.25}, {"level": 2.0, "percentage": 0.50}]}
    }
...
def load_config(fp: Path) -> BotConfig:
    ...
    try:
        ...
        LOGGER.error(f"{NEON_RED}Corrupt config file: {e}. Backing up and creating new default.{RESET}")
    ...
    for attempt in range(retries):
        try:
            ...
            if resp.status_code in RETRY_ERROR_CODES:
                raise requests.HTTPError(f"Retryable HTTP Error: {resp.status_code} {resp.reason}")

            resp.raise_for_status() # Raise for other non-2xx codes
            data = resp.json()

            if data.get("retCode") != 0:
                logger.error(f"{NEON_RED}Bybit API Error (retCode={data.get('retCode')}): {data.get('retMsg')}{RESET}")
                return None
            return data
...
def fetch_current_price(symbol: str, log: logging.Logger) -> Decimal | None:
    ...
    try:
        ...
        log.error(f"{NEON_RED}Price not found for symbol {symbol} in ticker data.{RESET}")
        return None
...
def fetch_klines(
    ...
    if not data or not (kline_list := data.get("result", {}).get("list")):
        log.warning(f"{NEON_YELLOW}No kline data received for {symbol} ({interval}).{RESET}")
        return pd.DataFrame()
...
class Indicators:
    ...
    def adx(high: pd.Series, low: pd.Series, close: pd.Series, window: int = 14) -> pd.DataFrame:
        ...
        plus_dm_s = Indicators.ema(plus_dm.where((plus_dm > 0) & (plus_dm > minus_dm), 0), span=window)
        minus_dm_s = Indicators.ema(minus_dm.where((minus_dm > 0) & (minus_dm > plus_dm), 0), span=window)
        ...
        minus_di = 100 * (minus_dm_s / tr_s).replace([np.inf, -np.inf], np.nan).fillna(0)
    ...
    def stochastic_oscillator(close: pd.Series, high: pd.Series, low: pd.Series, k_period: int = 14, d_period: int = 3) -> pd.DataFrame:
        ...
        if close.empty or high.empty or low.empty: return pd.DataFrame()
    ...
    def stoch_rsi(close: pd.Series, rsi_period: int = 14, k_period: int = 3, d_period: int = 3) -> pd.DataFrame:
        ...
        if rsi_vals.empty: return pd.DataFrame()
        ...
        k_line = stoch_rsi_val.rolling(window=k_period, min_periods=1).mean() # %K for Stoch RSI
        d_line = k_line.rolling(window=d_period, min_periods=1).mean()       # %D for Stoch RSI
        return pd.DataFrame({"rsi": rsi_vals, "stoch_rsi_k": k_line, "stoch_rsi_d": d_line})

    @staticmethod
    def psar(high: pd.Series, low: pd.Series, close: pd.Series, acceleration: float = 0.02, max_acceleration: float = 0.2) -> pd.Series:
    ...
    @staticmethod
    def fve(close: pd.Series, volume: pd.Series, atr: pd.Series, price_ema_p: int = 10, obv_norm_p: int = 20, atr_norm_p: int = 20) -> pd.Series:
        """Calculates a 'Fictional Value Estimate' - a composite indicator."""
        if close.empty or volume.empty or atr.empty: return pd.Series(dtype=float)
        min_points = max(obv_norm_p, atr_norm_p, price_ema_p, len(atr)) # Ensure enough data for all components
        if len(close) < min_points: return pd.Series([np.nan] * len(close))
        ...
        try:
            ...
            def safe_normalize(series: pd.Series) -> pd.Series:
                if series.empty or series.isna().all(): return pd.Series([Decimal('0')] * len(series), index=series.index)
                series_dec = pd.Series([Decimal(str(v)) if pd.notna(v) else Decimal('NaN') for v in series], index=series.index)
                s_mean = series_dec.mean()
                s_std = series_dec.std()
                if pd.isna(s_mean) or pd.isna(s_std) or s_std == Decimal('0'):
                    return pd.Series([Decimal('0')] * len(series_dec), index=series_dec.index)
            ...
            atr_inv = pd.Series([Decimal('1.0') / Decimal(str(v)) if pd.notna(v) and v != 0 else Decimal('NaN') for v in atr], index=atr.index)
    ...
    def obv(close: pd.Series, volume: pd.Series) -> pd.Series:
        ...
        if close.empty or volume.empty: return pd.Series(dtype=float)
    ...
    def adi(high: pd.Series, low: pd.Series, close: pd.Series, volume: pd.Series) -> pd.Series:
        ...
        if high.empty or low.empty or close.empty or volume.empty: return pd.Series(dtype=float)
    ...
    def cci(high: pd.Series, low: pd.Series, close: pd.Series, window: int = 20, constant: float = 0.015) -> pd.Series:
        ...
        if high.empty or low.empty or close.empty: return pd.Series(dtype=float)
        ...
        cci_series = (typical_price - sma_typical_price) / (constant * mean_deviation).replace(0, np.nan)
    ...
    def momentum(close: pd.Series, period: int = 10) -> pd.Series:
        ...
        if close.empty or len(close) < period: return pd.Series(dtype=float)
    ...
    def vwap(close: pd.Series, volume: pd.Series, window: int) -> pd.Series:
        ...
        if close.empty or volume.empty: return pd.Series(dtype=float)
        if window <= 0: return close # If window is invalid, return close price
        ...
        vwap_series = (cumulative_pv / cumulative_v.replace(0, np.nan)).fillna(close) # Fill initial NaNs with close
        return vwap_series
...
class TradingAnalyzer:
    ...
    def _calculate_ema_alignment(self) -> float:
        ...
        if len(self.df) < long_p: return 0.0
    ...
    def _detect_macd_divergence(self) -> str | None:
        ...
        lookback = 5
        if len(prices) < lookback: return None
        ...
        if price_ll and macd_hl:
            self.log.debug(f"{NEON_GREEN}Possible Bullish MACD Divergence detected.{RESET}")
            return "bullish"
        ...
        if price_hh and macd_lh:
            self.log.debug(f"{NEON_RED}Possible Bearish MACD Divergence detected.{RESET}")
            return "bearish"
        return None
    ...
    def analyze(self, price: Decimal, ts: str, order_book: dict[str, Any] | None):
        ...
        if indicators_cfg.get("rsi"):
            ...
            self.indicator_values["rsi"] = float(rsi_series.iloc[-1]) if not rsi_series.empty else np.nan
        if indicators_cfg.get("macd"):
            macd_df = Indicators.macd(self.df.close)
            self.indicator_values["macd"] = macd_df.iloc[-1].to_dict() if not macd_df.empty else {}
        if indicators_cfg.get("adx"):
            adx_df = Indicators.adx(self.df.high, self.df.low, self.df.close, periods_cfg["adx"])
            self.indicator_values["adx"] = float(adx_df["ADX"].iloc[-1]) if not adx_df.empty else np.nan
        if indicators_cfg.get("stochastic_oscillator"):
            stoch_osc_df = Indicators.stochastic_oscillator(self.df.close, self.df.high, self.df.low, periods_cfg["stoch_osc_k"], periods_cfg["stoch_osc_d"])
            self.indicator_values["stochastic_oscillator"] = stoch_osc_df.iloc[-1].to_dict() if not stoch_osc_df.empty else {}
        if indicators_cfg.get("stoch_rsi"):
            stoch_rsi_df = Indicators.stoch_rsi(self.df.close, periods_cfg["stoch_rsi_period"], periods_cfg["stoch_rsi_k_period"], periods_cfg["stoch_rsi_d_period"])
            self.indicator_values["stoch_rsi"] = stoch_rsi_df.iloc[-1].to_dict() if not stoch_rsi_df.empty else {}
        if indicators_cfg.get("momentum"):
            mom_series = Indicators.momentum(self.df.close, periods_cfg["momentum"])
            self.indicator_values["momentum"] = float(mom_series.iloc[-1]) if not mom_series.empty else np.nan
        if indicators_cfg.get("vwap"):
            vwap_series = Indicators.vwap(self.df.close, self.df.volume, periods_cfg["vwap"])
            self.indicator_values["vwap"] = float(vwap_series.iloc[-1]) if not vwap_series.empty else np.nan
        if indicators_cfg.get("obv"):
            obv_series = Indicators.obv(self.df.close, self.df.volume)
            self.indicator_values["obv"] = float(obv_series.iloc[-1]) if not obv_series.empty else np.nan
        if indicators_cfg.get("adi"):
            adi_series = Indicators.adi(self.df.high, self.df.low, self.df.close, self.df.volume)
            self.indicator_values["adi"] = float(adi_series.iloc[-1]) if not adi_series.empty else np.nan
        if indicators_cfg.get("cci"):
            cci_series = Indicators.cci(self.df.high, self.df.low, self.df.close, periods_cfg["cci"])
            self.indicator_values["cci"] = float(cci_series.iloc[-1]) if not cci_series.empty else np.nan
        if indicators_cfg.get("wr"):
            wr_series = Indicators.williams_r(self.df.high, self.df.low, self.df.close, periods_cfg["williams_r"])
            self.indicator_values["wr"] = float(wr_series.iloc[-1]) if not wr_series.empty else np.nan
        if indicators_cfg.get("mfi"):
            mfi_series = Indicators.mfi(self.df.high, self.df.low, self.df.close, self.df.volume, periods_cfg["mfi"])
            self.indicator_values["mfi"] = float(mfi_series.iloc[-1]) if not mfi_series.empty else np.nan
        if indicators_cfg.get("psar"):
            psar_series = Indicators.psar(self.df.high, self.df.low, self.df.close)
            self.indicator_values["psar"] = float(psar_series.iloc[-1]) if not psar_series.empty else np.nan
        if indicators_cfg.get("fve"):
            fve_series = Indicators.fve(self.df.close, self.df.volume, Indicators.atr(self.df.high, self.df.low, self.df.close, periods_cfg["atr"]),
                                        periods_cfg["fve_price_ema"], periods_cfg["fve_obv_sma"], periods_cfg["fve_atr_sma"])
            self.indicator_values["fve"] = float(fve_series.iloc[-1]) if not fve_series.empty else np.nan
        if indicators_cfg.get("sma_10"):
            sma_10_series = Indicators.sma(self.df.close, periods_cfg["sma_10"])
            self.indicator_values["sma_10"] = float(sma_10_series.iloc[-1]) if not sma_10_series.empty else np.nan
        ...
        self.indicator_values["order_book_walls"] = {
            "bullish": has_bull, "bearish": has_bear,
            "bullish_details": bullish_details, "bearish_details": bearish_details,
        }
        ...
        for name, value in self.indicator_values.items():
            ...
            if name in ["atr", "ema_alignment", "volume_confirmation", "stochastic_oscillator", "stoch_rsi", "macd", "order_book_walls", "divergence"]:
                continue
        ...
        if indicators_cfg.get("ema_alignment"):
            ...
            status = 'Bullish' if ema_score > 0 else 'Bearish' if ema_score < 0 else 'Neutral'
            self.log.info(f"{NEON_PURPLE}EMA Alignment:{RESET} Score={ema_score:+.1f} ({status})")
        if indicators_cfg.get("stochastic_oscillator") and isinstance(self.indicator_values.get("stochastic_oscillator"), dict):
            so_data = self.indicator_values["stochastic_oscillator"]
            self.log.info(f"{NEON_CYAN}Stochastic Oscillator:{RESET} K={so_data.get('k', np.nan):.1f}, D={so_data.get('d', np.nan):.1f}")
        if indicators_cfg.get("stoch_rsi") and isinstance(self.indicator_values.get("stoch_rsi"), dict):
            srsi_data = self.indicator_values["stoch_rsi"]
            self.log.info(f"{NEON_GREEN}Stoch RSI:{RESET} K={srsi_data.get('stoch_rsi_k', np.nan):.1f}, D={srsi_data.get('stoch_rsi_d', np.nan):.1f} (RSI: {srsi_data.get('rsi', np.nan):.1f})")
        if indicators_cfg.get("macd") and isinstance(self.indicator_values.get("macd"), dict):
            macd_data = self.indicator_values["macd"]
            self.log.info(f"{NEON_PURPLE}MACD:{RESET} MACD={macd_data.get('macd', np.nan):.2f}, Signal={macd_data.get('signal', np.nan):.2f}, Hist={macd_data.get('histogram', np.nan):.2f}")
        if indicators_cfg.get("divergence") and self.indicator_values.get("divergence"):
            div_type = self.indicator_values["divergence"]
            self.log.info(f"{NEON_YELLOW}Divergence:{RESET} {div_type.upper()} MACD Divergence detected.")

        # Order Book Walls
        self.log.info(f"\n{NEON_BLUE}Order Book Walls:{RESET}")
        if has_bull:
            self.log.info(f"{NEON_GREEN}  Bullish Walls Found: {', '.join([f'{k}:{v:,.0f}' for k, v in bullish_details.items()])}{RESET}")
        if has_bear:
            self.log.info(f"{NEON_RED}  Bearish Walls Found: {', '.join([f'{k}:{v:,.0f}' for k, v in bearish_details.items()])}{RESET}")
    ...
    def generate_trading_signal(self, price: Decimal) -> TradeSignal:
        ...
        if indicators_to_check.get("rsi") and self.indicator_values.get("rsi") is not None:
        ...
        if indicators_to_check.get("ema_alignment") and self.indicator_values.get("ema_alignment") is not None:
            ema_align = self.indicator_values["ema_alignment"]
            if ema_align > 0:
                bull_score += Decimal(str(self.weights.get("ema_alignment", 0))) * Decimal(str(abs(ema_align)))
                bull_conditions.append("Bullish EMA Align")
            elif ema_align < 0:
                bear_score += Decimal(str(self.weights.get("ema_alignment", 0))) * Decimal(str(abs(ema_align)))
                bear_conditions.append("Bearish EMA Align")

        if indicators_to_check.get("volume_confirmation") and self.indicator_values.get("volume_confirmation") is not None:
            if self.indicator_values["volume_confirmation"]:
                ...
                if bull_score > bear_score: # If leaning bullish
                    bull_score += Decimal(str(self.weights.get("volume_confirmation", 0)))
                    bull_conditions.append("Volume Confirm (Bullish)")
                else: # If leaning bearish or neutral
                    bear_score += Decimal(str(self.weights.get("volume_confirmation", 0)))
                    bear_conditions.append("Volume Confirm (Bearish)")


        if indicators_to_check.get("stochastic_oscillator") and isinstance(self.indicator_values.get("stochastic_oscillator"), dict):
            so_data = self.indicator_values["stochastic_oscillator"]
            k_osc, d_osc = Decimal(str(so_data.get('k', np.nan))), Decimal(str(so_data.get('d', np.nan)))
            if not pd.isna(k_osc) and not pd.isna(d_osc):
                if k_osc < 20 and k_osc > d_osc:
                    bull_score += Decimal(str(self.weights.get("stochastic_oscillator", 0)))
                    bull_conditions.append("StochOsc Oversold Cross")
                elif k_osc > 80 and k_osc < d_osc:
                    bear_score += Decimal(str(self.weights.get("stochastic_oscillator", 0)))
                    bear_conditions.append("StochOsc Overbought Cross")

        if indicators_to_check.get("stoch_rsi") and isinstance(self.indicator_values.get("stoch_rsi"), dict):
            srsi_data = self.indicator_values["stoch_rsi"]
            k_srsi, d_srsi = Decimal(str(srsi_data.get('stoch_rsi_k', np.nan))), Decimal(str(srsi_data.get('stoch_rsi_d', np.nan)))
            if not pd.isna(k_srsi) and not pd.isna(d_srsi):
                if k_srsi < self.cfg["stoch_rsi_oversold_threshold"] and k_srsi > d_srsi:
                    bull_score += Decimal(str(self.weights.get("stoch_rsi", 0)))
                    bull_conditions.append("StochRSI Oversold Cross")
                elif k_srsi > self.cfg["stoch_rsi_overbought_threshold"] and k_srsi < d_srsi:
                    bear_score += Decimal(str(self.weights.get("stoch_rsi", 0)))
                    bear_conditions.append("StochRSI Overbought Cross")

        if indicators_to_check.get("momentum") and self.indicator_values.get("momentum") is not None:
        ...
        if indicators_to_check.get("vwap") and self.indicator_values.get("vwap") is not None:
        ...
        if indicators_to_check.get("obv") and self.indicator_values.get("obv") is not None and len(self.df.index) > 1:
        ...
        if indicators_to_check.get("adi") and self.indicator_values.get("adi") is not None and len(self.df.index) > 1:
            adi_current = self.indicator_values["adi"]
            adi_prev = Indicators.adi(self.df.high, self.df.low, self.df.close, self.df.volume).iloc[-2]
        ...
        if indicators_to_check.get("cci") and self.indicator_values.get("cci") is not None:
        ...
        if indicators_to_check.get("wr") and self.indicator_values.get("wr") is not None:
        ...
        if indicators_to_check.get("mfi") and self.indicator_values.get("mfi") is not None:
        ...
        if indicators_to_check.get("psar") and self.indicator_values.get("psar") is not None:
        ...
        if indicators_to_check.get("fve") and self.indicator_values.get("fve") is not None:
        ...
        if indicators_to_check.get("sma_10") and self.indicator_values.get("sma_10") is not None:
        ...
        if indicators_to_check.get("divergence") and self.indicator_values.get("divergence"):
            div_type = self.indicator_values["divergence"]
            if div_type == "bullish":
        ...
        if order_book_walls_data.get("bullish"):
            bull_score += Decimal(str(self.cfg.get("order_book_support_confidence_boost", 0))) / Decimal("10.0")
            bull_conditions.append("Bullish Order Book Wall")
        if order_book_walls_data.get("bearish"):
            bear_score += Decimal(str(self.cfg.get("order_book_resistance_confidence_boost", 0))) / Decimal("10.0")
...
def interpret_indicator(logger: logging.Logger, indicator_name: str, value: Any) -> str | None:
    ...
    try:
        ...
        if indicator_name == "rsi":
            if val_float > 70: return f"{NEON_RED}RSI:{RESET} Overbought ({val_float:.2f})"
            if val_float < 30: return f"{NEON_GREEN}RSI:{RESET} Oversold ({val_float:.2f})"
            return f"{NEON_YELLOW}RSI:{RESET} Neutral ({val_float:.2f})"
        if indicator_name == "mfi":
            if val_float > 80: return f"{NEON_RED}MFI:{RESET} Overbought ({val_float:.2f})"
            if val_float < 20: return f"{NEON_GREEN}MFI:{RESET} Oversold ({val_float:.2f})"
            return f"{NEON_YELLOW}MFI:{RESET} Neutral ({val_float:.2f})"
        if indicator_name == "cci":
            if val_float > 100: return f"{NEON_RED}CCI:{RESET} Overbought ({val_float:.2f})"
            if val_float < -100: return f"{NEON_GREEN}CCI:{RESET} Oversold ({val_float:.2f})"
            return f"{NEON_YELLOW}CCI:{RESET} Neutral ({val_float:.2f})"
        if indicator_name == "wr":
            if val_float < -80: return f"{NEON_GREEN}Williams %R:{RESET} Oversold ({val_float:.2f})"
            if val_float > -20: return f"{NEON_RED}Williams %R:{RESET} Overbought ({val_float:.2f})"
            return f"{NEON_YELLOW}Williams %R:{RESET} Neutral ({val_float:.2f})"
        if indicator_name == "adx":
            if val_float > 25: return f"{NEON_GREEN}ADX:{RESET} Trending ({val_float:.2f})"
        ...
        if indicator_name == "fve":
            if val_float > 0.5: return f"{NEON_GREEN}FVE:{RESET} Bullish ({val_float:.2f})"
            if val_float < -0.5: return f"{NEON_RED}FVE:{RESET} Bearish ({val_float:.2f})"
            return f"{NEON_YELLOW}FVE:{RESET} Neutral ({val_float:.2f})"
        if indicator_name == "momentum":
            if val_float > 0: return f"{NEON_GREEN}Momentum:{RESET} Bullish ({val_float:.2f}%)"
            if val_float < 0: return f"{NEON_RED}Momentum:{RESET} Bearish ({val_float:.2f}%)"
        ...
        logger.error(f"{NEON_RED}Error interpreting {indicator_name}: {e}. Value: {value}{RESET}")
...
def main():
    ...
    try:
        while True:
            ...
            if price is None:
                symbol_logger.error(f"{NEON_RED}Failed to fetch current price for {symbol}. Retrying in {CFG['retry_delay']}s...{RESET}")
                time.sleep(CFG["retry_delay"])
                continue

            df = fetch_klines(symbol, interval, 200, symbol_logger, kline_cache)
            if df.empty or len(df) < max(CFG['atr_period'], CFG['ema_long_period'], CFG['indicator_periods']['adx'], CFG['indicator_periods']['vwap']): # Ensure enough data for indicators
                symbol_logger.warning(f"{NEON_YELLOW}Insufficient Kline data received for {symbol} ({interval}). Need more bars. Retrying in {CFG['retry_delay']}s...{RESET}")
                time.sleep(CFG["retry_delay"])
                continue

            current_time = time.time()
            if current_time - last_ob_fetch_time >= CFG["order_book_debounce_s"]:
                order_book = fetch_order_book(symbol, CFG["order_book_depth_to_check"], symbol_logger)
                last_ob_fetch_time = current_time
            else:
                symbol_logger.debug(f"{NEON_YELLOW}Order book fetch debounced. Next in ~{CFG['order_book_debounce_s'] - (current_time - last_ob_fetch_time):.1f}s{RESET}")
