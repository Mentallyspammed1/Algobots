    ...
    def generate_trading_signal(self, current_price: Decimal) -> TradeSignal:
        ...
        for name, weight_float in self.weights.items():
            ...
            if name == "ema_alignment":
                ...
                # This requires storing previous Ichimoku values across cycles, which would add state to Analyzer.
                # For compatibility/simplicity, using a simple check. A robust system would keep track of prev indicator state.
                # For now, a simplified check:
                if tenkan > kijun: # Current bullish alignment
                    raw_score += weight_dec * Decimal('0.3'); conditions_met.append("Ichimoku Bullish Alignment (Tenkan over Kijun)")
                elif tenkan < kijun: # Current bearish alignment
                    raw_score -= weight_dec * Decimal('0.3'); conditions_met.append("Ichimoku Bearish Alignment (Tenkan under Kijun)")
        ...
        # Removed the `ichimoku_prev` state logic for current compatibility as it requires state management across calls.
        # If true cross detection is desired, `TradingAnalyzer` needs to maintain state or receive it.
