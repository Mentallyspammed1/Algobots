    ...
    def supertrend(
        ...
        if len(close) > 0:
            final_upper_band.iloc[0] = (
                ...
                if not basic_upper_band.empty and not pd.isna(basic_upper_band.iloc[0])
                else Decimal("NaN")
            )
            final_lower_band.iloc[0] = (
                basic_lower_band.iloc[0]
                if not basic_lower_band.empty and not pd.isna(basic_lower_band.iloc[0])
                else Decimal("NaN")
            )

            # Decide initial supertrend value and trend direction
            if (
                not final_upper_band.iloc[0].is_nan()
                and not final_lower_band.iloc[0].is_nan()
                and not close.iloc[0].is_nan()
            ):
        ...
        for i in range(1, len(close)):
            if (
                pd.isna(basic_upper_band.iloc[i])
                or pd.isna(basic_lower_band.iloc[i])
                or pd.isna(final_upper_band.iloc[i - 1])
                or pd.isna(final_lower_band.iloc[i - 1])
                or pd.isna(close.iloc[i - 1])
                or pd.isna(close.iloc[i])
            ):
