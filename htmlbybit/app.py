...
def format_klines_for_gemini(klines_raw):
    ...
    if not isinstance(klines_raw, list) or (
        len(klines_raw) > 0 and not isinstance(klines_raw[0], (list, tuple))
    ):
    ...
    for indicator_name, values in indicators_raw.items():
        ...
        if isinstance(last_value, (int, float)):
            ...
            if indicator_name == "macd":
                if (
                    "macdLine" in last_value
                    and "signalLine" in last_value
                    and "histogram" in last_value
                ):
                    formatted[indicator_name] = {
                        "macdLine": f"{last_value['macdLine']:.6f}"
                        if last_value["macdLine"] is not None
                        else "N/A",
                        "signalLine": f"{last_value['signalLine']:.6f}"
                        if last_value["signalLine"] is not None
                        else "N/A",
                        "histogram": f"{last_value['histogram']:.6f}"
                        if last_value["histogram"] is not None
                        else "N/A",
                    }
            elif indicator_name == "bollinger":
                if (
                    "upper" in last_value
                    and "middle" in last_value
                    and "lower" in last_value
                ):
                    formatted[indicator_name] = {
                        "upper": f"{last_value['upper']:.6f}"
                        if last_value["upper"] is not None
                        else "N/A",
                        "middle": f"{last_value['middle']:.6f}"
                        if last_value["middle"] is not None
                        else "N/A",
                        "lower": f"{last_value['lower']:.6f}"
                        if last_value["lower"] is not None
                        else "N/A",
                    }
            elif indicator_name == "supertrend":
                if "line" in last_value and "trend" in last_value:
                    formatted[indicator_name] = {
                        "line": f"{last_value['line']:.6f}"
                        if last_value["line"] is not None
                        else "N/A",
                        "trend": "Up" if last_value["trend"] else "Down",
                    }
            elif indicator_name == "ichimoku":
                if (
                    "tenkan" in last_value
                    and "kijun" in last_value
                    and "senkouA" in last_value
                    and "senkouB" in last_value
                    and "chikou" in last_value
                ):
                    formatted[indicator_name] = {
                        "tenkan": f"{last_value['tenkan']:.6f}"
                        if last_value["tenkan"] is not None
                        else "N/A",
                        "kijun": f"{last_value['kijun']:.6f}"
                        if last_value["kijun"] is not None
                        else "N/A",
                        "senkouA": f"{last_value['senkouA']:.6f}"
                        if last_value["senkouA"] is not None
                        else "N/A",
                        "senkouB": f"{last_value['senkouB']:.6f}"
                        if last_value["senkouB"] is not None
                        else "N/A",
                        "chikou": f"{last_value['chikou']:.6f}"
                        if last_value["chikou"] is not None
                        else "N/A",
                    }
...
def analyze_signal_with_gemini():
    ...
    try:
        ...
        if indicators_raw_from_frontend is None:
            indicators_raw_from_frontend = {}
        if signals_raw_from_frontend is None:
