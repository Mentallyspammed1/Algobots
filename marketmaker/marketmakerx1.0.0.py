...
class BotState:
    ...
    recent_prices: deque = field(
        default_factory=lambda: deque(maxlen=240)
    )  # Stores last 4 minutes of 1s prices
...
class PositionManager:
    ...
    def process_virtual_fill(self, side: str, price: Decimal, qty: Decimal):
        ...
        logger.info(
            f"[DRY RUN] Fill simulated: {side} {qty} @ {price}. New Position: {self.size}"
        )
    ...
    async def check_and_manage_risk(self, state: BotState):
        ...
        if should_close:
            logger.warning(
                f"Risk Manager: Closing position due to: {reason}. Position Size: {self.size}"
            )
...
class EnhancedBybitMarketMaker:
    def __init__(self, config):
        ...
        if not self.api_key or not self.api_secret:
            raise ValueError(
                "API keys not found. Ensure a .env file exists in the script's directory with BYBIT_API_KEY and BYBIT_API_SECRET."
            )
    ...
    async def _api_call(self, method, **kwargs):
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                logger.error(
                    f"API Error ({method.__name__}): {msg} (retCode: {response.get('retCode')})"
                )
    ...
    async def check_circuit_breaker(self):
        ...
        if (
            ...
            if not self.state.is_circuit_breaker_active:
                ...
                logger.critical(
                    "[bold red]CIRCUIT BREAKER TRIGGERED: Too many consecutive API failures.[/bold red]"
                )
                await self.cancel_all_orders()
                cooldown = self.config["risk_management"][
                ...
                logger.info(
                    "[bold green]Circuit breaker cooldown finished. Resuming trading.[/bold green]"
                )
    ...
    async def fetch_instrument_info(self):
        ...
        if result and result.get("list"):
            ...
            logger.info(
                f"Instrument info fetched for {self.config['symbol']}: Tick Size={self.state.instrument_info.tick_size}, Step Size={self.state.instrument_info.step_size}"
            )
        else:
    ...
    async def place_and_cancel_orders_batch(self, orders_to_place, orders_to_cancel):
        ...
        if self.config["dry_run"]:
            ...
            if orders_to_cancel:
                logger.info(
                    f"[DRY RUN] Cancelled {len(orders_to_cancel)} virtual orders."
                )
            return
        ...
        if orders_to_place:
            ...
            place_result = await self._api_call(
                self.session.place_batch_order,
                category="linear",
                request=place_requests,
            )
            if place_result:
    ...
    async def close_position(self, side, qty):
        ...
        if self.config["dry_run"]:
            best_ask = (
                min(self.orderbook["asks"].keys())
                if self.orderbook["asks"]
                else self.state.recent_prices[-1]
                if self.state.recent_prices
                else Decimal("0")
            )
            best_bid = (
                max(self.orderbook["bids"].keys())
            ...
            logger.info(
                f"[DRY RUN] Market closed position: {side} {qty} @ {fill_price}"
            )
            return
        ...
        if close_result:
            logger.info(
                f"Market order placed to close position: {side} {formatted_qty}"
            )
        else:
            logger.error(
                f"Failed to place market order to close position: {side} {formatted_qty}"
            )
    ...
    async def _handle_message(self, msg):
        ...
        if topic.startswith("orderbook"):
            ...
            self.orderbook["bids"] = {
                Decimal(p): Decimal(q) for p, q in book_data.get("b", [])
            }
            self.orderbook["asks"] = {
                Decimal(p): Decimal(q) for p, q in book_data.get("a", [])
            }
            ...
            if topic == "position":
                ...
                for trade in data:
                    if (
                        ...
                        and trade.get("symbol") == self.config["symbol"]
                    ):
    ...
    def _calculate_tiered_quotes(self):
        ...
        if (
            len(self.state.recent_prices) >= 50
        ):  # Require sufficient data for meaningful std dev
            try:
                volatility = Decimal(
                    statistics.stdev(self.state.recent_prices) / mid_price
                )
            except statistics.StatisticsError:
                volatility = Decimal("0")  # Not enough unique values for std dev

        # Calculate dynamic spread
        total_spread_pct = Decimal(
            str(self.config["strategy"]["base_spread_percentage"])
        ) + volatility * Decimal(
            str(self.config["strategy"]["volatility_spread_multiplier"])
        )

        # Calculate inventory skew
        max_pos_size = Decimal(self.config["risk_management"]["max_position_size"])
        skew_intensity = Decimal(
            str(self.config["strategy"]["inventory_skew_intensity"])
        )

        # Skew factor between -1 and 1
        skew_factor = Decimal("0")
        if max_pos_size > 0:
            skew_factor = (self.position_manager.size / max_pos_size).clamp(
                Decimal("-1"), Decimal("1")
            )
        ...
        for i in range(self.config["order_management"]["order_tiers"]):
            tier_spread_adj = (
                Decimal(str(i))
                * Decimal(
                    str(self.config["order_management"]["tier_spread_increase_bps"])
                )
                / Decimal("10000")
            )

            # Calculate bid and ask prices
            bid_price = self._format_price(
                fair_value * (Decimal("1") - total_spread_pct - tier_spread_adj)
            )
            ask_price = self._format_price(
                fair_value * (Decimal("1") + total_spread_pct + tier_spread_adj)
            )

            # Adjust quantity for tiered orders
            qty = self._format_qty(
                base_order_size
                * Decimal(
                    str(1 + i * self.config["order_management"]["tier_qty_multiplier"])
                )
            )

            # Only place orders if they don't exceed max position size
            current_pos_abs = abs(self.position_manager.size)
            remaining_buy_capacity = max_pos_size - (
                current_pos_abs if self.position_manager.size > 0 else Decimal("0")
            )
            remaining_sell_capacity = max_pos_size - (
                current_pos_abs if self.position_manager.size < 0 else Decimal("0")
            )
    ...
    def generate_status_table(self) -> Table:
        ...
        table.add_row(
            "Position Size", f"[{pos_color}]{self.position_manager.size}[/{pos_color}]"
        )
        ...
        table.add_row(
            "Unrealized PNL",
            f"[{pnl_color}]{self.position_manager.unrealized_pnl:.4f}[/{pnl_color}]",
        )
        table.add_row(
            "Realized PNL",
            f"[{pnl_color}]{self.position_manager.realized_pnl:.4f}[/{pnl_color}]",
        )  # Added realized PNL

        best_bid = (
            max(self.orderbook["bids"].keys()) if self.orderbook["bids"] else "N/A"
        )
        best_ask = (
            min(self.orderbook["asks"].keys()) if self.orderbook["asks"] else "N/A"
        )
        table.add_row("Market Bid / Ask", f"{best_bid} / {best_ask}")

        open_orders = (
            self.virtual_orders if self.config["dry_run"] else self.active_orders
        )
        table.add_row(
    ...
    async def _websocket_stream_manager(
        self, ws_client: WebSocket, topics: list[str], stream_name: str
    ):
            ...
            try:
                ...
                async with websockets.connect(
                    ...
                    if "private" in stream_name:
                        ...
                        expires = int(
                            (time.time() + 10) * 1000
                        )  # Expiration time for signature
                        signature = ws_client._auth(
                            expires
                        )  # Use pybit's internal auth method
                        auth_payload = {
                            "op": "auth",
                        ...
                        if auth_response.get("success"):
                            logger.info(
                                f"{stream_name} WebSocket authenticated successfully."
                            )
                        else:
                            logger.error(
                                f"{stream_name} WebSocket authentication failed: {auth_response}"
                            )
                    ...
                    if sub_response.get("success"):
                        logger.info(
                            f"{stream_name} WebSocket subscribed to topics: {topics}"
                        )
                    else:
                        logger.error(
                            f"{stream_name} WebSocket subscription failed: {sub_response}"
                        )
                        raise websockets.exceptions.ConnectionClosedOK(
                            "Subscription Failed"
                        )
                ...
                logger.warning(
                    f"{stream_name} WebSocket connection lost: {e}. Reconnecting in 5s..."
                )
            except Exception as e:
                logger.error(
                    f"An unexpected error occurred in {stream_name} WebSocket manager: {e}",
                    exc_info=True,
                )
    ...
    async def run(self):
        ...
        mode_text = (
            ...
            if self.config["dry_run"]
            else "[bold green]LIVE TRADING[/bold green]"
        )
        env_text = "Testnet" if self.config["testnet"] else "Mainnet"
        logger.info(
            f"Starting bot for [bold]{self.config['symbol']}[/bold] on {env_text} in {mode_text} mode."
        )
        ...
        websocket_tasks = [
            asyncio.create_task(
                self._websocket_stream_manager(
                    public_ws_client, public_topics, "public"
                )
            )
        ]
        ...
        if not self.config["dry_run"]:
            ...
            websocket_tasks.append(
                asyncio.create_task(
                    self._websocket_stream_manager(
                        private_ws_client, private_topics, "private"
                    )
                )
            )
        ...
        if not self.state.recent_prices:
            logger.warning(
                "No ticker data received yet, strategy might be less effective initially."
            )
        if not self.orderbook["bids"] or not self.orderbook["asks"]:
            logger.warning(
                "No orderbook data received yet, market making will be paused."
            )
            ...
            while True:
                try:
                    ...
                    if await self.position_manager.check_and_manage_risk(self.state):
                        ...
                        )  # Cancel all orders if position closed
                        logger.warning(
                            "Position closed by risk manager. Pausing 60s for market to settle."
                        )
                    ...
                    if (not buy_orders and not sell_orders) and open_orders:
                        logger.info(
                            "No new quotes generated, cancelling all existing orders."
                        )
                        await self.cancel_all_orders()
                        await asyncio.sleep(
                            0.5
                        )  # Small pause before next re-evaluation
                        continue

                    # UPGRADE: Only reprice if quotes have significantly changed OR if there are no open orders
                    # This reduces unnecessary API calls.
                    current_quotes = sorted(
                        [
                    ...
                    if current_quotes != new_quotes or not open_orders:
                        logger.info(
                            f"Repricing: {len(buy_orders)} buys, {len(sell_orders)} sells. Cancelling {len(open_orders)} existing."
                        )
                        await self.place_and_cancel_orders_batch(
                            buy_orders + sell_orders, list(open_orders.keys())
                        )
...
async def main():
    try:
        ...
        logger.critical(
            "CRITICAL: config.json not found. Please create it in the same directory as the script."
        )
        return
    except json.JSONDecodeError as e:
        logger.critical(
            f"CRITICAL: Error parsing config.json: {e}. Please check your JSON syntax."
        )
        return
    ...
    if hasattr(signal, "SIGINT") and hasattr(signal, "SIGTERM"):
        for sig in [signal.SIGINT, signal.SIGTERM]:
            try:
                ...
                logger.warning(
                    "Signal handlers not fully supported on this system (e.g., Windows). Use Ctrl+C to stop."
                )
    else:
        logger.warning(
            "Signal handlers not available on this system. Use Ctrl+C to stop."
        )
