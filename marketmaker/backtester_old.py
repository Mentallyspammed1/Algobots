...
class BacktestEngine:
    ...
    def execute_order(self, side: str, price: float, size: float) -> Trade | None:
        ...
        if side.lower() == "buy":
            ...
            if self.balance < cost + fee:
                logger.warning(
                    f"Insufficient balance for buy order: {cost + fee:.2f} > {self.balance:.2f}"
                )
                return None
            ...
            if self.market_maker.position < size:
                logger.warning(
                    f"Insufficient position for sell order: {size:.4f} > {self.market_maker.position:.4f}"
                )
                return None
        ...
        if side.lower() == "buy":
            if self.market_maker.position > 0:
                total_cost = (
                    self.market_maker.avg_entry_price
                    * (self.market_maker.position - size)
                    + execution_price * size
                )
                self.market_maker.avg_entry_price = (
                    total_cost / self.market_maker.position
                )
            else:
        ...
        for order_id, order in list(self.market_maker.active_orders["buy"].items()):
            if current_price <= order["price"]:
                ...
                if trade:
                    ...
                    logger.debug(
                        f"Buy order filled: {order['size']:.4f} @ {order['price']:.2f}"
                    )
        ...
        for order_id, order in list(self.market_maker.active_orders["sell"].items()):
            if current_price >= order["price"]:
                ...
                if trade:
                    filled_orders.append(order_id)
                    logger.debug(
                        f"Sell order filled: {order['size']:.4f} @ {order['price']:.2f}"
                    )
    ...
    def update_market_data(self, row):
        ...
        self.market_maker.orderbook["bid"] = [
            (float(b[0]), float(b[1])) for b in orderbook["b"]
        ]
        self.market_maker.orderbook["ask"] = [
            (float(a[0]), float(a[1])) for a in orderbook["a"]
        ]
    ...
    def calculate_metrics(self) -> dict:
        ...
        win_rate = (
            len(profitable_trades) / len(df_trades) * 100 if len(df_trades) > 0 else 0
        )
        ...
        final_equity = self.balance + (
            self.market_maker.position * self.market_maker.last_price
            if self.market_maker.position != 0
            else 0
        )
        return_pct = (
    ...
    async def run_backtest(self):
        ...
        for index, row in self.historical_data.iterrows():
            ...
            if index % 100 == 0:
                logger.info(
                    f"Progress: {index}/{len(self.historical_data)} | "
    ...
    def save_results(self, filename: str = "backtest_results.csv"):
        ...
        df_equity = pd.DataFrame(
            {
                "timestamp": self.historical_data["timestamp"][
