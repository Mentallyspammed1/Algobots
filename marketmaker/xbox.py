            ...
            try:
                ...
                for s_cfg_data in raw_symbol_configs:
                    try:
                        ...
                        for strat_field, default_value in (
                            StrategyConfig().model_dump().items()
                        ):
    ...
    async def _process_private_message(self, message: dict[str, Any]):
        ...
        for item_data in data:
            ...
            if symbol in self.symbol_bots:
                ...
                self.logger.debug(
                    f"Received {topic} update for unmanaged symbol: {symbol}"
                )
        ...
        try:
            ...
            for topic in topics:
                if topic.startswith("orderbook."):
                    ...
                    parts = topic.split(".")
                    symbol_ws = parts[1]
                    ws_instance.public_trade_stream(
...
class AsyncSymbolBot:
    ...
    async def initialize(self):
        ...
        if self.state.daily_initial_capital == DECIMAL_ZERO or (
            ...
            self.logger.info(
                f"[{self.config.symbol}] Daily initial capital set to: {self.state.daily_initial_capital} {self.config.quote_currency}"
            )
    ...
    async def _process_execution_update(self, trade_data: dict[str, Any]):
        ...
        realized_pnl_impact = Decimal(
            "0"
        )  # PnL directly from this specific trade (used for gross profit/loss)
