    ...
    async def load_state(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Could not rename corrupted state file: {ose}")
            return None
...
class BybitWebSocketClient:
    ...
    def __init__(self, global_config: GlobalConfig, logger: logging.Logger):
        ...
        self.ws_trade: Optional[WebSocket] = None # pybit unified trading uses a single private stream for order/exec/position
    ...
    async def _process_ws_messages(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                self.state.last_ws_message_time = time.time() # Update main bot's last WS time
                if 'topic' in message:
                    topic = message['topic']
        ...
        for item_data in data:
            ...
            if symbol in self.symbol_bots:
                ...
                if topic == 'order':
                    ...
                    # Global wallet update, handled by main bot
                    pass
            else:
    ...
    async def start_streams(self, public_topics: List[str], private_topics: List[str]):
        ...
        if public_topics:
            ...
            self.ws_public = WebSocket(testnet=self.config.testnet, channel_type="linear") # Assuming linear for simplicity
            
            # Iterate through public_topics and subscribe correctly
            for topic in public_topics:
                if topic.startswith("orderbook."):
                    parts = topic.split('.')
                    if len(parts) >= 3:
                        symbol_ws = parts[2] # e.g., "BTCUSDT"
                        depth = int(parts[1]) # e.g., 50
        ...
        # This part seems generally correct for pybit, as it uses dedicated methods.
        if self.config.api_key and self.config.api_secret and private_topics:
            ...
            # pybit's private streams are usually handled by dedicated methods, but can also use a generic subscribe
            # For simplicity, we'll assume the private WS client handles all private topics.
            # The general private stream handles 'order', 'position', 'execution', 'wallet'
            # The private_topics list is not directly used here for subscription, but rather implies which streams to enable.
            # If the private_topics list is meant to control which specific private streams are enabled,
            # then this logic needs to be adjusted. For now, assuming all standard private streams are desired.
...
class AsyncSymbolBot:
    ...
    async def _check_and_handle_stale_orders(self, current_time: float):
        ...
        for exchange_id, client_order_id in orders_to_cancel:
            self.logger.warning(
                f"[{self.config.symbol}] Order {client_order_id} is stale (open for "
            ...
            if order_id.startswith('DRY_'):
                ...
                if filled:
                    ...
                    if side == 'Sell' and fill_qty > self.state.metrics.current_asset_holdings:
                        self.logger.warning(f"[{self.config.symbol}] DRY_RUN: Skipping simulated sell fill for order {order_id} (Qty: {fill_qty}) due to insufficient simulated holdings ({self.state.metrics.current_asset_holdings}).")
                        continue
...
class PyrmethusBot:
    ...
    async def _start_symbol_bots(self, symbol_configs: Dict[str, SymbolConfig]):
        ...
        for symbol in symbols_currently_active:
            if symbol not in symbols_in_new_config or not symbol_configs[symbol].trade_enabled:
                ...
                await self.symbol_bots[symbol].run_loop() # Allow it to finish its loop
    ...
    async def _stop_all_bots(self):
        ...
        for symbol, bot in list(self.symbol_bots.items()):
            bot.stop()
            await bot.run_loop() # Allow tasks to complete their shutdown logic
            self.ws_client.unregister_symbol_bot(symbol)
            del self.symbol_bots[symbol]
            del self.active_symbol_configs[symbol]
            ...
            try:
                if (time.time() - last_config_check_time) > self.global_config.system.config_refresh_interval_sec:
                    ...
                    reloaded_global_cfg, reloaded_symbol_configs = ConfigManager.load_config(
                        single_symbol=None if len(self.active_symbol_configs) > 1 else list(self.active_symbol_configs.keys())[0] if self.active_symbol_configs else None
                    )
    ...
    async def run(self):
        ...
        try:
            ...
            self.logger.warning(f"{Colors.YELLOW}No interactive input detected. Defaulting to single symbol BTC/USDT:USDT.{Colors.RESET}")
            selected_mode = 's'
