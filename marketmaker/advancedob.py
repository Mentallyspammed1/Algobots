...
import logging
import threading
from typing import Dict, List, Tuple, Optional, Any, Generic, TypeVar, Callable
...
class AdvancedOrderbookManager:
    ...
    def _validate_price_quantity(self, price: float, quantity: float) -> bool:
        ...
        if not (isinstance(price, (int, float)) and isinstance(quantity, (int, float))):
            logger.error(f"[{self.symbol}] Invalid type for price or quantity. Price: {type(price)}, Qty: {type(quantity)}")
            return False
        if price < 0 or quantity < 0:
            logger.error(f"[{self.symbol}] Negative price or quantity detected: price={price}, quantity={quantity}")
            return False
        return True
    ...
    def _validate_sequence(self, sequence: int) -> bool:
        ...
        if not isinstance(sequence, int):
            logger.error(f"[{self.symbol}] Invalid type for sequence number: {type(sequence)}")
            return False
        if sequence <= self.last_sequence:
            logger.warning(f"[{self.symbol}] Out-of-order sequence received: received={sequence}, last={self.last_sequence}. Skipping update.")
            return False
        ...
        try:
            with self._lock_context():
                ...
                if not isinstance(snapshot_data, dict) or 'b' not in snapshot_data or 'a' not in snapshot_data or 'u' not in snapshot_data:
                    logger.error(f"[{self.symbol}] Invalid snapshot data format: {snapshot_data}")
                ...
                for price_str, qty_str in snapshot_data.get("b", []):
                    try:
                        ...
                        if not self._validate_price_quantity(price, quantity):
                            logger.warning(f"[{self.symbol}] Invalid bid price/quantity in snapshot: {price_str}/{qty_str}. Skipping.")
                            continue
                        ...
                        logger.error(f"[{self.symbol}] Failed to parse bid data in snapshot: price={price_str}, quantity={qty_str}, error={e}")
                ...
                for price_str, qty_str in snapshot_data.get("a", []):
                    try:
                        ...
                        quantity = float(qty_str)
                        if not self._validate_price_quantity(price, quantity):
                            logger.warning(f"[{self.symbol}] Invalid ask price/quantity in snapshot: {price_str}/{qty_str}. Skipping.")
                            continue
                        if quantity > 0: # Only add if quantity is positive
                            level = PriceLevel(price, quantity, int(time.time() * 1000))
                            if self.use_skip_list:
                        ...
                        logger.error(f"[{self.symbol}] Failed to parse ask data in snapshot: price={price_str}, quantity={qty_str}, error={e}")
                ...
                logger.info(f"[{self.symbol}] Processed snapshot. New update_id={self.last_update_id}, sequence={self.last_sequence}.")
        
        except Exception as e:
            logger.critical(f"[{self.symbol}] CRITICAL ERROR processing snapshot: {e}", exc_info=True)
        ...
        try:
            with self._lock_context():
                ...
                if not isinstance(delta_data, dict) or not ('b' in delta_data or 'a' in delta_data):
                    logger.error(f"[{self.symbol}] Invalid delta data format: {delta_data}")
                ...
                if not self._validate_sequence(sequence):
                    ...
                    logger.warning(f"[{self.symbol}] Invalid sequence detected. Delta not processed.")
                ...
                for price_str, qty_str in delta_data.get("b", []):
                    try:
                        ...
                        quantity = float(qty_str)
                        if not self._validate_price_quantity(price, quantity):
                            logger.warning(f"[{self.symbol}] Invalid bid price/quantity in delta: {price_str}/{qty_str}. Skipping.")
                            continue
                        ...
                        logger.error(f"[{self.symbol}] Failed to parse bid delta data: price={price_str}, quantity={qty_str}, error={e}")
                ...
                for price_str, qty_str in delta_data.get("a", []):
                    try:
                        ...
                        quantity = float(qty_str)
                        if not self._validate_price_quantity(price, quantity):
                            logger.warning(f"[{self.symbol}] Invalid ask price/quantity in delta: {price_str}/{qty_str}. Skipping.")
                            continue
                        ...
                        logger.error(f"[{self.symbol}] Failed to parse ask delta data: price={price_str}, quantity={qty_str}, error={e}")
                ...
                self.update_count += 1
                self.total_update_time += time.perf_counter() - start_time
                logger.info(f"[{self.symbol}] Processed delta. New update_id={self.last_update_id}, sequence={self.last_sequence}.")
    ...
    def best_bid(self) -> Optional[PriceLevel]:
        ...
        with self._lock_context():
            if self.use_skip_list:
                ...
                if self.bids._size > 0: # Check if heap is not empty before attempting to extract
    ...
    def best_ask(self) -> Optional[PriceLevel]:
        ...
        with self._lock_context():
            if self.use_skip_list:
                ...
                if self.asks._size > 0: # Check if heap is not empty before attempting to extract
    ...
    def validate_orderbook(self) -> bool:
        ...
        with self._lock_context():
            ...
            if (self.use_skip_list and self.bids._size == 0 and self.asks._size == 0) or \
            ...
            if best_bid_level and best_ask_level:
                if best_bid_level.price >= best_ask_level.price:
                    logger.error(f"[{self.symbol}] Orderbook invalid: Inverted bid-ask spread! Best Bid: {best_bid_level.price}, Best Ask: {best_ask_level.price}")
                    is_valid = False
            ...
            if self.use_skip_list:
                ...
                if len(bid_prices) != self.bids._size: # Compare against internal size
                    logger.error(f"[{self.symbol}] Orderbook invalid: Duplicate bid price levels detected (SkipList).")
                    is_valid = False
                
                if len(ask_prices) != self.asks._size: # Compare against internal size
                    logger.error(f"[{self.symbol}] Orderbook invalid: Duplicate ask price levels detected (SkipList).")
                    is_valid = False
...
class BybitOrderbookHelper:
    ...
    def __init__(self,
        ...
        self.websocket_client: Optional[WebSocket] = None
        ...
        logger.info(f"Initialized BybitOrderbookHelper for {self.symbol} ({self.category}). Testnet: {self.testnet}")
    ...
    def _get_http_snapshot(self) -> None:
        ...
        """
        try:
            http_session = HTTP(testnet=self.testnet, api_key=self.api_key, api_secret=self.api_secret)
            ...
            if response and response['retCode'] == 0:
                ...
                snapshot_data = {
                    ...
                    'seq': data.get('u', 0), # Using `u` as `seq` for consistency with AdvancedOrderbookManager expects
                    'ts': data.get('ts', int(time.time() * 1000))
                }
                self.orderbook_manager.process_snapshot(snapshot_data)
                self._initial_http_snapshot_retrieved = True
                logger.info(f"[{self.symbol}] Successfully retrieved initial HTTP orderbook snapshot.")
            else:
                logger.error(f"[{self.symbol}] Failed to get HTTP snapshot: {response.get('retMsg', 'Unknown error')}. Response: {response}")
                raise Exception(f"Failed to get HTTP snapshot for {self.symbol}")
        except Exception as e:
            logger.error(f"[{self.symbol}] Error fetching initial HTTP snapshot: {e}", exc_info=True)
            raise
    ...
    def _on_orderbook_update(self, message: Dict[str, Any]) -> None:
        ...
        if message and message.get('type') == 'snapshot':
            ...
            try:
                ...
                logger.info(f"[{self.symbol}] Internal orderbook updated with WebSocket snapshot. Ready for deltas.")
            except Exception as e:
                logger.error(f"[{self.symbol}] Error processing WebSocket snapshot: {e}", exc_info=True)
            ...
            if not self._is_snapshot_received.is_set():
                logger.warning(f"[{self.symbol}] Received delta before snapshot. Skipping. (Current manager sequence: {self.orderbook_manager.last_sequence})")
                return
            ...
            delta_data = {
                ...
                'seq': data.get('seq', self.orderbook_manager.last_sequence + 1), # Sequence number from WS
                'ts': message.get('ts', int(time.time() * 1000))
            }
            ...
            try:
                ...
                if delta_data['seq'] > self.orderbook_manager.last_sequence:
                    ...
                    logger.warning(f"[{self.symbol}] Out-of-order delta received from WebSocket: Message Seq {delta_data['seq']} <= Manager Seq {self.orderbook_manager.last_sequence}. Skipping.")

            except ValueError as ve:
                logger.error(f"[{self.symbol}] Orderbook processing error from delta: {ve}. Attempting to re-sync.", exc_info=True)
                ...
                logger.error(f"[{self.symbol}] Generic error processing WebSocket delta: {e}", exc_info=True)
    ...
    def start_websocket_stream(self) -> None:
        ...
        if not self._initial_http_snapshot_retrieved:
            logger.info(f"[{self.symbol}] Fetching initial HTTP snapshot before starting WebSocket.")
            try:
                self._get_http_snapshot()
            except Exception as e:
                logger.error(f"[{self.symbol}] Failed to get initial HTTP snapshot. Cannot start WebSocket stream. Error: {e}")
                return

        # Ensure WebSocket is not already running or stop it cleanly if it is
        if self.websocket_client and self.websocket_client.is_connected():
            logger.warning(f"[{self.symbol}] WebSocket client already running. Stopping before restarting.")
        ...
        logger.info(f"[{self.symbol}] Subscribed to orderbook stream (depth {self.orderbook_stream_depth}). Waiting for WebSocket snapshot...")
        ...
        if not self._is_snapshot_received.wait(timeout=30): # Wait up to 30 seconds
            logger.error(f"[{self.symbol}] Timeout waiting for initial WebSocket orderbook snapshot. Orderbook might be inconsistent. Consider reconnecting.")
            # Depending on robustness requirements, may need to reconnect or raise error.
        else:
            logger.info(f"[{self.symbol}] Initial WebSocket snapshot received and processed.")
    ...
    def get_best_bid_ask(self) -> Tuple[Optional[PriceLevel], Optional[PriceLevel]]:
        ...
        """
        if not self._is_snapshot_received.is_set():
            logger.warning(f"[{self.symbol}] Orderbook not yet ready (no snapshot received). Returning None for best bid/ask.")
            return None, None
    ...
    def get_orderbook_depth(self, depth: int = 10) -> Tuple[List[PriceLevel], List[PriceLevel]]:
        ...
        """
        if not self._is_snapshot_received.is_set():
            logger.warning(f"[{self.symbol}] Orderbook not yet ready (no snapshot received). Returning empty lists for depth.")
            return [], []
    ...
    def validate_orderbook_integrity(self) -> bool:
        ...
        """
        if not self._is_snapshot_received.is_set():
            logger.warning(f"[{self.symbol}] Cannot validate orderbook integrity: no snapshot received yet.")
            return False
    ...
    def get_instrument_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response and response['retCode'] == 0:
                ...
                return response['result']['list'][0] if response['result']['list'] else {}
            else:
                logger.error(f"[{self.symbol}] Failed to get instrument info: {response.get('retMsg', 'Unknown error')}")
                return {}
        except Exception as e:
            logger.error(f"[{self.symbol}] Error fetching instrument info: {e}", exc_info=True)
            return {}
...
if __name__ == "__main__":
    ...
    try:
        ...
        if best_bid and best_ask:
            ...
            print(f"Current Best Ask ({ob_helper.symbol}): {best_ask.price} @ {best_ask.quantity}")
        ...
        for level in bids_depth:
            print(f"Price: {level.price}, Quantity: {level.quantity}, Timestamp: {level.timestamp}")
        ...
        if instrument_info:
            ...
            print(f"  Min Order Qty: {instrument_info.get('lotSizeFilter', {}).get('minOrderQty')}")
        else:
