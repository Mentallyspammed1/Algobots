...
class EnhancedBybitMarketMaker:
    ...
    def _start_streams(self):
        ...
        try:
            if hasattr(self.ws_public, "subscribe"):
                self.ws_public.subscribe(f"orderbook.{depth}.{symbol}", self._ws_callback)
    ...
    def _update_ta(self):
        ...
        try:
            ...
            ema_f = float(ema_f_series.iloc[-1]) if not pd.isna(ema_f_series.iloc[-1]) else None
            ema_s = float(ema_s_series.iloc[-1]) if not pd.isna(ema_s_series.iloc[-1]) else None
    ...
    def generate_status_table(self) -> Table:
        ...
        if self.ta_cfg.get("enabled"):
            if not PANDAS_TA_AVAILABLE:
                ...
                trend = "Up" if (ema_f is not None and ema_s is not None and ema_f > ema_s) else \
                        "Down" if (ema_f is not None and ema_s is not None and ema_f < ema_s) else "N/A"
