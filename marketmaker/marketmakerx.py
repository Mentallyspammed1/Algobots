...
class PositionManager:
    ...
    def process_virtual_fill(self, side: str, price: Decimal, qty: Decimal):
        ...
        logger.info(
            f"[DRY RUN] Fill: {side} {qty} @ {price}. Position: {old_size} → {self.size}"
        )
    ...
    async def check_and_manage_risk(self, state: BotState):
        ...
        try:
            position_value = abs(self.size) * (
                self.avg_entry_price if self.avg_entry_price != 0 else Decimal("1")
            )
            pnl_pct = (
        ...
        if should_close:
            ...
            logger.warning(
                f"Position: {self.size}, Unrealized PnL: {self.unrealized_pnl:.4f}"
            )
...
class EnhancedBybitMarketMaker:
    def __init__(self, config):
        ...
        self.api_key = (
            ...
            or str(config.get("credentials", {}).get("api_key", "")).strip()
        )
        self.api_secret = (
            os.getenv("BYBIT_API_SECRET")
            or str(config.get("credentials", {}).get("api_secret", "")).strip()
        )
        ...
        if not self.api_key or not self.api_secret:
            ...
            raise ValueError(
                "API keys not found. Ensure a .env file exists with BYBIT_API_KEY and BYBIT_API_SECRET."
            )
        ...
        self.price_tol_ticks = Decimal(
            str(config["order_management"].get("price_tolerance_ticks", 0.5))
        )
        self.qty_tol_steps = Decimal(
            str(config["order_management"].get("qty_tolerance_steps", 0))
        )
        self.mid_drift_threshold = Decimal(
            str(config["order_management"].get("mid_drift_threshold_ticks", 0.5))
        )
        ...
        if self.ta_cfg.get("enabled") and not PANDAS_TA_AVAILABLE:
            logger.warning(
                "TA enabled in config but pandas/pandas_ta not installed. TA features disabled."
            )
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if code == 10006 or (msg and "rate" in str(msg).lower()):
                    logger.warning(
                        f"Rate limit hit. Backing off {self._rate_limit_backoff_sec}s."
                    )
                    await asyncio.sleep(self._rate_limit_backoff_sec)
                    self._rate_limit_backoff_sec = min(
                        self._rate_limit_backoff_sec * 2, self._max_backoff_sec
                    )
                else:
                    logger.error(
    ...
    async def fetch_instrument_info(self):
        ...
        if result and result.get("list"):
            ...
            logger.info(
                f"Instrument: tick={ii.tick_size}, step={ii.step_size}, min={ii.min_order_size}"
            )
        else:
            logger.error(f"Failed to fetch instrument info for {self.config['symbol']}")

    async def place_and_cancel_orders_batch(
        self, orders_to_place: list[dict], orders_to_cancel: list[str]
    ):
        if self.config["dry_run"]:
            ...
            if orders_to_place or orders_to_cancel:
                logger.info(
                    f"[DRY RUN] Orders: +{len(orders_to_place)} -{len(orders_to_cancel)}"
                )
            return
    ...
    # ------------- WebSocket handling using pybit callbacks -------------
    def _ws_callback(self, message: dict):
        try:
            if self.loop:
                asyncio.run_coroutine_threadsafe(
                    self._handle_message(message), self.loop
                )
        except Exception as e:
        ...
        try:
            if hasattr(self.ws_public, "subscribe"):
                self.ws_public.subscribe(
                    f"orderbook.{depth}.{symbol}", self._ws_callback
                )
                self.ws_public.subscribe(f"tickers.{symbol}", self._ws_callback)
            else:
        ...
        if not self.config["dry_run"]:
            try:
                ...
                if hasattr(self.ws_private, "subscribe"):
                    self.ws_private.subscribe("position", self._ws_callback)
                    self.ws_private.subscribe("order", self._ws_callback)
                    self.ws_private.subscribe("execution", self._ws_callback)
                else:
                ...
                logger.critical(
                    f"Failed to start private WS subscriptions: {e}", exc_info=True
                )
                raise
    ...
    def _update_ta(self):
        ...
        try:
            ...
            ema_f = (
                ...
                if not pd.isna(ema_f_series.iloc[-1])
                else None
            )
            ema_s = (
                float(ema_s_series.iloc[-1])
                if not pd.isna(ema_s_series.iloc[-1])
                else None
            )
    ...
    def _format_price(self, p: Decimal) -> Decimal:
        ...
        return round_to_increment(
            p, self.state.instrument_info.tick_size, ROUND_HALF_UP
        )
    ...
    def _quotes_significantly_changed(self, current: list, new: list) -> bool:
        ...
        qty_tol = (
            ii.step_size * self.qty_tol_steps
            if self.qty_tol_steps > 0
            else Decimal("0")
        )
    ...
    def _calculate_tiered_quotes(self) -> tuple[list[dict], list[dict]]:
        ...
        if len(self.state.recent_prices) >= vol_window:
            try:
                prices_f = [
                    float(p) for p in list(self.state.recent_prices)[-vol_window:]
                ]
        ...
        if rsi is not None:
            signal = clamp_decimal(
                Decimal((rsi - 50.0) / 50.0), Decimal("-1"), Decimal("1")
            )
        ...
        if bbw is not None:
            try:
                ...
                total_spread_pct += bbw_dec * Decimal(
                    str(self.ta_cfg.get("bb_spread_mult", 0.5))
                )
            except Exception:
                pass
        ...
        tier_spread_bps = Decimal(
            str(self.config["order_management"]["tier_spread_increase_bps"])
        )
        tier_qty_mult = Decimal(
            str(self.config["order_management"]["tier_qty_multiplier"])
        )
        ...
        open_sell_qty = sum(
            o["qty"] for o in open_orders.values() if o["side"] == "Sell"
        )
        ...
        # If EMAs available, use their relation for direction; else fallback to RSI signal
        ...
        for i in range(tiers):
            ...
            bid_price = self._format_price(
                fair_value * (Decimal("1") - total_spread_pct - tier_adj)
            )
    ...
    def generate_status_table(self) -> Table:
        ...
        status = (
            "[bold red]CIRCUIT BREAKER[/bold red]"
            if self.state.is_circuit_breaker_active
            else "[bold green]Active[/bold green]"
        )
        ...
        table.add_row(
            "Position", f"[{pos_color}]{self.position_manager.size}[/{pos_color}]"
        )
    ...
    async def run(self):
        ...
        with Live(
            self.generate_status_table(), screen=True, refresh_per_second=2
        ) as live:
            while True:
                try:
                    ...
                    current_quotes = [
                        (o["price"], o["qty"], o["side"]) for o in open_orders.values()
                    ]
                    new_quotes = [
