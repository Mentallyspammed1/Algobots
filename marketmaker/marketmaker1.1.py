...
class TradeMetrics:
    ...
    def update_pnl_on_sell(self, quantity: Decimal, price: Decimal):
        ...
        if self.current_asset_holdings == Decimal('0'):
            self.average_entry_price = Decimal('0') # Reset average entry if all assets sold
        self.last_pnl_update_timestamp = datetime.now(timezone.utc)
...
class StrategyConfig:
    ...
    order_stale_threshold_pct: Decimal = Decimal('0.0005') # % change to consider an order stale
...
class SystemConfig:
    ...
    health_check_interval_sec: int = 10 # How often to re-fetch balance/position via HTTP
...
class Config:
    ...
    max_order_size_pct: Decimal = Decimal('0.1') # Max percentage of effective capital for a single order
    max_net_exposure_usd: Decimal = Decimal('500') # Max total USD value of open position
    ...
    initial_dry_run_capital: Decimal = Decimal('10000') # Virtual capital for DRY_RUN/SIMULATION
...
class TradingState:
    ...
    current_position_qty: Decimal = Decimal('0') # Current position in base currency (positive for long, negative for short)
    unrealized_pnl_derivatives: Decimal = Decimal('0') # Unrealized PnL from derivatives, reported by exchange
    ...
    circuit_breaker_cooldown_end_time: float = 0.0 # Timestamp when cooldown after pause ends
    ws_reconnect_attempts_left: int = 0 # Remaining reconnection attempts

    metrics: TradeMetrics = field(default_factory=TradeMetrics) # Trading performance metrics
    ...
    async def log_order_event(self, order_data: dict, message: str | None = None):
        ...
        try:
            await self.conn.execute(
                ...
                (datetime.now(timezone.utc).isoformat(), order_data.get('orderId'), order_data.get('orderLinkId'), order_data.get('symbol'), order_data.get('side'), order_data.get('orderType'), str(order_data.get('price','0')), str(order_data.get('qty','0')), order_data.get('orderStatus'), message)
            )
            await self.conn.commit()
        except Exception as e:
    ...
    async def log_trade_fill(self, trade_data: dict, realized_pnl_impact: Decimal):
        ...
        try:
            await self.conn.execute(
                ...
                (datetime.now(timezone.utc).isoformat(), trade_data.get('orderId'), trade_data.get('execId'), trade_data.get('symbol'), trade_data.get('side'), str(trade_data.get('execPrice','0')), str(trade_data.get('execQty','0')), str(trade_data.get('execFee','0')), trade_data.get('feeCurrency'), str(trade_data.get('pnl','0')), str(realized_pnl_impact))
            )
            await self.conn.commit()
        except Exception as e:
...
class TradingClient:
    ...
    async def get_instruments_info_impl(self) -> dict | None:
        ...
        result = self._handle_response(self.http_session.get_instruments_info(
    ...
    async def get_wallet_balance_impl(self) -> dict | None:
        ...
        result = self._handle_response(self.http_session.get_wallet_balance(
    ...
    async def set_leverage_impl(self, leverage: Decimal) -> bool:
        ...
        if self.config.category not in ['linear', 'inverse']: return True # Leverage not applicable for spot
    ...
    async def get_open_orders_impl(self) -> list[dict]:
        ...
        result = self._handle_response(self.http_session.get_open_orders(
    ...
    async def place_order_impl(self, params: dict) -> dict | None:
        ...
        return self._handle_response(self.http_session.place_order(**params),
...
class BybitMarketMaker:
    ...
    def __init__(self, config: Config):
        ...
        self.state = TradingState(
            ...
            mid_price_history=deque(maxlen=self.config.strategy.dynamic_spread.volatility_window_sec * 2),
            circuit_breaker_price_points=deque(maxlen=self.config.strategy.circuit_breaker.check_window_sec * 2)
        )

        self.market_info: MarketInfo | None = None # Stores symbol-specific market rules
        ...
        self.loop: asyncio.AbstractEventLoop | None = None # Reference to the main event loop
    ...
    async def _load_bot_state(self):
        ...
        if state_data:
            ...
            for attr in ['realized_pnl', 'gross_profit', 'gross_loss', 'total_fees', 'current_asset_holdings', 'average_entry_price']:
                current_value = getattr(self.state.metrics, attr)
                if isinstance(current_value, (str, float)): # Check for string or float if not Decimal already
            ...
            async with self.market_data_lock:
                ...
                self.state.mid_price_history.extend(state_data.get('mid_price_history', []))
    ...
    async def _main_loop_tick(self):
        ...
        if self.config.trading_mode not in ["DRY_RUN", "SIMULATION"]:
            ...
            if not self.is_running: return # Bot might have stopped during WS check
    ...
    async def _process_orderbook_message(self, message: dict):
        ...
        self.state.last_ws_message_time = time.time() # Update last received WS message time
        try:
            ...
            if 'data' in message:
                ...
                if topic == 'order':
                    for order_data in message['data']:
                        if order_data.get('symbol') == self.config.symbol: # Only process for configured symbol
                            await self._process_order_update(order_data)
                elif topic == 'position':
                    for pos_data in message['data']:
                        if pos_data['symbol'] == self.config.symbol: # Only process for configured symbol
    ...
    async def stop(self):
        ...
        state_to_save = {
            ...
            'mid_price': str(self.state.mid_price), # Store Decimals as strings for safe serialization
            'smoothed_mid_price': str(self.state.smoothed_mid_price),
            'mid_price_history': list(self.state.mid_price_history),
            'circuit_breaker_price_points': [(t, str(p)) for t, p in self.state.circuit_breaker_price_points] # Convert Decimals to strings
        }
        try:
    ...
    async def _update_mid_price(self, bids: list[list[str]], asks: list[list[str]]):
        ...
        async with self.market_data_lock:
            ...
            if new_mid_price != self.state.mid_price:
                ...
                self.state.mid_price_history.append(float(self.state.mid_price)) # For volatility calculation
                self.state.circuit_breaker_price_points.append((time.time(), self.state.mid_price)) # For circuit breaker
    ...
    async def _process_order_update(self, order_data: dict):
        ...
        self.logger.info(f"Order {order_id} status update: {status} (OrderLink: {order_data.get('orderLinkId')})")
        ...
        async with self.active_orders_lock:
            if order_id in self.state.active_orders:
                ...
                self.logger.warning(f"Received fill for untracked order {order_id}. Processing fill anyway.")
                await self._process_fill(order_data)
            else:
    ...
    async def _manage_orders(self):
        ...
        async with self.market_data_lock, self.balance_position_lock:
            ...
            pos_qty = self.state.metrics.current_asset_holdings # Use bot's internal holdings for strategy
    ...
    async def _check_circuit_breaker(self) -> bool:
        ...
        if len(recent_prices_window) < 2: return False # Need at least two points to calculate change
        ...
        if start_price == Decimal('0'): return False # Avoid division by zero
    ...
    async def _calculate_dynamic_spread(self) -> Decimal:
        ...
        async with self.market_data_lock:
            ...
            volatility_window_prices = [p for t, p in self.state.circuit_breaker_price_points if (current_time - t) <= ds_config.volatility_window_sec]
        ...
        if np.all(np.array(float_prices) == float_prices[0]): # Handle case of no price change
            volatility = 0.0
        else:
            volatility = np.std(float_prices) / np.mean(float_prices) # Coefficient of variation
    ...
    def _calculate_inventory_skew(self, mid_price: Decimal, pos_qty: Decimal) -> Decimal:
        ...
        if max_exposure_for_ratio <= 0: return Decimal('0') # Avoid division by zero

        # Calculate inventory ratio (-1 for max short, 1 for max long)
        inventory_ratio = current_inventory_value / max_exposure_for_ratio
        inventory_ratio = max(Decimal('-1.0'), min(Decimal('1.0'), inventory_ratio)) # Clamp between -1 and 1
    ...
    async def _reconcile_and_place_orders(self, target_bid: Decimal, target_ask: Decimal):
        ...
        async with self.active_orders_lock:
            ...
            for oid, odata, is_stale in current_active_orders_by_side['Buy']:
                if not is_stale and bid_order_to_keep is None:
                    ...
                    orders_to_cancel.append((oid, odata.get('orderLinkId'))) # Cancel other bids
            ...
            for oid, odata, is_stale in current_active_orders_by_side['Sell']:
                if not is_stale and ask_order_to_keep is None:
                    ...
                    orders_to_cancel.append((oid, odata.get('orderLinkId'))) # Cancel other asks
        ...
        # Plan new orders
        new_orders_to_place = []
        current_outstanding_orders = len(self.state.active_orders) - len(orders_to_cancel) # Orders that will remain after cancellation
    ...
    async def _log_status_summary(self):
        ...
        async with self.balance_position_lock, self.active_orders_lock, self.market_data_lock:
            ...
            if self.config.category in ['linear', 'inverse']:
                display_unrealized_pnl = self.state.unrealized_pnl_derivatives # Use exchange-reported UPNL for derivatives
            else:
                display_unrealized_pnl = unrealized_pnl_bot_calculated # Use bot-calculated UPNL for spot
            ...
            active_buys = sum(1 for o in self.state.active_orders.values() if o['side'] == 'Buy')
            active_sells = sum(1 for o in self.state.active_orders.values() if o['side'] == 'Sell')
    ...
    async def _fetch_market_info(self) -> bool:
        ...
        try:
            ...
            if maker_fee_rate_str is None:
                self.logger.critical(f"Missing 'makerFeeRate' for {self.config.symbol}. Full info: {info}")
                raise MarketInfoError(f"Critical fee information missing for {self.config.symbol}: makerFeeRate.")

            maker_fee_rate = Decimal(maker_fee_rate_str)
            taker_fee_rate = Decimal(taker_fee_rate_str) if taker_fee_rate_str else maker_fee_rate # Fallback if taker rate is missing
    ...
    async def _update_balance_and_position(self) -> bool:
        ...
        async with self.balance_position_lock:
            if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
                ...
                self.state.unrealized_pnl_derivatives = Decimal('0') # No derivatives PnL in dry run
                return True
            ...
            if self.config.category in ['linear', 'inverse']:
                ...
                self.state.current_position_qty = self.state.metrics.current_asset_holdings
                self.state.unrealized_pnl_derivatives = Decimal('0') # No separate unrealized PnL from exchange for spot
    ...
    async def _process_fill(self, order_data: dict):
        ...
        await self._update_balance_and_position() # Refresh balance/position after a fill
    ...
    async def _calculate_order_size(self, side: str, price: Decimal) -> Decimal:
        ...
        async with self.balance_position_lock:
            ...
            metrics_pos_qty = self.state.metrics.current_asset_holdings # Bot's internal tracked position
        ...
        if self.config.strategy.inventory.enabled and self.config.max_net_exposure_usd > Decimal('0'):
            ...
            if side == 'Buy':
                ...
                qty_to_reach_max_short = -max_allowed_pos_qty_abs - metrics_pos_qty # This will be negative if current position is not yet max short
    ...
    async def _place_limit_order(self, side: str, price: Decimal, quantity: Decimal):
        ...
        if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
            ...
            async with self.active_orders_lock:
                self.state.active_orders[oid] = {'side': side, 'price': price_f, 'qty': qty_f, 'status': 'New', 'orderLinkId': order_link_id, 'symbol': self.config.symbol, 'reduceOnly': params.get('reduceOnly', False)}
            await self.db_manager.log_order_event({**params, 'orderId': oid, 'orderStatus': 'New'}, f"{self.config.trading_mode} Order placed")
            return
