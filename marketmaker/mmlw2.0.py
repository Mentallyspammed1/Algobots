...
try:
    ...
    print("Install all dependencies with: pip install ccxt aiohttp pandas numpy websockets pydantic colorama python-dotenv")
    sys.exit(1)
...
class DynamicLeverageConfig(BaseModel):
    enabled: bool = False
    volatility_factor: PositiveFloat = 0.1 # Controls sensitivity to volatility (higher = more sensitive)
    ...
    benchmark_volatility: PositiveFloat = Decimal("0.001") # Empirical normal volatility (0.1%) for price ratio sources
...
class SymbolConfig(BaseModel):
    ...
    leverage: PositiveFloat = 10.0 # Default leverage if dynamic is disabled or not used
...
class ConfigManager:
    ...
    @classmethod
    async def load_config(cls, prompt_for_symbol: bool = False, input_symbol: Optional[str] = None) -> Tuple[GlobalConfig, List[SymbolConfig]]:
        global_data = {
        ...
        if prompt_for_symbol and input_symbol:
            ...
            if not input_symbol.upper().endswith("USDT"): # Simple check, might need refinement
            ...
            try:
                ...
                logging.critical(f"Symbol configuration validation error for {input_symbol}: {e}")
                sys.exit(1)
        else:
            try:
                ...
                if symbol_config_path.exists():
                    ...
                    for s_cfg_data in raw_symbol_configs:
                        ...
                        if symbol_name and not symbol_name.upper().endswith("USDT"): # Simple check
                            s_cfg_data["symbol"] = f"{symbol_name.upper()}USDT"
                        
                        try:
                            cls._symbol_configs.append(SymbolConfig(**s_cfg_data))
                        except ValidationError as e:
                            logging.warning(f"Symbol config validation error for {s_cfg_data.get('symbol', 'N/A')}: {e}")
                else:
...
class TelegramNotifier:
    ...
    async def send_message(self, message: str):
        ...
        try:
            ...
            self.logger.error(f"An unexpected error occurred sending Telegram message: {e}", exc_info=True)


async def initialize_exchange(logger: logging.Logger) -> Tuple[Optional[Any], Optional[aiohttp.ClientSession]]:
    ...
    try:
        ...
        aiohttp_session = aiohttp.ClientSession(
            connector=aiohttp.TCPConnector(resolver=aiohttp.AsyncResolver(), loop=loop, ssl=False)
        )
        ...
        logger.critical(f"Failed to initialize exchange due to event loop error: {e}", exc_info=True)
...
def calculate_vwap(prices: pd.Series, volumes: pd.Series, period: int = 20) -> pd.Series:
        ...
        async def wrapper(self, *args: Any, **kwargs: Any) -> Any:
            ...
            for i in range(attempts):
                try:
                    ...
                    ignorable_codes = ["110043", "110025", "110047", "100048"] # Example ignorable codes for Bybit
            ...
            if last_exception:
                ...
                raise RuntimeError(f"API call {func.__name__} failed after {attempts} attempts.")
        return wrapper
    return decorator
...
class BybitWebsocketClient:
    ...
    def __init__(self, symbols: List[str], logger: logging.Logger, message_queue: asyncio.Queue):
        self.symbols = symbols
        self.logger = logger
        self.message_queue = message_queue
        self.running = False
        self.ws_connection = None # Store the websocket connection object
        self.last_message_id: Dict[str, int] = {}  # Track message IDs to avoid duplicates per topic
        ...
        while self.running:
            try:
                ...
                async with websockets.connect(
                    ...
                    subscriptions = [f"orderbook.50.{s}" for s in self.symbols] + [f"publicTrade.{s}" for s in self.symbols]
                    subscribe_message = json.dumps({"op": "subscribe", "args": subscriptions})
                    ...
                    while self.running:
                        try:
                            ...
                            if topic and message_id and message_id <= self.last_message_id.get(topic, 0):
                                self.logger.debug(f"Skipping duplicate WebSocket message for {topic} (ID: {message_id})")
                                continue
                            ...
                            self.logger.warning(f"WebSocket connection closed with error: {e}. Reconnecting...")
                            break # Exit inner loop to reconnect
                        except json.JSONDecodeError:
                            self.logger.error(f"Failed to decode WebSocket message: {message}")
                        except Exception as e:
                            self.logger.error(f"Error receiving or processing WebSocket message: {e}", exc_info=True)
                            break # Break to attempt reconnect on unexpected errors

            except (websockets.exceptions.ConnectionClosedOK, websockets.exceptions.ConnectionClosedError) as e:
                self.logger.warning(f"WebSocket connection failed or closed: {e}. Retrying in {WS_RECONNECT_INTERVAL}s...")
            except asyncio.TimeoutError:
                self.logger.warning(f"WebSocket connection timed out. Retrying in {WS_RECONNECT_INTERVAL}s...")
            except Exception as e:
                self.logger.error(f"An unexpected error occurred during WebSocket connection: {e}. Retrying in {WS_RECONNECT_INTERVAL}s...", exc_info=True)
...
class OrderbookSnapshot:
    ...
    def mid_price(self) -> Optional[Decimal]:
        ...
        try:
            ...
            if best_bid is not None and best_ask is not None and best_ask > DECIMAL_ZERO:
                return (best_bid + best_ask) / Decimal("2")
            return None
        except Exception as e:
            # Use main_logger as strategy logger might not be available here directly
            main_logger.error(f"Error calculating mid_price for {self.symbol}: {e}", exc_info=True)
            return None

class OrderbookAnalyzer:
    def calculate_book_imbalance(self, symbol: str) -> Optional[Decimal]:
        ...
        try:
            ...
            main_logger.error(f"Error calculating book imbalance for {symbol}: {e}", exc_info=True)
            return None

    def calculate_trade_imbalance(self, recent_trades: Deque[Dict]) -> Optional[Decimal]:
    ...
    def calculate_book_pressure(self, symbol: str, depth_pct: float = 0.2) -> Optional[Decimal]:
        ...
        try:
            ...
            bid_volume = sum(level.quantity for level in snapshot.bids if level.price >= bid_pressure_limit)
            ask_volume = sum(level.quantity for level in snapshot.asks if level.price <= ask_pressure_limit)
            
            if ask_volume == DECIMAL_ZERO:
                return Decimal("5.0") # High pressure if ask volume is zero
            return bid_volume / ask_volume
        except Exception as e:
            main_logger.error(f"Error calculating book pressure for {symbol}: {e}", exc_info=True)
            return None

    def calculate_vwap(self, symbol: str) -> Optional[Decimal]:
        """Calculates Volume Weighted Average Price (VWAP) from the orderbook snapshot."""
        snapshot = self.get_snapshot(symbol)
        if not snapshot: return None
        
        s_cfg = SYMBOL_CONFIGS[symbol]
        levels = s_cfg.orderbook_analysis.obi_depth # Using obi_depth for VWAP calculation levels

        try:
            bid_pv = sum(level.price * level.quantity for level in snapshot.bids[:levels])
            bid_volume = sum(level.quantity for level in snapshot.bids[:levels])
            ask_pv = sum(level.price * level.quantity for level in snapshot.asks[:levels])
            ask_volume = sum(level.quantity for level in snapshot.asks[:levels])
            total_volume = bid_volume + ask_volume
            ...
            main_logger.error(f"Error calculating VWAP for {symbol}: {e}", exc_info=True)
            return None
...
class EnhancedMarketMakerStrategy:
    def __init__(self, global_config: GlobalConfig, symbol_configs: List[SymbolConfig], exchange: Any, aiohttp_session: aiohttp.ClientSession):
        ...
        # --- Data structures ---
        self.orderbooks: Dict[str, Optional[OrderbookSnapshot]] = {s: None for s in self.symbol_configs}
        self.prev_orderbooks: Dict[str, Optional[OrderbookSnapshot]] = {s: None for s in self.symbol_configs}
        self.positions: Dict[str, Decimal] = {s: DECIMAL_ZERO for s in self.symbol_configs}
        ...
        self.kline_data: Dict[str, pd.DataFrame] = {
            s: pd.DataFrame(columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
        ...
        self.last_market_data_time: Dict[str, float] = {s: 0 for s in self.symbol_configs}
        # Deque to store recent trades for imbalance calculation
        self.recent_trades: Dict[str, Deque[Dict]] = defaultdict(lambda: deque(maxlen=100))
        
        # --- State and performance tracking ---
        self.total_pnl = DECIMAL_ZERO
        self.daily_pnl = DECIMAL_ZERO
        self.last_daily_reset = date.today()
        self.trade_history: List[Trade] = [] # List to store completed trades (Trade objects)
        self.account_balance = Decimal('10000')  # Placeholder for account balance, updated from fetch_balance
    ...
    async def _load_state(self):
        ...
        if self.state_file_path.exists():
            try:
                ...
                reset_date_str = state_data.get('last_daily_reset', date.today().isoformat())
                ...
                self.trade_history = [Trade(**t) for t in loaded_history if isinstance(t, dict)]
                
                self.logger.info(f"Loaded state: Total PnL={self.total_pnl:.4f}, Daily PnL={self.daily_pnl:.4f}")
                self.logger.debug(f"Loaded {len(self.trade_history)} trades from history.")
            
            except (FileNotFoundError, json.JSONDecodeError, InvalidOperation, ValueError) as e:
                self.logger.error(f"Could not load state file '{self.state_file_path}': {e}. Attempting backup.")
                ...
                self.logger.error(f"An unexpected error occurred loading state: {e}", exc_info=True)
        else:
    ...
    async def _update_symbol_info(self):
        ...
        try:
            ...
            for symbol, s_cfg in self.symbol_configs.items():
                ...
                if market:
                    s_cfg.min_qty = Decimal(str(market.get('limits', {}).get('amount', {}).get('min'))) if market.get('limits') and market['limits'].get('amount') else None
                    s_cfg.qty_precision = int(market.get('precision', {}).get('amount')) if market.get('precision') else None
                    s_cfg.price_precision = int(market.get('precision', {}).get('price')) if market.get('precision') else None
                    s_cfg.tick_size = Decimal(str(market.get('precision', {}).get('price'))) if market.get('precision') and market['precision'].get('price') else None
                    s_cfg.min_notional = Decimal(str(market.get('limits', {}).get('cost', {}).get('min'))) if market.get('limits') and market['limits'].get('cost') else None
                    
                    self.logger.info(f"Updated info for {symbol}: Min Qty {s_cfg.min_qty}, Qty Precision {s_cfg.qty_precision}, Price Precision {s_cfg.price_precision}, Tick Size {s_cfg.tick_size}, Min Notional {s_cfg.min_notional}")
                    markets_updated += 1
                else:
                    self.logger.warning(f"Market information not found for symbol: {symbol}. Skipping updates.")
            self.logger.info(f"Finished refreshing symbol info. {markets_updated}/{len(self.symbol_configs)} symbols updated.")
        except Exception as e:
        ...
        while self.running:
            try:
                ...
                if 'orderbook' in topic:
                    symbol_from_topic = topic.split('.')[-1] # Extract symbol from topic like 'orderbook.50.BTCUSDT'
                    ...
                    bids = [OrderbookLevel(Decimal(p), Decimal(q)) for p, q in bids_raw if p is not None and q is not None]
                    asks = [OrderbookLevel(Decimal(p), Decimal(q)) for p, q in asks_raw if p is not None and q is not None]
                    
                    # Store the snapshot and update last data time
                    self.orderbooks[symbol_from_topic] = OrderbookSnapshot(symbol_from_topic, bids, asks, msg.get('ts', time.time() * 1000) / 1000.0)
                    ...
                    if not trades_data: continue # Skip if no trade data
                    ...
                    for trade in trades_data:
                        ...
                        self.recent_trades[symbol_from_topic].append({
                            'amount': Decimal(str(trade.get('v', 0))), # 'v' is volume/quantity
    ...
    def _get_vwap_for_symbol(self, symbol: str) -> Optional[Decimal]:
        """Helper to get VWAP for a symbol, using the analyzer."""
        return self.orderbook_analyzer.calculate_vwap(symbol)

    def _get_book_imbalance_for_symbol(self, symbol: str) -> Optional[Decimal]:
    ...
    def _analyze_liquidity_cliffs(self, symbol: str) -> Tuple[bool, bool]:
        ...
        try:
            ...
            next_bids_avg_qty = sum(q.quantity for q in snapshot.bids[1:depth]) / (depth - 1) if depth > 1 else snapshot.bids[1].quantity
            
            # A cliff occurs if the top bid quantity is significantly smaller than average of subsequent bids
            is_bid_cliff = next_bids_avg_qty > DECIMAL_ZERO and (top_bid_qty / next_bids_avg_qty) < (1/factor)

            top_ask_qty = snapshot.asks[0].quantity
            next_asks_avg_qty = sum(q.quantity for q in snapshot.asks[1:depth]) / (depth - 1) if depth > 1 else snapshot.asks[1].quantity
            
            # A cliff occurs if the top ask quantity is significantly smaller than average of subsequent asks
            is_ask_cliff = next_asks_avg_qty > DECIMAL_ZERO and (top_ask_qty / next_asks_avg_qty) < (1/factor)
            ...
            self.logger.error(f"Error analyzing liquidity cliffs for {symbol}: {e}", exc_info=True)
            return False, False
    ...
    def _detect_toxic_flow(self, symbol: str) -> Tuple[bool, bool]:
        ...
        if not prev_ob or not curr_ob:
            self.prev_orderbooks[symbol] = curr_ob # Store current as previous for next iteration
        ...
        try:
            ...
            if is_toxic_on_bid: self.logger.warning(f"Potential toxic flow detected on BID side for {symbol}")
            if is_toxic_on_ask: self.logger.warning(f"Potential toxic flow detected on ASK side for {symbol}")
            
        except IndexError: # Handle cases where snapshot might be empty
             self.logger.warning(f"Orderbook snapshot data insufficient for toxic flow detection for {symbol}")
        except Exception as e:
            self.logger.error(f"Error detecting toxic flow for {symbol}: {e}", exc_info=True)
        ...
        try:
            ...
            if s_cfg.signal_config.use_rsi:
                ...
                if len(klines_df) >= s_cfg.signal_config.rsi_period:
                    self.signals[symbol]['rsi'] = calculate_rsi(klines_df['close'], s_cfg.signal_config.rsi_period).iloc[-1]
                else:
            ...
            if s_cfg.signal_config.use_macd:
                ...
                required_macd_points = s_cfg.signal_config.macd_slow + s_cfg.signal_config.macd_signal
                if len(klines_df) >= required_macd_points:
                    _, _, macd_hist = calculate_macd(klines_df['close'], s_cfg.signal_config.macd_fast, s_cfg.signal_config.macd_slow, s_cfg.signal_config.macd_signal)
            ...
            if s_cfg.dynamic_spread.use_bollinger_bands:
                ...
                if len(klines_df) >= bb_period:
                    upper, middle, lower = calculate_bollinger_bands(klines_df['close'], bb_period, s_cfg.dynamic_spread.bb_std_dev)
            ...
            # ATR calculation
            if s_cfg.dynamic_spread.enabled and s_cfg.dynamic_spread.atr_update_interval > 0: # Check if enabled and has update interval
                # ATR uses a fixed period (e.g., 14) or configurable. Assume 14 here.
                if len(klines_df) >= 14: # Assuming ATR period of 14
                    self.signals[symbol]['atr'] = calculate_atr(klines_df['high'], klines_df['low'], klines_df['close'], 14).iloc[-1]
                else:
            ...
            if s_cfg.signal_config.use_volume_profile:
                ...
                if len(klines_df) >= vwap_period:
                    self.signals[symbol]['vwap'] = calculate_vwap(klines_df['close'], klines_df['volume'], vwap_period).iloc[-1]
                else:
                    self.signals[symbol]['vwap'] = None
        except Exception as e:
            self.logger.error(f"Error calculating technical signals for {symbol}: {e}", exc_info=True)
    ...
    def _get_directional_bias(self, symbol: str) -> TradingBias:
        ...
        try:
            # RSI Bias
            if s_cfg.signal_config.use_rsi and 'rsi' in signals and signals['rsi'] is not None:
                rsi = signals['rsi']
                if rsi < s_cfg.signal_config.rsi_oversold: score += 1 # Bullish signal
                if rsi > s_cfg.signal_config.rsi_overbought: score -= 1 # Bearish signal
            
            # MACD Histogram Bias
            if s_cfg.signal_config.use_macd and 'macd_hist' in signals and signals['macd_hist'] is not None:
                if signals['macd_hist'] > DECIMAL_ZERO: score += 1 # Bullish signal
                if signals['macd_hist'] < DECIMAL_ZERO: score -= 1 # Bearish signal
        
        except Exception as e:
            self.logger.error(f"Error assessing directional bias for {symbol}: {e}", exc_info=True)
    ...
    def _calculate_volatility(self, symbol: str) -> Optional[Decimal]:
        ...
        try:
            if source == "atr":
                ...
                snapshot = self.orderbooks.get(symbol)
                if snapshot and snapshot.mid_price and snapshot.mid_price > DECIMAL_ZERO:
                    # Calculate spread as a ratio
                    return (snapshot.asks[0].price - snapshot.bids[0].price) / snapshot.mid_price
                ...
                self.logger.warning(f"Unknown volatility source '{source}' configured for symbol {symbol}")
                return None
        except Exception as e:
            self.logger.error(f"Error calculating volatility for {symbol} using source '{source}': {e}", exc_info=True)
            return None
        return None
    ...
    def _calculate_target_leverage(self, symbol: str) -> Decimal:
        ...
        if volatility is None:
            self.logger.debug(f"Volatility calculation failed or disabled for {symbol}. Reverting to base leverage.")
            return Decimal(str(s_cfg.leverage)) 
        ...
        if benchmark_vol <= DECIMAL_ZERO: benchmark_vol = Decimal("0.00001") # Prevent division by zero
        ...
        normalized_volatility_0_1 = (clamped_volatility - min_vol_benchmark) / (max_vol_benchmark - min_vol_benchmark)
        ...
        effective_vol_scale = min(Decimal("1.0"), normalized_volatility_0_1 * vol_cfg.volatility_range_factor)
        ...
        target_leverage = vol_cfg.max_leverage * (1 - effective_vol_scale) + vol_cfg.min_leverage * effective_vol_scale
        
        # Ensure the calculated target leverage is clamped within the configured min/max leverage limits.
        target_leverage = max(vol_cfg.min_leverage, min(target_leverage, vol_cfg.max_leverage))
    ...
    async def _adjust_leverage_if_needed(self, symbol: str):
        ...
        if not vol_cfg.enabled or vol_cfg.leverage_update_interval == 0:
            ...
            if symbol in self.exchange_leverage_set and self.exchange_leverage_set[symbol] != Decimal(str(s_cfg.leverage)):
                self.logger.debug(f"Dynamic leverage disabled for {symbol}, resetting leverage to config default: {s_cfg.leverage}x")
                # Update internal tracking if it differs. Actual reset via exchange.set_leverage would be needed if desired.
                self.metrics["current_symbol_leverage"][symbol] = Decimal(str(s_cfg.leverage))
                self.exchange_leverage_set[symbol] = Decimal(str(s_cfg.leverage))
            return

        # Check if it's time to re-evaluate leverage based on interval
        last_update_time = self.metrics.get("last_leverage_update_time", {}).get(symbol, 0.0)
        ...
        last_kline_update_time = self.metrics.get("last_fetch_kline_time", {}).get(symbol, 0.0)
        # Fetch klines if they are older than, say, 10 minutes (600 seconds)
        if time.time() - last_kline_update_time > 600: 
             self.logger.debug(f"Kline data for {symbol} is old ({time.time() - last_kline_update_time:.0f}s ago). Fetching fresh data for leverage calculation.")
        ...
        current_set_leverage = self.exchange_leverage_set.get(symbol, Decimal(str(s_cfg.leverage))) # Default to config if not yet tracked
        ...
        if abs(target_leverage - current_set_leverage) >= threshold_pct * current_set_leverage:
            self.logger.info(f"Leverage adjustment needed for {symbol}: Current={current_set_leverage:.2f}x, Target={target_leverage:.2f}x (Threshold: {threshold_pct*100:.1f}%)")
            ...
            try:
                ...
                self.metrics["current_symbol_leverage"][symbol] = target_leverage # Update tracked leverage
                self.metrics["last_leverage_update_time"][symbol] = time.time()
                self.metrics["op_time_leverage_adjust"] = time.time() - adjust_start_time

                self.logger.info(f"Successfully set leverage for {symbol} to {target_leverage:.2f}x")
                # Notify via Telegram
                await self.notifier.send_message(f"📈 Leverage adjusted for {symbol} to **{target_leverage:.2f}x**")
            
            except Exception as e:
                self.logger.error(f"Failed to set leverage for {symbol} to {target_leverage:.2f}x: {e}", exc_info=True)
            ...
            self.metrics["last_leverage_update_time"][symbol] = time.time()
            self.logger.debug(f"Leverage for {symbol} ({current_set_leverage:.2f}x) is within target ({target_leverage:.2f}x) and threshold. No adjustment made.")
    ...
    def _process_executed_trades(self, symbol: str, placed_orders_results: List[Dict]):
        ...
        for order_result in placed_orders_results:
            ...
            if order_result and (order_result.get('status') == 'closed' or order_result.get('filled', 0) > 0):
    ...
    async def _update_status_for_symbol(self, symbol: str):
        ...
        if self.trading_halted or not s_cfg.trade_enabled:
            ...
            if self.open_orders.get(symbol):
                ...
                try:
                    ...
                    self.logger.info(f"Trading halted or disabled for {symbol}. Cancelled {len(self.open_orders[symbol])} open orders.")
                    self.metrics["orders_cancelled_total"] += len(self.open_orders[symbol])
                except Exception as e:
                    self.logger.error(f"Failed to cancel orders for {symbol} during halt/disable: {e}")
                finally:
                    self.open_orders[symbol] = [] # Clear list
                    self.metrics["op_time_order_cancellation"] = time.time() - cancel_start_time
            return # Skip further processing for this symbol

        # Check if market data is stale
        last_market_data_time = self.last_market_data_time.get(symbol, 0.0)
        if time.time() - last_market_data_time > s_cfg.market_data_stale_timeout_seconds:
            self.logger.warning(f"Market data for {symbol} is stale (last updated > {s_cfg.market_data_stale_timeout_seconds}s ago). Skipping quote update.")
        ...
        if open_orders_for_symbol:
            ...
            try:
                await self.exchange.cancel_all_orders(symbol)
                self.logger.debug(f"Cancelled {len(open_orders_for_symbol)} existing orders for {symbol} to refresh quotes.")
                self.metrics["orders_cancelled_total"] += len(open_orders_for_symbol)
            except Exception as e:
                self.logger.error(f"Failed to cancel orders for {symbol}: {e}")
            finally:
                self.open_orders[symbol] = [] # Clear the list after cancellation attempt
                self.metrics["op_time_order_cancellation"] = time.time() - cancel_start_time
        ...
        for quote in quotes:
            ...
            if quote.get('qty', DECIMAL_ZERO) > DECIMAL_ZERO:
                ...
                if min_notional is not None and quote['price'] * quote['qty'] < min_notional:
                    self.logger.warning(f"Skipping order for {symbol}: Notional value ({quote['price'] * quote['qty']:.4f}) is below minimum notional ({min_notional:.4f}).")
                    continue
                ...
                orders_to_place.append({
                    ...
                    'side': quote['side'],
                    'amount': float(quote['qty']), # CCXT often expects float for amount/price
                    'price': float(quote['price'])
                })

        # If there are valid orders to place
        if orders_to_place:
            self.logger.info(f"Placing {len(orders_to_place)} orders for {symbol}: {[f'{o['side']} {o['amount']}@{o['price']}' for o in orders_to_place]}")
            ...
            try:
                ...
                if s_cfg.use_batch_orders_for_refresh and len(orders_to_place) > 1:
                    placed_orders_results = await self.exchange.create_orders(orders_to_place)
                ...
                for order_result in placed_orders_results:
                    if order_result: # Ensure the result is valid
                        ...
                        if order_result.get('status') != 'closed':
                            ...
                            open_orders_for_processing.append(order_result) # Store open/partially filled orders
                        
                        # Update total orders placed metric
                        self.metrics["orders_placed_total"] += 1
                    else:
                         self.logger.warning(f"Received empty or invalid order result for {symbol} placement.")
                ...
                self.logger.error(f"Failed to place orders for {symbol}: {e}", exc_info=True)
                # If order placement fails, clear the open orders list for this symbol
                self.open_orders[symbol] = []
            finally:
                # Record time taken for order placement operation
                self.metrics["op_time_order_placement"] = time.time() - placement_start_time
        # else:
    ...
    async def _print_status_dashboard(self):
        ...
        while self.running:
            try:
                ...
                # --- Header ---
                print(f"{Style.BRIGHT}{Fore.CYAN}--- Hybrid Market Maker Status Dashboard ---{Style.RESET_ALL}")
                ...
                print(f"  Total Orders Placed: {self.metrics.get('orders_placed_total', 0)}")
                print(f"  Total Orders Cancelled: {self.metrics.get('orders_cancelled_total', 0)}")
                print(f"  Total Filled Legs: {self.metrics.get('trades_filled_total', 0)}")
                print(f"  Quote Calc Time: {self.metrics.get('op_time_quote_calc', 0.0):.4f}s")
                print(f"  Order Placement Time: {self.metrics.get('op_time_order_placement', 0.0):.4f}s")
                print(f"  Order Cancellation Time: {self.metrics.get('op_time_order_cancellation', 0.0):.4f}s")
                print(f"  Leverage Adjustment Time: {self.metrics.get('op_time_leverage_adjust', 0.0):.4f}s")
                ...
                for symbol in self.symbol_configs:
                    ...
                    # Display Leverage
                    current_leverage_val = self.metrics.get("current_symbol_leverage", {}).get(symbol, "N/A")
                    print(f"  Leverage: {current_leverage_val}x")

                    # Display market data if available
                    if snapshot and snapshot.mid_price:
                        book_imb = self.orderbook_analyzer.calculate_book_imbalance(symbol)
                        trade_imb = self.orderbook_analyzer.calculate_trade_imbalance(self.recent_trades[symbol])
                        ...
                        print(f"  Book Imbalance: {book_imb:.3f}" if book_imb is not None else "  Book Imbalance: N/A")
                        print(f"  Trade Imbalance: {trade_imb:.3f}" if trade_imb is not None else "  Trade Imbalance: N/A")
                        print(f"  Position: {pos:.4f}")
                        print(f"  Open Orders: {open_ord_count}")
                    else:
                        print("  Market Data: Not available (check WS connection or symbol config)")
    ...
    def round_price(self, symbol: str, price: Decimal) -> Decimal:
        ...
        if not s_cfg or s_cfg.tick_size is None:
            self.logger.warning(f"Tick size not available for {symbol}. Cannot round price.")
            return price
        
        try:
            # Use ROUND_HALF_UP for standard rounding
            rounded_price = (price / s_cfg.tick_size).quantize(DECIMAL_ZERO, rounding=ROUND_HALF_UP) * s_cfg.tick_size
            return rounded_price
        except Exception as e:
            self.logger.error(f"Error rounding price {price} for {symbol} with tick size {s_cfg.tick_size}: {e}", exc_info=True)
    ...
    def round_quantity(self, symbol: str, quantity: Decimal) -> Decimal:
        ...
        if not s_cfg or s_cfg.qty_precision is None:
            self.logger.warning(f"Quantity precision not available for {symbol}. Cannot round quantity.")
            return quantity
        ...
        try:
            ...
            self.logger.error(f"Error rounding quantity {quantity} for {symbol} with precision {s_cfg.qty_precision}: {e}", exc_info=True)
    ...
    async def run(self):
        ...
        await self.notifier.send_message("🚀 Hybrid Market Maker Bot v3.0 (Enhanced) Started")
        ...
        self.logger.info("Waiting 10 seconds for initial data and WebSocket connection...")
        ...
        try:
            ...
            while self.running:
                ...
                symbol_update_tasks = [self._update_status_for_symbol(s) for s in self.symbol_configs]
                ...
                if current_time - last_symbol_info_update_time > SYMBOL_INFO_REFRESH_INTERVAL:
    ...
    async def _cleanup(self):
        ...
        if 'ws_task' in locals() and ws_task and not ws_task.done(): tasks_to_cancel.append(ws_task)
        if 'ws_processor_task' in locals() and ws_processor_task and not ws_processor_task.done(): tasks_to_cancel.append(ws_processor_task)
        if 'dashboard_task' in locals() and dashboard_task and not dashboard_task.done(): tasks_to_cancel.append(dashboard_task)
        
        if tasks_to_cancel:
            for task in tasks_to_cancel:
                task.cancel()
            try:
                await asyncio.gather(*tasks_to_cancel, return_exceptions=True) # Wait for tasks to finish cancellation
            except Exception as e:
        ...
        for symbol in self.symbol_configs:
            try:
                if self.open_orders.get(symbol): # Check if there are orders to cancel
                    self.logger.info(f"Cancelling all open orders for {symbol} during cleanup...")
                    await self.exchange.cancel_all_orders(symbol)
                    self.metrics["orders_cancelled_total"] += len(self.open_orders.get(symbol, [])) # Update metrics
                    self.open_orders[symbol] = [] # Clear local tracking
            except Exception as e:
                self.logger.error(f"Error cancelling orders for {symbol} during cleanup: {e}")
        ...
        await self.notifier.send_message("🛑 Hybrid Market Maker Bot v3.0 (Enhanced) Stopped")
...
# ==============================================================================
async def main():
    parser = argparse.ArgumentParser(description="Hybrid Bybit Market Maker Bot v3.0 - Enhanced")
    parser.add_argument("--symbol", type=str, help="Run the bot for a single symbol (e.g., BTC), ignoring the symbols.json config file.")
    ...
    if not BYBIT_API_KEY or not BYBIT_API_SECRET:
        main_logger.critical("BYBIT_API_KEY and BYBIT_API_SECRET environment variables must be set. Exiting.")
        sys.exit(1)
    ...
    if not exchange_instance or not aiohttp_session_instance:
        main_logger.critical("Exchange or aiohttp session initialization failed. Exiting.")
        sys.exit(1)

    # Create and run the strategy
    strategy = EnhancedMarketMakerStrategy(GLOBAL_CONFIG, SYMBOL_CONFIGS, exchange_instance, aiohttp_session_instance)
    await strategy.run()
