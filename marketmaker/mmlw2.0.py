...
class ConfigManager:
    ...
    @classmethod
    async def load_config(cls, prompt_for_symbol: bool = False, input_symbol: str | None = None) -> tuple[GlobalConfig, list[SymbolConfig]]:
        global_data = {
...
async def initialize_exchange(logger: logging.Logger) -> tuple[Any | None, aiohttp.ClientSession | None]:
...
class OrderbookAnalyzer:
    ...
    def calculate_book_pressure(self, symbol: str, depth_pct: float = 0.2) -> Decimal | None:
...
class EnhancedMarketMakerStrategy:
    def __init__(self, global_config: GlobalConfig, symbol_configs: list[SymbolConfig], exchange: Any, aiohttp_session: aiohttp.ClientSession):
        ...
        # --- Data structures ---
        self.orderbooks: dict[str, OrderbookSnapshot | None] = dict.fromkeys(self.symbol_configs)
        self.prev_orderbooks: dict[str, OrderbookSnapshot | None] = dict.fromkeys(self.symbol_configs)
        self.positions: dict[str, Decimal] = dict.fromkeys(self.symbol_configs, DECIMAL_ZERO)
        ...
        self.last_market_data_time: dict[str, float] = dict.fromkeys(self.symbol_configs, 0)
