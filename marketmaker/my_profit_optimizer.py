...
async def run_simulation_for_trial(trial_config: Config, duration_ticks: int = 1000) -> Decimal:
    """
    ...
    try:
        ...
        for _ in range(duration_ticks):
            ...
            await asyncio.sleep(trial_config.system.loop_interval_sec) # Simulate time passing
...
def objective(trial: optuna.Trial) -> float:
    ...
    price_change_smoothing_factor = Decimal(str(trial.suggest_float("price_change_smoothing_factor", 0.1, 0.9))) if dynamic_spread_enabled else Decimal('0')

    # Inventory strategy parameters
    inventory_enabled = trial.suggest_categorical("inventory_enabled", [True, False])
    skew_intensity = Decimal(str(trial.suggest_float("skew_intensity", 0.1, 1.0))) if inventory_enabled else Decimal('0')
    max_inventory_ratio = Decimal(str(trial.suggest_float("max_inventory_ratio", 0.1, 0.9))) if inventory_enabled else Decimal('0')
...
if __name__ == "__main__":
    ...
    optimizer_logger.info(f"Starting optimization for {num_trials} trials or {timeout_seconds} seconds.")
    study.optimize(objective, n_trials=num_trials, timeout=timeout_seconds)

    optimizer_logger.info("Optimization finished.")
    optimizer_logger.info(f"Best trial: {study.best_trial.value:.4f} with params: {study.best_trial.params}")
