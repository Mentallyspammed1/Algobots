...
class BybitMarketMaker:
    ...
    async def _simulate_dry_run_price_movement(self, current_time: float):
        ...
        time_factor = Decimal(str(current_time % oscillation_period_sec)) / Decimal(
            str(oscillation_period_sec)
        )
        price_change = oscillation_amplitude * Decimal(
            str(math.sin(float(time_factor) * 2 * math.pi))
        )
    ...
    async def _simulate_dry_run_fills(self):
        ...
        async with self.active_orders_lock:
                ...
                if order_id.startswith("DRY_"):  # Only process mock orders
                    ...
                    if filled:
                        ...
                        if (
                            ...
                            self.logger.warning(
                                f"DRY_RUN: Skipping simulated sell fill for order {order_id} (Qty: {order_data['qty']}) due to insufficient simulated holdings ({self.state.metrics.current_asset_holdings})."
                            )
            ...
            for order_id, order_data in orders_to_process:
                self.logger.info(
                    f"DRY_RUN: Simulating fill for order {order_id} (Side: {order_data['side']}, Price: {order_data['price']}) at current mid_price {self.state.mid_price}"
                )
    ...
    async def _calculate_order_size(self, side: str, price: Decimal) -> Decimal:
        ...
        if (
            ...
            if side == "Buy":
                ...
                if metrics_pos_qty > Decimal(
                    "0"
                ):  # If currently long, cap sell quantity at current holdings
