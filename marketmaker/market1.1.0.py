...
def json_loads_decimal(s: str) -> Any:
    ...
    try:
        ...
        logging.getLogger("BybitMarketMaker").error(
...
def atr(
    ...
    return (
        tr["tr"].ewm(span=length, adjust=False).mean()
...
class InventorySkewConfig(BaseModel):
    ...
    max_inventory_ratio: PositiveFloat = (
        0.5  # Max inventory as ratio of max_net_exposure_usd
    )
    inventory_sizing_factor: NonNegativeFloat = (
        0.5  # How much inventory affects order size
    )
...
class OrderLayer(BaseModel):
    ...
    cancel_threshold_pct: PositiveFloat = (
        0.01  # Price change % to cancel and re-place orders
    )
...
class CircuitBreakerConfig(BaseModel):
    ...
    max_daily_loss_pct: PositiveFloat | None = Field(
        default=None,
...
class StrategyConfig(BaseModel):
    ...
    order_stale_threshold_pct: PositiveFloat = (
        0.0005  # Price change % to consider an order stale
    )
    ...
    market_data_stale_timeout_seconds: PositiveInt = (
        30  # Max age for orderbook/trade data
    )
...
class GlobalConfig(BaseModel):
    ...
    initial_dry_run_capital: Decimal = Field(
        default=Decimal("10000"), description="Virtual capital for DRY_RUN/SIMULATION"
    )
    dry_run_price_drift_mu: float = Field(
        default=0.0, description="Mean drift for simulated price movement"
    )
    dry_run_price_volatility_sigma: float = Field(
        default=0.0001, description="Volatility for simulated price movement"
    )
    dry_run_time_step_dt: float = Field(
        default=1.0, description="Time step for simulated price movement"
    )
    ...
    def load_from_env(cls) -> "GlobalConfig":
        ...
        env_data = {
            ...
            "system": {
                ...
                "order_refresh_interval_sec": float(
                    os.getenv("ORDER_REFRESH_INTERVAL_SEC", "5.0")
                ),
                "ws_heartbeat_sec": int(os.getenv("WS_HEARTBEAT_SEC", "30")),
                "cancellation_rate_limit_sec": float(
                    os.getenv("CANCELLATION_RATE_LIMIT_SEC", "0.2")
                ),
                "status_report_interval_sec": int(
                    os.getenv("STATUS_REPORT_INTERVAL_SEC", "30")
                ),
                "ws_reconnect_attempts": int(os.getenv("WS_RECONNECT_ATTEMPTS", "5")),
                "ws_reconnect_initial_delay_sec": int(
                    os.getenv("WS_RECONNECT_INITIAL_DELAY_SEC", "5")
                ),
                "ws_reconnect_max_delay_sec": int(
                    os.getenv("WS_RECONNECT_MAX_DELAY_SEC", "60")
                ),
                "api_retry_attempts": int(os.getenv("API_RETRY_ATTEMPTS", "5")),
                "api_retry_initial_delay_sec": float(
                    os.getenv("API_RETRY_INITIAL_DELAY_SEC", "0.5")
                ),
                "api_retry_max_delay_sec": float(
                    os.getenv("API_RETRY_MAX_DELAY_SEC", "10.0")
                ),
                "health_check_interval_sec": int(
                    os.getenv("HEALTH_CHECK_INTERVAL_SEC", "10")
                ),
                "config_refresh_interval_sec": int(
                    os.getenv("CONFIG_REFRESH_INTERVAL_SEC", "60")
                ),
            },
            "files": {
            ...
            "dry_run_price_volatility_sigma": float(
...
class SymbolConfig(BaseModel):
    ...
    def model_post_init__(self, __context: Any) -> None:
        ...
        if self.symbol.endswith("USDT"):
            ...
            logging.getLogger("BybitMarketMaker").warning(
                f"Cannot parse base/quote currency from symbol: {self.symbol}. Using UNKNOWN."
            )
...
class BybitWebSocketClient:
    ...
    def __init__(
        ...
        self.symbol_bots: dict[
            str, AsyncSymbolBot
        ] = {}  # Reference to active AsyncSymbolBot instances
            ...
            if "topic" in msg:
                ...
                if topic.startswith("orderbook."):
                    asyncio.run_coroutine_threadsafe(
                        self._process_orderbook_message(msg), self.main_event_loop
                    )
                elif topic.startswith("publicTrade."):
                    asyncio.run_coroutine_threadsafe(
                        self._process_public_trade_message(msg), self.main_event_loop
                    )
                elif topic in ["order", "position", "execution", "wallet"]:
                    asyncio.run_coroutine_threadsafe(
                        self._process_private_message(msg), self.main_event_loop
                    )
    ...
    async def _process_orderbook_message(self, message: dict[str, Any]):
        ...
        with self._data_lock:
            symbol_map = {
                bot.config.symbol.replace("/", "").replace(":", ""): bot.config.symbol
    ...
    async def _process_private_message(self, message: dict[str, Any]):
        ...
        for item_data in data:
            ...
            if not symbol_ws and topic != "wallet":
                continue  # Wallet updates don't have symbol

            with self._data_lock:
                symbol_map = {
                    bot.config.symbol.replace("/", "").replace(
                        ":", ""
                    ): bot.config.symbol
                    for bot in self.symbol_bots.values()
                }
                ...
                if symbol and symbol in self.symbol_bots:
                    ...
                    for bot_instance in self.symbol_bots.values():
                        ...
                        if (
                            item_data.get("coin")
                            == bot_instance.global_config.main_quote_currency
                        ):
                            await bot_instance._update_balance_from_wallet_ws(item_data)
                else:
                    self.logger.debug(
                        f"Received {topic} update for unmanaged symbol: {symbol_ws if symbol_ws else 'N/A'}"
                    )

    def get_order_book_snapshot(
        self, symbol: str
    ) -> dict[str, list[list[Decimal]]] | None:
            ...
            for topic in topics:
                if topic.startswith("orderbook."):
                    ...
                    ws_instance.orderbook_stream(
                        symbol=symbol_ws, depth=depth, callback=self._ws_message_handler
                    )
                    ...
                    ws_instance.trade_stream(
                        symbol=symbol_ws, callback=self._ws_message_handler
                    )
            ...
            if new_ws_instance:
                ...
                if attempts >= self.config.system.ws_reconnect_attempts:
                    self.logger.critical(
                        f"{Colors.NEON_RED}{stream_name} WebSocket reconnection failed after {self.config.system.ws_reconnect_attempts} attempts. Shutting down.{Colors.RESET}"
                    )
...
class AsyncSymbolBot:
            ...
            try:
                ...
                if not self._is_trading_hours(current_time):
                    if self.config.trade_enabled:
                        self.logger.info(
                            f"[{self.config.symbol}] Outside trading hours. Temporarily disabling trading and cancelling orders."
                        )
                        self.config = self.config.model_copy(
                            update={"trade_enabled": False}
                        )  # Temporarily disable
                    ...
                    self.logger.info(
                        f"[{self.config.symbol}] Within trading hours. Re-enabling trading."
                    )
    ...
    async def _load_state(self):
        ...
        if state_data:
            ...
            self.state.last_dry_run_price_update_time = state_data.get(
                "last_dry_run_price_update_time", time.time()
            )
        ...
        if new_mid_price != self.state.mid_price:
            ...
            if self.state.smoothed_mid_price == DECIMAL_ZERO:
                ...
                self.state.smoothed_mid_price = (alpha * new_mid_price) + (
                    (DECIMAL_ZERO - alpha) * self.state.smoothed_mid_price
                )
    ...
    async def _check_market_data_freshness(self, current_time: float) -> bool:
        ...
        with self.ws_client._data_lock:  # Access shared data via lock
            if self.config.symbol not in self.ws_client.last_orderbook_update_time or (
                current_time
                - self.ws_client.last_orderbook_update_time[self.config.symbol]
    ...
    async def _fetch_market_info(self) -> bool:
        ...
        if self.global_config.trading_mode == "SIMULATION":
            ...
            object.__setattr__(
                self.config,
                "min_notional_value",
                Decimal(str(self.config.min_order_value_usd)),
            )
        ...
        try:
            ...
            object.__setattr__(
                self.config,
                "maker_fee_rate",
                Decimal(info.get("makerFeeRate", "0.0002")),
            )  # Default if not provided
            object.__setattr__(
                self.config,
                "taker_fee_rate",
                Decimal(info.get("takerFeeRate", "0.0005")),
            )  # Default if not provided
    ...
    async def _process_position_update(self, pos_data: dict[str, Any]):
        ...
        if (
            ...
            self.logger.debug(
                f"[{self.config.symbol}] UNREALIZED PNL (WS): {self.state.unrealized_pnl_derivatives:+.4f} {self.config.quote_currency}"
            )
    ...
    async def _process_execution_update(self, trade_data: dict[str, Any]):
        ...
        if exec_type not in [
            ...
            "AdlTrade",
            "BustTrade",
        ]:  # Filter out non-trade related executions like 'Funding' etc.
            self.logger.debug(
        ...
        liquidity_role = trade_data.get(
            "execType", "UNKNOWN"
        )  # Bybit returns 'Trade' for taker, 'AdlTrade' for ADL, etc.
        ...
        if side == "Buy":
            ...
            if self.global_config.trading_mode in ["DRY_RUN", "SIMULATION"]:
                ...
                self.state.available_balance = self.state.current_balance
            self.logger.info(
                f"[{self.config.symbol}] Order FILLED: SELL {exec_qty} @ {exec_price}, Fee: {exec_fee}. Realized PnL from sale: {profit_loss_on_sale:+.4f}. Holdings: {metrics.current_asset_holdings}, Avg Entry: {metrics.average_entry_price}"
            )
        else:
            self.logger.warning(
        ...
        await self.db_manager.log_trade_fill(
            self.config.symbol, trade_data, realized_pnl_impact
        )
    ...
    async def _manage_orders(self):
        ...
        if (
            ...
            or self.config.price_precision is None
        ):
            self.logger.warning(
        ...
        skew_factor = self._calculate_inventory_skew(
            self.state.smoothed_mid_price, self.state.metrics.current_asset_holdings
        )
        skewed_mid_price = self.state.smoothed_mid_price * (
            DECIMAL_ZERO + skew_factor
        )  # Corrected to add to 1.0

        # Base target prices
        base_target_bid_price = skewed_mid_price * (
            DECIMAL_ZERO - spread_pct
        )  # Corrected to subtract from 1.0
        base_target_ask_price = skewed_mid_price * (
            DECIMAL_ZERO + spread_pct
        )  # Corrected to add to 1.0
    ...
    async def _calculate_atr_from_kline(self, current_mid_price: Decimal) -> Decimal:
        ...
        try:
            ...
            df[["high", "low", "close"]] = df[["high", "low", "close"]].applymap(
                Decimal
            )
            ...
            self.logger.error(
                f"{Colors.NEON_RED}[{self.config.symbol}] Error calculating ATR: {e}{Colors.RESET}",
                exc_info=True,
            )
    ...
    def _enforce_min_profit_spread(
        ...
        estimated_fee_per_side_pct = (
            self.config.taker_fee_rate
        )  # Assume taker for worst-case profit check
        min_gross_spread_pct = Decimal(
            str(self.config.strategy.min_profit_spread_after_fees_pct)
        ) + (estimated_fee_per_side_pct * DECIMAL_ZERO)  # Multiplied by 2
    ...
    async def _reconcile_and_place_orders(
        ...
        current_outstanding_orders = sum(
            1
            for oid, odata in self.state.active_orders.items()
            if odata["status"]
        ...
        for i, layer in enumerate(self.config.strategy.order_layers):
            if (
                ...
                >= self.config.strategy.max_outstanding_orders
            ):
                self.logger.debug(
                    f"[{self.config.symbol}] Max outstanding orders ({self.config.strategy.max_outstanding_orders}) reached. Skipping further layer placements."
                )
                break

            # Calculate layered prices
            layer_bid_price = base_target_bid * (
                DECIMAL_ZERO - Decimal(str(layer.spread_offset_pct))
            )  # Corrected
            ...
            if (
                len(current_active_orders_by_side["Buy"]) <= i
            ):  # Only place if this layer doesn't have an active order
                ...
                if buy_qty > DECIMAL_ZERO:
                    await self._place_limit_order(
                        "Buy", layer_bid_price, buy_qty, f"layer_{i}"
                    )
            ...
            if (
                current_outstanding_orders < self.config.strategy.max_outstanding_orders
                and len(current_active_orders_by_side["Sell"]) <= i
            ):  # Only place if this layer doesn't have an active order
                ...
                if sell_qty > DECIMAL_ZERO:
                    await self._place_limit_order(
                        "Sell", layer_ask_price, sell_qty, f"layer_{i}"
                    )
                    current_outstanding_orders += 1
                else:
                    self.logger.debug(
    ...
    async def _cancel_order(self, order_id: str, order_link_id: str | None = None):
        ...
        try:
            if await self.api_client.cancel_order(
                self.global_config.category, self.config.symbol, order_id, order_link_id
            ):
                self.logger.info(
    ...
    async def _update_take_profit_stop_loss(self):
        ...
        try:
            if await self.api_client.set_trading_stop(
                self.global_config.category, self.config.symbol, sl_price, tp_price
            ):
                self.logger.info(
    ...
    async def _check_daily_pnl_limits(self) -> bool:
        ...
        daily_loss_pct = (
            (daily_loss / self.state.daily_initial_capital)
            if self.state.daily_initial_capital > DECIMAL_ZERO
            else DECIMAL_ZERO
        )
    ...
    async def _simulate_dry_run_fills(self):
        ...
        for order_id, order_data, fill_qty in orders_to_process:
            ...
            mock_fill_data = {
                ...
                "execFee": str(
                    fill_qty * self.state.mid_price * self.config.taker_fee_rate
                ),
    ...
    async def _log_status_summary(self):
        ...
        pos_qty = (
            metrics.current_asset_holdings
        )  # Use metrics holdings for general reporting
        exposure_usd = (
...
class PyrmethusBot:
    ...
    async def _initialize_bot_components(self):
        ...
        self.ws_client = BybitWebSocketClient(
            self.global_config, self.logger, self.main_event_loop
        )
    ...
    async def _start_symbol_bots(self, symbol_configs: dict[str, SymbolConfig]):
        ...
        for symbol, config in symbol_configs.items():
            ...
            if symbol not in self.symbol_bots:
                self.logger.info(
                    f"{Colors.CYAN}Starting AsyncSymbolBot for {symbol}...{Colors.RESET}"
                )
    ...
    async def _update_websocket_subscriptions(self):
        ...
        private_topics: list[
            str
        ] = []  # Bybit private stream is usually generic, but topics can be specified
    ...
    async def _stop_all_bots(self):
        ...
        self.logger.info(
            f"{Colors.YELLOW}Signaling all AsyncSymbolBots to stop...{Colors.RESET}"
        )
        ...
        while not self._stop_event.is_set():
            try:
                if (
                    ...
                    single_symbol_active = (
                        len(self.active_symbol_configs) == 1
                        and list(self.active_symbol_configs.values())[0].symbol
                        == ConfigManager._symbol_configs.get(
                    ...
                    reloaded_global_cfg, reloaded_symbol_configs = (
                    ...
                    if reloaded_global_cfg != self.global_config:
                        self.logger.info(
                            f"{Colors.YELLOW}Global configuration changed. Applying updates (requires restart of core components).{Colors.RESET}"
                        )
    ...
    async def run(self):
        ...
        try:
            ...
            if sys.stdin.isatty():
                ...
                if selected_mode == "s":
                    input_symbol = (
                        input(
                            f"{Colors.CYAN}Enter single symbol (e.g., BTCUSDT): {Colors.RESET}"
                        )
                        .upper()
                        .strip()
                    )
        ...
        try:
            self.global_config, symbol_configs = ConfigManager.load_config(
