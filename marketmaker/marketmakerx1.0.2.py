...
class BaseConfigModel(BaseModel):
    ...
    def convert_to_decimal_if_needed(cls, v):
        if isinstance(v, (int, float, str)) and 'Decimal' in str(cls.model_fields.get(v).annotation):
            return to_decimal(v)
        return v
...
class BybitHTTPClient:
    ...
    async def _safe_api_call(self, method, *args, **kwargs):
        ...
        async for attempt in AsyncRetrying(
            ...
            retry=retry_if_exception_type(Exception), # Catch all exceptions for retries
            reraise=True,
    ...
    async def _check_circuit_breaker(self):
        ...
        if self.consecutive_api_failures >= threshold:
            if not self.circuit_breaker_active:
                ...
                await asyncio.sleep(cooldown) # Blocking sleep is fine for circuit breaker
                self.consecutive_api_failures = 0
...
class BybitWebSocketManager:
    ...
    def _ws_callback(self, message: dict):
        ...
        if self.main_event_loop and self.main_event_loop.is_running():
            try:
                asyncio.run_coroutine_threadsafe(self.message_handler(message), self.main_event_loop)
            except Exception as e:
        ...
        async with self._reconnect_lock:
            ...
            if self.public_ws and self.public_ws.is_connected() and not force_reconnect:
                ...
                try:
                    ...
                    asyncio.create_task(self._monitor_connection(self.public_ws, "Public"))
                except Exception as e:
            ...
            if not self.config.dry_run:
                if self.private_ws and self.private_ws.is_connected() and not force_reconnect:
                    ...
                    try:
                        ...
                        asyncio.create_task(self._monitor_connection(self.private_ws, "Private"))
                    except Exception as e:
...
class TAManager:
    ...
    def update_ta(self, prices: deque[Decimal]):
        ...
        try:
            ...
            min_len = max(rsi_len, ema_slow, bb_len) + 2 # Ensure enough data for all indicators
            if len(prices_list) < min_len:
                logger.debug(f"Not enough price data for TA. Need {min_len}, have {len(prices_list)}")
                return
...
class OrderManager:
    ...
    async def manage_orders(self, desired_buy_orders: List[Dict], desired_sell_orders: List[Dict]):
        ...
        current_quotes_tuple = [(o.price, o.qty, o.side) for o in current_open_orders.values()]
        new_target_quotes_tuple = [(o['price'], o['qty'], o['side']) for o in new_target_orders]

        if not self._quotes_significantly_changed(current_quotes_tuple, new_target_quotes_tuple):
            # Also check if mid price has drifted significantly
            if self.config.market.last_mid_price and self.config.order_management.mid_drift_threshold_ticks > 0:
                price_drift_threshold = self.instrument_info.tick_size * self.config.order_management.mid_drift_threshold_ticks
                if not approx_equal(self.config.market.last_mid_price, (self.config.market.last_known_best_bid + self.config.market.last_known_best_ask) / Decimal(2), price_drift_threshold):
                    logger.debug("Mid price drifted significantly, re-evaluating orders.")
                else:
        ...
        for target_order in new_target_orders:
            if not any(approx_equal(target_order['price'], co.price, self.instrument_info.tick_size * self.config.order_management.price_tolerance_ticks) and
                       ...
                       target_order['side'] == co.side for co in current_open_orders.values()):
    ...
    def _execute_virtual_orders(self):
        ...
        if self.orders_to_cancel or self.orders_to_place:
            logger.info(f"[DRY RUN] Orders: Cancelling {len(self.orders_to_cancel)}, Placing {len(self.orders_to_place)}")
    ...
    async def _execute_live_orders(self):
        ...
        if self.orders_to_cancel or self.orders_to_place:
            logger.info(f"Executing: Cancelling {len(self.orders_to_cancel)} orders, Placing {len(self.orders_to_place)} orders.")

        # Cancel orders first
        if self.orders_to_cancel:
            cancelled_ids = await self.http_client.cancel_batch_orders(self.orders_to_cancel)
        ...
        if self.orders_to_place:
            placed_orders_info = await self.http_client.place_batch_orders(self.orders_to_place)
            for info in placed_orders_info:
                order_id = info.get('orderId')
                if order_id:
                    original_order = next((o for o in self.orders_to_place if o.get('orderLinkId') == info.get('orderLinkId')), None)
                    if original_order:
...
class PositionManager:
    def __init__(self, bot_config: BotConfig, http_client: BybitHTTPClient, order_manager: OrderManager):
        self.config = bot_config
        self.http_client = http_client
    ...
    def process_virtual_fill(self, side: Literal["Buy", "Sell"], price: Decimal, qty: Decimal):
        ...
        if (self.size > 0 and new_position_size < self.size) or (self.size < 0 and new_position_size > self.size):
        ...
        logger.info(f"[DRY RUN] Fill: {side} {qty} @ {price}. Position: {old_size:.4f} → {self.size:.4f}")
...
class EnhancedBybitMarketMaker:
    def __init__(self, config: BotConfig):
        ...
        self.config.credentials.api_key = os.getenv('BYBIT_API_KEY', self.config.credentials.api_key)
        self.config.credentials.api_secret = os.getenv('BYBIT_API_SECRET', self.config.credentials.api_secret)

        if not self.config.credentials.api_key or not self.config.credentials.api_secret:
            raise ValueError("API keys not found. Ensure BYBIT_API_KEY and BYBIT_API_SECRET are set in .env or config.json.")
    ...
    async def _handle_websocket_message(self, msg: dict):
        ...
        if topic.startswith("orderbook"):
            ...
            if isinstance(data, dict) and data.get('midPrice') is not None:
                ...
                self.ta_manager.update_ta(self.bot_state.recent_prices) # Update TA on each new mid price
    ...
    def _calculate_tiered_quotes(self) -> Tuple[List[Dict], List[Dict]]:
        ...
        if best_bid >= best_ask:
            logger.debug(f"Market crossed: bid={best_bid}, ask={best_ask}. Skipping quotes.")
            return [], []
        ...
        if len(self.bot_state.recent_prices) >= vol_window:
            try:
                prices_f = [float(p) for p in list(self.bot_state.recent_prices)[-vol_window:]]
        ...
        if self.ta_manager.enabled:
            ...
            if trend_up:
                ...
                sell_bias_factor = Decimal("1") - qty_bias_max * signal_magnitude * Decimal("0.5") # Reduce selling
            elif trend_down:
    ...
    def generate_status_table(self) -> Table:
        ...
        if self.bot_state.last_known_best_bid and self.bot_state.last_known_best_ask:
            ...
            table.add_row("Best Bid/Ask", f"{best_bid:.{self.bot_state.instrument_info.price_precision}f} / {best_ask:.{self.bot_state.instrument_info.price_precision}f}")
            table.add_row("Spread", f"{spread:.{self.bot_state.instrument_info.price_precision}f} ([bold]{spread_bps:.1f}[/bold] bps)")
        else:
        ...
        position_size_str = f"[{pos_color}]{self.position_manager.size:.{self.bot_state.instrument_info.qty_precision}f}[/{pos_color}]" if self.position_manager else "N/A"
        avg_entry_str = f"{self.position_manager.avg_entry_price:.{self.bot_state.instrument_info.price_precision}f}" if self.position_manager else "N/A"
    ...
    async def _setup_initial_state(self):
        ...
        self.position_manager = PositionManager(self.config, self.http_client, self.order_manager)
            ...
            while True:
                try:
                    ...
                    if self.http_client.circuit_breaker_active:
                        await asyncio.sleep(1) # Sleep briefly while circuit breaker is active
                        continue

                    # Risk management check first
                    if self.position_manager and await self.position_manager.check_and_manage_risk(self.bot_state.last_mid_price):
                        ...
                        await asyncio.sleep(self.config.risk_management.circuit_breaker_cooldown_seconds) # Use circuit breaker cooldown
                        continue
                    ...
                    if not self.bot_state.last_mid_price or not self.bot_state.last_known_best_bid or not self.bot_state.last_known_best_ask:
                        logger.debug("No valid mid price/orderbook, skipping quote calculation.")
                        await asyncio.sleep(0.5)
                        continue
                    ...
                    if not buy_orders and not sell_orders:
                        if self.order_manager.current_orders: # If no quotes, but orders are open, cancel them
