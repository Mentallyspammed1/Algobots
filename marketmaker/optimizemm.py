...
async def run_simulation_for_trial(
    trial_config: Config, duration_ticks: int = 2000
) -> Decimal:
    """
        ...
        for i in range(duration_ticks):
            ...
            if (i + 1) % (duration_ticks // 10) == 0:
                optimizer_logger.debug(
                    f"  Simulating tick {i + 1}/{duration_ticks} for trial. Current PnL: {bot.state.metrics.net_realized_pnl:.4f}"
                )
...
def objective(trial: optuna.Trial) -> float:
    ...
    base_order_size_pct_of_balance = trial.suggest_float(
        "base_order_size_pct_of_balance",
        0.001,
        0.02,
        step=0.001,
...
if __name__ == "__main__":
    ...
    timeout_seconds = (
        7200  # Maximum optimization time in seconds (e.g., 7200s = 2 hours)
    )

    optimizer_logger.info(
        f"Starting optimization for {num_trials} new trials or {timeout_seconds} seconds."
    )
    optimizer_logger.info(
        "This process will run multiple bot simulations sequentially and might take significant time."
    )
    optimizer_logger.info(
