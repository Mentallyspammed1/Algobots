...
class StrategyConfig(BaseConfigModel):
    ...
    inventory_skew_decay_rate_per_order: Decimal = Field(
        Decimal("0.1"), ge=0, lt=1
    )  # How much skew decays with each order placed
...
class OrderManager:
    ...
    def _quotes_significantly_changed(
        ...
        if (
            ...
            if not approx_equal(mid_price, last_known_mid, price_drift_threshold):
                logger.debug(
                    f"Mid price drifted significantly (old: {last_known_mid}, new: {mid_price}), re-evaluating orders."
                )
                return True
    ...
    async def manage_orders(
        ...
        if not self._quotes_significantly_changed(
            current_open_orders_list, new_target_orders, mid_price, last_known_mid
        ):
...
class PositionManager:
    ...
    async def initialize_balance(self):
        ...
        balance = await self.http_client.get_wallet_balance(
            coin=self.config.market.symbol[:-4]
        )  # e.g., BTCUSDT -> BTC
        ...
        if balance is not None:
            ...
            logger.info(
                f"Initial wallet balance for PnL tracking: {self.initial_balance:.4f}"
            )
        else:
            logger.error(
                "Could not fetch initial wallet balance. Daily PnL tracking will be limited."
            )
    ...
    async def update_daily_pnl_snapshot(self):
        ...
        if now.date() > self.bot_state.daily_pnl_snapshot_time.date():
            current_balance = await self.http_client.get_wallet_balance(
                coin=self.config.market.symbol[:-4]
            ) or await self.http_client.get_wallet_balance(coin="USDT")
    ...
    def process_virtual_fill(
        ...
        logger.info(
            f"[DRY RUN] Fill: {side} {qty} @ {price}. Position: {old_size:.4f} -> {self.size:.4f}, Realized PnL: {self.realized_pnl:.4f}"
        )
    ...
    async def check_and_manage_risk(self, current_mid_price: Decimal | None) -> bool:
        ...
        current_wallet_balance = await self.http_client.get_wallet_balance(
            coin=self.config.market.symbol[:-4]
        ) or await self.http_client.get_wallet_balance(coin="USDT")

        if (
            self.initial_balance is not None
            and self.initial_balance > 0
            and current_wallet_balance is not None
        ):
    ...
    def _calculate_tiered_quotes(self) -> tuple[list[dict], list[dict]]:
        ...
        if self.ta_manager.enabled:
            ...
            if ema_f is not None and ema_s is not None:
                if (
                    ema_f > ema_s and self.bot_state.last_mid_price < mid_price
                ):  # Bullish cross
                    fair_value *= (
        ...
        if self.ta_manager.enabled:
            ...
            if trend_up_signal and not trend_down_signal:
                ...
                sell_bias_factor = Decimal("1") - qty_bias_max * abs(signal) * Decimal(
                    "0.5"
                )  # Reduce selling
            elif trend_down_signal and not trend_up_signal:
                sell_bias_factor = Decimal("1") + qty_bias_max * abs(signal)
                buy_bias_factor = Decimal("1") - qty_bias_max * abs(signal) * Decimal(
                    "0.5"
                )  # Reduce buying
        ...
        with Live(
            ...
            while True:
                try:
                    ...
                    now = time.time()
                    if (
                        now - self.order_manager.last_reprice_time
...
async def main():
    ...
    try:
        ...
        raw_config["credentials"]["api_key"] = os.getenv(
            "BYBIT_API_KEY", raw_config.get("credentials", {}).get("api_key", "")
        )
        raw_config["credentials"]["api_secret"] = os.getenv(
            "BYBIT_API_SECRET", raw_config.get("credentials", {}).get("api_secret", "")
        )
