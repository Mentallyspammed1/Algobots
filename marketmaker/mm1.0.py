...
class TradeMetrics:
    ...
    average_entry_price: Decimal = Decimal('0') # Weighted average price for current holdings
...
class InventoryStrategyConfig:
    enabled: bool = True
    skew_intensity: Decimal = Decimal('0.5') # How much inventory affects price, e.g., 0.5 means 1% inventory -> 0.5% price skew
    max_inventory_ratio: Decimal = Decimal('0.5') # Max inventory percentage (e.g., 0.5 means 50% of max_net_exposure)

@dataclass
class DynamicSpreadConfig:
    enabled: bool = True
    volatility_window_sec: int = 60 # Window for calculating volatility in seconds
    volatility_multiplier: Decimal = Decimal('2.0') # Multiplier for volatility to adjust spread
    min_spread_pct: Decimal = Decimal('0.0005') # 0.05%
    max_spread_pct: Decimal = Decimal('0.01') # 1%
    price_change_smoothing_factor: Decimal = Decimal('0.2') # Factor for exponential moving average of mid_price for smoother volatility
...
class CircuitBreakerConfig:
    ...
    cool_down_after_trip_sec: int = 300 # Additional cooldown after pause ends before re-enabling

@dataclass
class StrategyConfig:
    base_spread_pct: Decimal = Decimal('0.001') # 0.1%
    base_order_size_pct_of_balance: Decimal = Decimal('0.005') # 0.5% of available balance
    order_stale_threshold_pct: Decimal = Decimal('0.0005') # 0.05% difference to consider order stale
    min_profit_spread_after_fees_pct: Decimal = Decimal('0.0002') # Minimum profit spread after estimated fees
...
class SystemConfig:
    ...
    ws_heartbeat_sec: int = 30 # Max time without WS message before considering it stale
    ...
    api_retry_max_delay_sec: float = 10.0 # Max delay for API retries with exponential backoff
...
class Config:
    ...
    max_net_exposure_usd: Decimal = Decimal('500') # Max total exposure in USD (for linear contracts)
    ...
    metrics: TradeMetrics = field(default_factory=TradeMetrics) # Metrics now includes PnL tracking
    ...
    def __post_init__(self):
        ...
        if self.symbol.endswith("USDT"):
            ...
            if len(self.symbol) == 6: # Heuristic for 3-char base/quote
                ...
                raise ConfigurationError(f"Could not parse base/quote currency from symbol: {self.symbol}")
...
class StateManager:
    ...
    async def load_state(self) -> dict | None:
        ...
        try:
            ...
            self.logger.error(f"Error loading state from {self.file_path}: {e}. Starting fresh.")
            return None
...
class DBManager:
    ...
    async def log_order_event(self, o: dict, m: Optional[str] = None):
        ...
        try:
            await self.conn.execute("INSERT INTO order_events (timestamp, order_id, order_link_id, symbol, side, order_type, price, qty, status, message) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (datetime.now(timezone.utc).isoformat(), o.get('orderId'), o.get('orderLinkId'), o.get('symbol'), o.get('side'), o.get('orderType'), str(o.get('price','0')), str(o.get('qty','0')), o.get('orderStatus'), m))
            await self.conn.commit()
        except Exception as e:
    ...
    async def log_trade_fill(self, t: dict, realized_pnl_impact: Decimal):
        ...
        try:
            await self.conn.execute("INSERT INTO trade_fills (timestamp, order_id, trade_id, symbol, side, exec_price, exec_qty, fee, fee_currency, pnl, realized_pnl_impact) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (datetime.now(timezone.utc).isoformat(), t.get('orderId'), t.get('execId'), t.get('symbol'), t.get('side'), str(t.get('execPrice','0')), str(t.get('execQty','0')), str(t.get('execFee','0')), t.get('feeCurrency'), str(t.get('pnl','0')), str(realized_pnl_impact)))
            await self.conn.commit()
        except Exception as e:
    ...
    async def log_balance_update(self, c: str, wb: Decimal, ab: Optional[Decimal] = None):
        if not self.conn: return
        try:
            await self.conn.execute("INSERT INTO balance_updates (timestamp, currency, wallet_balance, available_balance) VALUES (?, ?, ?, ?)", (datetime.now(timezone.utc).isoformat(), c, str(wb), str(ab) if ab else None))
            await self.conn.commit()
        except Exception as e:
    ...
    async def log_bot_metrics(self, m: TradeMetrics, unrealized_pnl: Decimal):
        if not self.conn: return
        try:
            await self.conn.execute("INSERT INTO bot_metrics (timestamp, total_trades, total_pnl, realized_pnl, unrealized_pnl, gross_profit, gross_loss, total_fees, wins, losses, win_rate, current_asset_holdings, average_entry_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (datetime.now(timezone.utc).isoformat(), m.total_trades, str(m.total_pnl + unrealized_pnl), str(m.realized_pnl), str(unrealized_pnl), str(m.gross_profit), str(m.gross_loss), str(m.total_fees), m.wins, m.losses, m.win_rate, str(m.current_asset_holdings), str(m.average_entry_price)))
            await self.conn.commit()
        except Exception as e:
...
class TradingClient:
    ...
    def _handle_response(self, response: Any, action: str):
        ...
        self.logger.error(f"API {action} failed: {ret_msg} (ErrCode: {ret_code}). Full response: {response}")
        if ret_code == 10004:
    ...
    async def get_position_info_impl(self) -> Optional[dict]:
        ...
        response = self.http_session.get_positions(category=self.config.category, symbol=self.config.symbol)
    ...
    cancel_order: Callable[[str, Optional[str]], Coroutine[Any, Any, bool]]
...
class BybitMarketMaker:
    def __init__(self, config: Config):
        ...
        self.current_position_qty = Decimal('0') # Positive for long, negative for short (for derivatives)
        self.unrealized_pnl_derivatives = Decimal('0') # From exchange for derivatives

        self.active_orders: dict[str, dict] = {} # orderId -> {side, price, qty, status, orderLinkId, symbol}
        ...
        self.mid_price_history = deque(maxlen=self.config.strategy.dynamic_spread.volatility_window_sec * 2) # Store more data than window length
        self.circuit_breaker_price_points: deque[tuple[float, Decimal]] = deque(maxlen=self.config.strategy.circuit_breaker.check_window_sec * 2) # (timestamp, price)
    ...
    async def run(self):
        ...
        except (APIAuthError, WebSocketConnectionError, asyncio.CancelledError, KeyboardInterrupt) as e:
    ...
    async def _load_bot_state(self):
        ...
        if state:
            ...
            self.config.metrics.realized_pnl = Decimal(str(self.config.metrics.realized_pnl))
            self.config.metrics.current_asset_holdings = Decimal(str(self.config.metrics.current_asset_holdings))
            self.config.metrics.average_entry_price = Decimal(str(self.config.metrics.average_entry_price))

            self.logger.info(f"Loaded state with {len(self.active_orders)} active orders and metrics: {self.config.metrics}")
            # Restore any pause state
            self.is_paused = state.get('is_paused', False)
            self.pause_end_time = state.get('pause_end_time', 0)
            self.circuit_breaker_cooldown_end_time = state.get('circuit_breaker_cooldown_end_time', 0)
        else:
    ...
    async def _reconnect_websockets(self):
        ...
        delay = self.config.system.ws_reconnect_initial_delay_sec * (2 ** (self.config.system.ws_reconnect_attempts - self.ws_reconnect_attempts_left -1 ))
        delay = min(delay, 60) # Cap max delay to 60 seconds
        self.logger.warning(f"Attempting WebSocket reconnection in {delay:.1f} seconds... ({self.ws_reconnect_attempts_left} attempts left)")
    ...
    async def _main_loop_tick(self):
        ...
        if self.config.trading_mode not in ["DRY_RUN", "SIMULATION"]:
            ...
            if not self.is_running: return # Check if health check decided to stop bot

        await self._periodic_health_check(current_time)

        if self.is_paused and current_time < self.pause_end_time:
            self.logger.debug(f"Bot is paused. Resuming in {int(self.pause_end_time - current_time)}s.")
            return # Bot is paused
        elif self.is_paused:
            self.logger.info("Circuit breaker cooldown finished. Resuming trading.")
            self.is_paused = False
            self.circuit_breaker_cooldown_end_time = current_time + self.config.strategy.circuit_breaker.cool_down_after_trip_sec

        if current_time < self.circuit_breaker_cooldown_end_time:
            self.logger.debug(f"Circuit breaker in cooldown. Resuming trading in {int(self.circuit_breaker_cooldown_end_time - current_time)}s.")
            return
    ...
    async def _periodic_health_check(self, current_time: float):
        if (current_time - self.last_health_check_time) < self.config.system.health_check_interval_sec: return
    ...
    async def stop(self):
        ...
        try:
            unrealized_pnl = self.config.metrics.calculate_unrealized_pnl(self.mid_price)
    ...
    async def _update_mid_price(self, bids: list, asks: list):
        async with self.market_data_lock:
            ...
            if new_mid_price != self.mid_price:
                ...
                self.mid_price_history.append(float(self.mid_price)) # Store float for numpy
                self.circuit_breaker_price_points.append((time.time(), self.mid_price)) # Store Decimal for precision
                ...
                if self.smoothed_mid_price == Decimal('0'):
                    ...
                    self.smoothed_mid_price = (alpha * new_mid_price) + ((Decimal('1') - alpha) * self.smoothed_mid_price)

                self.logger.debug(f"Mid-price updated to: {self.mid_price}, Smoothed: {self.smoothed_mid_price}")
        ...
        try:
            if 'data' in message:
                for order_data in message['data']:
                    if order_data.get('symbol') == self.config.symbol: # Filter for our symbol
        ...
        async with self.active_orders_lock:
            if order_id in self.active_orders:
                ...
                if status == 'Filled':
                    ...
                    del self.active_orders[order_id]
            elif status == 'Filled': # Handle fills for orders not tracked locally (e.g., manual orders or bot restart)
                self.logger.warning(f"Received fill for untracked order {order_id}. Processing fill.")
                await self._process_fill(order_data)
            else:
    ...
    async def _process_position_update(self, pos_data: dict):
        async with self.balance_position_lock:
            ...
            if self.config.category in ['linear', 'inverse'] and 'unrealisedPnl' in pos_data:
                ...
                self.logger.debug(f"UNREALIZED PNL (WS): {self.unrealized_pnl_derivatives:+.4f} {self.config.quote_currency}")
    ...
    async def _manage_orders(self):
        ...
        async with self.market_data_lock, self.balance_position_lock:
            if self.mid_price == Decimal('0') or not self.market_info:
                ...
                return
            mid_price, pos_qty = self.mid_price, self.config.metrics.current_asset_holdings # Use bot's tracked holdings for inventory
    ...
    async def _check_circuit_breaker(self) -> bool:
        ...
        recent_prices_raw = [(t, p) for t, p in self.circuit_breaker_price_points if (current_time - t) <= cb_config.check_window_sec]

        if len(recent_prices_raw) < 2: return False # Not enough data for a meaningful check
        ...
        if price_change_pct > cb_config.pause_threshold_pct:
            ...
            self.circuit_breaker_cooldown_end_time = current_time + cb_config.pause_duration_sec + cb_config.cool_down_after_trip_sec
            await self._cancel_all_orders()
            return True
        return False
    ...
    def _calculate_dynamic_spread(self) -> Decimal:
        ...
        current_time = time.time()
        volatility_window_prices = [p for t, p in self.circuit_breaker_price_points if (current_time - t) <= ds_config.volatility_window_sec]
    ...
    def _enforce_min_profit_spread(self, mid_price: Decimal, bid_p: Decimal, ask_p: Decimal) -> tuple[Decimal, Decimal]:
        ...
        if ask_p <= bid_p:
            ...
            self.logger.debug(f"Adjusting inverted/zero spread. New Bid: {bid_p}, Ask: {ask_p}")

        # Ensure the spread is at least min_spread_val
        current_spread = ask_p - bid_p
        if current_spread < min_spread_val:
            self.logger.debug(f"Adjusting spread to enforce min profit. Original Spread: {current_spread:.6f}, Min Spread: {min_spread_val:.6f}")
            half_spread_adjustment = ((min_spread_val - current_spread) / Decimal('2')).quantize(self.market_info.price_precision)
            bid_p = (bid_p - half_spread_adjustment).quantize(self.market_info.price_precision)
            ask_p = (ask_p + half_spread_adjustment).quantize(self.market_info.price_precision)
        return bid_p, ask_p
        ...
        async with self.active_orders_lock:
            ...
            for oid, o in list(self.active_orders.items()):
                if o.get('symbol') != self.config.symbol:
                    self.logger.warning(f"Found untracked symbol order {oid} in active_orders. Removing.")
    ...
    async def _log_status_summary(self):
        async with self.balance_position_lock, self.active_orders_lock:
            ...
            total_current_pnl = metrics.total_pnl + unrealized_pnl # Realized + Unrealized PnL
            ...
            if self.config.category in ['linear', 'inverse']:
                display_unrealized_pnl = self.unrealized_pnl_derivatives # From exchange for derivatives
        ...
        self.logger.info(f"STATUS | Total PNL: {total_current_pnl:+.4f} | {pnl_summary} | Win Rate: {metrics.win_rate:.2f}% | Position: {pos_qty} {self.config.base_currency} (Exposure: {exposure_usd:+.2f} {self.config.quote_currency}) | Orders: {active_buys} Buy / {active_sells} Sell")
    ...
    async def _fetch_market_info(self) -> bool:
        ...
        try:
            ...
            self.logger.critical(f"Missing expected key in instrument info: {e}. Full info: {info}")
            return False
        except Exception as e:
    ...
    async def _update_balance_and_position(self) -> bool:
        async with self.balance_position_lock:
            if self.config.trading_mode == "DRY_RUN" or self.config.trading_mode == "SIMULATION":
                ...
                if self.current_balance == Decimal('0'):
                    ...
                    self.logger.info(f"{self.config.trading_mode}: Initialized virtual balance: {self.current_balance} {self.config.quote_currency}")
                return True
            ...
            if self.config.category in ['linear', 'inverse']:
                ...
                if position_data and position_data.get('size'):
                    ...
                    self.unrealized_pnl_derivatives = Decimal(position_data.get('unrealisedPnl', '0'))
                else:
                ...
                self.unrealized_pnl_derivatives = Decimal('0') # Not applicable for spot

            self.logger.info(f"Updated Balance: {self.current_balance} {self.config.quote_currency}, Position: {self.current_position_qty} {self.config.base_currency}, UPNL (Deriv): {self.unrealized_pnl_derivatives:+.4f}")
            return True
    ...
    async def _calculate_order_size(self, side: str, price: Decimal) -> Decimal:
        async with self.balance_position_lock:
            ...
            metrics_pos_qty = self.config.metrics.current_asset_holdings # Bot's view of holdings
        ...
        eff_capital = capital * self.config.leverage if self.config.category in ['linear', 'inverse'] else capital
        ...
        if self.config.max_net_exposure_usd > 0 and self.config.strategy.inventory.enabled:
    ...
    async def _place_limit_order(self, side: str, price: Decimal, quantity: Decimal):
        ...
        if self.config.trading_mode == "DRY_RUN" or self.config.trading_mode == "SIMULATION":
            ...
            async with self.active_orders_lock:
                self.active_orders[oid] = {'side': side, 'price': price_f, 'qty': qty_f, 'status': 'New', 'orderLinkId': order_link_id, 'symbol': self.config.symbol}
    ...
    async def _reconcile_orders_on_startup(self):
        ...
        try:
            ...
            self.logger.error(f"Failed to fetch open orders from exchange during reconciliation: {e}. Proceeding with local state only.")
            exchange_orders = {}
...
if __name__ == "__main__":
    try:
        ...
        print("\nBot stopped by user.")
    except (ValueError, APIAuthError, MarketInfoError, InitialBalanceError, WebSocketConnectionError, ConfigurationError) as e:
        print(f"\nCritical Initialization Error: {e}")
        logging.getLogger('MarketMakerBot').critical(f"Critical Initialization Error: {e}", exc_info=True)
        sys.exit(1)
    except Exception as e:
