...
try:
    ...
    try:
        ...
        print(f"{Fore.YELLOW}Warning: 'pandas_ta' not found. ATR calculation will use a basic rolling mean.{Style.RESET_ALL}")
    ...
    print(f"{Fore.YELLOW}Install all dependencies with: pip install numpy pandas websocket-client pydantic colorama python-dotenv aiofiles aiosqlite pybit tenacity pandas_ta{Style.RESET_ALL}")
    ...
    class np:
        @staticmethod
        def exp(*args, **kwargs): return 1.0
        @staticmethod
        def sqrt(*args, **kwargs): return 1.0
        @staticmethod
        def random():
            class Random:
                @staticmethod
                def normal(*args, **kwargs): return 0.0
            return Random()
    ...
    class aiofiles:
        @staticmethod
        async def open(*args, **kwargs): return DummyModel()
    class aiosqlite:
        @staticmethod
        async def connect(*args, **kwargs): return DummyModel()
        @staticmethod
        async def Row(*args, **kwargs): return DummyModel()
    class HTTP: pass
    class WebSocket: pass
    def retry(*args, **kwargs): return lambda f: f
    def stop_after_attempt(*args, **kwargs): return DummyModel()
    def wait_exponential_jitter(*args, **kwargs): return DummyModel()
    def retry_if_exception(*args, **kwargs): return DummyModel()
    def before_sleep_log(*args, **kwargs): return DummyModel()
...
class DynamicSpreadConfig(BaseModel):
    ...
    enabled: bool = True
    volatility_window_sec: PositiveInt = 60 # Window for price change to calculate volatility
    volatility_multiplier: PositiveFloat = 2.0 # Multiplier for volatility to adjust spread
    min_spread_pct: PositiveFloat = 0.0005 # Minimum allowed dynamic spread (e.g., 0.05%)
...
class InventorySkewConfig(BaseModel):
    ...
    max_inventory_ratio: PositiveFloat = 0.5 # Max inventory as ratio of max_net_exposure_usd to trigger full skew
...
class CircuitBreakerConfig(BaseModel):
    ...
    enabled: bool = True
    pause_threshold_pct: PositiveFloat = 0.02 # Price change % in window to trip circuit breaker
    ...
    cool_down_after_trip_sec: PositiveInt = 300 # Cooldown period before re-enabling circuit breaker logic
...
class StrategyConfig(BaseModel):
    ...
    order_stale_threshold_pct: PositiveFloat = 0.0005 # Price change % to consider an order stale and cancel
    min_profit_spread_after_fees_pct: PositiveFloat = 0.0002 # Minimum spread to ensure profitability after fees
    max_outstanding_orders: PositiveInt = 2 # Max number of active limit orders per side
    market_data_stale_timeout_seconds: PositiveInt = 30 # Max age for orderbook/trade data before considered stale
    enable_auto_sl_tp: bool = False # Enable automatic Stop-Loss and Take-Profit on market-making orders
    take_profit_target_pct: PositiveFloat = 0.005 # Take-Profit percentage from entry price
    stop_loss_trigger_pct: PositiveFloat = 0.005 # Stop-Loss percentage from entry price
...
class SystemConfig(BaseModel):
    ...
    order_refresh_interval_sec: PositiveFloat = 5.0 # How often to refresh/reconcile orders
    ws_heartbeat_sec: PositiveInt = 30 # WebSocket heartbeat interval
    cancellation_rate_limit_sec: PositiveFloat = 0.2 # Min delay between API cancellation requests
    ...
    api_retry_initial_delay_sec: PositiveFloat = 0.5 # Initial delay for API call retries
    api_retry_max_delay_sec: PositiveFloat = 10.0 # Max delay for API call retries
    health_check_interval_sec: PositiveInt = 10 # How often to check account health (balance, position)
...
class GlobalConfig(BaseModel):
    ...
    main_quote_currency: str = Field(default_factory=lambda: os.getenv("MAIN_QUOTE_CURRENCY", "USDT")) # For overall balance tracking
...
class SymbolConfig(BaseModel):
    ...
    def __pydantic_post_init__(self, __context: Any) -> None:
        ...
        elif self.symbol and len(self.symbol) > 4 and self.symbol[-4:].isupper(): # e.g., BTCUSDT
            self.base_currency = self.symbol[:-4]
            self.quote_currency = self.symbol[-4:]
        elif self.symbol and len(self.symbol) > 3 and self.symbol[-3:].isupper(): # e.g., BTCUSD (inverse)
...
class ConfigManager:
    ...
    def load_config(cls, single_symbol: Optional[str] = None) -> Tuple[GlobalConfig, Dict[str, SymbolConfig]]:
        ...
        """
        try:
            cls._global_config = GlobalConfig() # Pydantic v2 automatically loads from env
        ...
        if single_symbol:
            ...
            default_symbol_data = {
                ...
                "strategy": StrategyConfig().model_dump(mode='json_compatible') # Convert to dict
            }
            try:
...
def retry_api_call():
    ...
    if global_config is None: # Fallback if config not loaded yet (shouldn't happen in normal flow)
...
class TradeMetrics:
    ...
    current_asset_holdings: Decimal = DECIMAL_ZERO # For spot, tracks base currency holdings
    average_entry_price: Decimal = DECIMAL_ZERO # For spot, tracks average entry price of holdings
    ...
    def update_pnl_on_sell(self, quantity: Decimal, price: Decimal) -> None:
        ...
        self.realized_pnl += (price - self.average_entry_price) * quantity # Realized PnL from this specific sale
...
class TradingState:
    ...
    current_position_qty: Decimal = DECIMAL_ZERO # For base currency, e.g., BTC (signed: +long, -short)

    # For derivatives, this is the exchange-reported unrealized PnL
    unrealized_pnl_derivatives: Decimal = DECIMAL_ZERO

    active_orders: Dict[str, Dict[str, Any]] = field(default_factory=dict) # {order_id: order_details}
...
class BybitAPIClient:
    ...
    get_instruments_info: Callable[[str, str], Coroutine[Any, Any, Dict[str, Any] | None]] = field(init=False)
    get_wallet_balance: Callable[[str], Coroutine[Any, Any, Dict[str, Any] | None]] = field(init=False)
    get_position_info: Callable[[str, str], Coroutine[Any, Any, Dict[str, Any] | None]] = field(init=False)
    set_leverage: Callable[[str, str, Decimal], Coroutine[Any, Any, bool]] = field(init=False)
    get_open_orders: Callable[[str, str], Coroutine[Any, Any, List[Dict[str, Any]]]] = field(init=False)
    place_order: Callable[[Dict[str, Any]], Coroutine[Any, Any, Dict[str, Any] | None]] = field(init=False)
    cancel_order: Callable[[str, str, str, str | None], Coroutine[Any, Any, bool]] = field(init=False)
    cancel_all_orders: Callable[[str, str], Coroutine[Any, Any, bool]] = field(init=False)
    set_trading_stop: Callable[[str, str, Decimal, Decimal], Coroutine[Any, Any, bool]] = field(init=False)
    get_kline: Callable[[str, str, str, int], Coroutine[Any, Any, List[List[Any]]]] = field(init=False)
...
class BybitWebSocketClient:
    ...
    def _normalize_symbol_ws(self, bybit_symbol_ws: str) -> str:
        ...
        if self.config.category == 'spot':
            ...
            if len(bybit_symbol_ws) >= 6 and bybit_symbol_ws[-4:].isupper() and bybit_symbol_ws[:-4].isupper():
                return f"{bybit_symbol_ws[:-4]}/{bybit_symbol_ws[-4:]}"
            elif len(bybit_symbol_ws) >= 5 and bybit_symbol_ws[-3:].isupper() and bybit_symbol_ws[:-3].isupper():
        ...
        if bybit_symbol_ws.endswith("USDT"):
            return f"{bybit_symbol_ws[:-4]}/{bybit_symbol_ws[-4:]}:{bybit_symbol_ws[-4:]}"
            ...
            if kline_data.get('confirm'): # Only process confirmed (closed) candles
                ...
                with self.data_lock:
                    if symbol in self.symbol_bots:
                        ...
                        self.logger.debug(f"[{symbol}] Kline updated (interval: {interval}): C={close_price}, H={high_price}, L={low_price}")
                    ...
                    with self.data_lock: # Access symbol_bots safely
                        for bot in self.symbol_bots.values(): # Iterate through registered SymbolBot instances
                            if bot.config.symbol == normalized_symbol:
                                ...
                                elif topic == "position": await bot._process_position_update(item_data)
                                elif topic == "execution" and item_data.get("execType") in ["Trade", "AdlTrade", "BustTrade"]: await bot._process_execution_update(item_data)
                                elif topic == "wallet": await bot._update_balance_from_wallet_ws(item_data)
                                break
        ...
        while not self._stop_event.is_set():
            ...
            if new_ws_instance:
                ...
                if attempts >= self.config.system.ws_reconnect_attempts:
                    self.logger.critical(f"{Colors.NEON_RED}{stream_name} WebSocket reconnection failed after {self.config.system.ws_reconnect_attempts} attempts. Signalling shutdown.{Colors.RESET}")
    ...
    async def start_streams(self, public_topics: List[str], private_topics: List[str]) -> None:
...
class AsyncSymbolBot:
    ...
    def __init__(self, global_config: GlobalConfig, symbol_config: SymbolConfig,
        ...
        self.state.price_candlestick_history = deque(maxlen=200) # Keep enough history for ATR
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if not self._is_trading_hours(datetime.now(timezone.utc)):
                    self.logger.info(f"[{self.config.symbol}] Outside trading hours. Cancelling all orders and pausing.")
    ...
    async def _load_state(self):
        ...
        if state_data:
            ...
            self.state.active_orders = {
                oid: {k: Decimal(v) if isinstance(v, str) and k in ['price', 'qty', 'cumExecQty'] else v for k, v in odata.items()}
            ...
            self.state.price_candlestick_history.extend([(float(t), Decimal(str(h)), Decimal(str(l)), Decimal(str(c))) for t, h, l, c in state_data.get('price_candlestick_history', [])])
            ...
            self.state.last_dry_run_price_update_time = float(state_data.get('last_dry_run_price_update_time', time.time()))
    ...
    async def _update_mid_price(self, bids: List[List[Decimal]], asks: List[List[Decimal]]) -> None:
        """Updates mid-price and related historical data from orderbook."""
        if not bids or not asks:
            self.logger.debug(f"[{self.config.symbol}] Orderbook empty or incomplete. Cannot update mid-price.")
            return
        ...
        if new_mid_price != self.state.mid_price:
            ...
            if self.state.price_candlestick_history:
                ...
                if (current_time - last_ts) < self.global_config.system.loop_interval_sec * 2: # Arbitrary small window
    ...
    async def _check_market_data_freshness(self, current_time: float) -> bool:
        ...
        if self.config.symbol not in self.ws_client.last_kline_update_time or \
           (current_time - self.ws_client.last_kline_update_time.get(self.config.symbol, 0) > self.config.strategy.market_data_stale_timeout_seconds * 2): # Allow kline to be a bit older
            kline_stale = True
    ...
    async def _process_position_update(self, pos_data: Dict[str, Any]):
        ...
        if self.global_config.category in ['linear', 'inverse'] and 'unrealisedPnl' in pos_data:
            ...
            self.logger.debug(f"[{self.config.symbol}] UNREALIZED PNL (WS): {self.state.unrealized_pnl_derivatives:+.4f} {self.global_config.main_quote_currency}")
    ...
    async def _process_execution_update(self, trade_data: Dict[str, Any]):
        ...
        if side == 'Buy':
            ...
            if self.global_config.trading_mode in ["DRY_RUN", "SIMULATION"]:
                ...
                self.state.available_balance = self.state.current_balance
            self.logger.info(f"[{self.config.symbol}] Order FILLED: SELL {exec_qty} @ {exec_price}, Fee: {exec_fee}. Realized PnL from sale: {closed_pnl:+.4f}. Holdings: {metrics.current_asset_holdings}, Avg Entry: {metrics.average_entry_price}")
        else:
    ...
    async def _manage_orders(self):
        ...
        skew_factor = self._calculate_inventory_skew(self.state.smoothed_mid_price, self.state.current_position_qty)
    ...
    async def _calculate_atr_from_kline(self, current_mid_price: Decimal) -> Decimal:
        ...
        if self.global_config.trading_mode in ["DRY_RUN", "SIMULATION"]:
            ...
            if len(self.state.price_candlestick_history) < 15: # Need enough data points for 14-period ATR
                self.logger.debug(f"[{self.config.symbol}] Not enough internal OHLCV data for ATR in dry run ({len(self.state.price_candlestick_history)}). Returning cached or zero.")
                return self.cached_atr if self.cached_atr > DECIMAL_ZERO else DECIMAL_ZERO

            # Convert deque to pandas DataFrame
            df = pd.DataFrame(list(self.state.price_candlestick_history), columns=["timestamp", "high", "low", "close"])
        else:
            try:
                ...
                self.logger.error(f"{Colors.NEON_RED}[{self.config.symbol}] Error fetching kline data for ATR: {e}{Colors.RESET}", exc_info=True)
                return self.cached_atr if self.cached_atr > DECIMAL_ZERO else DECIMAL_ZERO
        ...
        if "high" not in df.columns or "low" not in df.columns or "close" not in df.columns:
            self.logger.warning(f"[{self.config.symbol}] Missing columns for ATR calculation in OHLCV data.")
            return self.cached_atr if self.cached_atr > DECIMAL_ZERO else DECIMAL_ZERO

        # Use pandas_ta.atr if available, otherwise fallback to local implementation
        if ta is not None:
            atr_series = ta.atr(df["high"], df["low"], df["close"], length=14) # Default ATR length
            atr_val = atr_series.iloc[-1] # Get the latest ATR value
        else:
            atr_val = atr(df["high"], df["low"], df["close"], length=14).iloc[-1] # Use local atr
    ...
    async def _reconcile_and_place_orders(self, base_target_bid: Decimal, base_target_ask: Decimal):
        ...
        current_active_orders_by_side_layer: Dict[str, Dict[str, List[Tuple[str, Dict[str, Any]]]]] = {'Buy': defaultdict(list), 'Sell': defaultdict(list)}
        ...
        for order_id, order_data in list(self.state.active_orders.items()):
            ...
            if order_data['status'] in ['Filled', 'Cancelled', 'Rejected', 'Deactivated', 'Expired']:
                if order_data.get('cumExecQty', DECIMAL_ZERO) >= order_data.get('qty', DECIMAL_ZERO): # Fully filled or effectively filled
            ...
            layer_config = next((l for l in self.config.strategy.order_layers if f"layer_{self.config.strategy.order_layers.index(l)}" == layer_tag), self.config.strategy.order_layers[0])

            layer_bid_price_target = base_target_bid * (Decimal('1') - Decimal(str(layer_config.spread_offset_pct)))
            layer_ask_price_target = base_target_ask * (Decimal('1') + Decimal(str(layer_config.spread_offset_pct)))

            if order_data['side'] == 'Buy':
                if abs(order_data['price'] - layer_bid_price_target) > (order_data['price'] * stale_threshold):
                    is_stale = True
                current_active_orders_by_side_layer['Buy'][layer_tag].append((order_id, order_data))
            else: # Sell order
                if abs(order_data['price'] - layer_ask_price_target) > (order_data['price'] * stale_threshold):
                    is_stale = True
                current_active_orders_by_side_layer['Sell'][layer_tag].append((order_id, order_data))
        ...
        for i, layer in enumerate(self.config.strategy.order_layers):
            if current_outstanding_orders >= self.config.strategy.max_outstanding_orders * 2: # Max outstanding orders for both sides
                self.logger.debug(f"[{self.config.symbol}] Max outstanding orders ({self.config.strategy.max_outstanding_orders*2}) reached. Skipping further layer placements.")
                break
            ...
            if not current_active_orders_by_side_layer['Buy'][layer_tag]:
                ...
                if buy_qty > DECIMAL_ZERO:
                    await self._place_limit_order("Buy", layer_bid_price, buy_qty, layer_tag)
            ...
            if current_outstanding_orders < self.config.strategy.max_outstanding_orders * 2 and not current_active_orders_by_side_layer['Sell'][layer_tag]:
                sell_qty = await self._calculate_order_size("Sell", layer_ask_price, layer.quantity_multiplier)
                if sell_qty > DECIMAL_ZERO:
                    await self._place_limit_order("Sell", layer_ask_price, sell_qty, layer_tag)
                    current_outstanding_orders += 1
                else:
    ...
    async def _place_limit_order(self, side: str, price: Decimal, quantity: Decimal, layer_tag: str = "base"):
        ...
        order_link_id = f"mm_{bybit_symbol}_{side}_{layer_tag}_{int(time.time() * 1000)}"
    ...
    async def _cancel_order(self, order_id: str, order_link_id: str | None = None):
        ...
        try:
            # Bybit expects symbol without / or : for order placement
            bybit_symbol = self.config.symbol.replace("/", "").replace(":", "")
            if await self.api_client.cancel_order(self.global_config.category, bybit_symbol, order_id, order_link_id):
    ...
    async def _cancel_all_orders(self):
        ...
        if self.global_config.trading_mode in ["DRY_RUN", "SIMULATION"]:
            ...
            try:
                # Bybit expects symbol without / or : for order placement
                bybit_symbol = self.config.symbol.replace("/", "").replace(":", "")
                if await self.api_client.cancel_all_orders(self.global_config.category, bybit_symbol):
    ...
    async def _reconcile_orders_on_startup(self):
        ...
        try:
            # Bybit expects symbol without / or : for order placement
            bybit_symbol = self.config.symbol.replace("/", "").replace(":", "")
            exchange_orders = {o['orderId']: o for o in await self.api_client.get_open_orders(self.global_config.category, bybit_symbol)}
        except Exception as e:
    ...
    async def _update_take_profit_stop_loss(self):
        ...
        try:
            # Bybit expects symbol without / or : for order placement
            bybit_symbol = self.config.symbol.replace("/", "").replace(":", "")
            if await self.api_client.set_trading_stop(self.global_config.category, bybit_symbol, sl_price, tp_price):
                self.logger.info(
    ...
    async def _check_circuit_breaker(self, current_time: float) -> bool:
        ...
        if current_time < self.state.circuit_breaker_cooldown_end_time:
            self.logger.debug(f"[{self.config.symbol}] Circuit breaker in cooldown. Not checking for new trip.")
            return False
    ...
    async def _check_daily_pnl_limits(self) -> bool:
        ...
        reset_date = self.state.daily_pnl_reset_date.date() if self.state.daily_pnl_reset_date else None
        ...
        daily_loss_pct = (daily_pnl / self.state.daily_initial_capital) if self.state.daily_initial_capital > DECIMAL_ZERO and daily_pnl < DECIMAL_ZERO else DECIMAL_ZERO
    ...
    def _is_trading_hours(self, current_time_utc: datetime) -> bool:
        ...
        try:
            ...
            self.logger.error(f"[{self.config.symbol}] Invalid trading hours format: {e}. Trading hours check skipped.", exc_info=True)
    ...
    async def _simulate_dry_run_price_movement(self, current_time: float) -> None:
        ...
        if price_float <= 0: price_float = 1e-10 # Prevent division by zero or negative prices

        new_price_float = price_float * np.exp(
            (mu - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * np.random.normal()
        )
        if new_price_float < 1e-8: new_price_float = 1e-8 # Prevent extremely low prices
    ...
    async def _simulate_dry_run_fills(self) -> None:
        ...
        for order_id, order_data, fill_qty in orders_to_process:
            ...
            mock_fill_data = {
                ...
                'execFee': str(fill_qty * self.state.mid_price * (self.config.taker_fee_rate if self.config.taker_fee_rate else DECIMAL_ZERO)),
            ...
            await self._process_execution_update(mock_fill_data) # Process as a real execution update
    ...
    async def _log_status_summary(self) -> None:
        ...
        pos_qty = self.state.current_position_qty # Use current_position_qty for general reporting
        ...
        daily_profit_pct = float(daily_pnl / self.state.daily_initial_capital) if self.state.daily_initial_capital > DECIMAL_ZERO and daily_pnl > DECIMAL_ZERO else 0.0
...
class PyrmethusBot:
    ...
    def _setup_signal_handlers(self) -> None:
        ...
        signal.signal(signal.SIGINT, lambda signum, frame: asyncio.create_task(handle_shutdown(signum, frame)))
        signal.signal(signal.SIGTERM, lambda signum, frame: asyncio.create_task(handle_shutdown(signum, frame)))
        self.logger.info(f"{Colors.CYAN}# Signal handlers for graceful shutdown attuned.{Colors.RESET}")
    ...
    async def _start_symbol_bots(self, symbol_configs: Dict[str, SymbolConfig]) -> None:
        ...
        for symbol in list(symbols_currently_active): # Iterate over a copy as we modify the dict
            if symbol not in symbols_in_new_config or not symbol_configs[symbol].trade_enabled:
                ...
                self.logger.info(f"{Colors.CYAN}# SymbolBot for {symbol} has ceased its ritual.{Colors.RESET}")
        ...
        for symbol, config in symbol_configs.items():
            ...
            if symbol not in self.symbol_bots:
                self.logger.info(f"{Colors.CYAN}Summoning AsyncSymbolBot for {symbol}...{Colors.RESET}")
    ...
    async def _update_websocket_subscriptions(self) -> None:
        ...
        for symbol_cfg in self.active_symbol_configs.values():
            ...
            public_topics.append(f"kline.{symbol_cfg.strategy.kline_interval}.{symbol_ws_format}")
    ...
    async def _stop_all_bots(self) -> None:
        ...
        self.logger.info(f"{Colors.YELLOW}# Initiating graceful shutdown of all SymbolBots...{Colors.RESET}")
        ...
        while not self._stop_event.is_set():
            try:
                if (time.time() - last_config_check_time) > self.global_config.system.config_refresh_interval_sec:
                    self.logger.info(f"{Colors.CYAN}Periodically checking for configuration changes...{Colors.RESET}")
                    ...
                    reloaded_global_cfg, reloaded_symbol_configs_dict = ConfigManager.load_config(
                    ...
                    if reloaded_global_cfg != self.global_config:
                        self.logger.info(f"{Colors.YELLOW}Global configuration changed. Updating references. (Full restart might be needed for some changes).{Colors.RESET}")
                        object.__setattr__(self, 'global_config', reloaded_global_cfg) # Update global_config
                        self.logger = setup_logger(self.global_config.files, "main") # Re-setup logger

                        # Update configs for API/WS/DB clients
                        if self.api_client: self.api_client.config = self.global_config
                        if self.ws_client: self.ws_client.config = self.global_config
                        if self.db_manager: self.db_manager.db_file = STATE_DIR / self.global_config.files.db_file
                    ...
                    await self._start_symbol_bots(reloaded_symbol_configs_dict) # Pass dict directly
                    last_config_check_time = time.time()
    ...
    async def run(self) -> None:
        ...
        if sys.stdin.isatty():
            try:
                ...
                if selected_mode == 's':
                    ...
                    self.logger.warning(f"{Colors.YELLOW}Invalid mode selected. Defaulting to file mode.{Colors.RESET}")
                    selected_mode = 'f'
            except EOFError:
        ...
        try:
            ...
            self.logger.info(f"[{self.global_config.category}] Bot main task cancelled.")
            ...
            self.logger.info(f"{Colors.YELLOW}# Finalizing shutdown sequence...{Colors.RESET}")
            ...
            self.logger.info(f"{Colors.NEON_GREEN}Pyrmethus Market Maker Bot has completed its grand ritual. Farewell, seeker.{Colors.RESET}")
...
async def main():
    ...
    try:
        ...
        if not LOG_DIR.exists():
            ...
            except OSError as e: logging.critical(f"Failed to create log directory {LOG_DIR}: {e}"); sys.exit(1)

        # Ensure state directory exists
        if not STATE_DIR.exists():
            try: STATE_DIR.mkdir(parents=True, exist_ok=True)
            except OSError as e: logging.critical(f"Failed to create state directory {STATE_DIR}: {e}"); sys.exit(1)
        ...
        if not temp_logger.handlers:
            logging.basicConfig(level=logging.INFO, format=f"{Colors.YELLOW}%(asctime)s - %(levelname)s - %(message)s{Colors.RESET}", stream=sys.stdout)
            temp_logger = logging.getLogger('BybitMarketMaker.init')
        ...
        try:
            ...
            config_file_path = Path(__file__).parent / temp_global_config.files.symbol_config_file
                ...
                default_config_content = [
                    {
                        ...
                        "strategy": StrategyConfig().model_dump(mode='json_compatible') # Use default strategy config
                    }
                ]
                with open(config_file_path, "w") as f:
                    json.dump(default_config_content, f, indent=4, cls=JsonDecimalEncoder)
                temp_logger.info(f"{Colors.NEON_GREEN}Created default symbol config file: {config_file_path}{Colors.RESET}")
                temp_logger.info(f"{Colors.YELLOW}Please review and adjust {config_file_path} with your desired symbols and settings.{Colors.RESET}")
            ...
            temp_logger.critical(f"{Colors.NEON_RED}Error creating default config file: {e}{Colors.RESET}", exc_info=True)
            sys.exit(1)
