...
STATUS_UPDATE_INTERVAL = 60 # Interval in seconds for displaying status updates
...
if EXTERNAL_LIBS_AVAILABLE:
    ...
    class ConfigManager:
        ...
        def load_config(cls, prompt_for_symbol: bool = False, input_symbol: Optional[str] = None) -> Tuple[GlobalConfig, List[SymbolConfig]]:
            ...
            """
            # This logic block handles the initial global config load AND then either loads from file
            # or creates a dynamic single symbol config.
            
            # --- Load/Initialize GlobalConfig ---
            global_data = {
            ...
            if prompt_for_symbol and input_symbol:
                ...
                single_symbol_data = {
                    ...
                    "enable_auto_sl_tp": False, # Default to false for interactive
...
# GLOBAL_CONFIG and SYMBOL_CONFIGS will be loaded after the prompt
...
def setup_logger(name_suffix: str) -> logging.Logger:
    ...
    log_level_str = "INFO"
    if GLOBAL_CONFIG:
        log_level_str = GLOBAL_CONFIG.log_level.upper()
...
if EXTERNAL_LIBS_AVAILABLE:
    ...
    # This might create a default logger if GLOBAL_CONFIG is not yet available,
    # and it will be replaced by the one from PyrmethusBot.
...
class SymbolBot(threading.Thread):
    ...
    def __init__(self, config: SymbolConfig, exchange: Any, ws_client: BybitWebSocket, logger: logging.Logger):
        ...
        self.last_status_display_time: float = 0.0 # For display updates
            ...
            try:
                with open(self.state_file, "r") as f:
                    ...
                    for date_str, daily_metric_dict in state_data.get("daily_metrics", {}).items():
                        try: self.daily_metrics[date_str] = daily_metric_dict
    ...
    def _reset_daily_metrics_if_new_day(self):
        ...
        if self.today_date != current_utc_date:
            ...
            if self.today_date in self.daily_metrics:
                self.daily_metrics[self.today_date]["unrealized_pnl_snapshot"] = str(self.unrealized_pnl)
            ...
            for pos in positions:
                if pos["symbol"] == self.symbol and abs( Decimal(str(pos.get("info", {}).get("size", "0"))) ) > DECIMAL_ZERO:
                    ...
                    if unrealized_pnl_percent >= Decimal(str(self.config.take_profit_target_pct)):
                        ...
                        if close_side == "sell":
                            valid_bids = bids_df[bids_df["price"] >= mid_price * Decimal("0.99")]
    ...
    def update_take_profit_stop_loss(self):
        ...
        try:
            ...
            self.logger.info(
                f"[{self.symbol}] Set TP: {take_profit_price:.{price_precision if price_precision is not None else 8}f}, "
                f"SL: {stop_loss_price:.{price_precision if price_precision is not None else 8}f} for {self.inventory:+.4f} position."
            )
        except Exception as e:
    ...
    def _check_and_handle_stale_orders(self):
        ...
        with self.ws_client.lock:
            for client_order_id, order_info in list(self.open_orders.items()):
                if order_info.get("status") not in ["Filled", "Canceled", "Deactivated", "Rejected"] and \
    ...
    @retry_api_call()
    def _fetch_account_balance(self, coin: str = "USDT") -> Decimal:
        """Fetches the current available balance for a specified coin."""
        try:
            balances = self.exchange.fetch_balance(params={'accountType': 'UNIFIED'})
            ...
            self.logger.error(f"[{self.symbol}] Error fetching account balance for {coin}: {e}", exc_info=True)
            return DECIMAL_ZERO
    ...
    def _check_daily_pnl_limits(self):
        ...
        if self.config.daily_pnl_stop_loss_pct is not None and self.config.daily_pnl_stop_loss_pct > 0:
            current_balance = self._fetch_account_balance()
            if current_balance <= DECIMAL_ZERO: current_balance = Decimal("10000")
            
            loss_threshold_usd = -Decimal(str(self.config.daily_pnl_stop_loss_pct)) * current_balance

            if net_realized_pnl <= loss_threshold_usd:
                self.logger.critical(
                    f"{Colors.NEON_RED}# [{self.symbol}] Daily PnL Stop Loss triggered! "
                    f"Net Realized PnL: {net_realized_pnl:+.4f} USD (Threshold: {loss_threshold_usd:+.4f} USD). "
                    f"Stopping trading for this symbol.{Colors.RESET}"
                )
        ...
        if self.config.daily_pnl_take_profit_pct is not None and self.config.daily_pnl_take_profit_pct > 0:
            current_balance = self._fetch_account_balance()
            if current_balance <= DECIMAL_ZERO: current_balance = Decimal("10000")
            
            profit_threshold_usd = Decimal(str(self.config.daily_pnl_take_profit_pct)) * current_balance
            
            if net_realized_pnl >= profit_threshold_usd:
                self.logger.info(
                    f"{Colors.NEON_GREEN}# [{self.symbol}] Daily PnL Take Profit triggered! "
                    f"Net Realized PnL: {net_realized_pnl:+.4f} USD (Threshold: {profit_threshold_usd:+.4f} USD). "
                    f"Stopping trading for this symbol.{Colors.RESET}"
                )
    ...
    def _check_market_data_freshness(self) -> bool:
        ...
        last_ob_update = self.ws_client.last_orderbook_update_time.get(symbol_id_ws, 0.0)
        last_trades_update = self.ws_client.last_trades_update_time.get(symbol_id_ws, 0.0)
        ...
        if (current_time - last_ob_update > self.config.market_data_stale_timeout_seconds) or \
           (current_time - last_trades_update > self.config.market_data_stale_timeout_seconds):
            self.logger.warning(
                ...
                f"Last Trades: {current_time - last_trades_update:.1f}s ago (Timeout: {self.config.market_data_stale_timeout_seconds}s). Pausing trading."
            )
            termux_notify(f"{self.symbol}: Market data stale! Pausing.", is_error=True)
            return False
        return True

    def _display_status(self, current_mid_price: Decimal, best_bid: Decimal, best_ask: Decimal):
        """Displays current status including PnL, price, and bid/ask in the console."""
        
        unrealized_pnl_color = Colors.NEON_GREEN if self.unrealized_pnl > DECIMAL_ZERO else Colors.NEON_RED if self.unrealized_pnl < DECIMAL_ZERO else Colors.YELLOW
        net_pnl_color = Colors.NEON_GREEN if self.performance_metrics["net_pnl"] > DECIMAL_ZERO else Colors.NEON_RED if self.performance_metrics["net_pnl"] < DECIMAL_ZERO else Colors.YELLOW
        
        current_daily_metrics = self.daily_metrics.get(self.today_date, {})
        daily_realized_pnl = Decimal(current_daily_metrics.get("realized_pnl", "0"))
        daily_fees = Decimal(current_daily_metrics.get("total_fees", "0"))
        daily_net_pnl = daily_realized_pnl - daily_fees
        daily_net_pnl_color = Colors.NEON_GREEN if daily_net_pnl > DECIMAL_ZERO else Colors.NEON_RED if daily_net_pnl < DECIMAL_ZERO else Colors.YELLOW

        price_prec = self.config.price_precision if self.config.price_precision is not None else 8
        
        status_message = (
            f"\n{Colors.CYAN}--- {self.symbol} Status ---{Colors.RESET}\n"
            f"  {Colors.NEON_BLUE}Mid Price:{Colors.RESET} {current_mid_price:.{price_prec}f}\n"
            f"  {Colors.NEON_GREEN}Best Bid:{Colors.RESET} {best_bid:.{price_prec}f} | {Colors.NEON_RED}Best Ask:{Colors.RESET} {best_ask:.{price_prec}f}\n"
            f"  {Colors.MAGENTA}Position:{Colors.RESET} {self.inventory:+.4f}\n"
            f"  {Colors.YELLOW}UPL:{Colors.RESET} {unrealized_pnl_color}{self.unrealized_pnl:+.4f}{Colors.RESET}\n"
            f"  {Colors.YELLOW}Daily Net PnL:{Colors.RESET} {daily_net_pnl_color}{daily_net_pnl:+.4f}{Colors.RESET} (Trades: {current_daily_metrics.get('trades_count', 0)})\n"
            f"  {Colors.YELLOW}Total Net PnL:{Colors.RESET} {net_pnl_color}{self.performance_metrics['net_pnl']:+.4f}{Colors.RESET} (Total Trades: {self.performance_metrics['trades']})\n"
    ...
    def run(self):
        ...
        if not self._fetch_symbol_info():
            ...
            return
        if GLOBAL_CONFIG and GLOBAL_CONFIG.category == "linear":
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if not orderbook or not orderbook.get("b") or not orderbook.get("a") or not orderbook["b"] or not orderbook["a"]:
                    self.logger.warning(f"[{self.symbol}] Order book data not fully available from WebSocket (bids/asks might be empty). Retrying in {MAIN_LOOP_SLEEP_INTERVAL}s.")
                    self.cancel_all_orders(self.symbol)
                    self._stop_event.wait(MAIN_LOOP_SLEEP_INTERVAL)
                    continue
                ...
                mid_price = (best_bid_price + best_ask_price) / DECIMAL_ZERO
                ...
                total_recent_volume_notional = sum(trade[0] * trade[1] for trade in recent_trades)
                ...
                if time.time() - self.last_status_display_time > STATUS_UPDATE_INTERVAL:
                    self._display_status(mid_price, best_bid_price, best_ask_price)
                    self.last_status_display_time = time.time()

                self._stop_event.wait(self.config.order_refresh_time)
                ...
                self.cancel_all_orders(self.symbol)
                self._stop_event.wait(MAIN_LOOP_SLEEP_INTERVAL * 2)
...
class PyrmethusBot:
    ...
    def __init__(self):
        if not EXTERNAL_LIBS_AVAILABLE:
            main_logger.critical(f"{Colors.NEON_RED}PyrmethusBot cannot start: External libraries not available.{Colors.RESET}")
            sys.exit(1)
            
        # These will be populated after the interactive prompt
        self.global_config: Optional[GlobalConfig] = None
        self.symbol_configs: List[SymbolConfig] = []
        
        self.exchange: Any = initialize_exchange(main_logger)
        if not self.exchange:
            main_logger.critical(f"{Colors.NEON_RED}# Failed to initialize exchange. Exiting.{Colors.RESET}")
            sys.exit(1)
        
        self.ws_client: Optional[BybitWebSocket] = None # Initialized after configs are loaded
        self.active_symbol_bots: Dict[str, SymbolBot] = {}
        self._main_stop_event: threading.Event = threading.Event()
    ...
    def run(self):
        ...
        print(f"\n{Colors.MAGENTA}Choose how to load trading symbols:{Colors.RESET}")
        print(f"  {Colors.CYAN}f{Colors.RESET} - Load from '{GLOBAL_CONFIG.symbol_config_file}' (multiple symbols)")
        print(f"  {Colors.CYAN}s{Colors.RESET} - Enter a single symbol manually (e.g., BTC/USDT:USDT)")
        
        load_choice = input(f"{Colors.YELLOW}Enter choice (f/s): {Colors.RESET}").lower().strip()

        if load_choice == 's':
            input_symbol = input(f"{Colors.YELLOW}Enter symbol (e.g., BTC/USDT:USDT): {Colors.RESET}").strip().upper()
            if not input_symbol:
                main_logger.critical(f"{Colors.NEON_RED}No symbol entered. Exiting.{Colors.RESET}")
                sys.exit(1)
            # Reload configs with the single symbol
            global GLOBAL_CONFIG, SYMBOL_CONFIGS
            GLOBAL_CONFIG, SYMBOL_CONFIGS = ConfigManager.load_config(prompt_for_symbol=True, input_symbol=input_symbol)
        elif load_choice == 'f':
            # Reload configs from file
            global GLOBAL_CONFIG, SYMBOL_CONFIGS
            GLOBAL_CONFIG, SYMBOL_CONFIGS = ConfigManager.load_config(prompt_for_symbol=False)
        else:
            main_logger.critical(f"{Colors.NEON_RED}Invalid choice. Exiting.{Colors.RESET}")
            sys.exit(1)

        # Re-initialize main_logger with potentially updated global_config.log_level
        global main_logger
        main_logger = setup_logger("main")

        # Initialize WebSocket client after global config and symbol configs are loaded
        ...
        # Start WebSocket streams with all configured symbols
        public_topics = [f"orderbook.50.{s.symbol.replace('/', '').replace(':', '')}" for s in SYMBOL_CONFIGS] + \
                        [f"publicTrade.{s.symbol.replace('/', '').replace(':', '')}" for s in SYMBOL_CONFIGS]
        private_topics = ["order", "execution", "position"]
        self.ws_client.start_streams(public_topics, private_topics)
        
        active_bots_count = 0
        for s_config in SYMBOL_CONFIGS:
    ...
    def shutdown(self):
        ...
        if self.ws_client: # Ensure ws_client was initialized
...
if __name__ == "__main__":
    ...
    # Note: GLOBAL_CONFIG should be available here if EXTERNAL_LIBS_AVAILABLE.
    # Its existence is checked to ensure .symbol_config_file path is valid.
    config_file_path = Path(GLOBAL_CONFIG.symbol_config_file) if GLOBAL_CONFIG else Path("symbols.json")
...
## 2. `profit_optimizer.py` (The Offline Profit Optimizer)

This script analyzes the state files generated by `pyrmethus_bot.py` to provide performance insights and heuristic-based parameter recommendations. It's designed to be run **separately** from the main bot.

### Features:

*   **Load Bot State**: Reads all `_state.json` files from the `STATE_DIR` to gather historical performance and trade data.
*   **Performance Metrics**: Calculates and displays:
    *   Overall Net PnL, Realized PnL, and Total Fees.
    *   Win Rate, Average Profit/Trade, Average Fee/Trade, and Average Spread Capture.
    *   Daily Net PnL and trade counts for a configurable number of past days.
*   **Parameter Recommendations**: Provides actionable suggestions for adjusting `SymbolConfig` parameters (e.g., `base_spread`, `take_profit_target_pct`, `dynamic_spread.volatility_multiplier`) based on observed performance.
*   **Color-Coded Output**: All reports are formatted with colors for enhanced readability.

### Setup Instructions for `profit_optimizer.py`:

1.  **Ensure your main bot (`pyrmethus_bot.py`) has run** for some time to generate state files in the `.bot_state` directory.
2.  **Make sure `symbols.json` (or the parameters used for single-symbol mode) reflects your bot's configuration**, as the optimizer uses this to understand the current parameter settings for recommendations.
3.  **Run this optimizer script** from your terminal:
    ```bash
    python profit_optimizer.py
    ```

### `profit_optimizer.py` Code:
...
# These are internal to the optimizer script.
class OptimizerConfig:
    ANALYZE_LAST_DAYS: int = 7 # Analyze trades from the last N days
    PNL_REPORT_CURRENCY: str = "USD" # Currency for PnL reports

# --- Assume these are available from the main bot script for consistency ---
# (In a real project, you'd ensure proper package structure to import directly.
# For this exercise, we'll redefine/copy minimal necessary parts,
# which are expected to be identical to pyrmethus_bot.py's definitions.)
getcontext().prec = 38
...
def json_loads_decimal(s: str) -> Any:
    try:
        ...
        print(f"{Colors.NEON_RED}Error decoding JSON with Decimal: {e} for input: {s[:100]}...{Colors.RESET}")
        raise ValueError(f"Invalid JSON or Decimal format: {e}") from e

# Simplified Trade model - enough for PnL calculation
class Trade:
    def __init__(self, side: str, qty: Decimal, price: Decimal, profit: Decimal, timestamp: int, fee: Decimal, trade_id: str, entry_price: Optional[Decimal]):
        self.side = side
        self.qty = qty
        self.price = price
        self.profit = profit
    ...
    def from_dict(cls, data: Dict[str, Any]):
        # Convert str to Decimal for relevant fields
        return cls(
            side=data['side'],
            ...
            entry_price=Decimal(str(data['entry_price'])) if data.get('entry_price') is not None else None
        )

# --- Define Pydantic Models for Config (needed for recommendations) ---
# These are copied from pyrmethus_bot.py to enable the optimizer to understand config.
# In a true package, these would be imported from a shared configuration module.
from pydantic import BaseModel, ConfigDict, Field, NonNegativeFloat, NonNegativeInt, PositiveFloat, ValidationError
...
class ConfigManager:
    ...
    @classmethod
    def load_config(cls, prompt_for_symbol: bool = False, input_symbol: Optional[str] = None) -> Tuple[GlobalConfig, List[SymbolConfig]]:
        # This implementation is simplified for the optimizer, to just load existing configs.
        # It does not have the interactive prompt logic here; it assumes the configs are already present.
        global_data = {
            "category": os.getenv("BYBIT_CATEGORY", "linear"),
            "api_max_retries": int(os.getenv("API_MAX_RETRIES", "5")),
            "api_retry_delay": int(os.getenv("API_RETRY_DELAY", "1")),
        ...
        try:
            ...
            print(f"{Colors.NEON_RED}Global configuration validation error: {e}{Colors.RESET}")
            sys.exit(1)
        ...
        try:
            ...
            print(f"{Colors.NEON_RED}Symbol configuration file '{cls._global_config.symbol_config_file}' not found. Cannot load symbol configs for optimizer.{Colors.RESET}")
            # If the file is not found, we can't provide specific recommendations, but can still analyze loaded states.
            # So, don't exit, just return empty symbol configs.
            return cls._global_config, []
        except json.JSONDecodeError as e:
            print(f"{Colors.NEON_RED}Error decoding JSON from symbol configuration file '{cls._global_config.symbol_config_file}': {e}{Colors.RESET}")
            sys.exit(1)
        except ValueError as e:
            print(f"{Colors.NEON_RED}Invalid format in symbol configuration file: {e}{Colors.RESET}")
            sys.exit(1)
        except Exception as e:
            print(f"{Colors.NEON_RED}Unexpected error loading symbol config: {e}{Colors.RESET}")
            sys.exit(1)
        ...
        for s_cfg in raw_symbol_configs:
            try:
                ...
                print(f"{Colors.NEON_RED}Symbol configuration validation error for {s_cfg.get('symbol', 'N/A')}: {e}{Colors.RESET}")
                sys.exit(1)
            except Exception as e:
                print(f"{Colors.NEON_RED}Unexpected error processing symbol config {s_cfg.get('symbol', 'N/A')}: {e}{Colors.RESET}")
                sys.exit(1)

        return cls._global_config, cls._symbol_configs

# Recreate STATE_DIR for optimizer
BASE_DIR = Path(os.getenv("HOME", "."))
STATE_DIR: Path = BASE_DIR / ".bot_state"

# --- Performance Metrics Calculation ---
class PerformanceAnalyzer:
    def __init__(self, state_data: Dict[str, Any], symbol: str):
        self.symbol = symbol
        self.performance_metrics: Dict[str, Decimal] = {
            "trades": Decimal(str(state_data.get("performance_metrics", {}).get("trades", "0"))),
            "profit": Decimal(str(state_data.get("performance_metrics", {}).get("profit", "0"))),
            "fees": Decimal(str(state_data.get("performance_metrics", {}).get("fees", "0"))),
            "net_pnl": Decimal(str(state_data.get("performance_metrics", {}).get("net_pnl", "0")))
        }
        self.trade_history: List[Trade] = [Trade.from_dict(t) for t in state_data.get("trade_history", [])]
        self.daily_metrics: Dict[str, Dict[str, Decimal]] = {
            date: {k: Decimal(str(v)) for k, v in metrics.items() if k not in ["date", "trades_count"]}
            for date, metrics in state_data.get("daily_metrics", {}).items()
        }
        self.daily_trades_count: Dict[str, int] = {
            date: metrics.get("trades_count", 0)
            for date, metrics in state_data.get("daily_metrics", {}).items()
        }
    ...
    def get_daily_metrics(self, last_days: int) -> Dict[str, Dict[str, Decimal]]:
        """Returns daily PnL and trade counts for the last N days."""
        metrics_by_day = {}
        ...
        for i in range(last_days):
            ...
            realized_pnl = self.daily_metrics.get(date_str, {}).get("realized_pnl", DECIMAL_ZERO)
            total_fees = self.daily_metrics.get(date_str, {}).get("total_fees", DECIMAL_ZERO)
            net_daily_pnl = realized_pnl - total_fees
            trades_count = self.daily_trades_count.get(date_str, 0)
            
            metrics_by_day[date_str] = {
                "Net PnL": net_daily_pnl,
                "Realized PnL": realized_pnl,
                "Fees": total_fees,
                "Trades": Decimal(str(trades_count))
            }
        return metrics_by_day

    def calculate_trade_characteristics(self) -> Dict[str, Decimal]:
        """Calculates win rate, average profit per trade, and average fee per trade."""
        if not self.trade_history:
            return {
                "Win Rate": DECIMAL_ZERO,
                "Avg Profit/Trade": DECIMAL_ZERO,
                "Avg Fee/Trade": DECIMAL_ZERO,
                "Avg Spread Capture (%)": DECIMAL_ZERO
            }

        profitable_trades = [t for t in self.trade_history if t.profit > DECIMAL_ZERO]
        total_profitable_pnl = sum(t.profit for t in profitable_trades)

        win_rate = Decimal(len(profitable_trades)) / Decimal(len(self.trade_history)) if self.trade_history else DECIMAL_ZERO
        avg_profit_per_trade = total_profitable_pnl / Decimal(len(self.trade_history)) if self.trade_history else DECIMAL_ZERO
        avg_fee_per_trade = sum(t.fee for t in self.trade_history) / Decimal(len(self.trade_history)) if self.trade_history else DECIMAL_ZERO
        
        total_notional_volume = sum(t.qty * t.price for t in self.trade_history)
        avg_spread_capture_pct = (self.performance_metrics["net_pnl"] / total_notional_volume * Decimal("100")) if total_notional_volume > DECIMAL_ZERO else DECIMAL_ZERO

        return {
            "Win Rate": win_rate,
            "Avg Profit/Trade": avg_profit_per_trade,
            "Avg Fee/Trade": avg_fee_per_trade,
            "Avg Spread Capture (%)": avg_spread_capture_pct
        }

# --- Parameter Recommendation Logic ---
class ParameterRecommender:
    def __init__(self, analyzer: PerformanceAnalyzer, symbol_config: SymbolConfig):
        self.analyzer = analyzer
        self.symbol_config = symbol_config

    def recommend_adjustments(self) -> List[str]:
        """Provides heuristic-based recommendations for parameter adjustments."""
        recommendations = []
        metrics = self.analyzer.calculate_trade_characteristics()
        summary_metrics = self.analyzer.get_summary_metrics()

        if summary_metrics["net_pnl"] <= DECIMAL_ZERO:
            recommendations.append(f"{Colors.YELLOW}Warning: Overall Net PnL for {self.symbol_config.symbol} is non-positive. Consider a comprehensive strategy review.{Colors.RESET}")
            if metrics["Win Rate"] < Decimal("0.5"):
                recommendations.append("- Your win rate is low. Consider widening your `base_spread` or adjusting `take_profit_target_pct`/`stop_loss_trigger_pct` for better entry/exit conditions.")
            if metrics["Avg Spread Capture (%)"] < Decimal("0.05"): # 0.05%
                recommendations.append("- Average spread capture is very low. Increase `base_spread` to improve per-trade profitability.")
            if self.symbol_config.inventory_skew.enabled and Decimal(str(self.symbol_config.inventory_skew.skew_factor)) > Decimal("0.0001"):
                 recommendations.append("- Review `inventory_skew.skew_factor`; a high skew might be overly aggressive in reducing inventory, potentially hurting PnL.")

        daily_trades = self.analyzer.daily_trades_count
        avg_daily_trades = sum(daily_trades.values()) / Decimal(len(daily_trades)) if daily_trades else DECIMAL_ZERO
        
        if avg_daily_trades > 50:
            recommendations.append("- High trading frequency detected. Consider slightly increasing `base_spread` or `order_layers` spread_offset to reduce over-trading and improve profit margins.")
        elif avg_daily_trades < 10 and summary_metrics["net_pnl"] > DECIMAL_ZERO:
            recommendations.append("- Low trading frequency, but profitable. You might be able to increase `order_amount` or slightly decrease `base_spread` to capture more volume.")

        if self.symbol_config.dynamic_spread.enabled:
            if Decimal(str(self.symbol_config.dynamic_spread.volatility_multiplier)) < Decimal("0.2"):
                recommendations.append("- `dynamic_spread.volatility_multiplier` is low. You might be taking on too much risk in volatile markets. Consider increasing it to widen spreads during high volatility.")
            elif Decimal(str(self.symbol_config.dynamic_spread.volatility_multiplier)) > Decimal("0.8"):
                recommendations.append("- `dynamic_spread.volatility_multiplier` is high. Your spreads might be too wide in volatile markets, reducing fill rates. Consider decreasing it if liquidity is good.")
        
        if self.symbol_config.enable_auto_sl_tp:
            if Decimal(str(self.symbol_config.take_profit_target_pct)) < Decimal(str(self.symbol_config.stop_loss_trigger_pct)):
                recommendations.append(f"{Colors.NEON_RED}Urgent for {self.symbol_config.symbol}: Your Take Profit ({self.symbol_config.take_profit_target_pct*100:.2f}%) is smaller than Stop Loss ({self.symbol_config.stop_loss_trigger_pct*100:.2f}%). This setup is highly unprofitable in the long run. Adjust `take_profit_target_pct` and `stop_loss_trigger_pct` immediately.{Colors.RESET}")
            elif Decimal(str(self.symbol_config.take_profit_target_pct)) < Decimal("0.002"):
                recommendations.append("- `take_profit_target_pct` is very low. Ensure it covers fees and provides adequate profit margin.")

        if not recommendations:
            recommendations.append(f"{Colors.NEON_GREEN}Current performance looks good for {self.symbol_config.symbol}. Keep monitoring!{Colors.RESET}")
        else:
            recommendations.insert(0, f"{Colors.CYAN}--- Recommendations for {self.symbol_config.symbol} ---{Colors.RESET}")
            recommendations.append(f"{Colors.CYAN}--- End Recommendations ---{Colors.RESET}")

        return recommendations


# --- Main Optimizer Script Execution ---
def run_optimizer():
    print(f"\n{Colors.MAGENTA}╔══════════════════════════════════════════╗")
    print(f"{Colors.MAGENTA}║      Pyrmethus Profit Optimizer v1.0       ║")
    print(f"{Colors.MAGENTA}║      Analyzing Market Maker Performance    ║")
    print(f"{Colors.MAGENTA}╚════════════════════════════════════════╝{Colors.RESET}\n")

    print(f"{Colors.CYAN}Loading bot state files from: {STATE_DIR}{Colors.RESET}")

    state_files = list(STATE_DIR.glob("*_state.json"))
    if not state_files:
        print(f"{Colors.YELLOW}No state files found in {STATE_DIR}. Ensure your bot has run and saved state.{Colors.RESET}")
        return

    all_analyzers: Dict[str, PerformanceAnalyzer] = {}
    for state_file_path in state_files:
        try:
            with open(state_file_path, 'r') as f:
                state_data = json_loads_decimal(f.read())
            
            symbol_raw = state_file_path.name.replace("_state.json", "").replace("_", "/")
            if symbol_raw.count('/') == 2:
                parts = symbol_raw.split('/')
                symbol_ccxt = f"{parts[0]}/{parts[1]}:{parts[2]}"
            else:
                symbol_ccxt = symbol_raw
            
            analyzer = PerformanceAnalyzer(state_data, symbol_ccxt)
            all_analyzers[symbol_ccxt] = analyzer
            print(f"{Colors.NEON_GREEN}Loaded state for {symbol_ccxt}{Colors.RESET}")

        except Exception as e:
            print(f"{Colors.NEON_RED}Error loading state file {state_file_path}: {e}{Colors.RESET}")

    if not all_analyzers:
        print(f"{Colors.YELLOW}No valid bot states loaded. Exiting.{Colors.RESET}")
        return

    print(f"\n{Colors.CYAN}--- Overall Performance Summary ---{Colors.RESET}")
    for symbol, analyzer in all_analyzers.items():
        summary = analyzer.get_summary_metrics()
        trade_chars = analyzer.calculate_trade_characteristics()

        print(f"\n{Colors.MAGENTA}Symbol: {symbol}{Colors.RESET}")
        print(f"  Total Trades: {summary['trades']}")
        print(f"  Total Realized PnL: {summary['profit']:.4f} {OptimizerConfig.PNL_REPORT_CURRENCY}")
        print(f"  Total Fees: {summary['fees']:.4f} {OptimizerConfig.PNL_REPORT_CURRENCY}")
        print(f"  Net PnL (Total): {Colors.NEON_GREEN if summary['net_pnl'] > DECIMAL_ZERO else Colors.NEON_RED}{summary['net_pnl']:.4f} {OptimizerConfig.PNL_REPORT_CURRENCY}{Colors.RESET}")
        print(f"  Win Rate: {trade_chars['Win Rate'] * 100:.2f}%")
        print(f"  Avg Profit/Trade: {trade_chars['Avg Profit/Trade']:.4f} {OptimizerConfig.PNL_REPORT_CURRENCY}")
        print(f"  Avg Fee/Trade: {trade_chars['Avg Fee/Trade']:.4f} {OptimizerConfig.PNL_REPORT_CURRENCY}")
        print(f"  Avg Spread Capture: {trade_chars['Avg Spread Capture (%)']:.4f}%")

    print(f"\n{Colors.CYAN}--- Daily Performance (Last {OptimizerConfig.ANALYZE_LAST_DAYS} Days) ---{Colors.RESET}")
    for symbol, analyzer in all_analyzers.items():
        daily_metrics = analyzer.get_daily_metrics(OptimizerConfig.ANALYZE_LAST_DAYS)
        print(f"\n{Colors.MAGENTA}Symbol: {symbol}{Colors.RESET}")
        for date_str, metrics in daily_metrics.items():
            net_pnl_color = Colors.NEON_GREEN if metrics['Net PnL'] > DECIMAL_ZERO else Colors.NEON_RED
            print(f"  {date_str}: Net PnL: {net_pnl_color}{metrics['Net PnL']:.4f}{Colors.RESET}, Trades: {metrics['Trades']}")

    print(f"\n{Colors.CYAN}--- Parameter Recommendations ---{Colors.RESET}")
    
    # Load configs specifically for the recommender, to get the actual SymbolConfig for heuristics
    # This will load global configs as well
    global_config_for_rec, symbol_configs_for_rec = [], []
    try:
        global_config_for_rec, symbol_configs_for_rec = ConfigManager.load_config(prompt_for_symbol=False)
    except SystemExit: # ConfigManager.load_config might sys.exit on critical error
        print(f"{Colors.NEON_RED}Configuration loading for recommendations failed. Skipping recommendations.{Colors.RESET}")
        return
    except Exception as e:
        print(f"{Colors.NEON_RED}Could not load configurations for recommendations: {e}. Skipping.{Colors.RESET}")
        return

    symbol_config_map = {cfg.symbol: cfg for cfg in symbol_configs_for_rec}

    for symbol, analyzer in all_analyzers.items():
        current_symbol_config = symbol_config_map.get(symbol)
        if current_symbol_config:
            recommender = ParameterRecommender(analyzer, current_symbol_config)
            recommendations = recommender.recommend_adjustments()
            for rec in recommendations:
                print(rec)
        else:
            print(f"{Colors.YELLOW}Warning: No current configuration found in '{global_config_for_rec.symbol_config_file}' for {symbol}. Cannot provide parameter recommendations.{Colors.RESET}")

    print(f"\n{Colors.MAGENTA}Optimizer analysis complete.{Colors.RESET}\n")
