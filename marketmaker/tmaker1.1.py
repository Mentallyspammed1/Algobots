...
class StrategyConfig:
    ...
    order_stale_threshold_pct: Decimal = Decimal(
        "0.0005"
    )  # If order price differs by this much from target, cancel.
...
class Config:
    ...
    trading_mode: str = (
        "DRY_RUN"  # "LIVE", "DRY_RUN", "TESTNET" (TESTNET implies testnet=True)
    )
    ...
    quote_currency: str = (
        "USDT"  # Currency used for balance tracking and order value calculation
    )
    ...
    post_only: bool = (
        True  # Ensures orders are placed as 'PostOnly' to avoid taker fees
    )
    min_order_value_usd: Decimal = Decimal(
        "10"
    )  # Minimum notional value for an order (Bybit specific)
    max_order_size_pct: Decimal = Decimal(
        "0.1"
    )  # Max percentage of total balance for a single order

    # System & Logging
    loop_interval_sec: int = 1  # Main loop check interval in seconds
    order_refresh_interval_sec: int = (
        5  # How often to check/replace orders (independent of loop_interval)
    )
    log_level: str = "INFO"
    ...
    async def log_trade_fill(self, trade_data: dict):
        ...
        await self.conn.execute(
            ...
            (
                ...
                str(trade_data.get("leavesQty", "0")),
            ),  # pnl is not directly available here, leavesQty could be used as a proxy for remaining, or calculate based on position.
        )
        await self.conn.commit()
...
class BybitMarketMaker:
    ...
    def __init__(self, config: Config):
        ...
        self.active_orders: dict[
            str, dict
        ] = {}  # {order_id: {side, price, qty, status, orderLinkId}}
    ...
    async def _fetch_market_info(self) -> MarketInfo | None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to fetch market info: {response.get('retMsg', 'Unknown error')}"
            )
            return None
        ...
        if self.config.trading_mode == "DRY_RUN":
            ...
            if self.current_balance == Decimal("0"):
                self.current_balance = Decimal(
                    "10000"
                )  # Start with a simulated balance
                self.logger.info(
                    f"DRY_RUN: Initial simulated balance: {self.current_balance} {self.config.quote_currency}"
                )
            return self.current_balance

        try:
            response = self.http_session.get_wallet_balance(
                accountType=self.config.category.upper()
            ...
            if response["retCode"] == 0:
                ...
                for coin in coins:
                    if coin["coin"] == self.config.quote_currency:
                        ...
                        self.logger.info(
                            f"Current Balance: {self.current_balance} {self.config.quote_currency}"
                        )
                        asyncio.create_task(
                            self.db_manager.log_balance_update(
                                self.config.quote_currency,
                                self.current_balance,
                                Decimal(coin.get("availableToWithdraw", "0")),
                            )
                        )
                        return self.current_balance
            self.logger.error(
    ...
    def _handle_orderbook_update(self, message: dict):
        ...
        try:
            if (
                "data" in message
                and message["topic"] == f"orderbook.1.{self.config.symbol}"
            ):
                data = message["data"]
                bids = data["b"]
                asks = data["a"]
        ...
        try:
            if "data" in message:
                for order_data in message["data"]:
                    ...
                    self.logger.info(
                        f"Order {order_id} status update: {status} (Order Link ID: {order_data.get('orderLinkId')})"
                    )
                    ...
                    async with self.active_orders_lock:
                        if order_id in self.active_orders:
                            ...
                            if status == "Filled":
                                self.logger.info(
                                    f"Order {order_id} FILLED: Price={order_data.get('execPrice')}, Qty={order_data.get('execQty')}, Fee={order_data.get('execFee')}"
                                )
                                asyncio.create_task(
                                    self.db_manager.log_trade_fill(order_data)
                                )

                                # Update metrics
                                self.config.metrics.total_trades += 1
                                self.config.metrics.total_fees += Decimal(
                                    order_data.get("execFee", "0")
                                )
                                ...
                                self.logger.info(
                                    f"Order {order_id} removed from active orders after fill."
                                )
                                # Re-update balance after a fill
                                asyncio.create_task(self._update_balance())
                            elif status in ["Cancelled", "Rejected"]:
                                self.logger.info(
                                    f"Order {order_id} removed from active orders due to {status}."
                                )
                                del self.active_orders[order_id]
                        else:
                            ...
                            if status in ["New", "PartiallyFilled", "Untriggered"]:
                                ...
                                self.logger.warning(
                                    f"Found untracked active order {order_id} (Status: {status}). Added to local state."
                                )
    ...
    async def _cancel_order(self, order_id: str, order_link_id: str | None = None):
        ...
        self.logger.info(
            f"Attempting to cancel order {order_id} (Link ID: {order_link_id})..."
        )
        try:
            params = {
            ...
            if response["retCode"] == 0:
                ...
                self.logger.warning(
                    f"Failed to cancel order {order_id}: {response.get('retMsg', 'Unknown error')}"
                )
                return False
    ...
    async def _cancel_all_orders(self):
        ...
        try:
            ...
            if response["retCode"] == 0:
                ...
                self.logger.error(
                    f"Failed to cancel all orders: {response.get('retMsg', 'Unknown error')}"
                )
                return False
    ...
    async def _calculate_order_size(self, price: Decimal) -> Decimal:
        ...
        if self.current_balance <= 0 or self.mid_price <= 0:
            self.logger.warning(
                "Cannot calculate order size: Balance or mid-price is zero."
            )
            return Decimal("0")
        ...
        if self.market_info:
            ...
            if qty_in_base < self.market_info.min_order_qty:
                self.logger.warning(
                    f"Calculated quantity {qty_in_base} is less than min order qty {self.market_info.min_order_qty}. Returning 0."
                )
                return Decimal("0")
            ...
            if notional_value < self.market_info.min_notional_value:
                self.logger.warning(
                    f"Calculated notional value {notional_value} is less than min notional value {self.market_info.min_notional_value}. Returning 0."
                )
                return Decimal("0")
        else:
    ...
    async def _place_limit_order(
        ...
        if self.config.trading_mode == "DRY_RUN":
            self.logger.info(
                f"DRY_RUN: Would place {side} order: Price={price}, Qty={quantity}"
            )
            ...
            async with self.active_orders_lock:
                self.active_orders[simulated_order_id] = {
                    ...
                    "qty": quantity,
                    "status": "New",
                    "orderLinkId": f"mm_{side}_{int(time.time() * 1000)}",
                }
        ...
        try:
            ...
            price_formatted = (
                self.market_info.format_price(price) if self.market_info else price
            )

            if qty_formatted == Decimal("0") or price_formatted == Decimal("0"):
                self.logger.warning(
                    f"Attempted to place order with zero quantity or price: Qty={qty_formatted}, Price={price_formatted}. Skipping."
                )
                return None
            ...
            if response["retCode"] == 0:
                ...
                self.logger.info(
                    f"Placed {side} order: ID={order_id}, Price={price_formatted}, Qty={qty_formatted}, LinkID={order_link_id}"
                )
                ...
                asyncio.create_task(
                    self.db_manager.log_order_event(
                        {
                            "orderId": order_id,
                            "orderLinkId": order_link_id,
                            "symbol": self.config.symbol,
            ...
            self.logger.error(
                f"Failed to place {side} order: {response.get('retMsg', 'Unknown error')}"
            )
            return None
    ...
    async def _reconcile_orders_on_startup(self):
        ...
        try:
            ...
            if response["retCode"] == 0:
                ...
                self.logger.error(
                    f"Failed to fetch open orders from exchange: {response.get('retMsg', 'Unknown error')}"
                )
                return

        except Exception as e:
            self.logger.error(
                f"Error fetching open orders from exchange during reconciliation: {e}"
            )
            return
        ...
        async with self.active_orders_lock:
            ...
            for order_id in orders_to_remove:
                self.logger.warning(
                    f"Local order {order_id} not found on exchange. Removing from local state."
                )
                del self.active_orders[order_id]
            ...
            for order_id, order_info in exchange_open_orders.items():
                if order_id not in self.active_orders:
                    ...
                    self.logger.warning(
                        f"Exchange order {order_id} (Status: {order_info['status']}) not in local state. Adding to local state."
                    )
                    asyncio.create_task(
                        self.db_manager.log_order_event(
                            {
        ...
        self.logger.info(
            f"Order reconciliation complete. {len(self.active_orders)} active orders in state."
        )
    ...
    async def _manage_orders(self):
        ...
        ) < self.config.order_refresh_interval_sec:
        ...
        async with self.market_data_lock:
            if self.mid_price == 0:
                self.logger.info(
                    "Waiting for initial mid-price before managing orders..."
                )
                return

            target_bid_price = self.mid_price * (
                Decimal("1") - self.config.strategy.spread_pct
            )
            target_ask_price = self.mid_price * (
                Decimal("1") + self.config.strategy.spread_pct
            )

            # Format prices to the correct precision
            target_bid_price = (
                self.market_info.format_price(target_bid_price)
                if self.market_info
                else target_bid_price
            )
            target_ask_price = (
                self.market_info.format_price(target_ask_price)
                if self.market_info
                else target_ask_price
            )
        ...
        async with self.active_orders_lock:
            for order_id, order_info in list(
                ...
                if order_info["side"] == "Buy":
                    ...
                    if (
                        not current_bid_order
                        and abs(order_info["price"] - target_bid_price)
                        <= target_bid_price
                        * self.config.strategy.order_stale_threshold_pct
                    ):
                        current_bid_order = order_info
                        self.logger.debug(
                            f"Existing bid order {order_id} is still valid at {order_info['price']}."
                        )
                    else:
                        self.logger.info(
                            f"Bid order {order_id} at {order_info['price']} is stale or a duplicate. Marking for cancellation. (Target: {target_bid_price})"
                        )
                        orders_to_cancel.append(
                            (order_id, order_info.get("orderLinkId"))
                        )
                elif order_info["side"] == "Sell":
                    # Check if ask order is stale
                    if (
                        not current_ask_order
                        and abs(order_info["price"] - target_ask_price)
                        <= target_ask_price
                        * self.config.strategy.order_stale_threshold_pct
                    ):
                        current_ask_order = order_info
                        self.logger.debug(
                            f"Existing ask order {order_id} is still valid at {order_info['price']}."
                        )
                    else:
                        self.logger.info(
                            f"Ask order {order_id} at {order_info['price']} is stale or a duplicate. Marking for cancellation. (Target: {target_ask_price})"
                        )
                        orders_to_cancel.append(
                            (order_id, order_info.get("orderLinkId"))
                        )
        ...
        if order_qty == Decimal("0"):
            self.logger.warning(
                "Calculated order quantity is zero or too small. Skipping order placement."
            )
            return

        if not current_bid_order:
            self.logger.info(
                f"No valid bid order found. Placing new bid at {target_bid_price} with Qty {order_qty}."
            )
        ...
        if not current_ask_order:
            self.logger.info(
                f"No valid ask order found. Placing new ask at {target_ask_price} with Qty {order_qty}."
            )
    ...
    async def run(self):
        ...
        if state:
            ...
            self.logger.info(
                f"Loaded state with {len(self.active_orders)} active orders."
            )
    ...
    async def stop(self):
        ...
        state = {"active_orders": self.active_orders, "metrics": self.config.metrics}
