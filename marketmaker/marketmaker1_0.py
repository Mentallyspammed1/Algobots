...
from tenacity import (
    before_sleep_log,
    retry,
    retry_if_exception,  # Use retry_if_exception for custom predicates
    stop_after_attempt,
...
class TradingClient:
    ...
    def _get_api_retry_decorator(self):
        ...
        return retry(
            ...
            retry=retry_if_exception(self._is_retryable_bybit_error), # Use the custom predicate
...
class BybitMarketMaker:
    ...
    async def _initialize_bot(self):
        ...
        if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
            # Set a mock mid_price for DRY_RUN/SIMULATION modes
            if self.state.mid_price == Decimal('0'):
                mock_price = Decimal('0.1') # A reasonable mock price for XLMUSDT
    ...
    async def _simulate_dry_run_price_movement(self, current_time: float):
        """
        Simulates price movement for DRY_RUN/SIMULATION modes to enable PnL tracking.
        Applies a small oscillation to the mid_price.
        """
        if self.state.mid_price == Decimal('0'):
            # Should ideally be set during initialization, but as a fallback
        ...
        oscillation_amplitude = Decimal('0.0001') # 0.01% price movement
        oscillation_period_sec = 10 # Full cycle every 10 seconds

        time_factor = Decimal(str(current_time % oscillation_period_sec)) / Decimal(str(oscillation_period_sec))
        ...
        async with self.market_data_lock:
            ...
            self.state.circuit_breaker_price_points.append((current_time, self.state.mid_price)) # For volatility calc
            self.state.mid_price_history.append(float(self.state.mid_price)) # For volatility calc
    ...
    async def _simulate_dry_run_fills(self):
        """
        Simulates order fills for DRY_RUN/SIMULATION modes based on current mid_price.
        """
        orders_to_process = []
        async with self.active_orders_lock:
                ...
                if order_id.startswith('DRY_'): # Only process mock orders
                    ...
                    if filled:
                        ...
                        if side == 'Sell' and order_data['qty'] > self.state.metrics.current_asset_holdings:
                            self.logger.warning(f"DRY_RUN: Skipping simulated sell fill for order {order_id} (Qty: {order_data['qty']}) due to insufficient simulated holdings ({self.state.metrics.current_asset_holdings}).")
                            continue # Skip this order
                        orders_to_process.append((order_id, order_data))
            ...
            for order_id, order_data in orders_to_process:
                ...
                mock_fill_data = {
                    ...
                    'orderStatus': 'Filled',
                    'execQty': str(order_data['qty']),
                    'execPrice': str(self.state.mid_price), # Fill at current mid_price
                    'execFee': str(order_data['qty'] * self.state.mid_price * self.market_info.taker_fee_rate), # Simulate taker fee
                    'feeCurrency': self.config.quote_currency,
                    'pnl': '0' # PnL will be calculated by _process_fill
                }
                await self._process_fill(mock_fill_data)
                del self.state.active_orders[order_id] # Remove after processing fill
    ...
    async def _update_mid_price(self, bids: list[list[str]], asks: list[list[str]]):
        ...
        async with self.market_data_lock:
            ...
            if new_mid_price != self.state.mid_price:
                self.state.mid_price = new_mid_price
                self.state.mid_price_history.append(float(self.state.mid_price))
                self.state.circuit_breaker_price_points.append((time.time(), self.state.mid_price))
    ...
    async def _reconcile_and_place_orders(self, target_bid: Decimal, target_ask: Decimal):
        ...
        new_orders_to_place = []
        current_outstanding_orders = len(self.state.active_orders) - len(orders_to_cancel)
    ...
    async def _fetch_market_info(self) -> bool:
        ...
        try:
            ...
            default_maker_fee = Decimal('0.0002') # Default maker fee
            default_taker_fee = Decimal('0.0005') # Default taker fee
    ...
    async def _calculate_order_size(self, side: str, price: Decimal) -> Decimal:
        ...
        if self.config.strategy.inventory.enabled and self.config.max_net_exposure_usd > Decimal('0'):
            ...
            if side == 'Buy':
                ...
                if metrics_pos_qty > Decimal('0'): # If currently long, cap sell quantity at current holdings
                    target_qty = min(target_qty, metrics_pos_qty)
                    self.logger.debug(f"Capping sell order quantity at current holdings: {target_qty}")
                else: # If not long, calculate based on max short exposure
