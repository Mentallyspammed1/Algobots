...
class TradeMetrics:
    ...
    def update_pnl_on_sell(self, quantity: Decimal, price: Decimal):
        if self.current_asset_holdings < quantity:
            raise ValueError(f"Attempted to sell {quantity} but only {self.current_asset_holdings} held.")
...
class CircuitBreakerConfig:
    ...
    max_daily_loss_pct: Decimal = Decimal('0.05') # New: Max percentage loss of initial capital for the day
...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.symbol.endswith("USDT"):
            ...
            raise ConfigurationError(f"Cannot parse base/quote currency from symbol: {self.symbol}. Use a standard format (e.g., BTCUSDT).")
        ...
        if self.category in ['linear', 'inverse']:
            if self.leverage <= 0:
                raise ConfigurationError("Leverage must be a positive decimal for linear/inverse categories.")
        elif self.category == 'spot':
            if self.leverage != Decimal('1'):
                _set_attr('leverage', Decimal('1'))
                logging.getLogger('MarketMakerBot').warning("Leverage is not applicable for spot trading. Setting leverage to 1.")

        if self.strategy.inventory.enabled and self.max_net_exposure_usd <= 0:
             raise ConfigurationError("max_net_exposure_usd must be positive when inventory strategy is enabled.")
        if not (Decimal('0') < self.max_order_size_pct <= Decimal('1')):
            raise ConfigurationError("max_order_size_pct must be between 0 and 1 (exclusive).")
        ...
        if self.strategy.dynamic_spread.enabled:
            if not (Decimal('0') <= self.strategy.dynamic_spread.min_spread_pct <= self.strategy.dynamic_spread.max_spread_pct):
                raise ConfigurationError("Dynamic spread min/max percentages are invalid.")
            if not (Decimal('0') < self.strategy.dynamic_spread.price_change_smoothing_factor < Decimal('1')):
                raise ConfigurationError("Price change smoothing factor must be between 0 and 1 (exclusive).")
        
        if not (Decimal('0') <= self.strategy.circuit_breaker.max_daily_loss_pct < Decimal('1')):
            raise ConfigurationError("max_daily_loss_pct must be between 0 and 1 (exclusive).")
...
class TradingState:
    ...
    # For dynamic spread and circuit breaker (now stores (timestamp, high, low, close))
...
class StateManager:
    ...
    async def load_state(self) -> dict | None:
        ...
        try:
            ...
            self.logger.error(f"Error loading state from {self.file_path}: {e}. Starting fresh.", exc_info=True)
            return None
...
class DBManager:
    ...
    async def create_tables(self):
        ...
        await _add_column_if_not_exists("trade_fills", "liquidity_role", "TEXT", "'UNKNOWN'")
        await _add_column_if_not_exists("bot_metrics", "realized_pnl", "TEXT", "'0'")
        await _add_column_if_not_exists("bot_metrics", "net_realized_pnl", "TEXT", "'0'")
        ...
        await _add_column_if_not_exists("bot_metrics", "current_asset_holdings", "TEXT", "'0'")
        await _add_column_if_not_exists("bot_metrics", "average_entry_price", "TEXT", "'0'")
    ...
    async def log_order_event(self, order_data: dict, message: str | None = None):
        ...
        try:
            await self.conn.execute(
                ...
                (datetime.now(timezone.utc).isoformat(), order_data.get('orderId'), order_data.get('orderLinkId'), order_data.get('symbol'), order_data.get('side'), order_data.get('orderType'), str(order_data.get('price','0')), str(order_data.get('qty','0')), order_data.get('orderStatus'), message, str(order_data.get('cumExecQty', '0')))
            )
            await self.conn.commit()
        except Exception as e:
    ...
    async def log_trade_fill(self, trade_data: dict, realized_pnl_impact: Decimal):
        ...
        try:
            await self.conn.execute(
                ...
                (datetime.now(timezone.utc).isoformat(), trade_data.get('orderId'), trade_data.get('execId'), trade_data.get('symbol'), trade_data.get('side'), str(trade_data.get('execPrice','0')), str(trade_data.get('execQty','0')), str(trade_data.get('execFee','0')), trade_data.get('feeCurrency'), str(trade_data.get('pnl','0')), str(realized_pnl_impact), trade_data.get('execType', 'UNKNOWN'))
            )
            await self.conn.commit()
        except Exception as e:
    ...
    async def log_bot_metrics(self, metrics: TradeMetrics, unrealized_pnl: Decimal, daily_pnl: Decimal, daily_loss_pct: float):
        if not self.conn: return
        try:
            await self.conn.execute(
                ...
                (datetime.now(timezone.utc).isoformat(), metrics.total_trades, str(metrics.net_realized_pnl), str(metrics.realized_pnl), str(unrealized_pnl), str(metrics.gross_profit), str(metrics.gross_loss), str(metrics.total_fees), metrics.wins, metrics.losses, metrics.win_rate, str(metrics.current_asset_holdings), str(metrics.average_entry_price), str(daily_pnl), daily_loss_pct)
            )
            await self.conn.commit()
        except Exception as e:
...
class BybitMarketMaker:
    def __init__(self, config: Config):
        ...
        self.state = TradingState(
            ...
            price_candlestick_history=deque(maxlen=self.config.strategy.dynamic_spread.volatility_window_sec + 1), # Store enough for ATR
    ...
    async def run(self):
        ...
        except (APIAuthError, WebSocketConnectionError, asyncio.CancelledError, KeyboardInterrupt, BybitAPIError) as e:
            self.logger.info(f"Bot stopping gracefully due to: {type(e).__name__} - {e}")
        except Exception as e:
            self.logger.critical(f"An unhandled critical error occurred in the main loop: {e}", exc_info=True)
        finally:
            await self.stop()
    ...
    async def _initialize_bot(self):
        ...
        if not await self._update_balance_and_position():
            raise InitialBalanceError("Failed to fetch initial balance/position. Shutting down.")
        
        # Initialize daily_initial_capital if not set or it's a new day
        if self.state.daily_initial_capital == Decimal('0') or \
           (self.state.daily_pnl_reset_date is not None and self.state.daily_pnl_reset_date.date() < datetime.now(timezone.utc).date()):
            self.state.daily_initial_capital = self.state.current_balance
            self.state.daily_pnl_reset_date = datetime.now(timezone.utc)
            self.logger.info(f"Daily initial capital set to: {self.state.daily_initial_capital} {self.config.quote_currency}")

        if self.config.trading_mode not in ["DRY_RUN", "SIMULATION"]:
            if self.config.category in ['linear', 'inverse'] and not await self.trading_client.set_leverage(self.config.leverage):
                raise InitialBalanceError("Failed to set leverage. Shutting down.")
        else:
            self.logger.info(f"{self.config.trading_mode} mode: Skipping leverage setting.")
        ...
        if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
            if self.state.mid_price == Decimal('0'):
                ...
                self.logger.info(f"{self.config.trading_mode} mode: Initialized mock mid_price to {mock_price}.")
    ...
    async def _load_bot_state(self):
        ...
        if state_data:
            ...
            self.state.metrics.last_pnl_update_timestamp = state_data.get('metrics_last_pnl_update_timestamp')
            ...
            self.state.daily_initial_capital = Decimal(str(state_data.get('daily_initial_capital', '0')))
            ...
            async with self.market_data_lock:
                ...
                self.state.smoothed_mid_price = Decimal(str(state_data.get('smoothed_mid_price', '0')))
            ...
            self.logger.info(f"Loaded state with {len(self.state.active_orders)} active orders and metrics: {self.state.metrics}")
        else:
    ...
    async def _connect_websockets(self):
        ...
        try:
            self.trading_client._init_public_ws(lambda msg: self._schedule_coro(self.orderbook_queue.put(msg)))
            self.trading_client._init_private_ws(
                lambda msg: self._schedule_coro(self.private_ws_queue.put(msg)),
                lambda msg: self._schedule_coro(self.private_ws_queue.put(msg))
            )
            self.state.ws_reconnect_attempts_left = self.config.system.ws_reconnect_attempts
            self.logger.info("WebSockets connected and subscribed. Queues active.")
        except Exception as e:
            self.logger.error(f"Failed to establish initial WebSocket connections: {e}", exc_info=True)
    ...
    async def _reconnect_websockets(self):
        ...
        delay = self.config.system.ws_reconnect_initial_delay_sec * (2 ** (self.config.system.ws_reconnect_attempts - self.state.ws_reconnect_attempts_left - 1))
        delay = min(delay, self.config.system.ws_reconnect_max_delay_sec)
        self.logger.warning(f"Attempting WebSocket reconnection in {delay:.1f} seconds... ({self.state.ws_reconnect_attempts_left} attempts left)")
    ...
    async def _main_loop_tick(self):
        ...
        if self.state.is_paused and current_time < self.state.pause_end_time:
            self.logger.debug(f"Bot is paused due to circuit breaker. Resuming in {int(self.state.pause_end_time - current_time)}s.")
            return
        ...
        if current_time < self.state.circuit_breaker_cooldown_end_time:
            self.logger.debug(f"Circuit breaker in cooldown. Resuming trading in {int(self.state.circuit_breaker_cooldown_end_time - current_time)}s.")
            return

        await self._manage_orders()

        if current_time - self.state.last_status_report_time > self.config.system.status_report_interval_sec:
        ...
        try:
            if 'data' in message and message['topic'].startswith("orderbook"):
                ...
                if bids and asks and bids[0] and asks[0] and bids[0][0] and asks[0][0]:
                    ...
                    self.logger.debug("Received empty or incomplete orderbook data. Skipping mid-price update.")
        except Exception as e:
    ...
    async def _process_private_ws_message(self, message: dict):
        ...
        try:
            ...
            self.logger.error(f"Error processing private WS message: {e}", exc_info=True)

    async def _websocket_health_check(self, current_time: float):
        ws_public_connected = self.trading_client.ws_public and self.trading_client.ws_public.is_connected()
        ws_private_connected = self.trading_client.ws_private and self.trading_client.ws_private.is_connected()

        if not ws_public_connected or not ws_private_connected:
            self.logger.warning(f"WebSocket {'Public' if not ws_public_connected else 'Private'} disconnected. Attempting reconnection.")
            await self._reconnect_websockets()
            return

        if current_time - self.state.last_ws_message_time > self.config.system.ws_heartbeat_sec:
            self.logger.warning("WebSocket heartbeat lost (no new messages received). Attempting reconnection.")
            await self._reconnect_websockets()
            return
    ...
    async def _simulate_dry_run_price_movement(self, current_time: float):
        if self.state.mid_price == Decimal('0'):
            ...
            self.state.price_candlestick_history.append((current_time, self.state.mid_price, self.state.mid_price, self.state.mid_price))
            self.logger.info("DRY_RUN/SIMULATION: Initializing mid_price for movement simulation.")
        ...
        # Ensure log(price_float) doesn't result in math domain error for very small prices
        if price_float <= 0:
            price_float = 1e-10 # Set a very small positive value to avoid error
        ...
        async with self.market_data_lock:
            ...
            if self.state.price_candlestick_history:
                ...
                # Keep the last entry's timestamp if it's within the same "candle" or update
        ...
        self.logger.debug(f"DRY_RUN Price Movement: Mid: {self.state.mid_price}, Smoothed: {self.state.smoothed_mid_price}")
            ...
            for order_id, order_data in list(self.state.active_orders.items()):
                if order_id.startswith('DRY_'):
                    ...
                    if filled:
                        ...
                        if fill_qty <= Decimal('0'): continue
                        
                        if side == 'Sell' and fill_qty > self.state.metrics.current_asset_holdings:
                            self.logger.warning(f"DRY_RUN: Skipping simulated sell fill for order {order_id} (Qty: {fill_qty}) due to insufficient simulated holdings ({self.state.metrics.current_asset_holdings}).")
                            continue
                        
                        orders_to_process.append((order_id, order_data, fill_qty))

            for order_id, order_data, fill_qty in orders_to_process:
                self.logger.info(f"DRY_RUN: Simulating fill for order {order_id} (Side: {order_data['side']}, Price: {order_data['price']}) with {fill_qty} at current mid_price {self.state.mid_price}")
                ...
                mock_fill_data = {
                    ...
                    'execFee': str(fill_qty * self.state.mid_price * self.market_info.taker_fee_rate),
    ...
    async def _periodic_health_check(self, current_time: float):
        if (current_time - self.state.last_health_check_time) < self.config.system.health_check_interval_sec:
            return
    ...
    async def stop(self):
        ...
        try:
            ...
            self.logger.error(f"Error during state saving on shutdown: {e}", exc_info=True)
        ...
        try:
            ...
            daily_loss_pct = float(abs(daily_pnl / self.state.daily_initial_capital)) if self.state.daily_initial_capital > 0 else 0.0
            await self.db_manager.log_bot_metrics(self.state.metrics, unrealized_pnl, daily_pnl, daily_loss_pct)
        except Exception as e:
            self.logger.error(f"Could not log final metrics to DB. Error: {e}", exc_info=True)
        ...
        async with self.market_data_lock:
            ...
            if new_mid_price != self.state.mid_price:
                ...
                if self.state.price_candlestick_history:
                    ...
                    if (current_time - last_ts) < self.config.system.loop_interval_sec * 2: # Arbitrary small window
                ...
                self.logger.debug(f"Mid-price updated to: {self.state.mid_price}, Smoothed: {self.state.smoothed_mid_price}")
    ...
    async def _process_order_update(self, order_data: dict):
        ...
        self.logger.info(f"Order {order_id} status update: {status} (OrderLink: {order_data.get('orderLinkId')}), CumExecQty: {cum_exec_qty}/{order_qty}")
        ...
        async with self.active_orders_lock:
            if order_id in self.state.active_orders:
                ...
                if status == 'Filled' or (status == 'PartiallyFilled' and cum_exec_qty > existing_order.get('cumExecQty', Decimal('0'))):
                    # For a true fill, we need the execQty from a trade event, not the order update.
                    # The _process_fill method is called by _simulate_dry_run_fills or directly from WS trade stream.
                    # This order update primarily manages the status and cumExecQty in our local state.
                    pass # The _process_fill will handle the actual PnL and trade logging

                if status == 'Filled':
                    self.logger.info(f"Order {order_id} fully filled. Removing from active orders.")
                    del self.state.active_orders[order_id]
                elif status in ['Cancelled', 'Rejected', 'Deactivated', 'Expired']:
                    self.logger.info(f"Order {order_id} removed from active orders due to status: {status}.")
                    del self.state.active_orders[order_id]
            elif status == 'Filled' or status == 'PartiallyFilled':
                self.logger.warning(f"Received fill/partial fill for untracked order {order_id}. Adding to state temporarily for processing.")
                ...
                if status == 'Filled':
                    self.logger.info(f"Order {order_id} (untracked) fully filled. Removing after processing.")
                    del self.state.active_orders[order_id]
            else:
                self.logger.debug(f"Received update for untracked order {order_id} with status {status}. Ignoring.")
    ...
    async def _process_position_update(self, pos_data: dict):
        async with self.balance_position_lock:
            ...
            if new_pos_qty != self.state.current_position_qty:
                ...
                self.logger.info(f"POSITION UPDATE (WS): Position is now {self.state.current_position_qty} {self.config.base_currency}")

            if self.config.category in ['linear', 'inverse'] and 'unrealisedPnl' in pos_data:
                self.state.unrealized_pnl_derivatives = Decimal(pos_data['unrealisedPnl'])
                self.logger.debug(f"UNREALIZED PNL (WS): {self.state.unrealized_pnl_derivatives:+.4f} {self.config.quote_currency}")
    ...
    async def _check_daily_loss_circuit_breaker(self) -> bool:
        ...
        if self.state.daily_initial_capital <= Decimal('0'):
            self.logger.warning("Daily initial capital is zero or negative, cannot check daily loss. Skipping.")
            return False

        current_total_capital = self.state.current_balance + self.state.metrics.calculate_unrealized_pnl(self.state.mid_price)
        
        daily_loss = self.state.daily_initial_capital - current_total_capital
        if daily_loss > (self.state.daily_initial_capital * cb_config.max_daily_loss_pct):
    ...
    async def _manage_orders(self):
        current_time = time.time()
        if (current_time - self.state.last_order_management_time) < self.config.system.order_refresh_interval_sec:
            return
        ...
        async with self.market_data_lock, self.balance_position_lock:
            if self.state.smoothed_mid_price == Decimal('0') or not self.market_info:
                self.logger.warning("Smoothed mid-price or market info not available, skipping order management.")
                return
        ...
        target_bid_price, target_ask_price = self._enforce_min_profit_spread(mid_price_for_strategy, target_bid_price, target_ask_price)
    ...
    async def _check_circuit_breaker(self) -> bool:
        ...
        if start_price == Decimal('0'): return False

        price_change_pct = abs(end_price - start_price) / start_price

        if price_change_pct > cb_config.pause_threshold_pct:
            self.logger.warning(f"CIRCUIT BREAKER TRIPPED: Price changed {price_change_pct:.2%} in {cb_config.check_window_sec}s. Pausing trading for {cb_config.pause_duration_sec}s.")
    ...
    async def _calculate_dynamic_spread(self) -> Decimal:
        ...
        async with self.market_data_lock:
            # Filter history for the volatility window
            relevant_candles = [c for c in self.state.price_candlestick_history if (current_time - c[0]) <= ds_config.volatility_window_sec]
        ...
        for i in range(len(relevant_candles)):
            ...
            # Use current mid-price as today's close if it's the latest data point
            if i == len(relevant_candles) - 1 and self.state.mid_price > Decimal('0'):
                close = self.state.mid_price

            if i == 0:
                # For the first candle, TR is (High - Low)
                tr = high - low
            else:
        ...
        if self.state.mid_price <= Decimal('0'):
            self.logger.warning("Mid-price is zero, cannot calculate ATR-based spread. Using base spread.")
            return self.config.strategy.base_spread_pct

        # Convert ATR (absolute value) to a percentage of the mid-price
        ...
        self.logger.debug(f"ATR: {atr_value:.6f}, Volatility_pct: {volatility_pct:.6f}, Dynamic Spread: {clamped_spread:.4%}")
    ...
    def _calculate_inventory_skew(self, mid_price: Decimal, pos_qty: Decimal) -> Decimal:
        ...
        if not inv_config.enabled or self.config.max_net_exposure_usd <= 0 or mid_price <= 0:
            return Decimal('0')

        current_inventory_value = pos_qty * mid_price
        max_exposure_for_ratio = self.config.max_net_exposure_usd * inv_config.max_inventory_ratio
        if max_exposure_for_ratio <= 0: return Decimal('0')
    ...
    def _enforce_min_profit_spread(self, mid_price: Decimal, bid_p: Decimal, ask_p: Decimal) -> tuple[Decimal, Decimal]:
        if not self.market_info or mid_price <= Decimal('0'):
            self.logger.warning("Mid-price or market info not available for enforcing minimum profit spread. Returning original bid/ask.")
            return bid_p, ask_p

        estimated_fee_per_side_pct = self.market_info.taker_fee_rate
        min_gross_spread_pct = self.config.strategy.min_profit_spread_after_fees_pct + (estimated_fee_per_side_pct * Decimal('2'))
        min_spread_val = mid_price * min_gross_spread_pct

        if ask_p <= bid_p or (ask_p - bid_p) < min_spread_val:
            self.logger.debug(f"Adjusting spread. Original Bid: {bid_p}, Ask: {ask_p}, Mid: {mid_price}, Current Spread: {ask_p - bid_p:.6f}, Min Spread: {min_spread_val:.6f}")
            half_min_spread = (min_spread_val / Decimal('2')).quantize(self.market_info.price_precision)
            bid_p = (mid_price - half_min_spread).quantize(self.market_info.price_precision)
            ask_p = (mid_price + half_min_spread).quantize(self.market_info.price_precision)
    ...
    async def _reconcile_and_place_orders(self, target_bid: Decimal, target_ask: Decimal):
        ...
        async with self.active_orders_lock:
            for order_id, order_data in list(self.state.active_orders.items()):
                if order_data.get('symbol') != self.config.symbol:
                    self.logger.warning(f"Found untracked symbol order {order_id} in active_orders. Cancelling.")
                ...
                if order_data['status'] in ['Filled', 'PartiallyFilled', 'Cancelled', 'Rejected', 'Deactivated', 'Expired']:
                    if order_data['qty'] == order_data.get('cumExecQty', Decimal('0')): # Fully filled
                        self.logger.debug(f"Removing fully processed order {order_id} from active orders.")
                        del self.state.active_orders[order_id]
                    else: # Partially filled, but remaining quantity isn't active anymore or needs re-evaluation
                        if order_data['status'] in ['Cancelled', 'Rejected', 'Deactivated', 'Expired']:
                             self.logger.debug(f"Removing partially filled but inactive order {order_id} from active orders.")
                             del self.state.active_orders[order_id]
                        else: # Still partially filled and active, but not fully executed. Keep for now, but don't count towards new placement.
                            current_active_orders_by_side[order_data['side']].append((order_id, order_data, False)) # Not stale, but could be replaced
                    continue

                is_stale = False
                if order_data['side'] == 'Buy':
                    if abs(order_data['price'] - target_bid) > (order_data['price'] * self.config.strategy.order_stale_threshold_pct):
                        is_stale = True
                    current_active_orders_by_side['Buy'].append((order_id, order_data, is_stale))
                else:
                    if abs(order_data['price'] - target_ask) > (order_data['price'] * self.config.strategy.order_stale_threshold_pct):
                        is_stale = True
                    current_active_orders_by_side['Sell'].append((order_id, order_data, is_stale))
        ...
        for oid, olid in orders_to_cancel:
            ...
            self.logger.info(f"Cancelling stale/duplicate order {oid} (Side: {order_info.get('side')}, Price: {order_info.get('price')}). Target Bid: {target_bid}, Target Ask: {target_ask}")
            await self._cancel_order(oid, olid)

        new_orders_to_place = []
        
        # Recalculate current outstanding orders after cancellations
        current_outstanding_orders = 0
        async with self.active_orders_lock:
            for order_id, order_data in self.state.active_orders.items():
                if order_data['status'] not in ['Filled', 'Cancelled', 'Rejected', 'Deactivated', 'Expired']:
                    current_outstanding_orders += 1

        if not bid_order_to_keep and current_outstanding_orders < self.config.strategy.max_outstanding_orders:
            buy_qty = await self._calculate_order_size("Buy", target_bid)
            if buy_qty > 0:
                new_orders_to_place.append(("Buy", target_bid, buy_qty))
                current_outstanding_orders += 1
            else:
                self.logger.debug("Calculated buy quantity is zero or too small, skipping bid order placement.")

        if not ask_order_to_keep and current_outstanding_orders < self.config.strategy.max_outstanding_orders:
            sell_qty = await self._calculate_order_size("Sell", target_ask)
            if sell_qty > 0:
                new_orders_to_place.append(("Sell", target_ask, sell_qty))
            else:
                self.logger.debug("Calculated sell quantity is zero or too small, skipping ask order placement.")
        elif not ask_order_to_keep:
            self.logger.warning(f"Skipping ask order placement: Max outstanding orders ({self.config.strategy.max_outstanding_orders}) reached or will be exceeded.")
    ...
    async def _log_status_summary(self):
        async with self.balance_position_lock, self.active_orders_lock, self.market_data_lock:
            ...
            current_market_price = self.state.mid_price if self.state.mid_price > Decimal('0') else self.state.smoothed_mid_price

            if current_market_price == Decimal('0'):
                self.logger.warning("Cannot calculate unrealized PnL, current market price is zero.")
            ...
            exposure_usd = pos_qty * current_market_price if current_market_price > Decimal('0') else Decimal('0')
            
            daily_pnl = current_total_capital = self.state.current_balance + display_unrealized_pnl - self.state.daily_initial_capital
            daily_loss_pct = float(abs(daily_pnl / self.state.daily_initial_capital)) if self.state.daily_initial_capital > 0 and daily_pnl < 0 else 0.0
        ...
        await self.db_manager.log_bot_metrics(metrics, display_unrealized_pnl, daily_pnl, daily_loss_pct)
    ...
    async def _fetch_market_info(self) -> bool:
        if self.config.trading_mode == "SIMULATION":
            ...
            self.logger.info(f"SIMULATION mode: Mock market info loaded for {self.config.symbol}: {self.market_info}")
            return True

        info = await self.trading_client.get_instruments_info()
        if not info:
            self.logger.critical("Failed to retrieve instrument info from API. Check symbol and connectivity.")
            return False
        ...
        try:
            ...
            if maker_fee_rate_str is None:
                if taker_fee_rate_str is not None:
                    self.logger.warning(f"Missing 'makerFeeRate' for {self.config.symbol}. Using 'takerFeeRate' as fallback for maker fee. Full info: {info}")
                    maker_fee_rate = Decimal(taker_fee_rate_str)
                else:
                    self.logger.warning(f"Missing 'makerFeeRate' and 'takerFeeRate' for {self.config.symbol}. Using default maker fee: {default_maker_fee}. Full info: {info}")
                    maker_fee_rate = default_maker_fee
            else:
                maker_fee_rate = Decimal(maker_fee_rate_str)

            if taker_fee_rate_str is None:
                self.logger.warning(f"Missing 'takerFeeRate' for {self.config.symbol}. Using default taker fee: {default_taker_fee}. Full info: {info}")
            ...
            if self.market_info.min_notional_value == Decimal('0'):
                self.logger.warning(f"minNotionalValue not found or is 0 for {self.config.symbol}. Using config.min_order_value_usd as fallback.")
            ...
            self.logger.critical(f"Error parsing market info (missing key or invalid format): {e}. Full info: {info}", exc_info=True)
            return False
        except Exception as e:
            self.logger.critical(f"Unexpected error while parsing market info: {e}. Full info: {info}", exc_info=True)
            return False
        ...
        async with self.balance_position_lock:
            if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
                if self.state.current_balance == Decimal('0'):
                    ...
                    self.logger.info(f"{self.config.trading_mode}: Initialized virtual balance: {self.state.current_balance} {self.config.quote_currency}")
            ...
            for coin in balance_data.get('coin', []):
                if coin['coin'] == self.config.quote_currency:
                    ...
                    self.logger.debug(f"Balance: {self.state.current_balance} {self.config.quote_currency}, Available: {self.state.available_balance}")
                    await self.db_manager.log_balance_update(self.config.quote_currency, self.state.current_balance, self.state.available_balance)
                    found_quote_balance = True
                    break

            if not found_quote_balance:
                self.logger.warning(f"Could not find balance for {self.config.quote_currency}. This might affect order sizing.")
            ...
            if self.config.category in ['linear', 'inverse']:
                ...
                if position_data and position_data.get('size'):
                    ...
                    self.state.unrealized_pnl_derivatives = Decimal(position_data.get('unrealisedPnl', '0'))
                else:
            ...
            self.logger.info(f"Updated Balance: {self.state.current_balance} {self.config.quote_currency}, Position: {self.state.current_position_qty} {self.config.base_currency}, UPNL (Deriv): {self.state.unrealized_pnl_derivatives:+.4f}")
            return True
    ...
    async def _process_fill(self, trade_data: dict):
        ...
        liquidity_role = trade_data.get('execType', 'UNKNOWN') # Bybit returns 'Trade' for taker, 'AdlTrade' for ADL, 'Funding' etc.
        ...
        if side == 'Buy':
            ...
            if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
                ...
                self.state.available_balance = self.state.current_balance
            self.logger.info(f"Order FILLED: BUY {exec_qty} @ {exec_price}, Fee: {exec_fee}. Holdings: {metrics.current_asset_holdings}, Avg Entry: {metrics.average_entry_price}")
        elif side == 'Sell':
            ...
            if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
                self.state.current_balance += (exec_qty * exec_price) - exec_fee
                self.state.available_balance = self.state.current_balance
            self.logger.info(f"Order FILLED: SELL {exec_qty} @ {exec_price}, Fee: {exec_fee}. Realized PnL from sale: {profit_loss_on_sale:+.4f}. Holdings: {metrics.current_asset_holdings}, Avg Entry: {metrics.average_entry_price}")
        else:
            self.logger.warning(f"Unknown side '{side}' for fill. Cannot update PnL metrics.")
    ...
    async def _cancel_order(self, order_id: str, order_link_id: str | None = None):
        self.logger.info(f"Attempting to cancel order {order_id} (OrderLink: {order_link_id})...")
        if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
            self.logger.info(f"{self.config.trading_mode}: Would cancel order {order_id}.")
            async with self.active_orders_lock:
                if order_id in self.state.active_orders:
                    del self.state.active_orders[order_id]
            return
        ...
        try:
            ...
            self.logger.error(f"Error during cancellation of order {order_id}: {e}", exc_info=True)

    async def _cancel_all_orders(self):
        self.logger.info("Canceling all open orders...")
        if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
            self.logger.info(f"{self.config.trading_mode}: Would cancel all open orders.")
        else:
            try:
                ...
                self.logger.error(f"Error during cancellation of all orders: {e}", exc_info=True)
    ...
    async def _calculate_order_size(self, side: str, price: Decimal) -> Decimal:
        async with self.balance_position_lock:
            capital = self.state.available_balance if self.config.category == 'spot' else self.state.current_balance
            metrics_pos_qty = self.state.metrics.current_asset_holdings

        if capital <= Decimal('0') or price <= Decimal('0') or not self.market_info:
            self.logger.debug("Insufficient capital, zero price, or no market info. Order size 0.")
            return Decimal('0')
        ...
        if self.config.strategy.inventory.enabled and self.config.max_net_exposure_usd > Decimal('0'):
            async with self.market_data_lock:
                current_mid_price = self.state.mid_price
            if current_mid_price == Decimal('0'):
                self.logger.warning("Mid-price is zero, cannot calculate max net exposure. Skipping exposure check.")
                return Decimal('0')

            max_allowed_pos_qty_abs = self.config.max_net_exposure_usd / current_mid_price

            if side == 'Buy':
                qty_to_reach_max_long = max_allowed_pos_qty_abs - metrics_pos_qty
                if qty_to_reach_max_long <= Decimal('0'):
                    self.logger.debug(f"Cannot place buy order: Current position {metrics_pos_qty} already at or above max long exposure ({max_allowed_pos_qty_abs}).")
                    return Decimal('0')
                target_qty = min(target_qty, qty_to_reach_max_long)
            else:
                if metrics_pos_qty > Decimal('0'):
                    target_qty = min(target_qty, metrics_pos_qty)
                    self.logger.debug(f"Capping sell order quantity at current holdings: {target_qty}")
                else:
                    qty_to_reach_max_short = -max_allowed_pos_qty_abs - metrics_pos_qty
                    if qty_to_reach_max_short >= Decimal('0'):
                        self.logger.debug(f"Cannot place sell order: Current position {metrics_pos_qty} already at or below max short exposure ({-max_allowed_pos_qty_abs}).")
                        return Decimal('0')
                    target_qty = min(target_qty, abs(qty_to_reach_max_short))

        if target_qty <= Decimal('0'):
            self.logger.debug("Calculated target quantity is zero or negative after exposure adjustments. Order size 0.")
            return Decimal('0')
    ...
    async def _place_limit_order(self, side: str, price: Decimal, quantity: Decimal):
        ...
        if qty_f <= Decimal('0') or price_f <= Decimal('0'):
            self.logger.warning(f"Attempted to place order with zero or negative quantity/price: Qty={qty_f}, Price={price_f}. Skipping.")
            return
        ...
        if self.config.category in ['linear', 'inverse']:
            ...
            if (side == 'Sell' and current_position > Decimal('0')) or \
               (side == 'Buy' and current_position < Decimal('0')):
                ...
                self.logger.debug(f"Setting reduceOnly=True for {side} order (current position: {current_position}).")

        if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
            oid = f"DRY_{side}_{int(time.time() * 1000)}"
            self.logger.info(f"{self.config.trading_mode}: Would place {side} order: ID={oid}, Qty={qty_f}, Price={price_f}")
            async with self.active_orders_lock:
                self.state.active_orders[oid] = {
                    'side': side, 'price': price_f, 'qty': qty_f, 'cumExecQty': Decimal('0'),
                    'status': 'New', 'orderLinkId': order_link_id, 'symbol': self.config.symbol,
                    'reduceOnly': params.get('reduceOnly', False)
                }
            await self.db_manager.log_order_event({**params, 'orderId': oid, 'orderStatus': 'New', 'cumExecQty': '0'}, f"{self.config.trading_mode} Order placed")
            return
        ...
        if result and result.get('orderId'):
            ...
            await self.db_manager.log_order_event({**params, 'orderId': oid, 'orderStatus': 'New', 'cumExecQty': '0'}, "Order placed")
        else:
    ...
    async def _reconcile_orders_on_startup(self):
        if self.config.trading_mode in ["DRY_RUN", "SIMULATION"]:
            self.logger.info(f"{self.config.trading_mode} mode: Skipping order reconciliation.")
            return
        ...
        try:
            ...
            self.logger.error(f"Failed to fetch open orders from exchange during reconciliation: {e}. Proceeding with local state only.", exc_info=True)
            exchange_orders = {}
        ...
        async with self.active_orders_lock:
            ...
            for oid in local_ids.intersection(exchange_ids):
                ...
                if local_order['status'] != exchange_order['orderStatus'] or \
                   local_order.get('cumExecQty', Decimal('0')) != Decimal(exchange_order.get('cumExecQty', '0')):
                    self.logger.info(f"Order {oid} status/cumExecQty mismatch. Updating from {local_order['status']}/{local_order.get('cumExecQty', Decimal('0'))} to {exchange_order['orderStatus']}/{exchange_order.get('cumExecQty', '0')}.")
                    local_order['status'] = exchange_order['orderStatus']
                    local_order['cumExecQty'] = Decimal(exchange_order.get('cumExecQty', '0'))

        self.logger.info(f"Order reconciliation complete. {len(self.state.active_orders)} active orders after reconciliation.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\nBot stopped by user.")
    except (ValueError, APIAuthError, MarketInfoError, InitialBalanceError, WebSocketConnectionError,
            ConfigurationError, OrderPlacementError, BybitAPIError, BybitRateLimitError, BybitInsufficientBalanceError) as e:
