...
class BaseConfigModel(BaseModel):
    ...
    model_config = ConfigDict(
        arbitrary_types_allowed=True, json_dumps=json.dumps, json_loads=json.loads
    )
    ...
    def convert_to_decimal_if_needed(cls, v, info):
        ...
        if (
            info.field_name
            and cls.model_fields[info.field_name].annotation == Decimal
...
class OrderManagementConfig(BaseConfigModel):
    ...
    min_order_reprice_interval_ms: int = Field(
        200, ge=50, le=5000
    )  # Minimum interval between reprice events
    max_outstanding_orders_per_side: int = Field(
        50, ge=1, le=100
...
class RiskManagementConfig(BaseConfigModel):
    ...
    max_daily_loss_pct: Decimal = Field(
        Decimal("0.05"), gt=0, lt=1
    )  # E.g., 0.05 for 5% daily loss
...
class StrategyConfig(BaseConfigModel):
    ...
    aggressiveness_multiplier: Decimal = Field(
        Decimal("1.0"), gt=0
    )  # General multiplier for order sizes and spread
    max_spread_percentage: Decimal = Field(Decimal("0.05"), gt=0)  # Cap the spread
    trade_fee_percentage: Decimal = Field(
        Decimal("0.00075"), ge=0, le=0.01
    )  # Default 0.075% for Bybit spot/linear, can be changed
...
class TAConfig(BaseConfigModel):
    ...
    ema_cross_signal_multiplier: Decimal = Field(
        Decimal("0.1"), ge=0
    )  # How much EMA cross impacts fair value
    rsi_overbought_threshold: Decimal = Field(Decimal("70"), ge=50, lt=100)
    rsi_oversold_threshold: Decimal = Field(Decimal("30"), gt=0, le=50)
    rsi_reversal_fair_value_bps: Decimal = Field(
        Decimal("5.0"), ge=0
    )  # Max BPS shift on strong RSI reversal
...
class BotState:
    ...
    recent_prices: deque[tuple[Decimal, float]] = field(
    ...
    last_reprice_mid_price: Decimal | None = None  # Mid price at last reprice event
    last_reprice_timestamp: float = 0.0  # Timestamp of last reprice event
...
class BybitHTTPClient:
    ...
    async def _safe_api_call(self, method, *args, **kwargs):
        ...
        # Define transient errors that should trigger a retry
        transient_errors = (
            10006,  # Rate limit
            10001,  # System error (often temporary)
        )
            ...
            with attempt:
                try:
                    ...
                    if (now - self.last_api_call_time) < self.min_api_call_interval:
                        await asyncio.sleep(
                            self.min_api_call_interval - (now - self.last_api_call_time)
                        )
                    ...
                    if response and response.get("retCode") == 0:
                        ...
                        if code in transient_errors:
                            logger.warning(
                                f"API transient error ({method.__name__}): {msg} (retCode: {code}). Retrying with backoff."
                            )
                            raise Exception(
                                "Transient API error encountered"
                            )  # Re-raise to trigger Tenacity retry
                        elif code == 30042 or (
                            msg and "order does not exist" in str(msg).lower()
                        ):
                            # Specific error for cancelling already filled/non-existent orders, not critical
                            logger.info(
                                f"API Info ({method.__name__}): Order not found/already processed. {msg} (retCode: {code})"
                            )
                            return (
                                {
                                    "list": [
                                        {
                                            "orderId": kwargs.get("request", [{}])[
                                                0
                                            ].get("orderId", "unknown"),
                                            "retCode": code,
                                            "retMsg": msg,
                                ...
                                if "cancel_batch_order" in method.__name__
                                else None
                            )
                        else:
                            logger.error(
                    ...
                    if "Transient API error encountered" in str(e):
                        raise  # Let tenacity handle this retry
                    ...
                    self.consecutive_api_failures += 1
                    await self._check_circuit_breaker()
                    raise  # Re-raise for tenacity to handle final attempts
    ...
    async def get_instrument_info(self) -> InstrumentInfo | None:
        ...
        if result and result.get("list"):
            ...
            instrument_info = InstrumentInfo(
                ...
                quote_coin=info.get("quoteCoin", "USDT"),  # Extract quoteCoin
            )
            logger.info(
                f"Instrument info: tick={instrument_info.tick_size}, step={instrument_info.step_size}, min={instrument_info.min_order_size}, quote_coin={instrument_info.quote_coin}"
            )
            ...
            if failures:
                ...
                for f in failures:
                    if f.get("retCode") == 30042:  # Order does not exist
                        logger.debug(
                            f"Order {f.get('orderId')} already cancelled or filled. {f.get('retMsg')}"
                        )
                    else:
                        logger.warning(
                            f"Failed to cancel order {f.get('orderId')}: {f.get('retMsg')}"
                        )
            return successes
        return []
    ...
    async def get_wallet_balance(self, coin: str) -> Decimal | None:
        ...
        result = await self._safe_api_call(
            self.session.get_wallet_balance, accountType="UNIFIED", coin=coin
        )
        if result and result.get("list"):
        ...
        while True:
            ...
            if not ws.is_connected():
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to reconnect {ws_type} WebSocket: {e}", exc_info=True
                    )
        ...
        async with self._reconnect_lock:
            ...
            if self.public_ws and self.public_ws.is_connected() and not force_reconnect:
                ...
                try:
                    ...
                    self._public_monitor_task = asyncio.create_task(
            ...
            if not self.config.market.dry_run:
                if (
                    ...
                    try:
                        ...
                        self._private_monitor_task = asyncio.create_task(
    ...
    def update_ta(self, prices_with_timestamps: deque[tuple[Decimal, float]]):
        ...
        try:
            ...
            if len(prices) < min_len:
                logger.debug(
                    f"Not enough price data for TA. Need {min_len}, have {len(prices)}"
                )
                return
            ...
            if macd is not None and not macd.empty:
                macd_col = [
                    ...
                    if "MACD_" in c and c.endswith("_" + str(macd_fast))
                ]
                hist_col = [
                    c
                    for c in macd.columns
                    if "MACDH_" in c and c.endswith("_" + str(macd_fast))
                ]
                signal_col = [
                    c
                    for c in macd.columns
                    if "MACDS_" in c and c.endswith("_" + str(macd_fast))
                ]
                if macd_col and hist_col and signal_col:
                    self.ta_state["macd"] = (
                        float(macd[macd_col[0]].iloc[-1])
                        if not pd.isna(macd[macd_col[0]].iloc[-1])
                        else None
                    )
                    self.ta_state["macdh"] = (
                        float(macd[hist_col[0]].iloc[-1])
                        if not pd.isna(macd[hist_col[0]].iloc[-1])
                        else None
                    )
                    self.ta_state["macds"] = (
                        float(macd[signal_col[0]].iloc[-1])
                        if not pd.isna(macd[signal_col[0]].iloc[-1])
                        else None
                    )
...
class OrderManager:
    ...
    def _quotes_significantly_changed(
        ...
        last_reprice_mid_price: Decimal | None,
    ) -> bool:
        ...
        # Check mid-price drift against the mid price at the LAST reprice event
        if (
            self.config.order_management.mid_drift_threshold_ticks > 0
            and last_reprice_mid_price is not None
        ):
            ...
            if not approx_equal(
                current_mid_price, last_reprice_mid_price, price_drift_threshold
            ):
                logger.debug(
                    f"Mid price drifted significantly (last reprice mid: {last_reprice_mid_price:.{self.instrument_info.price_precision}f}, current mid: {current_mid_price:.{self.instrument_info.price_precision}f}), re-evaluating orders."
                )
                return True
    ...
    async def manage_orders(
        self,
        desired_buy_orders: list[dict],
        desired_sell_orders: list[dict],
        current_mid_price: Decimal,
    ):
        """Compares desired orders with current open orders and updates accordingly."""
        now = time.time()

        # Use bot_state for last reprice time
        if (now - self.bot_state.last_reprice_timestamp) < (
            self.config.order_management.min_order_reprice_interval_ms / 1000
        ):
            return  # Too soon for reprice

        new_target_orders = desired_buy_orders + desired_sell_orders
        current_open_orders_list = [
            (o.price, o.qty, o.side) for o in self.current_orders.values()
        ]

        # Check for significant changes, passing the mid-price from the *last* reprice cycle
        if not self._quotes_significantly_changed(
            current_open_orders_list,
            new_target_orders,
            current_mid_price,
            self.bot_state.last_reprice_mid_price,
        ):
        ...
        for target_order in new_target_orders:
            if not any(
                ...
                for co in self.current_orders.values()
            ):
                orders_to_place_data.append(target_order)

        # Apply max outstanding orders per side limit
        current_buy_orders = [
            o for o in self.current_orders.values() if o.side == "Buy"
        ]
        current_sell_orders = [
            o for o in self.current_orders.values() if o.side == "Sell"
        ]
        ...
        temp_buy_count = len(current_buy_orders) - len(
            [
                oid
                for oid in orders_to_cancel_ids
                if self.current_orders.get(oid)
                and self.current_orders[oid].side == "Buy"
            ]
        )
        temp_sell_count = len(current_sell_orders) - len(
            [
                oid
                for oid in orders_to_cancel_ids
                if self.current_orders.get(oid)
                and self.current_orders[oid].side == "Sell"
            ]
        )
        ...
        for order_data in orders_to_place_data:
            if order_data["side"] == "Buy" and temp_buy_count < max_orders:
                ...
                logger.debug(
                    f"Skipping place order {order_data['side']} {order_data['price']} due to max_outstanding_orders_per_side limit."
                )

        if orders_to_cancel_ids or filtered_orders_to_place:
            if self.config.market.dry_run:
                self._execute_virtual_orders(
                    orders_to_cancel_ids, filtered_orders_to_place
                )
            else:
            ...
            # Update last reprice time and mid price AFTER execution attempt
            self.bot_state.last_reprice_timestamp = now
            self.bot_state.last_reprice_mid_price = (
                current_mid_price  # Store the mid-price that triggered this reprice
            )

    def _execute_virtual_orders(
        self, orders_to_cancel_ids: list[str], orders_to_place_data: list[dict]
    ):
        """Simulates order placement and cancellation for dry run."""
        if orders_to_cancel_ids or orders_to_place_data:
            logger.info(
                f"[DRY RUN] Orders: Cancelling {len(orders_to_cancel_ids)}, Placing {len(orders_to_place_data)}"
            )
    ...
    async def _execute_live_orders(
        ...
        if orders_to_cancel_ids or orders_to_place_data:
            logger.info(
                f"Executing: Cancelling {len(orders_to_cancel_ids)} orders, Placing {len(orders_to_place_data)} orders."
            )
...
class PositionManager:
    ...
    async def initialize_balance(self):
        ...
        if self.config.market.dry_run:
            ...
            logger.info(
                f"[DRY RUN] Initial virtual balance set to {self.initial_balance}"
            )
            return

        # Use the quote_coin from instrument_info if available, otherwise default to USDT
        quote_coin = (
            self.bot_state.instrument_info.quote_coin
            if self.bot_state.instrument_info
            else "USDT"
        )

        balance = await self.http_client.get_wallet_balance(coin=quote_coin)

        if balance is not None:
            self.initial_balance = balance
            self.bot_state.daily_pnl_start_balance = balance
            logger.info(
                f"Initial wallet balance for PnL tracking ({quote_coin}): {self.initial_balance:.4f}"
            )
        else:
            logger.error(
                f"Could not fetch initial wallet balance for {quote_coin}. Daily PnL tracking will be limited."
            )
            self.initial_balance = Decimal(
                "0"
            )  # Set to 0 to prevent DivisionByZero later
    ...
    async def update_daily_pnl_snapshot(self):
        ...
        if now.date() > self.bot_state.daily_pnl_snapshot_time.date():
            ...
            current_balance = await self.http_client.get_wallet_balance(coin=quote_coin)
            if current_balance is not None:
                self.bot_state.daily_pnl_start_balance = current_balance
                self.bot_state.daily_pnl_snapshot_time = now
                logger.info(
                    f"Daily PnL snapshot updated. New start balance: {current_balance:.4f}"
                )
            else:
                logger.warning(
                    "Failed to get current balance for daily PnL snapshot update."
                )
    ...
    def process_virtual_fill(
        ...
        """Simulates fills for dry-run mode, including fees."""
        if qty <= 0:
            return

        trade_fee = qty * price * self.config.strategy.trade_fee_percentage
        ...
        if self.size == 0:
        ...
        elif (self.size > 0 and side == "Buy") or (
            self.size < 0 and side == "Sell"
        ):  # Increasing position
            ...
            if remaining_qty >= 0:  # Position flips or closes exactly
                self.realized_pnl += (
                    (price - self.avg_entry_price) * self.size
                    if self.size > 0
                    else (self.avg_entry_price - price) * abs(self.size)
                )
                new_position_size = direction_factor * remaining_qty
            else:  # Position reduces without flipping
                pnl_realized_part = (
                    (price - self.avg_entry_price) * qty
                    if self.size > 0
                    else (self.avg_entry_price - price) * qty
                )
        ...
        if new_position_size == 0:
            ...
            old_size * new_position_size <= 0
        ):  # Direction flipped or new position from zero
            self.avg_entry_price = price
        else:  # Same direction, update average
            try:
                self.avg_entry_price = (abs(current_value) + qty * price) / abs(
                    new_position_size
                )
            except (InvalidOperation, DivisionByZero):
        ...
        self.realized_pnl -= trade_fee  # Subtract fee from PnL
        logger.info(
            f"[DRY RUN] Fill: {side} {qty} @ {price}. Position: {old_size:.4f} -> {self.size:.4f}, Realized PnL: {self.realized_pnl:.4f} (Fee: {trade_fee:.4f})"
        )
    ...
    async def check_and_manage_risk(self, current_mid_price: Decimal | None) -> bool:
        ...
        if self.initial_balance is not None and self.initial_balance > 0:
            ...
            estimated_current_equity = (
                self.bot_state.daily_pnl_start_balance
                + self.realized_pnl
                + self.unrealized_pnl
            )
            ...
            if self.bot_state.daily_pnl_start_balance > 0:
                daily_loss_pct = (
                    self.bot_state.daily_pnl_start_balance - estimated_current_equity
                ) / self.bot_state.daily_pnl_start_balance
                if daily_loss_pct >= self.config.risk_management.max_daily_loss_pct:
                    logger.critical(
                        f"[bold red]DAILY LOSS LIMIT REACHED! ({daily_loss_pct:.2%})[/bold red]"
                    )
...
class EnhancedBybitMarketMaker:
    def __init__(self, config: BotConfig):
        ...
        if (
            ...
            raise ValueError(
                "API keys are missing or empty. Ensure BYBIT_API_KEY and BYBIT_API_SECRET are set in your .env file or config.json."
            )
    ...
    def _calculate_tiered_quotes(self) -> tuple[list[dict], list[dict]]:
        ...
        if best_bid is None or best_ask is None or best_bid >= best_ask:
            logger.debug(
                f"Market crossed or no best bid/ask: bid={best_bid}, ask={best_ask}. Skipping quotes."
            )
            return [], []
        ...
        vol_data_points = [
            (p, t)
            for p, t in self.bot_state.recent_prices
            if current_time - t <= self.config.technical.vol_window_seconds
        ]

        if (
            len(vol_data_points) >= self.config.technical.vol_window_seconds // 5
        ):  # Require a reasonable number of points
            try:
        ...
        total_spread_pct = clamp_decimal(
            total_spread_pct, Decimal("0"), self.config.strategy.max_spread_percentage
        )
        ...
        if self.ta_manager.enabled:
            ...
            if rsi is not None:
                ...
                if rsi >= float(self.config.ta.rsi_overbought_threshold):
                    signal = (
                        Decimal("-1")
                        * (to_decimal(rsi) - self.config.ta.rsi_overbought_threshold)
                        / (Decimal("100") - self.config.ta.rsi_overbought_threshold)
                    )
                elif rsi <= float(self.config.ta.rsi_oversold_threshold):
                    signal = (
                        Decimal("1")
                        * (self.config.ta.rsi_oversold_threshold - to_decimal(rsi))
                        / self.config.ta.rsi_oversold_threshold
                    )
                else:
                ...
                if (
                    rsi >= float(self.config.ta.rsi_overbought_threshold)
                    and self.bot_state.last_mid_price is not None
                    and self.bot_state.last_mid_price > mid_price
                ) or (
                    rsi <= float(self.config.ta.rsi_oversold_threshold)
                    and self.bot_state.last_mid_price is not None
                    and self.bot_state.last_mid_price < mid_price
                ):
                    fair_value = fair_value * (
                        Decimal("1")
                        + signal
                        * self.config.ta.rsi_reversal_fair_value_bps
                        / Decimal("10000")
                    )
            ...
            if ema_f is not None and ema_s is not None:
                if (
                    ...
                    fair_value *= (
                        Decimal("1") + self.config.ta.ema_cross_signal_multiplier
                    )
                elif (
                    ema_f < ema_s
                    and self.bot_state.last_mid_price is not None
        ...
        # Order params
        base_size = (
            self.config.order_management.base_order_size
            * self.config.strategy.aggressiveness_multiplier
        )
        ...
        if self.ta_manager.enabled:
            ...
            if (
                rsi is not None
                and rsi > float(self.config.ta.rsi_oversold_threshold) * 1.2
            ):  # RSI showing strength (less strict than OB)
                is_bullish_trend = True
            if macd_hist is not None and macd_hist > 0:  # MACD Histogram positive
                is_bullish_trend = True
            ...
            if (
                rsi is not None
                and rsi < float(self.config.ta.rsi_overbought_threshold) * 0.8
            ):  # RSI showing weakness (less strict than OS)
                is_bearish_trend = True
            if macd_hist is not None and macd_hist < 0:  # MACD Histogram negative
                is_bearish_trend = True

            # Apply bias based on combined trend signals
            if is_bullish_trend and not is_bearish_trend:
                # Bullish bias: increase buy qty, slightly decrease sell qty
                buy_bias_factor = Decimal("1") + qty_bias_max
                sell_bias_factor = Decimal("1") - qty_bias_max * Decimal("0.5")
            elif is_bearish_trend and not is_bullish_trend:
                # Bearish bias: increase sell qty, slightly decrease buy qty
                sell_bias_factor = Decimal("1") + qty_bias_max
                buy_bias_factor = Decimal("1") - qty_bias_max * Decimal("0.5")
            # If both (choppy) or neither, factors remain 1 (no bias)
    ...
    def generate_status_table(self) -> Table:
        ...
        table = Table(
            title=title, style="cyan", title_justify="left", header_style="bold green"
        )
        ...
        if self.bot_state.last_known_best_bid and self.bot_state.last_known_best_ask:
            ...
            table.add_row(
                "Best Bid/Ask", f"{best_bid:.{price_prec}f} / {best_ask:.{price_prec}f}"
            )
            table.add_row(
                "Spread", f"{spread:.{price_prec}f} ([bold]{spread_bps:.1f}[/bold] bps)"
            )
        else:
        ...
        position_size_str = (
            f"[{pos_color}]{self.position_manager.size:.{qty_prec}f}[/{pos_color}]"
            if self.position_manager
            else "N/A"
        )
        avg_entry_str = (
            f"{self.position_manager.avg_entry_price:.{price_prec}f}"
            if self.position_manager
            and self.position_manager.avg_entry_price != Decimal("0")
            else "N/A"
        )
        ...
        if self.position_manager:
            ...
            if (
                self.position_manager.initial_balance is not None
                and self.position_manager.initial_balance > 0
            ):
                daily_pnl = (
                ...
                daily_pnl_pct = (
                    daily_pnl / self.bot_state.daily_pnl_start_balance
                ) * 100
                pnl_color = "green" if daily_pnl >= 0 else "red"
                table.add_row(
                    "Daily PnL",
                    f"[{pnl_color}]{daily_pnl:.4f} ({daily_pnl_pct:.2f}%)[/{pnl_color}]",
                )
        else:
        ...
        if self.ta_manager.enabled:
            ...
            macd_signal = (
                "Bullish"
                if (isinstance(macdh, float) and macdh > 0)
                else "Bearish"
                if (isinstance(macdh, float) and macdh < 0)
                else "Neutral"
            )
        ...
        if len(recent_prices_only) >= 30:  # At least 30 seconds of data for volatility
            try:
                ...
                if len(set(prices_f)) > 1:
                    ...
                    table.add_row(
                        f"Volatility ({self.config.technical.vol_window_seconds}s)",
                        f"{vol:.4%}",
                    )
                else:
                    table.add_row(
                ...
                table.add_row(
                    f"Volatility ({self.config.technical.vol_window_seconds}s)", "N/A"
                )
        else:
            table.add_row(
                f"Volatility ({self.config.technical.vol_window_seconds}s)",
    ...
    async def _setup_initial_state(self):
        ...
        # Initialize last_reprice_mid_price and timestamp
        self.bot_state.last_reprice_mid_price = (
            self.bot_state.last_mid_price
        )  # Will be updated by WS later
        self.bot_state.last_reprice_timestamp = time.time()

        self.order_manager = OrderManager(
            self.config,
            self.http_client,
            self.bot_state.instrument_info,
            self.bot_state,
        )
        self.position_manager = PositionManager(
            self.config, self.http_client, self.order_manager, self.bot_state
        )
    ...
    async def run(self):
        ...
        try:
            ...
            ) and (time.time() - timeout_start < 30):
        ...
        with Live(
            ...
            while True:
                try:
                    ...
                    if self.position_manager:
                        try:
                            if await self.position_manager.check_and_manage_risk(
                                self.bot_state.last_mid_price
                            ):
                                logger.info(
                            ...
                            logger.critical(
                                f"Critical error from risk manager: {e}", exc_info=True
                            )
                            # If daily loss is hit, propagate shutdown
                            raise asyncio.CancelledError(
                                "Bot shutdown due to critical risk event."
                            )
                    ...
                    if (
                        ...
                        logger.debug(
                            "No valid mid price/orderbook or managers not initialized, skipping quote calculation."
                        )
                        await asyncio.sleep(0.5)
                        continue
                    ...
                    now = time.time()
                    if (
                        now - self.bot_state.last_reprice_timestamp
                    ...
                    await self.order_manager.manage_orders(
                        buy_orders, sell_orders, self.bot_state.last_mid_price
                    )
                    ...
                    logger.info(
                        "Main loop cancelled (external request or critical event)."
                    )
                    break
                except Exception as e:
...
async def main():
    ...
    try:
        ...
        raw_config["credentials"]["api_key"] = raw_config.get("credentials", {}).get(
            "api_key", ""
        )
        raw_config["credentials"]["api_secret"] = raw_config.get("credentials", {}).get(
            "api_secret", ""
        )

        config = BotConfig(**raw_config)
        ContextFilter.set_symbol(config.market.symbol)
    ...
    def _signal_handler():
        logger.info(
            "Shutdown signal received (SIGINT/SIGTERM). Requesting bot shutdown."
        )
        main_task.cancel()
    ...
    for signame in ("SIGINT", "SIGTERM"):
        if hasattr(signal, signame):
            try:
                ...
                logger.warning(
                    f"Signal handler for {signame} not available on this platform."
                )
    ...
    try:
        ...
        logger.critical(
            f"Fatal unhandled error caught by main entry point: {e}", exc_info=True
        )
    finally:
        await bot.shutdown()
...
if __name__ == "__main__":
    try:
        ...
        logger.info(
            "Application interrupted by user (KeyboardInterrupt from main thread)."
        )
    except Exception as e:
