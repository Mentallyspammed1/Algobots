...
class StrategyConfig(BaseConfigModel):
    ...
    trade_fee_percentage: Decimal = Field(Decimal("0.00075"), ge=0, le=0.01) # Default 0.075% for Bybit spot/linear, can be changed
...
class BybitHTTPClient:
                ...
                try:
                    ...
                    if response and response.get('retCode') == 0:
                        ...
                        if code in transient_errors:
                            logger.warning(f"API transient error ({method.__name__}): {msg} (retCode: {code}). Retrying with backoff.")
                            raise Exception("Transient API error encountered") # Re-raise to trigger Tenacity retry
    ...
    async def get_instrument_info(self) -> InstrumentInfo | None:
        ...
        if result and result.get('list'):
            ...
            logger.info(f"Instrument info: tick={instrument_info.tick_size}, step={instrument_info.step_size}, min={instrument_info.min_order_size}, quote_coin={instrument_info.quote_coin}")
...
# =========================
class OrderManager:
    def __init__(self, bot_config: BotConfig, http_client: BybitHTTPClient, instrument_info: InstrumentInfo, bot_state: BotState):
    ...
    def _quotes_significantly_changed(self, current_quotes: list[tuple[Decimal, Decimal, str]], new_quotes: list[dict], current_mid_price: Decimal, last_reprice_mid_price: Decimal | None) -> bool:
        ...
        if self.config.order_management.mid_drift_threshold_ticks > 0 and last_reprice_mid_price is not None:
            price_drift_threshold = self.instrument_info.tick_size * self.config.order_management.mid_drift_threshold_ticks
            if not approx_equal(current_mid_price, last_reprice_mid_price, price_drift_threshold):
                logger.debug(f"Mid price drifted significantly (last reprice mid: {last_reprice_mid_price:.{self.instrument_info.price_precision}f}, current mid: {current_mid_price:.{self.instrument_info.price_precision}f}), re-evaluating orders.")
                return True
    ...
    async def manage_orders(self, desired_buy_orders: list[dict], desired_sell_orders: list[dict], current_mid_price: Decimal):
        """Compares desired orders with current open orders and updates accordingly."""
        now = time.time()

        # Use bot_state for last reprice time
        if (now - self.bot_state.last_reprice_timestamp) < (self.config.order_management.min_order_reprice_interval_ms / 1000):
        ...
        if not self._quotes_significantly_changed(current_open_orders_list, new_target_orders, current_mid_price, self.bot_state.last_reprice_mid_price):
        ...
        if orders_to_cancel_ids or filtered_orders_to_place:
            ...
            self.bot_state.last_reprice_mid_price = current_mid_price # Store the mid-price that triggered this reprice
...
class PositionManager:
    ...
    async def initialize_balance(self):
        ...
        quote_coin = self.bot_state.instrument_info.quote_coin if self.bot_state.instrument_info else "USDT"
        ...
        if balance is not None:
            ...
            logger.info(f"Initial wallet balance for PnL tracking ({quote_coin}): {self.initial_balance:.4f}")
        else:
            logger.error(f"Could not fetch initial wallet balance for {quote_coin}. Daily PnL tracking will be limited.")
            self.initial_balance = Decimal("0") # Set to 0 to prevent DivisionByZero later
    ...
    def process_virtual_fill(self, side: Literal["Buy", "Sell"], price: Decimal, qty: Decimal):
        ...
        logger.info(f"[DRY RUN] Fill: {side} {qty} @ {price}. Position: {old_size:.4f} -> {self.size:.4f}, Realized PnL: {self.realized_pnl:.4f} (Fee: {trade_fee:.4f})")
...
class EnhancedBybitMarketMaker:
    ...
    def _calculate_tiered_quotes(self) -> tuple[list[dict], list[dict]]:
        ...
        if self.ta_manager.enabled:
            ...
            if rsi is not None:
                ...
                if (rsi >= float(self.config.ta.rsi_overbought_threshold) and self.bot_state.last_mid_price is not None and self.bot_state.last_mid_price > mid_price) or \
                   (rsi <= float(self.config.ta.rsi_oversold_threshold) and self.bot_state.last_mid_price is not None and self.bot_state.last_mid_price < mid_price):
            ...
            if ema_f is not None and ema_s is not None:
                if ema_f > ema_s and self.bot_state.last_mid_price is not None and self.bot_state.last_mid_price < mid_price: # Bullish cross
                    fair_value *= (Decimal("1") + self.config.ta.ema_cross_signal_multiplier)
                elif ema_f < ema_s and self.bot_state.last_mid_price is not None and self.bot_state.last_mid_price > mid_price: # Bearish cross
                    fair_value *= (Decimal("1") - self.config.ta.ema_cross_signal_multiplier)
        ...
        if self.ta_manager.enabled:
            ...
            if ema_f is not None and ema_s is not None and ema_f > ema_s: # EMA Cross Up
                is_bullish_trend = True
            if rsi is not None and rsi > float(self.config.ta.rsi_oversold_threshold) * 1.2: # RSI showing strength (less strict than OB)
                is_bullish_trend = True
            if macd_hist is not None and macd_hist > 0: # MACD Histogram positive
                is_bullish_trend = True

            # Check for bearish indicators
            if ema_f is not None and ema_s is not None and ema_f < ema_s: # EMA Cross Down
                is_bearish_trend = True
            if rsi is not None and rsi < float(self.config.ta.rsi_overbought_threshold) * 0.8: # RSI showing weakness (less strict than OS)
                is_bearish_trend = True
    ...
    async def _setup_initial_state(self):
        ...
        self.bot_state.last_reprice_mid_price = self.bot_state.last_mid_price # Will be updated by WS later
        self.bot_state.last_reprice_timestamp = time.time()

        self.order_manager = OrderManager(self.config, self.http_client, self.bot_state.instrument_info, self.bot_state)
        ...
        with Live(self.generate_status_table(), screen=True, refresh_per_second=2, console=console) as live:
            while True:
                try:
                    ...
                    now = time.time()
                    if (now - self.bot_state.last_reprice_timestamp) < self.config.order_management.order_reprice_delay_seconds:
                        await asyncio.sleep(0.1)
                        continue
...
async def main():
    ...
    try:
        ...
        raw_config['credentials']['api_key'] = raw_config.get('credentials', {}).get('api_key', '')
        raw_config['credentials']['api_secret'] = raw_config.get('credentials', {}).get('api_secret', '')
