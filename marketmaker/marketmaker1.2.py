...
@dataclass
class TradeMetrics:
    total_trades: int = 0; gross_profit: Decimal = Decimal('0'); gross_loss: Decimal = Decimal('0')
    total_fees: Decimal = Decimal('0'); wins: int = 0; losses: int = 0; win_rate: float = 0.0
    @property
    def total_pnl(self) -> Decimal: return self.gross_profit - self.gross_loss - self.total_fees
    def update_win_rate(self): self.win_rate = (self.wins / self.total_trades * 100.0) if self.total_trades > 0 else 0.0

@dataclass
class InventoryStrategyConfig:
    enabled: bool = True; skew_intensity: Decimal = Decimal('0.5')

@dataclass
class DynamicSpreadConfig:
    enabled: bool = True; volatility_window_sec: int = 60; volatility_multiplier: Decimal = Decimal('2.0')
    min_spread_pct: Decimal = Decimal('0.0005'); max_spread_pct: Decimal = Decimal('0.01')

@dataclass
class CircuitBreakerConfig:
    enabled: bool = True; pause_threshold_pct: Decimal = Decimal('0.02')
    check_interval_sec: int = 10; pause_duration_sec: int = 60

@dataclass
class StrategyConfig:
    base_spread_pct: Decimal = Decimal('0.001'); base_order_size_pct_of_balance: Decimal = Decimal('0.005')
    order_stale_threshold_pct: Decimal = Decimal('0.0005'); min_profit_spread_after_fees_pct: Decimal = Decimal('0.0002')
...
@dataclass
class SystemConfig:
    loop_interval_sec: int = 1; order_refresh_interval_sec: int = 5; ws_heartbeat_sec: int = 30
    cancellation_rate_limit_sec: float = 0.2; status_report_interval_sec: int = 30
    ws_reconnect_attempts: int = 5; ws_reconnect_initial_delay_sec: int = 5
    api_retry_attempts: int = 3; api_retry_delay_sec: float = 1.0 # Fixed delay for simplicity, can be exponential

@dataclass
class FilesConfig:
    log_level: str = "INFO"; log_file: str = "market_maker.log"
    state_file: str = "market_maker_state.pkl"; db_file: str = "market_maker.db"

@dataclass
class Config:
    api_key: str = field(default_factory=lambda: os.getenv("BYBIT_API_KEY", "")); api_secret: str = field(default_factory=lambda: os.getenv("BYBIT_API_SECRET", ""))
    testnet: bool = os.getenv("BYBIT_TESTNET", "true").lower() == "true"; trading_mode: str = "DRY_RUN" # DRY_RUN, TESTNET, LIVE
    symbol: str = "XLMUSDT"; category: str = "linear"; leverage: Decimal = Decimal('1')
    min_order_value_usd: Decimal = Decimal('10'); max_order_size_pct: Decimal = Decimal('0.1'); max_net_exposure_usd: Decimal = Decimal('500')
    order_type: str = "Limit"; time_in_force: str = "GTC"; post_only: bool = True
    strategy: StrategyConfig = field(default_factory=StrategyConfig)
    system: SystemConfig = field(default_factory=SystemConfig)
    files: FilesConfig = field(default_factory=FilesConfig)
    metrics: TradeMetrics = field(default_factory=TradeMetrics)
    base_currency: str = field(init=False); quote_currency: str = field(init=False)

    def __post_init__(self):
        if self.symbol.endswith("USDT"): self.base_currency = self.symbol[:-4]; self.quote_currency = "USDT"
        elif self.symbol.endswith("USD"): self.base_currency = self.symbol[:-3]; self.quote_currency = "USD"
        else: self.base_currency = self.symbol[:3]; self.quote_currency = self.symbol[3:]

        # Harmonize testnet setting based on trading_mode
        if self.trading_mode == "TESTNET": self.testnet = True
        elif self.trading_mode == "LIVE": self.testnet = False
...
@dataclass
class MarketInfo:
    symbol: str; price_precision: Decimal; quantity_precision: Decimal; min_order_qty: Decimal; min_notional_value: Decimal
    def format_price(self, p: Decimal) -> Decimal: return p.quantize(self.price_precision, rounding=ROUND_DOWN)
    def format_quantity(self, q: Decimal) -> Decimal: return q.quantize(self.quantity_precision, rounding=ROUND_DOWN)

# =====================================================================
# LOGGING, STATE, and DB
# =====================================================================
def setup_logger(config: FilesConfig) -> logging.Logger:
    logger = logging.getLogger('MarketMakerBot'); logger.setLevel(getattr(logging, config.log_level.upper()))
    if not logger.handlers: # Prevent adding handlers multiple times if setup_logger is called more than once
        ...
        # File handler
        fh = logging.FileHandler(config.log_file); fh.setFormatter(formatter)
        logger.addHandler(fh)
    return logger

class StateManager:
    def __init__(self, file_path: str, logger: logging.Logger): self.file_path = file_path; self.logger = logger
    async def save_state(self, state: dict):
        try:
            async with aiofiles.open(self.file_path, 'wb') as f: await f.write(pickle.dumps(state))
            self.logger.info("Bot state saved successfully.")
        except Exception as e: self.logger.error(f"Error saving state: {e}")
    async def load_state(self) -> dict | None:
        if not os.path.exists(self.file_path): return None
        try:
            async with aiofiles.open(self.file_path, 'rb') as f: return pickle.loads(await f.read())
        except Exception as e: self.logger.error(f"Error loading state: {e}. Starting fresh."); return None

class DBManager:
    def __init__(self, db_file: str, logger: logging.Logger): self.db_file = db_file; self.conn: aiosqlite.Connection | None = None; self.logger = logger
    async def connect(self):
        try: self.conn = await aiosqlite.connect(self.db_file); self.conn.row_factory = aiosqlite.Row; self.logger.info(f"Connected to database: {self.db_file}")
        except Exception as e: self.logger.critical(f"Failed to connect to database: {e}"); sys.exit(1)
    async def close(self):
        if self.conn: await self.conn.close(); self.logger.info("Database connection closed.")
    async def create_tables(self):
        ...
        """); await self.conn.commit(); self.logger.info("Database tables checked/created.")
    async def log_order_event(self, o: dict, m: str | None = None):
        ...
        except Exception as e: self.logger.error(f"Error logging order event to DB: {e}")
    async def log_trade_fill(self, t: dict):
        ...
        try:
            ...
            await self.conn.commit()
        except Exception as e: self.logger.error(f"Error logging trade fill to DB: {e}")
    async def log_balance_update(self, c: str, wb: Decimal, ab: Decimal | None = None):
        ...
        try:
            ...
            await self.conn.commit()
        except Exception as e: self.logger.error(f"Error logging balance update to DB: {e}")
    async def log_bot_metrics(self, m: TradeMetrics):
        ...
        try:
            ...
            await self.conn.commit()
        except Exception as e: self.logger.error(f"Error logging bot metrics to DB: {e}")
...
class TradingClient:
    ...
    api_retry = retry(
        stop=stop_after_attempt(Config.system.api_retry_attempts),
        wait=wait_fixed(config.system.api_retry_delay_sec),
        retry=retry_if_exception_type(Exception), # Catch all for simplicity, can be refined for specific API errors
    ...
    def _handle_response(self, response: Any, action: str):
        if not isinstance(response, dict):
            self.logger.error(f"API {action} failed: Invalid response format. Response: {response}"); return None
        ret_code = response.get('retCode', -1)
        if ret_code == 0:
            self.logger.debug(f"API {action} successful."); return response.get('result', {})
        ret_msg = response.get('retMsg', 'Unknown error'); self.logger.error(f"API {action} failed: {ret_msg} (ErrCode: {ret_code}).")
        if ret_code == 10004:
    ...
    @api_retry
    async def get_instruments_info(self) -> dict | None:
    ...
    @api_retry
    async def cancel_order(self, order_id: str, order_link_id: str | None = None) -> bool:
        ...
        response = self.http_session.cancel_order(**params); self.last_cancel_time = time.time()
...
class BybitMarketMaker:
    def __init__(self, config: Config):
        self.config = config; self.logger = setup_logger(config.files)
        ...
        self.mid_price = Decimal('0'); self.current_balance = Decimal('0')
        self.available_balance = Decimal('0'); self.current_position_qty = Decimal('0')
        self.unrealized_pnl = Decimal('0') # For linear contracts
        self.active_orders: dict[str, dict] = {}
        self.is_running = False; self.last_order_management_time = 0
        self.last_ws_message_time = time.time(); self.last_status_report_time = 0
        self.market_data_lock = asyncio.Lock(); self.active_orders_lock = asyncio.Lock()
        ...
        self.is_paused = False; self.pause_end_time = 0
    ...
    async def _initialize_bot(self):
        ...
        await self.db_manager.connect(); await self.db_manager.create_tables()
    ...
    async def _main_loop_tick(self):
        ...
        if self.is_paused and current_time < self.pause_end_time: return
            ...
            self.logger.info("Circuit breaker cooldown finished. Resuming trading."); self.is_paused = False
    ...
    async def stop(self):
        ...
        self.is_running = False; self.logger.info("Initiating graceful shutdown...")
        if self.config.trading_mode != "DRY_RUN": await self._cancel_all_orders()
    ...
    def _handle_orderbook_update(self, message: dict):
        ...
        try:
            if 'data' in message and message['topic'].startswith("orderbook"):
                data = message['data']
                if data.get('b') and data.get('a'): self._schedule_coro(self._update_mid_price(data['b'], data['a']))
        except Exception as e: self.logger.error(f"Error handling orderbook update: {e}")
    ...
    def _handle_order_update(self, message: dict):
        ...
        try:
            if 'data' in message:
                for order_data in message['data']: self._schedule_coro(self._process_order_update(order_data))
        except Exception as e: self.logger.error(f"Error handling order update: {e}")
    ...
    async def _process_order_update(self, order_data: dict):
        ...
        async with self.active_orders_lock:
            if order_id in self.active_orders:
                ...
                if status == 'Filled': await self._process_fill(order_data); del self.active_orders[order_id]
                elif status in ['Cancelled', 'Rejected']: del self.active_orders[order_id]
        ...
        try:
            if 'data' in message:
                for pos_data in message['data']:
                    if pos_data['symbol'] == self.config.symbol: self._schedule_coro(self._process_position_update(pos_data))
        except Exception as e: self.logger.error(f"Error handling position update: {e}")
    ...
    async def _check_circuit_breaker(self) -> bool:
        ...
        if len(self.circuit_breaker_history) > 1:
            ...
            if price_change_pct > float(cb_config.pause_threshold_pct):
                ...
                self.is_paused = True; self.pause_end_time = time.time() + cb_config.pause_duration_sec
                await self._cancel_all_orders(); return True
        return False
    ...
    def _calculate_dynamic_spread(self) -> Decimal:
        ...
        if len(self.mid_price_history) < 2 or np.all(np.array(self.mid_price_history) == self.mid_price_history[0]):
            volatility = 0.0
        else:
            volatility = np.std(self.mid_price_history) / np.mean(self.mid_price_history)
        ...
        self.logger.debug(f"Volatility: {volatility:.6f}, Dynamic Spread: {clamped_spread:.4%}"); return clamped_spread
    ...
    async def _reconcile_and_place_orders(self, target_bid: Decimal, target_ask: Decimal):
        ...
        target_bid = self.market_info.format_price(target_bid); target_ask = self.market_info.format_price(target_ask)
    ...
    async def _fetch_market_info(self) -> bool:
        ...
        self.market_info = MarketInfo(symbol=self.config.symbol, price_precision=Decimal(info['priceFilter']['tickSize']), quantity_precision=Decimal(info['lotSizeFilter']['qtyStep']), min_order_qty=Decimal(info['lotSizeFilter']['minOrderQty']), min_notional_value=Decimal(info['lotSizeFilter'].get('minNotionalValue', '0')))
        self.logger.info(f"Market info fetched for {self.config.symbol}: {self.market_info}"); return True
    ...
    async def _calculate_order_size(self, side: str, price: Decimal) -> Decimal:
        ...
        self.logger.debug(f"Calculated {side} order size: {qty} {self.config.base_currency} (Notional: {order_notional_value:.2f} {self.config.quote_currency})"); return qty

    async def _place_limit_order(self, side: str, price: Decimal, quantity: Decimal):
        if self.config.trading_mode == "DRY_RUN":
            oid = f"DRY_{side}_{int(time.time() * 1000)}"; self.logger.info(f"DRY_RUN: Would place {side} order: ID={oid}, Qty={quantity}, Price={price}")
            async with self.active_orders_lock: self.active_orders[oid] = {'side': side, 'price': price, 'qty': quantity, 'status': 'New', 'orderLinkId': f"mm_{side}_{int(time.time() * 1000)}"}
            return
