...
class DBManager:
    ...
    async def log_trade_fill(self, t: dict):
        ...
        try:
            await self.conn.execute(
                "INSERT INTO trade_fills (timestamp, order_id, trade_id, symbol, side, exec_price, exec_qty, fee, fee_currency, pnl) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    datetime.now().isoformat(),
                    t.get("orderId"),
                    t.get("execId"),
    ...
    async def log_balance_update(self, c: str, wb: Decimal, ab: Decimal | None = None):
        ...
        try:
            await self.conn.execute(
                ...
                (datetime.now().isoformat(), c, str(wb), str(ab) if ab else None),
            )
            await self.conn.commit()
        except Exception as e:
    ...
    async def log_bot_metrics(self, m: TradeMetrics):
        ...
        try:
            await self.conn.execute(
                "INSERT INTO bot_metrics (timestamp, total_trades, total_pnl, gross_profit, gross_loss, total_fees, wins, losses, win_rate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    datetime.now().isoformat(),
                    m.total_trades,
                    str(m.total_pnl),
...
class TradingClient:
    ...
    api_retry = retry(
        ...
        retry=retry_if_exception_type(
            Exception
        ),  # Catch all for simplicity, can be refined for specific API errors
        before_sleep=before_sleep_log(
            logging.getLogger("MarketMakerBot"), logging.WARNING
        ),
    ...
    async def get_position_info(self) -> dict | None:
        ...
        response = self.http_session.get_positions(
            category=self.config.category, settleCoin=self.config.quote_currency
        )
    ...
    async def cancel_all_orders(self) -> bool:
        ...
        if self.config.category in ["linear", "inverse"]:
            params["settleCoin"] = self.config.quote_currency
        return (
...
class BybitMarketMaker:
    def __init__(self, config: Config):
        ...
        self.mid_price_history = deque(
            maxlen=self.config.strategy.dynamic_spread.volatility_window_sec
        )
        self.circuit_breaker_history = deque(
            maxlen=self.config.strategy.circuit_breaker.check_interval_sec
        )
    ...
    def _validate_config(self):
        ...
        if self.config.trading_mode != "DRY_RUN" and (
            not self.config.api_key or not self.config.api_secret
        ):
            raise ValueError(
                "API_KEY and API_SECRET must be set for non-DRY_RUN modes."
            )
    ...
    async def _reconnect_websockets(self):
        ...
        delay = self.config.system.ws_reconnect_initial_delay_sec * (
            self.config.system.ws_reconnect_attempts - self.ws_reconnect_attempts_left
        )
        self.logger.warning(
            f"Attempting WebSocket reconnection in {delay} seconds... ({self.ws_reconnect_attempts_left} attempts left)"
        )
    ...
    async def stop(self):
        ...
        try:
            await self.state_manager.save_state(
                {"active_orders": self.active_orders, "metrics": self.config.metrics}
            )
        except Exception as e:
    ...
    async def _process_position_update(self, pos_data: dict):
        async with self.balance_position_lock:
            ...
            if self.config.category == "linear" and "unrealisedPnl" in pos_data:
                ...
                self.logger.debug(
                    f"UNREALIZED PNL (WS): {self.unrealized_pnl} {self.config.quote_currency}"
                )
    ...
    async def _check_circuit_breaker(self) -> bool:
        ...
        if not cb_config.enabled:
            return False
        if len(self.circuit_breaker_history) < cb_config.check_interval_sec:
        ...
        if len(self.circuit_breaker_history) > 1:
            ...
            if price_change_pct > float(cb_config.pause_threshold_pct):
                self.logger.warning(
                    f"CIRCUIT BREAKER TRIPPED: Price changed {price_change_pct:.2%} in {cb_config.check_interval_sec}s. Pausing trading for {cb_config.pause_duration_sec}s."
                )
                self.is_paused = True
    ...
    def _calculate_dynamic_spread(self) -> Decimal:
        ...
        if (
            not ds_config.enabled
            or len(self.mid_price_history) < ds_config.volatility_window_sec
        ):
            return self.config.strategy.base_spread_pct

        # Avoid calculating std on constant prices or single entry
        if len(self.mid_price_history) < 2 or np.all(
            np.array(self.mid_price_history) == self.mid_price_history[0]
        ):
            volatility = 0.0
        else:
            volatility = np.std(self.mid_price_history) / np.mean(
    ...
    def _enforce_min_profit_spread(
        ...
        if not self.market_info:
            return bid_p, ask_p
        min_spread_val = (
            mid_price * self.config.strategy.min_profit_spread_after_fees_pct
        )
        ...
        if (ask_p - bid_p) < min_spread_val:
            self.logger.debug(
                f"Adjusting spread to enforce min profit. Original: {ask_p - bid_p:.6f}, Min: {min_spread_val:.6f}"
            )
            half_spread = (min_spread_val / Decimal("2")).quantize(
                self.market_info.price_precision
            )
    ...
    async def _reconcile_and_place_orders(
        ...
        for oid, olid in to_cancel_orders:
            self.logger.info(
                f"Cancelling stale/duplicate order {oid} (Side: {self.active_orders.get(oid, {}).get('side')}, Price: {self.active_orders.get(oid, {}).get('price')}). Target Bid: {target_bid}, Target Ask: {target_ask}"
            )
        ...
        if not cur_bid_order:
            ...
            if buy_qty > 0:
                ...
                self.logger.debug(
                    "Calculated buy quantity is zero, skipping bid order placement."
                )
        ...
        if not cur_ask_order:
            ...
            if sell_qty > 0:
                ...
                self.logger.debug(
                    "Calculated sell quantity is zero, skipping ask order placement."
                )

    async def _log_status_summary(self):
        async with self.balance_position_lock, self.active_orders_lock:
            pnl, win_rate, pos_qty = (
                self.config.metrics.total_pnl,
                self.config.metrics.win_rate,
        ...
        self.logger.info(
            f"STATUS | {pnl_summary} | Win Rate: {win_rate:.2f}% | Position: {pos_qty} {self.config.base_currency} (Exposure: {exposure_usd:+.2f} {self.config.quote_currency}) | Orders: {active_buys} Buy / {active_sells} Sell"
        )
        await self.db_manager.log_bot_metrics(self.config.metrics)
    ...
    async def _update_balance_and_position(self) -> bool:
        async with self.balance_position_lock:
            ...
            if not found_quote_balance:
                self.logger.warning(
                    f"Could not find balance for {self.config.quote_currency}."
                )
            ...
            if self.config.category == "linear":
                ...
                self.current_position_qty = Decimal(
                    "0"
                )  # Spot doesn't typically have 'positions' in the same way
                self.unrealized_pnl = Decimal("0")

            self.logger.info(
                f"Updated Balance: {self.current_balance} {self.config.quote_currency}, Position: {self.current_position_qty} {self.config.base_currency}, UPNL: {self.unrealized_pnl}"
            )
            return True
    ...
    async def _process_fill(self, o: dict):
        ...
        self.logger.info(
            f"Order FILLED: {side} {qty} @ {price}, Fee: {fee}, PnL: {pnl}"
        )
    ...
    async def _update_state_after_fill(self, fee: Decimal, pnl: Decimal):
        self.logger.debug(
            "Fill processed. Awaiting WS position update for confirmation of final balance/position."
        )
    ...
    async def _place_limit_order(self, side: str, price: Decimal, quantity: Decimal):
        if self.config.trading_mode == "DRY_RUN":
            ...
            self.logger.info(
                f"DRY_RUN: Would place {side} order: ID={oid}, Qty={quantity}, Price={price}"
            )
            async with self.active_orders_lock:
        ...
        if qty_f <= 0 or price_f <= 0:
            self.logger.warning(
                f"Attempted to place order with zero or negative quantity/price: Qty={qty_f}, Price={price_f}"
            )
            return
