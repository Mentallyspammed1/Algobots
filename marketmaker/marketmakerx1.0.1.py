...
class EnhancedBybitMarketMaker:
    ...
    def _start_streams(self):
        ...
        try:
            ...
            if hasattr(self.ws_public, "ticker_stream"):
                ...
                if hasattr(self.ws_public, "subscribe"):
                    self.ws_public.subscribe(
                        topic="tickers", symbol=[symbol], callback=self._ws_callback
                    )
            ...
            try:
                ...
                if hasattr(self.ws_private, "position_stream"):
                    ...
                    if hasattr(self.ws_private, "subscribe"):
                        self.ws_private.subscribe(
                            topic="position", callback=self._ws_callback
                        )
                # Order
                if hasattr(self.ws_private, "order_stream"):
                    ...
                    if hasattr(self.ws_private, "subscribe"):
                        self.ws_private.subscribe(
                            topic="order", callback=self._ws_callback
                        )
                # Execution
                if hasattr(self.ws_private, "execution_stream"):
                    ...
                    if hasattr(self.ws_private, "subscribe"):
                        self.ws_private.subscribe(
                            topic="execution", callback=self._ws_callback
                        )
    ...
    def generate_status_table(self) -> Table:
        ...
        if self.orderbook["bids"] and self.orderbook["asks"]:
            ...
            spread_bps = (
                (spread / best_bid * 10000)
                if isinstance(best_bid, Decimal) and best_bid > 0
                else 0
            )
