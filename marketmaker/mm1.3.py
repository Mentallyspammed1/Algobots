    ...
    def _get_api_retry_decorator(self):
        ...
        return retry(
            ...
            retry=retry_if_exception_type(
                self._is_retryable_bybit_error
...
class BybitMarketMaker:
    ...
    def _calculate_dynamic_spread(self) -> Decimal:
        ...
        if len(self.mid_price_history) < 2 or np.all(
            np.array(list(self.mid_price_history)) == self.mid_price_history[0]
        ):
            volatility = 0.0
        else:
            volatility = np.std(list(self.mid_price_history)) / np.mean(
                list(self.mid_price_history)
            )
