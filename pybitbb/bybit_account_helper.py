...
if __name__ == "__main__":
    ...
    if wallet_balance and wallet_balance.get('list'):
        for account in wallet_balance['list']:
            print(f"Account Type: {account.get('accountType')}, Total Equity: {account.get('totalEquity')}")
    ...
    if closed_pnl and closed_pnl.get('list'):
        for record in closed_pnl['list']:
            print(f"  Symbol: {record.get('symbol')}, PnL: {record.get('closedPnl')}, Side: {record.get('side')}, Created: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(record.get('createdTime')) / 1000))}")
    else:
        print("  Failed to retrieve closed PnL records.")

    print("\n--- Getting Executions (Linear category, BTCUSDT, last 1 record) ---")
    executions = account_helper.get_executions(category="linear", symbol="BTCUSDT", limit=1)
    if executions and executions.get('list'):
        for exec_record in executions['list']:
            print(f"  Symbol: {exec_record.get('symbol')}, Price: {exec_record.get('execPrice')}, Qty: {exec_record.get('execQty')}, Side: {exec_record.get('side')}")
    else:
    ...
    if fee_rates and fee_rates.get('list'):
        ...
        print(f"  Symbol: {rate_info.get('symbol')}, Maker Fee: {rate_info.get('makerFeeRate')}, Taker Fee: {rate_info.get('takerFeeRate')}")
    else:
    ...
    if account_info:
        print(f"  Unified Margin Account ID: {account_info.get('unifiedMarginAccount', {}).get('unifiedMarginAccountID')}")
    ...
    transaction_log = account_helper.get_transaction_log(account_type="UNIFIED", limit=1)
    if transaction_log and transaction_log.get('list'):
        for log_entry in transaction_log['list']:
            print(f"  Txn Type: {log_entry.get('type')}, Coin: {log_entry.get('coin')}, Amount: {log_entry.get('amount')}, Txn Time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(log_entry.get('transactionTime')) / 1000))}")
    else:
    ...
    if borrow_history and borrow_history.get('list'):
        for borrow_record in borrow_history['list']:
            print(f"  Coin: {borrow_record.get('currency')}, Borrow Amount: {borrow_record.get('borrowAmount')}, Interest: {borrow_record.get('interest')}")
    else:
