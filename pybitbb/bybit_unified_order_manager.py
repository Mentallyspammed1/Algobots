    ...
    def place_order(
        ...
        **kwargs,
        ...
        if actual_mode == "http":
            ...
            self._ws_trade_helper.place_ws_order(
                _temp_ws_callback,
                category,
                symbol,
                side,
    ...
    def amend_order(
        ...
        **kwargs,
        ...
        if actual_mode == "http":
            ...
            self._ws_trade_helper.amend_ws_order(
                _temp_ws_callback,
                category,
                symbol,
                order_id,
                    ...
                    self.tracked_orders[response["orderId"]].update(
                        {
                            "qty": new_qty
                            if new_qty
                            else self.tracked_orders[response["orderId"]].get("qty"),
                            "price": new_price
                            if new_price
                            else self.tracked_orders[response["orderId"]].get("price"),
                            "orderStatus": response.get(
                                "orderStatus",
    ...
    def cancel_order(
        ...
        if response and response.get("orderId"):
            with self._order_update_lock:
                ...
                if response["orderId"] in self.tracked_orders:
                    self.tracked_orders[response["orderId"]]["orderStatus"] = (
                        "Cancelled"
                    )
