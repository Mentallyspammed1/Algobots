...
class BybitMarketDataHelper:
    ...
    def get_tickers(self, category: str, symbol: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        if not isinstance(category, str) or not category:
            logger.error("Invalid or empty string provided for 'category' for get_tickers.")
            return None
    ...
    def get_kline(self, 
        ...
        if interval not in allowed_intervals:
            logger.warning(f"Provided interval '{interval}' might not be supported. Allowed: {allowed_intervals}")
        ...
        if start_time is not None:
            if not isinstance(start_time, int) or start_time < 0:
                logger.error("Invalid 'start_time' provided for get_kline. Must be a positive integer (ms).")
                return None
            params['start'] = start_time
        if end_time is not None:
            if not isinstance(end_time, int) or end_time < 0:
                logger.error("Invalid 'end_time' provided for get_kline. Must be a positive integer (ms).")
                return None
            params['end'] = end_time
    ...
    def get_instruments_info(self, category: str, symbol: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        # Input validation
        if not isinstance(category, str) or not category:
            logger.error("Invalid or empty string provided for 'category' for get_instruments_info.")
            return None
        if symbol is not None and (not isinstance(symbol, str) or not symbol):
    ...
    def get_public_trade_history(self, category: str, symbol: str, **kwargs) -> Optional[Dict[str, Any]]:
        ...
        if not all(isinstance(arg, str) and arg for arg in [category, symbol]):
            logger.error("Invalid or empty string provided for category or symbol for get_public_trade_history.")
            return None
    ...
    def get_funding_rate_history(self, category: str, symbol: str, **kwargs) -> Optional[Dict[str, Any]]:
        ...
        # Input validation
        if not all(isinstance(arg, str) and arg for arg in [category, symbol]):
            logger.error("Invalid or empty string provided for category or symbol for get_funding_rate_history.")
            return None
...
if __name__ == "__main__":
    ...
    market_data_helper = BybitMarketDataHelper(testnet=USE_TESTNET, api_key=API_KEY, api_secret=API_SECRET)
    ...
    if server_time:
        ...
        print(f"  Local Time (from server seconds): {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(server_time.get('timeSecond', 0))))}")
    else:
    ...
    if tickers and tickers.get('list'):
        ...
        print(f"  Symbol: {ticker.get('symbol')}, Last Price: {ticker.get('lastPrice')}, 24h Change: {ticker.get('price24hPcnt')}, Volume: {ticker.get('volume24h')}")
    else:
    ...
    if kline_data and kline_data.get('list'):
        ...
        for bar in kline_data['list']:
            ...
            ts = int(bar[0]) / 1000
            print(f"  Time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(ts))}, Open: {bar[1]}, High: {bar[2]}, Low: {bar[3]}, Close: {bar[4]}, Volume: {bar[5]}")
    else:
        print(f"  Failed to retrieve kline data for {SYMBOL}.")

    print(f"\n--- Getting Instruments Info for {SYMBOL} ({CATEGORY}) ---")
    instrument_info = market_data_helper.get_instruments_info(category=CATEGORY, symbol=SYMBOL)
    if instrument_info and instrument_info.get('list'):
        info = instrument_info['list'][0]
        print(f"  Symbol: {info.get('symbol')}, Base Coin: {info.get('baseCoin')}, Quote Coin: {info.get('quoteCoin')}")
        print(f"  Price Filter (tickSize): {info.get('priceFilter', {}).get('tickSize')}")
        print(f"  Lot Size Filter (minOrderQty): {info.get('lotSizeFilter', {}).get('minOrderQty')}")
    else:
        print(f"  Failed to retrieve instrument info for {SYMBOL}.")

    print(f"\n--- Getting Public Trade History for {SYMBOL} (last 3 records) ---")
    public_trades = market_data_helper.get_public_trade_history(category=CATEGORY, symbol=SYMBOL, limit=3)
    if public_trades and public_trades.get('list'):
        for trade in public_trades['list']:
            # Convert timestamp from string to int for readable format
            ts = int(trade.get('time')) / 1000
            print(f"  Price: {trade.get('price')}, Qty: {trade.get('qty')}, Side: {trade.get('side')}, Time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(ts))}")
    else:
        print(f"  Failed to retrieve public trade history for {SYMBOL}.")

    print(f"\n--- Getting Funding Rate History for {SYMBOL} (last 1 record) ---")
    funding_rate_history = market_data_helper.get_funding_rate_history(category=CATEGORY, symbol=SYMBOL, limit=1)
    if funding_rate_history and funding_rate_history.get('list'):
        ...
        print(f"  Funding Rate: {rate_record.get('fundingRate')}, Funding Time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(ts))}")
    else:
    ...
    long_short_ratio = market_data_helper.get_long_short_ratio(category=CATEGORY, symbol=SYMBOL, interval_time="1h", limit=1)
    if long_short_ratio and long_short_ratio.get('list'):
        ratio_record = long_short_ratio['list'][0]
        print(f"  Buy Ratio: {ratio_record.get('buyRatio')}, Sell Ratio: {ratio_record.get('sellRatio')}, Timestamp: {ratio_record.get('timestamp')}")
    else:
