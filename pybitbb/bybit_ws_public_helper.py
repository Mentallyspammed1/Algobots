...
class BybitWsPublicHelper:
    ...
    def __init__(self, category: str, testnet: bool = False):
        ...
        self._subscriptions: Dict[str, Tuple[Callable[[Dict[str, Any]], None], Dict[str, Any]]] = {} # {topic: (callback, kwargs)}

        logger.info(f"BybitWsPublicHelper initialized for category '{self.category}'. Testnet: {self.testnet}")
    ...
    def _on_message(self, message: Dict[str, Any]) -> None:
        ...
        if not topic:
            logger.debug(f"[{self.category}] Received WebSocket message without topic: {message}")
            return
        ...
        if matched_callback:
            try:
                ...
                logger.exception(f"[{self.category}] Error in user-defined callback for topic {topic}: {e}")
        else:
            logger.debug(f"[{self.category}] No registered callback for topic: {topic}. Message: {message}")
    ...
    def _resubscribe_all(self) -> None:
        ...
        if self.websocket_client and self.websocket_client.is_connected():
            logger.info(f"[{self.category}] Re-subscribing to {len(self._subscriptions)} topics after reconnection.")
            ...
            logger.warning(f"[{self.category}] Cannot re-subscribe: WebSocket client not connected.")
    ...
    def connect(self, wait_for_connection: bool = True, timeout: int = 10) -> bool:
        ...
        with self._connection_lock:
            if self.websocket_client and self.websocket_client.is_connected():
                logger.warning(f"[{self.category}] WebSocket client is already connected.")
                return True

            logger.info(f"[{self.category}] Connecting to WebSocket (Testnet: {self.testnet})...")
            try:
                ...
                if wait_for_connection:
                    if not self._is_connected_event.wait(timeout=timeout):
                        logger.error(f"[{self.category}] Timeout waiting for WebSocket connection.")
                ...
                logger.exception(f"[{self.category}] Failed to initialize WebSocket client: {e}")
                self.websocket_client = None
                return False
    ...
    def disconnect(self) -> None:
        ...
        with self._connection_lock:
            if self.websocket_client:
                logger.info(f"[{self.category}] Closing WebSocket connection for category '{self.category}'...")
                ...
                logger.info(f"[{self.category}] WebSocket client not active for category '{self.category}'.")
    ...
    def subscribe_to_stream(self, 
        ...
        if not self.is_connected():
            logger.error(f"[{self.category}] Cannot subscribe to {stream_type} for {symbol}: WebSocket not connected.")
            return False
        if not isinstance(symbol, str) or not symbol:
            logger.error(f"[{self.category}] Invalid 'symbol' provided for subscription to {stream_type}.")
            return False
        ...
        if topic in self._subscriptions:
            logger.warning(f"[{self.category}] Already subscribed to topic: {topic}. Updating callback.")
        ...
        try:
            if stream_type == "kline":
                self.websocket_client.kline_stream(interval=kwargs.get('interval', '1'), symbol=symbol, callback=self._on_message)
            elif stream_type == "ticker":
                self.websocket_client.ticker_stream(symbol=symbol, callback=self._on_message)
            elif stream_type == "trade":
                self.websocket_client.trade_stream(symbol=symbol, callback=self._on_message)
            elif stream_type == "orderbook":
                self.websocket_client.orderbook_stream(depth=kwargs.get('depth', 50), symbol=symbol, callback=self._on_message)
            elif stream_type == "liquidation":
                self.websocket_client.liquidation_stream(symbol=symbol, callback=self._on_message)
            elif stream_type == "all_liquidation":
                self.websocket_client.all_liquidation_stream(callback=self._on_message)
            
            logger.info(f"[{self.category}] Successfully initiated subscription to {topic}.")
            return True
        except BybitWebsocketError as e:
            logger.exception(f"[{self.category}] WebSocket error during subscription to {topic}: {e}")
            del self._subscriptions[topic] # Remove failed subscription
            return False
        except Exception as e:
            logger.exception(f"[{self.category}] Unexpected error during subscription to {topic}: {e}")
            del self._subscriptions[topic] # Remove failed subscription
            return False
    ...
    def unsubscribe_from_stream(self, stream_type: str, symbol: str, **kwargs) -> bool:
        ...
        """
        if not self.is_connected():
            logger.error(f"[{self.category}] Cannot unsubscribe from {stream_type} for {symbol}: WebSocket not connected.")
            return False
        if not isinstance(symbol, str) or not symbol:
            logger.error(f"[{self.category}] Invalid 'symbol' provided for unsubscription from {stream_type}.")
            return False
        ...
        if topic not in self._subscriptions:
            logger.warning(f"[{self.category}] Not subscribed to topic: {topic}. No action taken.")
            return False
        ...
        try:
            ...
            logger.info(f"[{self.category}] Removed internal subscription for {topic}. Pybit's client does not expose direct unsubscribe per topic.")
            return True
        except Exception as e:
            logger.exception(f"[{self.category}] Error during unsubscription from {topic}: {e}")
            return False
...
if __name__ == "__main__":
    ...
    def handle_ticker_update(message: Dict[str, Any]) -> None:
        ...
        if data:
            ticker = data[0]
            logger.info(f"[{ticker.get('symbol')}] Ticker Update: Last Price={ticker.get('lastPrice')}, Volume={ticker.get('volume24h')}")

    def handle_kline_update(message: Dict[str, Any]) -> None:
        data = message.get('data')
        if data:
            kline = data[0]
            logger.info(f"[{kline.get('symbol')}] Kline {kline.get('interval')} Update: Close={kline.get('close')}, Volume={kline.get('volume')}")

    def handle_trade_update(message: Dict[str, Any]) -> None:
        data = message.get('data')
        if data:
            trade = data[0]
            logger.info(f"[{trade.get('s')}] Trade: Price={trade.get('p')}, Qty={trade.get('v')}, Side={trade.get('S')}")

    def handle_liquidation_update(message: Dict[str, Any]) -> None:
        data = message.get('data')
        if data:
            liquidation = data[0]
            logger.warning(f"[LIQUIDATION] Symbol: {liquidation.get('s')}, Price: {liquidation.get('p')}, Qty: {liquidation.get('q')}, Side: {liquidation.get('S')}")
    ...
    try:
        if public_ws_helper.connect(wait_for_connection=True, timeout=15):
            ...
            public_ws_helper.subscribe_to_stream("kline", handle_kline_update, SYMBOL, interval="1")
            time.sleep(1)
            ...
            public_ws_helper.subscribe_to_stream("all_liquidation", handle_liquidation_update, symbol="ALL")
            time.sleep(1)
