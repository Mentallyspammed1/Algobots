...
class BybitSizingHelper:
    ...
    def __init__(self, testnet: bool = False, api_key: str = "", api_secret: str = ""):
        ...
        self._cache_expiry_time: Dict[str, float] = {} # {f"{category}_{symbol}": timestamp_of_expiry}
    ...
    def _get_instrument_info(self, category: str, symbol: str, force_update: bool = False) -> Optional[Dict[str, Any]]:
        """
        ...
        if not force_update and cache_key in self._instrument_info_cache and \
           current_time < self._cache_expiry_time.get(cache_key, 0):
            logger.debug(f"Using cached instrument info for {symbol}.")
            return self._instrument_info_cache[cache_key]

        logger.info(f"Fetching fresh instrument info for {symbol} (category: {category}).")
        info = self.market_data_helper.get_instruments_info(category=category, symbol=symbol)
        
        if info and info.get('list'):
            instrument_details = info['list'][0]
            self._instrument_info_cache[cache_key] = instrument_details
            self._cache_expiry_time[cache_key] = current_time + self.cache_duration_seconds
            logger.debug(f"Cached instrument info for {symbol} (expires in {self.cache_duration_seconds}s).")
            return instrument_details
        
        logger.error(f"Failed to retrieve instrument info for {symbol} (category: {category}).")
        return None

    def _get_filter_value(self, category: str, symbol: str, filter_type: str, key: str, default: float = 0.0) -> float:
        """
        ...
        if info:
            ...
            if value is not None:
                try:
                    ...
                    logger.error(f"Invalid numerical value for {key} in {filter_type} for {symbol}: {value}")
        logger.warning(f"Could not get {key} from {filter_type} for {symbol}. Using default: {default}")
        return default
    ...
    def get_min_order_qty(self, category: str, symbol: str) -> float:
        ...
        """
        return self._get_filter_value(category, symbol, 'lotSizeFilter', 'minOrderQty', default=0.0)
    ...
    def get_max_order_qty(self, category: str, symbol: str) -> float:
        ...
        return self._get_filter_value(category, symbol, 'lotSizeFilter', 'maxOrderQty', default=float('inf'))
    ...
    def get_qty_step(self, category: str, symbol: str) -> float:
        ...
        """
        return self._get_filter_value(category, symbol, 'lotSizeFilter', 'qtyStep', default=0.0)
    ...
    def get_min_price(self, category: str, symbol: str) -> float:
        ...
        """
        return self._get_filter_value(category, symbol, 'priceFilter', 'minPrice', default=0.0)
    ...
    def get_max_price(self, category: str, symbol: str) -> float:
        ...
        """
        return self._get_filter_value(category, symbol, 'priceFilter', 'maxPrice', default=float('inf'))
    ...
    def get_price_tick_size(self, category: str, symbol: str) -> float:
        ...
        """
        return self._get_filter_value(category, symbol, 'priceFilter', 'tickSize', default=0.0)
    ...
    def round_qty(self, category: str, symbol: str, quantity: Union[float, str]) -> float:
        """
    ...
    def round_price(self, category: str, symbol: str, price: Union[float, str]) -> float:
        """
    ...
    def is_valid_qty(self, category: str, symbol: str, quantity: Union[float, str]) -> bool:
        """
        ...
        if qty_float < min_qty:
            logger.warning(f"Quantity {qty_float} is less than min_qty {min_qty} for {symbol}.")
            return False
        if qty_float > max_qty:
            logger.warning(f"Quantity {qty_float} is greater than max_qty {max_qty} for {symbol}.")
            return False
        
        # Check if quantity is a multiple of qty_step
        if qty_step > 0 and (qty_float - min_qty) % qty_step > 1e-9: # Allow small float tolerance
            logger.warning(f"Quantity {qty_float} is not a multiple of qty_step {qty_step} (min_qty={min_qty}) for {symbol}.")
            return False
            
        return True

    def is_valid_price(self, category: str, symbol: str, price: Union[float, str]) -> bool:
        """
        ...
        if price_float < min_price:
            logger.warning(f"Price {price_float} is less than min_price {min_price} for {symbol}.")
            return False
        if price_float > max_price:
            logger.warning(f"Price {price_float} is greater than max_price {max_price} for {symbol}.")
            return False
        
        # Check if price is a multiple of tick_size
        if tick_size > 0 and price_float % tick_size > 1e-9: # Allow small float tolerance
            logger.warning(f"Price {price_float} is not a multiple of tick_size {tick_size} for {symbol}.")
            return False
...
if __name__ == "__main__":
    ...
    sizing_helper = BybitSizingHelper(testnet=USE_TESTNET, api_key=API_KEY, api_secret=API_SECRET)
    ...
    invalid_qty_step = min_qty + (qty_step / 2) if qty_step > 0 else min_qty + 0.000000001
    is_valid_step = sizing_helper.is_valid_qty(CATEGORY, SYMBOL, invalid_qty_step)
    print(f"  Is {invalid_qty_step} valid? {is_valid_step} (Expected: False, due to step)")
    ...
    is_valid_low_price = sizing_helper.is_valid_price(CATEGORY, SYMBOL, invalid_price_low)
    print(f"  Is {invalid_price_low} valid? {is_valid_low_price} (Expected: False)")

    invalid_price_tick = min_price + (price_tick_size / 2) if price_tick_size > 0 else min_price + 0.000000001
    is_valid_tick = sizing_helper.is_valid_price(CATEGORY, SYMBOL, invalid_price_tick)
    print(f"  Is {invalid_price_tick} valid? {is_valid_tick} (Expected: False, due to tick size)")
    ...
    print(f"  Min Order Quantity for {SPOT_SYMBOL}: {sizing_helper.get_min_order_qty(SPOT_CATEGORY, SPOT_SYMBOL)}")
    print(f"  Price Tick Size for {SPOT_SYMBOL}: {sizing_helper.get_price_tick_size(SPOT_CATEGORY, SPOT_SYMBOL)}")
