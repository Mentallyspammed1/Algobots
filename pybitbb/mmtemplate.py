...
ORDER_MODE: TradingMode = "websocket" # "http" or "websocket" for order placement/amendment/cancellation
...
class MarketMakerBot:
    ...
    def _manage_active_orders(self):
        ...
        with active_orders_lock:
            ...
            for order_id, order_details in active_market_maker_orders.items():
                if order_details.get('orderStatus') in ["Filled", "Cancelled", "Deactivated", "Rejected"]:
                    logger.info(f"Order {order_id} already in final state: {order_details.get('orderStatus')}. Removing from active tracking.")
                    orders_to_remove.append(order_id)
                    continue

                placed_time = order_details.get('placedTime')
                if placed_time and (time.time() * 1000 - placed_time) / 1000 > ORDER_EXPIRY_SECONDS:
    ...
    def _place_market_making_orders(self):
        ...
        target_bid_price = self.sizing_helper.round_price(CATEGORY, SYMBOL, target_bid_price)
        target_ask_price = self.sizing_helper.round_price(CATEGORY, SYMBOL, target_ask_price)
        ...
        if target_bid_price >= best_bid_level.price:
            target_bid_price = best_bid_level.price * (1 - self.sizing_helper.get_price_tick_size(CATEGORY, SYMBOL))
            target_bid_price = self.sizing_helper.round_price(CATEGORY, SYMBOL, target_bid_price)
        if target_ask_price <= best_ask_level.price:
            target_ask_price = best_ask_level.price * (1 + self.sizing_helper.get_price_tick_size(CATEGORY, SYMBOL))
            target_ask_price = self.sizing_helper.round_price(CATEGORY, SYMBOL, target_ask_price)
        ...
        if not self.sizing_helper.is_valid_qty(CATEGORY, SYMBOL, order_qty):
            logger.error(f"Calculated order quantity {order_qty} is invalid. Aborting order placement.")
            return
        ...
        for oid, details in self.order_manager.get_all_tracked_orders().items():
            if details.get('symbol') == SYMBOL and details.get('side') == 'Buy' and \
               details.get('orderStatus') == 'New' and \
               abs(float(details.get('price', 0)) - target_bid_price) < self.sizing_helper.get_price_tick_size(CATEGORY, SYMBOL):
        ...
        if place_bid:
            if existing_bid_order:
                ...
                if abs(float(existing_bid_order['price']) - target_bid_price) > self.sizing_helper.get_price_tick_size(CATEGORY, SYMBOL):
                    logger.info(f"Amending existing bid order {existing_bid_order['orderId']} to new price {target_bid_price}.")
                    self.order_manager.amend_order(
                        category=CATEGORY, symbol=SYMBOL, order_id=existing_bid_order['orderId'],
                        new_price=str(target_bid_price)
                    )
            else:
                logger.info(f"Placing new BID order: {order_qty} {SYMBOL} @ {target_bid_price}")
                bid_response = self.order_manager.place_order(
                    category=CATEGORY, symbol=SYMBOL, side="Buy", order_type="Limit",
                    qty=str(order_qty), price=str(target_bid_price), timeInForce="GTC",
                    orderLinkId=f"mm-bid-{int(time.time())}"
                )
                if bid_response:
                    with active_orders_lock:
                        active_market_maker_orders[bid_response['orderId']] = self.order_manager.get_tracked_order(bid_response['orderId'])
                else:
        ...
        if current_position <= -MAX_POSITION_SIZE:
            logger.warning(f"Position ({current_position:.4f}) is at or below -MAX_POSITION_SIZE. Skipping ask order.")
            place_ask = False

        # Check if there's an existing ask order at the target price
        existing_ask_order = None
        for oid, details in self.order_manager.get_all_tracked_orders().items():
            if details.get('symbol') == SYMBOL and details.get('side') == 'Sell' and \
               details.get('orderStatus') == 'New' and \
               abs(float(details.get('price', 0)) - target_ask_price) < self.sizing_helper.get_price_tick_size(CATEGORY, SYMBOL):
        ...
        if place_ask:
            if existing_ask_order:
                # Check if price needs amendment
                if abs(float(existing_ask_order['price']) - target_ask_price) > self.sizing_helper.get_price_tick_size(CATEGORY, SYMBOL):
                    logger.info(f"Amending existing ask order {existing_ask_order['orderId']} to new price {target_ask_price}.")
                    self.order_manager.amend_order(
                        category=CATEGORY, symbol=SYMBOL, order_id=existing_ask_order['orderId'],
                        new_price=str(target_ask_price)
                    )
            else:
                logger.info(f"Placing new ASK order: {order_qty} {SYMBOL} @ {target_ask_price}")
                ask_response = self.order_manager.place_order(
                    category=CATEGORY, symbol=SYMBOL, side="Sell", order_type="Limit",
                    qty=str(order_qty), price=str(target_ask_price), timeInForce="GTC",
                    orderLinkId=f"mm-ask-{int(time.time())}"
                )
                if ask_response:
                    with active_orders_lock:
                        active_market_maker_orders[ask_response['orderId']] = self.order_manager.get_tracked_order(ask_response['orderId'])
                else:
    ...
    def start_bot(self):
        ...
        if not self.sizing_helper.get_qty_step(CATEGORY, SYMBOL):
            logger.critical("Sizing helper failed to retrieve instrument info. Aborting bot start.")
            return

        self._running.set()
        self._main_loop_thread = threading.Thread(target=self._market_maker_loop, daemon=True)
    ...
    def stop_bot(self):
        ...
        if self._main_loop_thread and self._main_loop_thread.is_alive():
            ...
            if self._main_loop_thread.is_alive():
                logger.warning("Main market maker loop thread did not terminate gracefully.")
...
if __name__ == "__main__":
    if API_KEY == "YOUR_API_KEY" or API_SECRET == "YOUR_API_SECRET":
        logger.critical("Please replace YOUR_API_KEY and YOUR_API_SECRET with your actual credentials in bybit_market_maker_template.py.")
        exit()
