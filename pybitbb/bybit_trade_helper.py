...
class BybitTradeHelper:
    ...
    def place_order(self,
        ...
        if not all(isinstance(arg, str) and arg for arg in [category, symbol, side, order_type, qty]):
            logger.error("Invalid or empty string provided for one of the required parameters: category, symbol, side, order_type, qty.")
            return None
        try:
            float(qty) # Check if qty is convertible to float
            if price is not None: float(price) # Check if price is convertible to float
        except ValueError:
    ...
    def set_leverage(self, category: str, symbol: str, buy_leverage: str, sell_leverage: str) -> dict[str, Any] | None:
        ...
        """
        # Input validation
        if not all(isinstance(arg, str) and arg for arg in [category, symbol, buy_leverage, sell_leverage]):
            logger.error("Invalid or empty string provided for category, symbol, buy_leverage, or sell_leverage.")
            return None
        try:
            float(buy_leverage)
            float(sell_leverage)
        except ValueError:
            logger.error(f"Invalid numerical format for buy_leverage ('{buy_leverage}') or sell_leverage ('{sell_leverage}').")
            return None
    ...
    def switch_position_mode(self, category: str, mode: str) -> dict[str, Any] | None:
        ...
        if not all(isinstance(arg, str) and arg for arg in [category, mode]):
            ...
            return None
        if mode not in ["0", "3"]:
            logger.error(f"Invalid 'mode' for switch_position_mode. Expected '0' (One-Way) or '3' (Hedge Mode), got '{mode}'.")
            return None
        ...
        return self._make_request('switch_position_mode', 'Switch Position Mode', **params)
...
if __name__ == "__main__":
    ...
    if positions and positions.get('list'):
        for pos in positions['list']:
            print(f"  Symbol: {pos.get('symbol')}, Side: {pos.get('side')}, Size: {pos.get('size')}, PnL: {pos.get('unrealisedPnl')}")
    else:
        print(f"  No open positions for {SYMBOL} or failed to retrieve.")

    print(f"\n--- Setting Leverage for {SYMBOL} to 10x (Buy) and 10x (Sell) ---")
    set_leverage_response = trade_helper.set_leverage(category=CATEGORY, symbol=SYMBOL, buy_leverage="10", sell_leverage="10")
    ...
    current_price = None
    if ticker_response and ticker_response['retCode'] == 0 and ticker_response['result']['list']:
        current_price = float(ticker_response['result']['list'][0]['lastPrice'])
        print(f"\nCurrent market price for {SYMBOL}: {current_price}")
    else:
        logger.warning(f"Could not fetch current market price for {SYMBOL}. Using a placeholder price.")
        current_price = 40000.0 # Fallback placeholder

    buy_price = str(round(current_price * 0.99, 1)) # Place a buy limit order slightly below market
    sell_price = str(round(current_price * 1.01, 1)) # Place a sell limit order slightly above market
    ...
    if place_order_response:
        ...
        print(f"  Order placed. Order ID: {order_id}, Client Order ID: {client_order_id}")
    else:
    ...
    if order_id:
        ...
        amend_order_response = trade_helper.amend_order(
            ...
            symbol=SYMBOL,
            order_id=order_id,
            new_price=str(round(float(buy_price) * 1.005, 1)) # New price, adjust as needed
        )
    ...
    if place_sell_order_response:
        print(f"  Sell order placed. Order ID: {place_sell_order_response.get('orderId')}, Client Order ID: {client_order_id_sell}")
        ...
        cancel_by_client_id_response = trade_helper.cancel_order(
            category=CATEGORY,
            symbol=SYMBOL,
            order_link_id=client_order_id_sell
        )
    ...
    if open_orders and open_orders.get('list'):
        for order in open_orders['list']:
            print(f"  Order ID: {order.get('orderId')}, Side: {order.get('side')}, Price: {order.get('price')}, Qty: {order.get('qty')}, Status: {order.get('orderStatus')}")
    else:
        print(f"  No open orders for {SYMBOL} or failed to retrieve.")

    print(f"\n--- Getting Order History for {SYMBOL} (last 2 records) ---")
    order_history = trade_helper.get_order_history(category=CATEGORY, symbol=SYMBOL, limit=2)
    if order_history and order_history.get('list'):
        for order in order_history['list']:
            print(f"  Order ID: {order.get('orderId')}, Side: {order.get('side')}, Price: {order.get('price')}, Status: {order.get('orderStatus')}, Created: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(order.get('createdTime')) / 1000))}")
    else:
    ...
    switch_mode_response = trade_helper.switch_position_mode(category=CATEGORY, mode="0")
    if switch_mode_response:
        print(f"  Switch position mode response: {switch_mode_response}")
    else:
        print("  Failed to switch position mode (may already be in target mode or have open positions).")
