...
logging.basicConfig(
    level=getattr(logging, config.LOG_LEVEL.upper(), logging.INFO), # Set log level from config
...
class SupertrendCrossBot:
    def __init__(self): # Removed api_key, api_secret, testnet from __init__
        ...
        if self.api_key == "YOUR_API_KEY_HERE" or self.api_secret == "YOUR_API_SECRET_HERE":
            logger.critical("API Key and Secret are not set in config.py. Please update them.")
            raise ValueError("API Key and Secret must be provided in config.py.")

        # Initialize helpers with config values
        self.account_helper = BybitAccountHelper(self.api_key, self.api_secret, self.testnet)
        self.sizing_helper = BybitSizingHelper(testnet=self.testnet, api_key=self.api_key, api_secret=self.api_secret)
        self.order_manager = BybitUnifiedOrderManager(
            api_key=self.api_key, api_secret=self.api_secret, testnet=self.testnet,
            default_mode=config.ORDER_MODE, ws_recv_window=config.WS_RECV_WINDOW
        )
    ...
    def _load_sizing_info(self):
        ...
        logger.info(f"Loading sizing information for {config.SYMBOL} ({config.CATEGORY})...")
        self.sizing_helper._get_instrument_info(config.CATEGORY, config.SYMBOL, force_update=True)
        if not self.sizing_helper.get_qty_step(config.CATEGORY, config.SYMBOL):
            logger.critical("Sizing helper failed to retrieve instrument info. Cannot proceed.")
    ...
    def _main_bot_loop(self):
        ...
        global current_bot_position, last_supertrend_signal, last_processed_kline_timestamp
        ...
        while self._running.is_set():
            try:
                ...
                if df is None or df.empty:
                    logger.warning(f"No sufficient data to calculate Supertrend for {config.SYMBOL}. Retrying in 10 seconds.")
                    time.sleep(10)
                    continue

                # Ensure we only process new, closed klines
                latest_kline_timestamp = df.index[-1].timestamp() * 1000 # Convert to ms
                ...
                latest_supertrend_direction = df[supertrend_direction_column].iloc[-1] # 1 for uptrend, -1 for downtrend
                ...
                bybit_position_side, bybit_position_size = self._get_current_bybit_position()
                logger.info(f"Current Bybit position: {bybit_position_side.upper()} {bybit_position_size:.4f} {config.SYMBOL}")
    ...
    def start_bot(self):
        ...
        logger.info(f"Bot starting with current Bybit position: {bybit_pos_side.upper()} {bybit_pos_size:.4f} {config.SYMBOL}")

        self._running.set()
        self._main_loop_thread = threading.Thread(target=self._main_bot_loop, daemon=True)
