...
class BybitIndicators:
    ...
    def __init__(self, testnet: bool = False, api_key: str = "", api_secret: str = ""):
        ...
        """
        self.market_data_helper = BybitMarketDataHelper(testnet=testnet, api_key=api_key, api_secret=api_secret)
    ...
    def _fetch_kline_data(self, 
        ...
        :return: A pandas DataFrame with columns ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'turnover']
                 or None on failure.
        """
        logger.debug(f"Fetching kline data for {symbol}, interval {interval}, limit {limit}...")
        ...
        if not kline_data or not kline_data.get('list'):
            logger.warning(f"No kline data fetched for {symbol} with interval {interval}.")
            return None
    ...
    def get_historical_data_with_indicators(self,
                                            ...
                                            indicator_kwargs: Optional[Dict[str, Any]] = None) -> Optional[pd.DataFrame]:
        """
        ...
        """
        df = self._fetch_kline_data(category, symbol, interval, limit, start_time, end_time)
        if df is None:
            return None

        if indicators:
            if not isinstance(indicators, list) or not all(isinstance(i, str) for i in indicators):
                logger.error("Invalid 'indicators' parameter. Must be a list of strings.")
                return None
            ...
            for indicator_name in indicators:
                try:
                    ...
                    if hasattr(df.ta, indicator_name):
                        ...
                        logger.warning(f"Indicator '{indicator_name}' not found in pandas_ta. Skipping.")

                except Exception as e:
                    logger.error(f"Failed to calculate indicator '{indicator_name}' for {symbol}: {e}", exc_info=True)
        ...
        df = df.dropna()
        if df.empty:
            logger.warning(f"DataFrame became empty after dropping NaN values for {symbol}. Not enough data for indicators.")
            return None

        logger.info(f"Successfully calculated indicators for {symbol}, interval {interval}. DataFrame shape: {df.shape}")
        return df

    # --- Convenience Wrapper Methods for Specific Indicators ---
    # Existing Indicators
    def get_sma(self, category: str, symbol: str, interval: str, length: int = 20, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Simple Moving Average (SMA)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['sma'], indicator_kwargs={'sma': {'length': length, **kwargs}}
        )

    def get_ema(self, category: str, symbol: str, interval: str, length: int = 20, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Exponential Moving Average (EMA)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['ema'], indicator_kwargs={'ema': {'length': length, **kwargs}}
        )

    def get_rsi(self, category: str, symbol: str, interval: str, length: int = 14, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Relative Strength Index (RSI)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['rsi'], indicator_kwargs={'rsi': {'length': length, **kwargs}}
        )

    def get_macd(self, category: str, symbol: str, interval: str, fast: int = 12, slow: int = 26, signal: int = 9, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Moving Average Convergence Divergence (MACD)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['macd'], indicator_kwargs={'macd': {'fast': fast, 'slow': slow, 'signal': signal, **kwargs}}
        )
    
    def get_bollinger_bands(self, category: str, symbol: str, interval: str, length: int = 20, std: int = 2, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Bollinger Bands (BBANDS)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['bbands'], indicator_kwargs={'bbands': {'length': length, 'std': std, **kwargs}}
        )
    
    def get_stoch_rsi(self, category: str, symbol: str, interval: str, rsi_length: int = 14, k_length: int = 3, d_length: int = 3, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Stochastic RSI (STOCHRSI)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['stochrsi'], indicator_kwargs={'stochrsi': {'length': rsi_length, 'k': k_length, 'd': d_length, **kwargs}}
        )

    # --- 10+ New Indicators ---
    def get_supertrend(self, category: str, symbol: str, interval: str, length: int = 7, multiplier: float = 3.0, **kwargs) -> Optional[pd.DataFrame]:
        """
        ...
        """
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['supertrend'], indicator_kwargs={'supertrend': {'length': length, 'multiplier': multiplier, **kwargs}}
        )

    def get_atr(self, category: str, symbol: str, interval: str, length: int = 14, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Average True Range (ATR)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['atr'], indicator_kwargs={'atr': {'length': length, **kwargs}}
        )

    def get_cci(self, category: str, symbol: str, interval: str, length: int = 20, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Commodity Channel Index (CCI)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['cci'], indicator_kwargs={'cci': {'length': length, **kwargs}}
        )

    def get_mfi(self, category: str, symbol: str, interval: str, length: int = 14, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Money Flow Index (MFI)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['mfi'], indicator_kwargs={'mfi': {'length': length, **kwargs}}
        )

    def get_obv(self, category: str, symbol: str, interval: str, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates On-Balance Volume (OBV)."""
        # OBV typically only needs 'close' and 'volume', no length parameter
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['obv'], indicator_kwargs={'obv': {**kwargs}}
        )

    def get_stoch(self, category: str, symbol: str, interval: str, k_length: int = 14, d_length: int = 3, smooth_k: int = 3, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Stochastic Oscillator (STOCH)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['stoch'], indicator_kwargs={'stoch': {'k': k_length, 'd': d_length, 'smooth_k': smooth_k, **kwargs}}
        )

    def get_ao(self, category: str, symbol: str, interval: str, fast: int = 5, slow: int = 34, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Awesome Oscillator (AO)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['ao'], indicator_kwargs={'ao': {'fast': fast, 'slow': slow, **kwargs}}
        )

    def get_psar(self, category: str, symbol: str, interval: str, af0: float = 0.02, af: float = 0.02, max_af: float = 0.2, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Parabolic SAR (PSAR)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['psar'], indicator_kwargs={'psar': {'af0': af0, 'af': af, 'max_af': max_af, **kwargs}}
        )

    def get_vwap(self, category: str, symbol: str, interval: str, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Volume Weighted Average Price (VWAP)."""
        # VWAP typically needs 'open', 'high', 'low', 'close', 'volume'
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['vwap'], indicator_kwargs={'vwap': {**kwargs}}
        )

    def get_kama(self, category: str, symbol: str, interval: str, length: int = 10, **kwargs) -> Optional[pd.DataFrame]:
        """Calculates Kaufman's Adaptive Moving Average (KAMA)."""
        return self.get_historical_data_with_indicators(
            category, symbol, interval, indicators=['kama'], indicator_kwargs={'kama': {'length': length, **kwargs}}
        )
...
if __name__ == "__main__":
    ...
    indicators_helper = BybitIndicators(testnet=USE_TESTNET, api_key=API_KEY, api_secret=API_SECRET)
    ...
    df_multi_indicators = indicators_helper.get_historical_data_with_indicators(
        ...
        interval=INTERVAL,
        limit=200,
        indicators=['sma', 'ema', 'rsi', 'macd', 'bbands', 'supertrend', 'atr', 'cci', 'mfi', 'obv', 'stoch', 'ao', 'psar', 'vwap', 'kama'],
        indicator_kwargs={
            'sma': {'length': 20},
            'ema': {'length': 20},
            'rsi': {'length': 14},
    ...
    if df_multi_indicators is not None:
        ...
        if 'STOCHk_14_3_3' in df_multi_indicators.columns:
            print(f"Last STOCH %K (14,3,3) value: {df_multi_indicators['STOCHk_14_3_3'].iloc[-1]:.2f}")
        if 'AO' in df_multi_indicators.columns:
            print(f"Last AO value: {df_multi_indicators['AO'].iloc[-1]:.2f}")
        if 'PSARl_0.02_0.2' in df_multi_indicators.columns: # PSAR can have long/short variants
            print(f"Last PSAR Long value: {df_multi_indicators['PSARl_0.02_0.2'].iloc[-1]:.2f}")
    ...
    df_supertrend_only = indicators_helper.get_supertrend(category=CATEGORY, symbol=SYMBOL, interval=INTERVAL, length=10, multiplier=3.5, limit=100)
    if df_supertrend_only is not None:
        print(df_supertrend_only[['close', 'SUPERT_10_3.5', 'SUPERTd_10_3.5', 'SUPERTl_10_3.5', 'SUPERTs_10_3.5']].tail())
    else:
