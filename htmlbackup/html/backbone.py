        ...
        try:
            ...
            if BOT_STATE["current_position_info"]["order_id"] and current_position:
                ...
                current_sl_on_exchange = (
                    float(current_position["stopLoss"])
            ...
            if buy_signal or sell_signal:
                ...
                if stop_distance > 0:
                    ...
                    qty = (balance * (config["riskPct"] / 100)) / (
                    ...
                    if qty < MIN_ORDER_VALUE:
                        log_message(
                            f"Calculated quantity {qty:.{config['qty_precision']}f} is less than minimum order value {MIN_ORDER_VALUE}. Order not placed.",
                            "warning",
                        )
                        continue  # Skip order placement if qty is too small

                    log_message(
                        f"Placing {side} order for {qty:.{config['qty_precision']}f} {config['symbol']}",
                        "info",
                    )
...
def start_bot():
    ...
    if instrument_info.get("retCode") == 0 and instrument_info["result"]["list"]:
        ...
        log_message(
            f"Failed to fetch instrument info: {instrument_info.get('retMsg')}. Using default precisions.",
            "warning",
        )
    ...
    lev_res = BOT_STATE["bybit_session"].set_leverage(
        category="linear",
        symbol=config["symbol"],
