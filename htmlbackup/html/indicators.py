...
def calculate_indicators(klines: list, config: dict) -> dict | None:
    ...
    for i in range(len(klines)):
        ...
        if i > 0:
            ...
            ):  # Changed from final_upper_band to final_lower_band
                direction = -1
            elif (
                prev_st_data["direction"] in [0, -1]
                and kline["close"] >= final_upper_band
            ):  # Changed from final_lower_band to final_upper_band
                direction = 1
            else:
    ...
    upper_band = [
        s + (std * bb_std_dev) for s, std in zip(sma_bb, std_dev_values, strict=False)
    ]
    lower_band = [
        s - (std * bb_std_dev) for s, std in zip(sma_bb, std_dev_values, strict=False)
    ]
    ...
    return {
        ...
        "fisher": final_fisher,
        "macd": final_macd,
        "bollinger_bands": final_bollinger_bands,
    }
