...
def fetch_historical_klines(
    ...
    while current_time > start_time:
        ...
        if res and res["retCode"] == 0 and res["result"]["list"]:
            ...
            fetched_klines = [
                k for k in fetched_klines if k["timestamp"] / 1000 >= start_time
            ]
...
def run_backtest(klines: list, config: dict) -> dict:
    ...
    for i in range(len(klines)):
        ...
        buy_signal = (
            ...
            and last_supertrend_direction == -1
        ...
        if buy_signal and current_position_side != "Buy":
            ...
            if current_position_side == "Sell":
                pnl = (entry_price - current_price) * position_size - (
                    position_size * current_price * config["fee_rate"]
                )
                balance += pnl
                trades.append(
                    {
            ...
            if stop_distance > 0:
                ...
                if (
                    qty * current_price > 10
                ):  # Minimum order size (e.g., 10 USDT) - this check is now redundant with MIN_ORDER_VALUE but kept for safety
                    position_size = qty
                    entry_price = current_price
                    current_position_side = "Buy"
                    balance -= (
                        position_size * entry_price * config["fee_rate"]
                    )  # Deduct entry fee
                    trades.append(
                        {
                            "side": "Buy",
                    ...
                    current_stop_loss_price = (
                        sl_price  # Initialize current stop loss price
                    )
        ...
        if current_position_side == "Buy" and position_size > 0:
            ...
            if (
                new_trailing_stop_price > current_stop_loss_price
                and new_trailing_stop_price > entry_price
            ):
            ...
            if current_price <= sl_price:
                pnl = (sl_price - entry_price) * position_size - (
                    position_size * sl_price * config["fee_rate"]
                )
                balance += pnl
                trades.append(
                    {
                ...
                pnl = (tp_price - entry_price) * position_size - (
                    position_size * tp_price * config["fee_rate"]
                )
                balance += pnl
                trades.append(
                    {
    ...
    if position_size > 0:
        pnl = 0
        if current_position_side == "Buy":
            pnl = (klines[-1]["close"] - entry_price) * position_size - (
                position_size * klines[-1]["close"] * config["fee_rate"]
            )
        else:
            pnl = (entry_price - klines[-1]["close"]) * position_size - (
                position_size * klines[-1]["close"] * config["fee_rate"]
            )
        balance += pnl
        trades.append(
