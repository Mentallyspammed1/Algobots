...
# --- Backtesting Engine ---
def run_backtest(klines, config):
    ...
    for i in range(len(klines)):
        ...
        if buy_signal and current_position_side != "Buy":
            ...
            if stop_distance > 0:
                qty = (risk_amount / stop_distance) * config['leverage']
                ...
                if qty * current_price > 10: # Minimum order size (e.g., 10 USDT)
                    position_size = qty
        ...
        if current_position_side == "Buy" and position_size > 0:
            sl_price = entry_price * (1 - config['stopLossPct'] / 100)
...
def optimize_strategy(klines, param_ranges):
    best_pnl = -float('inf')
    best_config = None
