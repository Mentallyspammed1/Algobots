...
try:
    ...
    def calculate_indicators(klines, config):
        ...
        return {
            ...
            'bollinger_bands': {'middle_band': klines[-1]['close'], 'upper_band': 0, 'lower_band': 0}
        }
...
class HistoricalDataFetcher:
    ...
    def fetch_historical_klines(self,
        ...
        while current_end > start_time:
            ...
            current_start = max(
                start_time,
                current_end - timedelta(minutes=interval_minutes * 1000)
            )
...
class Backtester:
    ...
    def _generate_signal(self, indicators: dict, config: dict) -> int:
        ...
        # Buy signal
        if (st.get('direction') == 1 and
            self.last_supertrend.get('direction', 0) == -1 and
    ...
    def _manage_position(self, candle: pd.Series, config: dict):
        ...
        # Check stop loss
        if (trade.side == 'Buy' and candle['low'] <= trade.stop_loss) or (trade.side == 'Sell' and candle['high'] >= trade.stop_loss):
            self._close_position(candle, "Stop Loss", trade.stop_loss)
            return

        # Check take profit
        if (trade.side == 'Buy' and candle['high'] >= trade.take_profit) or (trade.side == 'Sell' and candle['low'] <= trade.take_profit):
    ...
    def _calculate_statistics(self, config: dict) -> BacktestResult:
        ...
        # Expectancy
        expectancy = (avg_win * len(winning_trades) / total_trades -
                     abs(avg_loss) * len(losing_trades) / total_trades) if total_trades > 0 else 0
...
class StrategyOptimizer:
    ...
    def get_best_config(self, metric: str = 'sharpe_ratio') -> tuple[dict, BacktestResult]:
...
class BacktestVisualizer:
    ...
    def plot_equity_curve(result: BacktestResult, save_path: str | None = None):
        ...
        ax1.axhline(y=result.initial_balance, color='gray', linestyle='--', label='Initial Balance')
        ...
        # Drawdown
        ax2.fill_between(result.timestamps, result.drawdown_curve, 0,
    ...
    @staticmethod
    def plot_optimization_results(optimizer: StrategyOptimizer,
        ...
        sns.heatmap(pivot, annot=True, fmt='.2f', cmap='YlOrRd', cbar_kws={'label': metric})
