...
def test_calculate_supertrend(sample_dataframe):
    ...
    assert (
        df_st["supertrend"].head(period - 1).isnull().any()
    )  # At least one NaN in the first (period-1) values
    # Assert that there are some non-NaNs later
    assert (
        df_st["supertrend"].tail(len(df) - (period - 1)).notnull().any()
    )  # At least one non-NaN after (period-1) values

    assert (
        df_st["supertrend_direction"].dropna().isin([-1, 1]).all()
