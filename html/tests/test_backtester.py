...
class TestBacktester(unittest.TestCase):
    ...
    def test_run_backtest_sl_hit(self):
        ...
        for i in range(len(klines), 250):
            klines.append(
                {
                    "timestamp": klines[-1]["timestamp"] + 3600000,
                    "open": klines[-1]["close"],
                    "high": klines[-1]["close"] + 1,
        ...
        with patch("backtester.calculate_indicators") as mock_calculate_indicators:
            ...
            position_size_usdt = (
                self.test_config["initial_balance"]
                * (self.test_config["riskPct"] / 100)
                * self.test_config["leverage"]
            )
            position_size_coin = position_size_usdt / entry_price
            expected_loss = (sl_price - entry_price) * position_size_coin
            fee = (position_size_usdt * 2) * self.test_config[
                "fee_rate"
            ]  # Entry and exit fees
    ...
    def test_run_backtest_tp_hit(self):
        ...
        klines = [
            ...
            },  # Buy signal here
            {
                ...
                "close": 111.3,
                "volume": 1200,
            },  # Price rises, TP should be hit (106 * 1.05 = 111.3)
            {
                "timestamp": 1678897200000,
                "open": 111.3,
                "high": 115,
    ...
    def test_run_backtest_trailing_stop_hit(self):
        ...
        klines = [
            ...
            },  # Price rises, peak is 110
            {
                ...
                "close": 107.5,
                "volume": 1300,
            },  # Price falls, TSL should hit at 110 * (1 - 0.02) = 107.8
        ]
        for i in range(len(klines), 250):
            klines.append(
                {
