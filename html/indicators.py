...
def _calculate_bollinger_bands(closes: list[float], period: int, std_dev: float) -> dict:
    ...
    upper_band = [m + (s * std_dev) for m, s in zip(middle_band, std_dev_values, strict=False)]
    lower_band = [m - (s * std_dev) for m, s in zip(middle_band, std_dev_values, strict=False)]
