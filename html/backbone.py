...
def _make_api_call(
    ...
    for attempt in range(max_retries):
        try:
            if method == "get":
                response = (
                    getattr(api_client, endpoint)(**params)
                    if params
                    else getattr(api_client, endpoint)()
                )
...
def trading_bot_loop():
    ...
    while BOT_STATE.get("running"):
        try:
            ...
            if not indicators:
                log_message(
                    "Insufficient data for indicators. Waiting for more klines.",
                    "warning",
                )
            ...
            balance_res = _make_api_call(
                ...
                "get_wallet_balance",
                params={
                    "accountType": config.get("accountType", "UNIFIED"),
                    "coin": "USDT",
                },
            )
            ...
            with STATE_LOCK:
                dashboard["currentPrice"] = (
                    f"${current_price:.{config['price_precision']}f}"
                )
                st = indicators["supertrend"]
                dashboard["stDirection"] = (
                    "UPTREND" if st["direction"] == 1 else "DOWNTREND"
                )
                dashboard["stValue"] = (
                    f"{st['supertrend']:.{config['price_precision']}f}"
                )
                ...
                dashboard["bbMiddle"] = (
                    f"{indicators['bollinger_bands']['middle_band']:.2f}"
                )
                dashboard["bbUpper"] = (
                    f"{indicators['bollinger_bands']['upper_band']:.2f}"
                )
                dashboard["bbLower"] = (
                    f"{indicators['bollinger_bands']['lower_band']:.2f}"
                )
                if current_position:
                    dashboard["currentPosition"] = (
                        f"{current_position['side']} {current_position['size']}"
                    )
                    pnl = (
                        (current_price - float(current_position["avgPrice"]))
                        * float(current_position["size"])
                        if current_position["side"] == "Buy"
                        else (float(current_position["avgPrice"]) - current_price)
                        * float(current_position["size"])
                    )
            ...
            with STATE_LOCK:
                if BOT_STATE["current_position_info"]["order_id"] and current_position:
                    ...
                    if pos_info["side"] == "Buy":
                        new_trailing_stop_price = pos_info["peak_price"] * (
                            1 - trailing_stop_pct
                        )
                    else:  # Sell
                        new_trailing_stop_price = pos_info["peak_price"] * (
                    ...
                    current_sl_on_exchange = (
                        float(current_position.get("stopLoss", 0))
                        if current_position.get("stopLoss")
                        else 0.0
                    )
                    ...
                    if (
                        pos_info["side"] == "Buy"
                        and new_trailing_stop_price > current_sl_on_exchange
                        and new_trailing_stop_price > pos_info["entry_price"]
                    ) or (
                        pos_info["side"] == "Sell"
                        and new_trailing_stop_price < current_sl_on_exchange
                        and new_trailing_stop_price < pos_info["entry_price"]
                    ...
                    if amend_sl:
                        log_message(
                            f"Amending trailing stop for {pos_info['side']} position from {current_sl_on_exchange:.{config['price_precision']}f} to {new_trailing_stop_price:.{config['price_precision']}f}",
                            "info",
                        )
            ...
            with STATE_LOCK:
                buy_signal = (
                    st["direction"] == 1
                    and BOT_STATE["last_supertrend"]["direction"] == -1
                    and indicators["rsi"] < config["rsi_overbought"]
                    and fisher > 0
                )  # Ehlers-Fisher confirmation
                sell_signal = (
                    st["direction"] == -1
                    and BOT_STATE["last_supertrend"]["direction"] == 1
                    and indicators["rsi"] > config["rsi_oversold"]
                    and fisher < 0
                )  # Ehlers-Fisher confirmation
            ...
            if buy_signal or sell_signal:
                ...
                if current_position and current_position["side"] != side:
                    log_message(
                        f"Closing opposite {current_position['side']} position.",
                        "warning",
                    )
                    ...
                    if close_res.get("retCode") == 0:
                        ...
                        if (
                            ...
                            log_message(
                                f"Failed to refresh balance after closing position: {balance_res.get('retMsg')}",
                                "error",
                            )
                    else:
                        log_message(
                            f"Failed to close opposite position: {close_res.get('retMsg')}",
                            "error",
                        )
                ...
                with STATE_LOCK:
                    precision_mgr = BOT_STATE["precision_manager"]
                sl_price = (
                    current_price * (1 - config["stopLossPct"] / 100)
                    if side == "Buy"
                    else current_price * (1 + config["stopLossPct"] / 100)
                )
                tp_price = (
                    current_price * (1 + config["takeProfitPct"] / 100)
                    if side == "Buy"
                    else current_price * (1 - config["takeProfitPct"] / 100)
                )
                ...
                if stop_distance > 0:
                    ...
                    position_value_usdt = (balance * (config["riskPct"] / 100)) / (
                    ...
                    if position_value_usdt < MIN_ORDER_VALUE:
                        log_message(
                            f"Calculated position value {position_value_usdt:.2f} USDT is less than minimum {MIN_ORDER_VALUE} USDT. Order not placed.",
                            "warning",
                        )
                        continue
                    ...
                    if current_price <= 0:
                        log_message(
                            f"Invalid current_price ({current_price}) for quantity calculation.",
                            "error",
                        )
                        continue
                    ...
                    log_message(
                        f"Calculated position: {position_value_usdt:.2f} USDT -> {qty_in_base_currency:.{config['qty_precision']}f} {config['symbol']}",
                        "info",
                    )
                    log_message(
                        f"Placing {side} order for {qty_in_base_currency:.{config['qty_precision']}f} {config['symbol']}",
                        "info",
                    )
        ...
        if interval_str.isdigit():
            ...
            log_message(
                f"Invalid interval format: {interval_str}. Defaulting to 60s.",
                "warning",
            )
...
def start_bot():
    ...
    balance_check = _make_api_call(
        ...
        "get",
        "get_wallet_balance",
        params={"accountType": config.get("accountType", "UNIFIED"), "coin": "USDT"},
    )
    ...
    position_info_res = _make_api_call(
        BOT_STATE["bybit_session"],
        "get",
        "get_positions",
    ...
    if position_info_res.get("retCode") == 0 and position_info_res["result"]["list"]:
        ...
        if current_leverage == leverage:
            log_message(
                f"Leverage is already set to {leverage}x for {config['symbol']}. Skipping.",
                "info",
            )
