...
import numpy as np
import pytest
from indicators_api import calculate_ema, calculate_rsi, calculate_macd, calculate_ehlers_fisher_transform, calculate_supertrend

# Fixture for a basic DataFrame
@pytest.fixture
def sample_dataframe():
    data = {
        'open': [10, 12, 15, 13, 16, 18, 17, 20, 22, 21, 25, 24, 28, 30, 29, 32, 35, 33, 36, 38],
        'high': [13, 16, 17, 16, 19, 20, 21, 23, 24, 23, 27, 26, 30, 32, 31, 34, 37, 36, 39, 40],
        'low': [9, 11, 12, 12, 14, 16, 15, 18, 20, 19, 22, 21, 25, 27, 26, 29, 31, 30, 33, 35],
        'close': [12, 15, 13, 16, 18, 17, 20, 22, 21, 25, 24, 28, 30, 29, 32, 35, 33, 36, 38, 37]
    }
...
def test_calculate_ema_basic(sample_dataframe):
    ...
    assert f'ema_3' in df_result.columns
    assert not df_result[f'ema_3'].isnull().all() # Should have some non-NaN values
    ...
    assert not np.isnan(df_result[f'ema_3'].iloc[-1])
...
def test_calculate_ema_insufficient_data(insufficient_dataframe):
    ...
    assert f'ema_10' in df_result.columns
    assert df_result[f'ema_10'].isnull().all()
...
def test_calculate_rsi_basic(sample_dataframe):
    ...
    assert f'rsi_3' in df_result.columns
    assert not df_result[f'rsi_3'].isnull().all()
    assert (df_result[f'rsi_3'].dropna() >= 0).all() and (df_result[f'rsi_3'].dropna() <= 100).all()
...
def test_calculate_rsi_insufficient_data(insufficient_dataframe):
    ...
    assert f'rsi_10' in df_result.columns
    assert df_result[f'rsi_10'].isnull().all()
...
def test_calculate_ehlers_fisher_transform_insufficient_data(insufficient_dataframe):
    df_result = calculate_ehlers_fisher_transform(insufficient_dataframe.copy(), period=10)
...
def test_calculate_ehlers_fisher_transform_value_error(sample_dataframe):
    with pytest.raises(ValueError):
        calculate_ehlers_fisher_transform(sample_dataframe.copy(), column='non_existent_column')
...
def test_calculate_supertrend_trend_change():
    ...
    assert 1 in directions and -1 in directions, "Supertrend direction should show both uptrend and downtrend."