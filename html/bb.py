    ...
    def update_metrics(self, trade_result: dict[str, Any]):
        ...
        if pnl > 0:
            ...
            self.max_consecutive_wins = max(
                self.max_consecutive_wins, self.current_streak
            )
        ...
        self.expectancy = (self.win_rate / 100.0) * self.avg_win - (
            1 - self.win_rate / 100.0
        ) * self.avg_loss
...
class PerformanceMonitor:
    ...
    _response_times: collections.deque = field(
    ...
    def capture_response_time(self, ms: float):
        ...
        if self._response_times:
            self.avg_api_response_ms = sum(self._response_times) / len(
    ...
    def get_health_status(self) -> dict[str, Any]:
        ...
        error_rate = (
            (self.api_errors_count / self.api_calls_count * 100.0)
            if self.api_calls_count
            else 0.0
        )
        status = "healthy"
...
class RiskManager:
    ...
    def check_daily_limits(self, account_balance: float) -> tuple[bool, str]:
        ...
        daily_loss_pct = (
            abs(self.daily_loss / account_balance * 100) if account_balance > 0 else 0
        )
        weekly_loss_pct = (
            abs(self.weekly_loss / account_balance * 100) if account_balance > 0 else 0
        )
    ...
    def calculate_position_size(
        ...
        vol_adjust = max(
            0.5, min(1.5, 1.0 / max(volatility_pct, 0.001))
        )  # between 0.5x and 1.5x
        adjusted_risk_pct = max(
            0.25, min(2.0, (risk_pct * self.risk_multiplier * vol_adjust))
        )
        base_position_size = (account_balance * adjusted_risk_pct / 100.0) / (
            max(1e-9, stop_distance_pct) / 100.0
        )
...
class MarketAnalyzer:
    ...
    def detect_condition(klines: list[dict], rsi: float) -> MarketCondition:
        ...
        change_pct = (
            (klines[-1]["close"] - klines[-50]["close"]) / klines[-50]["close"] * 100.0
        )
...
class BotState:
    ...
    trade_history: dict[str, int | list] = field(
        default_factory=lambda: {"wins": 0, "losses": 0, "history": []}
    )
    ...
    last_supertrend: dict[str, int | float] = field(
        default_factory=lambda: {"direction": 0, "value": 0}
    )
    current_position_info: dict[str, str | float | int | None] = field(
        default_factory=lambda: {
            "order_id": None,
            "entry_price": None,
            "side": None,
...
class PrecisionManager:
        ...
        for category in categories:
            try:
                response = self._make_api_call(
                    "get", "get_instruments_info", params={"category": category}
                )
                if response and response["retCode"] == 0:
                    for inst in response["result"]["list"]:
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"Could not parse specs for {symbol} ({category}): {parse_e}"
                            )
    ...
    def _make_api_call(
        ...
        for attempt in range(max_retries):
            try:
                if method == "get":
                    response = (
                        getattr(self.session, endpoint)(**params)
                        if params
                        else getattr(self.session, endpoint)()
                    )
                ...
                self.logger.warning(
                    f"Bybit API Error ({ret_code}): {ret_msg} for {endpoint}. Retrying in {initial_delay * (2**attempt)}s... (Attempt {attempt + 1})"
                )
    ...
    def round_price(self, symbol: str, price: float | Decimal) -> Decimal:
        ...
        if symbol not in self.instruments:
            self.logger.warning(
                f"Symbol {symbol} not found, using default price precision (0.01)."
            )
    ...
    def round_quantity(self, symbol: str, quantity: float | Decimal) -> Decimal:
        ...
        if symbol not in self.instruments:
            self.logger.warning(
                f"Symbol {symbol} not found, using default quantity precision (0.001)."
            )
    ...
    def get_decimal_places(self, symbol: str) -> tuple[int, int]:
        ...
        if symbol not in self.instruments:
            self.logger.warning(
                f"Symbol {symbol} not found for decimal places, returning defaults (2, 3)."
            )
            return 2, 3
        specs = self.instruments[symbol]
        price_decimals = (
            abs(specs.tick_size.as_tuple().exponent) if specs.tick_size != 0 else 2
        )
        qty_decimals = (
            abs(specs.qty_step.as_tuple().exponent) if specs.qty_step != 0 else 3
        )
...
try:
    ...
    logger.error(
        "Could not import 'calculate_indicators' from 'indicators.py'. Using a dummy fallback."
    )
    ...
    def calculate_indicators(klines, config):
        ...
        return {
            "supertrend": {
                "direction": 1 if len(klines) % 2 else -1,
    ...
    while BOT_STATE.running:
        try:
            ...
            if not session or not precision_mgr:
                log_message(
                    "Session or Precision Manager not initialized. Stopping bot thread.",
                    "error",
                )
            ...
            if not klines_res or klines_res.get("retCode") != 0:
                msg = (
                    klines_res.get("retMsg", "Unknown API error")
                    if klines_res
                    else "No response"
                )
            ...
            klines = sorted(
                [
                    ...
                    for k in klines_res["result"]["list"]
                ],
            ...
            if len(klines) >= 2:
                dashboard["priceChange"] = (
                    f"{((klines[-1]['close'] - klines[-2]['close']) / klines[-2]['close'] * 100):.2f}%"
                )
            if len(klines) >= 1440 // max(
                1, int(interval) if str(interval).isdigit() else 60
            ):
                start_idx = max(
                    0,
                    len(klines)
                    - (
                        1440 // max(1, int(interval) if str(interval).isdigit() else 60)
                    ),
                )
                first = klines[start_idx]["close"]
                dashboard["priceChange24h"] = (
                    f"{((current_price - first) / first * 100):.2f}%"
                )
                dashboard["volume24h"] = (
                    f"{sum(k['volume'] for k in klines[start_idx:]):.2f}"
                )
            ...
            if not indicators:
                log_message(
                    "Indicator calculation failed or returned no data. Waiting.",
                    "warning",
                )
            ...
            position_res = _make_api_call(
                session,
                "get",
                "get_positions",
                params={"category": "linear", "symbol": symbol},
            )
            balance_res = _make_api_call(
                session,
                "get",
                "get_wallet_balance",
            ...
            if position_res and position_res.get("retCode") == 0:
                ...
                pos_list = [
                    p
                    for p in position_res["result"]["list"]
                ...
                msg = (
                    position_res.get("retMsg", "Unknown API error")
                    if position_res
                    else "No response"
                )
            ...
            if (
                balance_res
                and balance_res.get("retCode") == 0
                and balance_res.get("result", {}).get("list")
            ):
                usdt_entry = balance_res["result"]["list"][0]
                account_balance = float(
                    usdt_entry.get("totalWalletBalance")
                    or usdt_entry.get("totalEquity")
                    or 0
                )
                available_balance = float(usdt_entry.get("availableBalance") or 0)
            else:
                msg = (
                    balance_res.get("retMsg", "Unknown API error")
                    if balance_res
                    else "No response"
                )
            ...
            dashboard["stDirection"] = (
                "UP" if st_dir == 1 else "DOWN" if st_dir == -1 else "SIDEWAYS"
            )
            dashboard["stValue"] = (
                f"{st_val:.{price_prec}f}" if st_val is not None else "---"
            )
            ...
            dashboard["macdLine"] = (
                f"{macd.get('macd_line', 0.0):.2f}" if "macd_line" in macd else "---"
            )
            dashboard["macdSignal"] = (
                f"{macd.get('signal_line', 0.0):.2f}"
                if "signal_line" in macd
                else "---"
            )
            dashboard["macdHistogram"] = (
                f"{macd.get('histogram', 0.0):.2f}" if "histogram" in macd else "---"
            )
            bb = indicators.get("bollinger_bands", {})
            dashboard["bbMiddle"] = (
                f"{bb.get('middle_band', 0.0):.{price_prec}f}"
                if "middle_band" in bb
                else "---"
            )
            dashboard["bbUpper"] = (
                f"{bb.get('upper_band', 0.0):.{price_prec}f}"
                if "upper_band" in bb
                else "---"
            )
            dashboard["bbLower"] = (
                f"{bb.get('lower_band', 0.0):.{price_prec}f}"
                if "lower_band" in bb
                else "---"
            )
            ...
            dashboard["signalStrength"] = (
                "N/A"  # placeholder for custom scoring if needed
            )
            dashboard["healthStatus"] = (
                BOT_STATE.performance_monitor.get_health_status().get(
                    "status", "unknown"
                )
            )
            ...
            if current_position:
                pnl_unreal = (
                    (current_price - entry_price) * open_position_size
                    if pos_side == "Buy"
                    else (entry_price - current_price) * open_position_size
                )
                pnl_pct = (
                ...
                dashboard["currentPosition"] = (
                    f"{pos_side} {open_position_size} @ {entry_price}"
                )
            ...
            if BOT_STATE.current_position_info["order_id"] and current_position:
                ...
                current_sl_on_exchange = float(
                    current_position.get("stopLoss", 0) or 0.0
                )

                amend_sl = False
                if (
                    pos_info["side"] == "Buy"
                    and new_sl > current_sl_on_exchange
                    and new_sl > pos_info["entry_price"]
                ) or (
                    pos_info["side"] == "Sell"
                    and (
                        current_sl_on_exchange == 0.0 or new_sl < current_sl_on_exchange
                    )
                    and new_sl < pos_info["entry_price"]
                ):
                    amend_sl = True
                ...
                if entry_time and BOT_STATE.risk_manager.check_position_timeout(
                    entry_time
                ):
                    log_message(
                        f"Position timeout reached. Closing position {pos_side}.",
                        "warning",
                    )
                    close_res = _make_api_call(
                        session,
                        "post",
                        "place_order",
                        params={
                    ...
                    if close_res and close_res.get("retCode") == 0:
                        ...
                        BOT_STATE.trade_analytics.update_metrics(
                            {
                                "pnl": closed_pnl,
                                "fees": 0.0,
                                "duration_hours": (
                        ...
                        continue
                    log_message(
                        f"Failed to close position on timeout: {close_res.get('retMsg', 'Unknown API error') if close_res else 'No response'}",
                        "error",
                    )

                if amend_sl:
                    log_message(
                        f"Amending trailing stop for {pos_info['side']} from {current_sl_on_exchange:.{price_prec}f} to {new_sl:.{price_prec}f}",
                        "info",
                    )
                    amend_res = _make_api_call(
                        session,
                        "post",
                        "amend_order",
                        params={
                    ...
                    if amend_res and amend_res.get("retCode") == 0:
                        ...
                        msg = (
                            amend_res.get("retMsg", "Unknown API error")
                            if amend_res
                            else "No response"
                        )
            ...
            buy_signal = (
                st_dir == 1
                and BOT_STATE.last_supertrend.get("direction", 0) == -1
                and rsi < config.get("rsi_overbought", 70)
                and fisher > fisher_thr
            )
            sell_signal = (
                st_dir == -1
                and BOT_STATE.last_supertrend.get("direction", 0) == 1
            ...
            within_limits, reason = BOT_STATE.risk_manager.check_daily_limits(
                account_balance
            )
            if not within_limits:
                ...
                if current_position and pos_side != side:
                    ...
                    if close_res and close_res.get("retCode") == 0:
                        ...
                        BOT_STATE.trade_analytics.update_metrics(
                            {
                                ...
                                "duration_hours": (
                                    datetime.now()
                                    - (
                                        BOT_STATE.current_position_info.get(
                                            "entry_time"
                                        )
                                        or datetime.now()
                                    )
                        ...
                        msg = (
                            close_res.get("retMsg", "Unknown API error")
                            if close_res
                            else "No response"
                        )
                        log_message(
                            f"Failed to close opposite position: {msg}", "error"
                        )
                        continue
                ...
                if stop_distance <= 0:
                    log_message(
                        "Stop loss distance is zero or negative. Cannot place order.",
                        "error",
                    )
                    continue

                stop_distance_pct = (
                    (stop_distance / current_price) * 100.0
                    if current_price > 0
                    else 0.0
                )
                risk_pct = float(config.get("riskPct", 1.0))
                position_value_usdt = BOT_STATE.risk_manager.calculate_position_size(
                    account_balance, risk_pct, stop_distance_pct, vol_pct
                )
                ...
                if position_value_usdt < MIN_ORDER_VALUE_USDT or current_price <= 0:
                    log_message(
                        f"Position value {position_value_usdt:.2f} USDT below min {MIN_ORDER_VALUE_USDT} or invalid price. Skipping.",
                        "warning",
                    )
                    continue
                ...
                if rounded_qty <= 0:
                    log_message(
                        "Rounded quantity is zero after precision adjustment. Skipping order.",
                        "warning",
                    )
                    continue

                log_message(
                    f"Calculated position: {position_value_usdt:.2f} USDT -> {rounded_qty:.{config['qty_precision']}f} {symbol}",
                    "info",
                )

                order_res = _make_api_call(
                    session,
                    "post",
                    "place_order",
                    params={
                ...
                if order_res and order_res.get("retCode") == 0:
                    ...
                    msg = (
                        order_res.get("retMsg", "Unknown API error")
                        if order_res
                        else "No response"
                    )
            ...
            dashboard["profitFactor"] = (
                "∞" if ta.profit_factor == float("inf") else f"{ta.profit_factor:.2f}"
            )
            ...
            log_message(
                f"An unexpected error occurred in the trading loop: {e}", "error"
            )
            dashboard = BOT_STATE.dashboard
        ...
        if interval_str.isdigit():
            ...
            log_message(
                f"Unsupported interval format: '{interval_str}'. Defaulting to 60s.",
                "warning",
            )
            interval_seconds = 60

        next_kline_start_ts = (
            last_kline_ts_sec // interval_seconds + 1
        ) * interval_seconds
        sleep_duration = next_kline_start_ts - now
        BOT_STATE.dashboard["nextCandle"] = datetime.utcfromtimestamp(
            next_kline_start_ts
        ).strftime("%Y-%m-%d %H:%M:%S UTC")

        if sleep_duration > 0:
            log_message(
                f"Waiting for {sleep_duration:.2f} seconds until next candle.", "info"
            )
            time.sleep(sleep_duration)
        else:
            log_message(
                f"Processing took longer than interval ({abs(sleep_duration):.2f}s over). Continuing immediately.",
                "warning",
            )
            time.sleep(1)
...
def _make_api_call(
    ...
    for attempt in range(max_retries):
        try:
            ...
            BOT_STATE.performance_monitor.last_api_error = ret_msg
            log_message(
                f"Bybit API Error ({ret_code}): {ret_msg}. Retrying {endpoint} in {initial_delay * (2**attempt)}s... (Attempt {attempt + 1})",
                "warning",
            )
            time.sleep(initial_delay * (2**attempt))
        except Exception as e:
...
def start_bot():
    ...
    if not all(param in config for param in required_params):
        ...
        return jsonify(
            {
                "status": "error",
                "message": f"Missing required configuration parameters: {', '.join(missing)}",
            }
        ), 400
    ...
    if not BYBIT_API_KEY or not BYBIT_API_SECRET:
        log_message(
            "CRITICAL: Bybit API Key or Secret not found in backend .env file.", "error"
        )
        return jsonify(
            {
                "status": "error",
    ...
    config["takeProfitPct"] = max(
        0.1, min(20.0, float(config.get("takeProfitPct", 2.0)))
    )
    config["trailingStopPct"] = max(
        0.1, min(5.0, float(config.get("trailingStopPct", 0.5)))
    )
    ...
    BOT_STATE.config["fisher_threshold"] = config.get(
        "fisher_threshold", 0.5
    )  # Add Fisher threshold
    ...
    try:
        BOT_STATE.bybit_session = HTTP(
            testnet=testnet, api_key=BYBIT_API_KEY, api_secret=BYBIT_API_SECRET
        )
        BOT_STATE.precision_manager = PrecisionManager(BOT_STATE.bybit_session, logger)

        # Verify API connection by fetching balance
        balance_check = _make_api_call(
            BOT_STATE.bybit_session,
            "get",
            "get_wallet_balance",
            params={"accountType": "UNIFIED", "coin": "USDT"},
        )
        if not balance_check or balance_check.get("retCode") != 0:
            log_message(
                f"API connection failed: {balance_check.get('retMsg', 'Unknown API error') if balance_check else 'No response'}",
                "error",
            )
            ...
            return jsonify(
                {
                    "status": "error",
                    "message": f"API connection failed: {balance_check.get('retMsg', 'Unknown API error') if balance_check else 'No response'}",
                }
            ), 400
        ...
        precision_mgr = BOT_STATE.precision_manager
        price_precision, qty_precision = precision_mgr.get_decimal_places(
            config["symbol"]
        )
        ...
        log_message(
            f"Fetched instrument info: Price Precision={price_precision}, Quantity Precision={qty_precision}",
            "info",
        )
        ...
        if symbol_specs:
            ...
            log_message(
                f"Adjusted leverage to {leverage}x based on instrument specs.", "info"
            )

        lev_res = _make_api_call(
            BOT_STATE.bybit_session,
            "post",
            "set_leverage",
            params={
        ...
        if lev_res and lev_res.get("retCode") == 0:
            ...
            log_message(
                f"Failed to set leverage: {lev_res.get('retMsg', 'Unknown API error') if lev_res else 'No response'}",
                "warning",
            )
        ...
        return jsonify(
            {"status": "error", "message": f"Failed to start bot: {e!s}"}
        ), 500
...
def get_gemini_insight():
    ...
    try:
        ...
        model = genai.GenerativeModel(
            ...
            system_instruction="You are a helpful trading assistant. Provide concise and actionable insights.",
        )
        ...
        if not getattr(response, "text", None):
            return jsonify(
                {
                    "status": "error",
                    "message": "Gemini returned an empty response. Please try again or rephrase your prompt.",
                }
            ), 500
        ...
        if not BOT_STATE.precision_manager:
            if not BYBIT_API_KEY or not BYBIT_API_SECRET:
                return jsonify(
                    {
                        "status": "error",
                        "message": "Bybit API Key/Secret not configured. Cannot fetch symbols.",
                    }
                ), 500
            session = HTTP(
...
if __name__ == "__main__":
    ...
    if not GEMINI_API_KEY:
        logger.warning(
            "Gemini API Key not found. The insight feature will be disabled."
        )
    else:
        try:
