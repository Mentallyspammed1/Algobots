...
def load_config(filepath: str) -> Dict[str, Any]:
    ...
    default_config = {
        # Core Settings
        "symbol": "BTCUSDT", "interval": "15m", "loop_delay": LOOP_DELAY_SECONDS, "orderbook_limit": 50,
        # Signal Generation
        ...
        "indicator_settings": {
            ...
            "cci_period": 20, "williams_r_period": 14, "mfi_period": 14,
            "psar_acceleration": 0.02, "psar_max_acceleration": 0.2,
            "sma_short_period": 10, "sma_long_period": 50, "fibonacci_window": 60,
            "ehlers_fast_period": 10, "ehlers_fast_multiplier": 2.0,
            "ehlers_slow_period": 20, "ehlers_slow_multiplier": 3.0,
            "rsi_oversold": 30, "rsi_overbought": 70, "stoch_rsi_oversold": 20, "stoch_rsi_overbought": 80,
            "cci_oversold": -100, "cci_overbought": 100, "williams_r_oversold": -80, "williams_r_overbought": -20,
        ...
        "weight_sets": {
            "default_scalping": {
                "ema_alignment": 0.25, "sma_trend_filter": 0.3, "momentum": 0.2, "volume_confirmation": 0.15,
                "stoch_rsi": 0.4, "rsi": 0.2, "bollinger_bands": 0.3, "vwap": 0.2, "cci": 0.15, "wr": 0.15,
                "psar": 0.2, "sma_10": 0.1, "mfi": 0.2, "orderbook_imbalance": 0.1,
                "ehlers_supertrend_alignment": 0.6,
                "mtf_trend_confluence": 0.4,
            }
        },
    }
...
def fetch_klines(symbol: str, interval: str, limit: int, logger: logging.Logger) -> Optional[pd.DataFrame]:
...
class PositionManager:
    ...
    def open_position(self, signal: str, current_price: Decimal, atr_value: Decimal) -> Optional[Dict]:
        ...
        position = {
            ...
            "qty": order_qty,
            "stop_loss": stop_loss.quantize(Decimal('0.01'), rounding=ROUND_DOWN),
            "take_profit": take_profit.quantize(Decimal('0.01'), rounding=ROUND_DOWN),
            "status": "OPEN"
        }
...
class TradingAnalyzer:
    ...
    def _safe_calculate(self, func: callable, name: str, min_data_points: int = 0, *args, **kwargs) -> Optional[Any]:
    ...
    def calculate_fibonacci_levels(self) -> None:
        ...
        self.fib_levels = {
            ...
            "23.6%": Decimal(str(recent_high - 0.236 * diff)).quantize(Decimal('0.01'), rounding=ROUND_DOWN),
            "38.2%": Decimal(str(recent_high - 0.382 * diff)).quantize(Decimal('0.01'), rounding=ROUND_DOWN),
            "50.0%": Decimal(str(recent_high - 0.500 * diff)).quantize(Decimal('0.01'), rounding=ROUND_DOWN),
            "61.8%": Decimal(str(recent_high - 0.618 * diff)).quantize(Decimal('0.01'), rounding=ROUND_DOWN),
            "78.6%": Decimal(str(recent_high - 0.786 * diff)).quantize(Decimal('0.01'), rounding=ROUND_DOWN),
...
def main() -> None:
    ...
    while True:
        try:
            ...
            df = fetch_klines(config['symbol'], config['interval'], 200, logger)
            ...
            if config["mtf_analysis"]["enabled"]:
                for htf_interval in config["mtf_analysis"]["higher_timeframes"]:
                    ...
                    htf_df = fetch_klines(config['symbol'], htf_interval, 200, logger)
