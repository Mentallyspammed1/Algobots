...
class OptimizationLogger:
    def __init__(self, name="optimizer", log_dir="optimization_logs"):
        ...
        if not self.logger.hasHandlers():
            ...
            console_handler.setLevel(getattr(logging, BOT_CONFIG.get("LOG_LEVEL", "INFO")))
...
class PerformanceMetrics:
    total_pnl: float; num_trades: int; win_rate: float; sharpe_ratio: float
    sortino_ratio: float; max_drawdown: float; max_drawdown_duration: int
    calmar_ratio: float; profit_factor: float; avg_win: float; avg_loss: float
    largest_win: float; largest_loss: float; consecutive_wins: int; consecutive_losses: int
    recovery_factor: float; risk_reward_ratio: float; expectancy: float; var_95: float
    cvar_95: float; kelly_criterion: float; annual_return: float; annual_volatility: float
...
class MarketRegimeDetector:
    ...
    def detect_regime(df: pd.DataFrame, lookback: int = 50) -> MarketRegime:
        if len(df) < lookback: return MarketRegime.RANGING
        ...
        if adx_val > 25:
            ...
            return MarketRegime.TRENDING_UP if sma_short > sma_long else MarketRegime.TRENDING_DOWN
...
class CacheManager:
    def __init__(self):
        ...
        if self.use_redis:
            try:
                self.cache = redis.Redis(host='localhost', port=6379, db=0, decode_responses=False)
                self.cache.ping(); opt_logger.info("Connected to Redis cache.")
            except Exception:
                opt_logger.warning("Redis unavailable, falling back to memory cache."); self.use_redis = False; self.cache = {}
        else: self.cache = {}; opt_logger.info("Using in-memory cache.")
    def get(self, key: str): return pickle.loads(self.cache.get(key)) if self.cache.get(key) else None
    def set(self, key: str, value: Any, expire: int = 3600):
        if self.use_redis: self.cache.setex(key, expire, pickle.dumps(value))
        else: self.cache[key] = value
    ...
    def save_run(self, data: dict) -> str:
        ...
        with self.conn:
            self.conn.execute("INSERT INTO runs (id, symbol, timeframe, start, end, params, metrics) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (run_id, data['symbol'], data['timeframe'], data['start_date'], data['end_date'], json.dumps(data['best_params']), json.dumps(asdict(data['best_metrics']))))
        return run_id
...
class AdvancedBacktester:
    ...
    def calculate_metrics(self, trades: list[dict], equity_curve: list[float]) -> PerformanceMetrics:
        ...
        equity_array = np.array(equity_curve); returns = np.diff(equity_array) / equity_array[:-1]; returns = returns[np.isfinite(returns)]
        sharpe = np.sqrt(252) * np.mean(returns) / np.std(returns) if len(returns) > 1 and np.std(returns) > 0 else 0
        down_returns = returns[returns < 0]; down_dev = np.std(down_returns)
        sortino = np.sqrt(252) * np.mean(returns) / down_dev if len(down_returns) > 1 and down_dev > 0 else 0
        peak = np.maximum.accumulate(equity_array); drawdown = (equity_array - peak) / peak
        max_dd = np.min(drawdown) if len(drawdown) > 0 else 0
        annual_return = (equity_array[-1] / equity_array[0]) ** (252 / len(equity_array)) - 1 if len(equity_array) > 1 else 0
        calmar = annual_return / abs(max_dd) if max_dd != 0 else 0
        gross_profit = sum(wins); gross_loss = abs(sum(losses))
        profit_factor = gross_profit / gross_loss if gross_loss > 0 else 0
        avg_win = np.mean(wins) if len(wins) > 0 else 0; avg_loss = np.mean(losses) if len(losses) > 0 else 0
        risk_reward = abs(avg_win / avg_loss) if avg_loss != 0 else 0
        expectancy = (win_rate * avg_win) - ((1 - win_rate) * abs(avg_loss))
        kelly = (win_rate * risk_reward - (1 - win_rate)) / risk_reward if risk_reward > 0 else 0
        return PerformanceMetrics(
            total_pnl=np.sum(pnls), num_trades=num_trades, win_rate=win_rate, sharpe_ratio=sharpe, sortino_ratio=sortino,
            max_drawdown=max_dd, max_drawdown_duration=0, calmar_ratio=calmar, profit_factor=profit_factor,
            avg_win=avg_win, avg_loss=avg_loss, largest_win=np.max(wins) if len(wins)>0 else 0,
            largest_loss=np.min(losses) if len(losses)>0 else 0, consecutive_wins=0, consecutive_losses=0,
            recovery_factor=np.sum(pnls) / (abs(max_dd) * self.initial_capital) if max_dd != 0 else 0,
            risk_reward_ratio=risk_reward, expectancy=expectancy,
    ...
    def run_backtest(self, df: pd.DataFrame, params: dict, symbol: str, price_precision: int, qty_precision: int) -> tuple[PerformanceMetrics, list[dict], list[float]]:
        ...
        if len(df) < min_candles: return self.calculate_metrics([], [capital]), [], [capital]
        df_indicators = calculate_ehl_supertrend_indicators(df.copy(), temp_config)
        for i in range(min_candles, len(df_indicators)):
            current_candle = df_indicators.iloc[i]; current_price = current_candle['Close']
            # Manage existing trades (SL/TP checks)
            for trade_symbol in list(active_trades.keys()):
                trade = active_trades[trade_symbol]; exit_price, exit_reason = None, None
                if (trade['side'] == 'Buy' and current_candle['Low'] <= trade['sl']) or (trade['side'] == 'Sell' and current_candle['High'] >= trade['sl']):
                    exit_price, exit_reason = trade['sl'], 'SL'
                elif trade['tp'] and ((trade['side'] == 'Buy' and current_candle['High'] >= trade['tp']) or (trade['side'] == 'Sell' and current_candle['Low'] <= trade['tp'])):
                    exit_price, exit_reason = trade['tp'], 'TP'
                if exit_price:
                    slipped_exit = exit_price * (1 - self.slippage_pct) if trade['side'] == 'Buy' else exit_price * (1 + self.slippage_pct)
                    pnl = calculate_pnl(trade['side'], trade['entry_price'], slipped_exit, trade['qty'])
                    commission = (trade['qty'] * trade['entry_price'] * self.commission_rate) + (trade['qty'] * slipped_exit * self.commission_rate)
                    net_pnl = pnl - commission; capital += net_pnl
                    closed_trades.append({**trade, 'pnl': net_pnl, 'exit_reason': exit_reason}); del active_trades[trade_symbol]
            # Generate new trade signals
            if symbol not in active_trades:
                signal, risk, tp, sl, _, _ = generate_ehl_supertrend_signals(df_indicators.iloc[:i+1], current_price, 0, 0, price_precision, qty_precision, temp_config)
                if signal != 'none' and risk and risk > 0:
                    entry_price = current_price * (1 + self.slippage_pct) if signal == 'Buy' else current_price * (1 - self.slippage_pct)
                    qty = round((capital * temp_config.get("RISK_PER_TRADE_PCT", 0.01)) / risk, qty_precision)
                    if qty > 0: active_trades[symbol] = {'side': signal, 'entry_price': entry_price, 'qty': qty, 'sl': sl, 'tp': tp}
            equity_curve.append(capital + sum(calculate_pnl(t['side'], t['entry_price'], current_price, t['qty']) for t in active_trades.values()))
        return self.calculate_metrics(closed_trades, equity_curve), closed_trades, equity_curve
...
class OptimizationEngine:
    ...
    def bayesian(self, df, space, symbol, pp, qp, objective='sharpe_ratio', n_trials=100) -> tuple[dict, PerformanceMetrics, list[dict]]:
        def objective_func(trial):
            ...
            return getattr(metrics, objective) if not np.isnan(getattr(metrics, objective)) else -1
        study = optuna.create_study(direction='maximize'); study.optimize(objective_func, n_trials=n_trials)
        best_params = study.best_params
        metrics, trades, _ = self.backtester.run_backtest(df, best_params, symbol, pp, qp)
...
class WalkForwardAnalyzer:
    ...
    def analyze(self, df, param_grid, symbol, pp, qp, n_splits=5) -> list[dict]:
        ...
        for fold, (train_idx, test_idx) in enumerate(tscv.split(df)):
            ...
            best_params, _, _ = self.optimizer.bayesian(train_df, param_grid, symbol, pp, qp, n_trials=50)
            test_metrics, test_trades, _ = self.optimizer.backtester.run_backtest(test_df, best_params, symbol, pp, qp)
            results.append({'fold': fold, 'best_params': best_params, 'test_metrics': asdict(test_metrics)})
        return results
...
class NotificationManager:
    def format_html_report(self, run_data: dict) -> str:
        ...
        params_html = json.dumps(params, indent=2).replace("\n", "<br>").replace(" ", "&nbsp;")
        return f"""
        <html><body>
    ...
    def send_email(self, subject: str, html_content: str):
        ...
        if not cfg: opt_logger.warning("Email config not found."); return
        msg = MIMEMultipart(); msg['From'] = cfg['sender']; msg['To'] = cfg['recipient']; msg['Subject'] = subject
        msg.attach(MIMEText(html_content, 'html'))
        try:
            with smtplib.SMTP(cfg['smtp_server'], cfg['smtp_port']) as server:
                server.starttls(); server.login(cfg['username'], cfg['password']); server.send_message(msg)
            opt_logger.info("Email notification sent successfully.")
        except Exception as e: opt_logger.error(f"Failed to send email: {e}")
...
class VisualizationSuite:
    ...
    def generate_report(self, run_data: dict, equity_curve: list[float], trades: list[dict]):
        ...
        # Equity Curve
        axes[0].plot(equity_curve, color='#08F7FE', lw=2); axes[0].set_title(f"Equity Curve for {run_data['symbol']}", fontsize=18)
        # Drawdown
        equity = pd.Series(equity_curve); peak = equity.expanding(min_periods=1).max()
        drawdown = (equity - peak) / peak
        axes[1].fill_between(drawdown.index, drawdown*100, 0, color='#FE53BB', alpha=0.6)
        axes[1].set_title("Drawdown (%)", fontsize=14)
        report_path = os.path.join(self.report_dir, f"report_{run_data['symbol']}_{datetime.date.today()}.png")
        plt.savefig(report_path, dpi=150, bbox_inches='tight'); plt.close()
        opt_logger.info(f"Visual report saved to {report_path}")

# --- 10. Main Orchestrator ---
class MainOrchestrator:
    def __init__(self, symbol, timeframe, data_path):
        self.symbol = symbol; self.timeframe = timeframe; self.data_path = data_path
        self.df = pd.read_csv(data_path, index_col=0, parse_dates=True)
        self.db = OptimizationDatabase(); self.backtester = AdvancedBacktester()
        self.optimizer = OptimizationEngine(self.backtester)
        self.notifier = NotificationManager(); self.visualizer = VisualizationSuite()
    ...
    def run(self, mode: str, param_space: dict, n_trials: int = 100):
        ...
        if mode == 'bayesian':
            best_params, best_metrics, trades = self.optimizer.bayesian(self.df, param_space, self.symbol, self.price_prec, self.qty_prec, n_trials=n_trials)
            _, _, equity_curve = self.backtester.run_backtest(self.df, best_params, self.symbol, self.price_prec, self.qty_prec)
        ...
        opt_logger.info(f"Optimization finished in {datetime.datetime.now() - start_time}.")
