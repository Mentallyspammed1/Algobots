...
const indicators = {
    ...
    calculate_stoch_rsi: function(df, rsiPeriod, kPeriod, dPeriod) {
        ...
        for (let i = 0; i < rsiPeriod - 1; i++) {
            stochRsi.push(undefined);
        }
        for (let i = rsiPeriod - 1; i < rsi.length; i++) {
            const slice = rsi.slice(i - rsiPeriod + 1, i + 1).filter(v => v !== undefined);
            if (slice.length === 0) {
                stochRsi.push(undefined);
                continue;
            }
            ...
            if (denominator.isZero()) {
                stochRsi.push(new Decimal(50)); // or some other neutral value
            } else {
                const value = rsi[i].minus(min).dividedBy(denominator).times(100);
                stochRsi.push(value);
            }
        }
        ...
        for (let i = kPeriod - 1; i < stochRsi.length; i++) {
            const slice = stochRsi.slice(i - kPeriod + 1, i + 1).filter(v => v !== undefined && !v.isNaN());
            if (slice.length === 0) {
                k.push(undefined);
                continue;
            }
            const sum = slice.reduce((acc, val) => acc.plus(val), new Decimal(0));
            k.push(sum.dividedBy(slice.length));
        }
        ...
        for (let i = dPeriod - 1; i < k.length; i++) {
            const slice = k.slice(i - dPeriod + 1, i + 1).filter(v => v !== undefined && !v.isNaN());
            if (slice.length === 0) {
                d.push(undefined);
                continue;
            }
            const sum = slice.reduce((acc, val) => acc.plus(val), new Decimal(0));
            d.push(sum.dividedBy(slice.length));
        }
    ...
    calculate_cci: function(df, period) {
        const tp = [];
        for (let i = 0; i < df.close.length; i++) {
            if (df.high[i] === undefined || df.low[i] === undefined || df.close[i] === undefined) {
                tp.push(undefined);
            } else {
        ...
        for (let i = 0; i < sma.length; i++) {
            ...
            const mean = sma[i];
            if (slice.includes(undefined) || mean === undefined) {
                mad.push(undefined);
            } else {
        ...
        for (let i = 0; i < sma.length; i++) {
            if (tp[i + period - 1] === undefined || sma[i] === undefined || mad[i] === undefined || mad[i].isZero()) {
                cci.push(undefined);
            } else {
    ...
    calculate_obv: function(df, period) {
        const obv = [];
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // OBV starts from the second bar, so pad the first
            obv.push(undefined);
        }
        if (df.volume[0] !== undefined) {
            obv.push(df.volume[0]);
        } else {
            obv.push(undefined);
        }

        for (let i = 1; i < df.close.length; i++) {
            if (df.close[i] !== undefined && df.close[i - 1] !== undefined && obv[i - 1] !== undefined) {
                ...
                obv.push(undefined); // Propagate undefined if data is missing
            }
        }
    ...
    calculate_cmf: function(df, period) {
        ...
        for (let i = period - 1; i < df.close.length; i++) {
            ...
            for (let j = i - period + 1; j <= i; j++) {
                ...
                // Check for undefined values before calculation
                if (high === undefined || low === undefined || close === undefined || volume === undefined) {
                    sumMoneyFlow = new Decimal(NaN); // Propagate NaN if data is missing
                    break;
                }
            ...
            if (sumMoneyFlow.isNaN()) { // If NaN propagated from inner loop
                cmf.push(undefined);
            } else {
    ...
    calculate_psar: function(df, acceleration, maxAcceleration) {
        ...
        maxAcceleration = new Decimal(maxAcceleration);
        const psar = [];
        const direction = [];
        
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // PSAR starts from the second bar, so pad the first
            psar.push(undefined);
        ...
        for (let i = 1; i < df.close.length; i++) {
            ...
            if (high === undefined || low === undefined || df.high[i - 1] === undefined || df.low[i - 1] === undefined || df.high[i - 2] === undefined || df.low[i - 2] === undefined) {
                psar.push(undefined);
                direction.push(undefined);
                continue;
            }
    ...
    calculate_vwap: function(df) {
        const vwap = [];
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // VWAP starts from the first bar, so no padding needed for period
            vwap.push(undefined);
        }
    ...
    calculate_ehlers_supertrend: function(df, period, multiplier) {
        ...
        const close = df.close.slice(14); // This slice needs to be handled carefully
        const supertrend = [];
        const direction = [];
        
        // Pad with undefined
        for (let i = 0; i < 14; i++) { // ATR period is 14, so first 14 values are undefined
            supertrend.push(undefined);
            direction.push(undefined);
        }

        if (close.length === 0 || atr.length === 0 || close[0] === undefined || atr[0] === undefined) {
            // If initial data is missing, pad the rest and return
            for (let i = 14; i < df.close.length; i++) {
                supertrend.push(undefined);
                direction.push(undefined);
            }
        ...
        for (let i = 1; i < close.length; i++) {
            if (close[i] === undefined || atr[i] === undefined) {
                supertrend.push(undefined);
                direction.push(undefined);
                continue;
            }
    ...
    calculate_macd: function(df, fastPeriod, slowPeriod, signalPeriod) {
        ...
        const macdLine = [];
        // Pad with undefined
        for (let i = 0; i < slowPeriod - 1; i++) { // MACD line starts after slow EMA period
        ...
        // Pad with undefined
        for (let i = 0; i < signalPeriod - 1; i++) { // Histogram starts after signal period
    ...
    calculate_adx: function(df, period) {
        const tr = [];
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // TR starts from the second bar, so pad the first
            tr.push(undefined);
        }
        for (let i = 1; i < df.close.length; i++) {
            const high = df.high[i];
        ...
        const minusDM = [];
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // DM starts from the second bar, so pad the first
        ...
        for (let i = 1; i < df.close.length; i++) {
            ...
            if (df.high[i] === undefined || df.high[i - 1] === undefined || df.low[i - 1] === undefined || df.low[i] === undefined) {
                plusDM.push(undefined);
                minusDM.push(undefined);
            } else {
        ...
        const dx = [];
        // Pad with undefined
        for (let i = 0; i < period - 1; i++) { // DI and DX start after period
        ...
        for (let i = period - 1; i < tr.length; i++) {
            ...
            if (trSlice.includes(undefined) || plusDMSlice.includes(undefined) || minusDMSlice.includes(undefined)) {
                plusDI.push(undefined);
                minusDI.push(undefined);
                dx.push(undefined);
                continue;
            ...
            if (trSum.isZero()) { // Avoid division by zero
                plusDI.push(undefined);
                minusDI.push(undefined);
                dx.push(undefined);
                continue;
        ...
        const adx = [];
        // Pad with undefined
        for (let i = 0; i < period - 1; i++) { // ADX starts after period
            adx.push(undefined);
        }
        for (let i = period - 1; i < dx.length; i++) {
            const dxSlice = dx.slice(i - period + 1, i + 1);
            if (dxSlice.includes(undefined)) {
                adx.push(undefined);
                continue;
            }
    ...
    calculate_volatility_index: function(df, period) {
        const returns = [];
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // Returns start from the second bar, so pad the first
            returns.push(undefined);
        }
        for (let i = 1; i < df.close.length; i++) {
            if (df.close[i] === undefined || df.close[i - 1] === undefined) {
                returns.push(undefined);
            } else {
        ...
        const volatility = [];
        // Pad with undefined
        for (let i = 0; i < period - 1; i++) { // Volatility starts after period
            volatility.push(undefined);
        }
        for (let i = period - 1; i < returns.length; i++) {
            ...
            const variance = slice.reduce((acc, val) => acc.plus(val.minus(mean).pow(2)), new Decimal(0)).dividedBy(period);
            const std = variance.sqrt();
            volatility.push(std.times(new Decimal(100)));
        }
    ...
    calculate_vwma: function(df, period) {
        ...
        for (let i = period - 1; i < df.close.length; i++) {
            ...
            for (let j = i - period + 1; j <= i; j++) {
                if (df.close[j] === undefined || df.volume[j] === undefined) {
                    sumMoneyFlow = new Decimal(NaN); // Propagate NaN if data is missing
                    break;
                }
                sumVolume = sumVolume.plus(df.volume[j]);
                sumMoneyFlow = sumMoneyFlow.plus(df.close[j].times(df.volume[j]));
            }

            if (sumMoneyFlow.isNaN() || sumVolume.isZero()) {
                vwma.push(undefined);
            } else {
                vwma.push(sumMoneyFlow.dividedBy(sumVolume));
            }
        }
    ...
    calculate_volume_delta: function(df, period) {
        ...
        for (let i = period - 1; i < df.close.length; i++) {
            ...
            for (let j = i - period + 1; j <= i; j++) {
                ...
                if (close === undefined || open === undefined || volume === undefined) {
                    buyVolume = new Decimal(NaN); // Propagate NaN if data is missing
                    break;
                }
            ...
            if (buyVolume.isNaN() || sellVolume.isNaN() || (buyVolume.plus(sellVolume)).isZero()) {
                volumeDelta.push(undefined);
            } else {
    ...
    calculate_kaufman_ama: function(df, period, fastPeriod, slowPeriod) {
        const change = [];
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // Change starts from the second bar
        ...
        const volatility = [];
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // Volatility starts from the second bar
            volatility.push(undefined);
        }
        for (let i = 1; i < df.close.length; i++) {
            let vol = new Decimal(0);
            for (let j = 1; j <= period; j++) {
                if (i - j >= 0 && df.close[i - j + 1] !== undefined && df.close[i - j] !== undefined) {
                    vol = vol.plus(df.close[i - j + 1].minus(df.close[i - j]).abs());
                } else {
                    vol = new Decimal(NaN); // Propagate NaN if data is missing
                    break;
                }
            }
            if (vol.isNaN()) {
        ...
        const er = [];
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // ER starts from the second bar
            er.push(undefined);
        }
        for (let i = 0; i < change.length; i++) {
            if (change[i] === undefined || volatility[i] === undefined || volatility[i].isZero()) {
                er.push(undefined);
            } else {
        ...
        const kama = [];
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // KAMA starts from the second bar
            kama.push(undefined);
        }
        if (df.close[0] !== undefined) {
            kama.push(df.close[0]);
        } else {
            kama.push(undefined);
        }

        for (let i = 1; i < df.close.length; i++) {
            if (er[i - 1] === undefined || kama[i - 1] === undefined || df.close[i] === undefined) {
                kama.push(undefined);
            }
            else {
    ...
    calculate_relative_volume: function(df, period) {
        ...
        for (let i = period - 1; i < volume.length; i++) {
            ...
            if (avgVolume.isZero()) { // Avoid division by zero
                relativeVolume.push(undefined);
                continue;
            }
    ...
    calculate_market_structure: function(df, lookbackPeriod) {
        ...
        for (let i = lookbackPeriod; i < df.close.length; i++) {
            ...
            if (highs.includes(undefined) || lows.includes(undefined) || currentHigh === undefined || currentLow === undefined) {
                trend.push(undefined);
                continue;
            }
    ...
    calculate_dema: function(df, period) {
        ...
        for (let i = 0; i < ema1.length; i++) {
            if (i < period - 1 || ema1[i] === undefined || ema2[i - period + 1] === undefined) {
                dema.push(undefined);
            } else {
    ...
    calculate_keltner_channels: function(df, period, multiplier, atrPeriod) {
        ...
        for (let i = 0; i < ema.length; i++) {
            ...
            if (atrIndex < atr.length && ema[i] !== undefined && atr[atrIndex] !== undefined) {
    ...
    calculate_roc: function(df, period) {
        ...
        for (let i = period; i < df.close.length; i++) {
            if (df.close[i] === undefined || df.close[i - period] === undefined || df.close[i - period].isZero()) {
                roc.push(undefined);
                continue;
            }
    ...
    detect_candlestick_patterns: function(df) {
        ...
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // Patterns start from the second bar
        ...
        for (let i = 1; i < df.close.length; i++) {
            ...
            if (Object.values(prev).includes(undefined) || Object.values(curr).includes(undefined)) {
                patterns.push(undefined);
                continue;
            }
    ...
    calculate_fibonacci_levels: function(df, window) {
        ...
        for (let i = 0; i < window - 1; i++) {
            // Fibonacci levels are not time-series, so we just return null if not enough data
            // This padding is just to match the length of other indicators
            levels[`${i}`] = undefined;
        }

        if (df.length < window) return null;
        
        const high_slice = df.high.slice(df.length - window);
        const low_slice = df.low.slice(df.length - window);
        
        if (high_slice.includes(undefined) || low_slice.includes(undefined)) {
            return null; // Cannot calculate if data is missing
        }

        const high = Decimal.max(...high_slice);
        const low = Decimal.min(...low_slice);
        const diff = high.minus(low);
        
        levels["0.0%"] = low;
        levels["23.6%"] = low.plus(diff.times(0.236));
        levels["38.2%"] = low.plus(diff.times(0.382));
        levels["50.0%"] = low.plus(diff.times(0.5));
        levels["61.8%"] = low.plus(diff.times(0.618));
        levels["78.6%"] = low.plus(diff.times(0.786));
        levels["100.0%"] = high;
    ...
    calculate_fibonacci_pivot_points: function(df) {
        const pivots = {};
        // Pad with undefined
        for (let i = 0; i < 1; i++) { // Pivots start from the second bar
            pivots[`${i}`] = undefined;
        }

        if (df.close.length < 2) return null;

        const last_index = df.close.length - 1;
        const high = new Decimal(df.high[last_index]);
        const low = new Decimal(df.low[last_index]);
        const close = new Decimal(df.close[last_index]);
        ...
        pivots["pivot"] = pivot;
        pivots["r1"] = pivot.plus(range.times(0.382));
        pivots["r2"] = pivot.plus(range.times(0.618));
        pivots["s1"] = pivot.minus(range.times(0.382));
        pivots["s2"] = pivot.minus(range.times(0.618));
...
class BybitClient {
    ...
    async fetch_current_price(symbol) {
        ...
        this.logger.warn(`${chalk.yellow("Could not fetch current price for ")}${symbol}${chalk.reset()}`);
        return null;
    }
    ...
    async fetch_klines(symbol, interval, limit) {
        ...
        if (response && response.result && response.result.list) {
            const df_data = response.result.list.map(k => {
                try {
                    // Check for null/undefined values in kline data
                    if (k.some(val => val === null || val === undefined)) {
                        return null; // Mark this kline as invalid
                    }
                    return {
                    ...
                    return null; // Mark as invalid on any conversion error
                }
            }).filter(k => k !== null); // Filter out invalid klines
        ...
        this.logger.warn(`${chalk.yellow("Could not fetch klines for ")}${symbol} ${interval}${chalk.reset()}`);
        return null;
    }
    ...
    async fetch_orderbook(symbol, limit) {
        ...
        this.logger.warn(`${chalk.yellow("Could not fetch orderbook for ")}${symbol}${chalk.reset()}`);
        return null;
    }
    ...
    async get_wallet_balance(coin = "USDT") {
        ...
        this.logger.warn(`${chalk.yellow("Could not fetch wallet balance for ")}${coin}${chalk.reset()}`);
...
class PositionManager {
    ...
    _calculate_order_size(current_price, atr_value, conviction = 1.0) {
        ...
        if (stop_loss_distance.lte(0)) {
            this.logger.warn(`${chalk.yellow("Stop loss distance is zero or negative. Cannot calculate order size.")}${chalk.reset()}`);
            return new Decimal("0");
        }
    ...
    async open_position(signal, current_price, atr_value, conviction) {
        ...
        if (order_qty.lte(0)) {
            this.logger.warn(`${chalk.yellow("Order quantity is zero. Cannot open position.")}${chalk.reset()}`);
            return null;
        }
    ...
    _check_and_close_position(position, current_price, slippage_percent, price_precision) {
        ...
        for (let i = 0; i < this.open_positions.length; i++) {
            ...
            if (position.status === "OPEN") {
                const { is_closed, adjusted_close_price, closed_by } = this._check_and_close_position(
                    position, current_price, this.slippage_percent, this.price_precision
                );
...
class TradingAnalyzer {
    constructor(df_raw, config, logger, symbol) {
        ...
        if (this.df.close.length === 0) { // Check length of a core column
            this.logger.warn(`${chalk.yellow("TradingAnalyzer initialized with an empty DataFrame. Indicators will not be calculated.")}${chalk.reset()}`);
            return;
        }
    ...
    _process_dataframe(df_raw) {
        ...
        df_raw.forEach(row => {
            try {
                const open = new Decimal(row.open);
                ...
                const turnover = new Decimal(row.turnover);

                if (open.isNaN() || high.isNaN() || low.isNaN() || close.isNaN() || volume.isNaN() || turnover.isNaN()) {
                    // Skip this row if any value is NaN
                    return;
                }

                processed_df.start_time.push(row.start_time);
                processed_df.open.push(open);
                processed_df.high.push(high);
                processed_df.low.push(low);
                processed_df.close.push(close);
                processed_df.volume.push(volume);
                processed_df.turnover.push(turnover);
            } catch (e) {
                // Skip this row if any value is invalid for Decimal conversion
                return;
            }
        });
    ...
    _calculate_ichimoku_cloud() {
        const isd = this.indicator_settings;
        const tenkanPeriod = isd.ichimoku_tenkan_period;
        const kijunPeriod = isd.ichimoku_kijun_period;
        const senkouBPeriod = isd.ichimoku_senkou_span_b_period;
        const chikouOffset = isd.ichimoku_chikou_span_offset;
        ...
        for (let i = 0; i < this.df.close.length; i++) {
            // Tenkan Sen
            if (i < tenkanPeriod - 1) {
                tenkanSen.push(undefined);
            } else {
                const high_slice = this.df.high.slice(i - tenkanPeriod + 1, i + 1);
                const low_slice = this.df.low.slice(i - tenkanPeriod + 1, i + 1);
                if (high_slice.includes(undefined) || low_slice.includes(undefined)) {
                    tenkanSen.push(undefined);
                } else {
                    const high = Decimal.max(...high_slice);
            ...
            if (i < kijunPeriod - 1) {
                ...
                const high_slice = this.df.high.slice(i - kijunPeriod + 1, i + 1);
                const low_slice = this.df.low.slice(i - kijunPeriod + 1, i + 1);
                if (high_slice.includes(undefined) || low_slice.includes(undefined)) {
                    kijunSen.push(undefined);
                } else {
                    const high = Decimal.max(...high_slice);
        ...
        for (let i = 0; i < this.df.close.length; i++) {
            // Senkou Span A
            const tenkan_val = tenkanSen[i];
            const kijun_val = kijunSen[i];
            if (tenkan_val && kijun_val) {
                senkouSpanA.push(tenkan_val.plus(kijun_val).dividedBy(2));
            } else {
            ...
            if (i < senkouBPeriod - 1) {
                ...
                const high_slice = this.df.high.slice(i - senkouBPeriod + 1, i + 1);
                const low_slice = this.df.low.slice(i - senkouBPeriod + 1, i + 1);
                if (high_slice.includes(undefined) || low_slice.includes(undefined)) {
                    senkouSpanB.push(undefined);
                } else {
                    const high = Decimal.max(...high_slice);
                    ...
                    if (high === undefined || low === undefined) {
                        ...
                        senkouSpanB.push(high.plus(low).dividedBy(2));
                    }
                }
            }
            ...
            // Chikou Span
            if (i >= chikouOffset) {
                chikouSpan.push(this.df.close[i - chikouOffset]);
            } else {
                chikouSpan.push(undefined);
            }
        }
        
        this.df["Tenkan_Sen"] = tenkanSen;
        this.df["Kijun_Sen"] = kijunSen;
        this.df["Senkou_Span_A"] = senkouSpanA;
        this.df["Senkou_Span_B"] = senkouSpanB;
        this.df["Chikou_Span"] = chikouSpan;
        if (tenkanSen.length > 0) this.indicator_values["Tenkan_Sen"] = tenkanSen[tenkanSen.length - 1];
        if (kijunSen.length > 0) this.indicator_values["Kijun_Sen"] = kijunSen[kijunSen.length - 1];
        if (senkouSpanA.length > 0) this.indicator_values["Senkou_Span_A"] = senkouSpanA[senkouSpanA.length - 1];
        if (senkouSpanB.length > 0) this.indicator_values["Senkou_Span_B"] = senkouSpanB[senkouSpanB.length - 1];
        if (chikouSpan.length > 0) this.indicator_values["Chikou_Span"] = chikouSpan[chikouSpan.length - 1];
    }
    ...
    _safe_calculate(func, name, min_data_points, ...args) {
        ...
        try {
            ...
            const result = func.call(indicators, this.df, ...args);
            ...
            if (is_empty) {
                this.logger.warn(`${chalk.yellow("Indicator '")}${name}${chalk.yellow("' returned empty or None after calculation. Not enough valid data?")}${chalk.reset()}`);
            }
    ...
    _calculate_all_indicators() {
        ...
        if (cfg_indicators.ema_alignment) {
            ...
            if (ema_short !== null && ema_short.length > 0) {
                this.df["EMA_Short"] = ema_short;
                this.indicator_values["EMA_Short"] = ema_short[ema_short.length - 1];
            }
            if (ema_long !== null && ema_long.length > 0) {
                this.df["EMA_Long"] = ema_long;
                this.indicator_values["EMA_Long"] = ema_long[ema_long.length - 1];
            }
        }
        ...
        if (cfg_indicators.stoch_rsi) {
            ...
            if (stoch_rsi_k_d !== null) {
                this.df["StochRSI_K"] = stoch_rsi_k_d.k;
                this.df["StochRSI_D"] = stoch_rsi_k_d.d;
        ...
        if (cfg_indicators.obv) {
            ...
            if (obv_ema_vals !== null) {
                this.df["OBV"] = obv_ema_vals.obv;
                this.df["OBV_EMA"] = obv_ema_vals.obv_ema;
        ...
        if (cfg_indicators.cmf) {
            ...
            if (cmf !== null && cmf.length > 0) {
                this.df["CMF"] = cmf;
                this.indicator_values["CMF"] = cmf[cmf.length - 1];
            }
        }
        
        // Ichimoku Cloud
        if (cfg_indicators.ichimoku_cloud) {
            this._calculate_ichimoku_cloud();
        }
        ...
        if (cfg_indicators.psar) {
            ...
            if (psar_vals !== null) {
                this.df["PSAR_Val"] = psar_vals.psar;
                this.df["PSAR_Dir"] = psar_vals.direction;
        ...
        if (cfg_indicators.vwap) {
            ...
            if (vwap !== null && vwap.length > 0) {
                this.df["VWAP"] = vwap;
                this.indicator_values["VWAP"] = vwap[vwap.length - 1];
            }
        }
        ...
        if (cfg_indicators.macd) {
            ...
            if (macd_vals !== null) {
                this.df["MACD_Line"] = macd_vals.macd_line;
                this.df["MACD_Signal"] = macd_vals.signal_line;
                this.df["MACD_Hist"] = macd_vals.histogram;
        ...
        if (cfg_indicators.volatility_index) {
            ...
            if (vol_idx !== null && vol_idx.length > 0) {
                this.df["Volatility_Index"] = vol_idx;
                this.indicator_values["Volatility_Index"] = vol_idx[vol_idx.length - 1];
            }
        }
        ...
        if (cfg_indicators.vwma) {
            ...
            if (vwma !== null && vwma.length > 0) {
                this.df["VWMA"] = vwma;
                this.indicator_values["VWMA"] = vwma[vwma.length - 1];
            }
        }
        ...
        if (cfg_indicators.kaufman_ama) {
            ...
            if (kama !== null && kama.length > 0) {
                this.df["Kaufman_AMA"] = kama;
                this.indicator_values["Kaufman_AMA"] = kama[kama.length - 1];
            }
        }
        ...
        if (cfg_indicators.dema) {
            ...
            if (dema !== null && dema.length > 0) {
                this.df["DEMA"] = dema;
                this.indicator_values["DEMA"] = dema[dema.length - 1];
            }
        }
    ...
    calculate_fibonacci_levels() {
        ...
        if (fib_levels) {
            ...
            this.logger.warn(`${chalk.yellow("[")}${this.symbol}${chalk.yellow("] Fibonacci retracement levels could not be calculated.")}${chalk.reset()}`);
        }
    }
    
    calculate_fibonacci_pivot_points() {
        if (this.df.close.length < 2) {
            this.logger.warn(`[${this.symbol}] DataFrame is too short for Fibonacci Pivot Points calculation. Need at least 2 bars.`);
            return;
        }

        const last_index = this.df.close.length - 1;
        const high = new Decimal(this.df.high[last_index]);
        const low = new Decimal(this.df.low[last_index]);
        const close = new Decimal(this.df.close[last_index]);

        if (high.isNaN() || low.isNaN() || close.isNaN()) {
            this.logger.warn(`[${this.symbol}] Invalid data for Fibonacci Pivot Points calculation.`);
            return;
        }
        ...
        const r1 = pivot.plus(range.times(0.382));
        const r2 = pivot.plus(range.times(0.618));
        const s1 = pivot.minus(range.times(0.382));
        const s2 = pivot.minus(range.times(0.618));

        this.indicator_values["Pivot"] = pivot.toDecimalPlaces(this.config.trade_management.price_precision, Decimal.ROUND_DOWN);
        this.indicator_values["R1"] = r1.toDecimalPlaces(this.config.trade_management.price_precision, Decimal.ROUND_DOWN);
        this.indicator_values["R2"] = r2.toDecimalPlaces(this.config.trade_management.price_precision, Decimal.ROUND_DOWN);
        this.indicator_values["S1"] = s1.toDecimalPlaces(this.config.trade_management.price_precision, Decimal.ROUND_DOWN);
        this.indicator_values["S2"] = s2.toDecimalPlaces(this.config.trade_management.price_precision, Decimal.ROUND_DOWN);
        this.logger.debug(`[${this.symbol}] Calculated Fibonacci Pivot Points.`);
    }
    ...
    calculate_support_resistance_from_orderbook(orderbook_data) {
        ...
        if (support_level.gt(0)) {
            this.indicator_values["Support_Level"] = support_level.toDecimalPlaces(this.config.trade_management.price_precision, Decimal.ROUND_DOWN);
        ...
        if (resistance_level.gt(0)) {
            this.indicator_values["Resistance_Level"] = resistance_level.toDecimalPlaces(this.config.trade_management.price_precision, Decimal.ROUND_DOWN);
    ...
    _score_momentum_indicators() {
        ...
        if (active_indicators.stoch_rsi) {
            ...
            if (!stoch_k.isNaN() && !stoch_d.isNaN() && this.df.length > 1) {
                ...
                if (prev_stoch_k === undefined || prev_stoch_d === undefined) { 
                    // Do nothing, or handle as appropriate
                } else if (stoch_k.gt(stoch_d) && prev_stoch_k.lte(prev_stoch_d) && stoch_k.lt(isd.stoch_rsi_oversold)) {
    ...
    _score_ichimoku_cloud(current_close, trend_multiplier) {
        ...
        if (cfg_indicators.ichimoku_cloud) {
            ...
            if (!tenkan_sen.isNaN() && !kijun_sen.isNaN() && !senkou_span_a.isNaN() && !senkou_span_b.isNaN() && !chikou_span.isNaN() && this.df.length > 1) {
                ...
                if (prev_tenkan_sen === undefined || prev_kijun_sen === undefined) {
                // Do nothing
            } else if (tenkan_sen.gt(kijun_sen) && prev_tenkan_sen.lte(prev_kijun_sen)) {
                ...
                if (current_close.gt(kumo_upper) && (this.df.iloc(this.df.length - 2).close === undefined || this.df.iloc(this.df.length - 2).close.lte(kumo_upper_prev))) {
                    ichimoku_contrib += weight * 0.7;
                    this.logger.debug(`${chalk.blue("Ichimoku: Price broke above Kumo (strong bullish).")}${chalk.reset()}`);
                } else if (current_close.lt(kumo_lower) && (this.df.iloc(this.df.length - 2).close === undefined || this.df.iloc(this.df.length - 2).close.gte(kumo_lower_prev))) {
                    ichimoku_contrib -= weight * 0.7;
                    this.logger.debug(`${chalk.blue("Ichimoku: Price broke below Kumo (strong bearish).")}${chalk.reset()}`);
                }
                
                // Chikou Span crossover price
                if (chikou_span.gt(current_close) && (prev_chikou_span === undefined || this.df.iloc(this.df.length - 2).close === undefined || prev_chikou_span.lte(this.df.iloc(this.df.length - 2).close))) {
                ...
                } else if (chikou_span.lt(current_close) && (prev_chikou_span === undefined || this.df.iloc(this.df.length - 2).close === undefined || prev_chikou_span.gte(this.df.iloc(this.df.length - 2).close))) {
                    ichimoku_contrib -= weight * 0.3;
    ...
    _score_dema_crossover(trend_multiplier) {
        ...
        if (cfg_indicators.dema && cfg_indicators.ema_alignment) {
            ...
            if (!dema.isNaN() && !ema_short.isNaN() && this.df.length > 1) {
                ...
                if (prev_dema !== undefined && prev_ema_short !== undefined) {
        ...
        if (cfg_indicators.roc) {
            ...
            if (!roc.isNaN()) {
                ...
                if (this.df.length > 1) { // Zero-line crossover (simple trend indication)
                    ...
                    if (prev_roc === undefined) {
                        // Do nothing
                    } else if (roc.gt(0) && prev_roc.lte(0)) {
    ...
    generate_trading_signal(current_price, orderbook_data, mtf_trends) {
        ...
        if (this.df.length === 0) {
            this.logger.warn(`${chalk.yellow("[")}${this.symbol}${chalk.yellow("] DataFrame is empty in generate_trading_signal. Cannot generate signal.")}${chalk.reset()}`);
            return ["HOLD", 0.0, {}];
        }
        ...
        for (const [scorer_func, args] of scorers_to_run) {
            const { contrib, breakdown } = scorer_func.call(this, ...args);
...
function display_indicator_values_and_price(config, logger, current_price, analyzer, orderbook_data, mtf_trends, signal_breakdown) {
    logger.info(chalk.blue("---") + "Current Market Data & Indicators" + chalk.blue("---"));
    logger.info(chalk.green("Current Price: ") + current_price.toString());
    
    if (analyzer.df.length === 0) {
        logger.warn(chalk.yellow("Cannot display indicators: DataFrame is empty after calculations."));
        return;
    }
    
    logger.info(chalk.cyan("---") + "Indicator Values" + chalk.cyan("---"));
    for (const indicator_name in analyzer.indicator_values) {
        ...
        if (value instanceof Decimal) {
            logger.info(color(`  ${indicator_name.padEnd(20)}: ${value.toString()}`)));
        } else if (typeof value === 'number') {
            logger.info(color(`  ${indicator_name.padEnd(20)}: ${value.toFixed(8)}`));
        } else {
            logger.info(color(`  ${indicator_name.padEnd(20)}: ${value}`));
        }
    }
    
    if (analyzer.fib_levels && Object.keys(analyzer.fib_levels).length > 0) {
        logger.info(chalk.cyan("---") + "Fibonacci Retracement Levels" + chalk.cyan("---"));
        for (const level_name in analyzer.fib_levels) {
            const level_price = analyzer.fib_levels[level_name];
            logger.info(chalk.yellow(`  ${level_name.padEnd(20)}: ${level_price.toString()}`)));
        }
    }
    ...
    if (config.indicators.fibonacci_pivot_points) {
        ...
        if (!pivot.isNaN() && !r1.isNaN() && !s1.isNaN()) {
            logger.info(chalk.cyan("---") + "Fibonacci Pivot Points" + chalk.cyan("---"));
            logger.info(INDICATOR_COLORS.Pivot(`  Pivot              : ${pivot.toString()}`)));
            logger.info(INDICATOR_COLORS.R1(`  R1                 : ${r1.toString()}`)));
            logger.info(INDICATOR_COLORS.R2(`  R2                 : ${r2.toString()}`)));
            logger.info(INDICATOR_COLORS.S1(`  S1                 : ${s1.toString()}`)));
            logger.info(INDICATOR_COLORS.S2(`  S2                 : ${s2.toString()}`)));
        }
    }
    
    if (analyzer.indicator_values.Support_Level || analyzer.indicator_values.Resistance_Level) {
        logger.info(chalk.cyan("---") + "Orderbook S/R Levels" + chalk.cyan("---"));
        if (analyzer.indicator_values.Support_Level && !analyzer.indicator_values.Support_Level.isNaN()) {
            logger.info(INDICATOR_COLORS.Support_Level(`  Support Level     : ${analyzer.indicator_values.Support_Level.toString()}`)));
        }
        if (analyzer.indicator_values.Resistance_Level && !analyzer.indicator_values.Resistance_Level.isNaN()) {
            logger.info(INDICATOR_COLORS.Resistance_Level(`  Resistance Level  : ${analyzer.indicator_values.Resistance_Level.toString()}`)));
        }
    }
    
    if (mtf_trends && Object.keys(mtf_trends).length > 0) {
        logger.info(chalk.cyan("---") + "Multi-Timeframe Trends" + chalk.cyan("---"));
        for (const tf_indicator in mtf_trends) {
            const trend = mtf_trends[tf_indicator];
            logger.info(chalk.yellow(`  ${tf_indicator.padEnd(20)}: ${trend}`)));
        }
    }
    
    if (signal_breakdown && Object.keys(signal_breakdown).length > 0) {
        logger.info(chalk.cyan("---") + "Signal Score Breakdown" + chalk.cyan("---"));
        ...
        for (const [indicator, contribution] of sorted_breakdown) {
            ...
            logger.info(color(`  ${indicator.padEnd(25)}: ${contribution.toFixed(2)}`));
        }
    }
    
    // Concise Trend Summary
    logger.info(chalk.magenta("---") + "Current Trend Summary" + chalk.magenta("---"));
    ...
    if (!ema_short.isNaN() && !ema_long.isNaN()) {
        if (ema_short.gt(ema_long)) trend_summary_lines.push(chalk.green("EMA Cross  : ▲ Up"));
        else if (ema_short.lt(ema_long)) trend_summary_lines.push(chalk.red("EMA Cross  : ▼ Down"));
        else trend_summary_lines.push(chalk.yellow("EMA Cross  : ↔ Sideways"));
    }
    
    // Ehlers SuperTrend (Slow)
    const st_slow_dir = analyzer._get_indicator_value("ST_Slow_Dir");
    if (!st_slow_dir.isNaN()) {
        if (st_slow_dir === 1) trend_summary_lines.push(chalk.green("SuperTrend : ▲ Up"));
        else if (st_slow_dir === -1) trend_summary_lines.push(chalk.red("SuperTrend : ▼ Down"));
        else trend_summary_lines.push(chalk.yellow("SuperTrend : ↔ Sideways"));
    }
    ...
    if (!macd_hist.isNaN()) {
        if (analyzer.df.length > 1) { // Check previous exists
            ...
            if (macd_hist.gt(0) && prev_macd_hist.lte(0)) trend_summary_lines.push(chalk.green("MACD Hist  : ↑ Bullish Cross"));
            else if (macd_hist.lt(0) && prev_macd_hist.gte(0)) trend_summary_lines.push(chalk.red("MACD Hist  : ↓ Bearish Cross"));
            else if (macd_hist.gt(0)) trend_summary_lines.push(chalk.lightGreen("MACD Hist  : Above 0"));
            else if (macd_hist.lt(0)) trend_summary_lines.push(chalk.lightRed("MACD Hist  : Below 0"));
        } else trend_summary_lines.push(chalk.yellow("MACD Hist  : N/A"));
    }
    ...
    if (!adx_val.isNaN()) {
        if (adx_val.gt(ADX_STRONG_TREND_THRESHOLD)) {
            ...
            if (!plus_di.isNaN() && !minus_di.isNaN()) {
                if (plus_di.gt(minus_di)) trend_summary_lines.push(chalk.lightGreen(`ADX Trend  : Strong Up (${adx_val.toFixed(0)})`));
                else trend_summary_lines.push(chalk.lightRed(`ADX Trend  : Strong Down (${adx_val.toFixed(0)})`));
            }
        } else if (adx_val.lt(ADX_WEAK_TREND_THRESHOLD)) trend_summary_lines.push(chalk.yellow(`ADX Trend  : Weak/Ranging (${adx_val.toFixed(0)})`));
        else trend_summary_lines.push(chalk.cyan(`ADX Trend  : Moderate (${adx_val.toFixed(0)})`));
    }
    ...
    if (!senkou_span_a.isNaN() && !senkou_span_b.isNaN()) {
        const kumo_upper = Decimal.max(senkou_span_a, senkou_span_b);
        const kumo_lower = Decimal.min(senkou_span_a, senkou_span_b);
        if (current_price.gt(kumo_upper)) trend_summary_lines.push(chalk.green("Ichimoku   : Above Kumo"));
        else if (current_price.lt(kumo_lower)) trend_summary_lines.push(chalk.red("Ichimoku   : Below Kumo"));
        else trend_summary_lines.push(chalk.yellow("Ichimoku   : Inside Kumo"));
    }
    ...
    if (mtf_trends && Object.keys(mtf_trends).length > 0) {
        ...
        if (total > 0) {
            if (up_count === total) trend_summary_lines.push(chalk.green(`MTF Confl. : All Bullish (${up_count}/${total})`));
            else if (down_count === total) trend_summary_lines.push(chalk.red(`MTF Confl. : All Bearish (${down_count}/${total})`));
            else if (up_count > down_count) trend_summary_lines.push(chalk.lightGreen(`MTF Confl. : Mostly Bullish (${up_count}/${total})`));
            else if (down_count > up_count) trend_summary_lines.push(chalk.lightRed(`MTF Confl. : Mostly Bearish (${down_count}/${total})`));
            else trend_summary_lines.push(chalk.yellow(`MTF Confl. : Mixed (${up_count}/${total} Bull, ${down_count}/${total} Bear)`));
        }
    }
        ...
        try {
            ...
            if (config.indicators.orderbook_imbalance) {
                ...
                if (orderbook_data === null) {
                    logger.warn(`${chalk.yellow("Failed to fetch orderbook data.")}${chalk.reset()}`);
                }
            }
            ...
            if (config.mtf_analysis.enabled) {
                for (const htf_interval of config.mtf_analysis.higher_timeframes) {
                    ...
                    if (htf_df_raw !== null && htf_df_raw.length > 0) {
                        ...
                        logger.warn(`${chalk.yellow("Could not fetch klines for higher timeframe ")}${htf_interval}${chalk.yellow(" or it was empty. Skipping MTF trend for this TF.")}${chalk.reset()}`);
                    }
