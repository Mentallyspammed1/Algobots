...
const WS_URL_PUBLIC = process.env.BYBIT_WS_PUBLIC_URL || "wss://stream.bybit.com/v5/public/linear";
const WS_URL_PRIVATE = process.env.BYBIT_WS_PRIVATE_URL || "wss://stream.bybit.com/v5/private";
...
const DEFAULT_LOOP_DELAY_SECONDS = 15;

const ADX_STRONG_TREND_THRESHOLD = new Decimal(25);
const ADX_WEAK_TREND_THRESHOLD = new Decimal(20);
const STOCH_RSI_MID_POINT = new Decimal(50);
...
function round_price(price, precision) {
    ...
    return price.toDecimalPlaces(precision, Decimal.ROUND_DOWN);
}
...
function in_allowed_session(config) {
    if (!config.session_filter.enabled) return true;

    const now = new Date();
    const current_utc_hour = now.getUTCHours();
    const current_utc_minute = now.getUTCMinutes();
    const current_time_in_minutes = current_utc_hour * 60 + current_utc_minute;

    for (const [start_str, end_str] of config.session_filter.utc_allowed) {
        const [start_hour, start_minute] = start_str.split(':').map(Number);
        const [end_hour, end_minute] = end_str.split(':').map(Number);

        const start_time_in_minutes = start_hour * 60 + start_minute;
        let end_time_in_minutes = end_hour * 60 + end_minute;

        // Handle overnight sessions (e.g., 22:00 to 06:00)
        if (end_time_in_minutes < start_time_in_minutes) {
            end_time_in_minutes += 24 * 60; // Add a day's worth of minutes
            if (current_time_in_minutes < start_time_in_minutes) {
                current_time_in_minutes += 24 * 60;
            }
        }
...
const INDICATOR_COLORS = {
    'EMA_Short': chalk.green, 'EMA_Long': chalk.blue, 'SMA_Long': chalk.cyan, 'RSI': chalk.magenta,
    'StochRSI_K': chalk.yellow, 'StochRSI_D': chalk.red, 'BB_Upper': chalk.red, 'BB_Middle': chalk.white,
    'BB_Lower': chalk.green, 'CCI': chalk.cyan, 'WR': chalk.magenta, 'MFI': chalk.yellow, 'OBV': chalk.blue,
    'OBV_EMA': chalk.cyan, 'CMF': chalk.green, 'Tenkan_Sen': chalk.red, 'Kijun_Sen': chalk.blue,
    'Senkou_Span_A': chalk.yellow, 'Senkou_Span_B': chalk.magenta, 'Chikou_Span': chalk.cyan,
    'PSAR_Val': chalk.white, 'PSAR_Dir': chalk.yellow, 'VWAP': chalk.green, 'ST_Fast_Dir': chalk.cyan,
    'ST_Fast_Val': chalk.white, 'ST_Slow_Dir': chalk.blue, 'ST_Slow_Val': chalk.white, 'MACD_Line': chalk.green,
    'MACD_Signal': chalk.red, 'MACD_Hist': chalk.yellow, 'ADX': chalk.magenta, 'PlusDI': chalk.green,
    'MinusDI': chalk.red, 'Volatility_Index': chalk.cyan, 'VWMA': chalk.blue, 'Volume_Delta': chalk.yellow,
    'Kaufman_AMA': chalk.magenta, 'Relative_Volume': chalk.green, 'Market_Structure_Trend': chalk.cyan,
    'DEMA': chalk.yellow, 'Keltner_Upper': chalk.red, 'Keltner_Middle': chalk.white, 'Keltner_Lower': chalk.green,
    'ROC': chalk.blue, 'Candlestick_Pattern': chalk.magenta, 'Pivot': chalk.white, 'R1': chalk.green,
    'R2': chalk.lightGreen, 'S1': chalk.red, 'S2': chalk.lightRed, 'Support_Level': chalk.green,
...
class ConfigManager {
    ...
    load_config() {
        const default_config = {
            "symbol": "BTCUSDT", "interval": "15", "loop_delay": DEFAULT_LOOP_DELAY_SECONDS, "orderbook_limit": 50,
            "signal_score_threshold": new Decimal(2.0), "cooldown_sec": 60, "hysteresis_ratio": new Decimal(0.85),
            "volume_confirmation_multiplier": new Decimal(1.5),
            "trade_management": {
                "enabled": true, "account_balance": new Decimal(1000.0), "risk_per_trade_percent": new Decimal(1.0),
                "stop_loss_atr_multiple": new Decimal(1.5), "take_profit_atr_multiple": new Decimal(2.0),
                "max_open_positions": 1, "order_precision": 5, "price_precision": 3, "slippage_percent": new Decimal(0.001),
                "trading_fee_percent": new Decimal(0.0005),
            },
            "risk_guardrails": {
                "enabled": true, "max_day_loss_pct": new Decimal(3.0), "max_drawdown_pct": new Decimal(8.0),
                "cooldown_after_kill_min": 120, "spread_filter_bps": new Decimal(5.0), "ev_filter_enabled": true,
            },
            "session_filter": { "enabled": false, "utc_allowed": [["00:00", "08:00"], ["13:00", "20:00"]], },
            "pyramiding": {
                "enabled": false, "max_adds": 2, "step_atr": new Decimal(0.7), "size_pct_of_initial": new Decimal(0.5),
            },
            "mtf_analysis": {
                "enabled": true, "higher_timeframes": ["60", "240"], "trend_indicators": ["ema", "ehlers_supertrend"],
                "trend_period": 50, "mtf_request_delay_seconds": 0.5,
            },
            "ml_enhancement": {"enabled": false},
            "indicator_settings": {
                "atr_period": 14, "ema_short_period": 9, "ema_long_period": 21, "rsi_period": 14, "stoch_rsi_period": 14,
                "stoch_k_period": 3, "stoch_d_period": 3, "bollinger_bands_period": 20, "bollinger_bands_std_dev": new Decimal(2.0),
                "cci_period": 20, "williams_r_period": 14, "mfi_period": 14, "psar_acceleration": new Decimal(0.02),
                "psar_max_acceleration": new Decimal(0.2), "sma_short_period": 10, "sma_long_period": 50, "fibonacci_window": 60,
                "ehlers_fast_period": 10, "ehlers_fast_multiplier": new Decimal(2.0), "ehlers_slow_period": 20,
                "ehlers_slow_multiplier": new Decimal(3.0), "macd_fast_period": 12, "macd_slow_period": 26,
                "macd_signal_period": 9, "adx_period": 14, "ichimoku_tenkan_period": 9, "ichimoku_kijun_period": 26,
                "ichimoku_senkou_span_b_period": 52, "ichimoku_chikou_span_offset": 26, "obv_ema_period": 20, "cmf_period": 20,
                "rsi_oversold": new Decimal(30), "rsi_overbought": new Decimal(70), "stoch_rsi_oversold": new Decimal(20),
                "stoch_rsi_overbought": new Decimal(80), "cci_oversold": new Decimal(-100), "cci_overbought": new Decimal(100),
                "williams_r_oversold": new Decimal(-80), "williams_r_overbought": new Decimal(-20), "mfi_oversold": new Decimal(20),
                "mfi_overbought": new Decimal(80), "volatility_index_period": 20, "vwma_period": 20, "volume_delta_period": 5,
                "volume_delta_threshold": new Decimal(0.2), "kama_period": 10, "kama_fast_period": 2, "kama_slow_period": 30,
                "relative_volume_period": 20, "relative_volume_threshold": new Decimal(1.5), "market_structure_lookback_period": 20,
                "dema_period": 14, "keltner_period": 20, "keltner_atr_multiplier": new Decimal(2.0), "roc_period": 12,
                "roc_oversold": new Decimal(-5.0), "roc_overbought": new Decimal(5.0),
            },
            "indicators": {
                ...
                "volatility_index": true, "vwma": true, "volume_delta": true, "kaufman_ama": true, "relative_volume": true,
                "market_structure": true, "dema": true, "keltner_channels": true, "roc": true, "candlestick_patterns": true,
                "fibonacci_pivot_points": true,
            },
            "weight_sets": {
                "default_scalping": {
                    "ema_alignment": new Decimal(0.30), "sma_trend_filter": new Decimal(0.20), "ehlers_supertrend_alignment": new Decimal(0.40),
                    "macd_alignment": new Decimal(0.30), "adx_strength": new Decimal(0.25), "ichimoku_confluence": new Decimal(0.35),
                    "psar": new Decimal(0.15), "vwap": new Decimal(0.15), "vwma_cross": new Decimal(0.10), "sma_10": new Decimal(0.05),
                    "bollinger_bands": new Decimal(0.25), "momentum_rsi_stoch_cci_wr_mfi": new Decimal(0.35),
                    "volume_confirmation": new Decimal(0.10), "obv_momentum": new Decimal(0.15), "cmf_flow": new Decimal(0.10),
                    "volume_delta_signal": new Decimal(0.10), "orderbook_imbalance": new Decimal(0.10),
                    "mtf_trend_confluence": new Decimal(0.25), "volatility_index_signal": new Decimal(0.10),
                    "kaufman_ama_cross": new Decimal(0.20), "relative_volume_confirmation": new Decimal(0.10),
                    "market_structure_confluence": new Decimal(0.25), "dema_crossover": new Decimal(0.18),
                    "keltner_breakout": new Decimal(0.20), "roc_signal": new Decimal(0.12), "candlestick_confirmation": new Decimal(0.15),
                    "fibonacci_pivot_points_confluence": new Decimal(0.20),
                }
            },
            "execution": {
                "use_pybit": false, "testnet": false, "account_type": "UNIFIED", "category": "linear",
                "position_mode": "ONE_WAY", "tpsl_mode": "Full", "buy_leverage": "3", "sell_leverage": "3",
                "tp_trigger_by": "LastPrice", "sl_trigger_by": "LastPrice", "default_time_in_force": "GTC",
                ...
                "sl_scheme": {
                    "type": "atr_multiple", "atr_multiple": new Decimal(1.5), "percent": new Decimal(1.0),
                    "use_conditional_stop": true, "stop_order_type": "Market",
                },
                "breakeven_after_tp1": {
                    "enabled": true, "offset_type": "atr", "offset_value": new Decimal(0.10),
                    "lock_in_min_percent": new Decimal(0), "sl_trigger_by": "LastPrice",
                },
                "live_sync": {
                    "enabled": true, "poll_ms": 2500, "max_exec_fetch": 200, "only_track_linked": true,
        ...
        if (!fs.existsSync(this.filepath)) {
            ...
            try {
                ...
                fs.writeFileSync(this.filepath, JSON.stringify(this.config, (k, v) => v instanceof Decimal ? v.toString() : v, 4), 'utf-8');
    ...
    const logger = winston.createLogger({
        ...
        transports: [
            new winston.transports.DailyRotateFile({
                dirname: LOG_DIRECTORY, filename: `${log_name}-%DATE%.log`, datePattern: 'YYYY-MM-DD',
                zippedArchive: true, maxSize: '10m', maxFiles: '5d'
            }),
...
class BybitClient {
    constructor(api_key, api_secret, base_url, logger, use_pybit_execution) {
        ...
        this.logger = logger;
        this.enabled = use_pybit_execution && api_key && api_secret;
    }
    ...
    async _send_signed_request(method, endpoint, params) {
        ...
        if (method === "GET") {
            ...
            request_body = params ? JSON.stringify(params) : "";
            param_str = timestamp + this.api_key + recv_window + request_body;
            headers["X-BAPI-API-KEY"] = this.api_key;
            headers["X-BAPI-TIMESTAMP"] = timestamp;
            headers["X-BAPI-SIGN"] = this._generate_signature(param_str);
            headers["X-BAPI-RECV-WINDOW"] = recv_window;
            this.logger.debug(`POST Request: ${url} with payload ${request_body}`);
            return fetch(url, { method: "POST", headers, body: request_body, timeout: REQUEST_TIMEOUT });
        }
    }
    ...
    async bybit_request(method, endpoint, params = null, signed = false) {
        ...
        for (let attempt = 0; attempt < MAX_API_RETRIES; attempt++) {
            ...
            await setTimeout(RETRY_DELAY_SECONDS * 1000 * Math.pow(2, attempt));
        }
        return null;
    }
    ...
    async set_leverage(symbol, buy_leverage, sell_leverage) {
        ...
        const params = {
            ...
            "buyLeverage": String(buy_leverage), "sellLeverage": String(sell_leverage)
        };
    ...
    async place_order(symbol, side, orderType, qty, price = null, stopLoss = null, takeProfit = null,
                      isLeverage = 1, timeInForce = "GTC", reduceOnly = false, postOnly = false,
                      orderLinkId = null, tpTriggerBy = "LastPrice", slTriggerBy = "LastPrice") {
        if (!this.enabled) {
        ...
        const params = {
            category: "linear", symbol, side: this._side_to_bybit(side), orderType,
            qty: qty.toString(), isLeverage, timeInForce, reduceOnly, postOnly
        };
        ...
        if (orderLinkId !== null) params.orderLinkId = orderLinkId;

        const response = await this.bybit_request("POST", endpoint, params, true);
        if (response && response.result) {
            this.logger.info(`${chalk.green("Order placed: ")}${side} ${qty.toString()} ${symbol} type=${orderType} SL: ${stopLoss ? stopLoss.toString() : 'N/A'}, TP: ${takeProfit ? takeProfit.toString() : 'N/A'} (LinkID: ${orderLinkId})${chalk.reset()}`);
            return response.result;
        }
        return null;
    }

    async cancel_order(symbol, orderId = null, orderLinkId = null) {
        if (!this.enabled) {
        ...
        if (response && response.retCode === 0) {
            this.logger.info(`${chalk.green("Order cancelled: ")}${orderId || orderLinkId}${chalk.reset()}`);
            return true;
        }
        this.logger.error(`${chalk.red("Failed to cancel order ")}${orderId || orderLinkId}: ${response?.retMsg || "Unknown error"}`);
        return false;
    }
    ...
    async get_open_orders(symbol, orderLinkId = null) {
        ...
        if (response && response.result && response.result.list) {
            return response.result.list.map(o => ({
                ...
                cumExecQty: new Decimal(o.cumExecQty),
                orderStatus: o.orderStatus,
                createdTime: new Date(parseInt(o.createdTime)),
                updatedTime: new Date(parseInt(o.updatedTime)),
                stopLoss: o.stopLoss ? new Decimal(o.stopLoss) : null,
                takeProfit: o.takeProfit ? new Decimal(o.takeProfit) : null,
                triggerPrice: o.triggerPrice ? new Decimal(o.triggerPrice) : null,
            }));
        }
        this.logger.warning(`${chalk.yellow("Could not fetch open orders for ")}${symbol}${chalk.reset()}`);
        return [];
    }
    ...
    async get_open_positions(symbol) {
        ...
        if (response && response.result && response.result.list) {
            return response.result.list.map(p => ({
                ...
                liQPrice: new Decimal(p.liQPrice),
                unrealisedPnl: new Decimal(p.unrealisedPnl),
                createdTime: new Date(parseInt(p.createdTime)),
                updatedTime: new Date(parseInt(p.updatedTime)),
                positionIdx: parseInt(p.positionIdx)
            }));
        }
    ...
    async set_trading_stop(symbol, positionIdx, takeProfit = null, stopLoss = null, trailingStop = null, tpTriggerBy = "LastPrice", slTriggerBy = "LastPrice") {
        if (!this.enabled) {
        ...
        const params = {
            category: "linear", symbol, positionIdx, tpTriggerBy, slTriggerBy,
        };
        ...
        const response = await this.bybit_request("POST", endpoint, params, true);
        if (response && response.retCode === 0) {
            this.logger.info(`${chalk.green("Trading stop updated for ")}${symbol} (positionIdx: ${positionIdx})${chalk.reset()}`);
            return true;
        }
...
class BybitWebSocketClient {
    constructor(ws_public_url, ws_private_url, api_key, api_secret, logger, config) {
        this.ws_public_url = ws_public_url;
        this.ws_private_url = ws_private_url;
        this.api_key = api_key;
        this.api_secret = api_secret;
        this.logger = logger;
        this.config = config;
        ...
        this.current_price = new Decimal(0);
        this.orderbook_data = { b: [], a: [] };
    ...
    on(event, listener) {
        if (this.listeners[event]) {
            ...
            this.logger.warn(`No such event: ${event}`);
        }
    }
    ...
    _generate_ws_signature() {
        const expires = Date.now() + 10000; // 10 seconds validity
        const message = "GET/realtime" + expires;
        const signature = crypto.createHmac('sha256', this.api_secret).update(message).digest('hex');
        return { api_key: this.api_key, expires, signature };
    }

    connectPublic() {
        this.public_ws = new WebSocket(this.ws_public_url);

        this.public_ws.onopen = () => {
            this.logger.info(`${chalk.green("Public WebSocket connected.")}`);
            this.isConnectedPublic = true;
            this.subscribePublic();
        };

        this.public_ws.onmessage = (event) => {
            ...
            if (data.topic && data.data) {
                if (data.topic.startsWith('kline')) {
                    const kline_data = data.data[0];
                    if (kline_data.confirm) { // Only process confirmed (closed) candles
                        this.emit('kline', kline_data);
                    }
                } else if (data.topic.startsWith('orderbook')) {
                    this.orderbook_data = data.data;
                    this.emit('orderbook', data.data);
                } else if (data.topic.startsWith('tickers')) {
                    this.current_price = new Decimal(data.data.lastPrice);
                    this.emit('price_update', this.current_price);
                }
            }
        };

        this.public_ws.onclose = () => {
            this.logger.warn(`${chalk.yellow("Public WebSocket disconnected. Reconnecting...")}`);
            this.isConnectedPublic = false;
            setTimeout(() => this.connectPublic(), this.reconnectInterval);
        };

        this.public_ws.onerror = (error) => {
            this.logger.error(`${chalk.red("Public WebSocket error: ")}${error.message}`);
            this.public_ws.close();
        };
    }

    connectPrivate() {
        this.private_ws = new WebSocket(this.ws_private_url);

        this.private_ws.onopen = () => {
            this.logger.info(`${chalk.green("Private WebSocket connected. Authenticating...")}`);
            this.isConnectedPrivate = true;
            this.authenticatePrivate();
        };

        this.private_ws.onmessage = (event) => {
            const data = JSON.parse(event.data);
            if (data.op === 'pong') {
                this.lastPongTime = Date.now();
                return;
            ...
            if (data.op === 'auth') {
                if (data.success) {
                    this.logger.info(`${chalk.green("Private WebSocket authenticated.")}`);
                    this.subscribePrivate();
                } else {
                    this.logger.error(`${chalk.red("Private WebSocket authentication failed: ")}${data.retMsg}`);
                    this.private_ws.close();
                }
            } else if (data.topic) {
                if (data.topic === 'order') {
                    this.emit('execution', data.data);
                } else if (data.topic === 'position') {
                    this.emit('position', data.data);
                }
            }
        };

        this.private_ws.onclose = () => {
            this.logger.warn(`${chalk.yellow("Private WebSocket disconnected. Reconnecting...")}`);
            this.isConnectedPrivate = false;
            setTimeout(() => this.connectPrivate(), this.reconnectInterval);
        };

        this.private_ws.onerror = (error) => {
            this.logger.error(`${chalk.red("Private WebSocket error: ")}${error.message}`);
            this.private_ws.close();
        };
    }

    authenticatePrivate() {
        if (this.private_ws && this.private_ws.readyState === WebSocket.OPEN) {
            const auth = this._generate_ws_signature();
            this.private_ws.send(JSON.stringify({
                op: "auth",
    ...
    subscribePublic() {
        if (this.public_ws && this.public_ws.readyState === WebSocket.OPEN) {
            const topics = [
                `kline.${this.config.interval}.${this.config.symbol}`,
                `orderbook.50.${this.config.symbol}`,
                `tickers.${this.config.symbol}`
            ];
            this.public_ws.send(JSON.stringify({
                op: "subscribe",
                args: topics
            }));
            this.logger.info(`${chalk.blue("Subscribed to public topics: ")}${topics.join(', ')}`);
        }
    }

    subscribePrivate() {
        if (this.private_ws && this.private_ws.readyState === WebSocket.OPEN) {
            const topics = ["order", "position"];
            this.private_ws.send(JSON.stringify({
                op: "subscribe",
                args: topics
            }));
            this.logger.info(`${chalk.blue("Subscribed to private topics: ")}${topics.join(', ')}`);
        }
    }
    ...
    startHeartbeat() {
        ...
        this.heartbeatInterval = setInterval(() => {
            if (this.public_ws && this.public_ws.readyState === WebSocket.OPEN) {
                this.public_ws.send(JSON.stringify({ op: "ping" }));
            }
            if (this.private_ws && this.private_ws.readyState === WebSocket.OPEN) {
                this.private_ws.send(JSON.stringify({ op: "ping" }));
            }
            // Check for pong response
            if (Date.now() - this.lastPongTime > this.config.execution.live_sync.heartbeat.interval_ms * 2) {
                this.logger.warn(`${chalk.yellow("No pong received, initiating reconnect.")}`);
                if (this.public_ws) this.public_ws.close();
                if (this.private_ws) this.private_ws.close();
            }
        }, this.config.execution.live_sync.heartbeat.interval_ms);
    }
    ...
    getOrderbook() {
        return this.orderbook_data;
    }
}

// --- Position Management ---
class PositionManager {
    constructor(config, logger, symbol, bybit_client, ws_client) {
        this.config = config;
        this.logger = logger;
        this.symbol = symbol;
        ...
        this.local_open_positions = [];
        this.active_tp_sl_orders = new Map(); // Map: orderLinkId -> { orderId, type, qty, price }

        if (this.bybit.enabled) {
            this._update_precision_from_exchange();
            this.ws_client.on('execution', this._handle_execution_report.bind(this));
            this.ws_client.on('position', this._handle_position_update.bind(this));
        }
    }
    ...
    async _get_current_balance() {
        ...
        // For simulation, track balance with PnL
        const initial_balance = this.config.trade_management.account_balance;
        const total_pnl = performance_tracker ? performance_tracker.total_pnl : new Decimal(0);
        return initial_balance.plus(total_pnl);
    }

    _calculate_order_size(current_price, atr_value, conviction = new Decimal(1.0)) {
    ...
    _calculate_take_profit_prices(signal, entry_price, atr_value) {
        const tp_scheme = this.config.execution.tp_scheme;
        if (tp_scheme.mode === "atr_multiples") {
            return tp_scheme.targets.map(target => {
                const tp_mult = target.atr_multiple;
                const tp = (signal === "BUY") ? entry_price.plus(atr_value.times(tp_mult)) : entry_price.minus(atr_value.times(tp_mult));
                return {
                    name: target.name,
                    price: round_price(tp, this.price_precision),
                    size_pct: target.size_pct,
        ...
        // Default to single TP based on trade_management config if no specific scheme
        const single_tp_mult = this.config.trade_management.take_profit_atr_multiple;
        const single_tp = (signal === "BUY") ? entry_price.plus(atr_value.times(single_tp_mult)) : entry_price.minus(atr_value.times(single_tp_mult));
        return [{
            name: "TP1",
            price: round_price(single_tp, this.price_precision),
            size_pct: new Decimal(1.0),
            order_type: "Market",
            tif: "GTC",
            post_only: false
        }];
    ...
    async open_position(signal, current_price, atr_value, conviction) {
        ...
        adjusted_entry_price_sim = round_price(adjusted_entry_price_sim, this.price_precision);

        const stop_loss_price = this._compute_stop_loss_price(signal, adjusted_entry_price_sim, atr_value);
        const take_profit_targets = this._calculate_take_profit_prices(signal, adjusted_entry_price_sim, atr_value);

        const position = {
            "entry_time": new Date(),
            "symbol": this.symbol,
            "side": signal,
            "entry_price": adjusted_entry_price_sim,
            "qty": order_qty,
            ...
            "tp_order_ids": {}, // { "TP1": "orderId", ... }
            "sl_order_id": null,
            "positionIdx": this.config.execution.position_idx_overrides[this.config.execution.position_mode] || 0
        };
        ...
        if (this.bybit.enabled) {
            try {
                ...
                const entry_resp = await this.bybit.place_order(
                    this.symbol, signal, "Market", order_qty, null,
                    null, null, 1, this.config.execution.default_time_in_force,
                    this.config.execution.reduce_only_default, this.config.execution.post_only_default,
                    `${position.link_prefix}_ENTRY`
                );

                if (entry_resp && entry_resp.orderId) {
                    position.order_id = entry_resp.orderId;
                    this.logger.info(`${chalk.green("Live entry submitted (Order ID: ")}${position.order_id}).${chalk.reset()}`);

                    // Set initial SL/TP on the position via set-trading-stop
                    const set_tpsl_success = await this.bybit.set_trading_stop(
                        this.symbol,
                        position.positionIdx,
                        take_profit_targets[0]?.price || null, // Use the first TP target for initial set-trading-stop
                        stop_loss_price,
                        null, // No trailing stop initially
                        this.config.execution.tp_trigger_by,
                        this.config.execution.sl_trigger_by
                    );
                    if (!set_tpsl_success) {
                        this.logger.error(`${chalk.red("Failed to set initial TP/SL for live position.")}`);
                    }

                    // Place additional TP orders if tp_scheme.mode is not 'Full' and has multiple targets
                    if (this.config.execution.tp_scheme.mode !== 'Full' && take_profit_targets.length > 1) {
                        for (let i = 1; i < take_profit_targets.length; i++) {
                            const tp_target = take_profit_targets[i];
                            const tp_qty = round_qty(order_qty.times(tp_target.size_pct), this.qty_step);
                            if (tp_qty.gt(0)) {
                                const tp_order_resp = await this.bybit.place_order(
                                    this.symbol,
                                    signal === "BUY" ? "SELL" : "BUY", // Opposite side to close
                                    tp_target.order_type,
                                    tp_qty,
                                    tp_target.price,
                                    null, null, 1, tp_target.tif, true, tp_target.post_only,
                                    `${position.link_prefix}_TP${i + 1}`
                                );
                                if (tp_order_resp && tp_order_resp.orderId) {
                                    position.tp_order_ids[tp_target.name] = tp_order_resp.orderId;
                                    this.active_tp_sl_orders.set(tp_order_resp.orderLinkId, {
                                        orderId: tp_order_resp.orderId,
                                        type: 'TP',
                                        qty: tp_qty,
                                        price: tp_target.price,
                                        parent_position_link_id: position.link_prefix,
                                        name: tp_target.name
                                    });
                                }
                            }
                    ...
                    this.logger.error(`${chalk.red("Live entry failed. Simulating only. Response: ")}${JSON.stringify(entry_resp)}${chalk.reset()}`);
                }
            } catch (e) {
        ...
        this.logger.info(`${chalk.green("Opened ")}${signal} position (local/simulated): ${JSON.stringify(position, (k,v) => v instanceof Decimal ? v.toString() : v)}${chalk.reset()}`);
        return position;
    }
    ...
    _check_and_close_position(position, current_price) {
        ...
        const first_tp_target = position.take_profit_targets[0];
        let closed_by = null;
        ...
        if (side === "BUY") {
            ...
            } else if (first_tp_target && current_price.gte(first_tp_target.price)) {
            ...
            } else if (first_tp_target && current_price.lte(first_tp_target.price)) {
                closed_by = "TAKE_PROFIT";
                close_price = current_price.times(new Decimal(1).plus(this.slippage_percent));
            }
        }
    ...
    async manage_positions(current_price, performance_tracker, atr_value) {
        ...
        // Synchronize with live positions if enabled
        if (this.bybit.enabled) {
            await this._sync_live_positions();
        }
        ...
        for (let i = 0; i < this.local_open_positions.length; i++) {
            ...
            if (position.status === "OPEN") {
                ...
                if (is_closed) {
                    ...
                    this.logger.info(`${chalk.magenta("Closed ")}${position.side} position by ${closed_by}: ${JSON.stringify(position, (k,v) => v instanceof Decimal ? v.toString() : v)}. PnL: ${pnl.normalize().toFixed(2)}${chalk.reset()}`);

                    if (this.bybit.enabled) {
                        this.logger.info(`${chalk.blue("Attempting to close live position for ")}${position.symbol}${chalk.reset()}`);
                        await this.bybit.place_order(
                            position.symbol,
                            position.side === "BUY" ? "SELL" : "BUY",
                            "Market",
                            position.qty,
                            null, null, null, 1, "GTC", true, false,
                            `${position.link_prefix}_CLOSURE`
                        );
                        await this.bybit.cancel_all_orders(position.symbol);
                    }
                } else {
                    this.trail_stop(position, current_price, atr_value);
                    await this.manage_breakeven(position, current_price, atr_value);
                }
            }
        }
    ...
    async _sync_live_positions() {
        if (!this.bybit.enabled) return;

        const live_positions = await this.bybit.get_open_positions(this.symbol);
        const live_open_orders = await this.bybit.get_open_orders(this.symbol);

        // Clear active TP/SL orders map and rebuild
        this.active_tp_sl_orders.clear();
        for (const order of live_open_orders) {
            if (order.orderLinkId && (order.orderLinkId.includes('_TP') || order.orderLinkId.includes('_SL'))) {
                const type = order.orderLinkId.includes('_TP') ? 'TP' : 'SL';
                this.active_tp_sl_orders.set(order.orderLinkId, {
                    orderId: order.orderId,
                    type: type,
                    qty: order.qty,
                    price: order.price,
                    parent_position_link_id: order.orderLinkId.split('_').slice(0, 2).join('_'),
                    name: type === 'TP' ? order.orderLinkId.split('_')[2] : 'SL'
                });
            }
        }

        // Reconcile local_open_positions with live_positions
        const new_local_positions = [];
        for (const live_pos of live_positions) {
            let found_local = this.local_open_positions.find(lp =>
                lp.symbol === live_pos.symbol && lp.side === live_pos.side
            );

            if (found_local) {
                // Update local position with live data
                found_local.qty = live_pos.size;
                found_local.entry_price = live_pos.entryPrice;
                found_local.stop_loss = live_pos.stopLoss;
                found_local.take_profit_targets[0].price = live_pos.takeProfit; // Assuming TP is updated on position
                found_local.status = live_pos.size.gt(0) ? "OPEN" : "CLOSED";
                // More fields as needed
                new_local_positions.push(found_local);
            } else if (live_pos.size.gt(0)) {
                // A new live position found that wasn't tracked locally.
                // This might happen if the bot restarted or an external trade occurred.
                // Create a new local position entry for it.
                this.logger.warn(`${chalk.yellow("Discovered untracked live position. Adding to local_open_positions.")}`);
                new_local_positions.push({
                    "entry_time": live_pos.createdTime,
                    "symbol": live_pos.symbol,
                    "side": live_pos.side,
                    "entry_price": live_pos.entryPrice,
                    "qty": live_pos.size,
                    "stop_loss": live_pos.stopLoss,
                    "take_profit_targets": [{ name: "TP1", price: live_pos.takeProfit, size_pct: new Decimal(1.0) }],
                    "status": "OPEN",
                    "link_prefix": `wgx_live_${Date.now()}`, // Generate new link_prefix
                    "adds": 0,
                    "order_id": null, // No entry order ID known
                    "best_price": live_pos.entryPrice,
                    "tp_order_ids": {},
                    "sl_order_id": null,
                    "positionIdx": live_pos.positionIdx
                });
            }
        }
        // Filter out local positions that are no longer open on the exchange
        this.local_open_positions = new_local_positions.filter(lp => lp.status === "OPEN");
    }

    _handle_execution_report(exec_reports) {
        for (const report of exec_reports) {
            this.logger.debug(`${chalk.blue("WS Execution Report: ")}${JSON.stringify(report)}${chalk.reset()}`);

            const orderStatus = report.orderStatus;
            const orderLinkId = report.orderLinkId;
            const cumExecQty = new Decimal(report.cumExecQty);
            const qty = new Decimal(report.qty);

            if (orderStatus === 'Filled' || orderStatus === 'PartiallyFilled') {
                if (orderLinkId && orderLinkId.includes('_ENTRY')) {
                    // Entry order filled
                    const position = this.local_open_positions.find(p => p.link_prefix === orderLinkId.split('_ENTRY')[0]);
                    if (position) {
                        position.entry_price = new Decimal(report.avgPrice);
                        position.qty = cumExecQty;
                        this.logger.info(`${chalk.green("WS: Entry order ")}${orderLinkId}${chalk.green(" filled. Avg Price: ")}${position.entry_price.normalize().toString()}, Qty: ${position.qty.normalize().toString()}${chalk.reset()}`);
                    }
                } else if (orderLinkId && orderLinkId.includes('_TP')) {
                    // TP order filled
                    const parent_prefix = orderLinkId.split('_').slice(0, 2).join('_');
                    const position = this.local_open_positions.find(p => p.link_prefix === parent_prefix);
                    if (position) {
                        position.qty = position.qty.minus(cumExecQty);
                        this.logger.info(`${chalk.green("WS: TP order ")}${orderLinkId}${chalk.green(" filled. Qty reduced to: ")}${position.qty.normalize().toString()}${chalk.reset()}`);
                        if (position.qty.lte(0)) {
                            position.status = "CLOSED";
                            this.logger.info(`${chalk.magenta("WS: Position ")}${position.link_prefix}${chalk.magenta(" fully closed by TP.")}${chalk.reset()}`);
                        }
                    }
                    this.active_tp_sl_orders.delete(orderLinkId);
                } else if (orderLinkId && orderLinkId.includes('_CLOSURE')) {
                    // Full closure order filled
                    const parent_prefix = orderLinkId.split('_CLOSURE')[0];
                    const position = this.local_open_positions.find(p => p.link_prefix === parent_prefix);
                    if (position) {
                        position.qty = new Decimal(0);
                        position.status = "CLOSED";
                        this.logger.info(`${chalk.magenta("WS: Position ")}${position.link_prefix}${chalk.magenta(" fully closed by manual closure.")}${chalk.reset()}`);
                    }
                }
            } else if (orderStatus === 'Cancelled') {
                this.active_tp_sl_orders.delete(orderLinkId);
                this.logger.info(`${chalk.yellow("WS: Order ")}${orderLinkId}${chalk.yellow(" cancelled.")}${chalk.reset()}`);
            }
        }
    }

    _handle_position_update(position_updates) {
        for (const update of position_updates) {
            this.logger.debug(`${chalk.blue("WS Position Update: ")}${JSON.stringify(update)}${chalk.reset()}`);
            const symbol = update.symbol;
            const size = new Decimal(update.size);
            const side = update.side === 'Long' ? 'BUY' : 'SELL';
            const avgPrice = new Decimal(update.avgPrice);
            const stopLoss = update.stopLoss ? new Decimal(update.stopLoss) : null;
            const takeProfit = update.takeProfit ? new Decimal(update.takeProfit) : null;

            let position = this.local_open_positions.find(p => p.symbol === symbol && p.side === side);

            if (position) {
                position.qty = size;
                position.entry_price = avgPrice;
                position.stop_loss = stopLoss;
                if (position.take_profit_targets.length > 0) {
                    position.take_profit_targets[0].price = takeProfit;
                }
                position.status = size.gt(0) ? "OPEN" : "CLOSED";
                this.logger.info(`${chalk.green("WS: Position ")}${symbol} ${side}${chalk.green(" updated. Qty: ")}${size.normalize().toString()}, Avg Price: ${avgPrice.normalize().toString()}${chalk.reset()}`);
                if (position.status === "CLOSED") {
                    // If position closed via exchange TP/SL, record trade
                    // This needs more robust PnL calculation based on actual fill price
                    this.logger.info(`${chalk.magenta("WS: Position ")}${symbol} ${side}${chalk.magenta(" is now CLOSED on exchange. Recording trade (estimated PnL).")}${chalk.reset()}`);
                    // A more accurate PnL would come from 'execution' topic for final fill price
                }
            } else if (size.gt(0)) {
                // New position opened externally or bot restarted. Add it to local tracking.
                this.logger.warn(`${chalk.yellow("WS: New untracked live position detected: ")}${symbol} ${side} ${size.normalize().toString()}${chalk.reset()}`);
                this.local_open_positions.push({
                    "entry_time": new Date(),
                    "symbol": symbol,
                    "side": side,
                    ...
                    "qty": size,
                    "stop_loss": stopLoss,
                    "take_profit_targets": [{ name: "TP1", price: takeProfit, size_pct: new Decimal(1.0) }],
                    "status": "OPEN",
                    "link_prefix": `wgx_live_${Date.now()}`,
                    "adds": 0,
                    "order_id": null,
                    "best_price": avgPrice,
                    "tp_order_ids": {},
    ...
    async trail_stop(pos, current_price, atr_value) {
        if (pos.status !== 'OPEN') return;

        const atr_mult = this.config.trade_management.stop_loss_atr_multiple;
        const side = pos.side;
        pos.best_price = pos.best_price || pos.entry_price;
        ...
        if (side === "BUY") {
            ...
            if (calculated_sl.gt(new_sl)) {
                ...
                if (this.bybit.enabled) {
                    await this.bybit.set_trading_stop(this.symbol, pos.positionIdx, pos.take_profit_targets[0]?.price, pos.stop_loss, null, this.config.execution.tp_trigger_by, this.config.execution.sl_trigger_by);
                }
            }
        } else {
    ...
    async manage_breakeven(pos, current_price, atr_value) {
        const breakeven_cfg = this.config.execution.breakeven_after_tp1;
        if (!breakeven_cfg.enabled || pos.status !== 'OPEN') return;

        const tp1_target = pos.take_profit_targets.find(t => t.name === "TP1");
        if (!tp1_target) return;

        let breakeven_trigger_price;
        if (pos.side === "BUY") {
            breakeven_trigger_price = tp1_target.price;
            if (current_price.gte(breakeven_trigger_price)) {
                const breakeven_sl = pos.entry_price.plus(atr_value.times(breakeven_cfg.offset_value));
                if (breakeven_sl.gt(pos.stop_loss)) { // Only move SL if it improves
                    pos.stop_loss = round_price(breakeven_sl, this.price_precision);
                    this.logger.info(`${chalk.magenta("Breakeven: ")}${pos.side} SL moved to ${pos.stop_loss.normalize().toString()} (above entry).${chalk.reset()}`);
                    if (this.bybit.enabled) {
                        await this.bybit.set_trading_stop(this.symbol, pos.positionIdx, pos.take_profit_targets[0]?.price, pos.stop_loss, null, this.config.execution.tp_trigger_by, breakeven_cfg.sl_trigger_by);
                    }
                }
            }
        } else { // SELL
            breakeven_trigger_price = tp1_target.price;
            if (current_price.lte(breakeven_trigger_price)) {
                const breakeven_sl = pos.entry_price.minus(atr_value.times(breakeven_cfg.offset_value));
                if (breakeven_sl.lt(pos.stop_loss)) {
                    pos.stop_loss = round_price(breakeven_sl, this.price_precision);
                    this.logger.info(`${chalk.magenta("Breakeven: ")}${pos.side} SL moved to ${pos.stop_loss.normalize().toString()} (below entry).${chalk.reset()}`);
                    if (this.bybit.enabled) {
                        await this.bybit.set_trading_stop(this.symbol, pos.positionIdx, pos.take_profit_targets[0]?.price, pos.stop_loss, null, this.config.execution.tp_trigger_by, breakeven_cfg.sl_trigger_by);
                    }
                }
                    ...
                    if (this.bybit.enabled) {
                        try {
                            const resp = await this.bybit.place_order(
                                this.symbol, pos.side, "Market", add_qty, null,
                                pos.stop_loss, pos.take_profit_targets[0]?.price, 1,
...
class PerformanceTracker {
    ...
    record_trade(position, pnl) {
        const trade_record = {
            "entry_time": position.entry_time, "exit_time": position.exit_time, "symbol": position.symbol,
            "side": position.side, "entry_price": position.entry_price, "exit_price": position.exit_price,
            "qty": position.qty, "pnl_gross": pnl, "closed_by": position.closed_by,
        };
    ...
    get_summary() {
        ...
        return {
            "total_trades": total_trades, "total_pnl": this.total_pnl, "gross_profit": this.gross_profit,
            "gross_loss": this.gross_loss, "profit_factor": profit_factor, "max_drawdown": this.max_drawdown,
            "wins": this.wins, "losses": this.losses, "win_rate": `${win_rate.toFixed(2)}%`,
...
class TradingAnalyzer {
    ...
    _process_dataframe(df_raw) {
        ...
        processed_df.iloc = (index) => {
            if (index < 0) index = processed_df.length + index;
            if (index < 0 || index >= processed_df.length) return {};
            const row = {};
            ...
            // Also add dynamically calculated indicator values for historical access
            for (const key in this._historical_indicator_series) {
                if (this._historical_indicator_series[key] && this._historical_indicator_series[key][index] !== undefined) {
                    row[key] = this._historical_indicator_series[key][index];
                }
            }
            return row;
        };
        this._historical_indicator_series = {}; // Store full series for dynamic access
        return processed_df;
    }
    ...
    _safe_calculate(func, name, min_data_points, ...args) {
        ...
        try {
            ...
            // Store full series for historical access
            if (Array.isArray(result) && result.length > 0) {
                this._historical_indicator_series[name] = result;
            } else if (typeof result === 'object' && result !== null && !result instanceof Decimal) {
                for (const key in result) {
                    if (Array.isArray(result[key]) && result[key].length > 0) {
                        this._historical_indicator_series[`${name}_${key}`] = result[key];
                    }
                }
            }
            return result;
    ...
    _get_historical_indicator_value(key, index, default_value = new Decimal(NaN)) {
        if (this._historical_indicator_series[key] && this._historical_indicator_series[key][index] !== undefined) {
            const value = this._historical_indicator_series[key][index];
            return (value instanceof Decimal && !value.isNaN()) ? value : default_value;
        }
    ...
    _check_orderbook(orderbook_data) {
        if (!orderbook_data || !orderbook_data.b || !orderbook_data.a) return new Decimal(0);
        ...
        if (bid_volume.plus(ask_volume).isZero()) return new Decimal(0);
    ...
    _score_momentum_indicators() {
        ...
        if (active_indicators.stoch_rsi) {
            ...
            if (!stoch_k.isNaN() && !stoch_d.isNaN() && this.df.length > 1) {
                const prev_stoch_k = this._get_historical_indicator_value("StochRSI_K_k", this.df.length - 2);
                const prev_stoch_d = this._get_historical_indicator_value("StochRSI_D_d", this.df.length - 2);
                ...
                if (!prev_stoch_k.isNaN() && !prev_stoch_d.isNaN()) {
                    ...
                    } else if (stoch_k.gt(stoch_d) && stoch_k.lt(STOCH_RSI_MID_POINT)) {
                        stoch_contrib = momentum_weight.times(0.2);
                    } else if (stoch_k.lt(stoch_d) && stoch_k.gt(STOCH_RSI_MID_POINT)) {
    ...
    _score_vwap(current_close, prev_close) {
        ...
        if (cfg_indicators.vwap) {
            ...
            if (!vwap.isNaN()) {
                ...
                if (this.df.length > 1) {
                    const prev_vwap = this._get_historical_indicator_value("VWAP", this.df.length - 2);
    ...
    _score_psar(current_close, prev_close) {
        ...
        if (cfg_indicators.psar) {
            ...
            if (!psar_val.isNaN() && !psar_dir.isNaN()) {
                ...
                if (this.df.length > 1) {
                    const prev_psar_val = this._get_historical_indicator_value("PSAR_psar", this.df.length - 2);
    ...
    _score_ehlers_supertrend(trend_multiplier) {
        ...
        if (cfg_indicators.ehlers_supertrend) {
            ...
            const prev_st_fast_dir = this._get_historical_indicator_value("EhlersSuperTrendFast_direction", this.df.length - 2);
    ...
    _score_macd(trend_multiplier) {
        ...
        if (cfg_indicators.macd) {
            ...
            if (!macd_line.isNaN() && !signal_line.isNaN() && !histogram.isNaN() && this.df.length > 1) {
                const prev_macd_line = this._get_historical_indicator_value("MACD_macd_line", this.df.length - 2);
                const prev_signal_line = this._get_historical_indicator_value("MACD_signal_line", this.df.length - 2);
                const prev_macd_hist = this._get_historical_indicator_value("MACD_histogram", this.df.length - 2);
    ...
    _score_ichimoku_cloud(current_close, trend_multiplier) {
        ...
        if (cfg_indicators.ichimoku_cloud) {
            ...
            if (!tenkan_sen.isNaN() && !kijun_sen.isNaN() && !senkou_span_a.isNaN() && !senkou_span_b.isNaN() && !chikou_span.isNaN() && this.df.length > 1) {
                const prev_tenkan_sen = this._get_historical_indicator_value("IchimokuCloud_tenkan_sen", this.df.length - 2);
                const prev_kijun_sen = this._get_historical_indicator_value("IchimokuCloud_kijun_sen", this.df.length - 2);
                const prev_senkou_span_a = this._get_historical_indicator_value("IchimokuCloud_senkou_span_a", this.df.length - 2);
                const prev_senkou_span_b = this._get_historical_indicator_value("IchimokuCloud_senkou_span_b", this.df.length - 2);
                const prev_chikou_span = this._get_historical_indicator_value("IchimokuCloud_chikou_span", this.df.length - 2);
                const prev_close = this.df.iloc(this.df.length - 2).close;
    ...
    _score_obv() {
        ...
        if (cfg_indicators.obv) {
            ...
            if (!obv_val.isNaN() && !obv_ema.isNaN() && this.df.length > 1) {
                const prev_obv_val = this._get_historical_indicator_value("OBV_obv", this.df.length - 2);
                const prev_obv_ema = this._get_historical_indicator_value("OBV_obv_ema", this.df.length - 2);
                ...
                if (!prev_obv_val.isNaN() && !prev_obv_ema.isNaN()) {
                    ...
                    if (this.df.length > 2) {
                        const prev_prev_obv_val = this._get_historical_indicator_value("OBV_obv", this.df.length - 3);
    ...
    _score_cmf() {
        ...
        if (cfg_indicators.cmf) {
            ...
            if (!cmf_val.isNaN()) {
                ...
                if (this.df.length > 2) {
                    const prev_cmf_val = this._get_historical_indicator_value("CMF", this.df.length - 2);
                    const prev_prev_cmf_val = this._get_historical_indicator_value("CMF", this.df.length - 3);
    ...
    _score_volatility_index(signal_score_current) {
        ...
        if (cfg_indicators.volatility_index) {
            ...
            if (!vol_idx.isNaN()) {
                if (this.df.length > 2) {
                    const prev_vol_idx = this._get_historical_indicator_value("Volatility_Index", this.df.length - 2);
                    const prev_prev_vol_idx = this._get_historical_indicator_value("Volatility_Index", this.df.length - 3);

                    if (!prev_vol_idx.isNaN() && !prev_prev_vol_idx.isNaN()) {
                        if (vol_idx.gt(prev_vol_idx) && prev_vol_idx.gt(prev_prev_vol_idx)) {
                        ...
                        } else if (vol_idx.lt(prev_vol_idx) && prev_vol_idx.lt(prev_prev_vol_idx)) {
                            this.logger.debug(`${chalk.blue("Volatility Index: Decreasing volatility.")}${chalk.reset()}`);
                            if (signal_score_current.abs().gt(0)) vol_contrib = signal_score_current.times(new Decimal(-0.2));
                        }
                    }
                }
    ...
    _score_vwma_cross(current_close, prev_close, trend_multiplier) {
        ...
        if (cfg_indicators.vwma) {
            ...
            if (!vwma.isNaN() && this.df.length > 1) {
                const prev_vwma = this._get_historical_indicator_value("VWMA", this.df.length - 2);

                if (!prev_vwma.isNaN()) {
                    if (current_close.gt(vwma) && prev_close.lte(prev_vwma)) {
                    ...
                    } else if (current_close.lt(vwma) && prev_close.gte(prev_vwma)) {
    ...
    _score_kaufman_ama_cross(current_close, prev_close, trend_multiplier) {
        ...
        if (cfg_indicators.kaufman_ama) {
            ...
            if (!kama.isNaN() && this.df.length > 1) {
                const prev_kama = this._get_historical_indicator_value("Kaufman_AMA", this.df.length - 2);

                if (!prev_kama.isNaN()) {
                    if (current_close.gt(kama) && prev_close.lte(prev_kama)) {
                    ...
                    } else if (current_close.lt(kama) && prev_close.gte(prev_kama)) {
    ...
    _score_relative_volume(current_close, prev_close) {
        ...
        if (cfg_indicators.relative_volume) {
            ...
            if (!relative_volume.isNaN()) {
                if (relative_volume.gte(volume_threshold)) {
                    if (current_close.gt(prev_close)) {
                        rv_contrib = weight;
                    ...
                    } else if (current_close.lt(prev_close)) {
    ...
    _score_dema_crossover(trend_multiplier) {
        ...
        if (cfg_indicators.dema && cfg_indicators.ema_alignment) {
            ...
            if (!dema.isNaN() && !ema_short.isNaN() && this.df.length > 1) {
                const prev_dema = this._get_historical_indicator_value("DEMA", this.df.length - 2);
                const prev_ema_short = this._get_historical_indicator_value("EMA_Short", this.df.length - 2);
    ...
    _score_keltner_channels(current_close, prev_close) {
        ...
        if (cfg_indicators.keltner_channels) {
            ...
            if (!kc_upper.isNaN() && !kc_lower.isNaN() && this.df.length > 1) {
                const prev_kc_upper = this._get_historical_indicator_value("KeltnerChannels_upper", this.df.length - 2);
                const prev_kc_lower = this._get_historical_indicator_value("KeltnerChannels_lower", this.df.length - 2);
    ...
    _score_roc_signals(trend_multiplier) {
        ...
        if (cfg_indicators.roc) {
            ...
            if (!roc.isNaN()) {
                ...
                } else if (roc.gt(isd.roc_overbought)) {
                ...
                if (this.df.length > 1) {
                    const prev_roc = this._get_historical_indicator_value("ROC", this.df.length - 2);
                    if (!prev_roc.isNaN()) {
                        if (roc.gt(0) && prev_roc.lte(0)) {
                            roc_contrib = roc_contrib.plus(weight.times(0.3).times(trend_multiplier));
                            this.logger.debug(`${chalk.blue("ROC: Bullish zero-line crossover.")}${chalk.reset()}`);
                        } else if (roc.lt(0) && prev_roc.gte(0)) {
                            roc_contrib = roc_contrib.minus(weight.times(0.3).times(trend_multiplier));
    ...
    _score_mtf_confluence(mtf_trends) {
        ...
        if (cfg_mtf.enabled && mtf_trends) {
            ...
            if (total_mtf_indicators > 0) {
                ...
                } else if (mtf_sell_count === total_mtf_indicators) {
    ...
    _dynamic_threshold(base_threshold) {
        ...
        const historical_atr_values = this._historical_indicator_series["ATR"];
        ...
        return base_threshold.times(ratio);
    }
    ...
    generate_trading_signal(current_price, orderbook_data, mtf_trends) {
        ...
        const current_close = this.df.close[this.df.length - 1];
        ...
        trend_strength_multiplier = new_trend_multiplier;
        ...
        const base_threshold = this.config.signal_score_threshold;
        ...
        this.config._last_score = signal_score.toNumber();
...
function display_indicator_values_and_price(config, logger, current_price, analyzer, orderbook_data, mtf_trends, signal_breakdown) {
    ...
    if (!macd_hist.isNaN()) {
        if (analyzer.df.length > 1) {
            const prev_macd_hist = analyzer._get_historical_indicator_value("MACD_histogram", analyzer.df.length - 2);
...
async function main() {
    ...
    const ws_client = new BybitWebSocketClient(WS_URL_PUBLIC, WS_URL_PRIVATE, API_KEY, API_SECRET, logger, config);
    ...
    const position_manager = new PositionManager(config, logger, config.symbol, bybit_client, ws_client);
    ...
    if (bybit_client.enabled) {
        ...
        await position_manager._update_precision_from_exchange();
    }

    let current_price = new Decimal(0);
    let orderbook_data = { b: [], a: [] };
    let kline_data_ready = false;

    // Listen to WebSocket for real-time updates
    ws_client.on('price_update', (price) => {
        current_price = price;
    });
    ws_client.on('orderbook', (data) => {
        orderbook_data = data;
    });
    ws_client.on('kline', (kline) => {
        // This is a confirmed (closed) kline, signal that new data is available for analysis
        logger.debug(`${chalk.blue("WS: New confirmed kline received.")}`);
    ...
    while (true) {
        ...
        try {
            ...
            if (config.execution.use_pybit && (!ws_client.isConnectedPublic || !ws_client.isConnectedPrivate)) {
                logger.warn(`${chalk.yellow("WebSockets not fully connected. Retrying in ")}${config.loop_delay}${chalk.yellow("s.")}${chalk.reset()}`);
            ...
            if (guard.enabled) {
                const current_account_balance = await position_manager._get_current_balance();
                ...
                if ((max_day_loss_amount.gt(0) && day_loss.lte(max_day_loss_amount.neg())) || (performance_tracker.max_drawdown.gte(max_dd_amount))) {
                    ...
                    await setTimeout(config.risk_guardrails.cooldown_after_kill_min * 60 * 1000);
                    continue;
                }
            }
            ...
            // If using WebSockets, `current_price` and `orderbook_data` are updated in real-time.
            // We still need to fetch historical klines via REST for indicator calculations.
            let latest_price_for_analysis = current_price;
            if (latest_price_for_analysis.isZero()) { // Fallback if WS price not yet received
                latest_price_for_analysis = await bybit_client.fetch_current_price(config.symbol);
            }
            if (latest_price_for_analysis === null || latest_price_for_analysis.isZero()) {
                alert_system.send_alert(`[${config.symbol}] Failed to get current price. Skipping loop.`, "WARNING");
                await setTimeout(config.loop_delay * 1000);
                continue;
            }
            ...
            const [trading_signal, signal_score, signal_breakdown] = analyzer.generate_trading_signal(latest_price_for_analysis, orderbook_data, mtf_trends);
            const atr_value = analyzer._get_indicator_value("ATR", new Decimal("0.01"));

            config._last_atr_value = atr_value.toString();

            display_indicator_values_and_price(config, logger, latest_price_for_analysis, analyzer, orderbook_data, mtf_trends, signal_breakdown);

            await position_manager.manage_positions(latest_price_for_analysis, performance_tracker, atr_value);
            await position_manager.try_pyramid(latest_price_for_analysis, atr_value);

            // --- EXECUTION ---
            if (trading_signal === "BUY" || trading_signal === "SELL") {
                const conviction_val = np_clip((signal_score.abs().minus(config.signal_score_threshold)).dividedBy(config.signal_score_threshold), new Decimal(0), new Decimal(1));

                if (signal_score.abs().gte(config.signal_score_threshold)) {
                    await position_manager.open_position(trading_signal, latest_price_for_analysis, atr_value, conviction_val);
                } else {
            ...
            if (open_positions_summary.length > 0) {
                ...
                for (const pos of open_positions_summary) {
                    logger.info(`  - ${pos.side} ${pos.qty.normalize().toString()} ${pos.symbol} @ ${pos.entry_price.normalize().toString()} (SL: ${pos.stop_loss.normalize().toString()}, TP1: ${pos.take_profit_targets[0]?.price.normalize().toString()}, Adds: ${pos.adds || 0})${chalk.reset()}`);
                }
            } else {
            ...
            await setTimeout(config.loop_delay * 2 * 1000);
        }
