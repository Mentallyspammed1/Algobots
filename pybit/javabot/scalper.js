...
class Config {
    constructor() {
        ...
        this.websocketUrl = process.env.BYBIT_WEBSOCKET_URL || 'wss://stream.bybit.com/v5/public/linear';
        ...
        this.defaults = {
            ...
            scalpingStrategy: {
                ...
                exitConditions: {
                    stopLossAtrMultiple: "1.5", takeProfitAtrMultiple: "2.0",
                    partialTakeProfit: true, partialTakeProfitPercent: "0.5",
                    partialTakeProfitAtrMultiple: "1.0", breakEvenThresholdAtrMultiple: "0.8",
                    breakEvenDelayMs: 5000, trailingStopEnabled: true, trailingAtrMultiple: "1.2",
    ...
    loadConfig() {
        ...
        if (!fs.existsSync(configPath)) {
            ...
            try {
                ...
                // Use lodash.mergeWith for deep merging
                const mergedConfig = _.mergeWith({}, this.defaults, userConfig, (objValue, srcValue) => {
                    if (_.isPlainObject(objValue) && _.isPlainObject(srcValue)) {
                        return _.mergeWith({}, objValue, srcValue, (o, s) => {
                            if (typeof s === 'string' && o instanceof Decimal) return new Decimal(s);
                            return;
                        });
                    }
                    if (Array.isArray(objValue) && Array.isArray(srcValue)) {
                        return srcValue; // Overwrite arrays instead of concatenating
                    }
                    if (typeof srcValue === 'string' && typeof objValue === 'string') return srcValue;
                    if (typeof srcValue === 'number' && typeof objValue === 'number') return srcValue;
                });
    ...
    applyConfig(configData) {
        ...
        this.scalpingStrategy.exitConditions.partialTakeProfitAtrMultiple = new Decimal(this.scalpingStrategy.exitConditions.partialTakeProfitAtrMultiple);
        this.scalpingStrategy.exitConditions.breakEvenThresholdAtrMultiple = new Decimal(this.scalpingStrategy.exitConditions.breakEvenThresholdAtrMultiple);
        this.scalpingStrategy.exitConditions.trailingAtrMultiple = new Decimal(this.scalpingStrategy.exitConditions.trailingAtrMultiple);
    }
    ...
    log(level, message) { // Simple logger for config messages
        ...
        const colorName = NEON_COLORS[level.toUpperCase()] || NEON_COLORS.INFO;
        const colorFunc = chalk[colorName] || chalk.white; // Fallback to white if colorName is not found
        const timestamp = new Date().toISOString();
        console.log(colorFunc(`[${timestamp}] [${level.toUpperCase()}] ${message}`));
    }
}
...
class BybitAPI {
    ...
    _getSignature(params, timestamp, recvWindow = '5000') {
        let paramStr = `${timestamp}${CONFIG.apiKey}${recvWindow}`;
    ...
    connectWebSocket(symbol) {
        ...
        this.ws.onopen = () => {
            ...
            const subscribeMessage = { op: "subscribe", args: [
                `orderbook.${symbol}`, `trade.${symbol}`, `publicTrade.${symbol}`
            ] };
...
class KlineDataStore {
    ...
    calculateSupportResistance() {
        ...
        this.supportLevels = _.sortBy(_.uniq(supports.map(d => d.toFixed(CONFIG.pricePrecision))), d => parseFloat(d)).map(d => new Decimal(d));
        this.resistanceLevels = _.sortBy(_.uniq(resistances.map(d => d.toFixed(CONFIG.pricePrecision))), d => parseFloat(d)).map(d => new Decimal(d));
...
class SignalGenerator {
    ...
    generateSignal() {
        let signal = "HOLD"; const signalData = { signal: "HOLD", strength: new Decimal(0), reason: [], indicators: {}, volatility: null, orderFlow: null, support: [], resistance: [] };
        ...
        if (totalBuyConditions > 0 && (buyConditionsMet / totalBuyConditions) >= strengthThreshold) {
            signal = "BUY"; signalData.strength = new Decimal(buyConditionsMet / totalBuyConditions); signalData.reason.push(`Strong BUY (${buyConditionsMet}/${totalBuyConditions})`);
        } else if (totalSellConditions > 0 && (sellConditionsMet / totalSellConditions) >= strengthThreshold) {
            signal = "SELL"; signalData.strength = new Decimal(sellConditionsMet / totalSellConditions); signalData.reason.push(`Strong SELL (${sellConditionsMet}/${totalSellConditions})`);
        } else if (totalBuyConditions > 0 && buyConditionsMet > 0) {
            signal = "BUY"; signalData.strength = new Decimal(buyConditionsMet / totalBuyConditions); signalData.reason.push(`Weak BUY (${buyConditionsMet}/${totalBuyConditions})`);
        } else if (totalSellConditions > 0 && sellConditionsMet > 0) {
            signal = "SELL"; signalData.strength = new Decimal(sellConditionsMet / totalSellConditions); signalData.reason.push(`Weak SELL (${sellConditionsMet}/${totalSellConditions})`);
        }
...
class PositionManager {
    ...
    _updateEquity() {
        ...
        let unrealizedPnl = new Decimal(0); // Placeholder: Implement PnL calculation if needed
    ...
    async closePosition(positionId, reason) {
        ...
        const currentTicker = await this.api.fetchTicker(CONFIG.symbol);
        const currentPrice = currentTicker?.lastPrice || pos.entryPrice; // Fallback if ticker fails
        const closeSide = pos.side === "BUY" ? "SELL" : "BUY";
    ...
    async checkForNewTrades(signalData) {
        ...
        if (this.config.correlationFilter && this.tradeHistory.length >= 5) {
            ...
            const sameSideTrades = recentTrades.filter(t => t.side.toLowerCase() === signalData.signal.toLowerCase());
...
async function startBot() {
    try { require('chalk'); require('node-fetch'); require('ws'); require('lodash'); require('decimal.js'); require('winston'); require('winston-daily-rotate-file'); require('dotenv'); require('double-ended-queue'); } 
    catch (e) { logMessage("error", `Missing dependency: ${e.message}. Install: npm install chalk node-fetch ws lodash decimal.js winston winston-daily-rotate-file dotenv double-ended-queue`); process.exit(1); }
