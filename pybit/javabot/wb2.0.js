...
const TIMEZONE = "UTC"; // Using string for Luxon compatibility if needed, otherwise for display
...
const MIN_CANDLESTICK_PATTERNS_BARS = 2; // For pattern detection
const MIN_DATA_POINTS_TR = 2;
const MIN_DATA_POINTS_SMOOTHER_INIT = 2; // For SuperSmoother initialization
...
class BybitClient {
    constructor(api_key, api_secret, base_url, logger) {
        ...
        this.logger = logger;
        // Node-fetch doesn't have a 'session' object like requests, retries are handled per request/wrapper
    }
    ...
    async place_order(symbol, side, orderType, qty, price = null, stopLoss = null, takeProfit = null, isLeverage = 1, timeInForce = "GTC") {
        ...
        if (stopLoss !== null) { params.stopLoss = stopLoss.toString(); params.slTriggerBy = "MarkPrice"; } // Or LastPrice
        if (takeProfit !== null) { params.takeProfit = takeProfit.toString(); params.tpTriggerBy = "MarkPrice"; } // Or LastPrice
...
class PositionManager {
    ...
    _calculate_order_size(current_price, atr_value, conviction = 1.0) {
        ...
        const risk_multiplier = new Decimal(np.clip(0.5 + conviction, 0.5, 1.5));
        ...
        if (this.config.execution.use_pybit && this.pybit && this.pybit.enabled) {
            try {
                ...
                const resp = await this.pybit.place_order(
                    this.symbol, this.pybit._side_to_bybit(signal), "Market", order_qty, null, stop_loss, take_profit
                );
            ...
            if (should_add) {
                ...
                if (add_qty.gt(0)) {
                    ...
                    if (this.config.execution.use_pybit && this.pybit && this.pybit.enabled) {
                        try {
                            const resp = await this.pybit.place_order(
                                this.symbol, this.pybit._side_to_bybit(pos.side), "Market", add_qty, null, pos.stop_loss, pos.take_profit
                            );
                            if (resp && resp.orderId) {
...
class TradingAnalyzer {
    ...
    _process_dataframe(df_raw) {
        ...
        // Ensure all arrays have the same length (implicit from data structure)
        // Also, convert to pd.Series-like for internal calculations for consistency with Python logic
        const df_like = {};
        for (const key in processed_df) {
            df_like[key] = processed_df[key];
        }
        // Add a .length property for compatibility with pandas len(df) checks
        df_like.length = df_like.close.length;
        // Add .iloc for pandas-like access for specific indicators
        df_like.iloc = (index) => {
            if (index < 0) index = df_like.length + index; // Handle negative indexing
            const row = {};
            for (const key in df_like) {
                if (Array.isArray(df_like[key])) {
                    row[key] = df_like[key][index];
                }
            }
            return row;
        };
    ...
    _calculate_all_indicators() {
        ...
        // Using helper functions from the `indicators` module
        // SMA
    ...
    generate_trading_signal(current_price, orderbook_data, mtf_trends) {
        ...
        const scorers_to_run = [
            ...
            [this._score_ehlers_supertrend, [trend_multiplier]],
            [this._score_macd, [trend_multiplier]],
    ...
    calculate_entry_tp_sl(current_price, atr_value, signal) {
        ...
        return [round_price(take_profit, this.config.trade_management.price_precision),
