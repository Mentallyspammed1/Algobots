...
async function generateComprehensiveSignal(data, config, newsSentiment) {
  if (!data || !data.latestCandle || !data.indicators || !Number.isFinite(data.indicators.atr)) {
    ...
    return {
      signal: chalk.red.bold('Error: Insufficient data for signal generation.'),
      reasons: [],
      score: { buy: 0, sell: 0 },
      type: 'ERROR',
      entry: null,
  ...
  if (indicators.stochasticK !== undefined && indicators.stochasticD !== undefined) {
    if (
      indicators.stochasticK <= signalThresholds.stochastic.oversold &&
      indicators.stochasticD <= signalThresholds.stochastic.oversold &&
      indicators.stochasticK > indicators.stochasticD
    ) {
      addWeightedScore(
        'buy',
        `Stochastic is oversold and crossing up (K:${indicators.stochasticK.toFixed(
          2
        )}, D:${indicators.stochasticD.toFixed(2)})`,
        weights.stochastic
      );
    } else if (
      indicators.stochasticK >= signalThresholds.stochastic.overbought &&
      indicators.stochasticD >= signalThresholds.stochastic.overbought &&
      indicators.stochasticK < indicators.stochasticD
    ) {
      addWeightedScore(
        'sell',
        `Stochastic is overbought and crossing down (K:${indicators.stochasticK.toFixed(
          2
        )}, D:${indicators.stochasticD.toFixed(2)})`,
        weights.stochastic
      );
  ...
  if (
    ...
    indicators.macdHistogram !== undefined
  ) {
    if (indicators.macdLine > indicators.signalLine && indicators.macdHistogram > 0) {
      addWeightedScore(
        'buy',
        `MACD bullish crossover (Histogram: ${indicators.macdHistogram.toFixed(4)})`,
        weights.macd
      );
    } else if (indicators.macdLine < indicators.signalLine && indicators.macdHistogram < 0) {
      addWeightedScore(
        'sell',
        `MACD bearish crossover (Histogram: ${indicators.macdHistogram.toFixed(4)})`,
        weights.macd
      );
    }
  }

  if (indicators.emaShort !== undefined && indicators.emaLong !== undefined) {
  ...
  if (newsSentiment && newsSentiment.sentimentScore !== undefined && weights.news !== undefined) {
    if (newsSentiment.sentimentScore > 0) {
      addWeightedScore(
        'buy',
        `Positive news sentiment (${newsSentiment.sentimentScore.toFixed(2)})`,
        weights.news * newsSentiment.sentimentScore
      );
    } else if (newsSentiment.sentimentScore < 0) {
      addWeightedScore(
        'sell',
        `Negative news sentiment (${newsSentiment.sentimentScore.toFixed(2)})`,
        weights.news * Math.abs(newsSentiment.sentimentScore)
      );
    }
  }
  ...
  let positionSize = 0;
  if (slPrice !== null && entryPrice !== slPrice && accountBalance !== null) {
  ...
  message += `  Weighted Score: ${signalColor(
    `Buy: ${weightedBuyScore.toFixed(2)}, Sell: ${weightedSellScore.toFixed(2)}`
  )}\n`;

  if (geminiReasoning) {
    message += `\n  ${chalk.magenta.bold('🔮 Gemini AI Analysis:')}\n`;
    message += `    ${chalk.cyan('Trend:')} ${
      geminiReasoning.trend ? signalColor(geminiReasoning.trend) : chalk.gray('N/A')
    }\n`;
    message += `    ${chalk.cyan('Opportunity:')} ${
      geminiReasoning.opportunity ? chalk.yellow(geminiReasoning.opportunity) : chalk.gray('N/A')
    }\n`;
    message += `    ${chalk.cyan('Rationale:')} ${geminiReasoning.reasoning || chalk.gray('N/A')}\n`;
  }

  const result = {
    signal: signalColor(message),
    reasons: signalReasons,
    score: { buy: weightedBuyScore, sell: weightedSellScore },
    type: signalType,
    entry: entryPrice,
    tp: tpPrice,
    sl: slPrice,
  ...
  while (true) {
    for (const symbol of CONFIG.symbols) {
      try {
        ...
        if (!formattedCandles || formattedCandles.length < CONFIG.periods.requiredCandlesForIndicators) {
          console.log(chalk.yellow(`\nSkipping signal generation for ${symbol} due to insufficient data.`));
          continue;
        }
        ...
        logSignal(signal.replace(/\x1b\[[0-9;]*m/g, ''), symbol);

        if (type.includes('BUY') || type.includes('SELL')) {
          sendTermuxNotification(signal, type);
        }
      } catch (error) {
        console.error(chalk.red(`Error in continuous loop for ${symbol}: ${error.message}`));
      }
    }
...
async function runBacktest(filePath) {
  ...
  const minCandlesForIndicators = CONFIG.periods.requiredCandlesForIndicators || 200; 

  console.log(chalk.gray(`  Simulating ${historicalData.length} candles. Min candles for indicators: ${minCandlesForIndicators}`));

  for (let i = minCandlesForIndicators - 1; i < historicalData.length - 1; i++) {
    const candleSlice = historicalData.slice(0, i + 1);

    if (candleSlice.length < minCandlesForIndicators) {
      console.log(chalk.yellow(`  Skipping candle ${i} due to insufficient historical data for indicators.`));
      continue;
    }
    ...
    if ((type.includes('BUY') || type.includes('SELL')) && entry && tp && sl) {
      ...
      if (tradeClosed) {
        totalPnl += pnl;
        balance += pnl;
        const currentCandleTime = new Date(parseInt(historicalData[i].openTime)).toLocaleString();
        console.log(
          chalk.gray(
            `    [${currentCandleTime}] ${type} signal at ${Indicators.nfmt(entry)}. Outcome: ${
              outcome === 'WIN' ? chalk.green('WIN') : chalk.red('LOSS')
  ...
  console.log(`  Starting Balance: ${10000..toFixed(2)}`);
