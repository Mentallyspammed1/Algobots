...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Trading pair (e.g., BTCUSDT). Default: {DEFAULT_PAIR}",
    )
    parser.add_argument(
        "--tf",
        type=str,
        default=DEFAULT_TIMEFRAME,
        help=f"Timeframe (e.g., 1m, 5m, 1h). Default: {DEFAULT_TIMEFRAME}",
    )
    parser.add_argument(
        "--candles",
        type=int,
        default=DEFAULT_CANDLES,
        help=f"Number of candles to fetch. Default: {DEFAULT_CANDLES}",
    )
