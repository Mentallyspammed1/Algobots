...
def calculate_atr(df: pd.DataFrame, length: int = 14) -> pd.Series:
    ...
    atr = (
        tr.ewm(alpha=1 / length, adjust=False)
        .mean()
        .fillna(Decimal("0"))
        .apply(Decimal)
    )
...
def calculate_fibonacci_pivot_points(
    df: pd.DataFrame,
    fib_ratios: list[float] = [0.382, 0.618, 1.000],
    atr_length: int = 14,
...
def calculate_ehlers_super_smoother(df: pd.DataFrame, length: int = 10) -> pd.Series:
    ...
    for i in range(len(close_prices)):
        ...
        if i == 0:
            ...
            filtered_values.iloc[i] = (
                c1 * (current_input + prev_input) / Decimal("2")
                + c2 * filtered_values.iloc[i - 1]
            )
        else:
            filt = (
                c1 * (current_input + prev_input) / Decimal("2")
                + c2 * filtered_values.iloc[i - 1]
                + c3 * filtered_values.iloc[i - 2]
            )
...
def handle_websocket_kline_data(
    ...
    for kline_data in message["data"]:
        ...
        try:
            ...
            if not all(
                key in kline_data and isinstance(kline_data[key], (str, int, float))
    ...
    new_klines_df = (
        pd.DataFrame(all_new_klines).set_index("timestamp").astype(dtype_mapping)
    )
...
def calculate_vwap(df: pd.DataFrame) -> pd.Series:
    ...
    typical_price = (
        df["high"].apply(Decimal)
        + df["low"].apply(Decimal)
        + df["close"].apply(Decimal)
    ) / Decimal("3")
...
def calculate_order_book_imbalance(
    order_book: dict[str, list[list[str]]],
...
def calculate_ehlers_fisher_strategy(
    ...
    for i in range(len(df)):
        if i >= length - 1:
            ...
            current_val1 = (
                Decimal("0.33") * (Decimal("2") * raw_norm_price - Decimal("1"))
                + Decimal("0.67") * prev_val1
            )
            ...
            fisher_val = (
                Decimal("0.5")
                * ((Decimal("1") + current_val1) / (Decimal("1") - current_val1)).ln()
            )
            ehlers_fisher[i] = fisher_val

    df["ehlers_fisher"] = ehlers_fisher
    df["ehlers_signal"] = (
        pd.Series(ehlers_fisher).shift(1).fillna(Decimal("0.0")).values
    )
...
def calculate_supertrend(
    ...
    df["tr2"] = (
        df["high"].apply(Decimal)
        - df["close"].shift(1).fillna(Decimal("0")).apply(Decimal)
    ).abs()
    df["tr3"] = (
        df["low"].apply(Decimal)
        - df["close"].shift(1).fillna(Decimal("0")).apply(Decimal)
    ...
    df["hl2"] = (
        (df["high"].apply(Decimal) + df["low"].apply(Decimal)) / Decimal("2")
    ).apply(Decimal)
    ...
    for i in range(1, len(df)):
        ...
        prev_final_upper = (
            ...
            if final_upper_band[i - 1] != 0
            else curr_upper_basic
        )
        prev_final_lower = (
            final_lower_band[i - 1]
            if final_lower_band[i - 1] != 0
            else curr_lower_basic
        )
        ...
        elif current_close > final_lower_band[i]:
            direction[i] = 1
            supertrend_values[i] = final_lower_band[i]
        else:
            direction[i] = -1
...
def interpret_indicator(
    ...
    indicator_name: str,
    values: list[Decimal] | Decimal | dict[str, Any] | pd.DataFrame,
) -> str | None:
    ...
    try:
        ...
        if isinstance(values, Decimal):
            ...
            if indicator_name == "stoch_rsi_vals":
                if (
                    not values.empty
                    and "k" in values.columns
                    and "d" in values.columns
                    and "stoch_rsi" in values.columns
                ):
            ...
            if indicator_name == "macd":
                if (
                    not values.empty
                    and "macd" in values.columns
                    and "signal" in values.columns
                    and "histogram" in values.columns
                ):
                    macd_line, signal_line, histogram = (
                        values["macd"].iloc[-1],
                        values["signal"].iloc[-1],
                        values["histogram"].iloc[-1],
                    )
        ...
        if indicator_name == "mfi":
            ...
            if last_value < Decimal(20):
                return (
                    f'{Fore.GREEN}MFI:{Style.RESET_ALL} Oversold ({last_value:.2f})"\n'
                )
        ...
        if indicator_name == "wr":
            ...
            return (
                f"{Fore.YELLOW}Williams %R:{Style.RESET_ALL} Neutral ({last_value:.2f})"
            )
        ...
        if indicator_name == "supertrend":
            if len(values_list) >= 2:
                ...
                status = (
                    "Above (Bullish)" if latest_direction == 1 else "Below (Bearish)"
                )
