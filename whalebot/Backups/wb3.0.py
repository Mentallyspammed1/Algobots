...
def create_pybit_client(testnet: bool = False) -> HTTP:
    ...
    if not api_key or not api_secret:
        ...
        try:
            ...
            print("API_KEY and API_SECRET must be set in environment variables.")
...
API_KEY = os.getenv("BYBIT_API_KEY")
API_SECRET = os.getenv("BYBIT_API_SECRET")
# BASE_URL is not explicitly needed with pybit HTTP client, but kept for context if direct requests were made.
...
class PositionManagerPybit:
    ...
    def manage_positions(
        ...
        # However, the provided performance_tracker is for the *simulated* trades in the original bot.
        # For a real bot using pybit, you would fetch executed orders/position updates.
...
class RealTimePositionTracker:
    ...
    def cancel_all_orders(self, symbol: str = None) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Pybit error cancelling orders: {e}")
            return False
        except Exception as e:
        ...
        if active_indicators.get("macd", False):
            ...
            if not pd.isna(macd_line) and not pd.isna(signal_line):
                if macd_line > signal_line:
                    score_contribution = weights.get("macd_alignment", 0)
                    signal_score += score_contribution
                    reasons.append(f"MACD: Bullish (Score +{score_contribution:.2f})")
                elif macd_line < signal_line:
                    score_contribution = -weights.get("macd_alignment", 0)
                    signal_score += score_contribution
                    reasons.append(f"MACD: Bearish (Score {score_contribution:.2f})")

        # ADX Trend Strength
        if active_indicators.get("adx", False):
            adx = self._get_indicator_value("ADX")
            if not pd.isna(adx) and adx > ADX_STRONG_TREND_THRESHOLD:
                score_contribution = weights.get("adx_strength", 0)
                signal_score += score_contribution
                reasons.append(f"ADX: Strong Trend (Score +{score_contribution:.2f})")
