    ...
    # ADX thresholds will be moved to indicator_parameters in config
    ...
    def load_config(filepath: str, logger: logging.Logger) -> dict[str, Any]:
        ...
        default_config = {
            ...
            "strategy_management": {
                ...
                "active_profile_name": "default_scalping",
                "profiles": {
                    "default_scalping": {
                        ...
                        "indicators_enabled": {
                            ...
                            "momentum": True,
                            "volume_confirmation": True, # This is a conceptual flag, not an indicator itself
                            "stoch_rsi": True,
                            "rsi": True,
                            ...
                            "vwma": True,
                            "volume_delta": True,
                            "volatility_filter": True, # This is a conceptual flag, not an indicator itself
                            "kaufman_ama": True,
                        ...
                        "weights": {
                            ...
                            "vwma_cross": 0.15,
                            "volume_delta_signal": 0.1,
                            "volatility_filter": 0.1, # This is a conceptual flag, not an indicator itself
                            "sentiment_signal": 0.15, # Placeholder, ML disabled
                            "kaufman_ama_cross": 0.2,
                            "relative_volume_confirmation": 0.1,
                            "market_structure_confluence": 0.25,
                            "dema_crossover": 0.18,
                            "keltner_breakout": 0.2,
                            "roc_signal": 0.12,
                            "candlestick_confirmation": 0.15,
                            "fibonacci_pivot_points_confluence": 0.2
                        },
                    ...
                    "trend_following": {
                        ...
                        "weights": {
                            "ema_alignment": 0.3,
                            "sma_trend_filter": 0.2,
                            "macd_alignment": 0.4,
                            "adx_strength": 0.35,
                            "ehlers_supertrend_alignment": 0.6,
                            "ichimoku_confluence": 0.5,
            ...
            "indicator_parameters": {
                ...
                "optimal_volatility_min": 0.005,
                "optimal_volatility_max": 0.03
            },
            "analysis_modules": {
                "mtf_analysis": {
                    "enabled": True,
            ...
            "execution": {
                ...
                "use_websocket": True,
                "slippage_adjustment": True,
                "max_fill_time_ms": 5000,
            ...
            "optional_features": {
                ...
                "optimization": {
                    "enabled": False,
                    "parameter_ranges": {
                        "signal_score_threshold": [1.5, 2.5],
                        "stop_loss_atr_multiple": [1.0, 2.0],
                        "take_profit_atr_multiple": [1.5, 3.0]
                    },
        ...
        if not Path(filepath).exists():
            try:
                ...
                logger.warning(
                    ...
                    f"Created default config at {filepath} for symbol {default_config['core_settings']['symbol']}{RESET}"
                )
    ...
    class PositionManager:
        ...
        def _calculate_order_size(
            ...
            risk_per_trade_percent = (
                Decimal(str(self.config["position_management"]["risk_per_trade_percent"]))
                / 100
            )
    ...
    class TradingAnalyzer:
        ...
        def __init__(
            ...
            active_profile_name = config["strategy_management"]["active_profile_name"]
            self.active_strategy_profile = config["strategy_management"]["profiles"][active_profile_name]
            self.weights = self.active_strategy_profile["weights"]
            self.indicators_enabled = self.active_strategy_profile["indicators_enabled"]
        ...
        def _calculate_all_indicators(self) -> None:
            ...
            # Use self.indicators_enabled and self.indicator_parameters
            ...
            indicator_map: dict[str, tuple[callable, dict, Any, int | None]] = {
                ...
                "atr_indicator": ( # Note: This is an internal flag for ATR calculation
                ...
                "ehlers_supertrend": ( # Special handling
                    self._calculate_ehlers_supertrend_internal, {},
                    ["ST_Fast_Dir", "ST_Fast_Val", "ST_Slow_Dir", "ST_Slow_Val"], # Placeholder, actual assignment is inside helper
            ...
            if cfg_indicators_enabled.get("atr_indicator", False) or \
               cfg_indicators_enabled.get("keltner_channels", False) or \
               cfg_indicators_enabled.get("volatility_index", False):
                ...
                if "ATR" not in self.df.columns or self.df["ATR"].isnull().all():
                    atr_series = self._safe_calculate(
                        self._calculate_atr_internal,
                        "ATR_Dependency",
                    ...
                    if atr_series is not None and not atr_series.empty:
                        ...
                        self.logger.warning(
                            f"{NEON_YELLOW}[{self.symbol}] ATR calculation failed, dependent indicators may be affected.{RESET}"
                        )
            ...
            for ind_key, (calc_func, func_kwargs, result_keys, min_dp) in indicator_map.items():
                ...
                if cfg_indicators_enabled.get(ind_key, False):
                    ...
                    # Skip ATR here as it's handled as a dependency or in its own block above
                    elif ind_key == "atr_indicator":
                        pass
                    else:
                        result = self._safe_calculate(
        ...
        def calculate_ehlers_supertrend(
            ...
            if df_copy["close"].iloc[first_valid_idx_loc] > upper_band.iloc[first_valid_idx_loc]:
                ...
                supertrend.iloc[first_valid_idx_loc] = lower_band.iloc[first_valid_idx_loc]  # Default to lower for 'uncertain'
        ...
        def calculate_kaufman_ama(
            ...
            if len(self.df) < period + slow_period:  # KAMA requires data for period, plus fast/slow EMA
                return pd.Series(np.nan, index=self.df.index)
            ...
            price_change[:period] = np.nan  # First 'period' values are not valid for full 'price_change'
            ...
            volatility[:period] = np.nan  # Same as price_change, first 'period' values are not valid
        ...
        def calculate_keltner_channels(
            self, period: int, atr_multiplier: float
        ) -> tuple[pd.Series, pd.Series, pd.Series]:
            """Calculate Keltner Channels."""
            # Ensure ATR is calculated first. If not available, trigger it or return NaN.
            if "ATR" not in self.df.columns or self.df["ATR"].isnull().all():
                self.logger.debug(f"[{self.symbol}] ATR not available for Keltner Channels. Attempting to calculate.")
                atr_series = self._calculate_atr_internal(self.indicator_parameters["atr_period"])
                if atr_series is not None and not atr_series.empty:
                    self.df["ATR"] = atr_series
                else:
                    return pd.Series(np.nan), pd.Series(np.nan), pd.Series(np.nan)
            ...
            if len(self.df) < period or self.df["ATR"].isnull().all():  # Re-check after potential ATR calculation
                return pd.Series(np.nan), pd.Series(np.nan), pd.Series(np.nan)
        ...
        def detect_candlestick_patterns(self) -> str:
            ...
            if (
                ...
                >= 2 * abs(current_bar["close"] - current_bar["open"])  # Long lower shadow (at least 2x body)
                and (current_bar["high"] - current_bar["close"])
                <= 0.5 * abs(current_bar["close"] - current_bar["open"])  # Small upper shadow (less than 0.5x body)
            ):
            ...
            if (
                ...
                >= 2 * abs(current_bar["close"] - current_bar["open"])  # Long upper shadow (at least 2x body)
                and (current_bar["close"] - current_bar["low"])
                <= 0.5 * abs(current_bar["close"] - current_bar["open"])  # Small lower shadow (less than 0.5x body)
            ):
        ...
        def _get_mtf_trend(self, higher_tf_df: pd.DataFrame, indicator_type: str) -> str:
            ...
            period = self.config["analysis_modules"]["mtf_analysis"]["trend_period"]
            ...
            if indicator_type == "ehlers_supertrend":
                # Create a temporary analyzer for the higher timeframe data, but use its own config
                # for indicator parameters, not the main analyzer's config
                ...
                st_result = temp_analyzer.calculate_ehlers_supertrend(
                    period=self.indicator_parameters["ehlers_slow_period"],
                    multiplier=self.indicator_parameters["ehlers_slow_multiplier"],
                )
        ...
        def generate_trading_signal(
            ...
            active_indicators = self.indicators_enabled # Use active strategy profile's indicators
            weights = self.weights # Use active strategy profile's weights
            isd = self.indicator_parameters # Use global indicator parameters
                ...
                if not pd.isna(adx_val) and not pd.isna(plus_di) and not pd.isna(minus_di):
                    ...
                    if adx_val > isd["ADX_STRONG_TREND_THRESHOLD"]:
                        if plus_di > minus_di:
                            ...
                            self.logger.debug(
                                f"ADX: Strong BUY trend (ADX > {isd['ADX_STRONG_TREND_THRESHOLD']}, +DI > -DI)."
                            )
                            trend_strength_multiplier = 1.2  # Boost trend-following indicators
                        elif minus_di > plus_di:
                            adx_contrib = -adx_weight  # Strong confirmation of bearish trend
                            self.logger.debug(
                                f"ADX: Strong SELL trend (ADX > {isd['ADX_STRONG_TREND_THRESHOLD']}, -DI > +DI)."
                            )
                            trend_strength_multiplier = 1.2  # Boost trend-following indicators
                    elif adx_val < isd["ADX_WEAK_TREND_THRESHOLD"]:
                        self.logger.debug(
                            f"ADX: Weak trend (ADX < {isd['ADX_WEAK_TREND_THRESHOLD']}). Neutral signal."
                        )
                    ...
                    if level_name not in ["0.0%", "100.0%"] and current_price > Decimal("0") and abs(
                        ...
                        if len(self.df) > 1:
                            if current_close > prev_close and current_close > level_price:  # Price broke above level
                                fib_contrib += weights.get("fibonacci_confluence", 0) * 0.1 # Corrected weight key
            ...
            if active_indicators.get("macd", False):
                ...
                if (
                    ...
                    and len(self.df) > 1 and "MACD_Line" in self.df.columns and "MACD_Signal" in self.df.columns
                ):
                    macd_contrib = 0.0
                    if (
            ...
            if active_indicators.get("ichimoku_cloud", False):
                ...
                if (
                    ...
                    and len(self.df) > 1
                    and "Tenkan_Sen" in self.df.columns and "Kijun_Sen" in self.df.columns
                    and "Senkou_Span_A" in self.df.columns and "Senkou_Span_B" in self.df.columns
            ...
            if active_indicators.get("obv", False):
                ...
                if not pd.isna(obv_val) and not pd.isna(obv_ema) and len(self.df) > 1 and "OBV" in self.df.columns and "OBV_EMA" in self.df.columns:
                    obv_contrib = 0.0
                    if (
                        obv_val > obv_ema
            ...
            if self.config["analysis_modules"]["mtf_analysis"]["enabled"] and mtf_trends:
                mtf_buy_count = 0
                mtf_sell_count = 0
    ...
    def display_indicator_values_and_price(
        ...
        if analyzer.indicators_enabled.get("fibonacci_pivot_points", False):
            if (
    ...
    def main() -> None:
        ...
        # Access MTF settings
        mtf_enabled = config["analysis_modules"]["mtf_analysis"]["enabled"]
        higher_timeframes = config["analysis_modules"]["mtf_analysis"]["higher_timeframes"]
        mtf_trend_indicators = config["analysis_modules"]["mtf_analysis"]["trend_indicators"]
        mtf_request_delay_seconds = config["analysis_modules"]["mtf_analysis"]["mtf_request_delay_seconds"]
        ...
        if primary_interval not in valid_bybit_intervals:
            logger.error(
                f"{NEON_RED}Invalid primary interval '{primary_interval}' in config.json. Please use Bybit's valid string formats (e.g., '15', '60', 'D'). Exiting.{RESET}"
            )
            sys.exit(1)
        ...
        logger.info(f"Symbol: {symbol}, Interval: {primary_interval}")
        logger.info(f"Trade Management Enabled: {config['position_management']['enabled']}")

        position_manager = PositionManager(config, logger, symbol)
            ...
            try:
                ...
                if current_price is None:
                    alert_system.send_alert(
                        f"[{symbol}] Failed to fetch current price. Skipping loop.",
                        "WARNING",
                    )
                    time.sleep(loop_delay)
                    continue

                df = fetch_klines(symbol, primary_interval, 1000, logger)
                if df is None or df.empty:
                    alert_system.send_alert(
                        f"[{symbol}] Failed to fetch primary klines or DataFrame is empty. Skipping loop.",
                        "WARNING",
                    )
                    time.sleep(loop_delay)
                    continue
                ...
                orderbook_data = None
                # Check orderbook_imbalance from the active strategy profile
                active_profile_name = config["strategy_management"]["active_profile_name"]
                active_indicators_enabled = config["strategy_management"]["profiles"][active_profile_name]["indicators_enabled"]
                if active_indicators_enabled.get("orderbook_imbalance", False):
                    orderbook_data = fetch_orderbook(
                        symbol, orderbook_limit, logger
                    )
                ...
                if mtf_enabled:
                    for htf_interval in higher_timeframes:
                        ...
                        htf_df = fetch_klines(symbol, htf_interval, 1000, logger)
                        if htf_df is not None and not htf_df.empty:
                            for trend_ind in mtf_trend_indicators:
                                temp_htf_analyzer = TradingAnalyzer(
                                    htf_df, config, logger, symbol
                                )
                ...
                if analyzer.df.empty:
                    alert_system.send_alert(
                        f"[{symbol}] TradingAnalyzer DataFrame is empty after indicator calculations. Cannot generate signal.",
                        "WARNING",
                    )
                    time.sleep(loop_delay)
                    continue
                ...
                if (
                    trading_signal == "BUY"
                    and signal_score >= signal_score_threshold
                ):
                    logger.info(
                ...
                logger.info(
                    f"{NEON_PURPLE}--- Analysis Loop Finished. Waiting {loop_delay}s ---{RESET}"
                )
                time.sleep(loop_delay)

            except Exception as e:
                alert_system.send_alert(
                    f"[{symbol}] An unhandled error occurred in the main loop: {e}",
                    "ERROR",
                )
