...
class TradingAnalyzer:
    ...
    def _safe_calculate(
        ...
        **kwargs,
        ...
        """
        Safely calculates an indicator, handling insufficient data and exceptions.
        Checks for empty or NaN-filled Series.
        """
        ...
        try:
            ...
            # Check if the result is None or an empty Series/Tuple of empty Series
            if result is None:
                self.logger.warning(
                    f"{NEON_YELLOW}[{self.symbol}] Indicator '{name}' returned None. Likely due to insufficient valid data.{RESET}",
                )
                return None
            ...
            if isinstance(result, pd.Series):
                if result.empty or result.isnull().all():
                    self.logger.warning(
                        f"{NEON_YELLOW}[{self.symbol}] Indicator '{name}' returned an empty or all-NaN Series. Likely due to insufficient valid data.{RESET}",
                    )
                    return None
            elif isinstance(result, tuple):
                if all(
                    r is None or (isinstance(r, pd.Series) and (r.empty or r.isnull().all()))
                    for r in result
                ):
                    self.logger.warning(
                        f"{NEON_YELLOW}[{self.symbol}] Indicator '{name}' returned a tuple of empty or all-NaN Series. Likely due to insufficient valid data.{RESET}",
                    )
                    return None
            ...
            self.logger.error(
                f"{NEON_RED}[{self.symbol}] IndexError calculating indicator '{name}': {e}. Check data indexing or required minimum data points.{RESET}",
            )
            return None
        except ValueError as e:
            self.logger.error(
                f"{NEON_RED}[{self.symbol}] ValueError calculating indicator '{name}': {e}. Check data types and values.{RESET}",
            )
            return None
        except Exception as e:
            self.logger.error(
                f"{NEON_RED}[{self.symbol}] Unexpected error calculating indicator '{name}': {e}{RESET}",
            )
            return None
