...
def confirm_signal(state: SentinelState, direction: str, required_candles: int) -> bool:
    ...
    progressed = False
    if (direction == "BUY" and close_p > open_p and volume > avg_vol) or (direction == "SELL" and close_p < open_p and volume > avg_vol):
...
class SentinelState:
    ...
    last_order_id: str | None = None # ID of the last placed order
...
class BybitApex:
    ...
    def _calculate_fisher_component(self) -> Decimal:
        s, cfg = self.state, self.cfg
        return min(cfg.MAX_FISHER_COMPONENT, abs(Decimal(str(s.fisher)))) / cfg.FISHER_COMPONENT_DIVISOR

    def _calculate_rsi_component(self) -> Decimal:
        s, cfg = self.state, self.cfg
        return min(cfg.MAX_RSI_COMPONENT, abs(Decimal(str(s.rsi)) - Decimal("50.0"))) / cfg.RSI_COMPONENT_DIVISOR
    ...
    def _update_oracle(self, live_p: float = None):
        ...
        score += float(cfg.ALPHA_OBI_WEIGHT) * float(self._calculate_obi_component())
        score += float(cfg.ALPHA_FISHER_WEIGHT) * float(self._calculate_fisher_component())
        score += float(cfg.ALPHA_RSI_WEIGHT) * float(self._calculate_rsi_component())
    ...
    async def confirm_order_filled(
        ...
        self._report_error(
            f"Order {order_id} not filled after {max_wait}s.",
            "error",
            106,
        )
        return False
    ...
    async def setup_trailing_stop(
        ...
        # The amend_position_tp_sl method handles quantization internally
        # and directly calls the /v5/position/trading-stop endpoint.
        result = await self.amend_position_tp_sl(new_sl=trail_atr)

        if result.get("retCode") != 0:
            state.log(
                f"Failed to set trailing stop: {result.get('retMsg')}",
                "error",
            )
            return False
        ...
        state.log(
            f"Trailing stop set at {trail_atr:.{state.price_prec}f}",
            "info",
        )
        return True
    ...
    async def strike(
        ...
        trailing_callback: str | None = None,  # "%" like "0.5"
        batch_orders: list[dict] | None = None,
        oco_tp: Decimal | None = None,
        oco_sl: Decimal | None = None
    ) -> dict[str, Any]:
        """
        Universal V5 order placement. Supports Market/Limit/Bracket/OCO/Trailing/Batch.
        """
        s, cfg = self.state, self.cfg
        ...
        if qty < s.min_qty:
            s.log(f"[yellow]Qty {qty} < min {s.min_qty}. Skipping.[/yellow]", "warn")
            return {"retCode": -1, "retMsg": "Qty too small"}

        price_step = Decimal(f"1e-{s.price_prec}")

        # === BATCH ORDERS ===
        if batch_orders:
            return await self.place_batch_orders(batch_orders)

        # === SINGLE ORDER ===
        params = {
            "category": cfg.category,
            "symbol": cfg.symbol,
            "side": side.title(),  # V5: "Buy"/"Sell"
        ...
        if not reduce and atr_dec > 0:
            if take_profit_price:
                params["takeProfit"] = str(quantize_to_step(take_profit_price, price_step))
            elif order_type != "TrailingStop":
                tp_dist = atr_dec * cfg.tp_partial_atr_mult
                tp_price = s.price + tp_dist if side == "Buy" else s.price - tp_dist
                params["takeProfit"] = str(quantize_to_step(tp_price, price_step))

            if stop_loss_price:
                params["stopLoss"] = str(quantize_to_step(stop_loss_price, price_step))
            else:
                sl_dist = atr_dec * cfg.sl_atr_mult
                sl_price = s.price - sl_dist if side == "Buy" else s.price + sl_dist
                params["stopLoss"] = str(quantize_to_step(sl_price, price_step))

        # === TRAILING ===
        if trailing_callback:
            params["trailingStop"] = trailing_callback
            params["slTriggerBy"] = "MarkPrice"

        # === OCO ===
        if oco_tp and oco_sl:
            params["takeProfit"] = str(oco_tp)
            params["stopLoss"] = str(oco_sl)
            params["tpslMode"] = "Full"

        action = "HARVEST" if reduce else "STRIKE"
        s.log(f"[cyan]{action} {side} {order_type} {qty} {params.get('price', 'Market')}[/cyan]",
              "exit" if reduce else "entry")

        result = await self.request("POST", "/v5/order/create", params)

        if result["retCode"] == 0:
            order_id = result["result"]["orderId"]
            s.last_order_id = order_id
            s.log(f"[green]✓ {action} {order_id}[/green]", "entry" if not reduce else "exit")

            # Auto-confirm non-reduce orders
            if not reduce:
                filled = await self.confirm_order_filled(order_id, max_wait=3.0)
                if filled:
                    s.active = True
                    s.side = side.title()
                    s.entry_p = s.price
    ...
    async def place_batch_orders(self, orders: list[dict]) -> dict[str, Any]:
        """
        Placeholder for batch order placement.
        """
        self.state.log("[red]Batch order placement not yet implemented.[/red]", "error")
        return {"retCode": -1, "retMsg": "Batch order placement not implemented"}

    async def place_scale_orders(self, side: str, total_qty: Decimal, levels: int = 3) -> dict[str, Any]:
        """Place pyramid of limit orders at improving prices."""
        s = self.state
        price_step = Decimal(f"1e-{s.price_prec}")
        base_qty = total_qty / Decimal(levels)

        orders = []
        for i in range(levels):
            offset = Decimal(i + 1) * price_step * Decimal("2")
            level_price = (s.price - offset if side == "Buy" else s.price + offset)
            level_qty = base_qty * Decimal("1.2") ** i  # Larger at better prices

            orders.append({
                "side": side,
                "orderType": "Limit",
                "qty": str(quantize_to_step(level_qty, s.qty_step)),
                "price": str(quantize_to_step(level_price, price_step)),
                "timeInForce": "GTC"
            })

        return await self.strike(side, Decimal("0"), batch_orders=orders)

    async def place_bracket(
        self, side: str, qty: Decimal,
        tp_atr_mult: float = 0.8, sl_atr_mult: float = 1.2
    ) -> dict[str, Any]:
        """Entry + auto TP/SL brackets."""
        s = self.state
        atr_dec = Decimal(str(s.atr))

        tp_price = s.price + (atr_dec * Decimal(str(tp_atr_mult))) if side == "Buy" \
                  else s.price - (atr_dec * Decimal(str(tp_atr_mult)))
        sl_price = s.price - (atr_dec * Decimal(str(sl_atr_mult))) if side == "Buy" \
                  else s.price + (atr_dec * Decimal(str(sl_atr_mult)))

        return await self.strike(
            side, qty,
    ...
    async def cancel_order(self, order_id: str) -> dict[str, Any]:
        """Cancel single order."""
        return await self.request("POST", "/v5/order/cancel", {
            "category": self.cfg.category,
            "symbol": self.cfg.symbol,
            "orderId": order_id
        })

    async def amend_position_tp_sl(
        self, new_tp: Decimal | None = None, new_sl: Decimal | None = None
    ) -> dict[str, Any]:
        """Update TP/SL on open position (no orderId needed)."""
        params = {
            "category": self.cfg.category,
            "symbol": self.cfg.symbol,
            "positionIdx": 0
        }
        if new_tp: params["takeProfit"] = str(new_tp)
        if new_sl: params["stopLoss"] = str(new_sl)

        return await self.request("POST", "/v5/position/trading-stop", params)
        ...
        if s.stage == 1:
            ...
            if current_pnl_usdt >= target_pnl_usdt:
                ...
                if await self.setup_trailing_stop(
                    ...
                    self._report_error(
                        "Failed to set Trailing Stop after partial TP. Manually track.",
                        "error",
                        120,
                    )
    ...
    async def _place_order_with_fallback(
        ...
        # Attempt to place a limit order
        limit_order_result = await self.strike(
            side=side,
            qty=qty,
            reduce=reduce_only,
            order_type="Limit",
        ...
        if limit_order_result["retCode"] == 0:
            order_id = limit_order_result["result"]["orderId"]
            # The new strike method already confirms non-reduce orders.
            # For reduce-only orders, we might need to explicitly confirm or assume immediate fill.
            # Given the context, let's assume reduce-only limit orders are either filled or cancelled quickly.
            # For now, we'll rely on the strike method's internal confirmation for non-reduce orders.
            # For reduce-only, we'll add a small wait and check.
            if reduce_only:
                await asyncio.sleep(wait_time) # Wait for a bit for reduce-only to fill
                filled = await self.confirm_order_filled(order_id, max_wait=wait_time * cfg.CONFIRM_ORDER_DELAY_SEC_MULTIPLIER)
                if filled:
                    s.log(f"{Fore.GREEN}Reduce-only Limit order filled as MAKER. ID={order_id}{Style.RESET_ALL}", log_level)
                    return order_id
                s.log(f"{Fore.YELLOW}Reduce-only Limit order not filled; switching to MARKET.{Style.RESET_ALL}", "warn")
                await self.cancel_order(order_id) # Cancel the limit order
                market_order_result = await self.strike(side, qty, reduce=reduce_only, order_type="Market")
                return market_order_result["result"]["orderId"] if market_order_result["retCode"] == 0 else None
            # For non-reduce orders, strike already handles confirmation and state update
            return order_id
    ...
    async def _hybrid_entry_order(
        ...
        await self._place_order_with_fallback(
            side=side,
            qty=qty, # Pass original qty, strike will quantize and check min_qty
