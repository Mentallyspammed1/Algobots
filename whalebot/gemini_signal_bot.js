...
const RESET = "\x1b[0m";
const NEON_GREEN = "\x1b[92m";
const NEON_BLUE = "\x1b[96m";
const NEON_PURPLE = "\x1b[95m";
const NEON_YELLOW = "\x1b[93m";
const NEON_RED = "\x1b[91m";
const NEON_CYAN = "\x1b[96m";
const Fore = {
...
const INDICATOR_COLORS = {
    ...
    "SMA_Long": Fore.BLUE,
    "EMA_Short": Fore.MAGENTA,
    "EMA_Long": Fore.MAGENTA,
    "ATR": Fore.YELLOW,
    "RSI": Fore.GREEN,
    "StochRSI_K": Fore.CYAN,
    "StochRSI_D": Fore.CYAN,
    "BB_Upper": Fore.RED,
...
class CustomLogger {
    ...
    exception(message) { this._log('ERROR', message, NEON_RED); }
}
...
async function fetchKlines(symbol, interval, limit, logger) {
    ...
    if (response && response.result && response.result.list) {
        ...
        const data = sortedList.map(item => ({
            start_time: DateTime.fromMillis(parseInt(item[0]), { zone: TIMEZONE }).toJSDate(),
            open: new Decimal(item[1]).toNumber(),
            high: new Decimal(item[2]).toNumber(),
            low: new Decimal(item[3]).toNumber(),
            close: new Decimal(item[4]).toNumber(),
            volume: new Decimal(item[5]).toNumber(),
            turnover: new Decimal(item[6]).toNumber(),
        }));

        if (data.length === 0) {
            logger.warning(
                `${NEON_YELLOW}Fetched klines for ${symbol} ${interval} but data array is empty after processing. Raw response: ${JSON.stringify(response)}${RESET}`
            );
            return null;
        }

        logger.debug(`Fetched ${data.length} ${interval} klines for ${symbol}.`);
        return data;
    }
    logger.warning(
...
const DEFAULT_CONFIG = {
    ...
    "gemini_ai": {
        ...
        "min_confidence_for_override": 60,
...
function loadConfig(filepath, logger) {
    ...
    try {
        ...
        _ensureConfigKeys(config, DEFAULT_CONFIG);
...
class GeminiSignalAnalyzer {
    constructor(apiKey, logger, model = "gemini-1.5-flash-latest", config = {}) {
        ...
        this._analysis_cache = new Map();
        this._cache_ttl = this.config.cache_ttl_seconds || 300;
        ...
        this.daily_limit = this.config.daily_api_limit || 1000;
        this.last_reset = DateTime.now().setZone(TIMEZONE).toISODate();
    }
    ...
    _prepareMarketSummary(df_data, indicatorValues, currentPrice, symbol, mtfTrends) {
        ...
        const priceStats = {
            ...
            "24h_high": last96.high.max().values[0],
            "24h_low": last96.low.min().values[0],
            "24h_change_pct": ((currentPrice.minus(new Decimal(lastClose96))).div(new Decimal(lastClose96)).times(100)).toNumber(),
            "volume_24h": last96.volume.sum().values[0],
            "avg_volume": last96.volume.mean().values[0]
        };
...
class PositionManager {
    ...
    _calculateOrderSize(currentPrice, atrValue, aiPositionSizingInfo = null) {
        ...
        const roundedQty = orderQty.toDecimalPlaces(4, Decimal.ROUND_DOWN);
...
class TradingAnalyzer {
    constructor(klines, config, logger, symbol) {
        this.klines = klines; // Use klines (plain array) instead of df
        this.config = config;
        this.logger = logger;
        this.symbol = symbol;
        ...
        if (this.klines.length === 0) {
            this.logger.warning(
                `${NEON_YELLOW}TradingAnalyzer initialized with an empty klines array. Indicators will not be calculated.${RESET}`
            );
            return;

    _safeCalculate(func, name, minDataPoints = 0, ...args) {
        if (this.klines.length < minDataPoints) {
            this.logger.debug(
                `Skipping indicator '${name}': Not enough data. Need ${minDataPoints}, have ${this.klines.length}.`
            );
            return null;
        }
        try {
            ...
            if (result === null ||
                ...
                (Array.isArray(result) && result.every(val => typeof val === 'number' && isNaN(val)))
            ) {
                this.logger.warning(
    ...
    _calculateAllIndicators() {
        ...
        if (cfg.indicators.sma_10) {
            const sma10 = this._safeCalculate(() => ti.SMA.calculate({ period: isd.sma_short_period, values: closes }), "SMA_10", isd.sma_short_period);
            if (sma10) {
                this.klines.forEach((k, i) => k.SMA_10 = sma10[i] || NaN);
                this.indicator_values["SMA_10"] = sma10[sma10.length - 1];
            }
        }
        if (cfg.indicators.sma_trend_filter) {
            const smaLong = this._safeCalculate(() => ti.SMA.calculate({ period: isd.sma_long_period, values: closes }), "SMA_Long", isd.sma_long_period);
            if (smaLong) {
                this.klines.forEach((k, i) => k.SMA_Long = smaLong[i] || NaN);
                this.indicator_values["SMA_Long"] = smaLong[smaLong.length - 1];
            }
        }

        if (cfg.indicators.ema_alignment) {
            const emaShort = this._safeCalculate(() => ti.EMA.calculate({ period: isd.ema_short_period, values: closes }), "EMA_Short", isd.ema_short_period);
            const emaLong = this._safeCalculate(() => ti.EMA.calculate({ period: isd.ema_long_period, values: closes }), "EMA_Long", isd.ema_long_period);
            if (emaShort) {
                this.klines.forEach((k, i) => k.EMA_Short = emaShort[i] || NaN);
                this.indicator_values["EMA_Short"] = emaShort[emaShort.length - 1];
            }
            if (emaLong) {
                this.klines.forEach((k, i) => k.EMA_Long = emaLong[i] || NaN);
                this.indicator_values["EMA_Long"] = emaLong[emaLong.length - 1];
            }
        }

        const tr = this._safeCalculate(() => this.calculateTrueRange(), "TR", MIN_DATA_POINTS_TR);
        if (tr) {
            this.klines.forEach((k, i) => k.TR = tr[i] || NaN);
            const atr = this._safeCalculate(() => ti.ATR.calculate({ period: isd.atr_period, high: highs, low: lows, close: closes }), "ATR", isd.atr_period);
            if (atr) {
                this.klines.forEach((k, i) => k.ATR = atr[i] || NaN);
                this.indicator_values["ATR"] = atr[atr.length - 1];
            }
        }

        if (cfg.indicators.rsi) {
            const rsi = this._safeCalculate(() => ti.RSI.calculate({ period: isd.rsi_period, values: closes }), "RSI", isd.rsi_period + 1);
            if (rsi) {
                this.klines.forEach((k, i) => k.RSI = rsi[i] || NaN);
                this.indicator_values["RSI"] = rsi[rsi.length - 1];
            }
        }

        if (cfg.indicators.stoch_rsi) {
            const stochRsiResult = this._safeCalculate(() => ti.StochasticRSI.calculate({
                period: isd.stoch_rsi_period,
                values: closes,
                signalPeriod: isd.stoch_k_period,
            }), "StochRSI", isd.stoch_rsi_period + isd.stoch_k_period);

            if (stochRsiResult) {
                const stochRsiK = stochRsiResult.map(r => r.k);
                const stochRsiD = ti.SMA.calculate({ period: isd.stoch_d_period, values: stochRsiK });

                this.klines.forEach((k, i) => k.StochRSI_K = stochRsiK[i] || NaN);
                this.klines.forEach((k, i) => k.StochRSI_D = stochRsiD[i] || NaN);
                this.indicator_values["StochRSI_K"] = stochRsiK[stochRsiK.length - 1];
                this.indicator_values["StochRSI_D"] = stochRsiD[stochRsiD.length - 1];
            }
        }

        if (cfg.indicators.bollinger_bands) {
            const bbResult = this._safeCalculate(() => ti.BollingerBands.calculate({
                period: isd.bollinger_bands_period,
                values: closes,
                stdDev: isd.bollinger_bands_std_dev
            }), "BollingerBands", isd.bollinger_bands_period);

            if (bbResult) {
                const bbUpper = bbResult.map(r => r.upper);
                const bbMiddle = bbResult.map(r => r.middle);
                const bbLower = bbResult.map(r => r.lower);

                this.klines.forEach((k, i) => k.BB_Upper = bbUpper[i] || NaN);
                this.klines.forEach((k, i) => k.BB_Middle = bbMiddle[i] || NaN);
                this.klines.forEach((k, i) => k.BB_Lower = bbLower[i] || NaN);
                this.indicator_values["BB_Upper"] = bbUpper[bbUpper.length - 1];
                this.indicator_values["BB_Middle"] = bbMiddle[bbMiddle.length - 1];
                this.indicator_values["BB_Lower"] = bbLower[bbLower.length - 1];
            }
        }

        if (cfg.indicators.cci) {
            const cci = this._safeCalculate(() => ti.CCI.calculate({
            ...
            if (cci) {
                this.klines.forEach((k, i) => k.CCI = cci[i] || NaN);
                this.indicator_values["CCI"] = cci[cci.length - 1];
            }
        }
        }

        if (cfg.indicators.wr) {
            const wr = this._safeCalculate(() => ti.WilliamsR.calculate({
                period: isd.williams_r_period,
                high: highs,
                low: lows,
                close: closes
            }), "WR", isd.williams_r_period);
            if (wr) {
                this.klines.forEach((k, i) => k.WR = wr[i] || NaN);
                this.indicator_values["WR"] = wr[wr.length - 1];
            }
        }

        if (cfg.indicators.mfi) {
            const mfi = this._safeCalculate(() => ti.MFI.calculate({
                period: isd.mfi_period,
                high: highs,
                low: lows,
                close: closes,
                volume: volumes
            }), "MFI", isd.mfi_period + 1);
            if (mfi) {
                this.klines.forEach((k, i) => k.MFI = mfi[i] || NaN);
                this.indicator_values["MFI"] = mfi[mfi.length - 1];
            }
        }

        if (cfg.indicators.obv) {
            const obvVal = this._safeCalculate(() => ti.OBV.calculate({
                values: closes,
                volume: volumes
            }), "OBV", 1);

            if (obvVal) {
                this.klines.forEach((k, i) => k.OBV = obvVal[i] || NaN);
                this.indicator_values["OBV"] = obvVal[obvVal.length - 1];

                const obvEma = this._safeCalculate(() => ti.EMA.calculate({
                    period: isd.obv_ema_period,
                    values: obvVal
                }), "OBV_EMA", isd.obv_ema_period);

                if (obvEma) {
                    this.klines.forEach((k, i) => k.OBV_EMA = obvEma[i] || NaN);
                    this.indicator_values["OBV_EMA"] = obvEma[obvEma.length - 1];
                }
            }
        }

        if (cfg.indicators.cmf) {
            const cmfVal = this._safeCalculate(() => ti.CMF.calculate({
            ...
            if (cmfVal) {
                this.klines.forEach((k, i) => k.CMF = cmfVal[i] || NaN);
                this.indicator_values["CMF"] = cmfVal[cmfVal.length - 1];
            }
        }
        ...
        this.df.columns.forEach(col => {
            if (!['start_time'].includes(col) && this.df[col] instanceof df.Series) {
    ...
    calculateTrueRange() {
        if (this.klines.length < MIN_DATA_POINTS_TR) {
            return Array(this.klines.length).fill(NaN);
        }
        const high = this.klines.map(k => k.high);
        const low = this.klines.map(k => k.low);
        const close = this.klines.map(k => k.close);

        const tr = [];
        for (let i = 0; i < this.klines.length; i++) {
            const highLow = high[i] - low[i];
            const highPrevClose = i === 0 ? 0 : Math.abs(high[i] - close[i - 1]);
            const lowPrevClose = i === 0 ? 0 : Math.abs(low[i] - close[i - 1]);
    ...
    calculateSuperSmoother(series, period) {
        ...
        const series_numeric = series.dropNa();
    ...
    calculateRsi(period) {
        ...
        const gain = delta.clip(0, Infinity);
        const loss = delta.clip(-Infinity, 0).abs();
        ...
        const rsi = rs.replace(Infinity, 0).fillNa(0).add(1).pow(-1).mul(-100).add(100);
        return rsi;
    }
    ...
    calculateStochRsi(period, k_period, d_period) {
        ...
        const stochRsiKRawFilled = stochRsiKRaw.replace(Infinity, NaN).replace(-Infinity, NaN).fillNa(0);
    ...
    calculateAdx(period) {
        ...
        const plusDm = highDiff.gt(lowDiff.abs()) ? highDiff.clip(0, Infinity) : new df.Series(Array(this.df.shape[0]).fill(0), { index: this.df.index });
        const minusDm = lowDiff.abs().gt(highDiff) ? lowDiff.abs().clip(0, Infinity) : new df.Series(Array(this.df.shape[0]).fill(0), { index: this.df.index });
    ...
    calculateCci(period) {
        ...
        const mad = tp.rolling(period, { min_periods: period }).apply((series) => {
            ...
            return new df.Series(series.values).sub(mean).abs().mean().values[0];
        });
    ...
    calculateMfi(period) {
        ...
        const mfi = mfRatio.replace(Infinity, NaN).replace(-Infinity, NaN).fillNa(50).add(1).pow(-1).mul(-100).add(100);
        return mfi;
    }
    ...
    calculateCmf(period) {
        ...
        const mfmFilled = mfm.replace(Infinity, NaN).replace(-Infinity, NaN).fillNa(0);

        const mfv = mfmFilled.mul(this.df.volume);

        const cmf = mfv.rolling(period).sum().div(this.df.volume.rolling(period).sum());
        const cmfFilled = cmf.replace(Infinity, NaN).replace(-Infinity, NaN).fillNa(0);
    ...
    calculatePsar(acceleration, max_acceleration) {
        ...
        for (let i = 1; i < this.df.shape[0]; i++) {
            ...
            if (currentBull) {
                newPsar = Math.min(newPsar, currentLow, psar.iloc({rows: [i -1]}).values[0]);
            } else {
                newPsar = Math.max(newPsar, currentHigh, psar.iloc({rows: [i -1]}).values[0]);
            }
        ...
        try {
            ...
            if (
                ...
                if (newPos && aiSignalAtOpen) {
                    newPos.ai_signal = aiSignalAtOpen;
                }
            ...
            console.error(e);
            await new Promise(resolve => setTimeout(resolve, config.loop_delay * 2 * 1000));
        }
    }
}
