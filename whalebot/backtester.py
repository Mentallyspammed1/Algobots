...
class Backtester:
    ...
    async def run_backtest(
        ...
        while fetched_count < max_fetches:
            ...
            if response and response["result"] and response["result"]["list"]:
                ...
                all_klines_df = (
                    pd.concat([df, all_klines_df]).drop_duplicates().sort_index()
                )

                # Update current_end_time to the start_time of the oldest bar fetched
                current_end_time = (
                    int(df.index.min().timestamp() * 1000) - 1
        ...
        klines_df = all_klines_df[
            (all_klines_df.index >= start_date) & (all_klines_df.index <= end_date)
        ]
        ...
        for i, (timestamp, bar) in enumerate(klines_df.iterrows()):
            ...
            elif (self.side == "buy" and signal == "SELL") or (
    ...
    async def _execute_trade(
        ...
        trade_amount_usd = (
            self.capital * Decimal("0.01") * self.leverage
        )  # 1% of capital, leveraged
    ...
    def _generate_report(
        ...
        winning_trades = [
            t for t in trade_history if t["type"] == "exit" and t["pnl"] > 0
        ]
        losing_trades = [
            t for t in trade_history if t["type"] == "exit" and t["pnl"] < 0
        ]
