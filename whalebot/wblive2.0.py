...
class PybitTradingClient:
    def __init__(self, config: dict[str, Any], logger: logging.Logger):
        ...
        self.rate_limit_window_start = (
            0  # Timestamp when current rate limit window started
        )
    ...
    def _check_rate_limit(
        ...
        if self.dry_run:
            return  # No rate limits in dry run
    ...
    def place_order(self, **kwargs) -> dict | None:
        ...
        if (
            self.cfg.get("execution", {}).get("slippage_adjustment", False)
            and kwargs.get("orderType") == "Limit"
        ):
    ...
    def get_open_orders(self, symbol: str) -> dict | None:
        ...
        def _get_open_orders():
            return self.session.get_open_orders(category=self.category, symbol=symbol)
...
class PositionManager:
    ...
    def manage_positions(self, current_price: Decimal, performance_tracker: Any):
        ...
        for i, pos in enumerate(self.open_positions):
            if pos["status"] == "OPEN":
                ...
                elif (pos["side"] == "BUY" and current_price >= pos["take_profit"]) or (
                ...
                if closed_by:
                    ...
                    # Update peak balance
                    current_balance = (
                        self._get_current_balance() + performance_tracker.total_pnl
                    )
    ...
    def _compute_stop_loss_price(
        ...
        buffer = (
            tick_size * 5
        )  # Small buffer to avoid immediate re-trigger on price fluctuations
    ...
    def check_risk_limits(self) -> tuple[bool, str]:
        ...
        if self.daily_pnl < 0:
            ...
            if self.initial_balance <= 0:
                return (
                    False,
                    "Initial account balance is zero or negative, cannot calculate daily loss percentage.",
                )

            daily_loss_pct = (
                abs(self.daily_pnl) / self.initial_balance * 100
            )  # Calculate against initial balance for the day
            if daily_loss_pct > self.max_day_loss_pct:
                return (
                    False,
                    f"Daily loss limit breached: {daily_loss_pct:.2f}% > {self.max_day_loss_pct}%",
                )
        ...
        if self.peak_balance > 0:
            ...
            if drawdown > self.max_drawdown_pct:
                return (
                    False,
                    f"Max drawdown limit breached: {drawdown:.2f}% > {self.max_drawdown_pct}%",
                )
    ...
    def _reset_daily_stats_if_new_day(self):
        ...
        if current_date > self._last_day_reset:
            ...
            if self._last_day_reset in self.daily_stats:
                self.daily_stats[self._last_day_reset]["end_pnl"] = self.daily_stats[
                    self._last_day_reset
                ]["pnl"]
...
class ExchangeExecutionSync:
    def __init__(
        ...
        self.last_exec_time_ms = (
            int(time.time() * 1000) - 5 * 60 * 1000
        )  # Look back 5 minutes initially
    ...
    def _is_ours(self, link_id: str | None) -> bool:
        ...
        if not self.cfg["execution"]["live_sync"]["only_track_linked"]:
            return True
        return (
            link_id.startswith("wgx_")
            or link_id.startswith("hb_")
            or link_id.startswith("sync_")
    ...
    def _move_stop_to_breakeven(self, open_pos: dict, atr_value: Decimal):
        ...
        try:
            ...
            old_sl_link = (
                f"{link_prefix}_sl" if link_prefix else None
            )  # Initial SL link
            ...
            for r in rows:
                ...
                open_pos = next(
                    (
                        ...
                        and link.startswith(p.get("link_prefix", "NO_PREFIX"))
                    ),
                    None,
                )
                ...
                if tag == "ENTRY" and open_pos:
                    ...
                    if open_pos.get("entry_price") == open_pos.get(
                        ...
                        open_pos["original_entry_price"] = (
                            exec_price  # Store original to track average
                        )
    ...
    def _check_open_orders(self):
        ...
        try:
            ...
            current_order_link_ids = {
                order.get("orderLinkId")
                for order in open_orders
                if self._is_ours(order.get("orderLinkId"))
            }

            # Remove orders from pending_orders that are no longer open on exchange
            to_remove = [
                link_id
                for link_id in self.pending_orders
                if link_id not in current_order_link_ids
            ]
...
class TradingAnalyzer:
    ...
    def _calculate_all_indicators(self) -> None:
        ...
        if cfg["indicators"].get("fibonacci_levels", False):
            ...
            fib_levels = self._safe_calculate(
                indicators.calculate_fibonacci_levels,
                "FibonacciLevels",
                df=self.df,
                window=isd["fibonacci_window"],
            )
            if fib_levels:
    ...
    def _dynamic_threshold(self, base_threshold: float) -> float:
        ...
        if (
            ...
            or self.df["ATR"].rolling(50).mean().empty
        ):
    ...
    def generate_trading_signal(
        ...
        if active.get("macd"):
            ...
            if (
                ...
                and not np.isnan(hist)
                and not np.isnan(prev_hist)
            ):
                if (
        ...
        if active.get("adx"):
            ...
            if not np.isnan(adx) and adx > 20:  # ADX > 20 indicates a trend
                if pdi > mdi:
                    score += current_weights.get("adx_strength", 0) * (
                        adx / 50.0
        ...
        if active.get("momentum"):
            ...
            if not np.isnan(rsi):
                if rsi < isd.get("rsi_oversold", 30):
                    mom_score_raw += 1
                elif rsi > isd.get("rsi_overbought", 70):
                    mom_score_raw -= 1
            ...
            if not np.isnan(cci):
                if cci < isd.get("cci_oversold", -100):
                    mom_score_raw += 1
                elif cci > isd.get("cci_overbought", 100):
                    mom_score_raw -= 1

            # Williams %R
            wr = self._nz(self._get_indicator_value("WR"))
            if not np.isnan(wr):
                if wr < isd.get("williams_r_oversold", -80):
                    mom_score_raw += 1
                elif wr > isd.get("williams_r_overbought", -20):
                    mom_score_raw -= 1

            # MFI
            mfi = self._nz(self._get_indicator_value("MFI"))
            if not np.isnan(mfi):
                if mfi < isd.get("mfi_oversold", 20):
                    mom_score_raw += 1
                elif mfi > isd.get("mfi_overbought", 80):
                    mom_score_raw -= 1
        ...
        if active.get("volatility_index"):
            ...
            if (
                "Volatility_Index" in self.df
                and len(self.df["Volatility_Index"]) >= 50
                and not self.df["Volatility_Index"].rolling(50).mean().empty
            ):
                vol_idx_ma = self.df["Volatility_Index"].rolling(50).mean().iloc[-1]
                if (
                    not np.isnan(vol_idx)
                    and not np.isnan(vol_idx_ma)
                    and vol_idx > vol_idx_ma * 1.5
                ):
        ...
        if active.get("fibonacci_levels"):
            ...
            if fib_levels and isinstance(fib_levels, dict):
                ...
                if "0.5" in fib_levels and "0.618" in fib_levels:
                    ...
                    if (
                        abs(price_decimal - level_50) < tolerance
                        or abs(price_decimal - level_618) < tolerance
                    ):
                        ...
                        if (
                            regime == "RANGING"
                        ):  # Fib levels might be more significant in ranging markets
...
def adapt_exit_params(pt: PerformanceTracker, cfg: dict) -> tuple[Decimal, Decimal]:
    ...
    if not recent or len(recent) < 20:
        return tp_mult, sl_mult  # Need sufficient data
...
def main() -> None:
    ...
    if (
        config["execution"]["live_sync"]["enabled"]
        and pybit_client.enabled
    ...
    if (
        config["execution"]["live_sync"]["heartbeat"]["enabled"]
        and pybit_client.enabled
        and not pybit_client.dry_run
    ):
