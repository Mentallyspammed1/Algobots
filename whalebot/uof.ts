// Pyrmethus Supreme: Final Consolidated Oracle v11.1 - Streamlined & Enhanced
...
const CONFIG = {
    ...
    ATR_FALLBACK_PERCENT: 0.005, // Fallback ATR percentage if calculation fails
...
function validateSymbol(symbol) { return symbol && typeof symbol === 'string' && symbol.length >= 3 && symbol.endsWith("USDT"); }
...
async function analyzeLogsAndGenerateSignal(logs, config) {
    ...
    let indicatorSnapshot = { ATR: null, RSI: null, MACD_Line: null, VWAP: null, PSAR_Val: null, EMA_Short: null };
    ...
    if (finalStatusLine) {
        ...
        const strength = Math.abs(score) >= config.signal_score_threshold * 1.5 ? "HIGH" : "MEDIUM";
        const rationale = `Python bot concluded with FINAL SIGNAL: ${signal} (Score ${score.toFixed(2)}). Path: FAST SCORE MATCH.`;

        // Attempt to fetch live price if available for more accurate TP/SL calculation
        lastPrice = await fetchCurrentPrice(assetSymbol);
...
async function runGeminiAnalysis(logs, symbol, price, snapshot, config, liveMode = false) {
    ...
    try {
        ...
        const signalData = JSON.parse(text.trim());
...
function createGeminiInferencePrompt(logs, symbol, price, snapshot, config) {
    ...
    const indicatorDetails = Object.entries(snapshot)
        .map(([key, value]) => `${key}: ${value === null || value === 'null' || isNaN(value) ? 'N/A' : (typeof value === 'number' ? value.toFixed(4) : value)}`)
        .join(', ');

    return `
    You are Pyrmethus, an advanced AI trading oracle. Analyze the provided trading data and generate a precise trading signal.

    **Context:**
    - **Symbol:** ${symbol}
    - **Current Estimated Price:** ${price ? price.toFixed(2) : 'N/A'}
    - **Active Indicators:** ${TA_CONTEXT}
    - **Indicator Snapshot:** ${indicatorDetails}
    - **Configuration:**
        - Signal Score Threshold: ${config.signal_score_threshold}
        - Risk Per Trade: ${config.trade_management.risk_per_trade_percent}%
        - SL Multiplier: ${CONFIG.SL_MULTIPLIER}
        - TP Multiplier: ${CONFIG.TP_MULTIPLIER}
        - Overbought Threshold: ${CONFIG.OVERBOUGHT_THRESHOLD}
        - Oversold Threshold: ${CONFIG.OVERSOLD_THRESHOLD}
        - Weight Set: ${JSON.stringify(config.weight_sets.default_scalping)}

    **Trading Log Analysis (if available):**
    ${logs ? logs.slice(-10000) : 'No specific log analysis available for this run.'}

    **Task:**
    Based on the above information, provide a JSON output with the following structure:
    {
      "AssetSymbol": "string",          // The trading symbol (e.g., "BTCUSDT")
      "CurrentPriceEstimate": number,   // The estimated current price
      "SignalDirection": "BUY" | "SELL" | "HOLD", // The trading signal
      ...
      "Strength": "LOW" | "MEDIUM" | "HIGH", // Confidence level of the signal
      "Rationale": "string",            // A brief explanation for the signal
      "IndicatorSnapshot": object,      // A snapshot of the key indicators used
      "ExecutionPath": "string"         // How this signal was generated (e.g., "DEEP GEMINI INFERENCE")
    }

    **Instructions for Signal Generation:**
    1.  **Analyze all provided data:** Consider the current price, indicator values, and any relevant log information.
    2.  **Determine Signal Direction:** Decide whether to BUY, SELL, or HOLD. Prioritize safety and conservativeness.
    3.  **Calculate Entry, TP, SL:**
        *   Entry Price: Should be close to the current estimated price.
        *   Stop Loss: Calculate based on ATR and SL_MULTIPLIER. If ATR is unavailable, use a small, sensible percentage (e.g., 0.5%) of the current price.
        *   Take Profit: Calculate based on ATR and TP_MULTIPLIER, aiming for a risk/reward ratio of at least 1:1.5.
        *   If SignalDirection is "HOLD", EntryPrice, TakeProfit, and StopLoss should be equal to CurrentPriceEstimate.
    4.  **Assess Strength:** Assign "LOW", "MEDIUM", or "HIGH" based on the confluence of signals and confidence. If indicators are conflicting or data is scarce, default to LOW or MEDIUM.
    5.  **Provide Rationale:** Briefly explain the reasoning, mentioning key indicators or patterns.
    6.  **Update IndicatorSnapshot:** Include the calculated indicator values.
    7.  **Set ExecutionPath:** This should be "DEEP GEMINI INFERENCE (Log Mode)".

    **Output Format:** Strictly JSON. Do not include any introductory or concluding text outside the JSON object.
    `;
}
...
async function runLiveAnalysis(symbol, config) {
    ...
    const klinesRaw = await fetchKlines(symbol, config.interval, 200); // Fetch enough bars for indicators
    const currentPrice = await fetchCurrentPrice(symbol);

    if (!klinesRaw || klinesRaw.length < 100 || !currentPrice) {
        log.error(`Failed to retrieve sufficient live data for ${symbol}. Klines: ${klinesRaw?.length}, Price: ${currentPrice}`);
    ...
    // Calculate Key Indicators for Snapshot if enabled in config
    if (config.indicators.atr_indicator) {
    ...
    if (config.indicators.ema) {
        if (config.indicator_settings.ema_short_period) {
            snapshot.EMA_Short = calculateEMA(closes, config.indicator_settings.ema_short_period)?.[closes.length - 1];
        }
        if (config.indicator_settings.ema_long_period) {
            snapshot.EMA_Long = calculateEMA(closes, config.indicator_settings.ema_long_period)?.[closes.length - 1];
        }
    }

    log.info(`Indicators calculated successfully from ${klines.length} bars.`);

    // Create a log entry for Gemini context
    const logs = `--- LIVE DATA ANALYSIS --- ASSET: ${symbol}, PRICE: ${currentPrice}. Snapshot generated. Indicators: ${Object.keys(snapshot).map(k => `${k}: ${snapshot[k]}`).join(', ')}.`;
...
async function executeOracle() {
    ...
    if (!validation.valid) {
        log.error(`Configuration validation failed: ${validation.errors.join(', ')}. Please fix your config file.`);
    ...
    let signalResult: SignalOutput | undefined;

    try {
        if (mode === 'L') {
            const selectedSymbol = promptUserForSymbol();
            config.symbol = selectedSymbol; // Update config with user's symbol for live run
            signalResult = await runLiveAnalysis(selectedSymbol, config);
        } else {
            // Past Log Analysis Mode
            if (!fs.existsSync(config.LOG_FILE_PATH)) {
                log.warning(`Log file not found at ${config.LOG_FILE_PATH}. Defaulting to Live analysis for ${config.symbol}.`);
        ...
        if (signalResult) {
            manifestSignal(signalResult);
             ...
             log.error("Signal generation failed, no result produced.");
        }

    } catch (error) {
        log.error(`Oracle failed during final execution: ${error.message}`);
        // Attempt to manifest a fallback signal if an error occurred before getting one
        if (!signalResult) {
            const fallbackSymbol = config.symbol || CONFIG.DEFAULT_SYMBOL;
            try {
                const fallbackPrice = await fetchCurrentPrice(fallbackSymbol) || 0;
                signalResult = createFallbackSignal(fallbackSymbol, fallbackPrice, {}, config, "UNEXPECTED_EXECUTION_ERROR");
                manifestSignal(signalResult);
            } catch (fallbackError) {
                log.error(`Failed to generate even a fallback signal: ${fallbackError.message}`);
            }
        }
    }
}
