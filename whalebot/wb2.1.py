...
def round_qty(qty: Decimal, qty_step: Decimal) -> Decimal:
    ...
    if qty_step is None or qty_step.is_zero():
        return qty.quantize(
            Decimal("1e-6"), rounding=ROUND_DOWN
...
class PybitTradingClient:
    ...
    def __init__(self, config: dict[str, Any], logger: logging.Logger):
        ...
        if proxy_conf.get("enabled", False):
            ...
            if proxies:
                ...
                self.logger.warning(
                    f"{NEON_YELLOW}Proxy enabled but no URLs provided.{RESET}"
                )
    ...
    def _log_api(self, action: str, resp: dict | None, log_level: str = "error"):
        ...
        if not self._ok(resp):
            getattr(self.logger, log_level)(
                f"{NEON_RED}{action}: Failed with code {resp.get('retCode')} - {resp.get('retMsg')}{RESET}"
            )
    ...
    def _q(self, x: Any) -> str:
        return str(x)  # Helper to convert value to string for API params
    ...
    def _handle_403_error(self, e: Exception):
        ...
        if isinstance(e, pybit.exceptions.FailedRequestError) and e.status_code == 403:
            self.logger.error(
                f"{NEON_RED}API Error 403 Forbidden: Check API key permissions and IP whitelist settings on Bybit. Disabling Pybit client temporarily.{RESET}"
            )
    ...
    def get_open_orders(self, symbol: str | None = None) -> list[dict]:
        ...
        try:
            params = {
                "category": self.category,
                "openOnly": 0,
            }  # 0 for all active orders, 1 for open only
            if symbol:
                params["symbol"] = symbol
    ...
    def fetch_klines(
        ...
        try:
            ...
            if self._ok(resp) and resp.get("result", {}).get("list"):
                ...
                return df
            self.logger.warning(
                f"Failed to fetch klines for {symbol} {interval}. Response: {resp}"
            )
            return None
    ...
    def fetch_orderbook(self, symbol: str, limit: int) -> dict | None:
        ...
        try:
            ...
            if self._ok(resp) and resp.get("result"):
                return resp["result"]
            self.logger.warning(
                f"Failed to fetch orderbook for {symbol}. Response: {resp}"
            )
            return None
    ...
    def fetch_instrument_info(self, symbol: str) -> dict | None:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to fetch instrument info for {symbol}. Response: {resp}"
            )
            return None
        except (pybit.exceptions.FailedRequestError, KeyError) as e:
            self.logger.error(
    ...
    def get_private_url(self) -> dict | None:
        ...
        try:
            ...
            self.logger.debug(
                "Pybit unified trading WebSocket authentication typically uses signed messages, not a separate 'listenKey' endpoint."
            )
            ...
            return {
                "retCode": 0,
                "result": {"listenKey": "dummy_listen_key_pybit"},
            }  # Placeholder
        except Exception as e:
            self.logger.error(
                f"get_private_url (placeholder) exception: {e}\n{traceback.format_exc()}"
            )
            self._handle_403_error(e)
            return None
    ...
    def _initialize_websocket(self):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to initialize WebSocket Manager: {e}\n{traceback.format_exc()}"
            )
    ...
    def save_state(self):
        ...
        state_data = {
            ...
            "last_exec_time_ms": self.ws_manager.last_exec_time_ms
            if self.ws_manager
            else 0,
    ...
    def load_state(self):
        ...
        try:
            ...
            if self.ws_manager:
                ...
                self.ws_manager.last_exec_time_ms = state_data.get(
                    "last_exec_time_ms", 0
                )
                self.state_data = (
                    state_data  # Store loaded data for other components if needed
                )
                self.logger.info(
                    f"Bot state loaded. Positions: {len(self.ws_manager.positions)}, Orders: {len(self.ws_manager.orders)}"
                )
            return True
        except (OSError, json.JSONDecodeError) as e:
...
class WebSocketManager:
    ...
    def start(self):
        ...
        self.public_ws_thread = threading.Thread(
            target=self._run_ws,
            args=(self.public_ws_url, "public"),
            daemon=True,
            name="BybitPublicWS",
        )
        ...
        if self.api_client.enabled:
            self.private_ws_thread = threading.Thread(
                target=self._run_ws,
                args=(self.private_ws_url, "private"),
                daemon=True,
                name="BybitPrivateWS",
            )
    ...
    def _run_ws(self, url: str, ws_type: Literal["public", "private"]):
        ...
        while not self.stop_event.is_set():
            ...
            if (
                ws_app and ws_app.connected
            ):  # Check if already connected (websocket-client library attribute)
                try:
                    ws_app.send(json.dumps({"op": "ping"}))  # Send ping for keepalive
                except Exception:
                    ws_app.close()  # Force reconnect on ping failure
                time.sleep(
            ...
            try:
                self.logger.info(
                    f"Attempting {ws_type} WebSocket connection to {url}..."
                )
                ws = websocket.WebSocketApp(
                    url,
                    on_message=self.on_message,
                    on_error=self.on_error,
                    on_close=lambda ws, status, msg, ws_type=ws_type: self.on_close(
                        ws, status, msg, ws_type
                    ),
                    on_open=lambda ws, ws_type=ws_type: self.on_open(ws, ws_type),
                )
                setattr(self, ws_app_attr, ws)  # Store the WebSocketApp instance
                ws.run_forever(
                    ping_interval=HEARTBEAT_INTERVAL_MS / 1000.0,
                    ping_timeout=HEARTBEAT_INTERVAL_MS / 2000.0,
                )  # Blocks
            except Exception as e:
                self.logger.error(
                    f"WebSocket run_forever error ({ws_type}): {e}. Retrying in {self.connect_delay}s.\n{traceback.format_exc()}"
                )
                time.sleep(self.connect_delay)
                self.connect_delay = min(
                    self.connect_delay * 2, 60
                )  # Exponential backoff
    ...
    def on_close(self, ws, close_status_code, close_msg, ws_type: str):
        ...
        self.logger.warning(
            f"{ws_type} WebSocket closed: {close_status_code} {close_msg}. Reconnecting..."
        )
        if ws_type == "public":
            self.ws_public = None
        else:
            self.ws_private = (
                None  # Clear WS object to trigger reconnect logic in _run_ws
            )
    ...
    def _subscribe_channels(self, ws: websocket.WebSocketApp, ws_type: str):
        ...
        if ws_type == "public":
            ...
            )  # Level 1 orderbook, check config for limit
            self.logger.info(
                f"Subscribing to public WS topics for {symbol}: {', '.join(subscriptions)}"
            )
            ...
            self.logger.info(
                f"Subscribing to private WS topics: {', '.join(subscriptions)}"
            )
        ...
        if subscriptions:
            ...
            payload = {
                "op": "subscribe",
                "args": [
                    f"{self.cfg['execution']['category']}.{s}"
                    if ws_type == "private"
                    else s
            ...
            try:
                ...
                self.logger.error(
                    f"Error sending WS subscription for {ws_type} topics: {e}\n{traceback.format_exc()}"
                )
    ...
    def _sync_initial_state(self):
        ...
        if pos_data and self.api_client._ok(pos_data):
            ...
            self.logger.warning(
                f"Could not sync initial positions for {symbol}. Response: {pos_data}"
            )
        ...
        if open_orders_data:
            ...
            self.logger.info(f"No open orders found for {symbol} during initial sync.")

        # Initialize last_exec_time_ms for fetching executions
        now_ms = int(time.time() * 1000)
        self.last_exec_time_ms = max(
            self.last_exec_time_ms, now_ms - 5 * 60 * 1000
        )  # Start fetch from 5 mins ago
    ...
    def _process_ws_message(self, data):
        ...
        if "op" in data and data["op"] == "subscribe":
            if data.get("success"):
                self.logger.info(
                    f"WS Subscription successful for args: {data.get('req_id')}"
                )
            elif data.get("retMsg"):
                self.logger.error(
                    f"WS Subscription failed for args: {data.get('req_id')} - {data.get('retMsg')}"
                )
            else:
                self.logger.warning(
                    f"WS Subscription unacknowledged or failed without message for args: {data.get('req_id')}"
                )
            return
        ...
        if topic and topic.startswith(f"{self.cfg['execution']['category']}."):
            if "order" in topic:
                ...
                self._handle_account_update(data.get("data", {}))
        # Handle public topic messages
        elif topic:
            if topic.startswith("kline."):
                self._handle_kline_update(data.get("data", []))
            elif topic.startswith("orderbook."):
                self._handle_orderbook_update(data.get("data", {}))
        elif data.get("retCode") == 0:
            self.logger.debug(f"WS General Success: {data.get('retMsg', 'OK')}")
        elif data.get("retCode") is not None and data.get("retCode") != 0:
            self.logger.error(
                f"WS Error Response: {data.get('retCode')} - {data.get('retMsg')}"
            )
    ...
    def _handle_kline_update(self, kline_data: list[dict]):
        ...
        for candle_info in kline_data:
            if (
                candle_info.get("confirm") is True
    ...
    def add_listener(self, event_type: str, callback: Callable):
        ...
        if event_type in self.listeners:
            ...
            self.logger.warning(
                f"Attempted to add listener for unknown event type: {event_type}"
            )
        ...
        for callback in self.listeners.get(event_type, []):
            try:
                ...
                self.logger.exception(
                    f"Error executing listener for event '{event_type}'"
                )
...
class PositionManager:
    ...
    def _load_state(self):
        ...
        if self.pybit.ws_manager and self.symbol in self.pybit.ws_manager.positions:
            ...
            self.logger.info(
                f"Loaded initial position state from WS for {self.symbol}."
            )
    ...
    def _convert_ws_position_to_local(self, ws_pos: dict) -> dict:
        ...
        return {
            ...
            ),  # 'time' is deprecated, use 'createdTime'
            ...
            "stop_loss": Decimal(
                ws_pos.get("stopLoss", "0")
            ),  # Note: SL/TP might need separate tracking if not in position update
    ...
    def _update_precision_from_exchange(self):
        ...
        if not self.live or not self.pybit or not self.pybit.enabled:
            self.logger.warning(
                f"Pybit client not ready. Cannot fetch precision for {self.symbol}. Using config values."
            )
            return
        ...
        if info:
            ...
            self.logger.warning(
                f"Could not fetch instrument info for {self.symbol}. Using config values."
            )
    ...
    def _calculate_order_size(
        ...
        if not self.config["trade_management"]["enabled"]:
            return Decimal("0")
        ...
        if stop_loss_distance <= 0 or current_price <= 0:
            self.logger.warning(
                f"Invalid SL distance ({stop_loss_distance}) or current price ({current_price}). Cannot calculate order size."
            )
            return Decimal("0")

        order_value = (
            account_balance * risk_pct
        ) / stop_loss_distance  # USD risked per unit price
        order_qty = order_value / current_price  # Convert USD risk to quantity

        return round_qty(
            order_qty, self.qty_step
    ...
    def _get_position_by_link_prefix(self, link_prefix: str) -> dict | None:
        ...
        return next(
            (p for p in self.open_positions if p.get("link_prefix") == link_prefix),
            None,
        )
    ...
    def open_position(
        ...
        if (
            not self.config["trade_management"]["enabled"]
            or len(self.get_open_positions()) >= self.max_open_positions
        ):
            self.logger.info(
                f"Cannot open position: Max limits reached ({len(self.get_open_positions())}/{self.max_open_positions}) or trade management disabled."
            )
            return None
        ...
        if order_qty <= 0:
            self.logger.warning(
                "Order quantity calculated as zero. Cannot open position."
            )
            return None
        ...
        position_data = {
            ...
            "symbol": self.symbol,
            "side": signal,
            "entry_price": round_price(adjusted_entry_price_sim, self.price_precision),
            "qty": order_qty,
            "stop_loss": stop_loss,
        ...
        if self.live and self.pybit and self.pybit.enabled:
            ...
            try:
                resp = self.pybit.place_order(
                    ...
                    isLeverage=1,
                    tpSlMode="Partial",  # Specify Partial mode for TP/SL
                )
                if self.pybit._ok(resp):
                    position_data["order_id"] = resp["result"]["orderId"]
                    self.logger.info(
                        f"Live entry submitted (Order ID: {position_data['order_id']}). Setting TP/SL."
                    )
    ...
    def set_tpsl_for_position(
        self, position: dict, current_price: Decimal, atr_value: Decimal
    ):
        ...
        pos_idx = self.pybit._pos_idx(
            position["side"]
        )  # Get correct positionIdx if hedge mode

        # --- Place Stop Loss Order ---
        if sl_cfg["use_conditional_stop"]:
            sl_price = self._compute_stop_loss_price(
                position["side"], position["entry_price"], atr_value
            )
            sl_link_id = (
                f"{position.get('link_prefix', 'pos')}_sl"  # Generate unique link ID
            )

            self.logger.info(
                f"Placing conditional SL for {self.symbol} at {sl_price}..."
            )
            sl_resp = self.pybit.place_order(
                ...
                orderType=sl_cfg["stop_order_type"],
                qty=self.pybit._q(position["qty"]),
                reduceOnly=True,
                orderLinkId=sl_link_id,
                triggerPrice=self.pybit._q(sl_price),
                triggerDirection=(
                    2 if position["side"] == "BUY" else 1
                ),  # Trigger condition based on side
                orderFilter="Stop",
                slTriggerBy=sl_cfg["sl_trigger_by"],
            )
            if self.pybit._ok(sl_resp):
                position["stop_loss_order_id"] = sl_resp["result"]["orderId"]
                self.logger.info(
                    f"Conditional SL placed successfully at {sl_price} (Order ID: {position['stop_loss_order_id']})."
                )
            else:
                self.logger.error(
                    f"Failed to place conditional SL: {sl_resp.get('retMsg')}"
                )
        ...
        if tp_scheme["mode"] == "atr_multiples" and tp_scheme.get("targets"):
            ...
            for target_detail in tp_requests:
                payload = {
                    ...
                    "tpSlMode": "Partial",  # Ensure Partial mode
                }
                batch_orders.append(payload)

            if batch_orders:
                self.logger.info(
                    f"Placing {len(batch_orders)} partial TP orders for {self.symbol}..."
                )
                ...
                if self.pybit._ok(batch_resp):
                    for res in batch_resp["result"]["list"]:
                        if res.get("orderId"):
                            ...
                            self.logger.info(
                                f"Partial TP placed successfully (Order ID: {res['orderId']})."
                            )
                        else:
                            self.logger.warning(
                                f"Failed to place one partial TP: {res.get('retMsg')}"
                            )
                else:
                    self.logger.error(
                        f"Batch TP placement failed: {batch_resp.get('retMsg')}"
                    )
            else:
                self.logger.info("No valid partial TP targets generated.")
    ...
    def build_partial_tp_targets(
        ...
        for i, target_cfg in enumerate(tp_targets_config):
            if cumulative_qty_pct >= Decimal("1.0"):
        ...
        if cumulative_qty_pct < Decimal("1.0") and tp_requests:
            ...
            if remaining_qty > 0:
                tp_requests[-1]["qty"] += (
                    remaining_qty  # Add remaining to the last target
                )
                tp_requests[-1]["qty"] = round_qty(
                    tp_requests[-1]["qty"], qty_step
                )  # Re-round
    ...
    def manage_positions(self, current_price: Decimal, performance_tracker: Any):
        ...
        if (
            self.live
            or not self.config["trade_management"]["enabled"]
        ...
        for i, pos in enumerate(self.open_positions):
            if pos["status"] == "OPEN":
                ...
                if closed_by:  # If TP or SL hit in simulation
                    pos.update(
                        {
                            ...
                            "exit_price": round_price(
                                adjusted_exit_price_sim, self.price_precision
                            ),
                            "closed_by": closed_by,
                        }
                    )
                    ...
                    self.logger.info(
                        f"Closed {pos['side']} simulated position by {closed_by}. PnL: {pnl.normalize():.4f}"
                    )
                    indices_to_remove.append(i)

        # Remove closed positions from the local list
        self.open_positions = [
            p for i, p in enumerate(self.open_positions) if i not in indices_to_remove
        ]
    ...
    def trail_stop(self, pos: dict, current_price: Decimal, atr_value: Decimal):
        ...
        if pos.get("status") != "OPEN" or self.live:
            return  # Only for simulated, open positions

        side, atr_mult = pos["side"], self.stop_loss_atr_multiple
        pos["best_price"] = pos.get(
    ...
    def try_pyramid(self, current_price: Decimal, atr_value: Decimal):
        ...
        for pos in self.open_positions:
            ...
            if adds >= int(py_cfg.get("max_adds", 0)):
                continue  # Max pyramiding adds reached
            ...
            target_price = (
                pos["entry_price"] + step_distance * (adds + 1)
                if pos["side"] == "BUY"
                else pos["entry_price"] - step_distance * (adds + 1)
            )
            ...
            if should_add:
                size_pct_of_initial = Decimal(
                    str(py_cfg.get("size_pct_of_initial", 0.5))
                )
                add_qty = round_qty(
                    pos["qty"] * size_pct_of_initial, self.qty_step
                )  # Calculate quantity for the add
                ...
                if add_qty > 0:
                    ...
                    self.logger.info(
                        f"Pyramiding add #{pos['adds']} qty={add_qty.normalize()}. New avg price: {pos['entry_price'].normalize():.4f}"
                    )
...
class PerformanceTracker:
    ...
    def __init__(self, logger: logging.Logger, config: dict):
        ...
        self._last_day_reset = datetime.now(
            TIMEZONE
        ).date()  # Date when daily stats were last reset
    ...
    def load_state(self, state_data: dict) -> bool:
        ...
        summary = state_data.get(
            "performance_tracker", {}
        )  # Assume performance tracker state is nested
        try:
            ...
            self.logger.info(
                f"Performance tracker state loaded. Total PnL: {self.total_pnl:.4f}"
            )
            return True
        except (InvalidOperation, TypeError, ValueError, KeyError) as e:
            self.logger.error(
                f"Error loading performance tracker state: {e}. Starting fresh.{traceback.format_exc()}"
            )
            return False
...
class ExchangeExecutionSync:
    ...
    def __init__(
        ...
        self.symbol, self.pybit, self.cfg, self.pm, self.pt = (
            symbol,
            pybit_client,
            cfg,
            pm,
        ...
        self.last_exec_time_ms = (
            pybit_client.ws_manager.last_exec_time_ms
            if pybit_client.ws_manager
            else int(time.time() * 1000) - 5 * 60 * 1000
        )
    ...
    def _is_ours(self, link_id: str | None) -> bool:
        ...
        if not self.cfg["execution"]["live_sync"]["only_track_linked"]:
            return True  # If not tracking linked, assume all are ours
        return link_id.startswith("wgx_") or link_id.startswith(
            "ws_"
    ...
    def _compute_breakeven_price(
        ...
        if offset_type == "atr":
            ...
            offset_amount = (
                offset_value  # Use as absolute value if type is unrecognized
            )
    ...
    def _move_stop_to_breakeven(self, open_pos: dict, atr_value: Decimal):
        ...
        try:
            ...
            old_sl_id, old_sl_link = (
                open_pos.get("stop_loss_order_id"),
                f"{open_pos.get('link_prefix')}_sl",
            )
            if old_sl_id:
                self.pybit.cancel_order(self.symbol, order_id=old_sl_id)
            elif old_sl_link:
                self.pybit.cancel_order(
                    self.symbol, order_link_id=old_sl_link
                )  # Try cancel by link ID
            else:
                self.pybit.logger.warning(
                    f"Could not find order ID or link ID for old SL of position {open_pos.get('link_prefix')}."
                )
            ...
            if remaining_qty <= 0:
                self.pybit.logger.warning(
                    f"No remaining quantity for position {open_pos.get('link_prefix')} to place breakeven SL."
                )
                return
            ...
            sresp = self.pybit.place_order(
                ...
                qty=self.pybit._q(remaining_qty),
                reduceOnly=True,  # Use remaining quantity
                orderLinkId=new_sl_link_id,
                triggerPrice=self.pybit._q(new_sl_price),
                triggerDirection=(2 if side == "BUY" else 1),
                orderFilter="Stop",
                slTriggerBy=self.cfg["execution"]["breakeven_after_tp1"][
                    "sl_trigger_by"
                ],
            )
            if self.pybit._ok(sresp):
                ...
                self.pybit.logger.info(
                    f"Moved SL to breakeven at {new_sl_price} (Order ID: {open_pos['stop_loss_order_id']})."
                )
            else:
                self.pybit.logger.error(
                    f"Failed to place breakeven SL: {sresp.get('retMsg')}"
                )

        except Exception as e:
            self.pybit.logger.error(
                f"Breakeven move exception for {self.symbol}: {e}\n{traceback.format_exc()}"
            )
    ...
    def poll(self):
        ...
        if not (
            self.pybit
            and self.pybit.enabled
            and self.cfg["execution"]["live_sync"]["enabled"]
        ):
            return
        ...
        try:
            ...
            rows.sort(
                key=lambda r: int(r.get("execTime", 0))
            ...
            for r in rows:
                ...
                if exec_time_ms < self.last_exec_time_ms or (
                    self.cfg["execution"]["live_sync"]["only_track_linked"]
                    and not self._is_ours(link)
                ):
                    continue
                ...
                open_pos = (
                    self.pm._get_position_by_link_prefix(pos_prefix)
                    if pos_prefix
                    else None
                )
                ...
                if tag in ("TP", "SL") and open_pos and open_pos["status"] == "OPEN":
                    ...
                    if open_pos["qty"] <= 0:  # Position fully closed
                        ...
                        self.pybit.logger.info(
                            f"Position for {self.symbol} fully closed by {tag}."
                        )
                    else:
                        self.pybit.logger.info(
                            f"Partial execution for {self.symbol} by {tag}. Remaining Qty: {open_pos['qty'].normalize()}"
                        )
                    ...
                    if (
                        ...
                        and not open_pos.get("breakeven_set", False)
                    ):
                        # Retrieve last known ATR for breakeven calculation
                        atr_val = Decimal(
                            str(self.cfg.get("_last_atr_value", "0.1"))
                        )  # Default ATR if unavailable
            ...
            self.pybit.logger.error(
                f"Execution sync error for {self.symbol}: {e}\n{traceback.format_exc()}"
            )
...
class PositionHeartbeat:
    ...
    def tick(self):
        ...
        try:
            resp = self.pybit.get_positions(
                self.symbol
            )  # Fetch positions from exchange
            if not self.pybit._ok(resp):
                self.pybit.logger.warning(
                    f"Heartbeat: Failed to fetch positions for {self.symbol}. Response: {resp}"
                )
                return
                ...
                for p in exchange_positions:
                    if p.get("symbol") == self.symbol and float(p.get("size", 0)) > 0:
                        ...
                        if p.get("side") == "Buy":
                            ...
                            avg_price_exchange = Decimal(
                                p.get("avgPrice", "0")
                            )  # Use avgPrice from exchange
            ...
            if net_qty_exchange == 0 and has_local_pos:
                for (
                    pos
                ) in local_open_positions:  # Mark all local open positions as closed
                    pos.update({"status": "CLOSED", "closed_by": "HEARTBEAT_SYNC"})
                self.pybit.logger.info(
                    f"Heartbeat: Closed local position(s) for {self.symbol} (exchange is flat)."
                )
            ...
            # Scenario 2: Exchange has a position, but no local position -> Create synthetic local position
                ...
                synthetic_pos = {
                    ...
                    "entry_price": round_price(
                        avg_price_exchange, self.pm.price_precision
                    ),
                    "qty": round_qty(abs(net_qty_exchange), self.pm.qty_step),
                    "stop_loss": Decimal("0"),
                    "take_profit": Decimal("0"),  # SL/TP need manual sync or logic
                    "status": "OPEN",
                ...
                self.pybit.logger.warning(
                    f"Heartbeat: Created synthetic local position for {self.symbol} to match exchange state."
                )

            # Scenario 3: Both have positions; compare and log mismatches
            elif net_qty_exchange != 0 and has_local_pos:
                local_pos = local_open_positions[
                    0
                ]  # Assume single position for simplicity
                # Compare quantities and log if they differ
                if abs(net_qty_exchange) != local_pos["qty"]:
                    self.pybit.logger.warning(
                        f"Heartbeat: Quantity mismatch for {self.symbol}. Local: {local_pos['qty'].normalize()}, Exchange: {abs(net_qty_exchange).normalize()}."
                    )
                # Add more checks here if needed (e.g., avgPrice, SL/TP)

        except (pybit.exceptions.FailedRequestError, Exception) as e:
            self.pybit.logger.error(
                f"Heartbeat error for {self.symbol}: {e}\n{traceback.format_exc()}"
            )
...
class TradingAnalyzer:
    ...
    def __init__(
        ...
        if self.df.empty:
            self.logger.warning(
                f"[{self.symbol}] Initialized with empty DataFrame. Indicators cannot be calculated."
            )
            return
    ...
    def _safe_calculate(
        ...
        try:
            result = func(
                self.df, *args, **kwargs
            )  # Execute the indicator calculation function
            ...
            is_empty_or_nan = (
                ...
                or (isinstance(result, pd.Series) and result.isnull().all())
            )
            if is_empty_or_nan:
                self.logger.warning(
                    f"[{self.symbol}] Indicator '{name}' returned empty or all NaNs. Check data quality or parameters."
                )
            return result
        except Exception as e:
            self.logger.error(
                f"Error calculating indicator '{name}': {e}\n{traceback.format_exc()}"
            )
            return None
    ...
    def _calculate_all_indicators(self):
        ...
        if self.config["indicators"].get("atr", False) or any(
            k in isd and "atr" in k for k in ["atr_period", "keltner_atr_multiplier"]
        ):
            ...
            if (
                "ATR" in self.df.columns
                and self.df["ATR"].iloc[-1] is not None
                and not pd.isna(self.df["ATR"].iloc[-1])
            ):
        ...
        if cfg["indicators"].get("ema_alignment", False):
            ...
            if "EMA_Short" in self.df.columns and not pd.isna(
                self.df["EMA_Short"].iloc[-1]
            ):
                self.indicator_values["EMA_Short"] = Decimal(
                    str(self.df["EMA_Short"].iloc[-1])
                )
            if "EMA_Long" in self.df.columns and not pd.isna(
                self.df["EMA_Long"].iloc[-1]
            ):
        ...
        if cfg["indicators"].get("sma_trend_filter", False):
            self.df["SMA_Short"] = self._safe_calculate(
            ...
            if "SMA_Short" in self.df.columns and not pd.isna(
                self.df["SMA_Short"].iloc[-1]
            ):
                self.indicator_values["SMA_Short"] = Decimal(
                    str(self.df["SMA_Short"].iloc[-1])
                )
            if "SMA_Long" in self.df.columns and not pd.isna(
                self.df["SMA_Long"].iloc[-1]
            ):
        ...
        if cfg["indicators"].get("rsi", False):
            ...
            if "RSI" in self.df.columns and not pd.isna(self.df["RSI"].iloc[-1]):
                self.indicator_values["RSI"] = Decimal(str(self.df["RSI"].iloc[-1]))

        if cfg["indicators"].get("stoch_rsi", False):
            stoch_rsi_res = self._safe_calculate(
                indicators.calculate_stoch_rsi,
                "StochRSI",
                isd["stoch_rsi_period"],
            ...
            if stoch_rsi_res is not None and not stoch_rsi_res.empty:
                ...
                if not pd.isna(self.df["StochRSI_K"].iloc[-1]):
                    self.indicator_values["StochRSI_K"] = Decimal(
                        str(self.df["StochRSI_K"].iloc[-1])
                    )
                if not pd.isna(self.df["StochRSI_D"].iloc[-1]):
                    self.indicator_values["StochRSI_D"] = Decimal(
                        str(self.df["StochRSI_D"].iloc[-1])
                    )

        if cfg["indicators"].get("bollinger_bands", False):
            bb_res = self._safe_calculate(
            ...
            if bb_res is not None and not bb_res.empty:
                ...
                if not pd.isna(self.df["BB_Upper"].iloc[-1]):
                    self.indicator_values["BB_Upper"] = Decimal(
                        str(self.df["BB_Upper"].iloc[-1])
                    )
                if not pd.isna(self.df["BB_Middle"].iloc[-1]):
                    self.indicator_values["BB_Middle"] = Decimal(
                        str(self.df["BB_Middle"].iloc[-1])
                    )
                if not pd.isna(self.df["BB_Lower"].iloc[-1]):
                    self.indicator_values["BB_Lower"] = Decimal(
                        str(self.df["BB_Lower"].iloc[-1])
                    )
        ...
        if cfg["indicators"].get("cci", False):
            ...
            if "CCI" in self.df.columns and not pd.isna(self.df["CCI"].iloc[-1]):
                self.indicator_values["CCI"] = Decimal(str(self.df["CCI"].iloc[-1]))
        ...
        if cfg["indicators"].get("wr", False):
            ...
            if "WR" in self.df.columns and not pd.isna(self.df["WR"].iloc[-1]):
                self.indicator_values["WR"] = Decimal(str(self.df["WR"].iloc[-1]))
        ...
        if cfg["indicators"].get("mfi", False):
            ...
            if "MFI" in self.df.columns and not pd.isna(self.df["MFI"].iloc[-1]):
                self.indicator_values["MFI"] = Decimal(str(self.df["MFI"].iloc[-1]))

        if cfg["indicators"].get("psar", False):
            psar_res = self._safe_calculate(
                indicators.calculate_psar,
                "PSAR",
                min_data_points=2,
                acceleration=isd["psar_acceleration"],
                maximum=isd["psar_max_acceleration"],
            )
            if psar_res is not None and not psar_res.empty:
                self.df["PSAR_Val"] = psar_res["PSAR"]
                self.df["PSAR_Dir"] = psar_res["Direction"]
                if not pd.isna(self.df["PSAR_Val"].iloc[-1]):
                    self.indicator_values["PSAR_Val"] = Decimal(
                        str(self.df["PSAR_Val"].iloc[-1])
                    )
                if not pd.isna(self.df["PSAR_Dir"].iloc[-1]):
                    self.indicator_values["PSAR_Dir"] = self.df["PSAR_Dir"].iloc[-1]

        if cfg["indicators"].get("vwap", False):
            self.df["VWAP"] = self._safe_calculate(
                indicators.calculate_vwap, "VWAP", min_data_points=1
            )
            if "VWAP" in self.df.columns and not pd.isna(self.df["VWAP"].iloc[-1]):
                self.indicator_values["VWAP"] = Decimal(str(self.df["VWAP"].iloc[-1]))

        if cfg["indicators"].get("ehlers_supertrend", False):
            est_res = self._safe_calculate(
                indicators.calculate_ehlers_supertrend,
                "EhlersSuperTrend",
                isd["ehlers_fast_period"],
                fast_period=isd["ehlers_fast_period"],
                fast_multiplier=isd["ehlers_fast_multiplier"],
                slow_period=isd["ehlers_slow_period"],
                slow_multiplier=isd["ehlers_slow_multiplier"],
            )
            if est_res is not None and not est_res.empty:
                ...
                if not pd.isna(self.df["ST_Slow_Dir"].iloc[-1]):
                    self.indicator_values["ST_Slow_Dir"] = self.df["ST_Slow_Dir"].iloc[
                        -1
                    ]
                if not pd.isna(self.df["ST_Slow_Val"].iloc[-1]):
                    self.indicator_values["ST_Slow_Val"] = Decimal(
                        str(self.df["ST_Slow_Val"].iloc[-1])
                    )

        if cfg["indicators"].get("macd", False):
            macd_res = self._safe_calculate(
                indicators.calculate_macd,
                "MACD",
                isd["macd_slow_period"],
            ...
            if macd_res is not None and not macd_res.empty:
                ...
                if not pd.isna(self.df["MACD_Line"].iloc[-1]):
                    self.indicator_values["MACD_Line"] = Decimal(
                        str(self.df["MACD_Line"].iloc[-1])
                    )
                if not pd.isna(self.df["MACD_Signal"].iloc[-1]):
                    self.indicator_values["MACD_Signal"] = Decimal(
                        str(self.df["MACD_Signal"].iloc[-1])
                    )
                if not pd.isna(self.df["MACD_Hist"].iloc[-1]):
                    self.indicator_values["MACD_Hist"] = Decimal(
                        str(self.df["MACD_Hist"].iloc[-1])
                    )

        if cfg["indicators"].get("adx", False):
            adx_res = self._safe_calculate(
            ...
            if adx_res is not None and not adx_res.empty:
                ...
                if not pd.isna(self.df["ADX"].iloc[-1]):
                    self.indicator_values["ADX"] = Decimal(str(self.df["ADX"].iloc[-1]))
                if not pd.isna(self.df["PlusDI"].iloc[-1]):
                    self.indicator_values["PlusDI"] = Decimal(
                        str(self.df["PlusDI"].iloc[-1])
                    )
                if not pd.isna(self.df["MinusDI"].iloc[-1]):
                    self.indicator_values["MinusDI"] = Decimal(
                        str(self.df["MinusDI"].iloc[-1])
                    )

        if cfg["indicators"].get("ichimoku_cloud", False):
            ichimoku_res = self._safe_calculate(
            ...
            if ichimoku_res is not None and not ichimoku_res.empty:
                ...
                if not pd.isna(self.df["Tenkan_Sen"].iloc[-1]):
                    self.indicator_values["Tenkan_Sen"] = Decimal(
                        str(self.df["Tenkan_Sen"].iloc[-1])
                    )
                if not pd.isna(self.df["Kijun_Sen"].iloc[-1]):
                    self.indicator_values["Kijun_Sen"] = Decimal(
                        str(self.df["Kijun_Sen"].iloc[-1])
                    )
                if not pd.isna(self.df["Senkou_Span_A"].iloc[-1]):
                    self.indicator_values["Senkou_Span_A"] = Decimal(
                        str(self.df["Senkou_Span_A"].iloc[-1])
                    )
                if not pd.isna(self.df["Senkou_Span_B"].iloc[-1]):
                    self.indicator_values["Senkou_Span_B"] = Decimal(
                        str(self.df["Senkou_Span_B"].iloc[-1])
                    )
                if not pd.isna(self.df["Chikou_Span"].iloc[-1]):
                    self.indicator_values["Chikou_Span"] = Decimal(
                        str(self.df["Chikou_Span"].iloc[-1])
                    )

        if cfg["indicators"].get("obv", False):
            self.df["OBV"] = self._safe_calculate(indicators.calculate_obv, "OBV")
            self.df["OBV_EMA"] = self._safe_calculate(
                indicators.calculate_ema,
                "OBV_EMA",
                isd["obv_ema_period"],
                series=self.df["OBV"],
                period=isd["obv_ema_period"],
            )
            if "OBV" in self.df.columns and not pd.isna(self.df["OBV"].iloc[-1]):
                self.indicator_values["OBV"] = Decimal(str(self.df["OBV"].iloc[-1]))
            if "OBV_EMA" in self.df.columns and not pd.isna(
                self.df["OBV_EMA"].iloc[-1]
            ):
                self.indicator_values["OBV_EMA"] = Decimal(
                    str(self.df["OBV_EMA"].iloc[-1])
                )

        if cfg["indicators"].get("cmf", False):
            self.df["CMF"] = self._safe_calculate(
                indicators.calculate_cmf,
                "CMF",
                isd["cmf_period"],
                period=isd["cmf_period"],
            )
            if "CMF" in self.df.columns and not pd.isna(self.df["CMF"].iloc[-1]):
                self.indicator_values["CMF"] = Decimal(str(self.df["CMF"].iloc[-1]))
        ...
        if cfg["indicators"].get("volatility_index", False):
            ...
            if "Volatility_Index" in self.df.columns and not pd.isna(
                self.df["Volatility_Index"].iloc[-1]
            ):
                self.indicator_values["Volatility_Index"] = Decimal(
                    str(self.df["Volatility_Index"].iloc[-1])
                )
        ...
        if cfg["indicators"].get("vwma", False):
            ...
            if "VWMA" in self.df.columns and not pd.isna(self.df["VWMA"].iloc[-1]):
        ...
        if cfg["indicators"].get("volume_delta", False):
            ...
            if "Volume_Delta" in self.df.columns and not pd.isna(
                self.df["Volume_Delta"].iloc[-1]
            ):
                self.indicator_values["Volume_Delta"] = Decimal(
                    str(self.df["Volume_Delta"].iloc[-1])
                )

        if cfg["indicators"].get("kaufman_ama", False):
            self.df["Kaufman_AMA"] = self._safe_calculate(
                indicators.calculate_kama,
                "Kaufman_AMA",
                isd["kama_period"],
            ...
            if "Kaufman_AMA" in self.df.columns and not pd.isna(
                self.df["Kaufman_AMA"].iloc[-1]
            ):
                self.indicator_values["Kaufman_AMA"] = Decimal(
                    str(self.df["Kaufman_AMA"].iloc[-1])
                )

        if cfg["indicators"].get("relative_volume", False):
            self.df["Relative_Volume"] = self._safe_calculate(
                indicators.calculate_relative_volume,
                "Relative_Volume",
                isd["relative_volume_period"],
                period=isd["relative_volume_period"],
            )
            if "Relative_Volume" in self.df.columns and not pd.isna(
                self.df["Relative_Volume"].iloc[-1]
            ):
                self.indicator_values["Relative_Volume"] = Decimal(
                    str(self.df["Relative_Volume"].iloc[-1])
                )

        if cfg["indicators"].get("market_structure", False):
            ms_res = self._safe_calculate(
                indicators.analyze_market_structure,
                "Market_Structure",
                isd["market_structure_lookback_period"],
                lookback_period=isd["market_structure_lookback_period"],
            )
            if ms_res is not None:
                self.indicator_values["Market_Structure_Trend"] = ms_res[
                    "Market_Structure_Trend"
                ]

        if cfg["indicators"].get("dema", False):
            self.df["DEMA"] = self._safe_calculate(
                indicators.calculate_dema,
                "DEMA",
                isd["dema_period"],
                period=isd["dema_period"],
            )
            if "DEMA" in self.df.columns and not pd.isna(self.df["DEMA"].iloc[-1]):
                self.indicator_values["DEMA"] = Decimal(str(self.df["DEMA"].iloc[-1]))

        if cfg["indicators"].get("keltner_channels", False):
            kc_res = self._safe_calculate(
                indicators.calculate_keltner_channels,
                "Keltner_Channels",
                isd["keltner_period"],
                period=isd["keltner_period"],
                atr_multiplier=isd["keltner_atr_multiplier"],
                atr_period=isd["atr_period"],
            )
            if kc_res is not None and not kc_res.empty:
                ...
                if not pd.isna(self.df["Keltner_Upper"].iloc[-1]):
                    self.indicator_values["Keltner_Upper"] = Decimal(
                        str(self.df["Keltner_Upper"].iloc[-1])
                    )
                if not pd.isna(self.df["Keltner_Middle"].iloc[-1]):
                    self.indicator_values["Keltner_Middle"] = Decimal(
                        str(self.df["Keltner_Middle"].iloc[-1])
                    )
                if not pd.isna(self.df["Keltner_Lower"].iloc[-1]):
                    self.indicator_values["Keltner_Lower"] = Decimal(
                        str(self.df["Keltner_Lower"].iloc[-1])
                    )

        if cfg["indicators"].get("roc", False):
            self.df["ROC"] = self._safe_calculate(
                indicators.calculate_roc,
                "ROC",
                isd["roc_period"],
                period=isd["roc_period"],
            )
            if "ROC" in self.df.columns and not pd.isna(self.df["ROC"].iloc[-1]):
                self.indicator_values["ROC"] = Decimal(str(self.df["ROC"].iloc[-1]))

        if cfg["indicators"].get("candlestick_patterns", False):
            self.df["Candlestick_Pattern"] = self._safe_calculate(
                indicators.detect_candlestick_patterns,
                "Candlestick_Pattern",
                MIN_CANDLESTICK_PATTERNS_BARS,
            )
            if (
                "Candlestick_Pattern" in self.df.columns
                and self.df["Candlestick_Pattern"].iloc[-1] is not None
            ):
                self.indicator_values["Candlestick_Pattern"] = self.df[
                    "Candlestick_Pattern"
                ].iloc[-1]
        ...
        self.df.fillna(
            0, inplace=True
        )  # Fill remaining NaNs, potentially after calculations
        ...
        if self.df.empty:
            self.logger.warning(
                "DataFrame became empty after calculations and cleanup. Cannot proceed."
            )
        else:
            self.logger.debug(
                f"Indicator calculations complete. Final DataFrame shape: {self.df.shape}"
            )
    ...
    def calculate_fibonacci_levels(self) -> None:
        ...
        if fib_levels:
            ...
            self.logger.warning("Fibonacci retracement levels could not be calculated.")
        ...
        if pivot_data:
            ...
            for key, value in pivot_data.items():
                try:
                    ...
                    self.logger.warning(
                        f"Could not convert pivot point value '{value}' to Decimal. Skipping."
                    )
    ...
    def _score_adx(self, trend_strength_multiplier_in: float) -> tuple[float, dict]:
        ...
        trend_strength_multiplier = (
            trend_strength_multiplier_in  # Initialize with input
        )
        ...
        return score, {
            "breakdown": breakdown,
            "trend_strength_multiplier": trend_strength_multiplier,
        }

    def _score_ema_alignment(
        self, current_close: Decimal, trend_multiplier: float
    ) -> tuple[float, dict]:
    ...
    def _score_momentum_indicators(self) -> tuple[float, dict]:
        ...
        if not pd.isna(stoch_k) and not pd.isna(stoch_d):
            if (
                stoch_k < self.indicator_settings["stoch_rsi_oversold"]
                and stoch_k > stoch_d
            ):  # Bullish cross from oversold
                total_momentum_score += 0.25 * momentum_weight
                breakdown["StochRSI Bull Cross"] = 0.25 * momentum_weight
            elif (
                stoch_k > self.indicator_settings["stoch_rsi_overbought"]
                and stoch_k < stoch_d
        ...
        if not pd.isna(wr):
            if (
                wr > self.indicator_settings["williams_r_oversold"]
            ):  # Note: WR is inverted, -80 is oversold
                total_momentum_score += 0.15 * momentum_weight
                breakdown["WR Oversold (Bull)"] = 0.15 * momentum_weight
            elif (
                wr < self.indicator_settings["williams_r_overbought"]
            ):  # -20 is overbought
                total_momentum_score -= 0.15 * momentum_weight
    ...
    def _score_bollinger_bands(self, current_close: Decimal) -> tuple[float, dict]:
        ...
        if not pd.isna(bb_upper) and not pd.isna(bb_middle) and not pd.isna(bb_lower):
            ...
            if (
            ...
            ):  # Price above upper band (potential bearish reversion or strong breakout)
            ...
            ):  # Price below lower band (potential bullish reversion or strong breakout)
                ...
                current_close > bb_middle
            ):  # Between middle and upper band (bullish tendency)
                score += bb_weight * 0.1
                breakdown["BB Mid-Upper (Bull)"] = bb_weight * 0.1
            elif (
                current_close < bb_middle
            ):  # Between middle and lower band (bearish tendency)
                score -= bb_weight * 0.1
    ...
    def _score_vwap(
        self, current_close: Decimal, prev_close: Decimal
    ) -> tuple[float, dict]:
    ...
    def _score_psar(
        ...
        psar_dir = self._get_indicator_value(
            "PSAR_Dir"
    ...
    def _score_fibonacci_levels(
        ...
        if self.fib_levels:
            ...
            for level_name, level_price in self.fib_levels.items():
                ...
                tolerance = (
                    self.df["ATR"].iloc[-1] * Decimal("0.1")
                    if "ATR" in self.df.columns and not pd.isna(self.df["ATR"].iloc[-1])
                    else current_close * Decimal("0.001")
                )

                if (
                    level_name in ["Support 1", "Support 2", "Support 3"]
                    and current_close >= level_price - tolerance
                    and current_close <= level_price + tolerance
                ):
                    # Price near support, check for bounce (previous candle below, current above or strongly bouncing)
                    if (
                        prev_close < level_price - tolerance
                        and current_close > level_price
                    ):
                        score += fib_weight * 0.5
                        breakdown[f"Fibonacci {level_name} Bounce (Bull)"] = (
                            fib_weight * 0.5
                        )

                if (
                    level_name in ["Resistance 1", "Resistance 2", "Resistance 3"]
                    and current_close >= level_price - tolerance
                    and current_close <= level_price + tolerance
                ):
                    ...
                    if (
                        ...
                        score -= fib_weight * 0.5
                        breakdown[f"Fibonacci {level_name} Rejection (Bear)"] = (
                            -fib_weight * 0.5
                        )
    ...
    def _score_fibonacci_pivot_points(
        ...
        if not pd.isna(pivot) and not pd.isna(r1) and not pd.isna(s1):
            tolerance = current_close * Decimal(
                "0.0005"
            )  # Small price-relative tolerance
            ...
            if (
                current_close < r1
                and prev_close > r1
                and abs(current_close - r1) < tolerance
            ):  # Rejection from R1
            ...
            if (
                current_close > s1
                and prev_close < s1
                and abs(current_close - s1) < tolerance
            ):  # Rejection from S1
                score += pivot_weight * 0.2
    ...
    def _score_ehlers_supertrend(self, trend_multiplier: float) -> tuple[float, dict]:
        ...
        if not pd.isna(st_slow_dir):
            ...
            prev_st_slow_dir = (
                self.df["ST_Slow_Dir"].iloc[-2] if len(self.df) > 1 else np.nan
            )
    ...
    def _score_macd(self, trend_multiplier: float) -> tuple[float, dict]:
        ...
        if (
            ...
            if len(self.df) > 1:
                ...
                if (
                    macd_line > macd_signal and prev_macd_line <= prev_macd_signal
                ):  # Bullish cross
    ...
    def _score_ichimoku_cloud(self, current_close: Decimal) -> tuple[float, dict]:
        ...
        if all(not pd.isna(x) for x in [tenkan, kijun, senkou_a, senkou_b, chikou]):
            ...
            if (
                chikou > self.df["close"].iloc[-isd["ichimoku_chikou_span_offset"]]
    ...
    def _score_volatility_index(
        self, signal_score_current: float
    ) -> tuple[float, dict]:
        ...
        if not pd.isna(volatility_index):
            ...
            if volatility_index > 1.5:  # Very high volatility
                ...
                score -= (
                    vol_weight * 0.5 * np.sign(signal_score_current)
                )  # Reduce signal confidence
                breakdown["Volatility Very High"] = (
                    -vol_weight * 0.5 * np.sign(signal_score_current)
                )
                ...
                breakdown["Volatility Very Low"] = (
                    -vol_weight * 0.2 * np.sign(signal_score_current)
                )
            else:  # Moderate volatility - maybe amplify
                score += vol_weight * 0.1 * np.sign(signal_score_current)
                breakdown["Volatility Moderate"] = (
                    vol_weight * 0.1 * np.sign(signal_score_current)
                )
        return score, {"breakdown": breakdown}
    ...
    def _score_relative_volume(
        ...
        if not pd.isna(relative_volume):
            ...
            threshold = Decimal(
                str(self.indicator_settings["relative_volume_threshold"])
            )
            if (
                relative_volume > threshold
            ):  # High relative volume confirms current price movement
    ...
    def _score_keltner_channels(
        ...
        if not pd.isna(kc_upper) and not pd.isna(kc_middle) and not pd.isna(kc_lower):
            ...
            if current_close > kc_upper and prev_close <= kc_upper:  # Bullish breakout
                ...
                current_close < kc_lower and prev_close >= kc_lower
            ):  # Bearish breakout
                score -= kc_weight
    ...
    def _score_orderbook_imbalance(
        ...
        if (
            self.config["indicators"].get("orderbook_imbalance", False)
            and orderbook_data
        ):
            try:
                ...
                if total_volume > 0:
                    imbalance = _safe_divide_decimal(
                        bid_volume - ask_volume, total_volume
                    )
                    imbalance_contrib = imbalance * self.weights.get(
                ...
                self.logger.error(
                    f"Error processing orderbook data for scoring: {e}\n{traceback.format_exc()}"
                )
    ...
    def _score_mtf_confluence(self, mtf_trends: dict[str, str]) -> tuple[float, dict]:
        ...
        if self.config["mtf_analysis"]["enabled"] and mtf_trends:
            ...
            if total_mtf_indicators > 0:
                ...
                self.logger.debug(
                    f"MTF Confluence: Buy={mtf_buy_count}, Sell={mtf_sell_count}, Total={total_mtf_indicators}. Score contribution: {mtf_contribution:.2f}"
                )
    ...
    def _dynamic_threshold(self, base_threshold: float) -> float:
        ...
        if (
            "ATR" not in self.df.columns
            or self.df["ATR"].isnull().all()
            or len(self.df["ATR"]) < 50
        ):
            return base_threshold

        atr_ma_series = self.df["ATR"].rolling(50).mean()
        if atr_ma_series.empty or atr_ma_series.iloc[-1] <= 0:
            return base_threshold
    ...
    def _market_regime(self) -> str:
        ...
        adx, bb_u, bb_m, bb_l = (
            self._get_indicator_value("ADX"),
            self._get_indicator_value("BB_Upper"),
            self._get_indicator_value("BB_Middle"),
            self._get_indicator_value("BB_Lower"),
        )
        ...
        band_width_pct = (
            float((bb_u - bb_l) / bb_m) if bb_m != Decimal("0") else 0
        )  # Calculate Bollinger Band width percentage

        # Determine regime based on ADX and Bandwidth
        if float(adx) >= ADX_STRONG_TREND_THRESHOLD or band_width_pct >= 0.03:
            return "TRENDING"
        if float(adx) <= ADX_WEAK_TREND_THRESHOLD and band_width_pct <= 0.01:
            return "RANGING"
        return "SIDEWAYS"
    ...
    def generate_trading_signal(
        ...
        if self.df.empty:
            return "HOLD", 0.0, {}  # Cannot generate signal if DataFrame is empty
        ...
        trend_strength_multiplier = adx_info[
            "trend_strength_multiplier"
        ]  # Update multiplier based on ADX
        ...
        imbalance_score, imbalance_breakdown = self._score_orderbook_imbalance(
            orderbook_data
        )
        ...
        should_override_hold = (
            True  # Assume we want to always consider the signal if strong enough
        )
        ...
        if (
            self.config["_last_score"] > 0
            and is_strong_sell
            and signal_score > (-dynamic_threshold * hysteresis_ratio)
        ) or (
            self.config["_last_score"] < 0
            and is_strong_buy
            and signal_score < (dynamic_threshold * hysteresis_ratio)
            ...
            self.logger.debug(
                f"Hysteresis check: Prev score {self.config['_last_score']:.2f}, Current score {signal_score:.2f}, Threshold {dynamic_threshold:.2f}, Hysteresis ratio {hysteresis_ratio}. Signal might be filtered."
            )
            should_override_hold = (
                False  # Do not issue a signal if hysteresis prevents it
            )

        if is_strong_buy and should_override_hold:
            final_signal = "BUY"
        elif is_strong_sell and should_override_hold:
            final_signal = "SELL"
        ...
        if final_signal != "HOLD":
            if now_ts - self._last_signal_ts < cooldown_sec:
                self.logger.info(
                    f"Signal '{final_signal}' ignored due to cooldown ({cooldown_sec - (now_ts - self._last_signal_ts)}s remaining)."
                )
                final_signal = "HOLD"
            else:
                self._last_signal_ts = (
                    now_ts  # Update signal timestamp only if signal is issued
                )
        ...
        self.logger.info(
            f"Generated Signal: {final_signal} (Score: {signal_score:.2f}, Threshold: {dynamic_threshold:.2f})"
        )
    ...
    for start_str, end_str in config["session_filter"]["utc_allowed"]:
        ...
        if start_time <= end_time:
            if start_time <= now_utc <= end_time:
                return True
        elif now_utc >= start_time or now_utc <= end_time:
            return True
...
def get_spread_bps(orderbook_data: dict) -> float:
    ...
    if best_bid.is_zero() or best_ask.is_zero():
        return float("inf")
...
def expected_value(performance_tracker: PerformanceTracker) -> Decimal:
    ...
    if performance_tracker.wins + performance_tracker.losses == 0:
        return Decimal("0")

    win_rate = Decimal(performance_tracker.wins) / (
        performance_tracker.wins + performance_tracker.losses
    )
    loss_rate = Decimal(performance_tracker.losses) / (
        performance_tracker.wins + performance_tracker.losses
    )

    avg_win = (
        performance_tracker.gross_profit / performance_tracker.wins
        if performance_tracker.wins > 0
        else Decimal("0")
    )
    avg_loss = (
        performance_tracker.gross_loss / performance_tracker.losses
        if performance_tracker.losses > 0
        else Decimal("0")
    )
...
def adapt_exit_params(
    performance_tracker: PerformanceTracker, config: dict
    ...
    if (
        win_rate > 0.6
    ):  # Good win rate, try to capture more profit and cut losses faster
        return base_tp * Decimal("1.1"), base_sl * Decimal("0.9")
    if (
        win_rate < 0.4
    ):  # Poor win rate, maybe adjust to avoid being stopped out too early
...
def display_indicator_values_and_price(
    ...
    if config["indicators"].get("fibonacci_pivot_points", False) and any(
        k in analyzer.indicator_values for k in ["PIVOT", "R1", "S1"]
    ):
        logger.info(f"{NEON_CYAN}--- Fibonacci Pivot Points ---{RESET}")
        for key in ["PIVOT", "R1", "R2", "S1", "S2"]:
            if key in analyzer.indicator_values:
                logger.info(
                    f"  {INDICATOR_COLORS.get(key, NEON_YELLOW)}{key:<20}: {analyzer.indicator_values[key].normalize()}{RESET}"
                )
    ...
    if (
        ...
        if "Support_Level" in analyzer.indicator_values:
            logger.info(
                f"  {INDICATOR_COLORS.get('Support_Level', NEON_YELLOW)}{'Support Level':<20}: {analyzer.indicator_values['Support_Level'].normalize()}{RESET}"
            )
        if "Resistance_Level" in analyzer.indicator_values:
            logger.info(
                f"  {INDICATOR_COLORS.get('Resistance_Level', NEON_YELLOW)}{'Resistance Level':<20}: {analyzer.indicator_values['Resistance_Level'].normalize()}{RESET}"
            )
    ...
    trend_summary_lines = []
    ema_short, ema_long = (
        analyzer._get_indicator_value("EMA_Short"),
        analyzer._get_indicator_value("EMA_Long"),
    )
    if not pd.isna(ema_short) and not pd.isna(ema_long):
        trend_summary_lines.append(
            f"{Fore.GREEN if ema_short > ema_long else (Fore.RED if ema_short < ema_long else Fore.YELLOW)}EMA Cross  : {'▲ Up' if ema_short > ema_long else ('▼ Down' if ema_short < ema_long else '↔ Sideways')}{RESET}"
        )

    st_slow_dir = analyzer._get_indicator_value("ST_Slow_Dir")
    if not pd.isna(st_slow_dir):
        trend_summary_lines.append(
            f"{Fore.GREEN if st_slow_dir == 1 else (Fore.RED if st_slow_dir == -1 else Fore.YELLOW)}SuperTrend : {'▲ Up' if st_slow_dir == 1 else ('▼ Down' if st_slow_dir == -1 else '↔ Sideways')}{RESET}"
        )

    macd_hist = analyzer._get_indicator_value("MACD_Hist")
    if (
        not pd.isna(macd_hist)
        and "MACD_Hist" in analyzer.df.columns
        and len(analyzer.df) > 1
    ):
    ...
    if not pd.isna(adx_val):
        ...
        if adx_val >= ADX_STRONG_TREND_THRESHOLD:
            plus_di, minus_di = (
                analyzer._get_indicator_value("PlusDI"),
                analyzer._get_indicator_value("MinusDI"),
            )
            if not pd.isna(plus_di) and not pd.isna(minus_di):
                if plus_di > minus_di:
                    trend_summary_lines.append(
                        f"{Fore.LIGHTGREEN_EX}ADX Trend  : Strong Up {strength}{RESET}"
                    )
                else:
                    trend_summary_lines.append(
                        f"{Fore.LIGHTRED_EX}ADX Trend  : Strong Down {strength}{RESET}"
                    )
    ...
    senkou_span_a, senkou_span_b = (
        analyzer._get_indicator_value("Senkou_Span_A"),
        analyzer._get_indicator_value("Senkou_Span_B"),
    )
    if not pd.isna(senkou_span_a) and not pd.isna(senkou_span_b):
        kumo_upper, kumo_lower = (
...
def main():
    ...
    if not API_KEY or not API_SECRET:
        global_logger.error(
            f"{NEON_RED}BYBIT_API_KEY and BYBIT_API_SECRET must be set in .env. Exiting.{RESET}"
        )
        sys.exit(1)
    ...
    if config["interval"] not in valid_bybit_intervals:
        global_logger.error(
            f"{NEON_RED}Invalid primary interval '{config['interval']}'. Use Bybit formats. Exiting.{RESET}"
        )
        sys.exit(1)
    ...
    position_manager = PositionManager(
        config, global_logger, config["symbol"], pybit_client
    )
    ...
    if pybit_client.enabled and pybit_client.session:
        ws_state_loaded = (
            pybit_client.load_state()
        )  # Loads into WS manager and populates pybit_client.state_data
        if not ws_state_loaded:
            global_logger.warning("Failed to load WS state, continuing without it.")

        # Load performance state using pybit_client.state_data
        if not performance_tracker.load_state(pybit_client.state_data):
            global_logger.warning(
                "Failed to load performance tracker state, starting fresh."
            )
        ...
        if leverage_set:
            global_logger.info(f"{NEON_GREEN}Leverage set successfully.{RESET}")
        else:
            global_logger.error(
                f"{NEON_RED}Failed to set leverage. Check API permissions or account status.{RESET}"
            )
            alert_system.send_alert(
                f"Failed to set leverage for {config['symbol']}. Check API settings.",
                "ERROR",
            )
        ...
        if config["execution"]["live_sync"]["enabled"]:
            ...
            heartbeat = PositionHeartbeat(
                config["symbol"], pybit_client, config, position_manager
            )
        else:
            global_logger.info(
                "Live sync components (Execution Poll, Heartbeat) are disabled."
            )
    else:  # If Pybit client is not enabled or failed to initialize
        global_logger.error(
            f"{NEON_RED}Pybit client is not enabled or failed to initialize. Cannot proceed with live trading.{RESET}"
        )
    ...
    while not pybit_client.stop_event.is_set():
        try:
            ...
            if guard.get("enabled", False):
                ...
                if (max_day_loss > 0 and day_loss <= -max_day_loss) or (
                    ...
                    global_logger.critical(
                        f"{NEON_RED}KILL SWITCH ACTIVATED: Risk limits hit! Day PnL: {day_loss.normalize():.2f} / Max Allowed: {max_day_loss.normalize():.2f}. Max DD: {performance_tracker.max_drawdown.normalize():.2f} / Allowed: {max_dd.normalize():.2f}. Cooling down.{RESET}"
                    )
                    alert_system.send_alert(
                        f"KILL SWITCH: Risk limits hit for {config['symbol']}. Cooling down.",
                        "ERROR",
                    )
                    time.sleep(
                        int(guard.get("cooldown_after_kill_min", 120)) * 60
            ...
            if not in_allowed_session(config):
                global_logger.info(
                    f"{NEON_BLUE}Outside allowed trading session. Holding.{RESET}"
                )
            ...
            df = pybit_client.fetch_klines(
                config["symbol"], config["interval"], 1000
            ...
            if config["indicators"].get("orderbook_imbalance", False):
                ...
                if orderbook_data is None:
                    global_logger.warning("Failed to fetch orderbook data.")
            ...
            if guard.get("enabled", False) and orderbook_data:
                ...
                if spread_bps > float(guard.get("spread_filter_bps", 5.0)):
                    global_logger.warning(
                        f"Spread too high ({spread_bps:.1f} bps). Holding.{RESET}"
                    )
                    time.sleep(config["loop_delay"])
                    continue
            ...
            if guard.get("ev_filter_enabled", True):
                ...
                if current_ev <= 0:
                    global_logger.warning(
                        f"Negative Expected Value ({current_ev:.2f}) detected. Holding.{RESET}"
                    )
                    time.sleep(config["loop_delay"])
                    continue
            ...
            tp_mult_adapted, sl_mult_adapted = adapt_exit_params(
                performance_tracker, config
            )
            config["trade_management"]["take_profit_atr_multiple"] = float(
                tp_mult_adapted
            )
            config["trade_management"]["stop_loss_atr_multiple"] = float(
                sl_mult_adapted
            )
            ...
            if config["mtf_analysis"]["enabled"]:
                for htf_interval in config["mtf_analysis"]["higher_timeframes"]:
                    ...
                    if htf_df is not None and not htf_df.empty:
                        ...
                        global_logger.warning(
                            f"Could not fetch klines for MTF {htf_interval} or it was empty. Skipping."
                        )
                    time.sleep(
            ...
            if analyzer.df.empty:  # Check if DataFrame became empty after analysis
                alert_system.send_alert(
                    f"[{config['symbol']}] TradingAnalyzer DataFrame is empty. Cannot generate signal.",
                    "WARNING",
                )
                time.sleep(config["loop_delay"])
                continue
            ...
            config["_last_atr_value"] = str(
                atr_value
            )  # Store for potential use by other modules
            ...
            if trading_signal in ("BUY", "SELL"):
                ...
                if (
                    ...
                    conviction = 0.1  # Minimum conviction for valid signal
                ...
                if abs(signal_score) >= config["signal_score_threshold"]:
                    ...
                    global_logger.info(
                        f"Signal ({trading_signal}) below threshold ({config['signal_score_threshold']:.2f}). Holding. Score: {signal_score:.2f}"
                    )
            else:
                global_logger.info(
                    f"No strong trading signal. Holding. Score: {signal_score:.2f}"
                )

            # --- Live Synchronization & Heartbeat ---
            if exec_sync:
                exec_sync.poll()  # Process recent executions from REST API polling
            if heartbeat:
                heartbeat.tick()  # Perform heartbeat check to reconcile state with exchange

            # --- Log Status Summary ---
            open_positions_summary = position_manager.get_open_positions()
            if open_positions_summary:
                global_logger.info(
                    f"{NEON_CYAN}Open Positions: {len(open_positions_summary)}{RESET}"
                )
                for pos in open_positions_summary:  # Log details of open positions
                    global_logger.info(
                        f"  - {pos['side']} {pos['qty'].normalize()} {pos['symbol']} @ {pos['entry_price'].normalize()} (SL: {pos['stop_loss'].normalize()}, TP: {pos['take_profit'].normalize()}, Adds: {pos['adds']}){RESET}"
                    )
            else:
            ...
            global_logger.info(
                f"{NEON_YELLOW}Performance Summary: Total PnL: {perf_summary['total_pnl']}, Daily PnL: {perf_summary['daily_pnl']}, Win Rate: {perf_summary['win_rate']}, Max Drawdown: {perf_summary['max_drawdown']}{RESET}"
            )
            ...
            global_logger.exception(
                f"{NEON_RED}Unhandled exception in main loop:{RESET}"
            )
            time.sleep(
                config["loop_delay"] * 2
...
if __name__ == "__main__":
    ...
    try:
        ...
        global_logger.exception(
            f"{NEON_RED}An unhandled error occurred outside the main loop: {e}{RESET}"
        )
    finally:
        if "pybit_client" in globals() and pybit_client.enabled:
            ...
            global_logger.info(
                "Pybit client not initialized or enabled, skipping client shutdown."
            )
