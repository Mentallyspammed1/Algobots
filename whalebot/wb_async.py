...
def load_config(filepath: str, logger: logging.Logger) -> dict[str, Any]:
    default_config = {
        "symbol": "BTCUSDT", "interval": "15", "loop_delay": 15, "orderbook_limit": 50,
        ...
        "indicators": {
            ...
            "volatility_index": True, "vwma": True, "volume_delta": True, "kaufman_ama": True,
        ...
        "weight_sets": {
            "default_scalping": {
                ...
                "bollinger_bands": 0.22, "vwap": 0.22, "psar": 0.22, "sma_10": 0.07,
                "orderbook_imbalance": 0.07, "ehlers_supertrend_alignment": 0.55,
...
class PositionManager:
    ...
    def _calculate_order_size(self, current_price: Decimal, atr_value: Decimal) -> Decimal:
        if not self.trade_management_enabled or atr_value <= 0: return Decimal("0")
    ...
    def open_position(self, signal: Literal["BUY", "SELL"], current_price: Decimal, atr_value: Decimal) -> dict | None:
        if not self.trade_management_enabled or len(self.open_positions) >= self.max_open_positions: return None
        ...
        for i, position in enumerate(self.open_positions):
            if position["status"] == "OPEN":
                ...
                if position["side"] == "BUY":
                    if current_price <= position["stop_loss"]: closed_by = "STOP_LOSS"
                    elif current_price >= position["take_profit"]: closed_by = "TAKE_PROFIT"
                elif position["side"] == "SELL":
                    if current_price >= position["stop_loss"]: closed_by = "STOP_LOSS"
                    elif current_price <= position["take_profit"]: closed_by = "TAKE_PROFIT"
                if closed_by:
                    position["status"] = "CLOSED"; position["exit_time"] = datetime.now(TIMEZONE)
                    position["exit_price"] = current_price; position["closed_by"] = closed_by
                    positions_to_close.append(i)
                    pnl = (position["exit_price"] - position["entry_price"]) * position["qty"] if position["side"] == "BUY" else (position["entry_price"] - position["exit_price"]) * position["qty"]
                    performance_tracker.record_trade(position, pnl)
                    self.logger.info(f"{NEON_PURPLE}[{self.symbol}] Closed {position['side']} by {closed_by}. PnL: {pnl:.2f}{RESET}")
...
class PerformanceTracker:
    def __init__(self, logger: logging.Logger, config: dict[str, Any]):
        self.logger = logger; self.config = config; self.trades: list[dict] = []
        self.total_pnl = Decimal("0"); self.wins = 0; self.losses = 0
    def record_trade(self, position: dict, pnl: Decimal) -> None:
        self.trades.append(position); self.total_pnl += pnl
        if pnl > 0: self.wins += 1
        else: self.losses += 1
        self.logger.info(f"{NEON_CYAN}Trade recorded. Total PnL: {self.total_pnl:.2f}, Wins: {self.wins}, Losses: {self.losses}{RESET}")
    def get_summary(self) -> dict:
        total_trades = len(self.trades)
        win_rate = (self.wins / total_trades) * 100 if total_trades > 0 else 0
        return {"total_trades": total_trades, "total_pnl": self.total_pnl, "wins": self.wins, "losses": self.losses, "win_rate": f"{win_rate:.2f}%"}
...
class TradingAnalyzer:
    def __init__(self, df: pd.DataFrame, config: dict[str, Any], logger: logging.Logger, symbol: str):
        ...
        self.indicator_values: dict[str, Any] = {}; self.weights = config["weight_sets"]["default_scalping"]
        self.indicator_settings = config["indicator_settings"]
        if not self.df.empty: self._calculate_all_indicators()
    def _safe_calculate(self, func: callable, name: str, min_data_points: int = 0, *args, **kwargs) -> Any | None:
        ...
        except Exception as e: self.logger.error(f"{NEON_RED}[{self.symbol}] Error calculating '{name}': {e}{RESET}"); return None
    def _calculate_all_indicators(self) -> None:
        cfg = self.config; isd = self.indicator_settings
        if cfg["indicators"].get("sma_10", False): self.df["SMA_10"] = self._safe_calculate(lambda: self.df["close"].rolling(window=isd["sma_short_period"]).mean(), "SMA_10", min_data_points=isd["sma_short_period"])
        if cfg["indicators"].get("sma_trend_filter", False): self.df["SMA_Long"] = self._safe_calculate(lambda: self.df["close"].rolling(window=isd["sma_long_period"]).mean(), "SMA_Long", min_data_points=isd["sma_long_period"])
        self.df["TR"] = self._safe_calculate(self.calculate_true_range, "TR", min_data_points=MIN_DATA_POINTS_TR)
    ...
    def generate_trading_signal(self, current_price: Decimal, orderbook_data: dict, mtf_trends: dict) -> tuple[str, float]:
        signal_score = 0.0; active_indicators = self.config["indicators"]; weights = self.weights; isd = self.indicator_settings
        if self.df.empty: return "HOLD", 0.0
        threshold = self.config["signal_score_threshold"]
        if signal_score >= threshold: return "BUY", signal_score
        if signal_score <= -threshold: return "SELL", signal_score
...
async def bybit_request(client: httpx.AsyncClient, method: Literal["GET", "POST"], endpoint: str, params: dict | None = None, signed: bool = False, logger: logging.Logger | None = None) -> dict | None:
    if logger is None: logger = setup_logger("bybit_api_async")
    url = f"{BASE_URL}{endpoint}"
    headers = {"Content-Type": "application/json"}
    if signed:
        if not API_KEY or not API_SECRET: logger.error(f"{NEON_RED}API keys not set.{RESET}"); return None
        timestamp = str(int(time.time() * 1000)); recv_window = "20000"
        if method == "GET":
            query_string = urllib.parse.urlencode(params) if params else ""
            param_str = timestamp + API_KEY + recv_window + query_string
            headers.update({"X-BAPI-API-KEY": API_KEY, "X-BAPI-TIMESTAMP": timestamp, "X-BAPI-SIGN": generate_signature(param_str, API_SECRET), "X-BAPI-RECV-WINDOW": recv_window})
            req = client.build_request("GET", url, params=params, headers=headers, timeout=REQUEST_TIMEOUT)
        else:
            json_params = json.dumps(params) if params else ""
            param_str = timestamp + API_KEY + recv_window + json_params
            headers.update({"X-BAPI-API-KEY": API_KEY, "X-BAPI-TIMESTAMP": timestamp, "X-BAPI-SIGN": generate_signature(param_str, API_SECRET), "X-BAPI-RECV-WINDOW": recv_window})
            req = client.build_request("POST", url, json=params, headers=headers, timeout=REQUEST_TIMEOUT)
    else:
        req = client.build_request("GET", url, params=params, headers=headers, timeout=REQUEST_TIMEOUT)
    try:
        ...
        data = response.json()
        if data.get("retCode") != 0: logger.error(f"{NEON_RED}Bybit API Error: {data.get('retMsg')}{RESET}"); return None
        return data
    except (httpx.HTTPStatusError, httpx.RequestError, json.JSONDecodeError) as e: logger.error(f"{NEON_RED}Request failed: {e}{RESET}"); return None

async def fetch_klines(client: httpx.AsyncClient, symbol: str, interval: str, limit: int, logger: logging.Logger) -> pd.DataFrame | None:
    endpoint = "/v5/market/kline"; params = {"category": "linear", "symbol": symbol, "interval": interval, "limit": limit}
    ...
    if response and response.get("result", {}).get("list"):
        ...
        return df
    logger.warning(f"{NEON_YELLOW}Could not fetch initial klines for {symbol}.{RESET}"); return None

def update_kline_data(klines_df: pd.DataFrame, kline_data: dict) -> tuple[pd.DataFrame, bool]:
    new_kline_time = pd.to_datetime(int(kline_data['start']), unit='ms', utc=True)
    is_new_candle = new_kline_time not in klines_df.index
    new_data = {'open': float(kline_data['open']), 'high': float(kline_data['high']), 'low': float(kline_data['low']), 'close': float(kline_data['close']), 'volume': float(kline_data['volume']), 'turnover': float(kline_data['turnover'])}
    klines_df.loc[new_kline_time] = new_data
    if is_new_candle:
        klines_df.sort_index(inplace=True)
        if len(klines_df) > 1000: klines_df = klines_df.iloc[-1000:]
    return klines_df, is_new_candle

class TradingBot:
    def __init__(self, config: dict, logger: logging.Logger):
        self.config = config; self.logger = logger; self.symbol = config['symbol']
        self.klines_df = pd.DataFrame(); self.is_processing = False
    ...
    async def initialize(self, client: httpx.AsyncClient):
        self.klines_df = await fetch_klines(client, self.symbol, self.config['interval'], 1000, self.logger)
        return self.klines_df is not None and not self.klines_df.empty
    async def process_kline(self, kline_data: dict):
        if self.is_processing: return
        self.is_processing = True
        try:
            self.klines_df, is_new_candle = update_kline_data(self.klines_df.copy(), kline_data)
            if not is_new_candle and not kline_data['confirm']: return
            self.logger.info(f"Processing candle for {self.symbol} at {pd.to_datetime(int(kline_data['start']), unit='ms', utc=True)}")
            analyzer = TradingAnalyzer(self.klines_df, self.config, self.logger, self.symbol)
            signal, score = analyzer.generate_trading_signal(current_price=Decimal(kline_data['close']), orderbook_data={}, mtf_trends={})
            self.logger.info(f"Signal: {signal}, Score: {score:.2f}")
            current_price = Decimal(kline_data['close'])
            self.position_manager.manage_positions(current_price, self.performance_tracker)
            if signal in ["BUY", "SELL"] and score >= self.config['signal_score_threshold']:
                atr_value = analyzer._get_indicator_value("ATR", default=Decimal("0"))
                if atr_value > 0: self.position_manager.open_position(signal, current_price, atr_value)
...
async def websocket_handler(bot: TradingBot):
    logger = bot.logger; symbol = bot.symbol; interval = bot.config['interval']
    topic = f"kline.{interval}.{symbol}"
    while True:
        try:
            async with websockets.connect(WS_BASE_URL) as websocket:
                await websocket.send(json.dumps({"op": "subscribe", "req_id": f"kline_{symbol}_{interval}", "args": [topic]}))
                while True:
                    data = json.loads(await websocket.recv())
                    if "topic" in data and "kline" in data["topic"]:
                        for kline in data['data']: await bot.process_kline(kline)
                    elif "op" in data and data["op"] == "subscribe" and data.get("success"):
                        logger.info(f"{NEON_GREEN}Subscribed to {data['req_id']}{RESET}")
        except Exception as e:
            logger.error(f"{NEON_RED}WebSocket error: {e}. Reconnecting...{RESET}")
            await asyncio.sleep(10)

async def main():
    logger = setup_logger("whalebot_async"); logger.info("Starting bot...")
    config = load_config(CONFIG_FILE, logger)
    if not config: return
    bot = TradingBot(config, logger)
    async with httpx.AsyncClient() as client:
        if not await bot.initialize(client): return
