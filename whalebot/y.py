...
def calculate_atr(high: np.ndarray, low: np.ndarray, close: np.ndarray, period: int) -> float:
...
def calculate_dynamic_qty(available: Decimal, risk_pct: Decimal, atr: float, price: Decimal, leverage: int, qty_step: Decimal) -> Decimal:
...
def confirm_signal(state: SentinelState, direction: str, required_candles: int) -> bool:
    ...
    state.signal_confirmed_candles = min(state.signal_confirmed_candles, required_candles)
...
class TermuxAPI:
    ...
    def toast(msg: str, short: bool = True):
        ...
        os.system(f"termux-toast -b '#00FFFF' -c '#000000' -g top {duration} '{msg}' &")
        ...
        try:
            ...
            for key, value in data.items():
                ...
                if isinstance(default_val, Decimal):
                    ...
                    filtered_data[key] = float(value) if value is not None else default_val
                elif isinstance(default_val, int):
                    filtered_data[key] = int(value) if value is not None else default_val
                elif isinstance(default_val, bool):
                    filtered_data[key] = bool(value) if value is not None else default_val
                else: # string
...
class SentinelState:
    ...
    ohlc_5m: Deque[Tuple[float, float, float, float, float]] = field(default_factory=lambda: deque(maxlen=100))
    ...
    def log(self, msg: str, level: str = "info") -> None:
        ...
        clean_msg = f"{ts_str} [{level.upper()}] {msg.replace('[', '<').replace(']', '>')}"
        with open(LOG_FILE, "a") as f:
            f.write(clean_msg + "\n")
...
class BybitApex:
    ...
    async def __aenter__(self):
        ...
        self.session = aiohttp.ClientSession(connector=conn, timeout=aiohttp.ClientTimeout(total=15))
        return self
        ...
        try:
            ...
            for attempt in range(retries):
                ...
                try:
                    ...
                    self._report_error(f"Network Error on {method} {path}: {e}", "error", 101)
                    await asyncio.sleep(delay)
                    delay *= backoff
                except json.JSONDecodeError:
                    self._report_error(f"Invalid JSON response on {method} {path}", "error", 101)
                    await asyncio.sleep(delay)
                    delay *= backoff
                except Exception as e:
                    self._report_error(f"Unexpected Error on {method} {path}: {e}", "error", 101)
                    await asyncio.sleep(delay)
                    delay *= backoff
            
            self._report_error(f"Max retries exceeded for {method} {path}", "error", 101)
    ...
    async def boot_ritual(self):
        ...
        self.state.log(f"{Fore.CYAN}# Syncing Instrument Ritual...{Style.RESET_ALL}", "health")
        ...
        if r.get("retCode") == 0:
            ...
            if lot_filter_data:
                ...
                self._report_error("Missing lot filter info in API response.", "error", 102)
        ...
        if leverage_res.get("retCode") == 0:
            ...
            self._report_error(f"Failed to set leverage: {leverage_res.get('retMsg')}", "error", 103)
        ...
        for _ in range(3):
            ...
            if self.state.initial_equity_session > Decimal("0"): break
            await asyncio.sleep(1)


    async def fetch_history(self):
        """# Summons ancient kline scrolls to warm up the oracle's vision."""
        self.state.log(f"{Fore.CYAN}# Summoning ancient kline scrolls...{Style.RESET_ALL}", "health")
        
        # Fetch main interval history
        limit = max(self.cfg.warmup_candles + 20, self.cfg.rsi_period + self.cfg.bollinger_window + 20)
        r = await self.request("GET", "/v5/market/kline", {
            "category": self.cfg.category, "symbol": self.cfg.symbol,
            "interval": self.cfg.kline_interval, "limit": str(limit)
        })
        if r.get("retCode") == 0:
            for k in reversed(r["result"]["list"]):
                self.state.ohlc.append((float(k[1]), float(k[2]), float(k[3]), float(k[4]), float(k[5])))
            self.state.log(f"{Fore.GREEN}Kline scrolls unveiled: {len(self.state.ohlc)} candles.{Style.RESET_ALL}", "info")
            self._update_oracle()
    ...
    async def update_wallet(self):
        ...
        if r.get("retCode") == 0:
            ...
            current_reset_time = current_utc.replace(hour=reset_hour, minute=0, second=0, microsecond=0)
            if current_utc < current_reset_time: current_reset_time -= timedelta(days=1)
            reset_date_str = current_reset_time.strftime("%Y-%m-%d")

            if self.state.last_daily_reset_date != reset_date_str:
                self.state.log(f"{Fore.CYAN}# Daily PnL reset initiated for {reset_date_str} (UTC {reset_hour}:00).{Style.RESET_ALL}", "health")
            ...
            self._report_error(f"Failed to update wallet: {r.get('retMsg')}", "error", 105)
    ...
    def _update_oracle(self, live_p: float = None):
        ...
        if len(s.ohlc) < max(cfg.warmup_candles, cfg.rsi_period, cfg.bollinger_window) + 10: return

        c = np.array([x[3] for x in s.ohlc])
        if live_p: c[-1] = live_p
        ...
        s.ema_fast = safe_decimal(alpha_f * last_price + (1.0 - alpha_f) * float(s.ema_fast))
        s.ema_macro = safe_decimal(alpha_m * last_price + (1.0 - alpha_m) * float(s.ema_macro))
        ...
        if len(c) >= window:
            ...
            if sma > 0:
                s.bollinger_width_pct = max(
                    0.0, min(1000.0, (s.bollinger_upper - s.bollinger_lower) / sma * 100.0)
                )
            else:
        ...
        if len(c) > cfg.superpass_period + 2:
            ...
            s.superpass_signal = float(s.superpass_signal) if s.superpass_signal > 0 else superpass_smooth
        ...
        s.ready = len(s.ohlc) >= max(cfg.warmup_candles, cfg.rsi_period, cfg.bollinger_window) + 10
    ...
    async def _auto_adjust_leverage(self):
        ...
        s, cfg = self.state, self.cfg
        if s.atr == 0 or s.active: return
        now = time.time()
        if (now - s.last_leverage_adjust_ts) < cfg.leverage_adjust_cooldown_sec: return

        win_rate = safe_div(Decimal(s.wins), Decimal(max(1, s.trade_count)), Decimal("0.5"))
        ...
        if new_lev != cfg.leverage:
            ...
            if res.get("retCode") == 0:
                ...
                s.log(f"{Fore.GREEN}# Adjusted leverage to {new_lev}x (from {self.cfg.leverage}x) due to volatility {s.atr:.2f} and win rate {win_rate:.2f}.{Style.RESET_ALL}", "health")
            else:
                self._report_error(f"Failed to adjust leverage: {res.get('retMsg')}", "error", 103)
        ...
        for attempt in range(max_retries):
            ...
            if res.get("retCode") == 0 and res["result"]:
                ...
                if order_status == "Filled":
                    ...
                    return False
            s.log(f"Order {order_id} not yet filled (status: {res['result'].get('orderStatus') if res.get('result') else 'N/A'}). Retrying in {delay_sec}s...", "debug")
            await asyncio.sleep(delay_sec)
        
        self._report_error(f"Order {order_id} not confirmed filled after {max_retries} attempts.", "error", 106)
        return False

    async def setup_trailing_stop(self, state: SentinelState, leverage: int, atr: float, price_prec: int, cfg: ScalperConfig):
        trail_atr = Decimal(str(atr)) * cfg.trail_atr_mult
        qty = state.qty
        pos_side = state.side
        res = await self.request("POST", "/v5/position/trading-stop", {
    ...
    def bollinger_band_filter(self, state: SentinelState, cfg: ScalperConfig) -> bool:
        ...
        if cfg.bollinger_band_strategy == "filter_extremes":
            ...
            if (price >= upper * 0.995 and rsi > 70) or (price <= lower * 1.005 and rsi < 30):
                return False
        elif cfg.bollinger_band_strategy == "mean_reversion":
            # Require strong contrarian RSI and band proximity
            if not ((price <= lower * 1.01 and rsi < 30 and width > 1.0) or (price >= upper * 0.99 and rsi > 70 and width > 1.0)):
                return False
        return True

    async def strike(self, side: str, qty: Decimal, reduce: bool = False, order_type: str = "Limit", price: Optional[Decimal] = None, take_profit_price: Optional[Decimal] = None):
        """
        ...
        if quantized_qty < s.min_qty:
            s.log(f"Calculated quantity {qty} too small ({quantized_qty}). Minimum is {s.min_qty}. Withheld strike.", "warn")
        ...
        if not reduce and atr_dec > 0:
            sl_raw = exit_ref_price - atr_dec * cfg.sl_atr_mult if side == "Buy" else exit_ref_price + atr_dec * cfg.sl_atr_mult
        ...
        if take_profit_price is not None:
            ...
            tp_raw = exit_ref_price + atr_dec * cfg.tp_partial_atr_mult if side == "Buy" else exit_ref_price - atr_dec * cfg.tp_partial_atr_mult
        ...
        if not reduce:
            s.log(f"{Fore.CYAN}Initiating STRIKE {side} {order_type} with Qty: {quantized_qty} and SL: {params.get('stopLoss')}{Style.RESET_ALL}", "entry")
        else:
    ...
    async def logic_loop(self, stop: asyncio.Event):
        ...
        while not stop.is_set():
            ...
            if s.api_latency > 1000:
                s.log(f"{Fore.RED}Bailing logic loop due to extreme API latency ({s.api_latency}ms).{Style.RESET_ALL}", "warn")
                await asyncio.sleep(5)
                continue

            if not s.ready or s.price <= 0 or s.atr == 0:
                s.log(f"{Fore.YELLOW}Awaiting readiness (Ready: {s.ready}, Price: {s.price}, ATR: {s.atr:.2f}).{Style.RESET_ALL}", "warn")
                await asyncio.sleep(1)
                continue
            ...
            if s.in_stasis():
                ...
                if s.stasis_reason == "Network Lost" and is_connected():
                    ...
                    s.log(f"{Fore.GREEN}Network Restored! Exiting Network Stasis early.{Style.RESET_ALL}", "health")
                
                await asyncio.sleep(5)
                continue
            
            limit_hit = False
            if s.daily_loss_reached or s.daily_profit_reached or (cfg.max_daily_trades > 0 and s.trade_count >= cfg.max_daily_trades):
                limit_hit = True
                reason = "Daily Loss Limit Reached" if s.daily_loss_reached else ("Daily Profit Target Reached" if s.daily_profit_reached else "Max Daily Trades Reached")
                s.log(f"{Fore.RED}Ritual Halted: {reason} ({s.daily_pnl_pct:.2%}).{Style.RESET_ALL}", "error")
            ...
            if limit_hit and s.active:
                ...
                exit_price = s.price + (Decimal(str(s.atr)) * Decimal("0.5")) if s.side == "Buy" else s.price - (Decimal(str(s.atr)) * Decimal("0.5"))
                await self.strike(exit_side, s.qty, reduce=True, order_type="Limit", price=exit_price)
                await asyncio.sleep(2)
                if s.active:
                    s.log(f"{Fore.RED}Forcing Market Close after Limit failed.{Style.RESET_ALL}", "error")
                    await self.strike(exit_side, s.qty, reduce=True, order_type="Market")
    ...
    async def _reconcile_position(self):
        ...
        r = await self.request("GET", "/v5/position/list",
                               {"category": cfg.category, "symbol": cfg.symbol})
        if r.get("retCode") != 0:
            self._report_error(f"Failed to reconcile position: {r.get('retMsg')}", "error", 118)
            return
        ...
        s.log(f"Position reconciled via REST. Qty: {s.qty}, Entry: {s.entry_p}.", "health")
    ...
    async def _manage_active_position(self):
        ...
        if s.entry_p == Decimal("0") or s.qty == Decimal("0") or atr_dec == Decimal("0"):
            self._report_error("Zero exit reference price/qty/ATR. Forcing closure.", "error", 118)
            await self.strike("Sell" if s.side == "Buy" else "Buy", s.qty, True)
            return

        # 1. Partial TP Harvest (Stage 1 -> Stage 2) (Improvement 6)
        if s.stage == 1:
            target_pnl_usdt = Decimal(str(s.atr * float(cfg.tp_partial_atr_mult))) * s.qty 
            current_pnl_usdt = abs(s.upnl)
            
            if current_pnl_usdt >= target_pnl_usdt:
                s.log(f"{Fore.GREEN}Stage 1 Harvest: {target_pnl_usdt:.2f} USDT Profit Taken. Setting trailing SL.{Style.RESET_ALL}", "exit")
                await self.strike("Sell" if s.side == "Buy" else "Buy", s.qty / 2, True)
                
                # Set trailing stop for remaining position
                if await self.setup_trailing_stop(s, cfg.leverage, s.atr, s.price_prec, cfg):
                    s.stage = 2
                    TermuxAPI.toast("Partial TP hit!")
                else:
                    self._report_error(f"Failed to set Trailing Stop after partial TP. Manually track.", "error", 120)
    ...
    async def _evaluate_entry_signals(self):
        ...
        if time.time() - s.last_trade_ts < cfg.cooldown_base: # **MAX FREQUENCY CHANGE**
            s.log(f"{Fore.BLUE}Entry cooldown active: {cfg.cooldown_base - (time.time() - s.last_trade_ts):.1f}s remaining.{Style.RESET_ALL}", "debug")
            return
        ...
        if win_rate > Decimal(str(cfg.dynamic_risk_win_rate_threshold)):
            ...
            s.log(f"DRM: High WR ({win_rate:.1%}) -> Risk {adjusted_risk_pct:.3%}.", "debug")
        elif win_rate < Decimal("0.4"):
            adjusted_risk_pct *= cfg.dynamic_risk_multiplier_low_win_rate
            s.log(f"DRM: Low WR ({win_rate:.1%}) -> Risk {adjusted_risk_pct:.3%}.", "debug")

        if s.atr < cfg.low_vol_atr_threshold:
            adjusted_risk_pct *= cfg.low_volatility_risk_multiplier
            s.log(f"DRM: Low Vol ({s.atr:.2f}) -> Risk {adjusted_risk_pct:.3%}.", "debug")
        ...
        ema_cross_long = ema_delta > Decimal("0") and safe_div(ema_delta, ema_m, Decimal("0")) > Decimal("0.0001")
        ema_cross_short = ema_delta < Decimal("0") and safe_div(-ema_delta, ema_m, Decimal("0")) > Decimal("0.0001")
        ...
        if s.alpha_score >= cfg.min_alpha_score and ema_proximity_ok:
            ...
            long_condition = (s.fisher > s.fisher_sig and s.rsi < 60 and ema_cross_long and trend_check)
            short_condition = (s.fisher < s.fisher_sig and s.rsi > 40 and ema_cross_short and trend_check)
        ...
        if not confirm_signal(s, current_signal_direction, cfg.entry_confirmation_candles):
            s.log(f"{Fore.BLUE}Signal '{current_signal_direction}' strength: {s.signal_strength}% confirmed: {s.signal_confirmed_candles}/{cfg.entry_confirmation_candles} candles. Awaiting further confirmation.{Style.RESET_ALL}", "debug")
            return

        # Signal confirmed, proceed with strike
        sl_dist = atr_dec * cfg.sl_atr_mult
        if sl_dist == Decimal("0"): 
            self._report_error("ATR is zero, cannot calculate SL distance for entry. Skipping strike.", "error", 119)
            return

        # --- Quantity Calculation (Improvement 22) ---
        qty = calculate_dynamic_qty(s.available, adjusted_risk_pct, s.atr, s.price, cfg.leverage, s.qty_step)
        ...
        if current_signal_direction == "BUY":
            s.log(f"{Fore.BLUE}Long entry signal CONFIRMED! Alpha: {s.alpha_score:.1f}. Preparing Limit strike.{Style.RESET_ALL}", "info")
            limit_p = s.bid.quantize(Decimal(f'1e-{s.price_prec}'))
            await self.strike(side="Buy", qty=qty, order_type="Limit", price=limit_p, take_profit_price=tp_price)
        elif current_signal_direction == "SELL":
            s.log(f"{Fore.BLUE}Short entry signal CONFIRMED! Alpha: {s.alpha_score:.1f}. Preparing Limit strike.{Style.RESET_ALL}", "info")
            limit_p = s.ask.quantize(Decimal(f'1e-{s.price_prec}'))
            await self.strike(side="Sell", qty=qty, order_type="Limit", price=limit_p, take_profit_price=tp_price)
    ...
    async def ws_nexus_public(self, stop: asyncio.Event):
        ...
        self.state.log(f"{Fore.BLUE}# Channeling public market data streams...{Style.RESET_ALL}", "health")
        reconnect_delay = 1
        while not stop.is_set():
            try:
                async with self.session.ws_connect(self.ws_pub, heartbeat=self.cfg.ws_heartbeat) as ws:
                    ...
                    async for msg in ws:
                        ...
                        if "tickers" in topic:
                            res = d.get("data", [{}])[0] if isinstance(d.get("data"), list) else d.get("data")
                            if not res: continue
                            ...
                            data = d["data"]
                            if d.get("type") == "snapshot": self.state.local_bids.clear(); self.state.local_asks.clear()
                            
                            for p, q in data.get("b", []):
                                price_dec = safe_decimal(p)
                                qty_dec = safe_decimal(q)
                                if qty_dec == Decimal("0") or price_dec <= Decimal("0"): continue
                            ...
                            k = d["data"][0]
                            if k.get("confirm"):
                                o, h, l, c, v = float(k["open"]), float(k["high"]), float(k["low"]), float(k["close"]), float(k["volume"])
                                self.state.ohlc.append((o, h, l, c, v))
                                
                                # Improvement 5: Confirmation based on closed candle
                                if not self.state.active and self.state.last_signal_direction != "NONE":
                                     if self.state.last_signal_direction == "BUY" and o < c:
                                         self.state.signal_confirmed_candles += 1
                                     elif self.state.last_signal_direction == "SELL" and o > c:
                                         self.state.signal_confirmed_candles += 1
    ...
    async def ws_nexus_private(self, stop: asyncio.Event):
        ...
        self.state.log(f"{Fore.MAGENTA}# Establishing secure channel for private account data...{Style.RESET_ALL}", "health")
        reconnect_delay = 1
        while not stop.is_set():
            try:
                        ...
                        if topic == "position":
                            for p in d.get("data", []):
                                ...
                                if was_active and not self.state.active: # Position just closed
                                    ...
                                    self.state.total_realized_pnl_session += self.state.last_trade_pnl
                                    self.state.smoothed_realized_pnl.push(self.state.last_trade_pnl)
                                    if self.state.last_trade_pnl > Decimal("0"):
                                        ...
                                        if self.state.loss_streak >= self.cfg.max_consecutive_losses:
                                            self.state.stasis_until = time.time() + self.cfg.stasis_duration_sec
                                            self.state.stasis_reason = "Loss Streak"
                                            self.state.log(f"{Fore.RED}Breaker: Max consecutive losses reached. Entering Stasis for {self.cfg.stasis_duration_sec}s.{Style.RESET_ALL}", "error")
    ...
    async def _check_battery_status(self, stop: asyncio.Event):
        ...
        self.state.log(f"{Fore.CYAN}# Initiating battery status vigil...{Style.RESET_ALL}", "health")
        while not stop.is_set():
        ...
        while not stop.is_set():
            if os.path.exists(config_filepath):
                ...
                if current_mod_time > self.state.last_config_mod_time:
                    self.state.log("config.json modified. Attempting hot reload...", "warn")
                    ...
                    if new_config:
                        ...
                        self.state.log("Failed to reload config.json. Keeping old config.", "error")
            await asyncio.sleep(5)
...
def build_ui(s: SentinelState) -> Layout:
    ...
    ora.add_row("BBands U/L/W", f"{s.bollinger_upper:.{s.price_prec}f} / {s.bollinger_lower:.{s.price_prec}f} / {s.bollinger_width_pct:.2f}%")
    
    # Ehlers Additions
    ora.add_row("Ehlers Trend", f"{'[green]UP[/]' if s.trend_signal == 1.0 else ('[red]DOWN[/]' if s.trend_signal == 0.0 else '[yellow]NEUTRAL[/]')}")
    ...
    if s.active:
        ...
        elif s.config.max_daily_trades > 0 and s.trade_count >= s.config.max_daily_trades:
        ...
        elif s.daily_loss_reached or s.daily_profit_reached or (s.config.max_daily_trades > 0 and s.trade_count >= s.config.max_daily_trades):
        ...
        tac.add_row("Realized PnL", f"[{realized_pnl_c}]{s.smoothed_realized_pnl.average():+.4f}[/]")
...
async def main():
    ...
    async with BybitApex(state) as apex:
        ...
        state.log(f"{Fore.CYAN}Initializing BCH OMNI-SENTINEL V22.3 (Resilient Hyper-Scalpel Mode)...{Style.RESET_ALL}", "health")
        ...
        with Live(build_ui(state), refresh_per_second=6, screen=True, console=console) as live:
            ...
            async def internet_watchdog():
                while not stop.is_set():
                    if not is_connected():
                        if state.stasis_reason not in ("Battery Critical", "Loss Streak"):
