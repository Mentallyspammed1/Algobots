...
def load_config(filepath: str) -> dict[str, Any]:
    ...
    try:
        ...
        print(f"{NEON_RED}Error loading config: {e}. Using default and attempting to save.{RESET}")
        try:
            with open(filepath, "w", encoding="utf-8") as f_default:
                json.dump(default_config, f_default, indent=4)
        except OSError as e_save:
            print(f"{NEON_RED}Could not save default config: {e_save}{RESET}")
        return default_config
...
def bybit_request(
    ...
    try:
        ...
        logger.error(f"{NEON_RED}HTTP Error: {e.response.status_code} - {e.response.text}{RESET}")
    except requests.exceptions.ConnectionError as e:
        logger.error(f"{NEON_RED}Connection Error: {e}{RESET}")
    except requests.exceptions.Timeout:
        logger.error(f"{NEON_RED}Request timed out after {REQUEST_TIMEOUT} seconds.{RESET}")
...
def fetch_klines(symbol: str, interval: str, limit: int, logger: logging.Logger) -> pd.DataFrame | None:
    ...
    if response and response['result'] and response['result']['list']:
        ...
        if df.empty:
            logger.warning(f"{NEON_YELLOW}Fetched klines for {symbol} {interval} but DataFrame is empty after processing. Raw response: {response}{RESET}")
            return None

        logger.debug(f"Fetched {len(df)} {interval} klines for {symbol}.")
        return df
    logger.warning(f"{NEON_YELLOW}Could not fetch klines for {symbol} {interval}. API response might be empty or invalid. Raw response: {response}{RESET}")
    return None
...
class PositionManager:
    ...
    def _calculate_order_size(self, current_price: Decimal, atr_value: Decimal) -> Decimal:
        ...
        if stop_loss_distance <= 0:
            self.logger.warning(f"{NEON_YELLOW}Calculated stop loss distance is zero or negative. Cannot determine order size.{RESET}")
            return Decimal('0')
        ...
        order_qty = order_qty.quantize(Decimal('0.0001'), rounding=ROUND_DOWN) # Example precision

        self.logger.info(f"Calculated order size: {order_qty} {self.symbol} (Risk: {risk_amount:.2f} USD)")
        return order_qty

    def open_position(self, signal: str, current_price: Decimal, atr_value: Decimal) -> dict | None:
        ...
        for i, position in enumerate(self.open_positions):
            if position["status"] == "OPEN":
                ...
                if closed_by:
                    ...
                    self.logger.info(f"{NEON_PURPLE}Closed {side} position by {closed_by}: {position}. PnL: {pnl:.2f}{RESET}")
...
class TradingAnalyzer:
        ...
        try:
            ...
            if result is None or (isinstance(result, pd.Series) and result.empty) or \
               (isinstance(result, tuple) and all(r is None or (isinstance(r, pd.Series) and r.empty) for r in result)):
                self.logger.warning(f"{NEON_YELLOW}Indicator '{name}' returned empty or None after calculation. Not enough valid data?{RESET}")
                return None
            return result
    ...
    def _calculate_all_indicators(self) -> None:
        ...
        if cfg["indicators"].get("sma_10", False):
            self.df['SMA_10'] = self._safe_calculate(lambda: self.df['close'].rolling(window=isd["sma_short_period"]).mean(), "SMA_10", min_data_points=isd["sma_short_period"])
        ...
        if cfg["indicators"].get("sma_trend_filter", False):
            self.df['SMA_Long'] = self._safe_calculate(lambda: self.df['close'].rolling(window=isd["sma_long_period"]).mean(), "SMA_Long", min_data_points=isd["sma_long_period"])
        ...
        if cfg["indicators"].get("ema_alignment", False):
            self.df['EMA_Short'] = self._safe_calculate(lambda: self.df['close'].ewm(span=isd["ema_short_period"], adjust=False).mean(), "EMA_Short", min_data_points=isd["ema_short_period"])
            self.df['EMA_Long'] = self._safe_calculate(lambda: self.df['close'].ewm(span=isd["ema_long_period"], adjust=False).mean(), "EMA_Long", min_data_points=isd["ema_long_period"])
        ...
        self.df['ATR'] = self._safe_calculate(lambda: self.df['TR'].ewm(span=isd["atr_period"], adjust=False).mean(), "ATR", min_data_points=isd["atr_period"])
        ...
        if cfg["indicators"].get("stoch_rsi", False):
            stoch_rsi_k, stoch_rsi_d = self._safe_calculate(self.calculate_stoch_rsi, "StochRSI",
                                                            min_data_points=isd["stoch_rsi_period"] + isd["stoch_d_period"] + isd["stoch_k_period"],
        ...
        if cfg["indicators"].get("bollinger_bands", False):
            bb_upper, bb_middle, bb_lower = self._safe_calculate(self.calculate_bollinger_bands, "BollingerBands",
        ...
        if cfg["indicators"].get("obv", False):
            obv_val, obv_ema = self._safe_calculate(self.calculate_obv, "OBV",
        ...
        if cfg["indicators"].get("cmf", False):
            cmf_val = self._safe_calculate(self.calculate_cmf, "CMF",
        ...
        if cfg["indicators"].get("ichimoku_cloud", False):
            tenkan_sen, kijun_sen, senkou_span_a, senkou_span_b, chikou_span = self._safe_calculate(
                self.calculate_ichimoku_cloud, "IchimokuCloud",
            ...
            if tenkan_sen is not None: self.df['Tenkan_Sen'] = tenkan_sen
            if kijun_sen is not None: self.df['Kijun_Sen'] = kijun_sen
            if senkou_span_a is not None: self.df['Senkou_Span_A'] = senkou_span_a
            if senkou_span_b is not None: self.df['Senkou_Span_B'] = senkou_span_b
            if chikou_span is not None: self.df['Chikou_Span'] = chikou_span

            if tenkan_sen is not None: self.indicator_values["Tenkan_Sen"] = tenkan_sen.iloc[-1]
            if kijun_sen is not None: self.indicator_values["Kijun_Sen"] = kijun_sen.iloc[-1]
            if senkou_span_a is not None: self.indicator_values["Senkou_Span_A"] = senkou_span_a.iloc[-1]
            if senkou_span_b is not None: self.indicator_values["Senkou_Span_B"] = senkou_span_b.iloc[-1]
            if chikou_span is not None: self.indicator_values["Chikou_Span"] = chikou_span.fillna(0).iloc[-1] # Fill NaN before taking last value

        # PSAR
        if cfg["indicators"].get("psar", False):
            psar_val, psar_dir = self._safe_calculate(self.calculate_psar, "PSAR",
        ...
        if cfg["indicators"].get("ehlers_supertrend", False):
            st_fast = self._safe_calculate(self.calculate_ehlers_supertrend, "EhlersSuperTrendFast",
                                           min_data_points=isd["ehlers_fast_period"] * 3, # Needs more data for smoothing
            ...
            st_slow = self._safe_calculate(self.calculate_ehlers_supertrend, "EhlersSuperTrendSlow",
                                           min_data_points=isd["ehlers_slow_period"] * 3, # Needs more data for smoothing
        ...
        if cfg["indicators"].get("macd", False):
            macd_line, signal_line, histogram = self._safe_calculate(self.calculate_macd, "MACD",
        ...
        if cfg["indicators"].get("adx", False):
            adx_val, plus_di, minus_di = self._safe_calculate(self.calculate_adx, "ADX",
        ...
        self.df.dropna(subset=['close'], inplace=True) # Ensure close price is always valid
        self.df.fillna(0, inplace=True) # Fill any remaining NaNs in indicator columns with 0
    ...
    def calculate_true_range(self) -> pd.Series:
        ...
        if len(self.df) < 2: return pd.Series(np.nan, index=self.df.index)
    ...
    def calculate_super_smoother(self, series: pd.Series, period: int) -> pd.Series:
        ...
        if period <= 0 or len(series) < 2: return pd.Series(np.nan, index=series.index)

        series = pd.to_numeric(series, errors='coerce').dropna()
        if len(series) < 2: return pd.Series(np.nan, index=series.index)
        ...
        if len(series) >= 2: filt.iloc[1] = (series.iloc[0] + series.iloc[1]) / 2
    ...
    def calculate_ehlers_supertrend(self, period: int, multiplier: float) -> pd.DataFrame | None:
        ...
        if len(self.df) < period * 3:
            self.logger.debug(f"Not enough data for Ehlers SuperTrend (period={period}). Need at least {period*3} bars.")
            return None
        ...
        if df_copy.empty:
            self.logger.debug("Ehlers SuperTrend: DataFrame empty after smoothing. Returning None.")
            return None
    ...
    def calculate_stoch_rsi(self, period: int, k_period: int, d_period: int) -> tuple[pd.Series, pd.Series]:
        ...
        if len(self.df) <= period: return pd.Series(np.nan, index=self.df.index), pd.Series(np.nan, index=self.df.index)
        ...
        stoch_rsi_k_raw = ((rsi - lowest_rsi) / (highest_rsi - lowest_rsi)).fillna(0) * 100
    ...
    def calculate_bollinger_bands(self, period: int, std_dev: float) -> tuple[pd.Series, pd.Series, pd.Series]:
        ...
        if len(self.df) < period: return pd.Series(np.nan, index=self.df.index), pd.Series(np.nan, index=self.df.index), pd.Series(np.nan, index=self.df.index)
    ...
    def calculate_ichimoku_cloud(self, tenkan_period: int, kijun_period: int, senkou_span_b_period: int, chikou_span_offset: int) -> tuple[pd.Series, pd.Series, pd.Series, pd.Series, pd.Series]:
        ...
        if len(self.df) < max(tenkan_period, kijun_period, senkou_span_b_period) + chikou_span_offset: return pd.Series(np.nan), pd.Series(np.nan), pd.Series(np.nan), pd.Series(np.nan), pd.Series(np.nan)
    ...
    def calculate_obv(self, ema_period: int) -> tuple[pd.Series, pd.Series]:
        ...
        if len(self.df) < 2: return pd.Series(np.nan), pd.Series(np.nan)
    ...
    def calculate_psar(self, acceleration: float, max_acceleration: float) -> tuple[pd.Series, pd.Series]:
        ...
        if len(self.df) < 2: return pd.Series(np.nan, index=self.df.index), pd.Series(np.nan, index=self.df.index)
    ...
    def _get_mtf_trend(self, higher_tf_df: pd.DataFrame, indicator_type: str) -> str:
        ...
        if indicator_type == "sma":
            ...
            if len(higher_tf_df) < period:
                self.logger.debug(f"MTF SMA: Not enough data for {period} period. Have {len(higher_tf_df)}.")
                return "UNKNOWN"
        ...
        if indicator_type == "ehlers_supertrend":
            ...
            st_result = temp_analyzer.calculate_ehlers_supertrend(period=self.indicator_settings["ehlers_slow_period"],
    ...
    def generate_trading_signal(
        ...
        if active_indicators.get("vwap", False):
            ...
            if not pd.isna(vwap):
                ...
                if len(self.df) > 1:
                    ...
                    if current_close > vwap and prev_close <= prev_vwap: # Bullish crossover
                        signal_score += weights.get("vwap", 0) * 0.3
                        self.logger.debug("VWAP: Bullish crossover detected.")
                    elif current_close < vwap and prev_close >= prev_vwap: # Bearish crossover
                        signal_score -= weights.get("vwap", 0) * 0.3
        ...
        if active_indicators.get("psar", False):
            ...
            if not pd.isna(psar_val) and not pd.isna(psar_dir):
                ...
                if len(self.df) > 1:
                    ...
                    if current_close > psar_val and prev_close <= prev_psar_val: # Bullish reversal
                        signal_score += weights.get("psar", 0) * 0.4
                        self.logger.debug("PSAR: Bullish reversal detected.")
                    elif current_close < psar_val and prev_close >= prev_psar_val: # Bearish reversal
                        signal_score -= weights.get("psar", 0) * 0.4
        ...
        if active_indicators.get("fibonacci_levels", False) and self.fib_levels:
            ...
            for level_name, level_price in self.fib_levels.items():
                if level_name not in ["0.0%", "100.0%"]: # Exclude high/low
                    if abs(current_price - level_price) / current_price < Decimal('0.001'): # Within 0.1%
        ...
        if active_indicators.get("macd", False):
            ...
            if not pd.isna(macd_line) and not pd.isna(signal_line) and not pd.isna(histogram):
                ...
                elif histogram > 0 and self.df['MACD_Hist'].iloc[-2] < 0: # Histogram crosses above zero
                    signal_score += weight * 0.2
                elif histogram < 0 and self.df['MACD_Hist'].iloc[-2] > 0: # Histogram crosses below zero
                    signal_score -= weight * 0.2
            ...
            if not pd.isna(adx_val) and not pd.isna(plus_di) and not pd.isna(minus_di):
                ...
                if adx_val > 25: # Strong trend
                    if plus_di > minus_di: # Bullish trend
                        signal_score += weight
                        self.logger.debug("ADX: Strong BUY trend (ADX > 25, +DI > -DI).")
                    elif minus_di > plus_di: # Bearish trend
                        signal_score -= weight
                        self.logger.debug("ADX: Strong SELL trend (ADX > 25, -DI > +DI).")
        ...
        if active_indicators.get("ichimoku_cloud", False):
            ...
            if not pd.isna(tenkan_sen) and not pd.isna(kijun_sen) and \
               not pd.isna(senkou_span_a) and not pd.isna(senkou_span_b) and not pd.isna(chikou_span):

                # Tenkan-sen crosses Kijun-sen
                if tenkan_sen > kijun_sen and self.df['Tenkan_Sen'].iloc[-2] <= self.df['Kijun_Sen'].iloc[-2]:
                    signal_score += weight * 0.5 # Tenkan-sen crosses above Kijun-sen (bullish)
                    ...
                    signal_score -= weight * 0.5 # Tenkan-sen crosses below Kijun-sen (bearish)
                    self.logger.debug("Ichimoku: Tenkan-sen crossed below Kijun-sen (bearish).")

                # Price breaks through Kumo (Cloud)
                if current_close > max(senkou_span_a, senkou_span_b) and \
                   self.df['close'].iloc[-2] <= max(self.df['Senkou_Span_A'].iloc[-2], self.df['Senkou_Span_B'].iloc[-2]):
                    signal_score += weight * 0.7 # Price breaks above Kumo (strong bullish)
                    self.logger.debug("Ichimoku: Price broke above Kumo (strong bullish).")
                elif current_close < min(senkou_span_a, senkou_span_b) and \
                     self.df['close'].iloc[-2] >= min(self.df['Senkou_Span_A'].iloc[-2], self.df['Senkou_Span_B'].iloc[-2]):
                    signal_score -= weight * 0.7 # Price breaks below Kumo (strong bearish)
                ...
                if chikou_span > current_close and self.df['Chikou_Span'].iloc[-2] <= self.df['close'].iloc[-2]:
                    signal_score += weight * 0.3 # Chikou Span crosses above price (bullish confirmation)
                    ...
                    signal_score -= weight * 0.3 # Chikou Span crosses below price (bearish confirmation)
        ...
        if active_indicators.get("cmf", False):
            ...
            if not pd.isna(cmf_val):
                ...
                if cmf_val > 0: signal_score += weight * 0.5 # Positive money flow
                elif cmf_val < 0: signal_score -= weight * 0.5 # Negative money flow
        ...
        if self.config["mtf_analysis"]["enabled"] and mtf_trends:
            ...
            for tf, trend in mtf_trends.items():
                if trend == "UP": mtf_buy_score += 1
                elif trend == "DOWN": mtf_sell_score -= 1 # Subtract for sell bias
            ...
            if mtf_trends:
                ...
                self.logger.debug(f"MTF Confluence: Score {normalized_mtf_score:.2f} (Buy: {mtf_buy_score}, Sell: {abs(mtf_sell_score)}). Total MTF contribution: {mtf_weight * normalized_mtf_score:.2f}")
    ...
    def calculate_entry_tp_sl(self, current_price: Decimal, atr_value: Decimal, signal: str) -> tuple[Decimal, Decimal]:
        ...
        return take_profit.quantize(Decimal('0.01'), rounding=ROUND_DOWN), stop_loss.quantize(Decimal('0.01'), rounding=ROUND_DOWN)

def display_indicator_values_and_price(
    config: dict[str, Any], logger: logging.Logger, current_price: Decimal,
    df: pd.DataFrame, orderbook_data: dict | None, mtf_trends: dict[str, str]
) -> None:
...
def main() -> None:
    ...
    for htf_interval in config["mtf_analysis"]["higher_timeframes"]:
        if htf_interval not in valid_bybit_intervals:
            logger.error(f"{NEON_RED}Invalid higher timeframe interval '{htf_interval}' in config.json. Please use Bybit's valid string formats (e.g., '1h' should be '60', '4h' should be '240'). Exiting.{RESET}")
            sys.exit(1)
    ...
    while True:
        try:
            ...
            if current_price is None:
                alert_system.send_alert("Failed to fetch current price. Skipping loop.", "WARNING")
            ...
            if df is None or df.empty:
                alert_system.send_alert("Failed to fetch primary klines or DataFrame is empty. Skipping loop.", "WARNING")
                time.sleep(config['loop_delay'])
                continue
            ...
            if config["indicators"].get("orderbook_imbalance", False):
                orderbook_data = fetch_orderbook(config['symbol'], config['orderbook_limit'], logger)
                ...
                for htf_interval in config["mtf_analysis"]["higher_timeframes"]:
                    ...
                    if htf_df is not None and not htf_df.empty:
                        ...
                        for trend_ind in config["mtf_analysis"]["trend_indicators"]:
                            # Pass the logger and symbol to the temporary analyzer
                            temp_htf_analyzer = TradingAnalyzer(htf_df, config, logger, config['symbol'])
                            trend = temp_htf_analyzer._get_mtf_trend(temp_htf_analyzer.df, trend_ind)
            ...
            display_indicator_values_and_price(config, logger, current_price, df, orderbook_data, mtf_trends)


            # --- Trading Signal Generation ---
            analyzer = TradingAnalyzer(df, config, logger, config['symbol'])

            if analyzer.df.empty: # Check if analyzer's df is empty after indicator calculations
                alert_system.send_alert("TradingAnalyzer DataFrame is empty after indicator calculations. Cannot generate signal.", "WARNING")
                time.sleep(config['loop_delay'])
                continue
            ...
            # --- Position Management ---
            position_manager.manage_positions(current_price, performance_tracker) # Check existing positions
            ...
            if open_positions:
                ...
                for pos in open_positions:
                    logger.info(f"  - {pos['side']} @ {pos['entry_price']} (SL: {pos['stop_loss']}, TP: {pos['take_profit']}){RESET}")
            else:
                logger.info(f"{NEON_CYAN}No open positions.{RESET}")

            # Log performance summary
            perf_summary = performance_tracker.get_summary()
            logger.info(f"{NEON_YELLOW}Performance Summary: Total PnL: {perf_summary['total_pnl']:.2f}, Wins: {perf_summary['wins']}, Losses: {perf_summary['losses']}, Win Rate: {perf_summary['win_rate']}{RESET}")

            logger.info(f"{NEON_PURPLE}--- Analysis Loop Finished. Waiting {config['loop_delay']}s ---{RESET}")
            time.sleep(config['loop_delay'])

        except Exception as e:
            alert_system.send_alert(f"An unhandled error occurred in the main loop: {e}", "ERROR")
