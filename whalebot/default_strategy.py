...
class DefaultStrategy(BaseStrategy):
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df.ta.ema(length=self.strategy_ema_fast_period, append=True, col_names=(f'EMA_{self.strategy_ema_fast_period}',))
        df.ta.ema(length=self.strategy_ema_slow_period, append=True, col_names=(f'EMA_{self.strategy_ema_slow_period}',))
        df.ta.rsi(length=self.strategy_rsi_period, append=True, col_names=(f'RSI_{self.strategy_rsi_period}',))
        df.ta.macd(fast=self.strategy_macd_fast_period, slow=self.strategy_macd_slow_period, signal=self.strategy_macd_signal_period, append=True)
        df.ta.bbands(length=self.strategy_bb_period, std=self.strategy_bb_std, append=True)
        df.ta.atr(length=self.strategy_atr_period, append=True, col_names=(f'ATR_{self.strategy_atr_period}',))
    ...
    def generate_signal(self, df: pd.DataFrame, current_market_price: float, market_conditions: dict[str, Any]) -> Signal:
        ...
        min_data_points = max(self.strategy_ema_slow_period, self.strategy_rsi_period, self.strategy_macd_slow_period, self.strategy_bb_period, self.strategy_atr_period, self.strategy_adx_period) + 2
        ...
        if latest['RSI'] < self.strategy_rsi_oversold and previous['RSI'] >= self.strategy_rsi_oversold:
            signal_score += rsi_weight * 1.5
            reasons.append(f"RSI Entering Oversold ({latest['RSI']:.2f})")
        elif latest['RSI'] > self.strategy_rsi_overbought and previous['RSI'] <= self.strategy_rsi_overbought:
            signal_score -= rsi_weight * 1.5
        ...
        elif current_market_price < latest['BB_Middle'] and latest['BB_Middle'] > previous['BB_Middle']:
    ...
    def get_indicator_values(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        for col in [
            ...
            'BB_Lower', 'BB_Middle', 'BB_Upper', 'ADX', 'PlusDI', 'MinusDI'
        ]:
