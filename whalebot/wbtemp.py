...
class TradingAnalyzer:
    ...
    def _calculate_market_structure_indicators(self) -> None:
    ...
    def _calculate_dema_indicators(self) -> None:
    ...
    def _calculate_keltner_channels_indicators(self) -> None:
    ...
    def _calculate_roc_indicators(self) -> None:
        """Calculate ROC indicators."""
        cfg = self.config
        isd = self.indicator_settings
    ...
    def _calculate_candlestick_patterns_indicators(self) -> None:
    ...
    def _calculate_all_indicators(self) -> None:
        ...
        self._calculate_market_structure_indicators()
        self._calculate_dema_indicators()
        self._calculate_keltner_channels_indicators()
        self._calculate_roc_indicators()
        self._calculate_candlestick_patterns_indicators()

        # Final dropna after all indicators are calculated
        initial_len = len(self.df)
        # Drop rows where 'close' price is NaN (should ideally not happen with Bybit klines, but for robustness)
        self.df.dropna(subset=["close"], inplace=True)
        # Fill any remaining NaNs in indicator columns with 0 or a sensible default
    ...
    def calculate_psar(
        ...
        af = acceleration
        # Initialize EP based on the first observed trend
        if self.df["close"].iloc[0] < self.df["close"].iloc[1]:
            ep = self.df["high"].iloc[0]  # Initial guess for bullish
            bull.iloc[0] = True
        else:
            ep = self.df["low"].iloc[0]  # Initial guess for bearish
            bull.iloc[0] = False
    ...
    def calculate_fibonacci_pivot_points(self) -> None:
        ...
        # In a live bot, you'd typically use the *previous day's* candle for daily pivots
        # For intra-day, we can use the previous full candle.
    ...
    def calculate_kaufman_ama(
        ...
        )  # Shifted to align with current price
...
def display_indicator_values_and_price(
    ...
    if not pd.isna(macd_hist):
        ...
        # Check if MACD_Hist column exists and has enough data
