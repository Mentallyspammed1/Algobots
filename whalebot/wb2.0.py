...
BASE_URL = os.getenv("BYBIT_BASE_URL", "https://api.bybit.com") # For REST API
WS_URL = os.getenv("BYBIT_WS_URL", "wss://stream.bytick.com/realtime") # For WebSocket API
...
INDICATOR_COLORS = {
    "SMA_10": NEON_BLUE, "SMA_Long": Fore.BLUE, "EMA_Short": NEON_MAGENTA,
    ...
    "OBV": Fore.BLUE, "OBV_EMA": NEON_BLUE, "CMF": NEON_MAGENTA, "Tenkan_Sen": NEON_CYAN,
    "Kijun_Sen": Fore.LIGHTCYAN_EX, "Senkou_Span_A": NEON_GREEN, "Senkou_Span_B": NEON_RED,
    "Chikou_Span": NEON_YELLOW, "PSAR_Val": NEON_MAGENTA, "PSAR_Dir": Fore.LIGHTMAGENTA_EX,
    "VWAP": Fore.WHITE, "ST_Fast_Dir": Fore.BLUE, "ST_Fast_Val": NEON_BLUE,
    "ST_Slow_Dir": NEON_MAGENTA, "ST_Slow_Val": Fore.LIGHTMAGENTA_EX, "MACD_Line": NEON_GREEN,
    ...
    "Relative_Volume": NEON_MAGENTA, "Market_Structure_Trend": Fore.LIGHTCYAN_EX,
    "DEMA": Fore.BLUE, "Keltner_Upper": NEON_MAGENTA, "Keltner_Middle": Fore.WHITE,
    "Keltner_Lower": NEON_MAGENTA, "ROC": NEON_GREEN, "Pivot": Fore.WHITE,
    "R1": NEON_CYAN, "R2": Fore.LIGHTCYAN_EX, "S1": NEON_MAGENTA, "S2": Fore.LIGHTMAGENTA_EX,
...
class PybitTradingClient:
    ...
    # --- REST API Methods ---
    # Ensure all methods check self.enabled and self.session before proceeding.
    ...
    # ... [Include all other REST API methods like get_wallet_balance, place_order, cancel_order, etc.] ...
    # Ensure each method checks for self.enabled and self.session, logs errors, and handles 403s.
    # For example, place_order:
    ...
    def get_private_url(self) -> dict | None:
        ...
        if not self.enabled or not self.session: return None
        try:
            # This method might vary based on Pybit version; assuming it exists for listenKey
            # If get_private_url is deprecated or changed, adjust accordingly.
            # Example: response = self.session.user_linked_term() might be used.
            # For now, assuming it's available or can be found in Pybit docs.
            # Let's use a placeholder if the exact method is unknown.
            # response = self.session.user_info() # Example of another potential method
            
            # If get_private_url is indeed correct for your Pybit version:
            response = self.session.get_private_url() # This is a guess based on common patterns
            self._log_api("get_private_url", response, "debug")
            return response
            
        except pybit.exceptions.FailedRequestError as e:
            self.logger.error(f"get_private_url exception: {e}\n{traceback.format_exc()}")
            self._handle_403_error(e)
            return None
        except AttributeError: # If get_private_url method doesn't exist
            self.logger.error("Method 'get_private_url' not found in Pybit client. Please check Pybit documentation for obtaining listenKey.")
            return None
...
class WebSocketManager:
    ...
    def start(self):
        ...
        self.ws_thread = threading.Thread(target=self._run_ws, daemon=True, name="BybitWS")
    ...
    def _run_ws(self):
        ...
        while not self.stop_event.is_set():
            if self.ws and self.ws.is_connected(): # If already connected, just maintain it
                try: self.ws.send(json.dumps({"op": "ping"})) # Send ping for keepalive
                except Exception: self.ws.close() # Force reconnect on ping failure
            ...
            try:
                ...
                self.ws.run_forever() # This call blocks until the connection is closed
            except Exception as e:
                self.logger.error(f"WebSocket run_forever error: {e}. Retrying in {self.connect_delay}s.\n{traceback.format_exc()}")
    ...
    def on_close(self, close_status_code, close_msg):
        ...
        self.logger.warning(f"WebSocket closed: {close_status_code} {close_msg}. Reconnecting...")
        self.ws = None # Clear WS object to trigger reconnect logic in _run_ws
    ...
    def on_open(self, ws):
        ...
        self._subscribe_channels() # Subscribe to necessary data streams
    ...
    def _subscribe_channels(self):
        ...
        try:
            # Obtain listenKey via REST API call
            listen_key_resp = self.api_client.get_private_url()
            listen_key = listen_key_resp.get('apiData', {}).get('listenKey')
            if not listen_key:
                self.logger.error("Could not obtain listen key for user stream subscription.")
                return

            # Define topics based on config and symbol
            subscriptions = [
                f"user.order.{listen_key}", # Order updates
                f"position.{self.cfg['symbol']}", # Position updates for main symbol
                f"execution.{self.cfg['symbol']}", # Execution updates
                # Add other topics like account updates if necessary
            ]
            
            payload = {"op": "subscribe", "args": subscriptions}
            if self.ws: self.ws.send(json.dumps(payload))
            self.logger.info(f"Subscribed to WS topics: {', '.join(subscriptions)}")

        except Exception as e:
            self.logger.error(f"Error subscribing to WS channels: {e}\n{traceback.format_exc()}")
    ...
    def _sync_initial_state(self):
        ...
        if pos_data and self.api_client._ok(pos_data):
            # Process positions, potentially filtering for the main symbol if needed
    ...
    def _process_ws_message(self, data):
        ...
        if ret_code == 0 and topic: # Process successful messages with a topic
            ...
            elif "account" in topic: self._handle_account_update(data.get("data", {}))
        elif "success" in data and data["success"]: self.logger.info(f"WS Subscription successful: {data}")
        elif "retCode" in data and data["retCode"] != 0:
            self.logger.error(f"WS Error Response: {data.get('retMsg', 'Unknown error')}")

    def _handle_order_update(self, order_list):
    ...
    def _handle_position_update(self, position_list):
    ...
    def _handle_execution_update(self, execution_list):
...
class PositionManager:
    ...
    def _convert_ws_position_to_local(self, ws_pos: Dict) -> Dict:
        ...
        return {
            "entry_time": datetime.fromtimestamp(int(ws_pos.get("time", 0)) / 1000, tz=TIMEZONE),
    ...
    def set_tpsl_for_position(self, position: dict, current_price: Decimal, atr_value: Decimal):
        ...
        if tp_scheme["mode"] == "atr_multiples" and tp_scheme.get("targets"):
            # Assuming build_partial_tp_targets helper function is correctly defined
...
class ExchangeExecutionSync:
    ...
    def _move_stop_to_breakeven(self, open_pos: dict, atr_value: Decimal):
        ...
        try:
            ...
            else: self.logger.warning(f"Could not find order ID or link ID for old SL of position {open_pos.get('link_prefix')}.")
            ...
            sresp = self.pybit.place_order(
                ...
                qty=self.pybit._q(open_pos["qty"]), reduceOnly=True, # Use remaining quantity
            ...
            if self.pybit._ok(sresp):
                ...
                self.logger.info(f"Moved SL to breakeven at {new_sl_price} (Order ID: {open_pos['stop_loss_order_id']}).")
            else:
                self.logger.error(f"Failed to place breakeven SL: {sresp.get('retMsg')}")

        except Exception as e:
            self.logger.error(f"Breakeven move exception for {self.symbol}: {e}\n{traceback.format_exc()}")
    ...
    def poll(self):
        ...
        try:
            ...
            for r in rows:
                ...
                if not self._is_ours(link) or exec_time_ms < self.last_exec_time_ms: continue
                ...
                if tag in ("TP", "SL") and open_pos and open_pos["status"] == "OPEN":
                    ...
                    if open_pos["qty"] <= 0: # Position fully closed
                        ...
                        self.logger.info(f"Position for {self.symbol} fully closed by {tag}.")
                    else:
                        self.logger.info(f"Partial execution for {self.symbol} by {tag}. Remaining Qty: {open_pos['qty'].normalize()}")
                    
                    # Trigger breakeven logic if TP1 was hit and enabled
                    if tag == "TP" and link.endswith("_tp1") and not open_pos.get("breakeven_set", False):
            ...
            self.logger.error(f"Execution sync error for {self.symbol}: {e}\n{traceback.format_exc()}")
...
class PositionHeartbeat:
    ...
    def tick(self):
        ...
        self.logger.debug(f"Performing position heartbeat for {self.symbol}...")

        try:
            resp = self.pybit.get_positions(self.symbol) # Fetch positions from exchange
            if not self.pybit._ok(resp):
                self.logger.warning(f"Heartbeat: Failed to fetch positions for {self.symbol}. Response: {resp}")
                return
            ...
            if net_qty_exchange == 0 and has_local_pos:
                ...
                self.logger.info(f"Heartbeat: Closed local position(s) for {self.symbol} (exchange is flat).")
            ...
            # Scenario 2: Exchange has a position, but no local position -> Create synthetic local position
                ...
                self.logger.warning(f"Heartbeat: Created synthetic local position for {self.symbol} to match exchange state.")
            ...
            # Scenario 3: Both have positions; compare and log mismatches
                ...
                if abs(net_qty_exchange) != local_pos["qty"]:
                    self.logger.warning(f"Heartbeat: Quantity mismatch for {self.symbol}. Local: {local_pos['qty'].normalize()}, Exchange: {abs(net_qty_exchange).normalize()}.")
                # Add more checks here if needed (e.g., avgPrice, SL/TP)

        except (pybit.exceptions.FailedRequestError, Exception) as e:
            self.logger.error(f"Heartbeat error for {self.symbol}: {e}\n{traceback.format_exc()}")
...
class TradingAnalyzer:
    ...
    def _calculate_all_indicators(self):
        ...
        if self.config["indicators"].get("atr", False) or any(k in isd and "atr" in k for k in ["atr_period", "keltner_atr_period"]):
        ...
        if cfg["indicators"].get("ema_alignment", False):
            ...
            if "EMA_Short" in self.df.columns and self.df["EMA_Short"].iloc[-1] is not None: self.indicator_values["EMA_Short"] = self.df["EMA_Short"].iloc[-1]
            if "EMA_Long" in self.df.columns and self.df["EMA_Long"].iloc[-1] is not None: self.indicator_values["EMA_Long"] = self.df["EMA_Long"].iloc[-1]
        
        # ... [Ensure all other indicator calculations from the original script are included here] ...
        # For example: RSI, MACD, ADX, Bollinger Bands, VWAP, etc.
        
        # --- DataFrame Cleanup ---
        initial_len = len(self.df)
        # Drop rows with NaNs in essential columns like price data or key indicators (e.g., ATR)
        essential_cols = ["close", "high", "low", "open", "volume", "ATR"]
    ...
    def calculate_fibonacci_levels(self) -> None:
        ...
        if fib_levels: self.fib_levels = fib_levels
    ...
    # Include all the _score_* methods here. For brevity, only showing a few examples.
    # Ensure these methods correctly handle NaNs from indicators and use weights.
    ...
    def _score_adx(self, trend_strength_multiplier_in: float) -> Tuple[float, Dict]:
        ...
        # ... [Ensure full implementation is present from previous response] ...
        return 0.0, {"breakdown": {}, "trend_strength_multiplier": trend_strength_multiplier_in}
    ...
    # ... [Include all other _score_* methods like _score_bollinger_bands, _score_macd, etc.] ...
    ...
    def calculate_support_resistance_from_orderbook(self, orderbook_data: dict) -> None:
        ...
        # Store levels with proper precision
        price_precision = self.config["trade_management"]["price_precision"]
        if support_level > 0:
    ...
    def _dynamic_threshold(self, base_threshold: float) -> float:
        ...
        atr_now = self._get_indicator_value("ATR", 0.0)
    ...
    def _market_regime(self) -> str:
        ...
        if pd.isna(adx) or pd.isna(bb_u) or pd.isna(bb_m) or pd.isna(bb_l) or bb_m == 0:
            return "UNKNOWN"
        
        band_width_pct = float((bb_u - bb_l) / bb_m) if bb_m != 0 else 0 # Calculate Bollinger Band width percentage
    ...
    def generate_trading_signal(self, current_price: Decimal, orderbook_data: Optional[dict], mtf_trends: Dict[str, str]) -> Tuple[str, float, Dict[str, float]]:
        ...
        scorers_config = [
            # List of (scorer_function, arguments_list)
        ...
        should_override_hold = False
        if self.config["_last_score"] != 0.0: # If there was a previous non-zero score
            if is_strong_buy and self.config["_last_score"] < (dynamic_threshold * hysteresis_ratio):
                should_override_hold = True
            elif is_strong_sell and self.config["_last_score"] > (-dynamic_threshold * hysteresis_ratio):
                should_override_hold = True

        if is_strong_buy and should_override_hold: final_signal = "BUY"
        elif is_strong_sell and should_override_hold: final_signal = "SELL"
        elif is_strong_buy: final_signal = "BUY"
        elif is_strong_sell: final_signal = "SELL"
    ...
    # --- Include all _score_* methods here ---
    # These methods are crucial and should be fully implemented as shown in previous responses.
    # For example: _score_adx, _score_ema_alignment, _score_momentum_indicators, etc.
    # Make sure they correctly use indicator values, weights, and handle NaNs.

    # Placeholder examples (ensure full implementation is present):
    def _score_adx(self, trend_strength_multiplier_in: float) -> Tuple[float, dict]:
        # ... [Full implementation required] ...
        return 0.0, {"breakdown": {}, "trend_strength_multiplier": trend_strength_multiplier_in}
    ...
    def _score_orderbook_imbalance(self, orderbook_data: Optional[dict]) -> Tuple[float, dict]:
        """Scores based on order book imbalance and populates S/R levels."""
        imbalance_contrib = 0.0
        signal_breakdown_contrib = {}
        if self.config["indicators"].get("orderbook_imbalance", False) and orderbook_data:
            try:
                ...
                # Calculate Support/Resistance levels from orderbook depth
                self.calculate_support_resistance_from_orderbook(orderbook_data)
                signal_breakdown_contrib["Orderbook Imbalance"] = imbalance_contrib
            except Exception as e:
                self.logger.error(f"Error processing orderbook data for scoring: {e}\n{traceback.format_exc()}")
    ...
    def _score_mtf_confluence(self, mtf_trends: Dict[str, str]) -> Tuple[float, Dict]:
        ...
        if self.config["mtf_analysis"]["enabled"] and mtf_trends:
            ...
            if total_mtf_indicators > 0:
                mtf_weight = self.weights.get("mtf_trend_confluence", 0.0)
                # Calculate score proportionally to agreement
                normalized_mtf_score = (mtf_buy_count - mtf_sell_count) / total_mtf_indicators
                mtf_contribution = mtf_weight * normalized_mtf_score
    ...
    def _market_regime(self) -> str:
        ...
        if pd.isna(adx) or pd.isna(bb_u) or pd.isna(bb_m) or pd.isna(bb_l) or bb_m == 0: return "UNKNOWN"
        band_width_pct = float((bb_u - bb_l) / bb_m) if bb_m != 0 else 0
...
def main():
    ...
    # Proceed only if Pybit client is enabled and initialized successfully
    if pybit_client.enabled and pybit_client.session:
        ...
        if not performance_tracker.load_state(pybit_client.state_data): # Load performance state
        ...
        # In a real scenario, you might want to exit or switch to simulation mode here.
        # For now, we'll allow the loop to run but all live trading functions will be no-ops.
        position_manager = PositionManager(config, global_logger, config["symbol"], pybit_client) # Still initialize PM for simulated trades
        performance_tracker = PerformanceTracker(global_logger, config)
    ...
    while not pybit_client.stop_event.is_set():
        try:
            ...
            tp_mult_adapted, sl_mult_mult_adapted = adapt_exit_params(performance_tracker, config)
            ...
            # Current implementation saves state on explicit shutdown.
            pass
