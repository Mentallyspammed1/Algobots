...
def create_session() -> requests.Session:
    ...
    retries = Retry(
        ...
        status_forcelist=[429, 500, 502, 503, 504], # Added common HTTP error codes
        ...
        for symbol, position in list(self.open_positions.items()): # Iterate over a copy to allow modification
            if position["status"] == "OPEN":
                ...
                if not is_trailing_activated:
                    ...
                    if (side == "BUY" and current_price >= entry_price * (Decimal("1") + self.trailing_stop_activation_percent)) or \
                       (side == "SELL" and current_price <= entry_price * (Decimal("1") - self.trailing_stop_activation_percent)):
                        ...
                        if side == "BUY":
                            ...
                            position["current_trailing_sl"] = initial_trailing_sl.quantize(Decimal(price_precision_str), rounding=ROUND_DOWN)
                        else: # SELL
                            initial_trailing_sl = current_price + (atr_value * trailing_stop_atr_multiple)
                            position["current_trailing_sl"] = initial_trailing_sl.quantize(Decimal(price_precision_str), rounding=ROUND_DOWN)

                        self.logger.info(f"Trailing stop activated for {symbol}. Initial SL: {position['current_trailing_sl'].normalize()}")
                    ...
                    if side == "BUY":
                        ...
                        if potential_new_sl > current_trailing_sl:
                            potential_new_sl = potential_new_sl.quantize(Decimal(price_precision_str), rounding=ROUND_DOWN)
                            position["current_trailing_sl"] = potential_new_sl
                            self.logger.info(f"Updating trailing stop for {symbol} to {position['current_trailing_sl'].normalize()}")
...
class PerformanceTracker:
            ...
            try:
                with self.config_file.open("r", encoding="utf-8") as f:
                    ...
                    for trade in raw_trades:
                        for key in ["pnl", "entry_price", "exit_price", "qty", "stop_loss", "take_profit", "current_trailing_sl"]:
    ...
    def record_trade(self, position: dict, pnl: Decimal) -> None:
        ...
        trade_record = {
            "entry_time": position.get("entry_time", datetime.now(TIMEZONE)).isoformat(),
...
class TradingAnalyzer:
    ...
    def _safe_series_op(self, series: pd.Series, name: str) -> pd.Series:
        ...
        if series is None or series.empty:
            self.logger.debug(f"Series '{name}' is empty or None. Returning empty Series.")
            return pd.Series(dtype=float) # Return an empty Series with float dtype
        if series.isnull().all():
            self.logger.debug(f"Series '{name}' contains all NaNs. Returning Series with NaNs.")
            return series
        return series
    ...
    def calculate_ehlers_supertrend(
        ...
        while first_valid_idx_loc < len(df_clean) and (
            pd.isna(df_clean["close"].iloc[first_valid_idx_loc]) or
            pd.isna(upper_band.iloc[first_valid_idx_loc]) or
            pd.isna(lower_band.iloc[first_valid_idx_loc])
        ):
        ...
        if df_clean["close"].iloc[first_valid_idx_loc] > upper_band.iloc[first_valid_idx_loc]:
    ...
    def calculate_psar(
        ...
        if psar_val_col not in psar_result.columns:
            self.logger.error(f"{NEON_RED}Missing expected PSAR column '{psar_val_col}' in result: {psar_result.columns.tolist()}{RESET}")
            return pd.Series(np.nan, index=self.df.index), pd.Series(np.nan, index=self.df.index)
        ...
        if first_valid_idx is not None:
            ...
            for i in range(self.df.index.get_loc(first_valid_idx) + 1, len(self.df)):
                ...
                else: # Was in a downtrend
                    if self.df["close"].loc[current_idx] > psar_val.loc[current_idx]:
                        direction.loc[current_idx] = 1 # Reversal to uptrend
                    else:
                        direction.loc[current_idx] = -1 # Continue downtrend

        direction.fillna(0, inplace=True) # Fill any remaining initial NaNs with 0
    ...
    def calculate_fibonacci_levels(self) -> None:
        ...
        self.fib_levels = {
            "0.0%": decimal_high.quantize(Decimal(price_precision_str), rounding=ROUND_DOWN),
            ...
            "100.0%": decimal_low.quantize(Decimal(price_precision_str), rounding=ROUND_DOWN),
        }
    ...
    def _check_orderbook(self, current_price: Decimal, orderbook_manager: Any) -> float:
        ...
        if not orderbook_manager:
            self.logger.warning("Orderbook manager not available for imbalance check. Returning 0.0.")
            return 0.0
    ...
    async def generate_trading_signal(
        ...
        orderbook_manager: Any, # Changed type hint to Any as AdvancedOrderbookManager is not defined
        ...
        if active_indicators.get("momentum", False):
            ...
            if active_indicators.get("stoch_rsi", False):
                ...
                if not pd.isna(stoch_k) and not pd.isna(stoch_d) and len(self.df) > 1:
                    prev_stoch_k = self.df["StochRSI_K"].iloc[-2] if "StochRSI_K" in self.df.columns else np.nan
                    prev_stoch_d = self.df["StochRSI_D"].iloc[-2] if "StochRSI_D" in self.df.columns else np.nan
                    if (
                        stoch_k > stoch_d
                        and (pd.isna(prev_stoch_k) or pd.isna(prev_stoch_d) or prev_stoch_k <= prev_stoch_d)
        ...
        if active_indicators.get("macd", False):
            ...
            if (
                ...
                elif histogram > 0 and (len(self.df) > 2 and "MACD_Hist" in self.df.columns and self.df["MACD_Hist"].iloc[-2] < 0):
        ...
        if self.config["gemini_ai_analysis"]["enabled"] and self.gemini_client:
            # gemini_prompt = self._summarize_market_for_gemini(current_close) # _summarize_market_for_gemini not implemented
...
async def main_async_loop(config, logger, position_manager, performance_tracker, alert_system, gemini_client):
    ...
    while True:
        try:
            ...
            orderbook_data = None # AdvancedOrderbookManager is not implemented, so this will remain None
            # if config["indicators"].get("orderbook_imbalance", False):
                    ...
                    if htf_df is not None and not htf_df.empty:
                        for trend_ind in config["mtf_analysis"]["trend_indicators"]:
                            ...
                            trend = temp_htf_analyzer._get_mtf_trend(
                                htf_df, trend_ind # Corrected from temp_htf_df to htf_df
                            )
            ...
            display_indicator_values_and_price(
                config, logger, current_price, analyzer, None, mtf_trends # Pass None for orderbook_manager
            )
