...
export function choppiness(highs, lows, closes, period) {
    ...
    for (let i = period - 1; i < highs.length; i++) {
        ...
        if (range === 0 || sumTR === 0) { // If no range or no true range, choppiness is 0 (fully trended)
            results[i] = 0;
            continue;
        }
        
        const logPeriod = Math.log10(period);
        if (logPeriod === 0) { // Should not happen for period > 1, but defensively
            results[i] = 0;
            continue;
        }

        const ratio = sumTR / range;
        if (ratio <= 0) { // Prevent log10(0) or log10(negative)
            results[i] = 0;
            continue;
        }

        results[i] = 100 * Math.log10(ratio) / logPeriod;
    }
    return results;
}
