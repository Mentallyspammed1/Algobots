/**
 * 🌊 WHALEWAVE PRO - TITAN EDITION (Final, Ultra-Comprehensive Build)
 * ----------------------------------------------------------------------
 * - ALL Standard/Advanced Indicators Included: RSI, MFI, Stoch, CCI, MACD, ADX, S/R, Fibs, FVG, ST, CE.
 * - Stability Fix: Chandelier Exit ReferenceError resolved.
 */
    ...
    evaluate(priceVal, signal) {
        ...
        if (this.pos) {
            ...
            try {
                ...
                this.balance = this.balance.sub(entry.mul(qty).mul(config.paper_trading.fee));
...
class GeminiBrain {
    ...
    async analyze(ctx) {
        ...
        DECISION REGIME (Must choose one based on Chop/R2/ADX):
        1. MOMENTUM: Chop < 40 AND R2 > 0.3 AND ADX > 25. Strategy: Long if all trend indicators (MTF, ST, Chande) are bullish, short otherwise. Use FVG/Fibs/Chande as entry/SL/TP.
        2. MEAN REVERSION: Chop > 60 OR RSI/MFI/CCI/Stoch is extreme. Strategy: Fade moves. Buy near Walls/S1/FVG Bottom. Sell near Walls/R1/FVG Top.
        3. NOISE/WAIT: Chop 40-60 OR ADX < 20. Strategy: HOLD.

        CRITICAL RULE: For any BUY/SELL signal, **ALL** active trend indicators (MTF, ST, Chande) must agree, *unless* Squeeze is active, which overrides all except the MTF trend.
...
function buildContext(d, analysis) {
    ...
    const atrVal = analysis.atr[last] || 1; 
    ...
    return {
        ...
        price: d.price,
        // Standard
        rsi: analysis.rsi[last]?.toFixed(2) || '0.00',
        stoch_k: analysis.stoch.k[last]?.toFixed(0) || '0',
        stoch_d: analysis.stoch.d[last]?.toFixed(0) || '0',
        cci: analysis.cci[last]?.toFixed(2) || '0.00',
        macd_hist: analysis.macd.hist[last]?.toFixed(4) || '0.0000',
        adx: analysis.adx[last]?.toFixed(2) || '0.00',
        // Advanced
        mfi: analysis.mfi[last]?.toFixed(2) || '0.00',
        chop: analysis.chop[last]?.toFixed(2) || '0.00',
        trend_angle: analysis.reg.slope[last]?.toFixed(4) || '0.0000',
        trend_quality: analysis.reg.r2[last]?.toFixed(2) || '0.00',
        trend_mtf: analysis.trendMTF,
        squeeze: analysis.isSqueeze,
        fvg: analysis.fvg,
        superTrend: analysis.st.trend[last] === 1 ? 'BULLISH' : 'BEARISH',
        chandelierExit: analysis.ce.trend[last] === 1 ? 'BULLISH' : 'BEARISH',
        // Levels
        walls: {
...
async function mainLoop() {
    try {
        ...
        logger.box(`WHALEWAVE TITAN | ${d.price.toFixed(2)}`, [
            `Regime: ${regimeColor(regimeTxt)} | MTF: ${ctx.trend_mtf === 'BULLISH' ? NEON.GREEN(ctx.trend_mtf) : NEON.RED(ctx.trend_mtf)}`,
