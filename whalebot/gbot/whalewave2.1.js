...
class UltraFastExchange {
    ...
    async evaluateUltraFast(priceVal, signal, analysis, ctx) {
        ...
        if (this.pos) {
            ...
            // 3. INSTANT BREAK-EVEN TRIGGER (Upgrade)
        ...
        // --- ENTRY LOGIC (UPGRADED) ---
            ...
            // 4. VOLATILITY CLAMPED TP (Upgrade)
            ...
            // Check Choppiness / Volatility from Context
            if (ctx.choppiness > 60) {
                riskMultiplier *= 0.6; // Reduce size in chop
            } else if (ctx.trendMTF === ctx.microTrend) {
                riskMultiplier *= 1.2; // Increase size in Trend Alignment
            }

            let riskPercent = this.cfg.riskPercent * riskMultiplier;
            if (this.consecutiveLosses > 0) riskPercent *= Math.pow(0.7, this.consecutiveLosses);
    ...
    async executeClose(price, reason) {
        ...
        console.log(`${COLORS.BOLD(reason)}! PnL: ${color(netPnL.toFixed(4))} [${this.pos.strategy}]`);
        this.pos = null;
    }
        ...
        try {
            ...
            if (loopCount % (config.delays.loop / 500) === 0 || signal) {
                ...
                if (exchange.pos) {
                    ...
                    const ratchetStatus = exchange.pos.sl !== exchange.pos.originalSl ? '🔒RATCHET' : '🔓OPEN';
