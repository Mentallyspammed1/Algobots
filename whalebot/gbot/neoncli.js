...
const helpMessages = {
    ...
    [CMD.READ]: "Read a file: /read <file_path>",
    [CMD.WRITE]: "Write to a file: /write <file_path> <content>",
    [CMD.EDIT]: "Edit a file: /edit <file_path> <search> <replace>"
};
...
async function processReadCommand(filePath) {
    if (!filePath) {
        logWarning(`Usage: ${CMD.READ} <file_path>`);
        return;
    }
    try {
...
async function processWriteCommand(args) {
    const match = args.match(/^(?:"([^"]+)"|'([^']+)'|(\S+))\s*(.*)$/s);

    if (!match) {
        logWarning(`Usage: ${CMD.WRITE} <file_path> <content>`);
        return;
    }
    ...
    try {
        ...
        logSystem(`File successfully written to ${filePath}`);
    } catch (error) {
        logError(`Error writing file: ${error.message}`);
    }
}

async function processEditCommand(args) {
    const match = args.match(/^(?:"([^"]+)"|'([^']+)'|(\S+))\s+(?:"([^"]+)"|'([^']+)'|(\S+))\s+(.*)$/s);

    if (!match) {
        logWarning(`Usage: ${CMD.EDIT} <file_path> <search> <replace>`);
        return;
    }

    const filePath = match[1] || match[2] || match[3];
    const search = match[4] || match[5] || match[6];
    const replace = match[7];
    ...
    try {
        ...
        logSystem(`File successfully edited: ${filePath}`);
    } catch (error) {
        logError(`Error editing file: ${error.message}`);
    }
}
        ...
        if (input.startsWith(CMD.PREFIX)) {
            ...
            switch (command) {
                ...
                case CMD.FILE.slice(1): case CMD.ALIAS.F.slice(1): case CMD.LOAD.slice(1): case CMD.ALIAS.L.slice(1):
                    ...
                    break;
                case CMD.READ.slice(1):
                    await processReadCommand(args);
                    break;
                case CMD.WRITE.slice(1):
                    await processWriteCommand(args);
                    break;
                case CMD.EDIT.slice(1):
                    await processEditCommand(args);
                    break;
