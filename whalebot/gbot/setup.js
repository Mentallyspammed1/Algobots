...
async function checkNodeVersion() {
    console.log(colors.info('ðŸ” Checking Node.js version...'));
    
    const nodeVersion = process.version;
    const requiredVersion = setupConfig.requiredNodeVersion;
    
    if (nodeVersion < requiredVersion) {
        console.error(colors.error(`âŒ Node.js ${requiredVersion}+ required. Current: ${nodeVersion}`));
        console.error(colors.info('ðŸ’¡ Update Node.js: https://nodejs.org/'));
        return false;
    }
    
    console.log(colors.success(`âœ… Node.js version: ${nodeVersion}`));
    return true;
}
...
async function checkRequiredFiles() {
    console.log(colors.info('ðŸ” Checking required files...'));
    
    for (const file of setupConfig.requiredFiles) {
        try {
            await fs.access(file);
            console.log(colors.success(`âœ… ${file} found`));
        } catch {
            console.error(colors.error(`âŒ ${file} missing`));
            return false;
        }
    }
...
async function checkEnvironment() {
    console.log(colors.info('ðŸ” Checking environment variables...'));
    
    // Check required variables
    let allValid = true;
    for (const envVar of setupConfig.requiredEnvVars) {
        if (!process.env[envVar]) {
            console.error(colors.error(`âŒ ${envVar} not set`));
            allValid = false;
        } else {
            console.log(colors.success(`âœ… ${envVar} configured`));
        }
    }
    ...
    for (const envVar of setupConfig.optionalEnvVars) {
        if (process.env[envVar]) {
            console.log(colors.info(`â„¹ï¸ ${envVar}: ${process.env[envVar]}`));
        }
    }
...
async function installDependencies() {
    console.log(colors.info('ðŸ“¦ Installing dependencies...'));
    ...
    try {
        ...
        try {
            ...
            console.log(colors.warning('âš ï¸ npm ci failed, trying npm install...'));
            execSync('npm install --silent', { stdio: 'inherit' });
        }
        
        console.log(colors.success('âœ… Dependencies installed successfully'));
        return true;
    } catch (error) {
        console.error(colors.error(`âŒ Failed to install dependencies: ${error.message}`));
        console.error(colors.info('ðŸ’¡ Try running: npm install'));
        return false;
    }
}
...
async function createConfigFile() {
    console.log(colors.info('âš™ï¸ Setting up configuration...'));
    
    const configPath = 'config.json';
    
    try {
        await fs.access(configPath);
        console.log(colors.warning('âš ï¸ config.json already exists'));
        const overwrite = await askQuestion('Do you want to overwrite it? (y/N): ');
        
        if (overwrite.toLowerCase() !== 'y') {
            console.log(colors.info('â„¹ï¸ Keeping existing configuration'));
            return true;
        }
    } catch {
    ...
    try {
        ...
        console.log(colors.success('âœ… Configuration file created'));
        console.log(colors.info('ðŸ’¡ Edit config.json to customize settings'));
        return true;
    } catch (error) {
        console.error(colors.error(`âŒ Failed to create config.json: ${error.message}`));
        return false;
    }
}
...
async function createEnvTemplate() {
    console.log(colors.info('ðŸ” Setting up environment...'));
...
`;
    ...
    try {
        ...
        console.log(colors.warning('âš ï¸ .env file already exists'));
        return true;
    } catch {
        try {
            await fs.writeFile(envPath, envTemplate);
            console.log(colors.success('âœ… Environment template created'));
            console.log(colors.warning('âš ï¸ Please edit .env and add your GEMINI_API_KEY'));
            return true;
        } catch (error) {
            console.error(colors.error(`âŒ Failed to create .env: ${error.message}`));
            return false;
        }
    }
}
...
async function main() {
    ...
    ðŸš€ WHALEWAVE TITAN v7.0 SETUP
    ==============================
    ...
    `)));
            ...
            if (start.toLowerCase() === 'y') {
                console.log(colors.info('ðŸš€ Starting WHALEWAVE TITAN...'));
                const { execSync } = await import('child_process');
