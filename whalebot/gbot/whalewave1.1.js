/**
 * 🌊 WHALEWAVE PRO - TITAN EDITION v4.0 (Final Integrated Strategy Arsenal)
 * ----------------------------------------------------------------------
 * - HYBRID MODEL: Quantitative (WSS) Pre-filter + Qualitative (Gemini) Strategy Selector.
 * - ARBITRARY PRECISION: All financial math uses decimal.js for ledger-grade accuracy.
 * - STRATEGIES: Trend Surfer, Breakout, Mean Reversion, Liquidity Grab, Divergence Hunter.
 * - ROBUSTNESS: API Retries (Exponential Backoff), Risk Circuit Breakers.
 */
...
class ConfigManager {
    ...
    static DEFAULTS = {
        ...
        indicators: {
            ...
            wss_weights: {
                trend_mtf_weight: 2.0, trend_scalp_weight: 1.5,
                extreme_rsi_mfi_weight: 1.0, extreme_stoch_weight: 0.5,
                momentum_regime_weight: 1.0, squeeze_vol_weight: 0.5,
                volatility_weight: 0.4, action_threshold: 1.1 // Higher WSS required
            }
        },
...
class TA {
    ...
    // [All 25+ Indicator Implementations (SMA, EMA, ATR, RSI, STOCH, MACD, ADX, MFI, CHOP, BB, KC, ST, CE, VWAP, FVG, LinReg, Volatility, Regime) are placed here] 
    // ... (Code from previous step is assumed to be fully present for full TA Library)
...
function getOrderbookLevels(bids, asks, currentClose, maxLevels) {
    ...
    let potentialSR = [];
    // Simplified SR based on volume imbalance
...
function calculateWSS(analysis) {
    ...
    if (rsi < 30) score += w.extreme_rsi_mfi_weight; else if (rsi > 70) score -= w.extreme_rsi_mfi_weight;
    const stoch_k = analysis.stoch.k[last];
    if (stoch_k < 20) score += w.extreme_stoch_weight; else if (stoch_k > 80) score -= w.extreme_stoch_weight;

    // Regime Alignment
    const chop = analysis.chop[last];
    const adx = analysis.adx[last];
    if (chop < 40 && adx > 25) score += (analysis.reg.slope[last] > 0 ? w.momentum_regime_weight : -w.momentum_regime_weight); // Momentum
    if (chop > 60 && adx < 20) score += (analysis.rsi[last] < 50 ? w.momentum_regime_weight : -w.momentum_regime_weight); // Mean Reversion bias
    ...
    if (volRatio > 1.5) score *= (1 - w.volatility_weight);
    if (volRatio < 0.5) score *= (1 + w.volatility_weight);
    
    return parseFloat(score.toFixed(2));
}

// --- 📡 ENHANCED DATA PROVIDER (from v3.0, verified) ---
...
// --- 💰 EXCHANGE & RISK MANAGEMENT (from v3.0, verified) ---
...
class EnhancedGeminiBrain {
    ...
    async analyze(ctx) {
        ...
        STRATEGY ARCHETYPES:
        1. TREND_SURFER: Trend is strong (ADX>25 & Slope>0.0001). Enter on pullback to VWAP/EMA.
        2. VOLATILITY_BREAKOUT: Squeeze is ACTIVE & ADX is rising. Anticipate explosion.
        3. MEAN_REVERSION: Ranging (Chop>60 or ADX<20). Fade RSI >70/<30 using BB/KC band as SL.
        4. LIQUIDITY_GRAB: Price hitting FVG or Orderbook Wall. Use the wall/FVG edge as entry.
        5. DIVERGENCE_HUNT: Clear RSI Divergence detected. Use local swing high/low as SL.

        INSTRUCTIONS:
        - If the WSS does not meet the threshold, return "HOLD".
...
class TradingEngine {
    ...
    async start() {
        console.clear();
        console.log(NEON.bg(NEON.PURPLE(` 🚀 WHALEWAVE TITAN v4.0 STARTING... `)));
    ...
    async performAnalysis(data) {
        ...
        const [rsi, stoch, macd, adx, mfi, chop, reg, bb, kc, atr, fvg, vwap, st, ce] = await Promise.all([
        ...
        const analysis = { 
            closes: c, rsi, stoch, macd, adx, mfi, chop, reg, bb, kc, atr, fvg, vwap, st, ce,
            isSqueeze, divergence, volatility, avgVolatility, trendMTF, buyWall, sellWall, fibs
        };
        analysis.wss = calculateWSS(analysis);
    ...
    buildContext(d, a) {
        ...
        return {
            price: d.price,
            rsi: a.rsi[last].toFixed(2), stoch_k: a.stoch.k[last].toFixed(0), macd_hist: a.macd.hist[last].toFixed(4),
            adx: a.adx[last].toFixed(2), chop: a.chop[last].toFixed(2),
            vwap: a.vwap[last].toFixed(2),
            trend_angle: linReg.slope, trend_mtf: a.trendMTF,
            isSqueeze: a.isSqueeze ? 'YES' : 'NO', fvg: a.fvg, divergence: a.divergence,
    ...
    displayDashboard(d, ctx, sig) {
        ...
        console.log(border);
        console.log(NEON.bg(NEON.BOLD(NEON.PURPLE(` WHALEWAVE TITAN v4.0 | ${config.symbol} | $${d.price.toFixed(4)} `).padEnd(80))));
        console.log(border);

        // Row 1: Strategy & Signal
        ...
        const ddCol = this.exchange.startBal.sub(this.exchange.balance).div(this.exchange.startBal).mul(100).gte(config.risk.max_drawdown * 0.5) ? NEON.ORANGE : NEON.GREEN;
        
        console.log(`Balance: ${NEON.GREEN('$' + this.exchange.balance.toFixed(2))} | Daily PnL: ${pnlCol('$' + this.exchange.dailyPnL.toFixed(2))} | DD Check: ${ddCol('OK')}`);
...
process.on('SIGINT', () => { 
    ...
    engine.exchange.handlePositionClose(new Decimal(engine.dataProvider.lastPrice || 0), "SHUTDOWN"); // Attempt force close
    process.exit(0); 
});
