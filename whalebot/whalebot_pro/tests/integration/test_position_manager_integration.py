...
async def test_sync_positions_from_exchange(position_manager, mock_bybit_client):
    mock_bybit_client.get_positions.return_value = [
        {"symbol": "BTCUSDT", "side": "Buy", "size": "0.01", "avgPrice": "40000", "stopLoss": "39000", "takeProfit": "41000", "positionIdx": 0}
    ]
...
@pytest.mark.asyncio
async def test_open_position_success(position_manager, mock_bybit_client, performance_tracker):
    ...
    mock_bybit_client.place_order.return_value = {"orderId": "test_order_id", "qty": "0.001", "avgPrice": "40000"}
    mock_bybit_client.set_trading_stop.return_value = True

    new_pos = await position_manager.open_position("Buy", Decimal("40000"), Decimal("100"))
...
@pytest.mark.asyncio
async def test_close_position_success(position_manager, mock_bybit_client, performance_tracker):
    ...
    mock_bybit_client.place_order.return_value = {"orderId": "close_order_id", "qty": "0.001", "avgPrice": "40050"}
    performance_tracker.record_trade = MagicMock()

    await position_manager.close_position(existing_pos, Decimal("40050"), performance_tracker)
    assert len(position_manager.open_positions) == 0
    mock_bybit_client.place_order.assert_called_once()
    performance_tracker.record_trade.assert_called_once()

@pytest.mark.asyncio
async def test_manage_positions_breakeven_activated(position_manager, mock_bybit_client, performance_tracker):
    ...
    mock_bybit_client.get_positions.return_value = [
        {"symbol": "BTCUSDT", "side": "Buy", "size": "0.001", "avgPrice": "40000", "stopLoss": "39000", "takeProfit": "42000", "positionIdx": 0}
    ]
    mock_bybit_client.set_trading_stop.return_value = True
...
@pytest.mark.asyncio
async def test_manage_positions_trailing_stop_activated(position_manager, mock_bybit_client, performance_tracker):
    ...
    mock_bybit_client.get_positions.return_value = [
        {"symbol": "BTCUSDT", "side": "Buy", "size": "0.001", "avgPrice": "40000", "stopLoss": "39000", "takeProfit": "45000", "positionIdx": 0}
    ]
    mock_bybit_client.set_trading_stop.return_value = True
    ...
    assert position_manager.open_positions[0]["trailing_stop_price"] == Decimal("40020.00")
