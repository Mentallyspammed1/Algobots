...
async def test_bybit_client_initialize(bybit_client):
    ...
    bybit_client.precision_manager.load_instrument_info.assert_called_once_with(bybit_client.symbol)
...
async def test_fetch_klines_ws_preferred(bybit_client):
    mock_df = pd.DataFrame({'close': [1,2,3]}, index=pd.to_datetime(['2023-01-01','2023-01-02','2023-01-03']))
...
async def test_place_order_success(bybit_client):
    ...
    bybit_client.http_session.place_order.return_value = {"retCode": 0, "result": {"orderId": "123"}}
...
async def test_set_trading_stop_success(bybit_client):
    ...
    bybit_client.http_session.set_trading_stop.return_value = {"retCode": 0, "result": {}}
...
async def test_ws_kline_message_processing(bybit_client):
    message = {
        ...
        "data": [{
            "start": "1672531200000", "open": "10", "high": "12", "low": "9", "close": "11", "volume": "100", "turnover": "1000"
        }]
    }
...
async def test_ws_ticker_message_processing(bybit_client):
    message = {
        ...
        "data": {"symbol": "BTCUSDT", "lastPrice": "40000.10", "bid1Price": "40000", "ask1Price": "40000.20"}
    }
...
async def test_ws_orderbook_message_processing(bybit_client):
    ...
    snapshot_message = {"type": "snapshot", "topic": "orderbook.50.BTCUSDT", "data": {"b":[], "a":[], "u":1}}
    ...
    delta_message = {"type": "delta", "topic": "orderbook.50.BTCUSDT", "data": {"b":[], "a":[], "u":2}}
