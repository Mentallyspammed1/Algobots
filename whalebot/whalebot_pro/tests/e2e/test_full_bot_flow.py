...
def mock_config():
    ...
    cfg.strategy_profiles = {"default_scalping": {"indicators_enabled": {},
                                                  "weights": {}}}
...
def mock_bybit_client(mock_config):
    ...
    client.precision_manager.round_qty.side_effect = lambda qty, sym: qty.quantize(Decimal("0.001"))
    client.precision_manager.round_price.side_effect = lambda price, sym: price.quantize(Decimal("0.01"))
...
@pytest.fixture
def bot(mock_config, mock_logger, mock_bybit_client, mock_kline_data_fetcher, mock_in_memory_cache):
    ...
    with patch('whalebot_pro.main.BybitClient', return_value=mock_bybit_client):
        with patch('whalebot_pro.main.PositionManager') as MockPositionManager:
            with patch('whalebot_pro.main.PerformanceTracker') as MockPerformanceTracker:
                with patch('whalebot_pro.main.IndicatorCalculator') as MockIndicatorCalculator:
                    with patch('whalebot_pro.main.TradingAnalyzer') as MockTradingAnalyzer:
                        with patch('whalebot_pro.main.KlineDataFetcher', return_value=mock_kline_data_fetcher):
                            with patch('whalebot_pro.main.InMemoryCache', return_value=mock_in_memory_cache):
                                ...
                                MockTradingAnalyzer.return_value.df = pd.DataFrame() # Ensure analyzer.df is not empty initially
                                MockTradingAnalyzer.return_value._get_indicator_value.return_value = Decimal("100") # Mock ATR
                                MockTradingAnalyzer.return_value.assess_market_conditions.return_value = {"adx_value": 30, "volatility_index_value": 0.01}
                                MockTradingAnalyzer.return_value.generate_trading_signal.return_value = ("HOLD", 0.0, {})
...
async def test_bot_start_and_shutdown(bot, mock_bybit_client, mock_logger):
    ...
    mock_logger.info.assert_any_call(f"{bot.logger.handlers[0].formatter.NEON_GREEN}--- Whalebot Trading Bot Shut Down ---{bot.logger.handlers[0].formatter.RESET}")
...
async def test_trading_loop_fetch_price_failure(bot, mock_bybit_client, mock_logger):
    ...
    mock_logger.warning.assert_called_once_with(f"\x1b[33mALERT: [{bot.config.symbol}] Failed to fetch current price. Skipping loop.\x1b[39m")
    mock_bybit_client.fetch_current_price.assert_called_once()

@pytest.mark.asyncio
async def test_trading_loop_fetch_klines_failure(bot, mock_bybit_client, mock_logger, mock_kline_data_fetcher):
    ...
    mock_logger.warning.assert_called_once_with(f"\x1b[33mALERT: [{bot.config.symbol}] Failed to fetch primary klines or DataFrame is empty. Skipping loop.\x1b[39m")
    mock_kline_data_fetcher.fetch_klines.assert_called_once()

@pytest.mark.asyncio
async def test_trading_loop_buy_signal_open_position(bot, mock_bybit_client, mock_logger, mock_kline_data_fetcher, mock_in_memory_cache):
    ...
    bot.position_manager.open_position.assert_called_once_with("Buy", Decimal("40000"), Decimal("100"))
    mock_logger.info.assert_any_call(f"\x1b[32m[{bot.config.symbol}] Strong BUY signal detected! Score: 3.00\x1b[39m")

@pytest.mark.asyncio
async def test_trading_loop_sell_signal_close_and_reverse(bot, mock_bybit_client, mock_logger, mock_kline_data_fetcher, mock_in_memory_cache):
    ...
    bot.position_manager.open_position.assert_called_once_with("Sell", Decimal("39000"), Decimal("100"))
    mock_logger.info.assert_any_call(f"\x1b[91m[{bot.config.symbol}] Strong SELL signal detected! Score: -3.00\x1b[39m")
