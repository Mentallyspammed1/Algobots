...
def bybit_client_for_performance(mock_config, mock_logger):
    client = BybitClient(mock_config.BYBIT_API_KEY, mock_config.BYBIT_API_SECRET, mock_config.get_config(), mock_logger)
    client.http_session = AsyncMock() # Mock the actual HTTP session
    client.http_session.get_tickers.return_value = {"retCode": 0, "result": {"list": [{"lastPrice": "40000"}]}}
    client.http_session.get_kline.return_value = {"retCode": 0, "result": {"list": [["1672531200000", "10", "12", "9", "11", "100", "1000"]]}}
    client.http_session.get_orderbook.return_value = {"retCode": 0, "result": {"b":[["40000", "10"]], "a":[["40001", "10"]]}}
    client.http_session.place_order.return_value = {"retCode": 0, "result": {"orderId": "123"}}
    client.http_session.set_trading_stop.return_value = {"retCode": 0, "result": {}}
    client.http_session.get_wallet_balance.return_value = {"retCode": 0, "result": {"list": [{"coin": [{"coin": "USDT", "walletBalance": "1000"}]}]}}
    client.http_session.get_positions.return_value = {"retCode": 0, "result": {"list": []}}
...
async def test_concurrent_api_calls_performance(bybit_client_for_performance):
    ...
    bybit_client_for_performance.logger.info(f"Performed {total_mocked_calls} mocked API calls in {duration:.4f} seconds.")
    ...
    assert bybit_client_for_performance.http_session.get_orderbook.call_count == num_calls
...
async def test_api_call_retry_delay_simulation(bybit_client_for_performance):
    ...
    bybit_client_for_performance.http_session.get_tickers.side_effect = [
        ...
        {"retCode": 0, "result": {"list": [{"lastPrice": "40000"}]}} # Second call succeeds
    ]
