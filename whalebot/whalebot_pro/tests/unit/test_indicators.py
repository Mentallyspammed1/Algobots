...
def sample_df():
    ...
    data = {
        'open': [10, 12, 15, 13, 16, 18, 17, 20, 22, 21, 25, 24, 23, 26, 28, 27, 30, 29, 32, 31, 35, 34, 33, 36, 38, 37, 40, 39, 42, 41],
        'high': [12, 15, 17, 16, 18, 20, 20, 23, 25, 24, 28, 27, 26, 29, 30, 30, 33, 32, 35, 34, 38, 37, 36, 39, 40, 40, 43, 42, 45, 44],
        'low': [9, 11, 13, 12, 14, 16, 15, 18, 20, 19, 23, 22, 21, 24, 26, 25, 28, 27, 30, 29, 32, 31, 30, 33, 35, 34, 37, 36, 39, 38],
        'close': [11, 14, 16, 14, 17, 19, 18, 21, 24, 20, 27, 23, 22, 28, 29, 26, 31, 30, 33, 32, 36, 35, 34, 37, 39, 36, 41, 40, 43, 42],
        'volume': [100, 110, 120, 105, 115, 130, 125, 140, 150, 135, 160, 145, 130, 170, 180, 165, 190, 175, 200, 185, 210, 195, 180, 220, 230, 215, 240, 225, 250, 235],
    }
    df = pd.DataFrame(data, index=pd.to_datetime(pd.date_range(start='2023-01-01', periods=30, freq='D')))
    return df
...
def test_calculate_true_range(indicator_calculator, sample_df):
    ...
    assert tr.iloc[-1] == pytest.approx(5.0) # (High-Low) = 45-38 = 7, (High-PrevClose) = 45-43 = 2, (Low-PrevClose) = 38-43 = 5. Max is 7.
...
def test_calculate_super_smoother(indicator_calculator, sample_df):
    smoothed = indicator_calculator.calculate_super_smoother(sample_df["close"], period=10)
...
def test_calculate_ehlers_supertrend(indicator_calculator, sample_df):
    st_result = indicator_calculator.calculate_ehlers_supertrend(sample_df, period=10, multiplier=2.0)
...
def test_calculate_macd(indicator_calculator, sample_df):
    macd_line, signal_line, histogram = indicator_calculator.calculate_macd(sample_df, fast_period=12, slow_period=26, signal_period=9)
...
def test_calculate_stoch_rsi(indicator_calculator, sample_df):
    stoch_k, stoch_d = indicator_calculator.calculate_stoch_rsi(sample_df, period=14, k_period=3, d_period=3)
...
def test_calculate_bollinger_bands(indicator_calculator, sample_df):
    upper, middle, lower = indicator_calculator.calculate_bollinger_bands(sample_df, period=20, std_dev=2.0)
...
def test_calculate_ichimoku_cloud(indicator_calculator, sample_df):
    tenkan, kijun, span_a, span_b, chikou = indicator_calculator.calculate_ichimoku_cloud(
        sample_df, tenkan_period=9, kijun_period=26, senkou_span_b_period=52, chikou_span_offset=26
    )
...
def test_calculate_psar(indicator_calculator, sample_df):
    psar_val, psar_dir = indicator_calculator.calculate_psar(sample_df, acceleration=0.02, max_acceleration=0.2)
...
def test_calculate_kaufman_ama(indicator_calculator, sample_df):
    kama = indicator_calculator.calculate_kaufman_ama(sample_df, period=10, fast_period=2, slow_period=30)
...
def test_calculate_keltner_channels(indicator_calculator, sample_df):
    # ATR is needed for this, so calculate it first
    sample_df["ATR"] = indicator_calculator.calculate_atr(sample_df, period=14)
    upper, middle, lower = indicator_calculator.calculate_keltner_channels(sample_df, period=20, atr_multiplier=2.0)
    assert isinstance(upper, pd.Series)
    assert isinstance(middle, pd.Series)
    assert isinstance(lower, pd.Series)
    assert not upper.isnull().all()
...
def test_detect_candlestick_patterns(indicator_calculator, sample_df):
    ...
    assert pattern in ["Bullish Engulfing", "Bearish Engulfing", "Bullish Hammer", "Bearish Shooting Star", "No Pattern"]
...
def test_calculate_support_resistance_from_orderbook(indicator_calculator):
    ...
    support, resistance = indicator_calculator.calculate_support_resistance_from_orderbook(bids, asks)
