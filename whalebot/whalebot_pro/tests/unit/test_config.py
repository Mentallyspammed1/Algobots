...
from whalebot_pro.config import Config, DEFAULT_CONFIG_FILE
...
def test_config_loading_existing(tmp_path, mock_logger):
    ...
    with patch('whalebot_pro.config.Path') as MockPath:
        ...
        MockPath.return_value.open = mock_open(read_data=json.dumps(dummy_config_content))
...
def test_ensure_config_keys_new_defaults(tmp_path, mock_logger):
    ...
    with patch('whalebot_pro.config.Path') as MockPath:
        ...
        assert config.trade_management["slippage_percent"] == 0.001 # New nested default

# Test __getattr__ for config access
def test_config_getattr(mock_logger):
    config_data = {
        "test_key": "test_value",
        "nested": {"nested_key": 123}
    }
    with patch('whalebot_pro.config.Path') as MockPath:
