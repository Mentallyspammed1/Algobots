...
async def test_load_instrument_info_success(precision_manager, mock_bybit_client):
    mock_bybit_client._bybit_request_with_retry.return_value = {
        ...
        "result": {
            "list": [{
                ...
                "lotSizeFilter": {"qtyStep": "0.001", "minOrderQty": "0.001", "maxOrderQty": "100", "minNotionalValue": "10"}
            }]
        }
    }
    ...
    assert precision_manager.instruments_info["BTCUSDT"]["price_precision_decimal"] == Decimal("0.01")
    assert precision_manager.instruments_info["BTCUSDT"]["qty_precision_decimal"] == Decimal("0.001")
    ...
    assert precision_manager.instruments_info["BTCUSDT"]["min_notional"] == Decimal("10")
...
def test_round_price(precision_manager):
    ...
    precision_manager.instruments_info["BTCUSDT"] = {
        ...
        'min_qty': Decimal("0.001"), 'max_qty': Decimal("100"), 'min_notional': Decimal("10")
    }
    
    assert precision_manager.round_price(Decimal("123.4567"), "BTCUSDT") == Decimal("123.46")
    assert precision_manager.round_price(Decimal("123.453"), "BTCUSDT") == Decimal("123.45")
...
def test_round_qty(precision_manager):
    ...
    assert precision_manager.round_qty(Decimal("0.12345"), "BTCUSDT") == Decimal("0.123")
    assert precision_manager.round_qty(Decimal("0.0009"), "BTCUSDT") == Decimal("0.000") # Should round down
