...
from whalebot_pro.utils.logger_setup import setup_logging, LOG_DIRECTORY
...
def test_basic_logging_setup(mock_path_mkdir, mock_os_path_join):
    with patch.dict(sys.modules, {'unanimous_logger': None}): # Simulate unanimous_logger not found
        ...
        assert any(isinstance(h, logging.FileHandler) and h.baseFilename == f"{LOG_DIRECTORY}/test_basic.log" for h in logger.handlers)
...
def test_logging_handlers_not_duplicated(mock_path_mkdir, mock_os_path_join):
    with patch.dict(sys.modules, {'unanimous_logger': None}):
        ...
        assert len(logger.handlers) == initial_handler_count # Handlers should not increase
