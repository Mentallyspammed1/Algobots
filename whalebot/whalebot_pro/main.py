...
from whalebot_pro.analysis.trading_analyzer import TradingAnalyzer, display_indicator_values_and_price, fetch_latest_sentiment
from whalebot_pro.utils.utilities import KlineDataFetcher, InMemoryCache
...
class BybitTradingBot:
    ...
    def __init__(self, config: Config):
        ...
        self.position_manager = PositionManager(self.config.get_config(), self.logger, self.bybit_client)
        self.performance_tracker = PerformanceTracker(self.logger, self.config.get_config())
        self.indicator_calculator = IndicatorCalculator(self.logger)
        self.analyzer = TradingAnalyzer(self.config.get_config(), self.logger, self.config.symbol, self.indicator_calculator)
        self.kline_data_fetcher = KlineDataFetcher(self.bybit_client, self.logger, self.config.get_config())
        self.kline_cache = InMemoryCache(ttl_seconds=self.config.loop_delay * 0.8, max_size=5)
    ...
    async def start(self):
        ...
        for htf_interval in self.config.mtf_analysis["higher_timeframes"]:
            if htf_interval not in valid_bybit_intervals:
                self.logger.error(f"{NEON_RED}Invalid higher timeframe interval '{htf_interval}'. Exiting.{RESET}")
                sys.exit(1)
        ...
        try:
            ...
            self.logger.info(f"{NEON_YELLOW}Bot stopping due to KeyboardInterrupt.{RESET}")
        except Exception as e:
            self.alert_system.send_alert(
                f"[{self.config.symbol}] An unhandled error occurred in the main loop: {e}", "ERROR"
            )
            self.logger.exception(f"{NEON_RED}[{self.config.symbol}] Unhandled exception in main loop:{RESET}")
            await asyncio.sleep(self.config.loop_delay * 2)
        finally:
            await self.bybit_client.stop_ws()
            self.logger.info(f"{NEON_GREEN}--- Whalebot Trading Bot Shut Down ---{RESET}")
    ...
    async def _trading_loop(self):
        ...
        if current_price is None:
            self.alert_system.send_alert(
                f"[{self.config.symbol}] Failed to fetch current price. Skipping loop.", "WARNING"
            )
            await asyncio.sleep(self.config.loop_delay)
            return

        kline_cache_key = self.kline_cache.generate_kline_cache_key(
            self.config.symbol, self.config.category, self.config.interval,
        ...
        if self.config.ml_enhancement.get("sentiment_analysis_enabled", False):
            sentiment_score = await fetch_latest_sentiment(self.config.symbol, self.logger)
        ...
        if self.config.adaptive_strategy_enabled:
            ...
            for profile_name, profile_details in self.config.strategy_profiles.items():
                ...
                adx_match = True
                if "adx_range" in criteria and market_conditions["adx_value"] is not None and not pd.isna(market_conditions["adx_value"]):
                    adx_min, adx_max = criteria["adx_range"]
                    if not (adx_min <= market_conditions["adx_value"] <= adx_max):
                        adx_match = False

                vol_match = True
                if "volatility_range" in criteria and market_conditions["volatility_index_value"] is not None and not pd.isna(market_conditions["volatility_index_value"]):
                ...
                if adx_match and vol_match:
                    if profile_name != suggested_strategy: # Check against suggested, not current_strategy_profile directly
            ...
            if suggested_strategy != self.config.current_strategy_profile:
                ...
                self.config.set_active_strategy_profile(suggested_strategy) # Update config object
                self.analyzer.config = self.config.get_config() # Update analyzer's config
                self.analyzer.weights = self.config.active_weights # Update analyzer's weights

        atr_value = Decimal(str(self.analyzer._get_indicator_value("ATR", Decimal("0.0001"))))
        if atr_value <= 0:
        ...
        trading_signal, signal_score, signal_breakdown = await self.analyzer.generate_trading_signal(
            current_price, self.bybit_client.orderbook_manager, mtf_trends, sentiment_score
        )

        await self.position_manager.manage_positions(current_price, self.performance_tracker, atr_value)

        await display_indicator_values_and_price(
            self.config.get_config(), self.logger, current_price, df, self.bybit_client.orderbook_manager, mtf_trends, signal_breakdown, self.indicator_calculator
        )

        signal_threshold = self.config.signal_score_threshold

        has_buy_position = any(p["side"].upper() == "BUY" for p in self.position_manager.get_open_positions())
        has_sell_position = any(p["side"].upper() == "SELL" for p in self.position_manager.get_open_positions())
        ...
        if trading_signal == "BUY" and signal_score >= signal_threshold:
            ...
            if has_sell_position:
                if self.config.trade_management["close_on_opposite_signal"]:
                    ...
                    sell_pos = next(p for p in self.position_manager.get_open_positions() if p["side"].upper() == "SELL")
                    await self.position_manager.close_position(
                        sell_pos, current_price, self.performance_tracker, closed_by="OPPOSITE_SIGNAL"
                    )
                    if self.config.trade_management["reverse_position_on_opposite_signal"]:
                        self.logger.info(
                            f"{NEON_GREEN}[{self.config.symbol}] Reversing position: Opening new BUY position after closing SELL.{RESET}"
                        )
                        await self.position_manager.open_position("Buy", current_price, atr_value)
                else:
                    self.logger.info(
            ...
            if has_buy_position:
                if self.config.trade_management["close_on_opposite_signal"]:
                    ...
                    buy_pos = next(p for p in self.position_manager.get_open_positions() if p["side"].upper() == "BUY")
                    await self.position_manager.close_position(
                        buy_pos, current_price, self.performance_tracker, closed_by="OPPOSITE_SIGNAL"
                    )
                    if self.config.trade_management["reverse_position_on_opposite_signal"]:
                        self.logger.info(
                            f"{NEON_RED}[{self.config.symbol}] Reversing position: Opening new SELL position after closing BUY.{RESET}"
                        )
                        await self.position_manager.open_position("Sell", current_price, atr_value)
                else:
                    self.logger.info(
        ...
        if open_positions:
            self.logger.info(f"{NEON_CYAN}[{self.config.symbol}] Open Positions: {len(open_positions)}{RESET}")
            ...
            self.logger.info(f"{NEON_CYAN}[{self.config.symbol}] No open positions.{RESET}")
