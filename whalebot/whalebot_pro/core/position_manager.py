...
TIMEZONE = timezone.utc # Default, will be overwritten by config

class PositionManager:
    """Manages open positions, stop-loss, and take-profit levels."""

    def __init__(self, config: dict[str, Any], logger: logging.Logger, bybit_client: BybitClient):
    ...
    async def _get_current_balance(self) -> Decimal:
        ...
        if balance is None:
            self.logger.warning(f"{NEON_YELLOW}Could not fetch real account balance. Using configured balance for simulation.{RESET}")
    ...
    async def _calculate_order_size(
        ...
        if order_qty < min_qty:
            self.logger.warning(f"{NEON_YELLOW}[{self.symbol}] Calculated order quantity {order_qty} is less than min_qty {min_qty}. Adjusting to min_qty.{RESET}")
            order_qty = min_qty
    ...
    async def sync_positions_from_exchange(self):
        ...
        for ex_pos in exchange_positions:
            ...
            if existing_pos:
                existing_pos.update(
                    {
                        "entry_price": self.precision_manager.round_price(entry_price, self.symbol),
                        "qty": self.precision_manager.round_qty(qty, self.symbol),
                        "stop_loss": self.precision_manager.round_price(stop_loss_price, self.symbol),
                        "take_profit": self.precision_manager.round_price(take_profit_price, self.symbol),
                        "trailing_stop_price": self.precision_manager.round_price(trailing_stop, self.symbol) if trailing_stop else None,
                ...
                new_open_positions.append(
                    {
                        ...
                        "initial_stop_loss": self.precision_manager.round_price(stop_loss_price, self.symbol),
                        "trailing_stop_activated": trailing_stop > 0 if self.enable_trailing_stop else False,
                        "trailing_stop_price": self.precision_manager.round_price(trailing_stop, self.symbol) if trailing_stop else None,
    ...
    async def open_position(
        ...
        initial_stop_loss = self.precision_manager.round_price(initial_stop_loss, self.symbol)
        ...
        new_position = {
            ...
            "symbol": self.symbol,
            "side": signal_side,
            "entry_price": self.precision_manager.round_price(filled_price, self.symbol),
    ...
    async def close_position(
        self, position: dict, current_price: Decimal, performance_tracker: PerformanceTracker, closed_by: str = "SIGNAL"
    ) -> None:
        ...
        if order_result:
            ...
            performance_tracker.record_trade(
                {**position, "exit_price": exit_price, "exit_time": datetime.now(self.bybit_client.timezone), "closed_by": closed_by},
                pnl,
            )
    ...
    async def manage_positions(
        self, current_price: Decimal, performance_tracker: PerformanceTracker, atr_value: Decimal
    ) -> None:
        """
        ...
        for position in current_internal_positions:
            ...
            if not latest_pos_from_sync:
                ...
                performance_tracker.record_trade(
                    {
                        **position,
                        "exit_price": self.precision_manager.round_price(close_price, self.symbol),
            ...
            if atr_value > 0:
                ...
                if (
                    ...
                    if side == "Buy":
                        potential_sl_update = max(current_stop_loss_on_exchange, breakeven_sl)
                    else:
                        potential_sl_update = min(current_stop_loss_on_exchange, breakeven_sl)
            ...
            if self.enable_trailing_stop and atr_value > 0:
                ...
                if (
                    (side == "Buy" and current_price >= profit_trigger_level) or
                    ...
                    if side == "Buy":
                        if new_trailing_stop_candidate > current_stop_loss_on_exchange:
                            proposed_sl = max(new_trailing_stop_candidate, position["initial_stop_loss"])
                            if potential_sl_update:
                        ...
                        if new_trailing_stop_candidate < current_stop_loss_on_exchange:
                            proposed_sl = min(new_trailing_stop_candidate, position["initial_stop_loss"])
                            if potential_sl_update:
            ...
            if potential_sl_update is not None and potential_sl_update != current_stop_loss_on_exchange:
                ...
                potential_sl_update = self.precision_manager.round_price(potential_sl_update, self.symbol)
