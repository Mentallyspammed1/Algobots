...
"""
Trading Bot – Refactored & Enhanced (Improved Edition)

This module implements a high‑frequency crypto‑trading bot that:
* Pulls market data from Bybit (v5 API)
...
- Fix BybitClient._send_signed_request usage and endpoint method calls.
- Robust NaN/empty handling across indicators and MTF analysis.
- Better logging and config validation.
- Request pacing and retries for MTF.
- Graceful fallbacks for ATR and other edge cases.
"""
...
# --------------------------------------------------------------------------- #
# Helper Decorators & Logging Formatter
# --------------------------------------------------------------------------- #
class SensitiveFormatter(logging.Formatter):
    """Formatter that scrubs secrets like API keys and signatures from logs."""
    SENSITIVE_KEYS = {"X-BAPI-API-KEY", "X-BAPI-SIGN", "api_key", "api_secret", "token", "password", "secret"}

    def format(self, record: logging.LogRecord) -> str:
        msg = super().format(record)
        # Scrub from URL/query parameters in debug messages
        try:
            if record.levelno <= logging.DEBUG:
                # Remove auth headers if present
                if hasattr(record, "args") and record.args:
                    # Not always safe to touch args; keep simple
                    pass
        except Exception:
            pass

        # Scrub sensitive tokens by key
        def scrub(s: str) -> str:
            if not isinstance(s, str):
                return s
            for k in self.SENSITIVE_KEYS:
                if k in s:
                    s = s.replace(k + "=", k + "=<REDACTED>")
            return s
...
class Config:
    ...
    ml_enhancement: Dict[str, Any] = field(
...
def load_config(filepath: str, logger: logging.Logger) -> Config:
    ...
    try:
        ...
        cfg = Config(
            ...
            ml_enhancement=raw.get("ml_enhancement", {}),
...
class TradingAnalyzer:
    ...
    def _calculate_all_indicators(self) -> None:
        ...
        self.indicator_values["SMA_10"] = self._safe_last(self.df["SMA_10"])
        self.indicator_values["SMA_Long"] = self._safe_last(self.df["SMA_Long"])
        ...
        self.indicator_values["EMA_Short"] = self._safe_last(ema_short)
        self.indicator_values["EMA_Long"] = self._safe_last(ema_long)

        # ATR
        tr = self._calculate_true_range()
        atr = tr.ewm(span=isd.atr_period, adjust=False).mean()
        self.df["ATR"] = atr
        self.indicator_values["ATR"] = self._safe_last(atr, default=np.nan)

        # RSI
        rsi = self.calculate_rsi(isd.rsi_period)
        self.df["RSI"] = rsi
        self.indicator_values["RSI"] = self._safe_last(rsi)
        ...
        self.indicator_values["StochRSI_K"] = self._safe_last(stoch_k)
        self.indicator_values["StochRSI_D"] = self._safe_last(stoch_d)
        ...
        self.indicator_values["BB_Upper"] = self._safe_last(bb_upper)
        self.indicator_values["BB_Middle"] = self._safe_last(bb_middle)
        self.indicator_values["BB_Lower"] = self._safe_last(bb_lower)

        # CCI
        cci = self.calculate_cci(isd.cci_period)
        self.df["CCI"] = cci
        self.indicator_values["CCI"] = self._safe_last(cci)

        # Williams %R
        wr = self.calculate_williams_r(isd.williams_r_period)
        self.df["WR"] = wr
        self.indicator_values["WR"] = self._safe_last(wr)

        # MFI
        mfi = self.calculate_mfi(isd.mfi_period)
        self.df["MFI"] = mfi
        self.indicator_values["MFI"] = self._safe_last(mfi)

        # OBV
        obv, obv_ema = self.calculate_obv(isd.obv_ema_period)
        self.df["OBV"] = obv
        self.df["OBV_EMA"] = obv_ema
        self.indicator_values["OBV"] = self._safe_last(obv)
        self.indicator_values["OBV_EMA"] = self._safe_last(obv_ema)

        # CMF
        cmf = self.calculate_cmf(isd.cmf_period)
        self.df["CMF"] = cmf
        self.indicator_values["CMF"] = self._safe_last(cmf)
        ...
        self.indicator_values["Tenkan_Sen"] = self._safe_last(tenkan)
        self.indicator_values["Kijun_Sen"] = self._safe_last(kijun)
        self.indicator_values["Senkou_Span_A"] = self._safe_last(senkou_a)
        self.indicator_values["Senkou_Span_B"] = self._safe_last(senkou_b)
        self.indicator_values["Chikou_Span"] = self._safe_last(chikou)
        ...
        self.indicator_values["PSAR_Val"] = self._safe_last(psar_val)
        self.indicator_values["PSAR_Dir"] = self._safe_last(psar_dir)

        # VWAP
        vwap = self.calculate_vwap()
        self.df["VWAP"] = vwap
        self.indicator_values["VWAP"] = self._safe_last(vwap)
        ...
        self.indicator_values["MACD_Line"] = self._safe_last(macd_line)
        self.indicator_values["MACD_Signal"] = self._safe_last(macd_signal)
        self.indicator_values["MACD_Hist"] = self._safe_last(macd_hist)
        ...
        self.indicator_values["ADX"] = self._safe_last(adx)
        self.indicator_values["PlusDI"] = self._safe_last(plus_di)
        self.indicator_values["MinusDI"] = self._safe_last(minus_di)

        # Volatility Index
        vol_index = self.calculate_volatility_index(isd.volatility_index_period)
        self.df["Volatility_Index"] = vol_index
        self.indicator_values["Volatility_Index"] = self._safe_last(vol_index)

        # VWMA
        vwma = self.calculate_vwma(isd.vwma_period)
        self.df["VWMA"] = vwma
        self.indicator_values["VWMA"] = self._safe_last(vwma)

        # Volume Delta
        vol_delta = self.calculate_volume_delta(isd.volume_delta_period)
        self.df["Volume_Delta"] = vol_delta
        self.indicator_values["Volume_Delta"] = self._safe_last(vol_delta)
        ...
        self.indicator_values["Kaufman_AMA"] = self._safe_last(kama)

        # Relative Volume
        rel_vol = self.calculate_relative_volume(isd.relative_volume_period)
        self.df["Relative_Volume"] = rel_vol
        self.indicator_values["Relative_Volume"] = self._safe_last(rel_vol)
        ...
        self.indicator_values["Market_Structure_Trend"] = market_struct.iloc[-1] if not market_struct.empty else "UNKNOWN"

        # DEMA
        dema = self.calculate_dema(self.df["close"], isd.dema_period)
        self.df["DEMA"] = dema
        self.indicator_values["DEMA"] = self._safe_last(dema)
        ...
        self.indicator_values["Keltner_Upper"] = self._safe_last(keltner_upper)
        self.indicator_values["Keltner_Middle"] = self._safe_last(keltner_middle)
        self.indicator_values["Keltner_Lower"] = self._safe_last(keltner_lower)

        # ROC
        roc = self.calculate_roc(isd.roc_period)
        self.df["ROC"] = roc
        self.indicator_values["ROC"] = self._safe_last(roc)
        ...
        # Ehlers SuperTrend (fast and slow for alignment scoring)
        st_fast = self.calculate_ehlers_supertrend(isd.ehlers_fast_period, isd.ehlers_fast_multiplier)
        if st_fast is not None:
            self.df["ST_Fast_Dir"] = st_fast["direction"]
            self.df["ST_Fast_Val"] = st_fast["supertrend"]
            self.indicator_values["ST_Fast_Dir"] = self._safe_last(st_fast["direction"])
            self.indicator_values["ST_Fast_Val"] = self._safe_last(st_fast["supertrend"])

        st_slow = self.calculate_ehlers_supertrend(isd.ehlers_slow_period, isd.ehlers_slow_multiplier)
        if st_slow is not None:
            self.df["ST_Slow_Dir"] = st_slow["direction"]
            self.df["ST_Slow_Val"] = st_slow["supertrend"]
            self.indicator_values["ST_Slow_Dir"] = self._safe_last(st_slow["direction"])
            self.indicator_values["ST_Slow_Val"] = self._safe_last(st_slow["supertrend"])
    ...
    # Helpers
    # ----------------------------------------------------------------------- #
    def _safe_last(self, series: pd.Series, default: Any = np.nan) -> Any:
    ...
    def calculate_super_smoother(self, series: pd.Series, period: int) -> pd.Series:
        ...
        filt = pd.Series(0.0, index=self.df.index)
        if len(series) >= 1:
    ...
    def calculate_volatility_index(self, period: int) -> pd.Series:
        if "ATR" not in self.df.columns:
            ...
            atr = self.df["ATR"]
        normalized_atr = atr / self.df["close"]
    ...
    # ----------------------------------------------------------------------- #
    # Ichimoku + Fibonacci + Support/Resistance helpers
    # ----------------------------------------------------------------------- #
    def calculate_ichimoku_cloud(
        ...
        tenkan = (self.df["high"].rolling(window=tenkan_period).max() + self.df["low"].rolling(window=tenkan_period).min()) / 2
        kijun = (self.df["high"].rolling(window=kijun_period).max() + self.df["low"].rolling(window=kijun_period).min()) / 2
        senkou_a = ((tenkan + kijun) / 2).shift(kijun_period)
        senkou_b = ((self.df["high"].rolling(window=senkou_span_b_period).max() + self.df["low"].rolling(window=senkou_span_b_period).min()) / 2).shift(kijun_period)
        chikou = self.df["close"].shift(-chikou_span_offset)
    ...
    def calculate_fibonacci_levels(self) -> None:
        ...
        if len(self.df) < window:
            self.logger.debug(
                f"[{self.symbol}] Fibonacci levels: not enough data. Need {window}, have {len(self.df)}."
            )
            return
        ...
        levels = {
            "0.0%": Decimal(str(low_val)),
            "23.6%": Decimal(str(low_val + diff * Decimal("0.236"))),
            "38.2%": Decimal(str(low_val + diff * Decimal("0.382"))),
            "50.0%": Decimal(str(low_val + diff * Decimal("0.5"))),
            "61.8%": Decimal(str(low_val + diff * Decimal("0.618"))),
            "78.6%": Decimal(str(low_val + diff * Decimal("0.786"))),
            "100.0%": Decimal(str(high_val)),
        }
    ...
    def calculate_fibonacci_pivot_points(self) -> None:
        ...
        r1 = pivot + (pivot - prev["low"])
        s1 = pivot - (prev["high"] - pivot)
        r2 = pivot + (prev["high"] - prev["low"])
        s2 = pivot - (prev["high"] - prev["low"])
        self.indicator_values["Pivot"] = pivot
        self.indicator_values["R1"] = r1
        self.indicator_values["R2"] = r2
        self.indicator_values["S1"] = s1
        self.indicator_values["S2"] = s2
    ...
    def generate_trading_signal(
        ...
        # Ehlers SuperTrend alignment (fast/slow)
        if active.get("ehlers_supertrend", False):
            st_fast_dir = self._get_indicator_value("ST_Fast_Dir", np.nan)
            st_slow_dir = self._get_indicator_value("ST_Slow_Dir", np.nan)
            if not pd.isna(st_fast_dir) and not pd.isna(st_slow_dir):
                contrib = weights.get("ehlers_supertrend_alignment", 0) * trend_strength
                if st_fast_dir == 1 and st_slow_dir == 1:
                    signal_score += contrib
                    signal_breakdown["Ehlers ST Align"] = contrib
...
def display_indicator_values_and_price(
    ...
    for indicator_name, value in analyzer.indicator_values.items():
        ...
        if isinstance(value, Decimal):
            ...
            logger.info(f"  {color}{indicator_name:<20}: {float(value):.8f}{RESET}")
        else:
    ...
    if config.indicators.get("fibonacci_pivot_points", False):
        pivot = analyzer.indicator_values.get("Pivot")
        r1 = analyzer.indicator_values.get("R1")
        r2 = analyzer.indicator_values.get("R2")
        s1 = analyzer.indicator_values.get("S1")
        s2 = analyzer.indicator_values.get("S2")
...
def main() -> None:
    ...
    while True:
        try:
            ...
            # Get ATR value for position sizing; fallback if not present
            atr_value = Decimal(str(analyzer._get_indicator_value("ATR", np.nan)))
            if pd.isna(atr_value):
                # Fallback to naive ATR estimation from last few bars
                high = df["high"].iloc[-min(14, len(df)):]
                low = df["low"].iloc[-min(14, len(df)):]
                close = df["close"].iloc[-min(14, len(df)):]
                prev_close = close.shift().fillna(close.iloc[0])
                tr = pd.concat([high - low, (high - prev_close).abs(), (low - prev_close).abs()], axis=1).max(axis=1)
                atr_value = Decimal(str(tr.mean())) if not tr.empty else Decimal("0.01")
