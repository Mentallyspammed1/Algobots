...
from typing import Any, ClassVar, Dict, List, Optional, Tuple, Generic, TypeVar
...
# --- Constants ---
# API keys are loaded via dotenv in main(), but declared here for type hinting if needed elsewhere
# For actual usage, they will be passed to BybitClient constructor.
API_KEY = os.getenv("BYBIT_API_KEY")
API_SECRET = os.getenv("BYBIT_API_SECRET")
# BASE_URL is handled by pybit's testnet/mainnet flag
CONFIG_FILE = "config.json"
LOG_DIRECTORY = "bot_logs"
Path(LOG_DIRECTORY).mkdir(parents=True, exist_ok=True)

TIMEZONE = ZoneInfo("America/Chicago") # Configurable in main, but default here
MAX_API_RETRIES = 5
RETRY_DELAY_SECONDS = 7
REQUEST_TIMEOUT = 20
LOOP_DELAY_SECONDS = 15 # Main loop delay
...
WS_RECONNECT_DELAY_SECONDS = 5     # Delay before attempting WebSocket reconnection
API_CALL_RETRY_DELAY_SECONDS = 3     # Delay before retrying failed HTTP API calls
...
def load_config(filepath: str, logger: logging.Logger) -> dict[str, Any]:
    ...
    default_config = {
        # Core Settings
        "symbol": "BTCUSDT",
        "interval": "15m", # Bybit API uses '15' for 15 minutes, 'D' for day, etc.
        "loop_delay": LOOP_DELAY_SECONDS,
        "orderbook_limit": 50, # Limit for fetching REST orderbook and WS depth
        "testnet": True, # New: Control testnet/mainnet from config
        "timezone": "America/Chicago", # New: Configurable timezone

        # Signal Generation
        "signal_score_threshold": 2.0,
        "volume_confirmation_multiplier": 1.5, # Not fully implemented in scoring yet

        # Position & Risk Management
        "trade_management": {
            "enabled": True,
            "account_balance": 1000.0, # For simulation fallback; actual balance from API
            "risk_per_trade_percent": 1.0,
            "stop_loss_atr_multiple": 1.5,
            "take_profit_atr_multiple": 2.0,
            "trailing_stop_atr_multiple": 0.5, # New: ATR multiple for trailing stop activation/step
            "max_open_positions": 1,
            "default_leverage": 5, # New: Default leverage to set
        },
        # Multi-Timeframe Analysis
        "mtf_analysis": {
            "enabled": True,
            "higher_timeframes": ["60", "240"], # '1h' -> '60', '4h' -> '240' for Bybit API
    ...
    try:
        with Path(filepath).open(encoding="utf-8") as f:
            config = json.load(f)
        _ensure_config_keys(config, default_config) # Ensure all default keys exist
        with Path(filepath).open("w", encoding="utf-8") as f_write: # Save back with any new default keys
...
class OptimizedSkipList(Generic[KT, VT]):
    ...
    def peek_top(self, reverse: bool = False) -> Optional[VT]:
        # For bids (max price), we want the highest key. For asks (min price), we want the lowest key.
        # This SkipList stores items in ascending key order.
    ...
    def insert(self, price_level: PriceLevel) -> None:
        if price_level.price in self.position_map:
            ...
            if abs(old_price - price_level.price) > 1e-8: # Price changed, unlikely for update, but robust
                 del self.position_map[old_price] # Clean up old price entry if price changed
...
class AdvancedOrderbookManager:
            ...
            for price_str, qty_str in data.get('b', []):
                try:
                    ...
                    if await self._validate_price_quantity(price, quantity) and quantity > 0:
                        ...
                        # SkipList uses key-value, Heap uses object directly
    ...
    async def get_depth(self, depth: int) -> Tuple[List[PriceLevel], List[PriceLevel]]:
        ...
        async with self._lock_context():
            ...
            else: # EnhancedHeap - involves temporary extraction/re-insertion (less efficient for full depth list)
                bids_list: List[PriceLevel] = []
                asks_list: List[PriceLevel] = []
                temp_bids_storage: List[PriceLevel] = []
                temp_asks_storage: List[PriceLevel] = []
                
                # Extract top 'depth' items from heap
...
# --- Bybit Client & Precision Management ---
class BybitClient:
    """
    ...
    async def _bybit_request_with_retry(self, method: str, func: callable, *args, **kwargs) -> dict | None:
        ...
        for attempt in range(MAX_API_RETRIES):
            try:
                # pybit functions already handle signature and base URL internally
    ...
    async def place_order(
        ...
        qty: str, # Must be string
        order_type: str = "Market", # "Market", "Limit"
        price: Optional[str] = None, # Must be string for Limit orders
        reduce_only: bool = False,
        stop_loss: Optional[str] = None, # Must be string
        take_profit: Optional[str] = None, # Must be string
        client_order_id: Optional[str] = None, # New: Custom client order ID
    ) -> dict | None:
    ...
    async def set_trading_stop(
        ...
        trailing_stop: Optional[str] = None, # Value for trailing stop (e.g., "10" for 10 USD or "0.01" for 1%)
        active_price: Optional[str] = None, # Activation price for trailing stop
        ...
        if active_price: # Only relevant for initial trailing stop setup or update
    ...
    async def get_wallet_balance(self) -> Decimal | None:
        ...
        response = await self._bybit_request_with_retry(
            ...
            accountType="UNIFIED" # Assuming UTA for linear/inverse
        )
        if response and response["result"] and response["result"]["list"]:
            for coin_data in response["result"]["list"][0]["coin"]:
                if coin_data["coin"] == "USDT": # Or your base currency
    ...
    async def start_public_ws(
        ...
        # Define a wrapper for the callback to handle JSON parsing if pybit doesn't
    ...
    async def start_private_ws(
        ...
        self.ws_private = WebSocket(
            channel_type="private", # Private streams always use 'private' channel type
            testnet=self.testnet,
            api_key=self.api_key,
            api_secret=self.api_secret
        )
    ...
    async def _monitor_ws_connection(self, ws_client: WebSocket, name: str):
        ...
        while True:
            ...
            if not ws_client.is_connected():
                self.logger.warning(f"{NEON_YELLOW}{name} is not connected. Attempting automatic reconnection...{RESET}")
                # pybit handles reconnection internally, just need to log.
            else:
...
class PrecisionManager:
    ...
    async def load_instrument_info(self, symbol: str):
        ...
        if response and response["result"] and response["result"]["list"]:
            ...
            self.instruments_info[symbol] = {
                ...
                'min_notional': Decimal(lot_size_filter.get("minNotionalValue", "0")), # Some categories might not have this
            }
    ...
    def _get_specs(self, symbol: str) -> Optional[dict]:
    ...
    def round_price(self, price: Decimal, symbol: str) -> Decimal:
        ...
        if specs:
            # Quantize to the tick size (e.g., 0.00001)
    ...
    def round_qty(self, qty: Decimal, symbol: str) -> Decimal:
        ...
        specs = self._get_specs(symbol)
        if specs:
            # Quantize to the quantity step (e.g., 0.001)
            return qty.quantize(specs['qty_precision_decimal'], rounding=ROUND_DOWN) # Use ROUND_DOWN for quantities
...
# --- Indicator Calculator (extracted from TradingAnalyzer) ---
class IndicatorCalculator:
    ...
    def calculate_super_smoother(self, series: pd.Series, period: int) -> pd.Series:
        ...
        return filt.reindex(series.index) # Use series.index, not self.df.index
    ...
    def calculate_ehlers_supertrend(
        ...
        if df_copy.empty: # Re-check after dropping NaNs
            return None
        ...
        for i in range(1, len(df_copy)):
            ...
            if prev_direction == 1: # Previous trend was UP
                ...
                if curr_close < supertrend.loc[current_idx]:
                    direction.loc[current_idx] = -1 # Trend change to DOWN
            else: # Previous trend was DOWN (-1)
                supertrend.loc[current_idx] = min(upper_band.loc[current_idx], prev_supertrend)
                if curr_close > supertrend.loc[current_idx]:
                    direction.loc[current_idx] = 1 # Trend change to UP

            # Initialize first bar's supertrend if it's NaN
        ...
        return result.reindex(df.index) # Reindex to original dataframe's index
    ...
    def calculate_vwap(self, df: pd.DataFrame) -> pd.Series:
        ...
        # pandas_ta.vwap is not session-based. It calculates VWAP from the start of the dataframe.
        # If we want per-day/per-session VWAP, we need to apply it grouped by day, or reset.
        # For typical usage, calculating over the entire fetched data is fine for a bot.
    ...
    def calculate_ichimoku_cloud(
        ...
        ichimoku = ta.ichimoku(
            ...
            kijun=kijun_period,
            senkou=senkou_span_b_period, # senkou param in ta.ichimoku is for senkou_span_b
            offset=chikou_span_offset,
        )
        # The output columns from pandas_ta are like 'ICS_9_26_52', 'ISA_9_26_52', 'ISB_9_26_52', 'CHIKOU_9_26_52'
        ...
        chikou_span = self._safe_series_op(ichimoku[f'ICH_B_{tenkan_period}_{kijun_period}_{senkou_span_b_period}'], "Chikou_Span") # Chikou Span (Lagging Span)
    ...
    def calculate_psar(
        ...
        # Determine direction: 1 for bullish (PSAR below price), -1 for bearish (PSAR above price)
        ...
        # Fill remaining where neither condition is met, assuming continuation
        psar_dir.mask(psar_dir == 0, psar_dir.shift(1), inplace=True)
        psar_dir.fillna(0, inplace=True) # Fill initial NaNs
...
class TradingAnalyzer:
    ...
    def _calculate_all_indicators(self) -> None:
        ...
        # self.df.dropna(subset=["close"], inplace=True) # Ensure close price is always valid, already done on fetch
        
        # Fill any remaining NaNs in indicator columns with 0 or last valid value
        # Using ffill then 0 to ensure continuity where possible
        for col in self.df.columns:
            if col not in ["open", "high", "low", "close", "volume", "turnover"]:
                self.df[col].fillna(method='ffill', inplace=True)
                self.df[col].fillna(0, inplace=True) # Fill leading NaNs if any
    ...
    async def _check_orderbook(self, current_price: Decimal, orderbook_manager: AdvancedOrderbookManager) -> float:
        ...
        # Get more depth for a better imbalance calculation if available
    ...
    def _get_mtf_trend(self, higher_tf_df: pd.DataFrame, indicator_type: str) -> str:
        ...
        if indicator_type == "sma":
            ...
            # Temporarily calculate Ehlers ST for the higher timeframe data
    ...
    async def generate_trading_signal(
        ...
        orderbook_manager: AdvancedOrderbookManager, # Pass manager for live data
        ...
        if active_indicators.get("adx", False):
            ...
            if not pd.isna(adx_val) and not pd.isna(plus_di) and not pd.isna(minus_di):
                ...
                if adx_val > ADX_STRONG_TREND_THRESHOLD:  # Strong trend
                ...
                elif adx_val < ADX_WEAK_TREND_THRESHOLD:  # Weak or no trend
        ...
        if active_indicators.get("ichimoku_cloud", False):
            ...
            if (
                ...
                # Ensure columns exist and enough history for comparison
        ...
        if self.config["mtf_analysis"]["enabled"] and mtf_trends:
            ...
            for _tf_indicator, trend in mtf_trends.items():
                if trend == "UP":
                    ...
                    mtf_sell_score += 1  # Count as a "down" factor

            mtf_weight = weights.get("mtf_trend_confluence", 0.0)
            
            total_mtf_trends = len(mtf_trends)
            if total_mtf_trends > 0:
                # Normalize MTF score: positive if more buy trends, negative if more sell trends
    ...
    def calculate_entry_tp_sl(
    ...
    ) -> tuple[Decimal, Decimal, Decimal, Decimal]: # Returns TP, SL, Trailing_Activation_Price, Trailing_Value
        ...
        trailing_value = Decimal("0") # This represents the 'distance' for the trailing stop

        if signal == "BUY":
            stop_loss = current_price - (atr_value * stop_loss_atr_multiple)
            take_profit = current_price + (atr_value * take_profit_atr_multiple)
            # Trailing stop activates when price moves ATR * X in profit direction from entry
            trailing_activation_price = current_price + (atr_value * Decimal("0.5")) # Example: activate after 0.5 ATR profit
            trailing_value = atr_value * trailing_stop_atr_multiple # ATR based trailing distance
        elif signal == "SELL":
            stop_loss = current_price + (atr_value * stop_loss_atr_multiple)
            take_profit = current_price - (atr_value * take_profit_atr_multiple)
            # Trailing stop activates when price moves ATR * X in profit direction from entry
            ...
            return Decimal("0"), Decimal("0"), Decimal("0"), Decimal("0") # Should not happen for valid signals
        ...
        quantized_trailing_value = trailing_value.quantize(Decimal("0.00001"), rounding=ROUND_DOWN) # Store as positive value
...
class PositionManager:
    ...
    def __init__(
        ...
        alert_system: Any, # Use AlertSystem class directly if imported, or Any
        performance_tracker: Any, # Use PerformanceTracker class directly, or Any
    ):
        ...
        self.open_positions: dict[str, dict] = {}  # Stores active positions, keyed by symbol
        ...
        self.current_account_balance: Decimal = Decimal("0") # Updated by WS or periodically
        
        # New: Trailing Stop tracking
        self.active_trailing_stops: dict[str, dict] = {} # {symbol: {side, entry_price, current_sl_price, activation_price, trail_value, is_activated}}
    ...
    async def _calculate_order_size(
        ...
        leverage = Decimal(str(self.config["trade_management"]["default_leverage"]))
        ...
        # Risk amount / (stop loss distance in USD per unit) = quantity in units
        # Example: Risk $10, SL distance $5 per BTC -> Order $10/$5 = 2 BTC
        order_qty_unleveraged = risk_amount / stop_loss_distance_usd
        
        # Apply leverage to get position size in base currency
        # This simplifies. In Bybit, you specify qty of base asset, leverage is for margin used.
        # So, calculate margin required for unleveraged position value, then divide by leverage
        # Or more simply, if risk_amount is what you're willing to lose from your margin,
        # then the actual USD value of the position is higher with leverage.
        # Let's say risk_amount is the amount you're willing to lose in USDT.
        # If 1 ATR is $100, and SL is 1.5 ATR ($150).
        # To lose $10 with 1.5 ATR SL, you'd open a position of $10 / $150 = 0.0667 units.
        # This qty is then leveraged by the exchange automatically based on your leverage setting.
        # So, the direct calculation for quantity (base asset) is:
        # Quantity = (Account_Balance * Risk_Per_Trade_%) / (ATR * SL_ATR_Multiple)
        ...
        # Check notional value (order_qty * current_price)
    ...
    async def open_position(
        ...
        if self.symbol in self.open_positions:
            ...
            if existing_pos["status"] == "OPEN" and existing_pos["side"] == signal:
                ...
                # Close opposing position first if config allows for one open position
        ...
        # Calculate initial TP/SL/Trailing Stop levels
        ...
        # Place the order with initial SL/TP
        ...
        if order_response:
            position = {
                ...
                "symbol": self.symbol,
                "side": signal,
                "entry_price": current_price, # This might differ from actual fill price, will update from WS
                "qty": order_qty,
                "stop_loss": stop_loss,
                "take_profit": take_profit,
                "trailing_stop_activation_price": trailing_activation_price,
                "trailing_stop_value": trailing_value, # Distance/offset for trailing stop
                "current_trailing_sl": Decimal("0"), # Actual trailing stop loss price
                "is_trailing_activated": False,
                "status": "OPEN",
                "order_id": order_response.get('orderId'), # Store Bybit's order ID
                "client_order_id": client_order_id,
            }
    ...
    async def close_position(self, position: dict) -> bool:
        ...
        order_response = await self.bybit_client.place_order(
            ...
            qty=str(close_qty),
            order_type="Market",
            reduce_only=True, # Ensure it only reduces position
        )

        if order_response:
            self.logger.info(f"{NEON_GREEN}Market order placed to close position: {order_response}{RESET}")
            # Update position status locally (actual update will come from WS)
    ...
    async def manage_positions(
        ...
        for symbol, position in list(self.open_positions.items()): # Iterate over copy as dict might change
            if position["status"] == "OPEN":
                side = position["side"]
                ...
                trailing_value = position["trailing_stop_value"] # This is the distance/offset
                ...
                closed_by = ""
                close_price = current_price # Default close price if triggered
                ...
                # If fixed SL/TP is hit, close position
                if closed_by:
                    self.logger.info(
                        f"{NEON_PURPLE}Fixed {closed_by} hit for {side} position on {symbol}. Closing position.{RESET}"
                    )
                    await self.bybit_client.cancel_all_orders(symbol) # Cancel any open TP/SL
                    await self.close_position(position)
                    # The actual PnL and final status update will come from WebSocket execution messages.
                    # For now, mark as PENDING_CLOSE.
                    position["status"] = "PENDING_CLOSE"
                    position["exit_time"] = datetime.now(ZoneInfo(self.config["timezone"]))
                    position["exit_price"] = current_price # Temporarily
                    position["closed_by"] = closed_by
                    # Record a temporary trade, actual will be from WS
                    pnl = (
                ...
                if not is_trailing_activated:
                    if (side == "BUY" and current_price >= trailing_activation_price) or \
                       (side == "SELL" and current_price <= trailing_activation_price):
                        ...
                        # Initial trailing stop price: current_price - trail_value (for BUY) or current_price + trail_value (for SELL)
                        ...
                        # Update on exchange immediately after activation
                        await self.bybit_client.set_trading_stop(
                            symbol=symbol,
                            trailing_stop=str(trailing_value), # Set trailing distance
                            active_price=str(current_price), # Set current price as active price for the trailing stop
                        )
                else: # Trailing stop is active, update if price moves favorably
                    updated_sl = Decimal("0")
                    if side == "BUY":
                        # New SL candidate: current_price - trailing_value
                        potential_new_sl = current_price - trailing_value
                        # Only update if new SL is higher than current trailing SL AND higher than entry price (to lock in profit)
                        if potential_new_sl > current_trailing_sl and potential_new_sl > entry_price:
                            updated_sl = potential_new_sl
                    else: # SELL
                        # New SL candidate: current_price + trailing_value
                        potential_new_sl = current_price + trailing_value
                        # Only update if new SL is lower than current trailing SL AND lower than entry price (to lock in profit)
                        if potential_new_sl < current_trailing_sl and potential_new_sl < entry_price:
                            updated_sl = potential_new_sl
                    
                    if updated_sl != Decimal("0") and updated_sl != current_trailing_sl:
                        position["current_trailing_sl"] = self.precision_manager.round_price(updated_sl, symbol)
                        self.logger.info(f"{NEON_CYAN}Updating trailing stop for {symbol} to {position['current_trailing_sl']:.5f}{RESET}")
                        await self.bybit_client.set_trading_stop(
                            symbol=symbol,
                            stop_loss=str(position["current_trailing_sl"]), # Amend the stopLoss
                        )
                
                self.open_positions[symbol] = position # Update the position object in the dict

    def update_position_from_ws(self, ws_position_data: Dict[str, Any]):
        """Update internal position state from WebSocket data."""
        symbol = ws_position_data.get('symbol')
        if symbol != self.symbol: # Only care about our trading symbol
            return
        ...
        if size == Decimal("0"):
            if symbol in self.open_positions and self.open_positions[symbol]["status"] != "CLOSED":
                ...
                # Ensure it's marked as closed and then record trade.
                if closed_pos["status"] not in ["CLOSED", "PENDING_CLOSE"]: # Avoid double counting
                    closed_pos["status"] = "CLOSED"
                    closed_pos["exit_time"] = datetime.now(ZoneInfo(self.config["timezone"]))
                    # Try to get actual close price from execution stream if available, else use current price
                    closed_pos["exit_price"] = avg_price # Avg price might not be exit price here, but placeholder
                    pnl = unrealized_pnl # At closure, unrealized becomes realized
                    self.performance_tracker.record_trade(closed_pos, pnl)
            return

        # If position exists and is still open, update its details
        if symbol in self.open_positions:
            position = self.open_positions[symbol]
            position["qty"] = size
            position["side"] = side
            position["entry_price"] = avg_price # Update entry_price to actual avgPrice from exchange
            position["unrealized_pnl"] = unrealized_pnl # For display/tracking
            position["status"] = "OPEN" # Ensure status is OPEN if we have size
            self.open_positions[symbol] = position
            self.logger.debug(f"Updated internal position for {symbol}: Size={size}, AvgPrice={avg_price}, PnL={unrealized_pnl}")
        else:
            # New position opened outside bot or initial sync
            ...
            new_pos = {
                ...
                "qty": size,
                "stop_loss": Decimal("0"), # Will need to fetch from API or set after detection
                "take_profit": Decimal("0"),
...
async def display_indicator_values_and_price(
    ...
    logger: logging.Logger,
    current_price: Decimal,
    trading_analyzer: TradingAnalyzer, # Pass existing analyzer
    ...
    for indicator_name, value in trading_analyzer.indicator_values.items():
        color = INDICATOR_COLORS.get(indicator_name, NEON_YELLOW) # Default to NEON_YELLOW if not found
...
async def main() -> None:
    ...
    global TIMEZONE # Update global TIMEZONE based on config
    ...
    position_manager = PositionManager(
        config, logger, config["symbol"], bybit_client, precision_manager, alert_system, performance_tracker
    )
    orderbook_manager = AdvancedOrderbookManager(config["symbol"], logger, use_skip_list=True) # Using SkipList


    # --- WebSocket Callbacks ---
    async def handle_kline_ws_message(message: Dict[str, Any]):
        """Handler for real-time kline data from WebSocket."""
        # This callback provides the LATEST kline, possibly still open.
        # For indicators requiring full candles, we need to fetch a series of closed candles.
        # This can be used for very fast updates but primary analysis still relies on fetched klines.
        logger.debug(f"WS Kline: {message.get('data')}")
        # Not directly updating df here, relying on HTTP fetch for complete OHLCV series.

    async def handle_ticker_ws_message(message: Dict[str, Any]):
        """Handler for real-time ticker data from WebSocket."""
        # Can be used to update current price immediately for fast signals
        logger.debug(f"WS Ticker: {message.get('data')}")

    async def handle_orderbook_ws_message(message: Dict[str, Any]):
        """Handler for real-time orderbook data from WebSocket."""
    ...
    async def handle_position_ws_message(message: Dict[str, Any]):
        """Handler for real-time position updates from WebSocket."""
    ...
    async def handle_order_ws_message(message: Dict[str, Any]):
        """Handler for real-time order updates from WebSocket."""
        # This could be used to track open orders, fill prices etc.
        # For simplicity, current position_manager relies on position stream for full picture.
        logger.debug(f"WS Order: {message.get('data')}")

    async def handle_execution_ws_message(message: Dict[str, Any]):
        """Handler for real-time execution updates from WebSocket."""
        # Crucial for accurate PnL tracking and updating filled orders.
        # Currently, performance_tracker relies on position_manager's simulated closure.
        # A more robust system would integrate this with PerformanceTracker directly.
        logger.debug(f"WS Execution: {message.get('data')}")

    async def handle_wallet_ws_message(message: Dict[str, Any]):
        """Handler for real-time wallet balance updates from WebSocket."""
        # Update internal balance for order sizing
        for wallet_data in message.get('data', []):
            if wallet_data.get('accountType') == "UNIFIED":
                for coin_data in wallet_data.get('coin', []):
                    if coin_data.get('coin') == "USDT": # Or your base currency
    ...
    await asyncio.sleep(WS_RECONNECT_DELAY_SECONDS) # Give WS time to connect and get first snapshot
    ...
    try:
        while True:
            try:
                ...
                # Update TradingAnalyzer with latest data
                ...
                if config["mtf_analysis"]["enabled"]:
                    for htf_interval in config["mtf_analysis"]["higher_timeframes"]:
                        ...
                        await asyncio.sleep(config["mtf_analysis"]["mtf_request_delay_seconds"]) # Delay between MTF requests
                ...
                # --- Position Management ---
                await position_manager.manage_positions(current_price, atr_value) # Check existing positions (TP/SL/Trailing Stop)
                ...
                if open_positions:
                    ...
                    for pos in open_positions:
                        logger.info(
                            f"  - {pos['side']} @ {pos['entry_price']} (SL: {pos['stop_loss']}, TP: {pos['take_profit']}, Trailing SL: {pos['current_trailing_sl'] if pos['is_trailing_activated'] else 'N/A'}){RESET}"
                        )
                else:
        ...
        await bybit_client.cancel_all_orders(config["symbol"]) # Attempt to cancel any remaining orders
