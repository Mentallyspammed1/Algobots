...
try:
    ...
    class HTTP:
        ...
        def __getattr__(self, name): return self._dummy_method
        def _dummy_method(self, **kwargs):
            logging.getLogger(__name__).warning("Pybit not available. Using dummy client.")
            return
    class WebSocket:
        def __init__(self, **kwargs): pass
        def __getattr__(self, name): return self._dummy_method
        def _dummy_method(self, *args, **kwargs):
            logging.getLogger(__name__).warning("Pybit not available. Using dummy WebSocket.")
    ...
    class indicators:
        ...
        def __getattr__(name):
            def dummy_indicator(*args, **kwargs):
                logging.getLogger(__name__).error(f"indicators.py not found. Using dummy function for '{name}'.")
                return pd.Series(np.nan) if "calculate" in name or name in ["calculate_dema", "calculate_roc", "calculate_vwap"] else {"Market_Structure_Trend": "UNKNOWN"}
            return dummy_indicator
    logging.getLogger(__name__).error("indicators.py not found. Please ensure it's in the same directory or accessible via PYTHONPATH.")
...
try:
    ...
    class AlertSystem:
        ...
        def send_alert(self, message, level="INFO"): self.logger.log(logging.INFO, f"ALERT (Dummy): {message}")
    logging.getLogger(__name__).error("alert_system.py not found. Using dummy alert system.")
...
INDICATOR_COLORS = {
    "SMA_10": NEON_BLUE, "SMA_Long": Fore.BLUE, "EMA_Short": NEON_PURPLE, "EMA_Long": Fore.MAGENTA,
    "ATR": NEON_YELLOW, "RSI": NEON_GREEN, "StochRSI_K": NEON_CYAN, "StochRSI_D": Fore.LIGHTCYAN_EX,
    "BB_Upper": NEON_RED, "BB_Middle": Fore.WHITE, "BB_Lower": NEON_RED, "CCI": NEON_GREEN,
    "WR": NEON_RED, "MFI": NEON_GREEN, "OBV": Fore.BLUE, "OBV_EMA": NEON_BLUE,
    "CMF": NEON_PURPLE, "Tenkan_Sen": NEON_CYAN, "Kijun_Sen": Fore.LIGHTCYAN_EX,
    "Senkou_Span_A": NEON_GREEN, "Senkou_Span_B": NEON_RED, "Chikou_Span": NEON_YELLOW,
    "PSAR_Val": NEON_PURPLE, "PSAR_Dir": Fore.LIGHTMAGENTA_EX, "VWAP": Fore.WHITE,
    "ST_Fast_Dir": Fore.BLUE, "ST_Fast_Val": NEON_BLUE, "ST_Slow_Dir": NEON_PURPLE,
    "ST_Slow_Val": Fore.LIGHTMAGENTA_EX, "MACD_Line": NEON_GREEN, "MACD_Signal": Fore.LIGHTGREEN_EX,
    "MACD_Hist": NEON_YELLOW, "ADX": NEON_CYAN, "PlusDI": Fore.LIGHTCYAN_EX, "MinusDI": NEON_RED,
    "Volatility_Index": NEON_YELLOW, "Volume_Delta": Fore.LIGHTCYAN_EX, "VWMA": Fore.WHITE,
    "Kaufman_AMA": NEON_GREEN, "Relative_Volume": NEON_PURPLE, "Market_Structure_Trend": Fore.LIGHTCYAN_EX,
    "DEMA": Fore.BLUE, "Keltner_Upper": NEON_PURPLE, "Keltner_Middle": Fore.WHITE, "Keltner_Lower": NEON_PURPLE,
    "ROC": NEON_GREEN, "Pivot": Fore.WHITE, "R1": NEON_CYAN, "R2": Fore.LIGHTCYAN_EX,
    "S1": NEON_PURPLE, "S2": Fore.LIGHTMAGENTA_EX, "Candlestick_Pattern": Fore.LIGHTYELLOW_EX,
    "Support_Level": NEON_CYAN, "Resistance_Level": NEON_RED,
}
...
def load_config(filepath: str, logger: logging.Logger) -> dict[str, Any]:
    default_config = {
        ...
        "trade_management": {
            ...
            "trailing_stop_atr_multiple": 0.5, "max_open_positions": 1,
            "order_precision": 5, "price_precision": 3, "slippage_percent": 0.001,
            "trading_fee_percent": 0.0005,
        },
        "risk_guardrails": {
            "enabled": True, "max_day_loss_pct": 3.0, "max_drawdown_pct": 8.0,
            "cooldown_after_kill_min": 120, "spread_filter_bps": 5.0,
            "ev_filter_enabled": True, "consecutive_losses_limit": 5,
            "api_error_limit_per_hour": 10, "ws_disconnect_limit_per_hour": 5,
        },
        "session_filter": {"enabled": False, "utc_allowed": [["00:00", "08:00"], ["13:00", "20:00"]]},
        "pyramiding": {"enabled": False, "max_adds": 2, "step_atr": 0.7, "size_pct_of_initial": 0.5},
        "mtf_analysis": {
        ...
        "execution": {
            "use_pybit": False, "testnet": False, "account_type": "UNIFIED",
            "category": "linear", "position_mode": "ONE_WAY", "tpsl_mode": "Partial",
            "buy_leverage": "3", "sell_leverage": "3", "tp_trigger_by": "LastPrice",
            "sl_trigger_by": "LastPrice", "default_time_in_force": "GoodTillCancel",
            ...
            "sl_scheme": {"type": "atr_multiple", "atr_multiple": 1.5, "percent": 1.0, "use_conditional_stop": True, "stop_order_type": "Market"},
            "breakeven_after_tp1": {"enabled": True, "offset_type": "atr", "offset_value": 0.10, "lock_in_min_percent": 0, "sl_trigger_by": "LastPrice"},
            "live_sync": {
                "enabled": True, "poll_ms": EXECUTION_POLL_INTERVAL_MS,
                "max_exec_fetch": 200, "only_track_linked": True,
        ...
        "indicators": {
            "atr": True, "ema_alignment": True, "sma_trend_filter": True,
            "momentum": True, "volume_confirmation": True, "stoch_rsi": True,
            ...
            "kaufman_ama": True, "relative_volume": True,
            "market_structure": True, "dema": True, "keltner_channels": True,
            "roc": True, "candlestick_patterns": True, "fibonacci_pivot_points": True,
        },
        "weight_sets": {
            "default_scalping": {
                "ema_alignment": 0.22, "sma_trend_filter": 0.28,
                "momentum_rsi_stoch_cci_wr_mfi": 0.18, "volume_confirmation": 0.12,
                "bollinger_bands": 0.22, "vwap": 0.22, "cci": 0.08, "wr": 0.08,
                "psar": 0.22, "sma_10": 0.07, "mfi": 0.12, "orderbook_imbalance": 0.07,
                "fibonacci_levels": 0.10, "ehlers_supertrend_alignment": 0.55,
                "macd_alignment": 0.28, "adx_strength": 0.18, "ichimoku_confluence": 0.38,
                "obv_momentum": 0.18, "cmf_flow": 0.12, "mtf_trend_confluence": 0.32,
                "volatility_index_signal": 0.15, "vwma_cross": 0.15,
                "volume_delta_signal": 0.10, "kaufman_ama_cross": 0.20,
                "relative_volume_confirmation": 0.10, "market_structure_confluence": 0.25,
...
async def bybit_request_async(
    ...
    if logger is None: logger = global_logger
    ...
    for attempt in range(MAX_API_RETRIES):
        try:
            ...
            logger.error(f"{NEON_RED}HTTP Error {e.status}: {e.message}. Retrying... ({attempt+1}/{MAX_API_RETRIES}){RESET}")
            await asyncio.sleep(RETRY_DELAY_SECONDS)
        except aiohttp.ClientConnectionError as e:
            logger.error(f"{NEON_RED}Connection Error: {e}. Retrying... ({attempt+1}/{MAX_API_RETRIES}){RESET}")
            await asyncio.sleep(RETRY_DELAY_SECONDS)
        except aiohttp.ClientError as e:
            logger.error(f"{NEON_RED}Request Exception: {e}. Retrying... ({attempt+1}/{MAX_API_RETRIES}){RESET}")
            await asyncio.sleep(RETRY_DELAY_SECONDS)
        except json.JSONDecodeError:
            logger.error(f"{NEON_RED}Failed to decode JSON response. Retrying... ({attempt+1}/{MAX_API_RETRIES}){RESET}")
            await asyncio.sleep(RETRY_DELAY_SECONDS)
        except Exception as e:
            logger.error(f"{NEON_RED}Unexpected error during API request: {e}. Retrying... ({attempt+1}/{MAX_API_RETRIES}){RESET}", exc_info=True)
            await asyncio.sleep(RETRY_DELAY_SECONDS)

    logger.error(f"{NEON_RED}Max API retries ({MAX_API_RETRIES}) exceeded. Trading halted.{RESET}")
    return None
...
class PriceLevel:
    ...
    timestamp: int
    order_count: int = 1
    def __lt__(self, other: 'PriceLevel') -> bool: return self.price < other.price
    def __eq__(self, other: 'PriceLevel') -> bool: return abs(self.price - other.price) < 1e-8
...
class OptimizedSkipList(Generic[KT, VT]):
    ...
    def insert(self, key: KT, value: VT) -> None:
        ...
        for i in range(self.level, -1, -1):
            while (current.forward[i] and current.forward[i].key is not None and current.forward[i].key < key):
    ...
    def delete(self, key: KT) -> bool:
        ...
        for i in range(self.level, -1, -1):
            ...
            update[i] = current
        current = current.forward[0]
        if not current or current.key != key: return False
    ...
    def get_sorted_items(self, reverse: bool = False) -> list[tuple[KT, VT]]:
        ...
        while current:
            if current.key is not None: items.append((current.key, current.value))
            current = current.forward[0]
...
class AdvancedOrderbookManager:
            ...
            for price_str, qty_str in data.get('b', []):
                try:
                    ...
                    if await self._validate_price_quantity(price, quantity) and quantity > 0:
                        ...
                        else: self.bids_ds.append(level)
            ...
            for price_str, qty_str in data.get('a', []):
                try:
                    ...
                    if await self._validate_price_quantity(price, quantity) and quantity > 0:
                        ...
                        else: self.asks_ds.append(level)
                except (ValueError, TypeError) as e:
        ...
        async with self._lock_context():
            ...
            for price_str, qty_str in data.get('b', []):
                try:
                    ...
                    if self.use_skip_list:
                        if quantity == 0.0: self.bids_ds.delete(price)
                        else: self.bids_ds.insert(price, PriceLevel(price, quantity, int(time.time() * 1000)))
                    else:
                        levels = [lvl for lvl in self.bids_ds if abs(lvl.price - price) > 1e-8]
                        if quantity > 0: levels.append(PriceLevel(price, quantity, int(time.time() * 1000)))
                        self.bids_ds = sorted(levels, key=lambda x: x.price, reverse=True)
                except (ValueError, TypeError) as e: self.logger.error(f"Failed to parse bid delta for {self.symbol}: {price_str}/{qty_str}, error={e}")
            for price_str, qty_str in data.get('a', []):
                try:
                    price = float(price_str)
                    quantity = float(qty_str)
                    if not await self._validate_price_quantity(price, quantity): continue
                    if self.use_skip_list:
                        if quantity == 0.0: self.asks_ds.delete(price)
                        else: self.asks_ds.insert(price, PriceLevel(price, quantity, int(time.time() * 1000)))
                    else:
                        levels = [lvl for lvl in self.asks_ds if abs(lvl.price - price) > 1e-8]
                        if quantity > 0: levels.append(PriceLevel(price, quantity, int(time.time() * 1000)))
                        self.asks_ds = sorted(levels, key=lambda x: x.price)
                except (ValueError, TypeError) as e: self.logger.error(f"Failed to parse ask delta for {self.symbol}: {price_str}/{qty_str}, error={e}")
    ...
    async def get_best_bid_ask(self) -> tuple[float | None, float | None]:
        async with self._lock_context():
            best_bid = self.bids_ds.peek_top(reverse=True) if self.use_skip_list else (self.bids_ds[0] if self.bids_ds else None)
            best_ask = self.asks_ds.peek_top(reverse=False) if self.use_skip_list else (self.asks_ds[0] if self.asks_ds else None)
    ...
    async def get_depth(self, depth: int) -> tuple[list[PriceLevel], list[PriceLevel]]:
        async with self._lock_context():
            bids = self.bids_ds.get_sorted_items(reverse=True)[:depth] if self.use_skip_list else self.bids_ds[:depth]
            asks = self.asks_ds.get_sorted_items()[:depth] if self.use_skip_list else self.asks_ds[:depth]
            if self.use_skip_list: bids = [item[1] for item in bids]
            if self.use_skip_list: asks = [item[1] for item in asks]
            return bids, asks

class KlineDataManager:
    def __init__(self, symbol: str, interval: str, max_klines: int, logger: logging.Logger):
        self.symbol = symbol
    ...
    async def initialize(self, pybit_client: 'PybitTradingClient'):
        df = await pybit_client.fetch_klines(self.symbol, self.interval, self.max_klines)
        async with self._df_lock:
            if df is not None and not df.empty:
                self._df = df
                self._last_candle_open_time = self._df.index[-1]
                self.logger.info(f"Initialized KlineDataManager for {self.symbol}@{self.interval} with {len(df)} klines.")
            else:
                self.logger.warning(f"Failed to initialize KlineDataManager for {self.symbol}@{self.interval}.")
    async def get_df(self) -> pd.DataFrame:
        async with self._df_lock: return self._df.copy()
    async def update_from_ws(self, ws_kline_data: dict[str, Any]):
        new_candle = {
            "start_time": pd.to_datetime(ws_kline_data["timestamp"], unit="ms", utc=True).dt.tz_convert(TIMEZONE),
            "open": float(ws_kline_data["open"]), "high": float(ws_kline_data["high"]),
            "low": float(ws_kline_data["low"]), "close": float(ws_kline_data["close"]),
        ...
        async with self._df_lock:
            if self._last_candle_open_time is None or new_df_row.index[0] > self._last_candle_open_time:
                self._df = pd.concat([self._df, new_df_row])
                self._df = self._df.iloc[-self.max_klines:]
                self._last_candle_open_time = self._df.index[-1]
                self.logger.debug(f"WS: Appended new candle {new_df_row.index[0]} to {self.symbol}@{self.interval} data.")
            elif new_df_row.index[0] == self._last_candle_open_time:
                self._df.loc[new_df_row.index[0]] = new_df_row.iloc[0]
                self.logger.debug(f"WS: Updated current candle {new_df_row.index[0]} for {self.symbol}@{self.interval} data.")

# --- Pybit Trading Client ---
class PybitTradingClient:
    def __init__(self, config: dict[str, Any], logger: logging.Logger):
        self.cfg, self.logger = config, logger
        self.enabled = bool(config.get("execution", {}).get("use_pybit", False)) and PYBIT_AVAILABLE and API_KEY and API_SECRET
        ...
        if not self.enabled:
            if not PYBIT_AVAILABLE: self.logger.error(f"{NEON_RED}Pybit library not found. Please install it: pip install pybit.{RESET}")
            if not (API_KEY and API_SECRET): self.logger.error(f"{NEON_RED}API keys (BYBIT_API_KEY, BYBIT_API_SECRET) not set in .env.{RESET}")
            self.logger.info(f"{NEON_YELLOW}PyBit execution disabled in config or due to missing dependencies.{RESET}")
            return
        ...
        try:
            self.session = HTTP(
                ...
                testnet=self.testnet, timeout=REQUEST_TIMEOUT,
            ...
            self.kline_data_managers[kline_key] = KlineDataManager(self.cfg['symbol'], self.cfg['interval'], 1000, self.logger)
            self.orderbook_data_manager = AdvancedOrderbookManager(self.cfg['symbol'], self.logger, use_skip_list=True)
        except pybit.exceptions.FailedRequestError as e:
            self.enabled = False
            self.logger.critical(f"{NEON_RED}Failed to initialize PyBit client due to API error: {e}. Check credentials and network.{RESET}")
        except Exception as e:
            self.enabled = False
            self.logger.critical(f"{NEON_RED}Failed to initialize PyBit client: {e}\n{traceback.format_exc()}{RESET}")
    ...
    def _track_api_error(self, success: bool):
        ...
        if not success: self.api_error_count_hourly += 1
    ...
    def is_circuit_breaker_tripped(self) -> bool:
        ...
        if self.api_error_count_hourly >= api_limit:
            self.logger.critical(f"{NEON_RED}CIRCUIT BREAKER: API error limit ({api_limit}) exceeded! Trading halted.{RESET}")
            return True
        if self.ws_disconnect_count_hourly >= ws_limit:
            self.logger.critical(f"{NEON_RED}CIRCUIT BREAKER: WebSocket disconnect limit ({ws_limit}) exceeded! Trading halted.{RESET}")
            return True
        return False
    def _log_api(self, action: str, resp: dict | None, log_level: str = "error"):
        ...
        if resp.get("retCode") != 0:
            ...
            getattr(self.logger, log_level)(f"{NEON_RED}{action}: Failed with code {ret_code} - {error_msg}{RESET}")
            self._track_api_error(False)
        else:
    ...
    def _side_to_bybit(self, side: Literal["BUY", "SELL"]) -> Literal["Buy", "Sell"]: return "Buy" if side == "BUY" else "Sell"
    ...
    def _handle_403_error(self, e: Exception):
        if isinstance(e, pybit.exceptions.FailedRequestError) and e.status_code == 403:
            self.logger.critical(f"{NEON_RED}API Error 403 Forbidden: Check API key permissions and IP whitelist settings. Disabling client permanently.{RESET}")
            self.enabled = False
            self.stop_event.set()
    async def set_leverage(self, symbol: str, buy: str, sell: str) -> bool:
        ...
        except (pybit.exceptions.FailedRequestError, Exception) as e: self.logger.error(f"set_leverage failed for {symbol}: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return False
    async def get_positions(self, symbol: str | None = None) -> dict | None:
        ...
        try:
            ...
            return resp
        except (pybit.exceptions.FailedRequestError, Exception) as e: self.logger.error(f"get_positions exception: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None
    async def get_wallet_balance(self, coin: str = "USDT") -> dict | None:
        ...
        try:
            ...
            return resp
        except (pybit.exceptions.FailedRequestError, Exception) as e: self.logger.error(f"get_wallet_balance exception: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None
    async def place_order(self, **kwargs) -> dict | None:
        ...
        try:
            ...
            return resp
        except (pybit.exceptions.FailedRequestError, Exception) as e: self.logger.error(f"place_order exception: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None
    async def batch_place_orders(self, orders: list[dict]) -> dict | None:
        ...
        try:
            ...
            return resp
        except (pybit.exceptions.FailedRequestError, Exception) as e: self.logger.error(f"batch_place_orders exception: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None
    async def cancel_order(self, symbol: str, order_id: str | None = None, order_link_id: str | None = None) -> dict | None:
        if not self.enabled or not self.session: return None
        try:
            ...
            else: self.logger.warning("No orderId or orderLinkId provided for cancel_order."); return None
            resp = self.session.cancel_order(**params)
            self._log_api("cancel_order", resp, "debug")
            return resp
        except (pybit.exceptions.FailedRequestError, Exception) as e: self.logger.error(f"cancel_order exception for {symbol}: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None
    async def get_open_orders(self, symbol: str | None = None) -> list[dict]:
        ...
        try:
            ...
            if self._ok(resp) and resp.get("result", {}).get("list"): return resp["result"]["list"]
            return []
        except (pybit.exceptions.FailedRequestError, Exception) as e: self.logger.error(f"get_open_orders exception: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return []
    async def fetch_current_price(self, symbol: str) -> Decimal | None:
        ...
        try:
            ...
            return None
        except (pybit.exceptions.FailedRequestError, Exception) as e: self.logger.error(f"fetch_current_price exception for {symbol}: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None
    async def fetch_klines(self, symbol: str, interval: str, limit: int) -> pd.DataFrame | None:
        ...
        try:
            ...
            if self._ok(resp) and resp.get("result", {}).get("list"):
                df = pd.DataFrame(resp["result"]["list"], columns=["start_time", "open", "high", "low", "close", "volume", "turnover"])
                ...
                if df.empty: self.logger.warning(f"Fetched klines for {symbol} {interval} but DataFrame is empty after processing."); return None
                return df
            self.logger.warning(f"Could not fetch klines for {symbol} {interval}. Response: {resp}")
            return None
        except (pybit.exceptions.FailedRequestError, KeyError, ValueError) as e: self.logger.error(f"fetch_klines exception for {symbol} {interval}: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None
    async def fetch_orderbook(self, symbol: str, limit: int) -> dict | None:
        ...
        try:
            ...
            if self._ok(response) and response.get("result"): return response["result"]
            self.logger.warning(f"Could not fetch orderbook for {symbol}. Response: {response}")
            return None
        except (pybit.exceptions.FailedRequestError, KeyError) as e: self.logger.error(f"fetch_orderbook exception for {symbol}: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None
    async def fetch_instrument_info(self, symbol: str) -> dict | None:
        if not self.enabled or not self.session: return None
        try:
            params = {"category": self.category, "symbol": symbol}
            response = self.session.get_instruments_info(**params)
            if self._ok(response) and response.get("result", {}).get("list"): return response["result"]["list"][0]
            self.logger.warning(f"Could not fetch instrument info for {symbol}. Response: {response}")
            return None
        except (pybit.exceptions.FailedRequestError, KeyError) as e: self.logger.error(f"fetch_instrument_info exception for {symbol}: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None
    async def get_executions(self, symbol: str, start_time_ms: int, limit: int) -> dict | None:
        if not self.enabled or not self.session: return None
        try:
            params = {"category": self.category, "symbol": symbol, "startTime": start_time_ms, "limit": limit}
            response = self.session.get_executions(**params)
            self._log_api("get_executions", response, "debug")
            return response
        except (pybit.exceptions.FailedRequestError, Exception) as e: self.logger.error(f"get_executions exception: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None
    async def cancel_all_orders(self, symbol: str) -> dict | None:
        ...
        try:
            ...
            return resp
        except (pybit.exceptions.FailedRequestError, Exception) as e: self.logger.error(f"cancel_all_orders exception for {symbol}: {e}\n{traceback.format_exc()}"); self._handle_403_error(e); return None

    # --- WebSocket Management (Asyncio) ---
    async def _run_ws(self, ws_type: Literal["public", "private"], pybit_ws_instance: WebSocket):
        while not self.stop_event.is_set():
            try:
                ...
                self.logger.info(f"{ws_type} WebSocket connection opened and subscribed.")

                while pybit_ws_instance.ws and pybit_ws_instance.ws.connected:
                    await asyncio.sleep(self.cfg['execution']['live_sync']['heartbeat']['interval_ms'] / 1000.0)

            except Exception as e:
                self.logger.error(f"WebSocket error ({ws_type}): {e}. Retrying in {WS_RECONNECT_DELAY_SECONDS}s.\n{traceback.format_exc()}")
    ...
    async def shutdown(self):
        self.stop_event.set()
        if self.ws_manager: await self.ws_manager.shutdown()
        self.save_state()
...
class WebSocketManager:
    ...
    async def start(self):
        self.ws_public = WebSocket(
            ...
            ping_interval=self.cfg['execution']['live_sync']['heartbeat']['interval_ms'] / 1000.0,
            ping_timeout=self.cfg['execution']['live_sync']['heartbeat']['interval_ms'] / 2000.0,
        )
        ...
        self.public_ws_task = asyncio.create_task(self._run_ws("public", self.ws_public))
        if self.api_client.enabled: self.private_ws_task = asyncio.create_task(self._run_ws("private", self.ws_private))

    async def shutdown(self):
        if self.ws_public: self.ws_public.close()
        if self.ws_private: self.ws_private.close()
        if self.public_ws_task: self.public_ws_task.cancel()
        if self.private_ws_task: self.private_ws_task.cancel()
        await asyncio.gather(self.public_ws_task, self.private_ws_task, return_exceptions=True)
    ...
    def _on_error(self, ws, error): self.logger.error(f"WebSocket Error: {error}"); self.api_client._track_ws_disconnect()
    def _on_close(self, ws_inst, close_status_code, close_msg): self.logger.warning(f"WebSocket closed: {close_status_code} {close_msg}. Reconnecting..."); self.api_client._track_ws_disconnect()
    def _on_open(self, ws_inst): self.logger.info("WebSocket connection opened."); self.api_client.new_kline_event.set()
    async def _process_ws_message(self, data):
        topic = data.get("topic")
        if topic and topic.startswith("position"): await self.handle_position_update(data.get("data", []))
        elif topic and topic.startswith("order"): await self.handle_order_update(data.get("data", []))
        elif topic and topic.startswith("execution"): await self.handle_execution_update(data.get("data", []))
        elif topic and topic.startswith("wallet"): await self.handle_account_update(data.get("data", {}))
        elif topic and topic.startswith("kline."): await self.handle_kline_update(data.get("data", []))
        elif topic and topic.startswith("orderbook."): await self.handle_orderbook_update(data.get("data", {}))
        elif topic and topic.startswith("tickers."): await self.handle_ticker_update(data.get("data", []))
    ...
    async def handle_position_update(self, position_list: list[dict]):
        for pos_info in position_list:
            ...
            if symbol and symbol == self.cfg["symbol"]:
                ...
                if local_pos:
                    async with self.state_lock: self.positions[symbol] = local_pos
    async def handle_order_update(self, order_list: list[dict]):
        for order_info in order_list:
            order_id = order_info.get("orderId")
            if order_id:
                async with self.state_lock: self.orders[order_id] = order_info
    async def handle_execution_update(self, execution_list: list[dict]):
        for exec_info in execution_list:
            ...
            if exec_time_ms > self.last_exec_time_ms:
                self.last_exec_time_ms = exec_time_ms
    async def handle_account_update(self, account_data: dict): pass
    async def handle_kline_update(self, kline_data: list[dict]):
        if kline_data and kline_data[0].get('confirm') is True:
            kline_manager = self.api_client.kline_data_managers.get(f"{self.cfg['symbol']}_{self.cfg['interval']}")
            if kline_manager: await kline_manager.update_from_ws(kline_data[0])
    ...
    async def handle_orderbook_update(self, orderbook_data: dict):
        if self.api_client.orderbook_data_manager:
            if orderbook_data.get("u") is not None: await self.api_client.orderbook_data_manager.update_delta(orderbook_data)
            else: await self.api_client.orderbook_data_manager.update_snapshot(orderbook_data)
    async def handle_ticker_update(self, ticker_data: list[dict]): pass
    ...
    def _convert_ws_position_to_local(self, ws_pos: dict) -> dict | None:
        ...
        try:
            return {
                "entry_time": datetime.fromtimestamp(int(ws_pos.get("createdTime", 0)) / 1000, tz=TIMEZONE),
                "symbol": symbol, "side": "BUY" if ws_pos.get("side") == "Buy" else "SELL",
                "entry_price": Decimal(ws_pos.get("avgPrice", "0")), "qty": Decimal(ws_pos.get("size", "0")),
                "stop_loss": Decimal(ws_pos.get("stopLoss", "0")), "take_profit": Decimal(ws_pos.get("takeProfit", "0")),
                "status": "OPEN", "link_prefix": f"ws_{int(time.time()*1000)}", "adds": 0, "order_id": None,
                "stop_loss_order_id": None, "take_profit_order_ids": [], "breakeven_set": False,
                "unrealized_pnl": Decimal(ws_pos.get("unrealisedPnl", "0")),
            }
        except (ValueError, KeyError) as e: self.logger.error(f"Error converting WS position data for {symbol}: {e}\n{traceback.format_exc()}"); return None
...
class PositionManager:
    def __init__(self, config: dict, logger: logging.Logger, symbol: str, pybit_client: PybitTradingClient, performance_tracker: 'PerformanceTracker'):
        ...
        self.trailing_stop_atr_multiple = Decimal(str(config["trade_management"].get("trailing_stop_atr_multiple", 0.5)))
        self.trade_management_enabled = bool(config["trade_management"]["enabled"])
        self.current_account_balance = Decimal(str(config["trade_management"]["account_balance"]))
    async def _update_precision_from_exchange(self):
        ...
        if info:
            if "lotSizeFilter" in info:
                ...
                if not self.qty_step.is_zero(): self.order_precision = abs(self.qty_step.as_tuple().exponent)
            if "priceFilter" in info:
                price_filter = info["priceFilter"]
                tick_size = Decimal(str(price_filter.get("tickSize")))
                if not tick_size.is_zero(): self.price_precision = abs(tick_size.as_tuple().exponent)
            ...
            if resp and self.pybit._ok(resp) and resp.get("result", {}).get("list"):
                for account in resp["result"]["list"]:
                    for coin_balance in account.get("coin", []):
                        if coin_balance["coin"] == "USDT": return Decimal(coin_balance["walletBalance"])
    ...
    def _calculate_order_size(self, current_price: Decimal, atr_value: Decimal, conviction: float = 1.0) -> Decimal:
        ...
        if stop_loss_distance_usd <= 0: return Decimal("0")
        order_qty = (risk_amount / stop_loss_distance_usd) / current_price
        if self.qty_step and self.qty_step > Decimal(0): return round_qty(order_qty, self.qty_step)
    ...
    def get_open_positions(self) -> list[Position]: return [pos for pos in self.open_positions.values() if pos.status == "OPEN"]
    def update_position_from_ws(self, ws_data: dict):
        ...
        if local_pos:
            ...
            self.logger.debug(f"Position updated from WS: {self.open_positions[self.symbol]}")

    async def open_position(self, signal: Literal["BUY", "SELL"], current_price: Decimal, atr_value: Decimal, conviction: float = 1.0) -> Position | None:
        if not self.trade_management_enabled or (self.symbol in self.open_positions and self.open_positions[self.symbol].status == "OPEN"):
            self.logger.info(f"[{self.symbol}] Position management disabled or already open. Skipping.")
            return None
        ...
        position = Position(
            ...
            link_prefix=f"wgx_{int(time.time()*1000)}", adds=0,
        ...
        if self.live and self.pybit and self.pybit.enabled:
            ...
            resp = await self.pybit.place_order(
                ...
                orderType="Market", qty=self.pybit._q(order_qty), orderLinkId=entry_link_id,
            )
    ...
    async def close_position(self, position: Position) -> bool:
        ...
        if self.live and self.pybit and self.pybit.enabled:
            ...
            order_response = await self.pybit.place_order(
                category=self.pybit.category, symbol=self.symbol, side=self.pybit._side_to_bybit(opposing_side),
                orderType="Market", qty=self.pybit._q(position.qty), reduceOnly=True,
            )
            if order_response and self.pybit._ok(order_response):
                position.status = "PENDING_CLOSE"
                self.logger.info(f"[{self.symbol}] Placed order to close {position.side} {position.qty.normalize()}.")
                return True
            self.logger.error(f"[{self.symbol}] Failed to place market order to close position. Response: {order_response}")
            return False
        return True
    ...
    async def manage_positions(self, current_price: Decimal, atr_value: Decimal):
        ...
        for symbol, pos in self.open_positions.items():
            ...
            if pos.side == "BUY":
                ...
                elif pos.best_price is not None and current_price <= (pos.best_price - (atr_value * self.trailing_stop_atr_multiple)): closed_by = "TRAILING_STOP"
                if closed_by: close_price = current_price * (Decimal("1") - self.slippage_percent)
            elif pos.side == "SELL":
                if current_price >= pos.stop_loss: closed_by = "STOP_LOSS"
                elif current_price <= pos.take_profit: closed_by = "TAKE_PROFIT"
                elif pos.best_price is not None and current_price >= (pos.best_price + (atr_value * self.trailing_stop_atr_multiple)): closed_by = "TRAILING_STOP"
            ...
            if closed_by:
                ...
                self.logger.info(f"[{self.symbol}] Closed simulated {pos.side} position by {closed_by}. PnL: {pnl:.2f}")
                symbols_to_remove.append(symbol)

        for symbol in symbols_to_remove: del self.open_positions[symbol]

    async def trail_stop(self, pos: Position, current_price: Decimal, atr_value: Decimal):
        if pos.status != "OPEN": return
        pos.best_price = max(pos.best_price or Decimal("0"), current_price) if pos.side == "BUY" else min(pos.best_price or Decimal("99999999"), current_price)

        trailing_value = self.trailing_stop_atr_multiple * atr_value
        if trailing_value <= 0: return

        new_sl_price = Decimal("0")
        if pos.side == "BUY":
            new_sl_price = pos.best_price - trailing_value
            if new_sl_price <= pos.stop_loss: return
        else:
            new_sl_price = pos.best_price + trailing_value
            if new_sl_price >= pos.stop_loss: return
        ...
        if abs(new_sl_price - pos.stop_loss) > Decimal("0.001"):
            self.logger.info(f"Updating trailing stop for {self.symbol} from {pos.stop_loss:.5f} to {new_sl_price:.5f}")
            pos.stop_loss = new_sl_price
            if self.live and self.pybit and self.pybit.enabled:
                await self.pybit.set_trading_stop(symbol=self.symbol, stop_loss=self.pybit._q(new_sl_price))

    async def try_pyramid(self, current_price: Decimal, atr_value: Decimal):
        if not self.config.get("pyramiding", {}).get("enabled", False) or not self.open_positions: return
        py_cfg = self.config["pyramiding"]
        for symbol, pos in list(self.open_positions.items()):
            if pos.status != "OPEN" or pos.adds >= py_cfg.get("max_adds", 0): continue
            step_atr_mult = Decimal(str(py_cfg.get("step_atr", 0.7)))
            step_distance = step_atr_mult * atr_value
            target_price = pos.entry_price + step_distance if pos.side == "BUY" else pos.entry_price - step_distance

            should_add = (pos.side == "BUY" and current_price >= target_price) or (pos.side == "SELL" and current_price <= target_price)
            if should_add:
                add_qty = round_qty(pos.qty * Decimal(str(py_cfg.get("size_pct_of_initial", 0.5))), self.qty_step)
                if add_qty <= 0: continue

                self.logger.info(f"Pyramiding add #{pos.adds+1} for {self.symbol}. Adding {add_qty.normalize()} at {current_price.normalize()}")
                ...
                if self.live and self.pybit and self.pybit.enabled:
                    ...
                    resp = await self.pybit.place_order(
                        category=self.pybit.category, symbol=self.symbol, side=self.pybit._side_to_bybit(pos.side),
                        orderType="Market", qty=self.pybit._q(add_qty), orderLinkId=add_link_id,
                    )
                    if not self.pybit._ok(resp): self.logger.error(f"Live pyramiding add failed: {resp}")
...
class PerformanceTracker:
    ...
    def record_trade(self, position: dict | Position, pnl: Decimal):
        ...
        entry_fee = Decimal(str(position.entry_price)) * Decimal(str(position.qty)) * self.trading_fee_percent
        exit_fee = Decimal(str(position.exit_price)) * Decimal(str(position.qty)) * self.trading_fee_percent
        ...
        if pnl_net > 0: self.wins += 1; self.gross_profit += pnl_net; self.consecutive_losses = 0
        else: self.losses += 1; self.gross_loss += abs(pnl_net); self.consecutive_losses += 1
        self.logger.info(
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "daily_pnl": f"{self._daily_pnl:.4f}", "consecutive_losses": self.consecutive_losses
        }
...
class TradingAnalyzer:
    ...
    def _safe_calculate(self, func: Callable, name: str, min_data_points: int = 0, *args, **kwargs) -> Any | None:
        ...
        try:
            ...
            if (isinstance(result, (pd.Series, pd.DataFrame)) and result.empty): return None
            return result
        except Exception as e:
            self.logger.error(f"[{self.symbol}] Error calculating indicator '{name}': {e}\n{traceback.format_exc()}")
            return None
    ...
    def _calculate_all_indicators(self):
        ...
        if cfg["indicators"].get("atr", False):
            self.df["ATR"] = ta.atr(self.df.high, self.df.low, self.df.close, length=isd["atr_period"])
    ...
    def calculate_fibonacci_pivot_points(self) -> None:
        ...
        if pivot_data:
            ...
            for key, value in pivot_data.items():
                try: self.indicator_values[key.upper()] = Decimal(str(value)).quantize(Decimal(f"1e-{price_precision}"), rounding=ROUND_DOWN)
                except InvalidOperation: self.logger.warning(f"[{self.symbol}] Could not convert pivot point value '{value}' to Decimal. Skipping.")

    def _get_indicator_value(self, key: str, default: Any = np.nan) -> Any:
        return self.indicator_values.get(key, default)

    async def _check_orderbook(self, orderbook_manager: AdvancedOrderbookManager) -> float:
        best_bid, best_ask = await orderbook_manager.get_best_bid_ask()
        if best_bid is None or best_ask is None: return 0.0
    ...
    async def generate_trading_signal(
        self, current_price: Decimal, orderbook_manager: AdvancedOrderbookManager,
        mtf_trends: dict[str, str], pybit_client: 'PybitTradingClient'
        ...
        if self.config["mtf_analysis"]["enabled"]:
            for htf_interval in self.config["mtf_analysis"]["higher_timeframes"]:
                ...
                if kline_key not in pybit_client.kline_data_managers:
                    pybit_client.kline_data_managers[kline_key] = KlineDataManager(self.symbol, htf_interval, 1000, self.logger)
                    await pybit_client.kline_data_managers[kline_key].initialize(pybit_client)
                higher_tf_df = await pybit_client.kline_data_managers[kline_key].get_df()
                trend = indicators._get_mtf_trend(higher_tf_df, htf_interval, self.config["mtf_analysis"]["trend_indicators"][0])
                if trend != "UNKNOWN": mtf_trends[f"{htf_interval}_{self.config['mtf_analysis']['trend_indicators'][0]}"] = trend
                await asyncio.sleep(self.config["mtf_analysis"]["mtf_request_delay_seconds"])
        ...
        self.logger.info(f"[{self.symbol}] Generated Signal: {final_signal} (Score: {signal_score:.2f}, Threshold: {threshold:.2f})")
        return final_signal, signal_score, signal_breakdown

# --- Main Bot Logic ---
def check_manual_pause() -> bool:
    if Path(PAUSE_FILE).exists():
        global_logger.warning(f"{NEON_YELLOW}Manual pause file '{PAUSE_FILE}' detected. Trading halted.{RESET}")
        return True
    return False
...
def in_allowed_session(config: dict) -> bool:
    ...
    for start_str, end_str in config["session_filter"]["utc_allowed"]:
        ...
        if start_time < end_time:
            ...
            if current_time >= start_time or current_time <= end_time: return True
    return False
...
async def main() -> None:
    ...
    if not pybit_client.enabled:
        logger.error(f"{NEON_RED}Pybit client is not enabled or failed to initialize. Cannot proceed with live trading. Exiting.{RESET}")
        return

    await pybit_client.kline_data_managers[f"{config['symbol']}_{config['interval']}"].initialize(pybit_client)
    await pybit_client.start_websocket_manager()
    await asyncio.sleep(WS_RECONNECT_DELAY_SECONDS)

    position_manager = PositionManager(config, logger, config["symbol"], pybit_client, PerformanceTracker(logger, config))
    performance_tracker = PerformanceTracker(logger, config)
    analyzer = TradingAnalyzer(pd.DataFrame(), config, logger, config["symbol"])
    exec_sync = ExchangeExecutionSync(config["symbol"], pybit_client, config, position_manager, performance_tracker) if config["execution"]["live_sync"]["enabled"] else None
    heartbeat = PositionHeartbeat(config["symbol"], pybit_client, config, position_manager) if config["execution"]["live_sync"]["heartbeat"]["enabled"] else None

    while not pybit_client.stop_event.is_set():
        try:
            if pybit_client.is_circuit_breaker_tripped() or check_manual_pause() or performance_tracker.consecutive_losses >= config['risk_guardrails']['consecutive_losses_limit']:
                logger.critical(f"{NEON_RED}Trading halted by risk-management or manual pause. Cooling down.{RESET}")
            ...
            if not in_allowed_session(config):
                logger.info(f"{NEON_BLUE}Outside allowed trading sessions. Holding.{RESET}")
                await asyncio.sleep(config["loop_delay"])
                continue

            logger.info(f"{NEON_PURPLE}--- New Analysis Loop ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---{RESET}")

            df = await pybit_client.kline_data_managers[f"{config['symbol']}_{config['interval']}"].get_df()
            if df.empty:
                logger.warning(f"[{config['symbol']}] Kline Data Manager DataFrame is empty. Waiting for data.")
                await asyncio.sleep(config["loop_delay"])
                continue
            ...
            ob_data = await pybit_client.orderbook_data_manager.get_depth(config['orderbook_limit'])
            if ob_data and get_spread_bps_from_levels({"bids": ob_data[0], "asks": ob_data[1]}) > float(config['risk_guardrails']['spread_filter_bps']):
                logger.warning("Spread too high. Holding.")
                await asyncio.sleep(config["loop_delay"])
                continue
            if config['risk_guardrails']['ev_filter_enabled'] and expected_value(performance_tracker) <= 0:
            ...
            trading_signal, signal_score, signal_breakdown = await analyzer.generate_trading_signal(current_price, pybit_client.orderbook_data_manager, mtf_trends, pybit_client)
            ...
            if trading_signal in ("BUY", "SELL"):
                conviction = min(1.0, max(0.0, (abs(signal_score) - config["signal_score_threshold"]) / config["signal_score_threshold"]))
                if abs(signal_score) >= config["signal_score_threshold"]:
                    logger.info(f"Strong {trading_signal} signal detected! Score: {signal_score:.2f}")
                    await position_manager.open_position(trading_signal, current_price, atr_value, conviction)

            if exec_sync: await exec_sync.poll()
            if heartbeat: await heartbeat.tick()
...
if __name__ == "__main__":
    ...
    try:
        ...
        global_logger.critical(f"{NEON_RED}Critical error during bot startup or top-level execution: {e}{RESET}", exc_info=True)
        sys.exit(1)
