...
class PositionManager:
    ...
    def sync_positions_from_exchange(self):
        ...
        for ex_pos in exchange_positions:
            ...
            existing_pos = next(
                (
                    ...
                    if p.get("position_id") == ex_pos["positionIdx"]
                    and p.get("side") == side
                ),
                None,
            )
                ...
                new_open_positions.append(
                    {
                        ...
                        "position_id": ex_pos.get(
                            "positionId", str(ex_pos["positionIdx"])
                        ),  # Use positionIdx as ID if no explicit positionId
        ...
        for tracked_pos in self.open_positions:
            is_still_open = any(
                ex_pos["positionIdx"] == tracked_pos.get("position_id")
                ...
                if (side == "BUY" and current_price >= profit_trigger_level) or (
                    ...
                    if side == "BUY":
                        ...
                        if new_trailing_stop_candidate > current_stop_loss_on_exchange:
                            ...
                            if (
                                updated_sl_value > current_stop_loss_on_exchange
                            ):  # Only update if it actually moves further into profit
                    ...
                    if should_update_sl:
                        ...
                        if tpsl_update_result:
                            ...
                            position["trailing_stop_price"] = (
                                updated_sl_value  # Store the TSL value
                            )
                            self.logger.info(
...
class BybitWebSocketManager:
    ...
    def __init__(self, config: dict[str, Any], logger: logging.Logger):
        ...
        self.latest_trades: deque = deque(
            maxlen=config.get("orderbook_limit", 50)
    ...
    def _on_message_public(self, ws, message):
        ...
        if op == "subscribe":
            ...
            self._update_ticker(
                data["data"][0]
            )  # Tickers usually come as a list of one item
        else:
            self.logger.debug(
        ...
        while not self._stop_event.is_set() and retries < WS_RECONNECT_ATTEMPTS:
            try:
                ...
                ws.run_forever(
                    ...
                    },  # For some environments, might need to ignore cert validation
                )
                ...
                for index, row in df_new.iterrows():
                    if index in self.latest_klines.index:
                        ...
                        self.latest_klines = pd.concat(
                            [self.latest_klines, df_new]
                        )  # Appending just one row more efficient
                        self.logger.debug(
    ...
    def wait_for_initial_data(self, timeout: int = 30):
        ...
        private_ready = self.initial_private_data_received.wait(
            timeout
        )  # Private might take longer
    ...
    while True:
        try:
            ...
            has_buy_position = any(
                p["side"] == "Buy" for p in position_manager.get_open_positions()
            )
            has_sell_position = any(
                p["side"] == "Sell" for p in position_manager.get_open_positions()
            )
