...
def make_v5_request(
    ...
    # Sort params for signature if GET request
    if method == 'GET':
...
def get_category(symbol: str) -> str:
    ...
    if symbol.endswith('USDT') or symbol.endswith('USDC'):
        return "linear"
    elif symbol.endswith('USD'): # Inverse contracts
        return "inverse"
    else: # Spot market by default if no clear suffix
        return "spot"
...
def get_account_balance(config: BotConfig, coin: Optional[str] = None) -> float:
    """
    ...
    for account_type in account_type_priority:
        ...
        resp = make_v5_request("/v5/account/wallet-balance", "GET", params, config)

        if resp.get("retCode") != 0:
            log_neon(
                f"Wallet balance query failed for accountType={account_type}: {resp.get('retMsg', 'Unknown error')}",
                NEON_YELLOW, "WARNING"
            )
            continue
...
def calculate_market_indicators(df: pd.DataFrame) -> Dict[str, float]:
    ...
    if len(df) < 50: 
        log_neon("Not enough data for full indicator calculation.", NEON_YELLOW, "WARNING")
        return {}
...
class OrderManager:
    ...
    def place_market_buy(self, qty: Decimal) -> Dict[str, Any]:
        ...
        order_params = {
            ...
            "orderType": "Market",
            "qty": str(qty),
            "orderLinkId": f"GRID_MKT_BUY_{int(time.time()*1000)}",
            "positionIdx": 0
        }
    ...
    if config.initial_market_buy:
        ...
        if current_price_for_initial_buy == 0.0:
            ...
            if temp_balance == 0.0 and not config.test_mode:
                log_neon("Cannot perform initial market buy with zero available balance.", NEON_RED, "ERROR")
            else:
