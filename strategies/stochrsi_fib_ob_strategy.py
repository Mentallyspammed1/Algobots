...
class StochRSI_Fib_OB_Strategy(StrategyTemplate):
    ...
    def generate_signals(self,
        ...
        if df.empty or len(df) < min_rows_needed:
            self.logger.debug(f"{COLOR_YELLOW}DataFrame too short or empty for signal generation. Required at least {min_rows_needed} rows. Skipping signal generation.{COLOR_RESET}")
            return signals

        required_cols = ['stoch_k', 'stoch_d', 'sma', 'ehlers_fisher', 'ehlers_fisher_signal', 'ehlers_supersmoother', 'close']
        missing_cols = [col for col in required_cols if col not in df.columns]
        if missing_cols:
            self.logger.error(f"{COLOR_RED}DataFrame is missing crucial columns for signal generation: {missing_cols}. Skipping signal generation.{COLOR_RESET}")
            return signals
        ...
        if enable_fib_pivot_actions:
            ...
            if current_trend_is_up:
                ...
                if not fib_long_confirm:
                    self.logger.debug(f"Buy signal considered, but price {latest_close:.2f} not near any Fib support level within {fib_entry_confirm_dec*100:.3f}%. (Current: {pivot_support_levels})")
            ...
            if current_trend_is_down:
                ...
                if not fib_short_confirm:
                    self.logger.debug(f"Sell signal considered, but price {latest_close:.2f} not near any Fib resistance level within {fib_entry_confirm_dec*100:.3f}%. (Current: {pivot_resistance_levels})")
        else:
            fib_long_confirm = True
            fib_short_confirm = True
            ...
            if confluence_found_buy:
                ...
                if stoch_condition_met:
                    if fisher_buy_signal or fisher_long_bias:
                        ...
                        self.logger.info(f"{COLOR_GREEN}BUY Signal ({stoch_type_str}) at {latest_close:.2f}. {confluence_reason_buy}. {fib_reason_part}. Ehlers Fisher confirmed.{COLOR_RESET}")
                    else:
                        self.logger.debug(f"Buy signal considered, but Ehlers Fisher not confirming (Fisher: {latest_ehlers_fisher:.2f}, Signal: {latest_ehlers_fisher_signal:.2f}, Prev Fisher: {prev_ehlers_fisher:.2f}).")
                else:
                    self.logger.debug(f"Buy signal considered, but StochRSI condition not met (K: {latest_stoch_k:.2f}, D: {latest_stoch_d:.2f}).")
            else:
                self.logger.debug(f"Buy signal considered, but no confluence found: {confluence_reason_buy}")
        else:
            self.logger.debug(f"Buy signal skipped: Trend Up={current_trend_is_up}, FibConfirm={fib_long_confirm}.")
            ...
            if confluence_found_sell:
                ...
                if stoch_condition_met:
                    if fisher_sell_signal or fisher_short_bias:
                        ...
                        self.logger.info(f"{COLOR_RED}SELL Signal ({stoch_type_str}) at {latest_close:.2f}. {confluence_reason_sell}. {fib_reason_part}. Ehlers Fisher confirmed.{COLOR_RESET}")
                    else:
                        self.logger.debug(f"Sell signal considered, but Ehlers Fisher not confirming (Fisher: {latest_ehlers_fisher:.2f}, Signal: {latest_ehlers_fisher_signal:.2f}, Prev Fisher: {prev_ehlers_fisher:.2f}).")
                else:
                    self.logger.debug(f"Sell signal considered, but StochRSI condition not met (K: {latest_stoch_k:.2f}, D: {latest_stoch_d:.2f}).")
            else:
                self.logger.debug(f"Sell signal considered, but no confluence found: {confluence_reason_sell}")
        else:
            self.logger.debug(f"Sell signal skipped: Trend Down={current_trend_is_down}, FibConfirm={fib_short_confirm}.")
    ...
    def generate_exit_signals(self,
        ...
        min_rows_needed = max(SMA_PERIOD, stoch_k_period, stoch_d_period, EHLERS_FISHER_SIGNAL_PERIOD, 2)
        if df.empty or len(df) < min_rows_needed:
            self.logger.warning(f"{COLOR_YELLOW}DataFrame too short or empty for exit signal generation. Required at least {min_rows_needed} rows.{COLOR_RESET}")
            return exit_signals

        required_cols = ['stoch_k', 'stoch_d', 'sma', 'ehlers_fisher', 'ehlers_fisher_signal', 'ehlers_supersmoother', 'close']
        missing_cols = [col for col in required_cols if col not in df.columns]
        if missing_cols:
            self.logger.error(f"{COLOR_RED}DataFrame is missing crucial columns for exit signal generation: {missing_cols}{COLOR_RESET}")
            return exit_signals
        ...
        if enable_fib_pivot_actions:
            if not pivot_support_levels and not pivot_resistance_levels:
                self.logger.warning(f"{COLOR_YELLOW}Fib Pivot exit check enabled, but no pivot levels calculated. Exit check may be impacted.{COLOR_RESET}")
            ...
            if current_position_side == 'BUY':
                for name, price in pivot_resistance_levels.items():
                    if price > Decimal('0') and abs(latest_close - price) / price <= fib_exit_warn_dec:
                        ...
                        if fib_exit_action == 'exit':
                            ...
                            self.logger.info(f"{COLOR_YELLOW}Fibonacci Exit Triggered (BUY position): {fib_exit_reason}{COLOR_RESET}")
                        else:
                            self.logger.warning(f"{COLOR_YELLOW}Fibonacci Exit Warning (BUY position): {fib_exit_reason}{COLOR_RESET}")
                        break
                ...
                for name, price in pivot_support_levels.items():
                    if price > Decimal('0') and abs(latest_close - price) / price <= fib_exit_warn_dec:
                        ...
                        if fib_exit_action == 'exit':
                            fib_exit_triggered = True
                            self.logger.info(f"{COLOR_YELLOW}Fibonacci Exit Triggered (SELL position): {fib_exit_reason}{COLOR_RESET}")
                        else:
        ...
        if current_position_side == 'BUY':
            ...
            if (stoch_exit_condition or ehlers_exit_condition or trend_reversal_buy_exit or confluence_found_exit or fib_exit_triggered):
                ...
                self.logger.info(f"{COLOR_CYAN}EXIT BUY Signal at {latest_close:.2f}. Reason: {exit_reason_str}{COLOR_RESET}")
            ...
            if (stoch_exit_condition or ehlers_exit_condition or trend_reversal_sell_exit or confluence_found_exit or fib_exit_triggered):
                ...
                self.logger.info(f"{COLOR_CYAN}EXIT SELL Signal at {latest_close:.2f}. Reason: {exit_reason_str}{COLOR_RESET}")
        else:
            self.logger.debug(f"No active position to generate exit signals for, or unknown position side: {current_position_side}.")
