...
class StrategyConfig:
    ...
    max_inventory_deviation_base: float = (
        0.1  # Max BTC deviation before aggressive rebalancing
    )
    ...
    max_loss_per_day_usd: float = (
        500.0  # Simple daily loss limit (requires PnL tracking)
    )
    ...
    cancel_stale_orders_after_sec: int = (
        60  # Cancel orders older than this (if not filled)
    )
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("marketmaking.log"), logging.StreamHandler()],
)
...
class MockExchange:
    ...
    def get_open_orders(self, symbol: str) -> list[Order]:
        ...
        return [
            ...
            if order.symbol == symbol and order.status in ["open", "partial_fill"]
        ]
    ...
    def _process_order(self, order: Order):
        ...
        if order.side == "buy":
            ...
            if order.price >= order_book["ask"]:
                ...
                order.status = (
                    "filled" if order.filled_amount == order.amount else "partial_fill"
                )
...
class MarketMakingStrategy:
    ...
    def _calculate_target_prices(self, bid: float, ask: float) -> tuple[float, float]:
        ...
        if abs(deviation) > self.config.max_inventory_deviation_base:
            ...
            # If we have too little base currency (short), raise buy price, lower sell price (to buy more)
                ...
                adjustment = (
                    abs(deviation) * self.config.inventory_bias_factor * mid_price
                )
    ...
    def _manage_orders(self, target_buy_price: float, target_sell_price: float):
        ...
        for order in open_orders:
            if order.status == "open" and (
                (order.side == "buy" and order.price > target_buy_price)
                or (order.side == "sell" and order.price < target_sell_price)
                or (
                    time.time() - order.timestamp
                    > self.config.cancel_stale_orders_after_sec
                )
            ):
        ...
        if num_sell_orders < self.config.max_open_orders_per_side:
            ...
            if (
                self.exchange.get_balance(self.config.base_currency)
                >= self.config.order_size_base
            ):
                self.exchange.place_order(
                    self.config.symbol,
    ...
    def _check_risk_limits(self) -> bool:
        ...
        """
        # --- Max Exposure Check ---
        current_base_value_usd = self.current_position_base * (
            self.last_mid_price if self.last_mid_price else 0
        )
        current_quote_value_usd = self.exchange.get_balance(self.config.quote_currency)
        total_exposure_usd = (
            current_base_value_usd + current_quote_value_usd
        )  # Simplified, often includes open orders
