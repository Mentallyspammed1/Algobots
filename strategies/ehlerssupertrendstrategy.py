...
class EhlersSupertrendStrategy:
    ...
    def generate_signals(
        ...
        **kwargs,
        ...
        min_required_bars = (
            max(self.ehlers_period, self.supertrend_period, self.sma_period) + 2
        )
        ...
        if any(
            col not in last_bar or pd.isna(last_bar[col]) for col in required_cols
        ) or any(
        ...
        # --- Signal Logic ---
        is_supertrend_bullish_turn = (
            last_bar["supertrend_direction"] == 1
            and prev_bar["supertrend_direction"] == -1
        )
        is_fisher_bullish_cross = (
            last_bar["ehlers_fisher"] > last_bar["ehlers_signal"]
            and prev_bar["ehlers_fisher"] <= prev_bar["ehlers_signal"]
        )
