...
class SMA_Crossover_Strategy(StrategyTemplate):
    ...
    def _calculate_sma(self, df: pd.DataFrame, length: int) -> pd.Series:
        if 'close' not in df.columns:
            self.logger.error("DataFrame must contain a 'close' column for SMA calculation.")
    ...
    def generate_signals(self,
        ...
        if df.empty or len(df) < SMA_PERIOD + 1: # Need at least SMA_PERIOD + 1 for crossover
            self.logger.warning(f"{COLOR_YELLOW}DataFrame too short for SMA Crossover signal generation. Required at least {SMA_PERIOD + 1} rows.{COLOR_RESET}")
            return signals
        ...
        if pd.isna(df['sma'].iloc[-1]) or pd.isna(df['sma'].iloc[-2]):
            self.logger.debug("SMA values are NaN for recent candles. Skipping signal generation.")
            return signals
        ...
        if prev_close < prev_sma and latest_close > latest_sma:
            signals.append(('BUY', latest_close, current_timestamp, {'strategy': 'SMA_Crossover'}))
            self.logger.info(f"{COLOR_GREEN}BUY Signal (SMA Crossover) at {latest_close:.2f}.{COLOR_RESET}")

        # Sell signal: Price crosses below SMA
        elif prev_close > prev_sma and latest_close < latest_sma:
            signals.append(('SELL', latest_close, current_timestamp, {'strategy': 'SMA_Crossover'}))
            self.logger.info(f"{COLOR_RED}SELL Signal (SMA Crossover) at {latest_close:.2f}.{COLOR_RESET}")
    ...
    def generate_exit_signals(self,
        ...
        if current_position_side == 'BUY':
            ...
            if prev_close > prev_sma and latest_close < latest_sma:
                exit_signals.append(('EXIT_BUY', latest_close, current_timestamp, {'strategy': 'SMA_Crossover_Exit'}))
                self.logger.info(f"{COLOR_CYAN}EXIT BUY Signal (SMA Crossover) at {latest_close:.2f}.{COLOR_RESET}")
        elif current_position_side == 'SELL':
            # Exit short: Price crosses above SMA
            if prev_close < prev_sma and latest_close > latest_sma:
                exit_signals.append(('EXIT_SELL', latest_close, current_timestamp, {'strategy': 'SMA_Crossover_Exit'}))
                self.logger.info(f"{COLOR_CYAN}EXIT SELL Signal (SMA Crossover) at {latest_close:.2f}.{COLOR_RESET}")
