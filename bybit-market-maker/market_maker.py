...
class MarketMaker:
    ...
    def _init_client(self):
        ...
        self.logger.info(f"Initialized Bybit client for {self.env_config['environment']}")
            ...
            for position in data:
                if position['symbol'] == self.symbol:
                    self.position = {
                        ...
                        'side': position['side'],
                        'avg_price': float(position['avgPrice']) if position['avgPrice'] else 0,
                        'mark_price': float(position['markPrice']) if position['markPrice'] else 0,
                        'unrealized_pnl': float(position['unrealisedPnl']) if position['unrealisedPnl'] else 0
                    }
                    self._calculate_inventory_ratio()

    def get_market_data(self) -> dict:
        """Fetch current market data"""
        if self.config['advanced']['enable_websocket'] and self.ws_manager and self.ws_manager.is_connected:
            return {
                'bid': self.bid_price,
                'ask': self.ask_price,
                'mid': self.mid_price,
            ...
            try:
                ...
                if orderbook['retCode'] == 0:
                    ...
                    if bids and asks:
                        ...
                        if len(self.price_history) > self.config['advanced']['price_history_length']:
                            self.price_history = self.price_history[-self.config['advanced']['price_history_length']:]
            ...
            try:
                ...
                if response['retCode'] == 0 and response['result']['list']:
                    ...
                    self.position = {
                        ...
                        'mark_price': float(position_data['markPrice']) if position_data['markPrice'] else 0,
                        'unrealized_pnl': float(position_data['unrealisedPnl']) if position_data['unrealisedPnl'] else 0
                    }
    ...
    def get_account_balance(self) -> float:
        ...
        try:
            response = self.client.get_wallet_balance(
                accountType="UNIFIED" if self.config['trading']['market_type'] == 'linear' else "SPOT"
            )
            ...
            if response['retCode'] == 0:
                ...
                for balance in balances:
                    if (self.config['trading']['market_type'] == 'linear' and balance['coin'] == 'USDT') or (self.config['trading']['market_type'] == 'spot' and balance['coin'] == self.symbol.replace('USDT', '')):
    ...
    def execute_market_making_cycle(self):
        ...
        try:
            ...
            if (datetime.now() - self.last_performance_log).seconds > self.config['monitoring']['performance_log_interval']:
    ...
    def close_position(self, position: dict):
        ...
        try:
            ...
            if response['retCode'] == 0:
                self.logger.info(f"Position closed: {side} {position['size']} {self.symbol}")
            else:
    ...
    def log_performance(self):
        ...
        try:
            ...
            self.logger.info(f"P&L: ${stats.get('total_pnl', 0):.2f} ({stats.get('roi', 0):.2f}%)")
            self.logger.info(f"Trades: {stats.get('total_trades', 0)} (Win Rate: {stats.get('win_rate', 0):.1f}%)")
            ...
            self.logger.info(f"Position: {self.position['size']} @ ${self.position['avg_price']:.2f}")
            self.logger.info(f"Inventory Ratio: {self.inventory_ratio:.2%}")
            self.logger.info("-" * 50)
            self.logger.info("ORDER STATUS")
            self.logger.info(f"Active Orders: {order_summary['total_orders']} (Buy: {order_summary['buy_orders']}, Sell: {order_summary['sell_orders']})")
    ...
    def run(self):
        ...
        self.logger.info(f"Trading {self.symbol} on {self.config['trading']['market_type']} market")
        self.running = True
