#!/bin/bash
# ----------------------------------------------------------------------
# SCRIPT: setup_whalewave_titan.sh - FIXED VERSION (Shell Escaping Corrected)
# ----------------------------------------------------------------------

PROJECT_DIR="whalewave-titan-pro-fixed"
SERVER_PORT=3000
...
{
  "name": "whalewave-titan-pro-fixed",
  "version": "7.0.0",
  "description": "Integrated Quantitative + AI Trading Engine",
  "main": "server.js",
  "type": "module",
  "scripts": {
    "start": "node server.js",
    "trade": "node engine.js"
  },
  "dependencies": {
    "axios": "^1.6.8",
    "chalk": "^5.3.0",
...
echo "⚙️ Generating config.json (Engine Configuration)..."
cat << 'EOF' > config.json
{
    "symbol": "BTCUSDT",
    "interval": "3",
    ...
    "indicators": {
        ...
        "wss_weights": {
            "trend_mtf_weight": 2.2, "trend_scalp_weight": 1.2, "momentum_normalized_weight": 1.8, "macd_weight": 1.0,
            "regime_weight": 0.8, "squeeze_vol_weight": 1.0, "liquidity_grab_weight": 1.5, "divergence_weight": 2.5,
...
# --- 5. GENERATE SERVER.JS (Express Backend) ---
echo "💻 Generating server.js (Express Backend)..."
cat << 'EOF' > server.js
...
function startEngine() {
    ...
    console.log("🚀 Starting Trading Engine (node engine.js)...");

    engineProcess = spawn('node', ['engine.js'], { stdio: ['inherit', 'pipe', 'pipe'] });
...
# --- 6. GENERATE ENGINE.JS (The Core Trading Logic - FIXED MODULES) ---
echo "🧠 Generating engine.js (Core Logic - FIXED)..."
cat << 'EOF' > engine.js
/**
...
// --- 📐 COMPLETE TECHNICAL ANALYSIS LIBRARY (Consolidated) ---
class TA {
    static safeArr(len) { return new Array(Math.floor(len)).fill(0); }
    static getFinalValue(data, key, precision = 2) {
        if (!data.closes || data.closes.length === 0) return { slope: 0, r2: 0 };
        const last = data.closes.length - 1;
        const value = data[key];
        if (Array.isArray(value)) return parseFloat(value[last] || 0);
        if (typeof value === 'object') {
            if (value.k) return parseFloat(value.k[last] || 0);
            if (value.hist) return parseFloat(value.hist[last] || 0);
            if (value.slope) return { slope: parseFloat(value.slope[last] || 0), r2: parseFloat(value.r2[last] || 0) };
        }
        return 0;
    }
    ...
    static marketRegime(closes, volatility, period = 50) {
        ...
        const avgVolValue = avgVol[volatility.length - 1] || 1;
...
class TradingEngine {
    ...
    async pushDashboardData(data, context, signal) {
        ...
        try {
            await this.dashboardClient.post('/api/dashboard', dashboardData);
        } catch (e) {
            // Error handling for failed push to dashboard (expected if server starts late)
        }
    }
}
...
# --- 7. GENERATE INDEX.HTML (Frontend Monitor) ---
echo "🌐 Generating index.html (Frontend Monitor)..."
cat << 'EOF' > index.html
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>🌊 WhaleWave Titan Monitor (Linked)</title>
    <style>
...
echo "---------------------------------------------------------"
echo "NEXT STEPS (Please do this IN ORDER):"
echo "1. IMPORTANT: Edit the .env file in the $PROJECT_DIR directory"
echo "   and replace 'YOUR_GEMINI_API_KEY_HERE' with your actual Gemini API Key."
echo "2. Run the command below to start the server and the engine:"
echo -e "\n  cd $PROJECT_DIR && npm start"
echo -e "\n3. Open your browser to: http://localhost:$SERVER_PORT"
echo "========================================================="

# Go back to the original directory (optional, for convenience)
cd ..
