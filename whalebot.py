...
class TradingAnalyzer:
    ...
    def analyze(self, current_price: Decimal, timestamp: str):
        ...
        indicator_values = {} # Store indicator values for signal generation

        if self.config["indicators"]["obv"]: # Respect config flags
            obv = self._calculate_obv()
            indicator_values["obv"] = obv.tail(3).tolist()
        if self.config["indicators"]["rsi"]:
            rsi = self._calculate_rsi()
            indicator_values["rsi"] = rsi.tail(3).tolist()
        if self.config["indicators"]["mfi"]:
            mfi = self._calculate_mfi()
            indicator_values["mfi"] = mfi.tail(3).tolist()
        if self.config["indicators"]["cci"]:
            cci = self._calculate_cci()
            indicator_values["cci"] = cci.tail(3).tolist()
        if self.config["indicators"]["wr"]:
            wr = self._calculate_williams_r()
            indicator_values["wr"] = wr.tail(3).tolist()
        if self.config["indicators"]["adx"]:
            adx = self._calculate_adx()
            indicator_values["adx"] = [adx] * 3 # Keep ADX in indicator_values for signal logic
        if self.config["indicators"]["adi"]:
            adi = self._calculate_adi()
            indicator_values["adi"] = adi.tail(3).tolist()
        if self.config["indicators"]["momentum"]:
            trend_data = self.determine_trend_momentum()
            indicator_values["mom"] = [trend_data] * 3 # Keep trend data for signal logic
        if self.config["indicators"]["sma_10"]: # Example for SMA, add config flag in config.json if needed
            sma = self._calculate_sma(10)
            indicator_values["sma"] = [self.df["close"].iloc[-1]] # Keep SMA for logging
        if self.config["indicators"]["psar"]:
            psar = self._calculate_psar()
            indicator_values["psar"] = psar.tail(3).tolist()
        if self.config["indicators"]["fve"]:
            fve = self._calculate_fve()
            indicator_values["fve"] = fve.tail(3).tolist()
        if self.config["indicators"]["macd"]:
            macd_df = self._calculate_macd()
            indicator_values["macd"] = macd_df.tail(3).values.tolist() if not macd_df.empty else
        if self.config["indicators"]["stoch_rsi"]:
            indicator_values["stoch_rsi_vals"] = self.stoch_rsi_df.tail(3).values.tolist() # Store Stoch RSI values
        if self.config["indicators"]["ema_alignment"]: # New: EMA Alignment
            ema_alignment_score = self._calculate_ema_alignment()
            indicator_values["ema_alignment"] = [ema_alignment_score] * 3 # Keep EMA alignment score
...
{NEON_BLUE}ATR:{RESET} {indicator_values.get("atr", [atr.iloc[-1]])[0]:.4f if atr is not None and not atr.empty else 'N/A'}
{NEON_BLUE}Trend:{RESET} {indicator_values.get("mom", [{"trend": "N/A"}])[0].get("trend", "N/A")} (Strength: {indicator_values.get("mom", [{"strength": 0}]) [0].get("strength", 0):.2f})
"""
        ...
        if self.config["indicators"]["ema_alignment"]: # Log EMA Alignment separately
            ema_alignment_score = indicator_values.get("ema_alignment",)[0] # Default to 0.0 if missing
        ...
        if self.config["indicators"]["stoch_rsi"]: # Log Stoch RSI separately as it has k/d lines
            stoch_rsi_vals = indicator_values.get("stoch_rsi_vals",)
...
"""
        ...
        signal, confidence, conditions_met = self.generate_trading_signal(indicator_values, current_price) # Get conditions met
