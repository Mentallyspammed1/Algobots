...
const NEW_TRADE_HIGHLIGHT_DURATION = 1500;
...
_c = LoadingRows;
function RecentTrades({ symbol, className }) {
    _s();
    ...
    const connectWebSocket = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "RecentTrades.useCallback[connectWebSocket]": ()=>{
            if (!symbol || !isMounted.current) return;
            cleanup();
            setWsStatus(retryCount > 0 ? 'reconnecting' : 'connecting');
            if (retryCount === 0) {
                setIsLoading(true);
                setTrades([]); // Clear trades on first connection attempt
            }
            const ws = new WebSocket(__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$bybit$2d$api$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BYBIT_WEBSOCKET_URL"]);
            wsRef.current = ws;
            ws.onopen = ({
                "RecentTrades.useCallback[connectWebSocket]": ()=>{
                    ...
                    // Add a timeout to handle cases where connection is open but no data is received
                    const connectionTimeout = setTimeout({
                        "RecentTrades.useCallback[connectWebSocket].connectionTimeout": ()=>{
                            if (wsRef.current === ws && trades.length === 0 && isMounted.current) {
                                setError('Connection timed out. No data received.');
                                ws.close();
                            }
                        }
                    }["RecentTrades.useCallback[connectWebSocket].connectionTimeout"], __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$bybit$2d$api$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["DEFAULT_REQUEST_TIMEOUT"]);
                    timeoutIds.current.push(connectionTimeout);
                }
            })["RecentTrades.useCallback[connectWebSocket]"];
            ws.onmessage = ({
                "RecentTrades.useCallback[connectWebSocket]": (event)=>{
                    if (!isMounted.current) return;
                    const data = JSON.parse(event.data);
                    if (data.topic && data.topic.startsWith('publicTrade') && Array.isArray(data.data)) {
                        const newTrades = data.data.map({
                            "RecentTrades.useCallback[connectWebSocket].newTrades": (t)=>({
                                    execId: t.i,
                                    price: t.p,
                                    qty: t.v,
                                    side: t.S,
                        ...
                        }["RecentTrades.useCallback[connectWebSocket].newTrades"]);
                        setTrades({
                            "RecentTrades.useCallback[connectWebSocket]": (prev)=>[
                                    ...newTrades,
                                    ...prev
                                ].slice(0, MAX_VISIBLE_TRADES)
                        }["RecentTrades.useCallback[connectWebSocket]"]);
                        const newIds = new Set(newTrades.map({
                            "RecentTrades.useCallback[connectWebSocket]": (t)=>t.execId
                        }["RecentTrades.useCallback[connectWebSocket]"]));
                        setNewTradeIds(newIds);
                        if (isLoading) setIsLoading(false);
                        const highlightTimeout = setTimeout({
                            "RecentTrades.useCallback[connectWebSocket].highlightTimeout": ()=>{
                                if (isMounted.current) setNewTradeIds({
                                    "RecentTrades.useCallback[connectWebSocket].highlightTimeout": (current)=>{
                                        const updated = new Set(current);
                                        newIds.forEach({
                                            "RecentTrades.useCallback[connectWebSocket].highlightTimeout": (id)=>updated.delete(id)
                                        }["RecentTrades.useCallback[connectWebSocket].highlightTimeout"]);
                                        return updated;
                                    }
                                }["RecentTrades.useCallback[connectWebSocket].highlightTimeout"]);
                            }
                        }["RecentTrades.useCallback[connectWebSocket].highlightTimeout"], NEW_TRADE_HIGHLIGHT_DURATION);
            ...
            ws.onclose = ({
                "RecentTrades.useCallback[connectWebSocket]": ()=>{
                    if (wsRef.current !== ws || !isMounted.current) return;
                    console.log(`WebSocket disconnected for ${symbol}`);
                    if (retryCount < MAX_RETRY_ATTEMPTS) {
                        const delay = RETRY_DELAY_BASE * Math.pow(2, retryCount);
                        console.log(`Attempting to reconnect in ${delay}ms (attempt ${retryCount + 1})`);
                        const reconnectTimeout = setTimeout({
                            "RecentTrades.useCallback[connectWebSocket].reconnectTimeout": ()=>{
                                if (isMounted.current) {
                                    setRetryCount({
                                        "RecentTrades.useCallback[connectWebSocket].reconnectTimeout": (prev)=>prev + 1
                                    }["RecentTrades.useCallback[connectWebSocket].reconnectTimeout"]);
                                    connectWebSocket();
                                }
                            }
                        }["RecentTrades.useCallback[connectWebSocket].reconnectTimeout"], delay);
                        timeoutIds.current.push(reconnectTimeout);
                        setWsStatus('reconnecting');
                    } else {
                        setError(`Failed to connect after ${MAX_RETRY_ATTEMPTS} attempts. Please refresh manually.`);
                        setWsStatus('disconnected');
                        setIsLoading(false);
                    }
                }
    ...
    const handleManualReconnect = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "RecentTrades.useCallback[handleManualReconnect]": ()=>{
            setRetryCount(0);
            setError(null);
            connectWebSocket();
        }
...
_s(RecentTrades, "T8VtzGJh1nqsXwVUVcH8p/cFHQw=");
_c1 = RecentTrades;
var _c, _c1;
...
"[project]/src/lib/data:e4cc2e [app-client] (ecmascript) <text/javascript>": ((__turbopack_context__) => {
"use strict";
...
;
var getAiTradingSignal = /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$action$2d$client$2d$wrapper$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["createServerReference"])("70c6125dbbad3595350503c517b30b35b439281e36", __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$action$2d$client$2d$wrapper$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["callServer"], void 0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$action$2d$client$2d$wrapper$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["findSourceMapURL"], "getAiTradingSignal"); //# sourceMappingURL=data:application/json;base64,
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(module, globalThis.$RefreshHelpers$);
}
}}),
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$lucide$2d$react$2f$dist$2f$esm$2f$icons$2f$chart$2d$no$2d$axes$2d$column$2d$increasing$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__$3c$export__default__as__BarChart$3e$__ = __turbopack_context__.i("[project]/node_modules/lucide-react/dist/esm/icons/chart-no-axes-column-increasing.js [app-client] (ecmascript) <export default as BarChart>");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$data$3a$e4cc2e__$5b$app$2d$client$5d$__$28$ecmascript$29$__$3c$text$2f$javascript$3e$__ = __turbopack_context__.i("[project]/src/lib/data:e4cc2e [app-client] (ecmascript) <text/javascript>");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$components$2f$ui$2f$skeleton$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/components/ui/skeleton.tsx [app-client] (ecmascript)");
...
function AiSignal({ symbol, timeframe, indicatorSettings }) {
    ...
    const handleGenerateSignal = async ()=>{
        setIsLoading(true);
        setAnalysis(null);
        const result = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$data$3a$e4cc2e__$5b$app$2d$client$5d$__$28$ecmascript$29$__$3c$text$2f$javascript$3e$__["getAiTradingSignal"])(symbol, timeframe, indicatorSettings);
...
"[project]/src/lib/data:c3475b [app-client] (ecmascript) <text/javascript>": ((__turbopack_context__) => {
"use strict";
...
{
__turbopack_context__.s({
    "AtrResultSchema": (()=>AtrResultSchema),
    "AwesomeOscillatorResultSchema": (()=>AwesomeOscillatorResultSchema),
    "BollingerBandsResultSchema": (()=>BollingerBandsResultSchema),
    "CciResultSchema": (()=>CciResultSchema),
    "IchimokuCloudResultSchema": (()=>IchimokuCloudResultSchema),
    "IndicatorDataSchema": (()=>IndicatorDataSchema),
    "IndicatorSettings": (()=>IndicatorSettings),
    "MacdResultSchema": (()=>MacdResultSchema),
    "MfiResultSchema": (()=>MfiResultSchema),
    "ObvResultSchema": (()=>ObvResultSchema),
    "RocResultSchema": (()=>RocResultSchema),
    "RsiResultSchema": (()=>RsiResultSchema),
    "SmaResultSchema": (()=>SmaResultSchema),
    "StochasticResultSchema": (()=>StochasticResultSchema),
    "SupertrendResultSchema": (()=>SupertrendResultSchema),
    "WilliamsRResultSchema": (()=>WilliamsRResultSchema),
...
const SupertrendResultSchema = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$lib$2f$index$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["z"].object({
    ...
    upperBand: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$lib$2f$index$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["z"].number(),
    lowerBand: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$lib$2f$index$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["z"].number()
});
const IndicatorDataSchema = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$lib$2f$index$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["z"].object({
    rsi: RsiResultSchema.nullable(),
    macd: MacdResultSchema.nullable(),
    bollingerBands: BollingerBandsResultSchema.nullable(),
    stochastic: StochasticResultSchema.nullable(),
    atr: AtrResultSchema.nullable(),
    obv: ObvResultSchema.nullable(),
    williamsR: WilliamsRResultSchema.nullable(),
    cci: CciResultSchema.nullable(),
    roc: RocResultSchema.nullable(),
    mfi: MfiResultSchema.nullable(),
    awesomeOscillator: AwesomeOscillatorResultSchema.nullable(),
    ichimokuCloud: IchimokuCloudResultSchema.nullable(),
    sma: SmaResultSchema.nullable(),
    supertrendFast: SupertrendResultSchema.nullable(),
    supertrendSlow: SupertrendResultSchema.nullable()
});
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$components$2f$ui$2f$card$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/components/ui/card.tsx [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$components$2f$ui$2f$skeleton$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/components/ui/skeleton.tsx [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$data$3a$c3475b__$5b$app$2d$client$5d$__$28$ecmascript$29$__$3c$text$2f$javascript$3e$__ = __turbopack_context__.i("[project]/src/lib/data:c3475b [app-client] (ecmascript) <text/javascript>");
        ...
        "IndicatorSuite.useEffect": ()=>{
            const fetchData = {
                "IndicatorSuite.useEffect.fetchData": async ()=>{
                    setIsLoading(true);
                    const indicatorData = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$data$3a$c3475b__$5b$app$2d$client$5d$__$28$ecmascript$29$__$3c$text$2f$javascript$3e$__["getIndicatorData"])(symbol, timeframe, settings);
...
//# sourceMappingURL=src_a92d7d1a._.js.map