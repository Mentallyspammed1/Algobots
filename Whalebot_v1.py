    ...
    for retry in range(MAX_API_RETRIES):
        try:
            ...
            if response.status_code in RETRY_ERROR_CODES:
                if logger:
                    logger.warning(
                        f"{NEON_YELLOW}Rate limit or server error, retrying {retry + 1}/{MAX_API_RETRIES}...{RESET}"
                    )
...
def fetch_current_price(
    ...
    if (
        ...
        tickers = response_data["result"].get(
            "list",
        )
...
) -> dict | None:  # Function to fetch order book
...
class TradingAnalyzer:
    def __init__(
        ...
        )  # Select weight set dynamically
        self.stoch_rsi_df = self._calculate_stoch_rsi()
        self.indicator_values = {}  # Initialize indicator_values here

    def _select_weight_set(
        self,
    ):  # Function to select weight set based on volatility (ATR)
        atr_value = (
            self._calculate_atr(window=14).iloc[-1]
            if not self.df.empty and len(self.df) >= 14
            else 0
        )
        if (
            atr_value > self.config["atr_change_threshold"]
        ):  # Example threshold for high volatility
            return self.weight_sets.get(
                "high_volatility", self.weight_sets["low_volatility"]
            )  # Default to low_volatility if high_volatility not defined
        return self.weight_sets["low_volatility"]  # Default to low volatility

    def _calculate_sma(self, window: int) -> pd.Series:
        if "close" not in self.df.columns:
            self.logger.error(
                f"{NEON_RED}Missing 'close' column for SMA calculation{RESET}"
            )
            return pd.Series(dtype=float)
        return self.df["close"].rolling(window=window).mean()

    def _calculate_ema_alignment(
        self,
    ) -> float:  # New: Function to calculate EMA alignment score
        ...
        if (
            latest_short_ema > latest_long_ema and current_price > latest_short_ema
        ):  # Bullish alignment
    ...
    def _calculate_momentum(self, period: int = 10) -> pd.Series:
        if "close" not in self.df.columns:
            self.logger.error(
                f"{NEON_RED}Missing 'close' column for Momentum calculation{RESET}"
            )
            return pd.Series(dtype=float)
        return (
    ...
    def _calculate_cci(self, window: int = 20, constant: float = 0.015) -> pd.Series:
        ...
        for col in required_columns:
            if col not in self.df.columns:
                self.logger.error(
                    f"{NEON_RED}Missing '{col}' column for CCI calculation{RESET}"
                )
                return pd.Series(dtype=float)
    ...
    def _calculate_williams_r(self, window: int = 14) -> pd.Series:
        ...
        for col in required_columns:
            if col not in self.df.columns:
                self.logger.error(
                    f"{NEON_RED}Missing '{col}' column for Williams %R calculation{RESET}"
                )
                return pd.Series(dtype=float)
    ...
    def _calculate_mfi(self, window: int = 14) -> pd.Series:
        ...
        for col in required_columns:
            if col not in self.df.columns:
                self.logger.error(
                    f"{NEON_RED}Missing '{col}' column for MFI calculation{RESET}"
                )
                return pd.Series(dtype=float)
        typical_price = (self.df["high"] + self.df["low"] + self.df["close"]) / 3
    ...
    def _calculate_atr(self, window: int = 20) -> pd.Series:
        ...
        for col in required_columns:
            if col not in self.df.columns:
                self.logger.error(
                    f"{NEON_RED}Missing '{col}' column for ATR calculation{RESET}"
                )
                return pd.Series(dtype=float)
    ...
    def _calculate_momentum_ma(self) -> None:
        ...
        self.df["momentum_ma_short"] = (
            self.df["momentum"].rolling(window=self.config["momentum_ma_short"]).mean()
        )
        self.df["momentum_ma_long"] = (
            self.df["momentum"].rolling(window=self.config["momentum_ma_long"]).mean()
        )
        self.df["volume_ma"] = (
    ...
    def _calculate_ema(self, window: int) -> pd.Series:
        if "close" not in self.df.columns:
            self.logger.error(
                f"{NEON_RED}Missing 'close' column for EMA calculation{RESET}"
            )
            return pd.Series(dtype=float)
    ...
    def _calculate_adx(self, window: int = 14) -> float:
        ...
        for col in required_columns:
            if col not in df.columns:
                self.logger.error(
                    f"{NEON_RED}Missing '{col}' column for ADX calculation{RESET}"
                )
                return 0.0
        df["TR"] = pd.concat(
            [
    ...
    def _calculate_adi(self) -> pd.Series:
        ...
        for col in required_columns:
            if col not in self.df.columns:
                self.logger.error(
                    f"{NEON_RED}Missing '{col}' column for ADI calculation{RESET}"
                )
                return pd.Series(dtype=float)
    ...
    def _calculate_psar(self, acceleration=0.02, max_acceleration=0.2) -> pd.Series:
        ...
        if self.df.empty:
            return psar  # Return empty if no data
    ...
    def _calculate_fve(self) -> pd.Series:  # Example FVE calculation - improved example
        ...
        try:
            ...
            sma_obv = self._calculate_sma(
                window=20, series=obv
            )  # Need to adjust _calculate_sma to accept series
            atr = self._calculate_atr()
            ...
            normalized_obv = (
                obv - sma_obv
            ) / sma_obv.abs()  # Normalize OBV by SMA_OBV - handles zero SMA_OBV
            normalized_atr = (
                atr - sma_atr
            ) / sma_atr.abs()  # Normalize ATR by SMA_ATR - handles zero SMA_ATR

            fve = (
                sma_close + normalized_obv - normalized_atr
            )  # Example combination - you can adjust this formula
            return fve
        except Exception as e:
            self.logger.error(f"{NEON_RED}Error calculating FVE: {e}{RESET}")
            return pd.Series(
                [np.nan] * len(self.df)
            )  # Return NaN series in case of error

    def _calculate_volume_confirmation(
        self,
    ) -> bool:  # New: Volume confirmation indicator
        if "volume" not in self.df.columns:
            self.logger.error(
                f"{NEON_RED}Missing 'volume' column for Volume Confirmation{RESET}"
            )
            return False
        current_volume = self.df["volume"].iloc[-1]
        average_volume = self.df["volume_ma"].iloc[
            -1
        ]  # Assuming volume_ma is calculated
    ...
    def analyze(self, current_price: Decimal, timestamp: str):
        ...
        self.indicator_values["atr"] = (
            [atr.iloc[-1]] if atr is not None and not atr.empty else ["N/A"]
        )  # Store ATR value in indicator_values
        ...
        if self.config["indicators"]["adx"]:
            ...
            self.indicator_values["adx"] = [
                adx
            ] * 3  # Keep ADX in indicator_values for signal logic
        ...
        if self.config["indicators"]["momentum"]:
            ...
            self.indicator_values["mom"] = [
                trend_data
            ] * 3  # Keep trend data for signal logic
        if self.config["indicators"][
            "sma_10"
        ]:  # Example for SMA, add config flag in config.json if needed
            sma = self._calculate_sma(10)
            self.indicator_values["sma"] = [
                self.df["close"].iloc[-1]
            ]  # Keep SMA for logging
        ...
        if self.config["indicators"]["macd"]:
            ...
            self.indicator_values["macd"] = (
                macd_df.tail(3).values.tolist() if not macd_df.empty else []
            )
        if self.config["indicators"]["stoch_rsi"]:
            self.indicator_values["stoch_rsi_vals"] = self.stoch_rsi_df.tail(
                3
            ).values.tolist()  # Store Stoch RSI values
        if self.config["indicators"]["ema_alignment"]:  # New: EMA Alignment
            ema_alignment_score = self._calculate_ema_alignment()
            self.indicator_values["ema_alignment"] = [
                ema_alignment_score
            ] * 3  # Keep EMA alignment score

        # Fetch Order Book Data (Basic Integration)
        order_book = fetch_order_book(
            self.symbol, API_KEY, API_SECRET, self.logger, limit=10
        )  # Fetch order book
        top_bid_price = (
            order_book["bids"][0][0] if order_book and order_book.get("bids") else "N/A"
        )  # Get top bid
        top_ask_price = (
            order_book["asks"][0][0] if order_book and order_book.get("asks") else "N/A"
        )  # Get top ask
...
"""
        for indicator_name, values in self.indicator_values.items():
            if (
                indicator_name not in ["mom", "atr", "stoch_rsi_vals", "ema_alignment"]
            ):  # Skip 'mom'/'atr'/'stoch_rsi_vals'/'ema_alignment' as they are already logged above or handled separately
                output += (
                    interpret_indicator(self.logger, indicator_name, values) + "\n"
                )
        ...
        if self.config["indicators"][
            "stoch_rsi"
        ]:  # Log Stoch RSI separately as it has k/d lines
...
"""
        ...
        signal, confidence, conditions_met = self.generate_trading_signal(
            self.indicator_values, current_price
        )  # Get conditions met
    ...
    def generate_trading_signal(
        self, indicator_values: dict, current_price: Decimal
    ) -> tuple[str | None, float, list[str]]:  # Return conditions met
        signal_score = 0
        signal = None
        ...
        if self.config["indicators"]["stoch_rsi"] and indicator_values.get(
            "stoch_rsi_vals"
        ):
            stoch_rsi_k = (
                indicator_values["stoch_rsi_vals"][-1][1]
                if indicator_values["stoch_rsi_vals"]
                and len(indicator_values["stoch_rsi_vals"][-1]) >= 2
                else None
            )
            stoch_rsi_d = (
                indicator_values["stoch_rsi_vals"][-1][2]
                if indicator_values["stoch_rsi_vals"]
                and len(indicator_values["stoch_rsi_vals"][-1]) >= 3
                else None
            )
            ...
            if (
                stoch_rsi_k
                and stoch_rsi_d
                and stoch_rsi_k < self.config["stoch_rsi_oversold_threshold"]
                and stoch_rsi_k > stoch_rsi_d
            ):  # Stoch RSI Oversold and K crossing above D
        ...
        if self.config["indicators"]["ema_alignment"] and indicator_values.get(
            "ema_alignment"
        ):
            ema_alignment_score = indicator_values["ema_alignment"][-1]
            if ema_alignment_score > 0.5:  # Bullish EMA Alignment
                bullish_conditions += self.user_defined_weights[
                    "ema_alignment"
        ...
        if (
            bullish_conditions >= self.config["signal_score_threshold"]
        ...
        if self.config["indicators"]["stoch_rsi"] and indicator_values.get(
            ...
            if (
                stoch_rsi_k
                and stoch_rsi_d
                and stoch_rsi_k > self.config["stoch_rsi_overbought_threshold"]
                and stoch_rsi_k < stoch_rsi_d
            ):  # Stoch RSI Overbought and K crossing below D
        ...
        if self.config["indicators"]["ema_alignment"] and indicator_values.get(
            ...
            if ema_alignment_score < -0.5:  # Bearish EMA Alignment
                bearish_conditions += self.user_defined_weights[
                    "ema_alignment"
                ]  # Use configured weight
        ...
        if (
            bearish_conditions >= self.config["signal_score_threshold"]
        ):  # Configurable signal threshold
            signal = "sell"
...
def interpret_indicator(
    logger: logging.Logger, indicator_name: str, values: list[float]
) -> str | None:
    if not values:
    ...
    try:
        ...
        if (
            indicator_name == "stoch_rsi_vals"
        ):  # Stoch RSI interpretation is handled directly in analyze function for K/D lines
            return None  # Interpretation done directly in analyze function
        if (
            indicator_name == "ema_alignment"
        ):  # EMA Alignment interpretation is handled directly in analyze function
            return None
    ...
    while True:
        try:
            ...
            if current_price is None:
                symbol_logger.error(
                    f"{NEON_RED}Failed to fetch current price for {symbol}. Retrying in {retry_delay} seconds...{RESET}"
                )
            ...
            if df.empty:
                symbol_logger.error(
                    f"{NEON_RED}Failed to fetch Kline data for {symbol}. Retrying in {retry_delay} seconds...{RESET}"
                )
                time.sleep(retry_delay)
                continue
            ...
            signal, confidence, conditions_met = analyzer.generate_trading_signal(
                analyzer.indicator_values, current_price
            )  # Get conditions met

            if signal and (
                current_time_seconds - last_signal_time >= CONFIG["signal_cooldown_s"]
            ):  # Check for signal and cooldown
                symbol_logger.info(
                    f"{NEON_PURPLE}--- TRADING SIGNAL TRIGGERED ---{RESET}"
                )
                symbol_logger.info(
            ...
            symbol_logger.exception(
                f"{NEON_RED}Unexpected error: {e}. Retrying in {retry_delay} seconds...{RESET}"
            )
            time.sleep(retry_delay)
