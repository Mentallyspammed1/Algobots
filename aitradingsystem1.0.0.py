...
class BybitAdapter:
    def __init__(
        ...
        self.account_info_cache: dict[str, Any] | None = None
        self.cache_expiry_time: datetime | None = None
        ...
        for attempt in range(1, self.retry_cfg.retries + 1):
            try:
                ...
                if is_last or not self._is_retryable(e):
                    ...
                    raise
                sleep_for = (
                    min(delay * (2 ** (attempt - 1)), self.retry_cfg.max_delay)
    ...
    def get_real_time_market_data(
        ...
        try:
            ...
            if (
                ticker_info
                and ticker_info["retCode"] == 0
                and ticker_info["result"]["list"]
            ):
                latest_ticker = ticker_info["result"]["list"][0]
                latest_kline_1d = (
                    klines_1d["result"]["list"][0]
                    if klines_1d
                    and klines_1d["retCode"] == 0
                    and klines_1d["result"]["list"]
                    else None
                )
                return {
                    "symbol": symbol,
                    ...
                    "price_change_24h_pct": float(latest_kline_1d[8])
                    if latest_kline_1d
                    else 0,
    ...
    async def _get_cached_account_info(self) -> dict[str, Any]:
        ...
        if (
            ...
            and self.cache_expiry_time
            and now < self.cache_expiry_time
        ):
    ...
    def get_account_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                and wallet_balance_response["retCode"] == 0
                and wallet_balance_response["result"]["list"]
            ):
            ...
            if (
                positions_response
                and positions_response["retCode"] == 0
                and positions_response["result"]["list"]
            ):
    ...
    def place_order(
        ...
        if not client_order_id:
            client_order_id = (
                f"AI_{symbol}_{side}_{int(time.time())}_{np.random.randint(1000, 9999)}"
            )
        if order_type in ["Limit", "StopLimit"] and price is None:
            return {
                "status": "failed",
                "message": "Price required for Limit/StopLimit orders.",
            }
        if qty <= 0:
            ...
            if response and response["retCode"] == 0 and response["result"]:
                ...
                if internal_order:
                    ...
                    internal_order.price = (
                        float(order_data.get("price", internal_order.price))
                        if order_data.get("price")
                        else internal_order.price
                    )
                    internal_order.qty = (
                        float(order_data.get("qty", internal_order.qty))
                        if order_data.get("qty")
                        else internal_order.qty
                    )
                    internal_order.stop_loss = (
                        float(order_data.get("stopLoss", internal_order.stop_loss))
                        if order_data.get("stopLoss")
                        else internal_order.stop_loss
                    )
                    internal_order.take_profit = (
                        float(order_data.get("takeProfit", internal_order.take_profit))
    ...
    def cancel_order(
        ...
        if not order_id and not client_order_id:
            return {
                "status": "failed",
                "message": "order_id or client_order_id required for cancellation.",
            }
        internal_order = self.orders.get(client_order_id) if client_order_id else None
        if internal_order and internal_order.status not in [
            ...
            return {
                "status": "failed",
                "message": f"Order not in cancellable state: {internal_order.status}",
            }
        if internal_order:
...
class RiskPolicy:
    ...
    async def validate_trade_proposal(
        ...
        if stop_loss is not None and estimated_entry_price is not None:
            risk_per_unit = (
                (estimated_entry_price - stop_loss)
                if side == "Buy"
                else (stop_loss - estimated_entry_price)
            )
            if risk_per_unit > 0:
        ...
        if trade_risk_usd > total_balance * self.max_risk_per_trade_pct:
            return (
                False,
                f"Trade risk ({trade_risk_usd:.2f}) exceeds max allowed ({total_balance * self.max_risk_per_trade_pct:.2f}).",
            )
...
class TradingFunctions:
    def __init__(self, bybit_adapter: BybitAdapter | None = None):
        self.bybit_adapter = bybit_adapter
        self.stub_data = {
            ...
            "get_portfolio_status": {
                ...
                "total_balance_usd": 50_000.00,
                "available_balance": 25_000.00,
                "positions": [
                    {
                        "symbol": "BTCUSDT",
                        "size": 0.5,
            ...
            "execute_risk_analysis": {
                ...
                "recommended_stop_loss": 44100.0,
                "recommended_take_profit": 47250.0,
            },
        }
    ...
    def execute_risk_analysis(
        ...
        if stop_loss is not None and entry_price is not None and position_value > 0:
            risk_per_unit = (
                entry_price - stop_loss if side == "Buy" else (stop_loss - entry_price)
            )
            if risk_per_unit > 0:
                trade_risk_usd = risk_per_unit * qty
            else:
    ...
    def place_order(
        ...
        if self.bybit_adapter:
            return self.bybit_adapter.place_order(
                symbol,
                side,
                order_type,
                qty,
...
class TradingAISystem:
    ...
    async def execute_ai_trade_suggestion(self, **kwargs):
        ...
        return {
            "status": "success",
            "order": {
                "bybit_order_id": "dummy_id",
                "client_order_id": "dummy_client_id",
            },
        }
    ...
    while True:
        ...
        if current_price:
            ...
            if trade_execution_result.get("status") == "success":
                ...
                updated_order = system.bybit_adapter.get_order(
                    symbol_to_trade,
                    order_id=order_id_to_check,
                    client_order_id=client_id_to_check,
                )
                if updated_order:
