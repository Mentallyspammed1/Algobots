...
CONFIG = {
    ...
    "LIMIT_ORDER_SELECTION_TYPE": os.environ.get("LIMIT_ORDER_SELECTION_TYPE", "interactive").lower(),
...
def calculate_rsi(closes, period):
    ...
    gains = [max(decimal.Decimal('0'), closes[i] - closes[i-1]) for i in range(1, len(closes))]
    losses = [max(decimal.Decimal('0'), closes[i-1] - closes[i]) for i in range(1, len(closes))]

    avg_gain = sum(gains[-period:]) / period
    avg_loss = sum(losses[-period:]) / period

    if avg_loss == 0: return decimal.Decimal('100')
    rs = avg_gain / avg_loss
    rsi = decimal.Decimal('100') - (decimal.Decimal('100') / (decimal.Decimal('1') + rs))
    return rsi.quantize(decimal.Decimal('0.00'))

def calculate_stoch_rsi(rsi_values, k_period, d_period):
    if len(rsi_values) < max(k_period, d_period): return None
    lowest_rsi = min(rsi_values[-k_period:])
    highest_rsi = max(rsi_values[-k_period:])

    if (highest_rsi - lowest_rsi) == 0: return decimal.Decimal('0')

    stoch_rsi_k = ((rsi_values[-1] - lowest_rsi) / (highest_rsi - lowest_rsi)) * decimal.Decimal('100')
    ...
    for i in range(k_period - 1, len(rsi_values)):
        ...
        if highest - lowest == 0:
            ...
            stoch_rsi_d_list.append(((rsi_values[i] - lowest) / (highest - lowest)) * decimal.Decimal('100'))
    
    if len(stoch_rsi_d_list) < d_period: return None, None
    stoch_rsi_d = sum(stoch_rsi_d_list[-d_period:]) / d_period

    return stoch_rsi_k.quantize(decimal.Decimal('0.00')), stoch_rsi_d.quantize(decimal.Decimal('0.00'))
...
def analyze_orderbook_volume(exchange, symbol, market_info, config):
    try:
        orderbook = exchange.fetch_order_book(symbol, limit=config["MAX_ORDERBOOK_DEPTH_DISPLAY"])
        bids = orderbook['bids']
        asks = orderbook['asks']
        ...
        for price, amount in bids:
            ...
            bid_map.append({'price': price_d, 'amount': amount_d, 'cumulative_vol': total_bid_vol})
        
        for price, amount in asks:
            price_d = decimal.Decimal(str(price))
            amount_d = decimal.Decimal(str(amount))
            total_ask_vol += amount_d
            ask_map.append({'price': price_d, 'amount': amount_d, 'cumulative_vol': total_ask_vol})
        
        return {'bids': bid_map, 'asks': ask_map, 'timestamp': exchange.iso8601(exchange.milliseconds())}, False
    except Exception as e:
...
def display_header(symbol, timestamp, balance, config):
    ...
    print(f"║ {Style.BRIGHT}{symbol.upper():<20}{Style.RESET_ALL}{Fore.CYAN}{Style.BRIGHT}│ {timestamp:<40} {Style.RESET_ALL}{Fore.CYAN}{Style.BRIGHT}│ Balance ({config['FETCH_BALANCE_ASSET']}): {Fore.GREEN}{format_decimal(balance, config['BALANCE_DISPLAY_PRECISION']):<10}{Style.RESET_ALL}{Fore.CYAN}{Style.BRIGHT} ║")
...
def display_ticker_and_trend(ticker_info, indicators_info, config, market_info):
    ...
    last_price = decimal.Decimal(str(ticker_info['last'])) if ticker_info else decimal.Decimal('0')
    price_change = decimal.Decimal(str(ticker_info['percentage'])) if ticker_info and 'percentage' in ticker_info else decimal.Decimal('0')
    
    price_color = Fore.GREEN if price_change > 0 else Fore.RED if price_change < 0 else Fore.WHITE
    
    trend_color = Fore.WHITE
    if indicators_info['sma9'] and indicators_info['sma20'] and indicators_info['ema12'] and indicators_info['ema34']:
    ...
    print_color(f"  Last: {price_color}{format_decimal(last_price, price_prec, min_disp_prec)}{Style.RESET_ALL} | 24h Change: {price_color}{price_change:+.2f}%{Style.RESET_ALL}", end='')
    print_color(f" | Trend: {trend_color}{'Up' if trend_color==Fore.GREEN else 'Down' if trend_color==Fore.RED else 'Neutral'}{Style.RESET_ALL}")
    return last_price
...
def display_indicators(indicators_info, config, market_info, last_price):
    ...
    print_color(" | EMA12: {} | EMA34: {}".format(
    ...
    print_color("  Momentum: {} | RSI: {}".format(
    ...
    if stoch_rsi_k is not None:
        if stoch_rsi_k < CONFIG['STOCH_RSI_OVERSOLD']: stoch_k_color = Fore.GREEN
        elif stoch_rsi_k > CONFIG['STOCH_RSI_OVERBOUGHT']: stoch_k_color = Fore.RED

    stoch_d_color = Fore.WHITE
    if stoch_rsi_d is not None:
        if stoch_rsi_d < CONFIG['STOCH_RSI_OVERSOLD']: stoch_d_color = Fore.GREEN
        elif stoch_rsi_d > CONFIG['STOCH_RSI_OVERBOUGHT']: stoch_d_color = Fore.RED
    
    print_color(" | Stoch-RSI: {}{}{} / {}{}{}".format(
        stoch_k_color, format_decimal(stoch_rsi_k, stoch_rsi_prec), Style.RESET_ALL,
        stoch_d_color, format_decimal(stoch_rsi_d, stoch_rsi_prec), Style.RESET_ALL
    ))
...
def display_position(position_info, ticker_info, market_info, config):
    ...
    if position_info.get('has_position'):
        ...
        liq_price = format_decimal(pos.get('liquidationPrice', 'N/A'), price_prec, min_disp_prec)
        mark_price = format_decimal(pos.get('markPrice', 'N/A'), price_prec, min_disp_prec)
...
def display_pivots(pivots_info, current_price, market_info, config):
    ...
    for level_key in ['r3', 'r2', 'r1', 'pivot', 's1', 's2', 's3']:
        ...
        if level_key.startswith('r'): level_color = Fore.LIGHTRED_EX
        elif level_key.startswith('s'): level_color = Fore.LIGHTGREEN_EX
        else: level_color = Fore.CYAN

        if current_price and abs(current_price - level_value) < current_price * decimal.Decimal('0.0005'):
            level_color = Fore.MAGENTA + Style.BRIGHT

        print_color(f"{level_key.upper()}: {level_color}{format_decimal(level_value, price_prec, min_disp_prec)}{Style.RESET_ALL}", end=' | ')
    print()
...
def display_orderbook(analyzed_orderbook, market_info, config):
    ...
    print_color("\n╔═══════════════════════════════════════════════════════════════════════════════════╗", color=Fore.BLUE)
    print_color("║                           Order Book Depth & Heatmap                              ║", color=Fore.BLUE)
    print_color("╠═══════════════════════════════════════════════════════════════════════════════════╣", color=Fore.BLUE)
    print_color("║   Price (ASK)      Volume (ASK)      Cum. Vol. (ASK)   │   Price (BID)      Volume (BID)      Cum. Vol. (BID)   ║", color=Fore.BLUE)
    print_color("╠═══════════════════════════════════════════════════════════════════════════════════╣", color=Fore.BLUE)
...
def display_volume_analysis(analyzed_orderbook, market_info, config):
    ...
    if bids: total_bid_vol = bids[-1]['cumulative_vol']
    if asks: total_ask_vol = asks[-1]['cumulative_vol']
    ...
    vol_color = Fore.GREEN if vol_diff > 0 else Fore.RED if vol_diff < 0 else Fore.YELLOW

    print_color(f"  Volume Analysis: Bid {Fore.GREEN}{bid_percentage:.1f}%{Style.RESET_ALL} | Ask {Fore.RED}{ask_percentage:.1f}%{Style.RESET_ALL} | Bias: {vol_color}{vol_diff:.1f}%{Style.RESET_ALL}")
...
def display_open_orders(open_orders):
    ...
    for idx, order in enumerate(open_orders, 1):
        ...
        print_color(f"  [{idx}] ID: {order_id} | {side_color}{side}{Style.RESET_ALL} {amount} @ {price} | Type: {order.get('type')}")

def display_account_info(account_data, balance_info, config):
    print_color("\n--- Account Info ---", color=Fore.BLUE)
    equity = format_decimal(account_data.get('equity', 'N/A'), config['BALANCE_DISPLAY_PRECISION'])
    margin = format_decimal(account_data.get('availableMargin', 'N/A'), config['BALANCE_DISPLAY_PRECISION'])
    ...
    print_color(f"  Wallet Balance ({config['FETCH_BALANCE_ASSET']}): {Fore.GREEN}{balance_str}{Style.RESET_ALL}")
...
def place_market_order(exchange, symbol, side, amount_str, market_info):
    try:
        ...
        if amount_d < market_info['min_amount']:
            print_color(f"Error: Amount {amount_d} is below minimum {market_info['min_amount']}", color=Fore.RED)
            return

        amount_d = (amount_d // market_info['amount_step']) * market_info['amount_step']
        
        confirm = input(f"Confirm {side.upper()} market order for {amount_d} {symbol}? (y/n): ").strip().lower()
        if confirm != 'y':
            print_color("Order cancelled.", color=Fore.YELLOW)
            return

        order = exchange.create_market_order(symbol, side, float(amount_d), params={'positionIdx': 0})
...
def place_limit_order(exchange, symbol, side, amount_str, price_str, market_info):
    try:
        ...
        amount_d = (amount_d // market_info['amount_step']) * market_info['amount_step']
        price_d = (price_d // market_info['price_tick_size']) * market_info['price_tick_size']

        confirm = input(f"Confirm {side.upper()} limit order for {amount_d} {symbol} @ {price_d}? (y/n): ").strip().lower()
        if confirm != 'y':
            print_color("Order cancelled.", color=Fore.YELLOW)
            return

        order = exchange.create_limit_order(symbol, side, float(amount_d), float(price_d), params={'positionIdx': 0})
        print_color(f"Order placed: {order['id']}", color=Fore.GREEN)
        termux_toast(f"Limit {side.upper()} {amount_d} @ {price_d} on {symbol}", duration="short")
    except Exception as e:
...
def manage_close_position(exchange, symbol, positions, market_info):
    ...
    if choice == 'all':
        confirm = input("Are you sure you want to close ALL positions for this symbol? (y/n): ").strip().lower()
        if confirm != 'y':
        ...
        try:
            ...
            if 0 <= idx < len(positions):
                ...
                amount_input = input(f"Amount to close ({available_amount} available, or 'all'): ").strip()
                amount_to_close = available_amount if amount_input.lower() == 'all' else decimal.Decimal(amount_input)
                ...
                if order_type_input == 'l':
                    ...
                    close_position(exchange, symbol, pos['side'], str(amount_to_close), market_info, is_market=False, price_str=price_input)
                elif order_type_input == 'm':
                    close_position(exchange, symbol, pos['side'], str(amount_to_close), market_info, is_market=True)
                else:
...
def manage_cancel_order(exchange, symbol, open_orders):
    ...
    if choice == 'all':
        confirm = input("Are you sure you want to cancel ALL open orders for this symbol? (y/n): ").strip().lower()
        if confirm != 'y':
...
def set_leverage(exchange, symbol, leverage):
    try:
        ...
        if not 1 <= leverage_int <= 100: # Example range, check Bybit for actual limits
            print_color("Invalid leverage value. Must be between 1 and 100.", color=Fore.RED)
            return

        confirm = input(f"Confirm set leverage to {leverage_int}x for {symbol}? (y/n): ").strip().lower()
        if confirm != 'y':
...
class TradingAnalytics:
    ...
    def calculate_metrics(self):
        ...
        avg_loss = sum(t['pnl'] for t in losing_trades) / len(losing_trades) if losing_trades and len(losing_trades) != 0 else 0
    ...
    def send_webhook_alert(self, message, priority):
        try:
            ...
            if response.status_code == 204: # Discord webhook success
                ...
                print_color(f"Webhook alert failed: {response.status_code}", color=Fore.RED)
        except Exception as e:
    ...
    def run_backtest(self, start_date, end_date, timeframe='1h'):
        ...
        for i in range(len(historical_data)):
            ...
            if signal['action'] == 'buy' and position is None:
                position = {
                    'entry_price': decimal.Decimal(str(current_candle[4])), # close price
                    'size': balance * decimal.Decimal('0.95') / decimal.Decimal(str(current_candle[4])),
            ...
            if position:
                current_equity += float((decimal.Decimal(str(current_candle[4])) - position['entry_price']) * position['size'])
...
class OrderBookAnalyzer:
    ...
    def calculate_order_flow_imbalance(self, orderbook):
        ...
        bids = [{'price': decimal.Decimal(str(b[0])), 'amount': decimal.Decimal(str(b[1]))} for b in bids_raw]
        asks = [{'price': decimal.Decimal(str(a[0])), 'amount': decimal.Decimal(str(a[1]))} for a in asks_raw]
...
class SimpleStrategy:
    ...
    def generate_signal(self, ohlcv_data):
        ...
        elif sma_fast < sma_slow and closes[-1] < sma_fast:
...
class MLTradingBot:
    def __init__(self, exchange, symbol, model_path='ml_model.joblib'):
        ...
        self.feature_columns = ['sma_fast', 'sma_slow', 'rsi', 'momentum'] # Example features
        
    def _load_model(self, path):
        try:
            return joblib.load(path)
        except FileNotFoundError:
            print_color(f"ML Model not found at {path}. Please train and save a model.", color=Fore.RED)
            return None
    ...
    def _prepare_features(self, ohlcv_data):
        ...
        features = np.array([float(sma_fast), float(sma_slow), float(rsi), float(momentum)]).reshape(1, -1)
        return features
    ...
    def generate_ml_signal(self, ohlcv_data):
        ...
        if features is None:
            return {'action': 'hold', 'confidence': 0, 'reason': 'Insufficient data for ML features'}
        ...
        if prediction == 1: # Assuming 1 = buy, 0 = hold, -1 = sell
            ...
            return {'action': 'sell', 'confidence': proba[-1], 'reason': 'ML Sell Signal'}
        else:
    ...
    def train_model(self, historical_data_labeled, model_path='ml_model.joblib'):
        print_color("Training ML model (this is a placeholder and requires actual labeled data)...", color=Fore.YELLOW)
        ...
        for i in range(len(historical_data_labeled)):
            ...
            label = historical_data_labeled[i]['label'] # e.g., 1 for buy, -1 for sell, 0 for hold
...
def run_analysis_cycle(exchange, symbol, market_info, config, alert_system=None):
    ...
    indicators_info = {
        'sma9': None, 'sma20': None, 'ema12': None, 'ema34': None,
        'momentum': None, 'rsi': None, 'stoch_rsi_k': None, 'stoch_rsi_d': None
    }
    pivot_info = None

    if ohlcv_data_indicator and len(ohlcv_data_indicator) > max(CONFIG['SMA2_PERIOD'], CONFIG['EMA2_PERIOD']):
        ...
        indicators_info['momentum'] = calculate_momentum(closes, CONFIG['MOMENTUM_PERIOD'])
        indicators_info['rsi'] = calculate_rsi(closes, CONFIG['RSI_PERIOD'])
        
        stoch_k, stoch_d = calculate_stoch_rsi(all_rsi_values, CONFIG['STOCH_K_PERIOD'], CONFIG['STOCH_D_PERIOD'])
    ...
    if fetched_data['positions']:
        ...
        if fetched_data['ticker'] and fetched_data['ticker'].get('last'):
            ...
            if position.get('side') == 'long':
                position_info['unrealizedPnl'] = (current_last_price - entry_price) * contracts
            elif position.get('side') == 'short':
                position_info['unrealizedPnl'] = (entry_price - current_last_price) * contracts
    ...
    return not data_error, ask_map, bid_map, fetched_data['positions'], fetched_data['open_orders'], fetched_data


def main():
    if not CONFIG["API_KEY"] or not CONFIG["API_SECRET"]:
        print_color("API Key or Secret not found in .env. Exiting.", color=Fore.RED, style=Style.BRIGHT)
        sys.exit(1)
    ...
    if os.environ.get("BYBIT_TESTNET", "false").lower() == 'true':
        ...
        exchange_config['options']['defaultType'] = 'future' # Testnet might need 'future'
    ...
    while True:
        cycle_successful, ask_map, bid_map, positions, open_orders, fetched_data = run_analysis_cycle(exchange, symbol, market_info, CONFIG, alert_system)

        if cycle_successful:
            action = input(f"\n{Style.BRIGHT}{Fore.BLUE}Action (refresh/buy/sell/close/orders/cancel/account/leverage/alert/backtest/exit): {Style.RESET_ALL}").strip().lower()
            ...
            if action in ['buy', 'sell']:
                ...
                if order_type == 'market':
                    ...
                    if CONFIG['LIMIT_ORDER_SELECTION_TYPE'] == 'interactive':
                        ...
                        if price_val:
                            place_limit_order(exchange, symbol, side, amount_str, str(price_val), market_info)
                        else:
                            price_str = price_choice # Assume it's a manual price if not a map key
                            try:
                                decimal.Decimal(price_str) # Validate it's a number
                                place_limit_order(exchange, symbol, side, amount_str, price_str, market_info)
                            except decimal.InvalidOperation:
                                print_color("Invalid price selection. Please enter A1/B1 or a numerical price.", color=Fore.RED)
                ...
                display_account_info(fetched_data['account'], fetched_data['balance'], CONFIG)
                ...
                print_color(f"Alert set for {symbol} {condition} {price_level}", color=Fore.GREEN)
            elif action == 'backtest':
                start_date = input("Enter start date (YYYY-MM-DD): ").strip()
                end_date = input("Enter end date (YYYY-MM-DD): ").strip()
                # backtest_engine.run_backtest(start_date, end_date)
                # backtest_engine.display_backtest_results()
                print_color("Backtesting feature is currently commented out. Uncomment to use.", color=Fore.YELLOW)
